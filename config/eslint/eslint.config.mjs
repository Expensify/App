...
const expensifyRulesDir = path.resolve(
...
const restrictedImportPaths = [
  ...
  },
  {
    ...
    message:
      "forwardRef is deprecated. Please use ref as a prop instead. See: contributingGuides/STYLE.md#forwarding-refs",
  },
  {
    name: "@styles/index",
...
const restrictedReportNameImportPatterns = [
  {
    ...
    message:
      "Do not import computeReportName. Use getReportName instead, which properly uses derived report attributes.",
  },
];
...
const victoryChartRendererRestrictedImportPaths = restrictedImportPaths.filter(
  (restriction) => restriction.name !== "@styles/theme",
);
const victoryChartRendererRestrictedImportPatterns = [
  ...restrictedImportPatterns.filter(
    (restriction) => restriction.group[0] !== "@styles/theme/themes/**",
  ),
...
const config = defineConfig([
...

  {
    ...
    rules: {
      ...
      "no-restricted-syntax": [
        ...
        {
          selector:
            "ImportNamespaceSpecifier[parent.source.value=/^@userActions/]",
          message:
        ...
        },
        {
          ...
          message:
            'Namespace imports from parent directories are not allowed. Use named imports instead. Example: import { method } from "../libs/module"',
        },
        {
          selector: "ImportNamespaceSpecifier[parent.source.value=/^\\./]",
          message:
            'Namespace imports from sibling modules are not allowed. Use named imports instead. Example: import { method } from "./libs/module"',
        },
        {
          selector:
      ...
      "no-restricted-properties": [
        "error",
        {
          ...
          property: "getSize",
          message:
            "Usage of Image.getSize is restricted. Please use the `react-native-image-size`.",
        },
          ...
          patterns: [
            {
              group: ["@src/**"],
              message:
                "Do not import files from src/ directory as they can break the GH Actions build script.",
            },
          ],
        },
      ],
  ...
  {
    files: [
      "server/victory-chart-renderer/**/*.ts",
      "server/victory-chart-renderer/**/*.tsx",
    ],
    languageOptions: {
      parserOptions: {
