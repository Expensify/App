import type {Chunk, Compiler, NormalModule} from '@rspack/core';
...
function createTimingRuntimeModule(): InstanceType<typeof rspack.RuntimeModule> {
    const mod = new rspack.RuntimeModule('module init timing', rspack.RuntimeModule.STAGE_ATTACH);
    ...
    mod.generate = (): string =>
        dedent(`
            ...
            if (typeof self !== 'undefined' && !self.__moduleInitTimes) {
                ...
                ${rspack.RuntimeGlobals.interceptModuleExecution}.push(function(options) {
...
class ModuleInitTimingPlugin {
    apply(compiler: Compiler): void {
        ...
        compiler.hooks.compilation.tap(PLUGIN_NAME, (compilation) => {
            // Ensure rspack generates the __webpack_require__.i interception
            ...
            compilation.hooks.additionalTreeRuntimeRequirements.tap(PLUGIN_NAME, (_chunk: Chunk, set: Set<string>) => {
                set.add(rspack.RuntimeGlobals.interceptModuleExecution);
            });

            // Once rspack has processed that requirement, inject our RuntimeModule.
            compilation.hooks.runtimeRequirementInTree.for(rspack.RuntimeGlobals.interceptModuleExecution).tap(PLUGIN_NAME, (chunk: Chunk) => {
        ...
        compiler.hooks.emit.tap(PLUGIN_NAME, (compilation) => {
            ...
            if (Object.keys(names).length > 0) {
                ...
                compilation.assets['module-names.json'] = new rspack.sources.RawSource(JSON.stringify(names));
            }
        });
    }
}
