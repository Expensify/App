...
module.exports = async function oxcReactCompilerLoader(source) {
  ...
  try {
    ...
    const sourceMaps =
      options.sourcemap !== undefined ? options.sourcemap : !!this.sourceMap;
    ...
    const rcErrors = (result.errors || []).filter(
      (e) => e.message && e.message.includes("[ReactCompiler]"),
    );
    const fatalErrors = (result.errors || []).filter(
      (e) => !e.message?.includes("[ReactCompiler]"),
    );
