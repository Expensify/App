...
function isFragment(openingElement) {
  ...
  if (
    t.isJSXIdentifier(name, { name: "Fragment" }) ||
    t.isJSXIdentifier(name, { name: "React.Fragment" })
  )
    return true;
...
function addAttribute(openingElement, attrName, value) {
  if (
    ...
    hasAttribute(openingElement, attrName)
  )
    return;
...
function annotateJSXNode(jsxPath, componentName, sourceFileName) {
  ...
  const elementName = t.isJSXIdentifier(openingEl.name)
    ? openingEl.name.name
    : "unknown";

  // dataElement — skip for HTML primitives
  if (
    !IGNORED_HTML_ELEMENTS.has(elementName) &&
    !hasAttribute(openingEl, ATTR_ELEMENT)
  ) {
    openingEl.attributes.push(
      t.jSXAttribute(
        t.jSXIdentifier(ATTR_ELEMENT),
...
module.exports = function fullstoryAnnotationLoader(source) {
  ...
  if (
    KNOWN_INCOMPATIBLE.some(
      (m) =>
        resourcePath.includes(`/node_modules/${m}/`) ||
  ...
  const { code, map } = generate(
    ast,
    { sourceMaps: !!this.sourceMap, sourceFileName: resourcePath },
    source,
  );
