...
PORT_PLAN = {
    ...
    # The twelve React Compiler rules. Ported and proven on oxc-transform-react 0.147.0, then blocked by 0.148.0:
    # oxc-project/oxc#26128 narrowed result.errors to fatal React Compiler diagnostics, and should_panic answers
    # false for the default panicThreshold, so the Rust engine analyzes every file and returns nothing.
    'react-hooks/refs': {
        'mechanism': 'blocked upstream - rc/refs needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/refs and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/set-state-in-effect': {
        'mechanism': 'blocked upstream - rc/set-state-in-effect needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/set-state-in-effect and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/set-state-in-render': {
        'mechanism': 'blocked upstream - rc/set-state-in-render needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/set-state-in-render and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/preserve-manual-memoization': {
        'mechanism': 'blocked upstream - rc/preserve-manual-memoization needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/preserve-manual-memoization and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/immutability': {
        'mechanism': 'blocked upstream - rc/immutability needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/immutability and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/static-components': {
        'mechanism': 'blocked upstream - rc/static-components needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/static-components and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/use-memo': {
        'mechanism': 'blocked upstream - rc/use-memo needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/use-memo and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/globals': {
        'mechanism': 'blocked upstream - rc/globals needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/globals and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/error-boundaries': {
        'mechanism': 'blocked upstream - rc/error-boundaries needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/error-boundaries and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/purity': {
        'mechanism': 'blocked upstream - rc/purity needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/purity and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/incompatible-library': {
        'mechanism': 'blocked upstream - rc/incompatible-library needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/incompatible-library and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'react-hooks/unsupported-syntax': {
        'mechanism': 'blocked upstream - rc/unsupported-syntax needs non-fatal React Compiler diagnostics',
        'effort': 'blocked', 'proven': True,
        'notes': 'wired as rc/unsupported-syntax and measured working on oxc-transform-react 0.147.0; off in .oxlintrc.json since '
                 '0.148.0 removed the only channel that returns it. See the rc/* block there for why '
                 'panicThreshold: all_errors is not a workaround',
    },
    'progress/activate': {
