...
function buildCommentIntentInput(commentBody: string, isTrustedCommenter = false): string {
    const authorContext = isTrustedCommenter
        ? 'trusted: the commenter is an approved contributor or has identified themselves as working for an approved partner'
        : 'untrusted: no approved contributor or partner affiliation was verified';
