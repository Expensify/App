...
if git log origin/main.. --name-status --pretty=format: $ARTICLES_DIRECTORY | grep -q -E "^(R|D)"
then
    hasRenamedOrDeletedArticle=true
fi

if git log origin/main.. --name-status --pretty=format: $REDIRECTS_FILE | grep -E "^(M)"
then
