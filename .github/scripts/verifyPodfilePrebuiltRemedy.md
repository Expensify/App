...
1. Build the artifacts for these patches: run [Publish React Native Artifacts](https://github.com/Expensify/App/actions/workflows/publishReactNativeArtifacts.yml). Set `app_pull_request_url` and `mobile_expensify_pull_request_url` to this PR and its companion PR in the other repo, whichever apply. Tick "Build HybridApp artifacts".
2. Once it succeeds, run `npm run pod-install` from your checkout. It logs `[PatchedArtifacts] Using patched react-native artifacts` to stderr on a match; check for it before committing.
