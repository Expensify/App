...
delcare -r RED='\033[0;31m'
declare -r NC='\033[0m'

if [[ $(git diff --exit-code) ]]; then
    echo "${GREEN}Github Actions are up to date!${NC}"
    exit 0
else
    echo "${RED}Error: Diff found when Github Actions were rebuilt. Did you forget to run ${NC}\`npm run gh-actions-build\`${RED}?${NC}"
    exit 1
fi
