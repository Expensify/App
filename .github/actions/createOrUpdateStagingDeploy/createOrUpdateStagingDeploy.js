...
Promise.all([
    ...
    .then((results) => {
        ...
        // Look at the state of the most recent StagingDeployCash – if it is open then we'll update the existing one,
        // otherwise, we'll create a new one.
        shouldCreateNewStagingDeployCash = Boolean(stagingDeployResponse.data[0].state !== 'open');

        // Parse the data from the previous StagingDeployCash (newest if there are none open, otherwise second-newest)
