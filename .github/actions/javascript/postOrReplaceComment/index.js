...
var postOrReplaceComment_exports = {};
__export(postOrReplaceComment_exports, {
  default: () => postOrReplaceComment_default
});
module.exports = __toCommonJS(postOrReplaceComment_exports);
...
function getTestBuildMessage(appPr, mobileExpensifyPr) {
  ...
  const result = inputs.reduce(
    (acc, platform) => {
      const input = core2.getInput(platform, { required: false });
      if (!input) {
      ...
      switch (input) {
        case "success":
          link = core2.getInput(`${platform}_LINK`);
...
:eyes: [View the workflow run that generated this build](https://github.com/${import_github.context.repo.owner}/${import_github.context.repo.repo}/actions/runs/${import_github.context.runId}) :eyes:
`;
  return message;
}
async function commentPR(REPO, PR, message) {
  ...
  try {
    ...
    console.log(`Comment created on #${PR} (${REPO}) successfully \u{1F389}`);
  } catch (err) {
    console.log(`Unable to write comment on #${PR} \u{1F61E}`);
...
async function hidePreviousComment(repo, issueNumber, commentPrefix) {
  const comments = await GithubUtils_default.paginate(
    GithubUtils_default.octokit.issues.listComments,
    {
      owner: CONST_default.GITHUB_OWNER,
      repo,
  ...
  await GithubUtils_default.graphql(
    `
...
async function run() {
  const APP_PR_NUMBER = Number(core2.getInput("APP_PR_NUMBER", { required: false }));
  const MOBILE_EXPENSIFY_PR_NUMBER = Number(core2.getInput("MOBILE_EXPENSIFY_PR_NUMBER", { required: false }));
  const REPO = String(core2.getInput("REPO", { required: true }));
  const COMMENT_BODY = core2.getInput("COMMENT_BODY", { required: false });
  const COMMENT_PREFIX = core2.getInput("COMMENT_PREFIX", { required: true });
  if (REPO !== CONST_default.APP_REPO && REPO !== CONST_default.MOBILE_EXPENSIFY_REPO) {
    core2.setFailed(`Invalid repository used to place output comment: ${REPO}`);
    return;
  }
  if (REPO === CONST_default.APP_REPO && !APP_PR_NUMBER || REPO === CONST_default.MOBILE_EXPENSIFY_REPO && !MOBILE_EXPENSIFY_PR_NUMBER) {
    core2.setFailed(`Please provide ${REPO} pull request number`);
    return;
  }
  const destinationPRNumber = REPO === CONST_default.APP_REPO ? APP_PR_NUMBER : MOBILE_EXPENSIFY_PR_NUMBER;
