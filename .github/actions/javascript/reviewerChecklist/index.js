...
async function isTeamMember(octokit, org, teamSlug, username) {
  try {
    ...
    if (error2 instanceof import_request_error2.RequestError && error2.status === 404) {
      return false;
    }
    const message = error2 instanceof Error ? error2.message : String(error2);
    core2.warning(`Could not verify ${teamSlug} membership for ${username}. Assuming they are not a member: ${message}`);
    return false;
  }
}
...
var issue = github.context.payload.issue?.number ?? github.context.payload.pull_request?.number ?? -1;
...
function getNumberOfItemsFromReviewerChecklist() {
  ...
  return new Promise((resolve, reject) => {
    import_https.default.get(pathToReviewerChecklist, (res) => {
      let fileContents = "";
      res.on("data", (chunk) => {
        fileContents += chunk;
      });
...
function checkIssueForCompletedChecklist(numberOfChecklistItems) {
  GithubUtils_default.getAllReviewComments(issue).then((reviewComments) => {
    console.log(`Pulled ${reviewComments.length} review comments, now adding them to the list...`);
    combinedComments.push(...reviewComments);
  }).then(() => GithubUtils_default.getAllComments(issue)).then((comments) => {
    ...
    if (!foundReviewerChecklist) {
      core3.setFailed("No PR Reviewer Checklist was found");
      return;
    }
    ...
    core3.setFailed("PR Reviewer Checklist is not completely filled out. Please check every box to verify you've thought about the item.");
  });
}
async function hasStandingInternalApproval(orgToken) {
  ...
  const reviews = await GithubUtils_default.paginate(GithubUtils_default.octokit.pulls.listReviews, {
    owner,
    repo,
  ...
  const membershipResults = await Promise.all(reviewerLogins.map((login) => isTeamMember_default(orgOctokit.rest, CONST_default.GITHUB_OWNER, ENGINEERING_TEAM_SLUG, login)));
  const internalReviewerLogins = new Set(reviewerLogins.filter((_, index) => membershipResults.at(index)));
  const latestStateByInternalReviewer = /* @__PURE__ */ new Map();
...
hasStandingInternalApproval(core3.getInput("OS_BOTIFY_TOKEN")).then((isApproved) => {
  if (isApproved) {
