...
var checkDeployBlockers_exports = {};
__export(checkDeployBlockers_exports, {
  default: () => checkDeployBlockers_default
});
module.exports = __toCommonJS(checkDeployBlockers_exports);
...
var run = function() {
  const issueNumber = Number(core2.getInput("ISSUE_NUMBER", { required: true }));
  console.log(`Fetching issue number ${issueNumber}`);
  return GithubUtils_default.octokit.issues.get({
    owner: CONST_default.GITHUB_OWNER,
    repo: CONST_default.APP_REPO,
    issue_number: issueNumber
  }).then(({ data }) => {
    ...
    if (uncheckedBoxRegex.test(data.body ?? "")) {
      ...
      core2.setOutput("HAS_DEPLOY_BLOCKERS", true);
      return;
    }
    return GithubUtils_default.octokit.issues.listComments({
      owner: CONST_default.GITHUB_OWNER,
      repo: CONST_default.APP_REPO,
      issue_number: issueNumber,
      per_page: 100
    ...
    if (!shipItRegex.exec(lastComment?.body ?? "")) {
      ...
      core2.setOutput("HAS_DEPLOY_BLOCKERS", false);
    }
  }).catch((error2) => {
