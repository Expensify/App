...
var formatCodeCovComment_exports = {};
__export(formatCodeCovComment_exports, {
  default: () => formatCodeCovComment_default
});
module.exports = __toCommonJS(formatCodeCovComment_exports);
...
function extractCoverageDeltaTable(body) {
  ...
  for (const line of lines) {
    ...
    if (trimmedLine.includes("\u{1F680} New features") || trimmedLine.includes(":rocket: New features") || trimmedLine.startsWith("<details>")) {
      break;
    }
    if (trimmedLine === "") {
      emptyLineCount++;
...
function formatAllLinesCoveredComment(originalBody) {
  ...
  for (const line of lines) {
    ...
    if (trimmedLine.includes(":rocket:") || trimmedLine.includes("\u{1F680}") || trimmedLine.startsWith("<details>")) {
      break;
    }
...
function formatCodeCovComment(originalBody) {
  ...
  if (hasDecreasedCoverage(originalBody)) {
    message = "\u274C Looks like you've decreased code coverage for some files. Please write tests to increase, or at least maintain, the existing level of code coverage. See our documentation [here](https://github.com/Expensify/App/blob/main/contributingGuides/CodeCov.md) for how to interpret this table.";
  } else {
...
async function run() {
  try {
    ...
    const commentId = import_github.context.payload.comment?.id;
    const commentBody = import_github.context.payload.comment?.body;
    const commentUser = import_github.context.payload.comment?.user;
    ...
    const isCodeCovReport = commentBody.includes("Codecov") && commentBody.includes("Report") && (commentBody.includes("Coverage \u0394") || commentBody.includes("All modified and coverable lines are covered by tests"));
    if (!isCodeCovReport) {
    ...
    await GithubUtils_default.octokit.issues.updateComment({
      owner: CONST_default.GITHUB_OWNER,
      repo: CONST_default.APP_REPO,
    ...
    console.log("Successfully formatted CodeCov comment! \u{1F389}");
  } catch (error2) {
    console.error("Error formatting CodeCov comment:", error2);
