...
var getPullRequestIncrementalChanges_exports = {};
__export(getPullRequestIncrementalChanges_exports, {
  default: () => getPullRequestIncrementalChanges_default
});
module.exports = __toCommonJS(getPullRequestIncrementalChanges_exports);
...
function exec(command, options) {
  ...
  return (0, import_util.promisify)(import_child_process.exec)(command, optionsWithEncoding);
}
function execSync(command, options) {
  ...
  return (0, import_child_process.execSync)(command, optionsWithEncoding);
}
...
var Git = class _Git {
  ...
  static show(ref, filePath) {
    try {
      ...
      throw new Error(`Failed to get file content from git: ${error3 instanceof Error ? error3.message : String(error3)}`);
    }
  }
  /**
  ...
  static async ensureRef(ref, remote = "origin") {
    ...
    try {
      console.log(`\u{1F504} Fetching missing ref: ${ref}`);
      ...
      throw new Error(`Failed to fetch git reference ${ref}: ${error3 instanceof Error ? error3.message : String(error3)}`);
    }
  }
  static async getMainBranchCommitHash(remote) {
    ...
    if (IS_CI) {
      const mainBaseRef2 = remote ? `${remote}/${baseRefName}` : `origin/${baseRefName}`;
      try {
        const { stdout: revParseOutput } = await exec(`git rev-parse ${mainBaseRef2}`);
        const mergeBaseHash2 = revParseOutput.trim();
        if (!mergeBaseHash2 || !/^[a-fA-F0-9]{40}$/.test(mergeBaseHash2)) {
          throw new Error(`git rev-parse returned unexpected output: ${mergeBaseHash2}`);
        }
        return mergeBaseHash2;
      } catch (error3) {
        error2(`Failed to get commit hash for ${mainBaseRef2}:`, error3);
        throw new Error(`Could not get commit hash for ${mainBaseRef2}`);
      }
    }
    ...
    try {
      ...
      warn(`Warning: Could not find merge base between ${mainBaseRef} and HEAD.`);
      try {
        const { stdout: revParseOutput } = await exec(`git rev-parse ${mainBaseRef}`);
        mergeBaseHash = revParseOutput.trim();
      } catch (fallbackError) {
        error2(`Failed to find merge base with ${mainBaseRef}:`, fallbackError);
  ...
  static async getChangedFilesWithStatus(fromRef, toRef, shouldIncludeUntrackedFiles = false) {
    if (IS_CI) {
      const files = await GithubUtils_default.paginate(GithubUtils_default.octokit.pulls.listFiles, {
        ...
        pull_number: import_github.context.payload.pull_request?.number ?? 0,
        // eslint-disable-next-line @typescript-eslint/naming-convention
        per_page: 100
      });
  ...
  static getUntrackedFiles(filePaths) {
    try {
      ...
      if (filePaths) {
        const pathsArray = Array.isArray(filePaths) ? filePaths : [filePaths];
        const normalizedPaths = pathsArray.map((p) => import_path.default.normalize(p));
        untrackedFiles = untrackedFiles.filter((file) => {
          const normalizedFile = import_path.default.normalize(file);
          return normalizedPaths.some((p) => normalizedFile === p || normalizedFile.startsWith(p + import_path.default.sep));
        });
      }
  ...
  static createFileDiffsForUntrackedFiles(untrackedFiles) {
    ...
    for (const filePath of untrackedFiles) {
      const absolutePath = import_path.default.join(process.cwd(), filePath);
      if (!import_fs.default.existsSync(absolutePath) || !import_fs.default.statSync(absolutePath).isFile()) {
        continue;
      }
      let fileContent;
      try {
        fileContent = import_fs.default.readFileSync(absolutePath, "utf8");
      } catch (error3) {
        continue;
      }
...
var import_request_error2 = __toESM(require_dist_node13());
async function run() {
  ...
  if (pullRequestNumberInput) {
    ...
    isOpenedAction = true;
    console.log(`\u{1F522} Using provided PR number ${prNumber} - treating as opened action`);
  } else {
    if (import_github2.context.eventName !== "pull_request") {
      throw new Error(`This action can only be run on pull_request events, but was run on: ${import_github2.context.eventName}. Provide PULL_REQUEST_NUMBER input to use with other event types.`);
    }
    const eventPayload = import_github2.context.payload;
  ...
  let changedFiles = [];
  if (isOpenedAction) {
    console.log("\u{1F195} PR treated as opened, including all files in the PR");
    changedFiles = (await GithubUtils_default.paginate(GithubUtils_default.octokit.pulls.listFiles, {
      owner: CONST_default.GITHUB_OWNER,
      repo: CONST_default.APP_REPO,
      // eslint-disable-next-line @typescript-eslint/naming-convention
    ...
    console.log("\u{1F504} PR was updated, checking only the new commits");
    if (import_github2.context.eventName !== "pull_request" || import_github2.context.payload.action !== "synchronize") {
    ...
    console.log(`\u{1F50D} Checking for local changes with push range ${beforeSha}..${afterSha}${filePathsInput ? `, looking for files ${JSON.stringify(filePathsInput)}` : ""}`);
    await Promise.all([Git_default.ensureRef(beforeSha), Git_default.ensureRef(afterSha)]);
    const localDiff = Git_default.diff(beforeSha, afterSha, filePathsInput);
    const localChangedFiles = /* @__PURE__ */ new Map();
    for (const file of localDiff.files) {
      localChangedFiles.set(file.filePath, file);
    }
    console.log(`\u{1F4DD} Found ${localChangedFiles.size} files with local changes in push`);
    if (localChangedFiles.size === 0) {
      console.log(`\u23ED\uFE0F No files changed in push - skipping API validation`);
      core3.setOutput("CHANGED_FILES", JSON.stringify([]));
      return;
    }
    console.log(`\u{1F310} Using GitHub API to validate ${localChangedFiles.size} files with local changes`);
    let prDiffString;
    try {
      prDiffString = await GithubUtils_default.getPullRequestDiff(prNumber);
    } catch (error3) {
      const isTooLarge = error3 instanceof import_request_error2.RequestError && typeof error3.response?.data === "object" && error3.response.data !== null && "errors" in error3.response.data && (error3.response.data.errors ?? []).some((e) => e.code === "too_large");
      if (!isTooLarge) {
        throw error3;
      }
      core3.warning(`PR #${prNumber} diff is too large for the GitHub API. Skipping incremental change detection.`);
      core3.setOutput("CHANGED_FILES", JSON.stringify([]));
      core3.setOutput("HAS_CHANGES", false);
      return;
    }
    const prDiff = Git_default.parseDiff(prDiffString);
    for (const prFileDiff of prDiff.files) {
      ...
      const localModifiedContent = /* @__PURE__ */ new Set();
      const prModifiedContent = /* @__PURE__ */ new Set();
  ...
  console.log(`\u{1F4C8} Total files changed: ${changedFiles.length}`);
  core3.startGroup("\u{1F4CA} Changed files:");
  console.log(changedFiles);
  core3.endGroup();
  core3.setOutput("CHANGED_FILES", JSON.stringify(changedFiles));
  core3.setOutput("HAS_CHANGES", changedFiles.length > 0);
}
...
var getPullRequestIncrementalChanges_default = run;
