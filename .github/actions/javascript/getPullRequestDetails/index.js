...
var GithubUtils = class {
  ...
  static getPullRequestMergerLogin(pullRequestNumber2) {
    return this.octokit.pulls.get({
      owner: CONST_default.GITHUB_OWNER,
      repo: CONST_default.APP_REPO,
      pull_number: pullRequestNumber2
    }).then(({ data: pullRequest }) => pullRequest.merged_by?.login);
  }
  static getPullRequestBody(pullRequestNumber2) {
    return this.octokit.pulls.get({
      owner: CONST_default.GITHUB_OWNER,
      repo: CONST_default.APP_REPO,
      pull_number: pullRequestNumber2
  ...
  static async getPullRequestMergeBaseSHA(pullRequestNumber2) {
  ...
  static getAllReviewComments(pullRequestNumber2) {
    return this.paginate(
  ...
  static async getPullRequestChangedSVGFileNames(pullRequestNumber2) {
  ...
  static async getPullRequestDiff(pullRequestNumber2) {
    if (!this.internalOctokit) {
      this.initOctokit();
    }
...
var pullRequestNumber = getJSONInput("PULL_REQUEST_NUMBER", { required: false }, null);
var user = core3.getInput("USER", { required: true });
...
function outputMergeActor(PR) {
  if (user === CONST_default.OS_BOTIFY) {
    core3.setOutput("MERGE_ACTOR", PR.merged_by?.login);
  } else {
    core3.setOutput("MERGE_ACTOR", user);
  }
}
function outputForkedRepoUrl(PR) {
  if (PR.head?.repo?.html_url === CONST_default.APP_REPO_URL) {
    core3.setOutput("FORKED_REPO_URL", "");
  } else {
    core3.setOutput("FORKED_REPO_URL", `${PR.head?.repo?.html_url}.git`);
  }
}
GithubUtils_default.octokit.pulls.get({
  ...DEFAULT_PAYLOAD,
  ...
  if (!isEmptyObject(PR)) {
    ...
    core3.setOutput("MERGE_COMMIT_SHA", PR.merge_commit_sha);
    core3.setOutput("HEAD_COMMIT_SHA", PR.head?.sha);
    core3.setOutput("IS_MERGED", PR.merged);
    outputMergeActor(PR);
