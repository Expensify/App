...
var getPreviousVersion_exports = {};
__export(getPreviousVersion_exports, {
  default: () => getPreviousVersion_default
});
module.exports = __toCommonJS(getPreviousVersion_exports);
...
function tagExists(tag) {
  try {
    (0, import_child_process.execSync)(`git show-ref --tags ${tag}`, { stdio: "ignore" });
    return true;
  } catch (error2) {
    ...
    while (shouldRetry) {
      try {
        if (needsRepack) {
          (0, import_child_process.execSync)("git repack -d", { stdio: "inherit" });
        }
        (0, import_child_process.execSync)(`git ls-remote --exit-code --tags origin ${tag}`, { stdio: "ignore" });
        doesTagExist = true;
        shouldRetry = false;
      } catch (e) {
        if (!needsRepack) {
...
function getValidMergedPRs(commits) {
  const mergedPRs = /* @__PURE__ */ new Map();
  for (const commit of commits) {
    const author = commit.authorName;
    if (author === CONST_default.OS_BOTIFY) {
      continue;
    }
...
async function getMergedPRsDeployedBetween(fromTag, toTag, repositoryName) {
  ...
  core2.startGroup("Parsed PRs:");
  core2.info(mergedPRs.map((pr) => pr.prNumber).join(", "));
  core2.endGroup();
  if (submoduleUpdates.length > 0) {
    core2.startGroup("Submodule updates:");
    core2.info(submoduleUpdates.map((u) => u.version).join(", "));
    core2.endGroup();
  }
...
function run() {
  const semverLevel = core3.getInput("SEMVER_LEVEL", { required: true });
  if (!semverLevel || !isValidSemverLevel(semverLevel)) {
    core3.setFailed(`'Error: Invalid input for 'SEMVER_LEVEL': ${semverLevel}`);
  }
  const { version: currentVersion } = JSON.parse((0, import_fs.readFileSync)("./package.json", "utf8"));
  if (!currentVersion) {
    core3.setFailed("Error: Could not read package.json");
  }
  const previousVersion = GitUtils_default.getPreviousExistingTag(currentVersion, semverLevel);
  core3.setOutput("PREVIOUS_VERSION", previousVersion);
  return previousVersion;
}
