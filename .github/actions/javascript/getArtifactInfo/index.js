...
var getArtifactInfo_exports = {};
__export(getArtifactInfo_exports, {
  default: () => getArtifactInfo_default
});
module.exports = __toCommonJS(getArtifactInfo_exports);
...
var run = function() {
  const artifactName = core2.getInput("ARTIFACT_NAME", { required: true });
  return GithubUtils_default.getArtifactByName(artifactName).then((artifact) => {
    if (artifact === void 0) {
      ...
      core2.setOutput("ARTIFACT_FOUND", false);
      return;
    }
    console.log("Artifact info", artifact);
    core2.setOutput("ARTIFACT_FOUND", true);
    core2.setOutput("ARTIFACT_ID", artifact.id);
    core2.setOutput("ARTIFACT_WORKFLOW_ID", artifact.workflow_run?.id);
  }).catch((error2) => {
    console.error("A problem occurred while trying to communicate with the GitHub API", error2);
