...
// .github/actions/javascript/verifyStagingBuildsSucceeded/verifyStagingBuildsSucceeded.ts
var core2 = __toESM(require_core(), 1);
var STAGING_BRANCH = "staging";
var DEPLOY_WORKFLOW = "deploy.yml";
var NATIVE_UPLOAD_JOBS = [
  { platform: "Android", job: "Upload Android to Google Play" },
  { platform: "iOS", job: "Upload iOS to TestFlight" }
];
function setResult(allSucceeded, reason, workflowRunURL = "") {
  const message = workflowRunURL ? `${reason} ${workflowRunURL}` : reason;
  console.log(message);
  core2.setOutput("ALL_NATIVE_BUILDS_SUCCEEDED", allSucceeded);
  core2.setOutput("REASON", message);
}
async function getStagingTag() {
  const packageJSON = JSON.parse(await GithubUtils_default.getFileContents("package.json", STAGING_BRANCH));
...
async function run() {
  try {
    ...
    const { data: ref } = await GithubUtils_default.octokit.git.getRef({
      owner: CONST_default.GITHUB_OWNER,
      repo: CONST_default.APP_REPO,
      ref: `tags/${tag}`
    });
    ...
    const { data: jobsResponse } = await GithubUtils_default.octokit.actions.listJobsForWorkflowRun({
      owner: CONST_default.GITHUB_OWNER,
      repo: CONST_default.APP_REPO,
      /* eslint-disable-next-line @typescript-eslint/naming-convention */
      run_id: deployRun.id,
    ...
    core2.error(error3 instanceof Error ? error3 : String(error3));
...
var verifyStagingBuildsSucceeded_default = run;
export {
  verifyStagingBuildsSucceeded_default as default
};
