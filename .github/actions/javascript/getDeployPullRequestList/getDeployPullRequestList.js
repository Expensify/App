...
const isProductionDeploy = ActionUtils.getJSONInput(
...
getTagsOrReleases(isProductionDeploy)
    ...
    .then(({data}) => {
        ...
        if (priorTagIndex === 0) {
            console.log(
                `No ${itemToFetch} was found for input tag ${inputTag}.` +
        ...
        if (priorTagIndex === tags.length) {
            const err = new Error(
                'Somehow, the input tag was at the end of the paginated result, ' +
