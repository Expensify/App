...
var getDeployPullRequestList_exports = {};
__export(getDeployPullRequestList_exports, {
  default: () => getDeployPullRequestList_default
});
module.exports = __toCommonJS(getDeployPullRequestList_exports);
...
async function getMergedPRsDeployedBetween(fromTag, toTag, repositoryName) {
  ...
  core3.startGroup("Parsed PRs:");
  core3.info(mergedPRs.map((pr) => pr.prNumber).join(", "));
  core3.endGroup();
  if (submoduleUpdates.length > 0) {
    core3.startGroup("Submodule updates:");
    core3.info(submoduleUpdates.map((u) => u.version).join(", "));
    core3.endGroup();
  }
...
async function run() {
  try {
    const inputTag = core4.getInput("TAG", { required: true });
    ...
    await GithubUtils_default.paginate(
      GithubUtils_default.octokit.repos.listReleases,
      {
    ...
    const prList = await GitUtils_default.getPullRequestsDeployedBetween(priorTag, inputTag, CONST_default.APP_REPO);
    console.log("Found the pull request list: ", prList);
    core4.setOutput("PR_LIST", prList);
    let mobileExpensifyPRList = [];
    try {
      mobileExpensifyPRList = await GitUtils_default.getPullRequestsDeployedBetween(priorTag, inputTag, CONST_default.MOBILE_EXPENSIFY_REPO);
      ...
      if (process.env.GITHUB_REPOSITORY !== `${CONST_default.GITHUB_OWNER}/${CONST_default.APP_REPO}`) {
        ...
        console.error("Failed to fetch Mobile-Expensify PRs from main repository:", error2);
      }
    }
    core4.setOutput("MOBILE_EXPENSIFY_PR_LIST", mobileExpensifyPRList);
  } catch (error2) {
