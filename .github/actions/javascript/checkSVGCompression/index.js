...
// ../97013-distance-receipt/node_modules/@actions/github/lib/github.js
var require_github = __commonJS({
  "../97013-distance-receipt/node_modules/@actions/github/lib/github.js"(exports) {
    "use strict";
    var __createBinding = exports && exports.__createBinding || (Object.create ? (function(o, m, k, k2) {
      if (k2 === void 0) k2 = k;
      Object.defineProperty(o, k2, { enumerable: true, get: function() {
...
var GithubUtils = class {
   ...
   */
  static async getFileContents(path2, ref = CONST_default.DEFAULT_BASE_REF) {
    ...
    if (Array.isArray(data)) {
      throw new Error(`Provided path ${path2} refers to a directory, not a file`);
    }
    if (!("content" in data)) {
      throw new Error(`Provided path ${path2} is invalid`);
    }
...
function compressSvgFile(filePath, isSavingFile) {
  ...
  try {
    ...
    console.error(`\u274C Error compressing ${filePath}:`, error2);
    return {
      filePath,
      originalSize,
...
function compressSvgFiles(regularSvgFiles, ignoredFiles = []) {
  console.log(`\u{1F680} Starting compression ${regularSvgFiles.length} SVG file(s)`);
...
function checkCompressedSvgFiles(regularSvgFiles, ignoredFiles = []) {
  console.log(`\u{1F680} Checking if all SVG files are compressed...`);
...
async function getChangedSvgFilesFromGithub() {
  try {
    ...
    const changedFiles = await GithubUtils_default.getPullRequestChangedSVGFileNames(pullRequestNumber);
    const svgFiles = changedFiles.filter((file) => path.extname(file.toLowerCase()) === ".svg").map((file) => path.resolve(file)).filter((file) => fs.existsSync(file));
    console.log(`Found ${svgFiles.length} changed SVG file(s) in PR`);
    return svgFiles;
  } catch (error2) {
    console.error("\u274C Error getting files from GitHub:", error2);
    return [];
  }
}
...
async function run(mode, options) {
  console.log("\u{1F50D} Searching for SVG files...");
  switch (mode) {
    case "directory": {
...
async function run2() {
  try {
    ...
    const summary = await compressSvg_default("pullRequest", { token });
...
export {
  checkSVGCompression_default as default
};
