  ...
  "node_modules/@actions/github/node_modules/@octokit/request/dist-node/index.js"(exports2) {
    ...
    function fetchWrapper(requestOptions) {
      ...
      return fetch(requestOptions.url, Object.assign(
        ...
        if (error3 instanceof requestError.RequestError) throw error3;
  ...
  "node_modules/@octokit/plugin-throttling/dist-node/index.js"(exports2) {
    ...
    async function doRequest(state, request, options) {
      ...
      if (isGraphQL) {
        ...
        res.data.errors.some((error3) => error3.type === "RATE_LIMITED")) {
    ...
    `);
      ...
      state.retryLimiter.on("failed", async function(error3, info3) {
        const options = info3.args[info3.args.length - 1];
        const {
          pathname
        } = new URL(options.url, "http://github.test");
        ...
        const {
          ...
          if (/\bsecondary rate\b/i.test(error3.message)) {
            const retryAfter2 = Math.max(~~error3.response.headers["retry-after"], state.minimumSecondaryRateRetryAfter);
          ...
          if (error3.response.headers != null && error3.response.headers["x-ratelimit-remaining"] === "0") {
            const rateLimitReset = new Date(~~error3.response.headers["x-ratelimit-reset"] * 1e3).getTime();
...
// .github/actions/javascript/waitForPreviousRuns/waitForPreviousRuns.ts
var waitForPreviousRuns_exports = {};
__export(waitForPreviousRuns_exports, {
  default: () => waitForPreviousRuns_default
});
module.exports = __toCommonJS(waitForPreviousRuns_exports);
...
var GithubUtils = class {
  ...
  static async getWorkflowRunURLForCommit(commitSha, workflowFile) {
    try {
      ...
      console.warn(`Failed to find workflow run for commit ${commitSha}:`, error3);
      return void 0;
    }
  }
  /**
  ...
  static async getCommitHistoryBetweenTags(fromTag, toTag, repositoryName) {
    ...
    try {
      ...
      if (error3 instanceof import_request_error.RequestError && error3.status === 404) {
        core.error(
...
var DEFAULT_POLL_RATE_S = 20;
var DEFAULT_QUEUE_LIMIT = 20;
var MAX_API_RETRIES = 2;
var ACTIVE_STATUSES = /* @__PURE__ */ new Set(["in_progress", "queued", "waiting", "requested", "pending"]);
async function getOlderActiveRuns(workflowID, currentRunID, queueLimit) {
  const response = await GithubUtils_default.octokit.actions.listWorkflowRuns({
    owner: CONST_default.GITHUB_OWNER,
    repo: CONST_default.APP_REPO,
...
function run() {
  const workflowID = core2.getInput("WORKFLOW_ID", { required: true });
  const currentRunID = Number(core2.getInput("CURRENT_RUN_ID", { required: true }));
  const pollRateSeconds = Number(core2.getInput("POLL_RATE_SECONDS")) || DEFAULT_POLL_RATE_S;
  const pollRateMs = pollRateSeconds * 1e3;
  const queueLimit = Number(core2.getInput("QUEUE_LIMIT")) || DEFAULT_QUEUE_LIMIT;
  core2.info(`Current run ID: ${currentRunID}`);
  core2.info(`Workflow ID: ${workflowID}`);
  core2.info(`Poll rate: ${pollRateSeconds}s, Queue limit: ${queueLimit}`);
  core2.info("Waiting for all earlier runs of this workflow to complete...");
  return new Promise((resolve, reject) => {
    let intervalId;
    ...
    const check = () => {
      ...
      isChecking = true;
      pollCount++;
      getOlderActiveRuns(workflowID, currentRunID, queueLimit).then((olderActiveRuns) => {
        consecutiveErrors = 0;
        maxQueueDepth = Math.max(maxQueueDepth, olderActiveRuns.length);
        if (olderActiveRuns.length === 0) {
          core2.notice(`Queue summary: maxRunsAhead=${maxQueueDepth}, iterations=${pollCount}, waitTime=${(pollCount - 1) * pollRateSeconds}s`);
          core2.info("No earlier runs in progress. Proceeding with build.");
          clearInterval(intervalId);
          resolve();
          return;
        }
        const runIDs = olderActiveRuns.map((workflowRun) => `#${workflowRun.id} (${workflowRun.status})`).join(", ");
        core2.info(`Waiting for ${olderActiveRuns.length} earlier run(s): ${runIDs}. Polling again in ${pollRateSeconds}s...`);
      }).catch((error3) => {
        consecutiveErrors++;
        if (consecutiveErrors > MAX_API_RETRIES) {
          core2.error(`API failed ${consecutiveErrors} times in a row. Giving up.`);
          clearInterval(intervalId);
          reject(error3);
          return;
        }
        core2.warning(`API error (attempt ${consecutiveErrors}/${MAX_API_RETRIES}). Retrying next poll...`);
      }).finally(() => {
        isChecking = false;
      });
    };
...
var waitForPreviousRuns_default = run;
