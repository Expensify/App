...
var isAuthorizedContributor_exports = {};
__export(isAuthorizedContributor_exports, {
  default: () => isAuthorizedContributor_default,
  isAuthorizedContributor: () => isAuthorizedContributor,
  isContributorPlusMember: () => isContributorPlusMember,
  isInternalExpensifyEngineer: () => isInternalExpensifyEngineer
});
module.exports = __toCommonJS(isAuthorizedContributor_exports);
...
var ISSUE_URL_PATTERN = /https:\/\/github\.com\/(Expensify\/[^/]+)\/issues\/(\d+)/g;
...
function logVerificationError(repoFullName, number, error2) {
  const message = error2 instanceof Error ? error2.message : String(error2);
  console.log(`Could not verify ${repoFullName}#${number}: ${message}`);
}
async function isContributorPlusMember(username, orgToken) {
  GithubUtils_default.initOctokitWithToken(orgToken);
  const isMember = await isTeamMember_default(GithubUtils_default.octokit, CONST_default.GITHUB_OWNER, CONTRIBUTOR_PLUS_TEAM_SLUG, username);
...
async function isInternalExpensifyEngineer(username, orgToken) {
  GithubUtils_default.initOctokitWithToken(orgToken);
  return isTeamMember_default(GithubUtils_default.octokit, CONST_default.GITHUB_OWNER, ENGINEERING_TEAM_SLUG, username);
}
async function isAuthorizedViaLinkedIssues(prBody, actor) {
  for (const match of prBody.matchAll(ISSUE_URL_PATTERN)) {
    ...
    try {
      const { data: issue } = await GithubUtils_default.octokit.issues.get({
        owner,
        repo,
      ...
      logVerificationError(repoFullName, number, error2);
    }
  }
  return false;
}
async function isAuthorizedContributor({ prNumber, actor, actorAssociation, repoOwner, repoName, githubToken, orgToken }) {
  ...
  GithubUtils_default.initOctokitWithToken(githubToken);
...
async function run() {
  const prNumber = Number.parseInt(core3.getInput("PR_NUMBER", { required: true }), 10);
  const actor = core3.getInput("ACTOR", { required: true });
  const actorAssociation = core3.getInput("ACTOR_ASSOCIATION", { required: true });
  const githubToken = core3.getInput("GITHUB_TOKEN", { required: true });
  const orgToken = core3.getInput("OS_BOTIFY_TOKEN", { required: true });
  ...
  core3.setOutput("IS_AUTHORIZED", isAuthorized);
  const isInternal = await isInternalExpensifyEngineer(actor, orgToken);
  core3.setOutput("IS_INTERNAL", isInternal);
}
...
var isAuthorizedContributor_default = run;
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  isAuthorizedContributor,
  isContributorPlusMember,
  isInternalExpensifyEngineer
});
