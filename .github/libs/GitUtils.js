...
function getMergeLogsAsJSON(fromRef, toRef) {
    ...
    console.log(
        'Getting pull requests merged between the following refs:',
        fromRef,
        toRef,
    );
    ...
    return new Promise((resolve, reject) => {
        let stdout = '';
        let stderr = '';
        const spawnedProcess = spawn('git', [
            'log',
            '--format={"commit": "%H", "subject": "%s"},',
            `${fromRef}...${toRef}`,
        ]);
    ...
    return _.reduce(
        ...
        (mergedPRs, commitMessage) => {
            ...
            const match = commitMessage.match(
                /Merge pull request #(\d+) from (?!Expensify\/(?:main|version-|update-staging-from-main|update-production-from-staging))/,
            );
...
function getPullRequestsMergedBetween(fromRef, toRef) {
    ...
    return getMergeLogsAsJSON(fromRef, toRef)
        .then((mergeList) => {
            console.log(
                `Commits made between ${fromRef} and ${toRef}:`,
                mergeList,
            );
        ...
        .then((fullMergeList) => {
            ...
            const duplicateMergeList = _.chain(fullMergeList)
                ...
                .pluck('commit')
                .value();
            const finalMergeList = _.filter(
                targetMergeList,
                (i) => !_.contains(duplicateMergeList, i.commit),
            );
            console.log(
                'Filtered out the following commits which were duplicated in the full git log:',
                _.difference(targetMergeList, finalMergeList),
            );

            // Find which commit messages correspond to merged PR's
            const pullRequestNumbers = getValidMergedPRs(
                _.pluck(finalMergeList, 'subject'),
            );
            console.log(
                `List of pull requests merged between ${fromRef} and ${toRef}`,
                pullRequestNumbers,
            );
