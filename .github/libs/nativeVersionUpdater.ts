...
const PLIST_PATH_SVC = './ios/ShareViewController/Info.plist';
...
function updateiOSVersion(version: string): string {
    ...
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleShortVersionString ${shortVersion}" ${PLIST_PATH_SVC}`);
    ...
    execSync(`/usr/libexec/PlistBuddy -c "Set :CFBundleVersion ${cfVersion}" ${PLIST_PATH_SVC}`);
