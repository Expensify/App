 ...
 */
 ...
 db.execute('CREATE TABLE IF NOT EXISTS magic_map (id INTEGER PRIMARY KEY, record_key TEXT NOT NULL, value TEXT NOT NULL)');
 ...
 const provider = {
      ...
      */
     getItem(key) {
         return db.executeAsync("SELECT value from magic_map where record_key=?;", [key]).then(({ rows }) => {
      ...
      */
     multiGet(keys) {
      return db.executeAsync("SELECT value from magic_map where record_key IN ?;", [keys]).then(({ rows }) => {
      ...
      */
     setItem(key, value) {
        return db.executeAsync("INSERT into magic_map (record_key, value) VALUES (?, ?);", [key, value]);
     },
      ...
      */
     multiSet(pairs) {
        return db.executeAsync("INSERT into magic_map (record_key, value) VALUES (?);", [pairs]);
     },
