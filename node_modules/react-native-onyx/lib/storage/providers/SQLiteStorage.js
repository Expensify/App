 ...
 */
 ...
 const provider = {
      ...
      */
     multiMerge(pairs) {
        return db.executeBatchAsync([["INSERT into magic_map (record_key, value) VALUES (?, ?) ON CONFLICT DO UPDATE SET value = json_patch(value, ?);", pairs.map(ele => [ele[0], ele[1], ele[1]])]]);
     ...
     getAllKeys: () => {
      return db.executeAsync("SELECT record_key from magic_map;", [key]).then(({ rows }) => {
        const res = rows[0]._array.map(row => row.record_key);
        return res;
       });
     },
