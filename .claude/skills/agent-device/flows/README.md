...
- **`--settle` is not available here.** Replay folds it into the selector and the step fails with `Invalid selector term "--settle"`. It also would not help: it never fails, so it cannot enforce a `@post`. Guard transitions with `wait "<selector>" <timeoutMs>`. `wait stable` does replay, but it reports success from whatever screen the app happens to be on.
