...
- Native RHP swipe-back automatically disabled for exactly as long as a pre-insert exists (handled by `RightModalNavigator.tsx`, not by this hook or its callers - see Swipe-back gating below), since a swipe can dismiss the RHP before `cleanupPreMount()`/unmount cleanup runs, flashing the pre-inserted destination
...
- There is no caller-facing option for this - it's automatic and not configurable. `RightModalNavigator.tsx` listens for the same `DISABLE_RHP_ANIMATION`/`RESTORE_RHP_ANIMATION` events `preInsertFullscreenUnderRHP`/`clearFullscreenPreInsertedFlag`/`removePreInsertedFullscreenIfNeeded` already emit, and sets `gestureEnabled: false`/`true` alongside the existing `animation` toggle. No separate vote count or hook-side state: the RHP route's native swipe-to-dismiss is disabled for exactly as long as `Navigation.getIsFullscreenPreInsertedUnderRHP()` would report true, because that's precisely when the flash hazard exists - not tied to any hook's mount lifecycle, so it's correct even through `shouldPreservePreInsertedRouteOnUnmount: () => true` unmounts (the global pre-insert flag, and therefore the gesture disable, survives until whoever finishes the dismiss actually clears it).
- This only gates swiping the whole RHP away. Swiping between steps of a multi-step RHP flow is a separate gesture (each step's own inner stack) and is never affected.
...
- New code outside `RightModalNavigator.tsx` disables/re-enables the RHP's `gestureEnabled` manually (e.g. via `navigation.setOptions` or a custom gesture) - `RightModalNavigator.tsx` is the one sanctioned place this is set, since it's the only component holding the root-stack-level `navigation` this feature needs, and it's already tied to the pre-insert lifecycle via `DISABLE_RHP_ANIMATION`/`RESTORE_RHP_ANIMATION`
...
- `gestureEnabled` (on `RightModalNavigator`/RHP-related navigation calls)
