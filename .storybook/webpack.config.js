...
module.exports = ({config}) => {
    ...
    const definePluginIndex = _.findIndex(
        config.plugins,
        (plugin) => plugin.constructor.name === 'DefinePlugin',
    );
    config.plugins[definePluginIndex].definitions.__REACT_WEB_CONFIG__ =
        JSON.stringify(env);
    config.resolve.extensions = custom.resolve.extensions;

    const babelRulesIndex = _.findIndex(
        custom.module.rules,
