...
### [@shopify+flash-list+2.3.0+017+native-sticky-header-release.patch](@shopify+flash-list+2.3.0+017+native-sticky-header-release.patch)

- Reason: During Android table search resets, native scrolling can return the normal header to view before FlashList's JS sticky-index callback removes its overlay. The two copies then flash together. The opt-in `stickyHeaderConfig.hideWhenInactive` drives overlay opacity from FlashList's existing animated scroll position and the active header's absolute start. The normal cell and its layout stay untouched. A layout notification refreshes that boundary when the page header or item geometry changes.
- Boundary: Off by default, enabled by Table only on Android. Limited to one non-inverted sticky header at offset zero with `hideRelatedCell` disabled. Other configurations retain their existing behavior, including grouped expense search which hides its related cell. This is not a general multi-section sticky-header rewrite.
- Files changed: `src/FlashListProps.ts`, `src/recyclerview/RecyclerView.tsx`, `src/recyclerview/components/StickyHeaders.tsx`, and their `dist` implementation/declaration counterparts.
- Regression coverage: `tests/ui/FlashListStickyHeadersTest.tsx` exercises a scroll-value change before JS sticky-index delivery, layout-boundary refresh, and opt-out cases; `tests/ui/TableTest.tsx` covers platform opt-in.
