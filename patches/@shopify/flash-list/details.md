...
- Reason: Fixes a "Maximum update depth exceeded" (#185) infinite render loop on web with classic (non-overlay) scrollbars — i.e. Windows/Linux Chrome and macOS with "Always show scroll bars". For a vertical list FlashList derives its cross-axis bounded size from `firstChildViewLayout.width` (the scroll viewport's **client** width, which excludes the scrollbar), and that size is what `getLayoutSize().width` — and therefore `ViewHolderCollection`'s `fixedContainerSize` — reports. When the vertical scrollbar toggles, the width steps by the scrollbar size (e.g. 375 ↔ 355); `ViewHolderCollection`'s `useLayoutEffect([fixedContainerSize])` calls `recyclerViewContext.layout()`, which re-renders and re-measures, which changes content height, which toggles the scrollbar again — an infinite layout-effect → setState loop that throws #185 from `<ViewHolderCollection>`. This patch breaks the loop inside `LinearLayoutManager.updateLayoutParams` (via `settleScrollbarOscillation`) by detecting the scrollbar ping-pong and pinning `boundedSize` instead of thrashing it — reserving no width.
  ...
  4. **The current frame is the smaller (with-scrollbar) value**. The flip count alone looks at the whole window, so two *separate* finished toggles (e.g. `B,A,B,A`) can reach the threshold while the list currently has **no** scrollbar. Pinning to the narrow width then would wrongly narrow the rows *and* — because `boundedSize` would be unchanged from the previous smaller measurement — schedule no recompute to correct itself, leaving it stuck until an unrelated resize/remount. Requiring the current measurement to be the smaller value means the pin always matches present reality; a live ping-pong keeps producing smaller frames, so this delays a genuine pin by at most a frame or two.

  When detected, `boundedSize` is pinned to the **smaller** of the two values, which already accounts for the scrollbar so items never overflow the client width.

  While pinned, only the **smaller** (with-scrollbar) value keeps the pin — **any other** measured size releases it immediately, honoring the real size and restarting detection. This covers both a genuine resize *outside* the pair and the scrollbar genuinely disappearing *within* the pair (e.g. content is filtered/removed so a vertical list no longer needs to scroll, and the browser reports the wider no-scrollbar width). The release must be immediate — on the *first* larger measurement — because pinning holds `boundedSize` constant, so returning the pinned value again schedules no `recomputeLayouts`; a data shrink that removes the scrollbar often produces a single layout pass, so anything that waited for the larger value to repeat would stick at the stale, scrollbar-narrowed width (rows too narrow/wrapped) until an unrelated resize or remount.

  Releasing on the first larger value does **not** re-open the crash: pinning to the smaller width halts the internal size↔layout loop (constant `boundedSize` → no recompute → no re-render), so post-pin measurements come only from genuine external changes. And because content reflow is monotonic (a **wider** layout is never **taller**), forcing the narrower width can only keep — never remove — a scrollbar the list needed at the boundary, so the measured size settles on the smaller value and stays pinned. The larger value re-appears only when the content itself shrinks, which is exactly when the pin *should* release.
