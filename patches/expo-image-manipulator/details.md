...
- Reason:
    ...
    `ImageContextLostException` for every 10-bit HDR HEIC, blocking HEIC to JPEG conversion. This is PR #50011
    verbatim: skip the redraw when the image is already upright, otherwise redraw through `drawInNewContext`.
    ```
  
- Known gap: the early return is gated on orientation alone, so an upright PQ-tagged source keeps its PQ profile and
  renders black in decoders without PQ support. Apple's HDR HEICs are plain Display P3 and come out correct.
