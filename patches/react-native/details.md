...
### [react-native+0.86.0+042+fix-merged-transaction-mount-order.patch](react-native+0.86.0+042+fix-merged-transaction-mount-order.patch)

- Reason: Fixes the Android Fabric mount-item failures behind `APP-7AR` (`addViewAt: failed to insert view into parent`, underlying `IndexOutOfBoundsException: index=1 count=0`) and `APP-H7A` (`Unable to remove a view from a view that is not a ViewGroup`). Both fire when a mount batch is applied to a native view tree that no longer matches the shadow tree the batch was computed against. `FabricMountingManager::executeMount` writes `Delete` mount items last, and React Native's own comment on that block says the optimization "is incorrect when multiple transactions are merged together" — which is exactly what `FabricUIManagerBinding::schedulerDidFinishTransaction` does on Android when commits outpace UI-thread mounting. Patch `+015` narrowed that hazard with `MountingTransaction::canMergeWith`, but left three holes, each closed by one hunk here:
    - `canMergeWith` scanned the incoming transaction for `Type::Create` only, while its own doc comment names the `CREATE -> INSERT` sequence. `Insert` is a separate mutation type that re-attaches an existing tag, so a pending `Delete(tag)` merged with an incoming `Insert(same tag)` passed the guard and the deferred `Delete` then destroyed a view the shadow tree still considered mounted. The guard now also refuses `Insert`, and refuses any incoming mutation parented under a tag the pending transaction deletes (the remove-side shape behind `APP-H7A`).
    - Because `canMergeWith` can refuse a merge, a surface can hold more than one pending transaction — something stock React Native never allows. The merge-target lookup still scanned from the front of `pendingTransactions_` and `mergeWith` merges in place, so a newer transaction could be folded into an *older* pending entry and execute ahead of the entry in between, against a tree that entry had not built yet. The lookup now searches from the back.
    - `schedulerShouldRenderTransactions` swapped the whole queue into a local vector before mounting, justified in-source by "we're already combining all the transactions for the same surface ID in a single transaction ... so operations won't run out of order". Patch `+015` made that premise false. A state update committed synchronously from inside `executeMount` re-enters this method, finds the shared queue empty, and mounts the new transaction ahead of the entries still waiting in the local copy. The method is also called from both the JS thread (scheduled rendering updates) and the UI thread (synchronous mounts). Now only one call drains the queue at a time (`isDrainingPendingTransactions_`): a call that finds a drain in progress, whether re-entrant or on another thread, leaves its transaction queued and the draining call mounts it after everything queued before it. Taking entries one at a time without that guard is not enough, because a concurrent drain could take a later entry and mount it first. The lock is still released before mounting, and the guard is reset if `executeMount` throws.
    - Finally, `maintainMutationOrder` is now also set when the transaction is a merge (`MountingTransaction::wasMerged()`), so a merged batch is mounted in differ order instead of relying on the guard to enumerate every unsafe mutation pairing. Non-merged transactions keep the delete-batching optimization unchanged.
- Verification: `./patches/react-native/tests/run.sh` compiles the real, unmodified `MountingTransaction.cpp` against minimal stand-in headers and asserts the guard and ordering invariants. It fails on unpatched `node_modules` and passes with this patch; no device or manual reproduction required.
- Upstream PR/issue: React Native's own `FIXME: this optimization is incorrect when multiple transactions are merged together` in [`FabricMountingManager.cpp`](https://github.com/facebook/react-native/blob/v0.86.0/packages/react-native/ReactAndroid/src/main/jni/react/fabric/FabricMountingManager.cpp#L974-L980). The transaction merging this patch repairs is Expensify's own (patch `+015`), so there is no upstream fix to adopt.
...
- 0.86.0 migration note: this patch layers on patch `+015`. If `+015` is ever dropped — for example because upstream stops merging mounting transactions on Android, or `disableMountItemReorderingAndroid` is enabled by default — re-evaluate this one at the same time: the `wasMerged()` hunk becomes redundant when reordering is off, and the queue hunks become redundant when a surface can only hold one pending transaction.
