...
- 0.86.0 migration note: **drop this patch with the RN 0.87 upgrade** — upstream commit `45904c8` is absent from every 0.86.x release but ships in `v0.87.0`, and the patch will not apply against it. The `scripts/cxx-api/api-snapshots/*.api` hunks of the upstream commit were omitted, since those files are not shipped in the npm package.
