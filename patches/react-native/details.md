...
- Upstream PR/issue: https://github.com/react/react-native/issues/58175
- E/App issue: https://github.com/Expensify/App/issues/98791
- PR introducing patch: https://github.com/Expensify/App/pull/99608
- Validation note: **Validated manually on Android with the patch applied**, on (1) the standalone Android app on a physical device and (2) Android web (Chrome/mWeb) — no crash, and no `addViewAt`/`removeViewAt` mounting exception in `logcat`, including under rapid navigation and while offline. Screen recordings of both runs are attached to the PR. Note these runs confirm the post-fix state only; a pre-fix crash reproduction was not separately recorded, as the crash is timing-dependent with no deterministic on-demand repro. APP-H7A was not confirmed in this pass — still open per the C+ reviewer's outstanding request.
