...
### [react-native+0.86.0+040+restore-non-linear-text.patch](react-native+0.86.0+040+restore-non-linear-text.patch)

- Reason: Reverts RN 0.86's unconditional `Paint.LINEAR_TEXT_FLAG` on Android, restoring 0.85.3 rendering. Upstream [#56409](https://github.com/facebook/react-native/pull/56409) deleted the `enableAndroidLinearText` feature flag and made both of its call sites unconditional; the flag defaulted to `false` in OSS, so 0.86.0 silently turned linear text on for every app. `LINEAR_TEXT_FLAG` disables font hinting, which changes glyph advance widths by a fraction of a pixel — and `TextLayoutManager` rounds the measured text width up (`ceil(Layout.getDesiredWidth(...))`) while rounding the available width down (`floor(width)`). Line breaking is a step function, so a sub-pixel shift moves a whole trailing word onto a second line inside a view whose height was computed for one line, and that line is clipped: the word disappears with no ellipsis and no change in row height. Both call sites are reverted together so `Text` and `TextInput` keep rasterising glyphs identically.
- Upstream PR/issue: [#56409](https://github.com/facebook/react-native/pull/56409) (the rollout being reverted) — related, still open: [#56402](https://github.com/facebook/react-native/issues/56402) (Android 15/16 text clipping via `useBoundsForWidth`, whose fix was reverted in [#56282](https://github.com/facebook/react-native/commit/5964a197) and is absent from 0.86)
...
- 0.86.0 migration note: this is a *deviation from upstream*, not a backport — re-evaluate on the RN 0.87 upgrade. Drop it if upstream reinstates the flag, or if the `useBoundsForWidth` fix behind [#56402](https://github.com/facebook/react-native/issues/56402) lands and removes the underlying measure/draw width mismatch.
