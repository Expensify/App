...
### [react-native+0.86.0+042+fix-mounting-transaction-merge-order.patch](react-native+0.86.0+042+fix-mounting-transaction-merge-order.patch)

- Reason: Fixes Android Fabric mount crashes on HybridApp (Sentry APP-7AR, `IndexOutOfBoundsException`/`IllegalStateException: addViewAt: failed to insert view into parent`). `FabricUIManagerBinding::schedulerDidFinishTransaction` picks a merge target for an incoming mounting transaction via `std::find_if(pendingTransactions_.begin(), pendingTransactions_.end(), ...)` — a forward search that returns the *oldest* pending transaction for a surface. Once patch `+015`'s `canMergeWith` guard can refuse a merge, a surface can hold more than one pending transaction at once (that's the whole reason the guard exists). With `[T1, T2]` pending, an incoming `T3` was merging into `T1` (`[T1+T3, T2]`, executing as `T1, T3, T2`) even though `T3` was diffed against a tree that already included `T2` — corrupting native-vs-shadow tree order and producing a stale insert index. This patch changes the lookup to `rbegin()/rend()` so a new transaction only ever merges into the *newest* pending entry for its surface. Patch `+015`'s `canMergeWith` guard (`Delete`↔`Create` only) is unchanged.
- Also adds `MountingTransactionMergeOrderTest.cpp` (new file, `ReactCommon/react/renderer/mounting/tests/`), a deterministic native regression test with 3 cases: a baseline guard on the existing `Delete`↔`Create` `canMergeWith` check, an execution-order invariant proving a 3rd incoming transaction merges into the newest of two pending transactions rather than the oldest, and one `DISABLED_` case documenting — not fixing — a separate, unclaimed gap where `canMergeWith` never checks a `Delete`↔`Insert` pairing. That gap is out of scope for this patch; extending `canMergeWith` was not part of the accepted proposal. Like its two siblings (`StateReconciliationTest.cpp`, `ShadowTreeLifeCycleTest.cpp`), this test file has no build/run target in this fork — `mounting/CMakeLists.txt` only globs `*.cpp` at the `mounting/` level, never `tests/*.cpp` — so it ships as a deterministic proof/documentation artifact matching upstream test convention, not a CI gate. Android on-device validation is the proof mechanism for this patch, consistent with this repo's standing rule that unit tests alone are insufficient proof for native-layer bugs.
- Scope note: Sentry APP-H7A (`IllegalStateException: Unable to remove a view from a view that is not a ViewGroup`) was raised on the same issue and shares the same merge-order root cause in theory, but the C+ reviewer's last technical comment on the issue thread explicitly states the tested proposal only confirms APP-7AR and suggested APP-H7A be treated separately; that was never revisited. This patch does not claim to fix APP-H7A. A separate, unrelated `addViewAt` recovery-path asymmetry (`SurfaceMountingManager.kt`, `removeViewAt` re-scans the live parent tree on a stale tag/index, `addViewAt` does not) was also identified during investigation and is intentionally not touched here — different mechanism, no proposal on the issue addresses it, and it's out of scope for this fix.
- Upstream PR/issue: 🛑 — not filed upstream; this is a fork-local fix for interaction with fork-local patch `+015`.
- E/App issue: https://github.com/Expensify/App/issues/98791
- PR introducing patch: 🛑 (fill in once the PR is opened)
- Validation note: **not yet validated on an Android device/emulator.** Per `contributingGuides/HYBRID_APP.md`, building HybridApp requires access to the closed-source `Mobile-Expensify` repository, which this contributor does not have — matching the pattern already in this issue's thread, where the C+ reviewer (not the proposal authors) performed device validation for the accepted proposal. Requesting the same here: reproduce APP-7AR pre-fix, confirm it no longer reproduces post-fix, and separately attempt APP-H7A and report the actual result (not assumed fixed by this patch).
