...
- PR introducing patch: https://github.com/Expensify/App/pull/PR_NUMBER
- 0.86.0 migration note: **this patch can be dropped with the RN 0.87 upgrade.** Upstream commit `45904c8` is not in any 0.86.x release but is contained in `v0.87.0`, so once `react-native` is bumped to >= 0.87.0 the fix ships in the package and this patch must be deleted (it will fail to apply against 0.87.0 regardless). The five patched files are byte-for-byte identical to upstream's pre-fix state in 0.86.0, so the upstream diff applies with zero fuzz; only the `scripts/cxx-api/api-snapshots/*.api` hunks were dropped, since those files are not shipped in the npm package.
