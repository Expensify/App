...
### [react-native+0.86.0+044+fix-merged-transaction-mount-order.patch](react-native+0.86.0+044+fix-merged-transaction-mount-order.patch)
...
- Reason: Fixes the Android Fabric mount-item failures behind `APP-7AR` (`addViewAt: failed to insert view into parent`, underlying `IndexOutOfBoundsException: index=1 count=0`) and `APP-H7A` (`Unable to remove a view from a view that is not a ViewGroup`). Both fire when a mount batch is applied to a native view tree that no longer matches the shadow tree the batch was computed against. `FabricMountingManager::executeMount` writes `Delete` mount items last, and React Native's own comment on that block says the optimization "is incorrect when multiple transactions are merged together" — which is exactly what `FabricUIManagerBinding::schedulerDidFinishTransaction` does on Android when commits outpace UI-thread mounting. Patch `+015` narrowed that hazard with `MountingTransaction::canMergeWith`, but left three holes, each closed by one hunk here:
    ...
    - `schedulerShouldRenderTransactions` swapped the whole queue into a local vector before mounting, justified in-source by "we're already combining all the transactions for the same surface ID in a single transaction ... so operations won't run out of order". Patch `+015` made that premise false. A state update committed synchronously from inside `executeMount` re-enters this method, finds the shared queue empty, and mounts the new transaction ahead of the entries still waiting in the local copy. The method is also called from both the JS thread (scheduled rendering updates) and the UI thread (synchronous mounts, which is the default for non-React commits). Now the queue records the thread mounting it (`pendingTransactionsDrainer_`): a re-entrant call on that same thread returns and lets the outer call mount what it queued, in order, while a call on another thread waits for the drain to finish and then drains itself. Waiting rather than skipping keeps the synchronous contract that `commitMutex_` used to provide - the caller still returns with its transaction mounted - and it cannot deadlock, because the draining thread holds no lock while mounting. The drainer is cleared and waiters are woken even if `executeMount` throws.
