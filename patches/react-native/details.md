...
- Reason: Fixes Fabric reusing shadow nodes that hold a stale font scale, so text does not re-layout correctly after the OS font size changes (text stays at its old measured size until something else forces a re-measure). RN 0.86.0 handles font scale changes in `SurfaceHandler::constraintLayout`: when `layoutContext.fontSizeMultiplier` differs from the previous root's, it walks the tree and dirties every measurable node. That only runs on the commit where the multiplier changes, and it compares against the *root's* value — so a node cloned from a parent that still carries an obsolete `fontSizeMultiplier` (while the root already has the correct one) keeps the stale measurement and is never re-dirtied. This patch backports the upstream rewrite: `fontSizeMultiplier` is stored on `LayoutMetrics` and threaded through `YogaLayoutableShadowNode::configureYogaTree` alongside `pointScaleFactor`, so every commit re-checks each node's own stored multiplier and calls `markDirtyAndPropagate()` on measurable nodes whose value is out of date. The now-redundant `dirtyMeasurableNodes`/`dirtyMeasurableNodesRecursive` helpers are removed from `SurfaceHandler`. Behavior stays behind RN's existing `enableFontScaleChangesUpdatingLayout` feature flag, which defaults to `true` in 0.86.0.
...
- 0.86.0 migration note: **this patch can be dropped with the RN 0.87 upgrade.** Upstream commit `45904c8` is not in any 0.86.x release but is contained in `v0.87.0`, so once `react-native` is bumped to >= 0.87.0 the fix ships in the package and this patch must be deleted (it will fail to apply against 0.87.0 regardless). The patched regions are byte-for-byte identical to upstream's pre-fix state in 0.86.0, so no conflict resolution was needed, but the upstream diff cannot be used verbatim: `YogaLayoutableShadowNode.cpp`/`.h` sit ~10 lines earlier in the released 0.86.0 than on upstream `main`, and `patch-package` matches hunks by position rather than searching for context like GNU `patch` does. The patch was therefore regenerated with `patch-package react-native --append` against a tree with patches 001–036 already applied. The `scripts/cxx-api/api-snapshots/*.api` hunks were dropped, since those files are not shipped in the npm package.
