...
- Reason: In the web floating RHP every stacked card draws its own drop shadow (`getRHPExtendedCardFrame`). `CardA11yWrapper` wraps each card with `overflow: hidden` whenever the card is not the top of the stack (`active === false`, and `active` is `index === routes.length - 1`), and the wrapper is sized to the card. That clips the shadow of a card sitting under the focused one, so opening an expense over an expense report cuts the report card's left/bottom shadow. This adds a `keepCardOverflowVisible` card option, read in `CardStack` and passed down through `CardContainer` to `CardA11yWrapper`, which changes the style to `overflow: active || keepCardOverflowVisible ? undefined : 'hidden'`.
- Upstream PR/issue: https://github.com/react-navigation/react-navigation/issues/13254. We asked upstream for an opt-out. The clip is deliberate (it stops a buried, wider page from expanding the web scroll area), but our floating RHP cards are absolute/fixed inside a fixed frame, so that scroll-area concern does not apply to them. Re-evaluate or drop this patch if upstream lands an escape hatch.
...
- PR Updating Patch: N/A
- Note: The opt-out is scoped to the RHP. It is gated behind the `keepCardOverflowVisible` screen option (declared in `src/libs/Navigation/PlatformStackNavigation/types/NavigationOptions.ts`, set to `true` only in `useRHPScreenOptions`), so only the RHP navigator skips the clip and every other stack keeps react-navigation's default. Re-evaluate on v8.
