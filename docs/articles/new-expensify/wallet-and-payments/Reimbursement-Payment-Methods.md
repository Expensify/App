...
- **US-based business bank account:**  
  ACH reimbursement is used for payments in USD from a US business bank account to a USD personal deposit account. The member's deposit account must be in USD. A deposit account in another currency (for example CAD) cannot receive ACH reimbursement, and the report stays unpaid until the member adds a USD deposit account. [Learn how to set up ACH reimbursement](/articles/new-expensify/wallet-and-payments/Connect-a-Business-Bank-Account).
