---
title: Check supported banks and card issuers in Expensify
description: See which banks and card issuers Expensify supports for connecting company credit cards.
keywords: [check supported banks, supported card issuers, Plaid, connect credit card, bank list, direct feed banks]
internalScope: Audience is any Expensify user checking bank compatibility. Covers checking whether a bank or card issuer is supported, does not cover how to connect a card feed or troubleshoot connection errors.
---

# Check which banks and card issuers are supported in Expensify

Expensify supports most major banks and card issuers worldwide through direct and commercial feeds. New banks are added regularly, so even if your bank isn't listed below, it may still be supported.

This page lists the most commonly connected banks and card issuers. For a complete list of supported institutions, check the Plaid coverage pages linked below.

---

## Who can check supported banks

Any Expensify user can check whether their bank or card issuer is supported. No specific role or plan is required to view this information. To connect a company card feed, you need a workspace on the Collect or Control plan.

---

## How to check if your bank or card issuer is supported in Expensify

1. Review the list of commonly supported banks and card issuers below.
2. If your bank is listed, learn how to set up a [direct feed](/articles/new-expensify/connect-credit-cards/Direct-feeds) or [commercial feed](/articles/new-expensify/connect-credit-cards/Commercial-feeds) to connect it.
3. If your bank is not listed, check the Plaid coverage pages for your region:
   - US/Canada: https://plaid.com/docs/institutions/
   - UK/Europe: https://plaid.com/docs/institutions/europe/
4. If your bank appears on the Plaid coverage pages, you can connect it through Plaid when setting up a direct feed.

---

## What banks and card issuers does Expensify support?

The following banks and card issuers are most commonly connected to Expensify. This is not a complete list.

### Major US Banks
- JPMorgan Chase
- Bank of America
- American Express (Amex)
- Discover
- Wells Fargo
- Citi
- U.S. Bank
- PNC Bank
- Truist
- Capital One
- Fifth Third Bank
- Citizens Bank
- KeyBank
- Huntington Bank
- M&T Bank
- Regions Bank
- Ally Bank
- Comerica
- First Citizens Bank
- Zions Bancorporation
- Western Alliance Bank
- East West Bank
- Synovus
- Webster Bank
- Valley National Bank
- Associated Bank
- First Horizon
- Wintrust Financial
- Cullen/Frost Bankers
- BOK Financial

### Major Canadian Banks
- Royal Bank of Canada (RBC)
- Toronto-Dominion Bank (TD Bank)
- BMO Harris / BMO
- Scotiabank
- CIBC
- National Bank of Canada

### Major European Banks
- BNP Paribas
- HSBC
- Barclays
- Lloyds Banking Group
- Société Générale
- Deutsche Bank
- Santander
- Groupe BPCE
- Crédit Agricole
- ING
- UBS
- Nordea
- SEB
- Intesa Sanpaolo
- Commerzbank
- NatWest Group
- UniCredit
- BBVA
- CaixaBank
- Banco Sabadell
- Crédit Mutuel
- La Banque Postale
- ABN AMRO
- Rabobank
- KBC Group
- Belfius
- Danske Bank
- Svenska Handelsbanken
- Swedbank
- DNB
- Erste Group
- Raiffeisen Bank International
- Bank Austria (UniCredit)
- Credit Suisse (UBS)
- Zürcher Kantonalbank
- PostFinance
- PKO Bank Polski
- Bank Pekao
- Santander Bank Polska
- mBank
- Millennium BCP
- Caixa Geral de Depósitos
- Novobanco
- Allied Irish Banks (AIB)
- Bank of Ireland

### Corporate Card and Spend Management Platforms
- Brex
- Ramp
- Navan (TripActions)
- BILL (Divvy)
- Revolut Business
- Wise Business
- Qonto
- Pleo
- Spendesk
- Soldo
- Airwallex
- N26
- Starling Bank
- Tide
- Monzo Business
- Moss
- Pliant
- Wallester
- Bunq

---

# FAQ

## If my bank isn’t listed, is it still supported in Expensify?

This page shows only the most commonly connected banks. Expensify supports hundreds of additional banks through Plaid. Check the [Plaid coverage page for US/Canada](https://plaid.com/docs/institutions/) or [Plaid coverage page for UK/Europe](https://plaid.com/docs/institutions/europe/) to see if your bank is supported.

## How often are supported banks added to Expensify?

New banks are added regularly through Plaid's network. Even if your bank isn't supported today, it may be added in the future. Check back periodically or contact Concierge for the latest information.

## What if my bank isn't supported by Plaid?

If your bank isn't supported through Plaid, you may still be able to connect it using a [commercial feed](/articles/new-expensify/connect-credit-cards/Commercial-feeds). Commercial feeds work directly with your bank to establish a connection. Contact your bank to confirm whether they support commercial card feeds with Expensify.
