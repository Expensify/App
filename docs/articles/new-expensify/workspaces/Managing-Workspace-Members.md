...
- **Payments Admin** – Can manage workspace payments in Workflows, including the workspace bank account, preferred payer, payment methods, and auto-pay. A Payments Admin can be selected as the workspace payer. They have read-only access to the workspace overview and to the Approvals and Submissions sections of Workflows, and all other workspace editor pages are hidden. (Control workspaces only)
