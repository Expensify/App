...
- **Require company cards for all purchases** - Flag all cash spend, including mileage and per-diem expenses. This toggle is locked until **Company cards** or **Expensify Card** is enabled under **More features**.
...
## Why does the “Company card purchases required” violation stay on a submitted expense?

Once a report is submitted, the violation message stays visible on the expense, but it no longer marks the workspace chat with a red dot, shows a **Fix** button, or adds an item to **To-dos** for the member who submitted it. Unlike other expense rules, this violation has no field for a member to correct and can’t be dismissed, so a member can’t clear it themselves.

Only a Workspace Admin can clear it, by turning off **Require company cards for all purchases** in the **Rules** tab. Approvers and admins still see the violation when they review the report.

## Why can’t I turn on the company card rule?

The **Require company cards for all purchases** toggle is locked until **Company cards** or **Expensify Card** is enabled for the workspace. Enable one of them under **Workspaces > [Workspace Name] > More features**, then return to the **Rules** tab.
