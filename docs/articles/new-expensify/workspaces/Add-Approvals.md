...
**Note:** Some workspaces store each approval workflow separately, so the same approver can forward to different people in different workflows. On those workspaces the warning above doesn't appear, and editing an approver in one workflow leaves the other workflows unchanged. The rest of this answer applies to workspaces that don't have that behavior yet.
