...
| **Change approver** | Submitted reports | Workspace Admin | Adds, bypasses, or reassigns approvers |
...
| **Change approver** | On submitted reports, before the report is final-approved | Admin | **Add approver** inserts an extra approver into the approval workflow at the current point. **Bypass approvers** makes the admin the final approver and skips any remaining approvers, including when the report is already waiting on that admin. **Reassign approver** skips the current approver and assigns a new one |
...
Only workspace admins can bypass approvers. Some settings may prevent self-approval. Being the report's current approver does not hide the option — a workspace admin still sees **Bypass approvers** on a report that is waiting on them.
