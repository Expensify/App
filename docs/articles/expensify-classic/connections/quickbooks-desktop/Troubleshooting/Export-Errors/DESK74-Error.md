...
- A transaction window is open.
- A confirmation or warning message is waiting for input.
- A background pop-up is open but not visible.
- QuickBooks Desktop is not idle.

QuickBooks Desktop must be fully idle before the Web Connector can process a sync or export request.
...
3. Close all pop-up messages or confirmation dialogs.
...
## Can a Hidden Pop-Up Cause This Error?

Yes. Even a background confirmation or warning window can block the Web Connector and trigger the DESK74 error.
