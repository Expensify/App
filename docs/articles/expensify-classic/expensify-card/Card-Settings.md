...
Go to Settings > Domains > Domain Name > Company Cards > Reconciliation > Expenses
Enter your start and end dates, then click Run
The Imported Total will show all Expensify Card transactions for the period
You’ll also see a list of all Expensify Cards, the total spend on each card, and a snapshot of expenses that have and have not been approved (Approved Total and Unapproved Total, respectively)
...
Log into the Expensify web app 
Click Settings > Domains > Domain Name > Company Cards > Reconciliation tab > Settlements
Use the Search function to generate a statement for the specific period you need
...
Review the individual transactions (debits) and the payments (credits) that settled them
Every cardholder will have a virtual and a physical card listed. They’re handled the same way for settlements, reconciliation, and exporting.
Click Download CSV for reconciliation
This will list everything that you see on screen
To reconcile pre-authorizations, you can use the Transaction ID column in the CSV file to locate the original purchase
Review account payments
You’ll see payments made from the accounts listed under Settings > Account > Payments > Bank Accounts. Payment data won’t show for deleted accounts. 
...
Go to Settings > Domains > Domain Name > Company Cards > Settings tab
Use the Expensify Card settlement account dropdown to select a new account
Click Save
...
Monthly settlement is only available if the settlement account hasn’t had a negative balance in the last 90 days
There will be an initial settlement to settle any outstanding spend that happened before switching the settlement frequency 
The date that the settlement is changed to monthly is the settlement date going forward (e.g. If you switch to monthly settlement on September 15th, Expensify Cards will settle on the 15th of each month going forward)
To change the settlement frequency:
Go to Settings > Domains > Domain Name > Company Cards > Settings tab
Click the Settlement Frequency dropdown and select Monthly
...
If a transaction amount exceeds the available limit on your Expensify Card, the transaction will be declined. It’s essential to be aware of the available balance before making a purchase to avoid this - you can see the balance under Settings > Account > Credit Card Import on the web app or mobile app 
Submitting expenses and having them approved will free up your limit for more spend.
Your card hasn’t been activated yet, or has been canceled
If the card has been canceled or not yet activated, it won’t process any transactions. 
Your card information was entered incorrectly. Entering incorrect card information, such as the CVC, ZIP or expiration date will also lead to declines.There was suspicious activity
If Expensify detects unusual or suspicious activity, we may block transactions as a security measure. This could happen due to irregular spending patterns, attempted purchases from risky vendors, or multiple rapid transactions. 
Check your Expensify Home page to approve unsual merchants and try again.
