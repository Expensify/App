 ...
 *
 * Two rules are policed here: `rulesdir/no-onyx-connect` (shipped by eslint-config-expensify) and
 * `rulesdir/no-onyx-get-in-render` (this repo's own, in `eslint-plugin-local-rules/`). Both are
 * normal lint rules, so an inline `eslint-disable` can silence either one. ESLint records such
 * silenced violations as "suppressed messages". This module finds suppressed violations of those
...
const POLICED_RULE_IDS: ReadonlySet<string> = new Set([BANNED_RULE_ID, RENDER_READ_RULE_ID]);
 ...
 *
 * `no-onyx-get-in-render` starts with no allowance at all, which is the point of adding it before
 * any read is converted: there is nothing to grandfather, so every disable of it is new.
 */
...
export {BANNED_RULE_ID, RENDER_READ_RULE_ID, POLICED_RULE_IDS, GRANDFATHERED_BYPASSES, collectSuppressedBans, findNewBypasses};
