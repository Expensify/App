 ...
 *
 * Four rules are policed here: `rulesdir/no-onyx-connect` (shipped by eslint-config-expensify), plus
 * this repo's own `no-onyx-get-in-render`, `no-onyx-read-at-module-scope` and `no-onyx-read-after-write`
 * (in `eslint-plugin-local-rules/`). All are normal lint rules, so an inline `eslint-disable` can
 * silence any of them. ESLint records such silenced violations as "suppressed messages". This module
 * finds suppressed violations of those rules and flags any that go beyond the disables already present
 * on `main`, so a new bypass can be re-elevated to an error at the runner level, where no disable
 * directive can reach it.
 */
...
/** Rule id of the ban on synchronous Onyx reads during render. */
const RENDER_READ_RULE_ID = 'rulesdir/no-onyx-get-in-render';

/** Rule id of the ban on synchronous Onyx reads at module scope, which run before the cache is hydrated. */
const MODULE_SCOPE_READ_RULE_ID = 'rulesdir/no-onyx-read-at-module-scope';

/** Rule id of the ban on a synchronous Onyx read that follows an un-awaited write in the same body. */
const READ_AFTER_WRITE_RULE_ID = 'rulesdir/no-onyx-read-after-write';

/** Every rule whose inline disables are policed. A suppressed violation of anything else is ignored. */
const POLICED_RULE_IDS: ReadonlySet<string> = new Set([BANNED_RULE_ID, RENDER_READ_RULE_ID, MODULE_SCOPE_READ_RULE_ID, READ_AFTER_WRITE_RULE_ID]);
 ...
 *
 * The three read rules start with no allowance at all, which is the point of adding them before any read is
 * converted: there is nothing to grandfather, so every disable of one of them is new.
 */
const GRANDFATHERED_BYPASSES: ReadonlyMap<string, ReadonlyMap<string, number>> = new Map<string, ReadonlyMap<string, number>>([
    ...
    [RENDER_READ_RULE_ID, new Map()],
    [MODULE_SCOPE_READ_RULE_ID, new Map()],
    [READ_AFTER_WRITE_RULE_ID, new Map()],
]);
...
export {BANNED_RULE_ID, RENDER_READ_RULE_ID, MODULE_SCOPE_READ_RULE_ID, READ_AFTER_WRITE_RULE_ID, POLICED_RULE_IDS, GRANDFATHERED_BYPASSES, collectSuppressedBans, findNewBypasses};
