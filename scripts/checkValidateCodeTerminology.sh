...
LANG_HITS=$(grep -rni --include="*.ts" "magic.code\|magic code|magic-code" "$ROOT_DIR/src/languages" 2>/dev/null | filter_comments || true)
CODE_HITS=$(grep -rn  --include="*.ts" --include="*.tsx" "magicCode\|magic-code|MagicCode\|MAGIC_CODE" "$ROOT_DIR/src" "$ROOT_DIR/tests" 2>/dev/null | filter_comments || true)
DOCS_HITS=$(grep -rn --include="*.md" "magic.code\|magic-code|magic code\|magicCode\|MagicCode\|MAGIC_CODE" "$ROOT_DIR" 2>/dev/null | grep -v "node_modules" || true)
