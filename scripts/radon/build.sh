...
# Get the directory where other scripts live (one level up from radon folder)
    ...
    --ios)
        ...
        if [ -n "$APP_PATH" ]; then
            ...
            if [[ "$APP_PATH" == /* ]]; then
                ...
                ABSOLUTE_APP_PATH="$APP_PATH"
            else
                # Relative path - resolve to absolute
                ABSOLUTE_APP_PATH="$(cd "$(dirname "$APP_PATH")" && pwd)/$(basename "$APP_PATH")"
            fi
            echo "$ABSOLUTE_APP_PATH"
        fi
        ;;
    --android)
