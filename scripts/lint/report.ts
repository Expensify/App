...
function renderReport(messages: LintMessage[], options: ReportOptions): {text: string; errorCount: number; warningCount: number} {
    const visible = options.showWarnings ? messages : messages.filter((message) => message.severity >= 2);
    ...
    for (const fileMessages of byFile.values()) {
        blocks.push(fileMessages.map((message) => formatMessage(options.projectRoot, message)).join('\n'));
    }

    const summary = options.showWarnings
