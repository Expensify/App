...
class TranslationGenerator {
   ...
   */
  private static readonly CONTEXT_REGEX =
    /^\s*(?:\/{2}|\*|\/\*)?\s*@context\s+([^\n*/]+)/;
   ...
   */
  private readonly translatedSpanHashToEnglishSpanHash = new Map<
    number,
    number
  >();
  ...
  constructor() {
    this.languagesDir =
      process.env.LANGUAGES_DIR ?? path.join(__dirname, "../src/languages");
    ...
    this.cli = new CLI({
      flags: {
        "dry-run": {
          description:
            "If true, just do local mocked translations rather than making real requests to an AI translator.",
        },
        verbose: {
      ...
      namedArgs: {
        ...
        "compare-ref": {
          ...
          parse: (val: string): string => {
            ...
            if (!Git.isValidRef(val)) {
              throw new Error(
                `Invalid git reference: "${val}". Please provide a valid branch, tag, or commit hash.`,
              );
            }
            return val;
          },
        ...
        "pr-number": {
          description:
            "PR number to get diff from GitHub API (CI only, requires GITHUB_TOKEN). When provided, uses GitHub API for accurate PR-specific diff instead of git diff.",
          default: 0,
          parse: (val: string): number => {
            const parsed = parseInt(val, 10);
            if (Number.isNaN(parsed) || parsed <= 0) {
              throw new Error(
                `Invalid PR number: "${val}". Please provide a valid positive integer.`,
              );
            }
            return parsed;
          },
        ...
        paths: {
          description:
            'Comma-separated list of specific translation paths to retranslate (e.g., "common.save,errors.generic").',
          parse: (val: string): Set<TranslationPaths> => {
            const rawPaths = val
              .split(",")
              .map((translationPath) => translationPath.trim());
            ...
            if (invalidPaths.length > 0) {
              throw new Error(
                `found the following invalid paths: ${JSON.stringify(invalidPaths)}`,
              );
            }
    ...
    this.pathsToModify =
      this.cli.namedArgs.paths ?? new Set<TranslationPaths>();
    this.pathsToRemove = new Set<TranslationPaths>();
    this.verbose = this.cli.flags.verbose;
    this.isIncremental =
      this.pathsToModify.size > 0 || !!this.compareRef || !!this.prNumber;

    if (this.prNumber && !process.env.GITHUB_TOKEN) {
      throw new Error(
        "GITHUB_TOKEN environment variable is required when using --pr-number",
      );
    }
  ...
  public async generateTranslations(): Promise<void> {
    ...
    if (this.verbose) {
      ...
      console.log(
        `   pathsToModify: ${Array.from(this.pathsToModify).join(", ")}`,
      );
      console.log(`   pathsToAdd: ${Array.from(this.pathsToAdd).join(", ")}`);
      console.log(
        `   pathsToRemove: ${Array.from(this.pathsToRemove).join(", ")}`,
      );
    }
    ...
    for (const targetLanguage of this.targetLanguages) {
      await this.generateTranslationsForLocale(
        targetLanguage,
        stringsToTranslate,
        translations.get(targetLanguage) ?? new Map<number, string>(),
      );
    }
  ...
  private printErrorSummary(): void {
    ...
    console.log(
      "(The original text was used as a fallback for these translations)\n",
    );
    ...
    for (const [locale, localeFailures] of byLocale) {
      ...
      for (const failure of localeFailures) {
        const truncatedText =
          failure.text.length > 60
            ? `${failure.text.slice(0, 60)}...`
            : failure.text;
  ...
  private async generateTranslationsForLocale(
    ...
    for (const [key, { text, context, id }] of stringsToTranslate) {
      ...
      const translationPromise = translationPool.add(async () => {
        ...
        let result = await this.translator.translate(
          targetLanguage,
          textToTranslate,
          context,
          id,
    ...
    if (this.isIncremental) {
      ...
      if (!fs.existsSync(targetPath)) {
        throw new Error(
          `Target file ${targetPath} does not exist for incremental translation`,
        );
      }
      ...
      const enResult = ts.transform(this.sourceFile, [
        this.createTranslationTransformer(translationsForLocale),
      ]);
      ...
      const targetTransformer =
        this.createIncrementalTargetTransformer(translatedCodeMap);
      const targetResult = ts.transform(existingSourceFile, [
        targetTransformer,
      ]);
      ...
      const transformer = this.createTranslationTransformer(
    ...
    transformedSourceFile = TSCompilerUtils.addImport(
    ...
    const translatedCode = decodeUnicode(
      tsPrinter.printFile(transformedSourceFile),
    );
   ...
   */
  private async promptForCostApproval(
    stringsToTranslate: Map<number, StringWithContext>,
  ): Promise<void> {
    ...
    const estimatedCost = await this.translator.estimateCost(
      Array.from(stringsToTranslate.values()),
    ...
    if (estimatedCost > COST_CONFIRMATION_THRESHOLD) {
      ...
      if (!this.isIncremental) {
        const scriptPath = path.relative(
          process.cwd(),
          path.resolve(__dirname, "generateTranslations.ts"),
        );
        console.log(
      ...
      const userConfirmed = await this.cli.promptUserConfirmation(
        `Do you want to proceed? ${COLORS.BOLD}Estimated cost: $${estimatedCost.toFixed(2)} USD.${COLORS.RESET} (y/n) `,
      );
  ...
  private findTranslationsNode(
    ...
    const defaultExportIdentifier =
      TSCompilerUtils.extractIdentifierFromExpression(defaultExport);
    const variableDeclaration = TSCompilerUtils.resolveDeclaration(
    ...
    if (
      ...
      throw new Error(
        "Could not find translations object literal in source file",
      );
    }
  ...
  private shouldTranslateNode(
      ...
      (ts.isImportDeclaration(node.parent) ||
        ...
        (ts.isBinaryExpression(node.parent) &&
          ...
          !(
            ...
            TSCompilerUtils.isStringConcatenationChain(node.parent)
          )));
    ...
    const isArgumentToLogFunction =
      ...
      ((ts.isIdentifier(node.parent.expression.expression) &&
        node.parent.expression.expression.getText() === "console") ||
        (ts.isIdentifier(node.parent.expression.expression) &&
          node.parent.expression.expression.getText() === "Log"));
  ...
  private shouldTranslatePath(currentPath: string): boolean {
    ...
    const allPathsToTranslate = new Set([
      ...this.pathsToModify,
      ...this.pathsToAdd,
    ]);
   ...
   */
  private isDedentCallExpression(
    ...
    return (
      ...
      expression.expression.text === "Str" &&
      expression.name.text === "dedent"
    );
  }
  ...
  private isArgumentToDedent(node: ts.Node): boolean {
    ...
    return (
      this.isDedentCallExpression(callExpression.expression) &&
      callExpression.arguments.includes(node as ts.Expression)
    );
  }
  ...
  private isSimpleTemplateExpression(node: ts.TemplateExpression): boolean {
    return node.templateSpans.every((span) =>
      this.isSimpleExpression(span.expression),
    );
  }
   ...
   */
  private lineContainsContextAnnotation(
    lineNumber: number,
    sourceFile: ts.SourceFile,
  ): boolean {
  ...
  private getContextForNode(node: ts.Node): string | undefined {
    ...
    const inlineContext = this.extractContextAnnotationFromString(
      node.getFullText(),
    );
    if (inlineContext) {
    ...
    const nearestPropertyAssignmentAncestor = TSCompilerUtils.findAncestor(
      node,
      (n): n is ts.PropertyAssignment => ts.isPropertyAssignment(n),
    );
    ...
    const commentRanges =
      ts.getLeadingCommentRanges(
        ...
        nearestPropertyAssignmentAncestor.getFullStart(),
      ) ?? [];
    for (const range of commentRanges.reverse()) {
      const commentText = this.sourceFile
        .getFullText()
  ...
  private getTranslationKey(node: ts.Node): number {
    if (
      ...
      throw new Error(
        `Cannot generate translation key for node: ${node.getText()}`,
      );
    }
   ...
   */
  private extractStringsToTranslate(
    node: ts.Node,
    stringsToTranslate: Map<number, StringWithContext>,
    currentPath = "",
  ) {
    if (this.shouldTranslateNode(node)) {
      ...
      // Template expressions must be encoded directly before they can be translated
        ...
        if (this.isSimpleTemplateExpression(node)) {
          ...
          if (this.verbose) {
            console.debug(
              "😵‍💫 Encountered complex template, recursively translating its spans first:",
              node.getText(),
            );
          }
          for (const span of node.templateSpans) {
            this.extractStringsToTranslate(
              span,
              stringsToTranslate,
              currentPath,
            );
   ...
   */
  private stringToTemplateExpression(
    input: string,
    translatedComplexExpressions = new Map<number, ts.Expression>(),
  ): ts.TemplateExpression {
    const regex = /\$\{([^}]*)}/g;
    ...
    for (let i = 0; i < matches.length; i++) {
      ...
      if (/^\d+$/.test(trimmed)) {
        ...
        const translatedExpression =
          translatedComplexExpressions.get(hashed) ??
          translatedComplexExpressions.get(
            this.translatedSpanHashToEnglishSpanHash.get(hashed) ?? hashed,
          );
      ...
      const nextStaticTextEnd =
        i + 1 < matches.length ? matches.at(i + 1)?.index : input.length;
      const staticText = input.slice(nextStaticTextStart, nextStaticTextEnd);

      const literal =
        i === matches.length - 1
          ? ts.factory.createTemplateTail(staticText)
          : ts.factory.createTemplateMiddle(staticText);
  ...
  private async buildPathsFromGitDiff(): Promise<void> {
    try {
      ...
      const relativePath = path.relative(
        process.cwd(),
        path.join(this.languagesDir, "en.ts"),
      );
      ...
      if (this.prNumber) {
        ...
        try {
          ...
          this.diffBase = await GitHubUtils.getPullRequestMergeBaseSHA(
            this.prNumber,
          );
          ...
          const enTsFile = parsedDiff.files.find(
          ...
          if (this.verbose) {
            ...
            console.log(
              `📄 GitHub API diff: ${parsedDiff.files.length} files total, ${enTsMatchCount} matching en.ts`,
            );
          }
          ...
          const isLargeDiff =
            ...
            "status" in apiError &&
            apiError.status === TranslationGenerator.HTTP_STATUS_DIFF_TOO_LARGE;

          if (isLargeDiff) {
            console.warn(
              "⚠️ GitHub API diff too large (>20K lines or >1MB). Falling back to git diff.",
            );
          } else {
            console.warn(
              `⚠️ GitHub API failed: ${TranslationGenerator.getErrorMessage(apiError)}. Falling back to git diff.`,
            );
          }
          ...
          if (this.compareRef) {
            ...
            throw new Error(
              "GitHub API failed and no --compare-ref provided for fallback",
            );
          }
        }
      } else {
        ...
        if (!this.compareRef) {
          throw new Error(
            "--compare-ref is required when --pr-number is not provided for incremental translation",
          );
        }

        this.diffBase = this.compareRef;
        try {
          const mergeBaseResult = execFileSync(
            "git",
            ["merge-base", this.compareRef, "HEAD"],
            { encoding: "utf8" },
          );
          ...
          if (mergeBase && TranslationGenerator.GIT_SHA_REGEX.test(mergeBase)) {
            ...
            if (this.verbose) {
              console.log(
                `🔀 Using merge-base ${mergeBase.slice(0, 8)} (common ancestor of ${this.compareRef} and HEAD)`,
              );
            }
          }
          ...
          if (this.verbose) {
            console.log(
              `⚠️ Could not calculate merge-base (${TranslationGenerator.getErrorMessage(mergeBaseError)}), using ${this.compareRef} directly`,
            );
          }
        }
      ...
      if (this.verbose) {
        console.log(
          `🔍 Found ${changedLines.addedLines.size} added lines, ${changedLines.removedLines.size} removed lines, ${changedLines.modifiedLines.size} modified lines`,
        );
      }

      // Traverse current en.ts for added and modified paths
      this.extractPathsFromChangedLines(
        translationsNode,
        new Set([...changedLines.addedLines, ...changedLines.modifiedLines]),
      ...
      for (const targetLanguage of this.targetLanguages) {
        ...
        if (fs.existsSync(targetPath)) {
          ...
          const existingTranslationsNode =
            this.findTranslationsNode(existingSourceFile);
          for (const pathToCheck of this.pathsToModify) {
            if (
              !TSCompilerUtils.objectHas(existingTranslationsNode, pathToCheck)
            ) {
      ...
      if (this.verbose) {
        console.log(
          `🔄 Paths to modify: ${Array.from(this.pathsToModify).join(", ")}`,
        );
        console.log(
          `➕ Paths to add: ${Array.from(this.pathsToAdd).join(", ")}`,
        );
        console.log(
          `🗑️ Paths to remove: ${Array.from(this.pathsToRemove).join(", ")}`,
        );
      }
    } catch (error: unknown) {
      throw new Error(
        `Error building paths from diff, giving up on incremental translation: ${TranslationGenerator.getErrorMessage(error)}`,
      );
    }
  }
  ...
  private extractPathsFromChangedLines(
    ...
    const isOnAddedLine = nodeLines.some((lineNumber) =>
      addedLines.has(lineNumber),
    );
    const isOnRemovedLine = nodeLines.some((lineNumber) =>
      removedLines.has(lineNumber),
    );
    ...
    if (this.shouldTranslateNode(node)) {
      ...
      if (nearestPropertyAssignmentAncestor) {
        ...
        for (const range of commentRanges) {
          const commentStart = sourceFile.getLineAndCharacterOfPosition(
            range.pos,
          );
          ...
          const commentLines = Array.from(
            { length: commentEnd.line - commentStart.line + 1 },
            (_, i) => commentStart.line + i + 1,
          );
          for (const commentLineNumber of commentLines) {
            if (
              (addedLines.has(commentLineNumber) ||
                removedLines.has(commentLineNumber)) &&
              this.lineContainsContextAnnotation(commentLineNumber, sourceFile)
            ) {
    ...
    if (hasChanges && this.shouldTranslateNode(node)) {
      ...
      const translationsNode = this.findTranslationsNode(
        node.getSourceFile() ?? this.sourceFile,
      );
      const dotPath = TSCompilerUtils.buildDotNotationPath(
        node,
        translationsNode ?? undefined,
      );
      if (dotPath) {
        ...
        if (this.verbose) {
          console.log(
            `🔄 Found changed path: ${dotPath} (added: ${isOnAddedLine}, removed: ${isOnRemovedLine}, contextChange: ${hasContextChange})`,
          );
        }
      }
    }
  ...
  private translateNode(
    ...
    if (ts.isStringLiteral(node)) {
      ...
      return translatedText
        ? ts.factory.createStringLiteral(translatedText)
        : nodeToUse;
    }
    ...
    if (ts.isTemplateExpression(node)) {
      ...
      if (!translatedTemplate) {
        console.warn(
          "⚠️ No translation found for template expression",
          node.getText(),
        );
        return nodeToUse;
      }
      ...
      const transformedTemplateNode =
        transformedNode && ts.isTemplateExpression(transformedNode)
          ? transformedNode
          : node;
      for (let i = 0; i < node.templateSpans.length; i++) {
        const originalExpression = node.templateSpans[i].expression;
        const transformedExpression =
          transformedTemplateNode.templateSpans[i].expression;
      ...
      return this.stringToTemplateExpression(
        translatedTemplate,
        translatedComplexExpressions,
      );
    }
   ...
   */
  private extractTranslatedNodes(
    sourceFile: ts.SourceFile,
    translatedCodeMap: Map<string, string>,
  ): void {
    const visitWithPath = (node: ts.Node, currentPath = "") => {
      ...
      const isModifiedPath = this.pathsToModify.has(
        currentPath as TranslationPaths,
      );

      if ((isAddedPath || isModifiedPath) && ts.isPropertyAssignment(node)) {
        if (!node.initializer) {
          throw new Error(
            "Found a dangling property without an initializer in a translation object. This should never happen.",
          );
        }

        // Extract the value (property initializer) as code string
        const codeString = tsPrinter.printNode(
      ...
      node.forEachChild(
        TSCompilerUtils.createPathAwareVisitor(visitWithPath, currentPath),
      );
    };
   ...
   */
  private async extractRemovedPaths(
    removedLines: Set<number>,
    relativePath: string,
  ): Promise<void> {
    try {
      ...
      if (this.useGitHubAPI) {
        oldEnContent = await GitHubUtils.getFileContents(
          relativePath,
          this.diffBase,
        );
      } else {
   ...
   */
  private createTranslationTransformer(
    translations: Map<number, string>,
  ): ts.TransformerFactory<ts.SourceFile> {
    return TSCompilerUtils.createPathAwareTransformer(
      (node: ts.Node, currentPath = ""): TransformerResult => {
        if (
          this.shouldTranslateNode(node) &&
          this.shouldTranslatePath(currentPath)
        ) {
          return {
            action: TransformerAction.Replace,
            newNode: (transformedChildNode) =>
              this.translateNode(node, translations, transformedChildNode),
          };
        }
  ...
  private formatDedentCallsInFile(filePath: string): void {
    ...
    const result = ts.transform(sourceFile, [
      this.createDedentFormattingTransformer(),
    ]);
      ...
      (sourceFile: ts.SourceFile) => {
        const visit = (node: ts.Node): ts.Node => {
          if (
            ...
            !this.isDedentCallExpression(node.expression) ||
            !node.arguments[0]
          ) {
          ...
          const templateIndentation = TSCompilerUtils.getIndentationOfNode(
            argument,
            sourceFile,
          );
          ...
          const formatFragment = (text: string, isTail: boolean) =>
            this.formatDedentTemplateContent(
              text,
              contentIndentation,
              templateIndentation,
              isTail,
            ...
            return ts.factory.createCallExpression(
              ...
              [
                ts.factory.createNoSubstitutionTemplateLiteral(
                  formatFragment(argument.text, true),
                ),
              ],
            );
          }
          ...
          if (ts.isTemplateExpression(argument)) {
            const newHead = ts.factory.createTemplateHead(
              formatFragment(argument.head.text, false),
            );
            const newSpans = argument.templateSpans.map((span, i) => {
              ...
              const newLiteral = isTail
                ? ts.factory.createTemplateTail(formattedText)
                : ts.factory.createTemplateMiddle(formattedText);
            ...
            return ts.factory.createCallExpression(
              node.expression,
              node.typeArguments,
              [ts.factory.createTemplateExpression(newHead, newSpans)],
            );
          }
   ...
   */
  private formatDedentTemplateContent(
    text: string,
    contentIndentation: number,
    closingIndentation: number,
    isTail: boolean,
  ): string {
    if (!text.includes("\n")) {
      return text;
    ...
    if (
      isTail &&
      (text.endsWith(" ".repeat(closingIndentation)) || !text.endsWith("\n"))
    ) {
   ...
   */
  private createIncrementalTargetTransformer(
    translatedCodeMap: Map<string, string>,
  ): ts.TransformerFactory<ts.SourceFile> {
    let mainTranslationsNode: ts.ObjectLiteralExpression | undefined;
    return TSCompilerUtils.createPathAwareTransformer(
      (node: ts.Node, currentPath: string): TransformerResult => {
        if (!mainTranslationsNode) {
          mainTranslationsNode = this.findTranslationsNode(
            node.getSourceFile(),
          );
        }
        ...
        if (
          currentPath &&
          this.pathsToRemove.has(currentPath as TranslationPaths)
        ) {
        ...
        if (
          ...
          currentPath &&
          translatedCodeMap.has(currentPath)
        ) {
          ...
          const translatedExpression =
            TSCompilerUtils.parseCodeStringToAST(translatedCodeString);
          ...
          return {
            ...
            newNode: (transformedNode) => {
                  ...
                  if (isEmpty) {
                    ...
                    if (this.verbose) {
                      const propName = ts.isIdentifier(prop.name)
                        ...
                        : prop.getText();
                      console.log(
                        `🧹 Removing empty object after incremental update: "${propName}"`,
                      );
                    }
              ...
              if (node === mainTranslationsNode) {
                ...
                for (const [
                  addPath,
                  translatedCodeString,
                ] of translatedCodeMap) {
                  ...
                  const updatedObject = TSCompilerUtils.injectDeepObjectValue(
                    currentObject,
                    addPath,
                    translatedExpression,
                  );
