...
const FIXTURE_NAMES = readdirSync(fixturesDir)
  .filter(
    (name) => name.endsWith(".xml") && !name.startsWith("missing-dimensions"),
  )
  .map((name) => name.replace(/\.xml$/, ""));

const FIXTURE_EXPECTED_SIZES = new Map<
  string,
...
function getLocalCompileTarget(): string {
  ...
  throw new Error(
    `Standalone binary tests are not supported on ${hostPlatform} (${hostArch})`,
  );
}
