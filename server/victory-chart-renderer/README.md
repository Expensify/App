...
This package is an [npm workspace](https://docs.npmjs.com/cli/using-npm/workspaces) child of the App root. Native React Native modules are stubbed at bundle time by [`@server/plugins/rnStubPlugin`](../plugins/rnStubPlugin.ts), which `scripts/dev.ts` and `scripts/build.ts` pass to `Bun.build`. The plugin's `onResolve` hooks redirect `react-native` (and other native-only modules) to the no-op stubs under [`../stubs/`](../stubs/) across the entire bundle graph — including `victory-native`, which is hoisted to the App root and therefore cannot be redirected via workspace `package.json` `overrides`/dependencies. App import aliases (`@components/*`, etc.) and `@server/*` come from [`../tsconfig.json`](../tsconfig.json) (duplicated from the repo root because `paths` does not merge on `extends`). This package’s [`tsconfig.json`](./tsconfig.json) extends that file for ESLint and [Bun path re-mapping](https://bun.com/docs/guides/runtime/tsconfig-paths).
