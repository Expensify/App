...
function MultifactorAuthenticationContextProvider({children}: MultifactorAuthenticationContextProviderProps) {
    ...
    const navigate = (status: MultifactorAuthenticationStatus<MultifactorAuthenticationScenarioStatus>, softPrompt?: boolean, noEligibleMethods?: boolean) => {
        ...
        if (noEligibleMethods) {
            ...
            const notification = getNotificationPath(scenarioLowerCase, 'no-eligible-methods');
            Navigation.navigate(ROUTES.MULTIFACTOR_AUTHENTICATION_NOTIFICATION.getRoute(notification));
            return;
        }
    ...
    const register = (async <T extends MultifactorAuthenticationScenario>(
        ...
        if (!allowedMethods(scenario, softPromptAccepted).biometrics) {
            return setStatus(ContextStatus.createBiometricsNotAllowedStatus<T>(params), scenario, notificationPaths);
        }
    ...
    const proceed = async <T extends MultifactorAuthenticationScenario>(
        ...
        if (!doesDeviceSupportBiometrics()) {
            return setStatus((prevStatus) => prevStatus, scenario, notificationPaths, {
        ...
        if (!NativeBiometrics.setup.isLocalPublicKeyInAuth) {
            ...
            if (!params) {
                return setStatus((prevStatus) => ContextStatus.badRequestStatus(prevStatus), scenario, notificationPaths);
            }
