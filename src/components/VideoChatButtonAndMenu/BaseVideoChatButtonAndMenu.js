...
class BaseVideoChatButtonAndMenu extends Component {
    ...
    render() {
        return (
            <>
                <View ref={(el) => (this.videoChatIconWrapper = el)} onLayout={this.measureVideoChatIconPosition}>
                    <Tooltip text={this.props.translate('videoChatButtonAndMenu.tooltip')}>
                        ...
                        >
                            <Icon src={Expensicons.Phone} fill={this.state.isVideoChatMenuActive ? themeColors.heading : themeColors.icon} />
                        </Pressable>
                    </Tooltip>
                </View>
                <Popover
                ...
                >
                    {_.map(this.menuItemData, ({icon, text, onPress}) => (
                        <MenuItem wrapperStyle={styles.mr3} key={text} icon={icon} title={text} onPress={onPress} />
                    ))}
                </Popover>
            </>
        );
