...
function MoneyRequestConfirmationList(props) {
    ...
    const shouldShowAllFields = props.isDistanceRequest || shouldExpandFields || (!props.isReadOnly && props.isEditingSplitBill);
    const isTypeRequest = props.iouType === CONST.IOU.MONEY_REQUEST_TYPE.REQUEST || props.iouType === CONST.IOU.MONEY_REQUEST_TYPE.SPLIT;
    ...
    const shouldShowAmountInput = useMemo(() => {
        if (props.iouType === CONST.IOU.MONEY_REQUEST_TYPE.SPLIT && !props.isReadOnly) {
            return true;
        }
        ...
        >
            ...
            {shouldShowAmountInput && (
                <MenuItemWithTopDescription
                    ...
                    brickRoadIndicator={TransactionUtils.hasMissingSmartscanFields(transaction) && _.includes(missingSmartScanFields, CONST.EDIT_REQUEST_FIELD.AMOUNT) && 'error'}
            ...
            {(shouldShowAllFields || TransactionUtils.hasMissingSmartscanFields(transaction)) && (
                <>
                    <MenuItemWithTopDescription
                        ...
                        brickRoadIndicator={TransactionUtils.hasMissingSmartscanFields(transaction) && _.includes(missingSmartScanFields, CONST.EDIT_REQUEST_FIELD.DATE) && 'error'}
                    />
                    {props.isDistanceRequest ? (
                        <MenuItemWithTopDescription
                            shouldShowRightIcon={!props.isReadOnly && isTypeRequest}
