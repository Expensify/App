...
import type {
  PlaybackSpeed,
  VideoPopoverMenuActionsContextType,
  VideoPopoverMenuStateContextType,
} from "./types";

const VideoPopoverMenuStateContext =
  React.createContext<VideoPopoverMenuStateContextType | null>(null);
const VideoPopoverMenuActionsContext =
  React.createContext<VideoPopoverMenuActionsContextType | null>(null);
...
function VideoPopoverMenuContextProvider({ children }: ChildrenProps) {
  ...
  const [currentPlaybackSpeed, setCurrentPlaybackSpeed] =
    useState<PlaybackSpeed>(CONST.VIDEO_PLAYER.PLAYBACK_SPEEDS[3]);
  const isLocalFile =
    !!source &&
    CONST.ATTACHMENT_LOCAL_URL_PREFIX.some((prefix) =>
  ...
  const updatePlaybackSpeed = (speed: PlaybackSpeed) => {
    ...
    if (
      !videoPopoverMenuPlayerRef.current ||
  ...
  const downloadAttachment = () => {
    ...
    fileDownload(
      translate,
      addEncryptedAuthTokenToURL(source, encryptedAuthToken),
    );
  };
  ...
  return (
    <VideoPopoverMenuStateContext.Provider value={stateValue}>
      <VideoPopoverMenuActionsContext.Provider value={actionsValue}>
        {children}
      </VideoPopoverMenuActionsContext.Provider>
...
function useVideoPopoverMenuState() {
  ...
  if (!context) {
    throw new Error(
      "useVideoPopoverMenuState must be used within a VideoPopoverMenuContextProvider",
    );
  }
  return context;
}
...
function useVideoPopoverMenuActions() {
  ...
  if (!context) {
    throw new Error(
      "useVideoPopoverMenuActions must be used within a VideoPopoverMenuContextProvider",
    );
  }
  return context;
}
