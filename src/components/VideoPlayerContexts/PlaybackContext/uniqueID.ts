 ...
 * console.log(idGenerator.noReportID);  // "NO_REPORT_ID_1"
 * console.log(idGenerator.noReportID);  // "NO_REPORT_ID_2"
 * ```
 *
 ...
 * console.log(noReportID);  // "NO_REPORT_ID_1"
 * console.log(noReportID);  // "NO_REPORT_ID_1"
 * ```
 *
 * Note:
 ...
 *
 * @returns An object containing a getter `noReportID` that can be used to access unique, sequential IDs.
 */
function uniqueIDForVideoWithoutReport() {
    return {
        get noReportID(): `NO_REPORT_ID_${number}` {
            return `NO_REPORT_ID_${uniqueIDForVideoWithoutReportRef.value++}`;
        },
    };
}

export default uniqueIDForVideoWithoutReport;
