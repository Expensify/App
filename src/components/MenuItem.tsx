...
const findFormatMarkerAhead = (text: string, formatStack: string[]): string | null => {
...
const replaceFormatMarkersWithPlaceholders = (text: string): string =>
  text
...
const replaceFormatPlaceholdersWithMarkers = (text: string): string =>
  text
...
const truncate = (text: string, limit: number, ellipsis: boolean): string => {
    let count = 0;

  const truncateString = (tText:string) => {
    const formatStack = [] as string[];
    let skipCountIncrement = false;
    let outputText = '';
    let index = 0;

    while (count < limit && index < tText.length) {
      const formatMarker = findFormatMarkerAhead(tText.substring(index), formatStack);
      if (formatMarker) {
      ...
      const formatPlaceholder = findFormatPlaceholderAhead(tText.substring(index));
      ...
      const hyperlinkMatch = hyperlinkAheadRegexp.exec(tText.substring(index));
      if (hyperlinkMatch) {
