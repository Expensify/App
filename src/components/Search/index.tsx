...
import openInternalRouteInNewTab, {
  isModifiedMousePress,
} from "@libs/Navigation/helpers/openInternalRouteInNewTab";
...
import {
  cancelSpan,
  endSpanWithAttributes,
  getSpan,
  startSpan,
} from "@libs/telemetry/activeSpans";
import {
...
} from "@libs/telemetry/submitFollowUpAction";
...
type SearchProps = {
  ...
  onDestinationVisible?: (
    wasListEmpty: boolean,
    source: "focus" | "layout",
  ) => void;
};

type HoldMenuCallback = (
...
function Search({
  ...
  const navigation =
    useNavigation<
      PlatformStackNavigationProp<SearchFullscreenNavigatorParamList>
    >();
  ...
  const [nonPersonalAndWorkspaceCards] = useOnyx(
    ONYXKEYS.DERIVED.NON_PERSONAL_AND_WORKSPACE_CARD_LIST,
  );
  ...
  const searchListRef = useRef<SelectionListHandle<SearchListItem> | null>(
    null,
  );

  const savedSearchSelector = useCallback(
    (searches: OnyxEntry<SaveSearch>) => searches?.[hash],
    [hash],
  );
  ...
  const isSearchResultsEmpty =
    !searchResults?.data ||
    isSearchResultsEmptyUtil(searchResults, validGroupBy);
  ...
  const isCardFeedsLoading =
    validGroupBy === CONST.SEARCH.GROUP_BY.CARD &&
    cardFeedsResult?.status === "loading";
  ...
  const isDataLoaded =
    shouldUseLiveData || isSearchDataLoaded(searchResults, queryJSON);

  const hasErrors =
    Object.keys(searchResults?.errors ?? {}).length > 0 && !isOffline;
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      const isPendingSearchNavigation =
        pendingSubmitFollowUpAction?.followUpAction ===
  ...
  const deferredWorkReasonAttributes = useMemo(
    () => ({ context: "Search.DeferredWork" }) as const,
    [],
  );
  const pendingExpenseReasonAttributes = useMemo(
    () => ({ context: "Search.PendingExpensePlaceholder" }) as const,
    [],
  );
  ...
  const isSearchLoadingWithNoResults =
  ...
  const shouldShowLoadingState =
    isDeferringHeavyWork || isWaitingForInitialData;
  const shouldShowRowSkeleton =
    (!skeletonWasDisplayed || shouldShowLoadingState) &&
    showPendingExpensePlaceholder &&
    !hasErrors;
  ...
  const shouldShowLoadingMoreItems =
    ...
    searchResults?.search?.isLoading &&
    searchResults?.search?.offset > 0;

  const loadMoreSkeletonReasonAttributes =
    useMemo<SkeletonSpanReasonAttributes>(
      () => ({
  ...
  useEffect(() => {
    ...
    setShouldShowFiltersBarLoading(
      shouldShowLoadingState && lastSearchType !== type,
    );
  }, [
    lastSearchType,
    setShouldShowFiltersBarLoading,
  ...
  useEffect(() => {
    ...
    const isMigratedModalDisplayed =
      focusedRoute?.name === NAVIGATORS.MIGRATED_USER_MODAL_NAVIGATOR ||
      focusedRoute?.name === SCREENS.MIGRATED_USER_WELCOME_MODAL.DYNAMIC_ROOT;

    const comingBackOnlineWithNoResults =
      prevIsOffline && !isOffline && isEmptyObject(searchResults?.data);
    if (
      !comingBackOnlineWithNoResults &&
      ((!isFocused && !isMigratedModalDisplayed) || isOffline)
    ) {
      return;
    }
    ...
    if (
      hasPendingWriteOnMountRef.current.hasPendingWriteOnMount &&
      hasDeferredWrite(CONST.DEFERRED_LAYOUT_WRITE_KEYS.SEARCH)
    ) {
      return;
    }
    ...
    if (searchResults?.search?.isLoading) {
      if (
        validGroupBy ||
        (shouldCalculateTotals && searchResults?.search?.count === undefined)
      ) {
  ...
  useEffect(() => {
    if (
      !shouldRetrySearchWithTotalsOrGroupedRef.current ||
      searchResults?.search?.isLoading ||
      (!shouldCalculateTotals && !validGroupBy)
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    prevIsSearchResultEmpty,
    clearSelectedTransactions,
  ]);

  const isUnmounted = useRef(false);
  const hasHadFirstLayout = useRef(false);
  const navigateToReportsSpanOnMount = useRef(
    getSpan(CONST.TELEMETRY.SPAN_NAVIGATE_TO_REPORTS),
  );
  ...
  useEffect(
    () => () => {
      ...
      if (
        getSpan(CONST.TELEMETRY.SPAN_NAVIGATE_TO_REPORTS) ===
        navigateToReportsSpanOnMount.current
      ) {
        cancelSpan(CONST.TELEMETRY.SPAN_NAVIGATE_TO_REPORTS);
      }

      cancelNavigateToReportsSpansIfSame(
        newNavigateToReportsSpansOnMount.current,
      );
    },
    [],
  );
  ...
  const totalSelectableItemsCount = useMemo(() => {
    ...
    return (filteredData as TransactionGroupListItemType[]).reduce(
      (count, item) => {
        ...
        const selectableTransactions = item.transactions.filter(
          (transaction) => !isTransactionPendingDelete(transaction),
        );
  ...
  const onSelectRow = useCallback(
    (
      ...
      if (isTransactionItem && !item?.reportAction?.childReportID) {
        ...
        const shouldOpenTransactionThread =
          !isOneTransactionReport(item.report) ||
          item.reportID === CONST.REPORT.UNREPORTED_REPORT_ID;
        const shouldOpenTransactionThreadInNewTab =
          shouldOpenTransactionThread && isModifiedMousePress(event);
        const targetReportID = createAndOpenSearchTransactionThread({
          ...
          shouldNavigate:
            shouldOpenTransactionThread && !shouldOpenTransactionThreadInNewTab,
        });
        if (shouldOpenTransactionThreadInNewTab && targetReportID) {
          openInternalRouteInNewTab(
            ROUTES.SEARCH_REPORT.getRoute({ reportID: targetReportID, backTo }),
            event,
          );
        }
      ...
      if (
        isTransactionGroupListItemType(item) &&
        !isTransactionReportGroupListItemType(item) &&
        item.transactionsQueryJSON
      ) {
        handleSearch({
      ...
      if (
        !isTransactionItem &&
        !isReportActionListItemType(item) &&
        !isTaskListItemType(item) &&
        !isTransactionGroupListItemType(item)
      ) {
        return;
      }
      ...
      if (isTransactionItem && transactionItem?.reportAction?.childReportID) {
        ...
        const isFromOneTransactionReport = isOneTransactionReport(
      ...
      if (isReportActionListItemType(item)) {
        ...
        const isOptimisticCreatedTaskAction =
          reportActionItem.isOptimisticAction ?? false;
        const shouldSkipReportActionID =
          isCreatedTaskReportAction(reportActionItem) &&
          (isOptimisticCreatedTaskAction ||
            reportActionItem.pendingAction ===
              CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD);

        const reportActionID = shouldSkipReportActionID
          ? undefined
          : reportActionItem.reportActionID;
  ...
  const [columnsToShow, setColumnsToShow] =
    useState<SearchColumnType[]>(getEmptyArray);
  ...
  useEffect(() => {
    ...
    opacity.set(
      withTiming(0, { duration: CONST.SEARCH.ANIMATION.FADE_DURATION }, () => {
        setColumnsToShow(currentColumns);
        opacity.set(
          withTiming(1, { duration: CONST.SEARCH.ANIMATION.FADE_DURATION }),
        );
      }),
    );
  }, [
  ...
  const canSelectMultiple =
    !isChat && !isTask && (!isSmallScreenWidth || isMobileSelectionModeEnabled);
  ...
  const fetchMoreResults = useCallback(() => {
    if (
      ...
      offset > allDataLength - CONST.SEARCH.RESULTS_PAGE_SIZE
    ) {
      return;
    }
  ...
  const onLayoutBase = useCallback(() => {
    ...
    endNavigateToReportsFirstPaint(
      CONST.TELEMETRY.NAVIGATE_TO_REPORTS_START_TYPE.WARM_FIRST,
    );
    endNavigateToReportsContentLoad();
  ...
  const cancelNavigationSpans = useCallback(() => {
    ...
    cancelNavigateToReportsSpans();
    if (
      getPendingSubmitFollowUpAction()?.followUpAction ===
      CONST.TELEMETRY.SUBMIT_FOLLOW_UP_ACTION.NAVIGATE_TO_SEARCH
    ) {
      cancelSubmitFollowUpActionSpan();
    }
  ...
  useEffect(() => {
    if (
      !didBailToFallbackState.current ||
      !hasDeferredWrite(CONST.DEFERRED_LAYOUT_WRITE_KEYS.SEARCH)
    ) {
      return;
    }
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      endNavigateToReportsFirstPaint(
        CONST.TELEMETRY.NAVIGATE_TO_REPORTS_START_TYPE.WARM_SUBSEQUENT,
      );
      endNavigateToReportsContentLoad();
    ...
    () =>
      filteredData.some(
        (item) =>
          "reportID" in item &&
          item.reportID &&
          isActionLoadingSet.has(
            `${ONYXKEYS.COLLECTION.RAM_ONLY_REPORT_LOADING_STATE}${item.reportID}`,
          ),
      ),
    [filteredData, isActionLoadingSet],
  );

  const visibleDataLength = useMemo(
    () =>
      filteredData.filter(
        (item) =>
          item.pendingAction !== CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE ||
  ...
  if (shouldShowRowSkeleton) {
    return (
      <SearchRowSkeleton
        ...
        containerStyle={
          shouldUseNarrowLayout
            ? styles.searchListContentContainerStyles(!!hasFilterBars)
            : styles.mt3
        }
  ...
  if (hasErrors) {
    const isInvalidQuery =
      searchRequestResponseStatusCode === CONST.JSON_CODE.INVALID_SEARCH_QUERY;
    cancelNavigationSpans();
    return (
      <View
        style={[
  ...
  if (shouldShowChartView && isGroupedItemArray(sortedData)) {
    ...
    if (savedSearch) {
      ...
      const sortMatches = doesSearchItemMatchSort(
        currentSearchKey,
        suggestedSearch.searchQueryJSON?.sortBy,
        suggestedSearch.searchQueryJSON?.sortOrder,
        sortBy,
        sortOrder,
      );
      if (sortMatches) {
    ...
    return (
      <SearchScopeProvider>
        ...
        >
          ...
          >
            <SearchChartWrapper title={chartTitle} groupBy={validGroupBy}>
              <SearchChartView
                queryJSON={queryJSON}
                view={view}
  ...
  const listFooterComponent =
    shouldShowLoadingMoreItems || showPendingExpensePlaceholder ? (
      <SearchRowSkeleton
        ...
        reasonAttributes={
          showPendingExpensePlaceholder
            ? pendingExpenseReasonAttributes
            : loadMoreSkeletonReasonAttributes
        }
        isLoadMore
      />
    ) : undefined;

  const isFlatExpenseView =
    type === CONST.SEARCH.DATA_TYPES.EXPENSE && !validGroupBy;
  const isExpenseGroupedView =
    type === CONST.SEARCH.DATA_TYPES.EXPENSE && !!validGroupBy;
