...
function mapTransactionItemToSelectedEntry(
    ...
    return [
        item.keyForList,
        {
            ...
            ownerAccountID: item.reportID === CONST.REPORT.UNREPORTED_REPORT_ID ? currentUserAccountID : item.report?.ownerAccountID,
        },
    ];
}
...
function toggleTransactionInList(
    item: TransactionListItemType,
    selectedTransactions: SelectedTransactions,
    currentUserAccountID: number,
    outstandingReportsByPolicyID?: OutstandingReportsByPolicyIDDerivedValue,
    ...
    const [key, selectedInfo] = mapTransactionItemToSelectedEntry(item, currentUserAccountID, outstandingReportsByPolicyID, false);
    return {
...
function Search({
    ...
    useEffect(() => {
            ...
            data.forEach((transactionGroup) => {
                ...
                transactionGroup.transactions.forEach((transactionItem) => {
                    ...
                    newTransactionList[transactionItem.transactionID] = {
                        ...
                        ownerAccountID: transactionItem.reportID === CONST.REPORT.UNREPORTED_REPORT_ID ? accountID : transactionItem.report?.ownerAccountID,
                    };
                });
            });
        } else {
        ...
        // eslint-disable-next-line react-compiler/react-compiler, react-hooks/exhaustive-deps
    }, [data, setSelectedTransactions, areAllMatchingItemsSelected, isFocused, outstandingReportsByPolicyID, accountID]);
    ...
    const toggleTransaction = useCallback(
        (item: SearchListItem, itemTransactions?: TransactionListItemType[]) => {
            ...
            if (isTransactionListItemType(item)) {
                ...
                const updatedTransactions = toggleTransactionInList(item, selectedTransactions, accountID, outstandingReportsByPolicyID);
            ...
            const updatedTransactions = {
                ...
                ...Object.fromEntries(
                    currentTransactions
                        ...
                        .map((transactionItem) => mapTransactionItemToSelectedEntry(transactionItem, accountID, outstandingReportsByPolicyID)),
                ),
            };
            setSelectedTransactions(updatedTransactions, data);
            updateSelectAllMatchingItemsState(updatedTransactions);
        },
        [data, selectedTransactions, outstandingReportsByPolicyID, setSelectedTransactions, updateSelectAllMatchingItemsState, accountID],
    );
