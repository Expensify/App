...
function SearchFiltersChatsSelector({initialReportIDs, onFiltersUpdate, isScreenTransitionEnd}: SearchFiltersParticipantsSelectorProps) {
    ...
    const selectedOptions: OptionData[] = selectedReportIDs.map((id) => {
        ...
        const report = getSelectedOptionData(
            createOptionFromReport({...reportData, reportID: id}, personalDetails, privateIsArchived, reportPolicy, formatPhoneNumber, reportAttributesDerived),
        );
        ...
        const alternateText = getAlternateText(report, {}, {isReportArchived, formatPhoneNumber, policy, reportAttributesDerived, policyTags: reportPolicyTags});
