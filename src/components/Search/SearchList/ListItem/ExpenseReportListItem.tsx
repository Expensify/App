...
function ExpenseReportListItem<TItem extends ListItem>({
    ...
    // `snapshotTransactionIDs` is a fresh `.map()` array each render, so memoize the selector on the
    // IDs' contents — otherwise it gets a new identity every render and re-subscribes endlessly under
    // the store-based engine (the `dependencies` 3rd arg this used to rely on was removed from useOnyx).
    const snapshotTransactionIDsKey = snapshotTransactionIDs.join(',');
    // eslint-disable-next-line react-hooks/exhaustive-deps
    const liveViolationsSelector = useMemo(() => transactionViolationsByIDsSelector(snapshotTransactionIDs), [snapshotTransactionIDsKey]);
