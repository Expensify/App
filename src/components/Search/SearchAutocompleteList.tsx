...
import type {
  ListItem as NewListItem,
  UserListItemProps,
} from "@components/SelectionList/ListItem/types";
import SelectionListWithSections from "@components/SelectionList/SelectionListWithSections";
import type {
  Section,
  SelectionListWithSectionsHandle,
} from "@components/SelectionList/SelectionListWithSections/types";
...
} from "./SearchList/ListItem/SearchQueryListItem";
...
type AutocompleteListItem = NewListItem &
  Partial<Omit<OptionData, keyof NewListItem>> &
  Partial<Omit<SearchQueryItem, keyof NewListItem>>;

type GetAdditionalSectionsCallback = (
  options: Options,
  sectionIndex: number,
) => Array<Section<AutocompleteListItem>> | undefined;
...
function getStableRankKey(option: {
...
function isSearchQueryListItem(
  listItem: UserListItemProps<AutocompleteListItem> | SearchQueryListItemProps,
): listItem is SearchQueryListItemProps {
...
function SearchRouterItem(
  props: UserListItemProps<AutocompleteListItem> | SearchQueryListItemProps,
) {
  ...
  const fsClass = FS.getChatFSClass(
    (item as SearchOption<Report> | undefined)?.item,
  );
...
function SearchAutocompleteList({
  ...
  const styles = useThemeStyles();
  const { translate, localeCompare, formatPhoneNumber, dateFnsLocale } =
    useLocalize();
  ...
  const [recentSearches, recentSearchesMetadata] = useOnyx(
  ...
  const [policies = getEmptyObject<NonNullable<OnyxCollection<Policy>>>()] =
    useOnyx(ONYXKEYS.COLLECTION.POLICY);
  const [visibleReportActionsData] = useOnyx(
    ONYXKEYS.DERIVED.VISIBLE_REPORT_ACTIONS,
  );
  ...
  const hasActiveSearchResults =
    hasEffectiveInputQuery && autocompleteQueryValue.trim() !== "";
  ...
  const isRecentSearchesDataLoaded = !isLoadingOnyxValue(
    recentSearchesMetadata,
  );
  ...
  useEffect(() => {
    ...
    if (parentSpan) {
      parentSpan.setAttribute(
        CONST.TELEMETRY.ATTRIBUTE_COLD_START,
        isLoadingOptions,
      );
  ...
  useEffect(() => {
    ...
    const updateFocus = () => {
      innerListRef.current?.updateExternalTextInputFocus(
  ...
  const autocompleteQueryWithoutFilters = getQueryWithoutFilters(
    autocompleteQueryValue,
  );

  const recentSearchesData = useMemo(() => {
    const sortedRecentSearches = Object.entries(recentSearches ?? {}).sort(
      ([, firstRecentSearch], [, secondRecentSearch]) =>
        localeCompare(
          secondRecentSearch.timestamp,
          firstRecentSearch.timestamp,
        ),
    );

    return sortedRecentSearches
      .slice(0, 5)
      .map(([recentSearchHash, { query }]) => {
  ...
  const recentReportsOptions = useMemo(() => {
    ...
    const orderedOptions = combineOrderingOfReportsAndPersonalDetails(
      searchOptions,
      autocompleteQueryValue,
      {
    ...
    const reportOptions: OptionData[] = [
      ...orderedOptions.recentReports,
      ...orderedOptions.personalDetails,
    ];
  ...
  const [frozenLocalRank, setFrozenLocalRank] =
    useState<ReadonlyMap<string, number>>(EMPTY_RANK_MAP);
  const [prevAutocompleteQuery, setPrevAutocompleteQuery] = useState(
    autocompleteQueryValue,
  );
  ...
  if (prevAutocompleteQuery !== autocompleteQueryValue) {
    ...
    frozenLocalRank.size === 0 &&
    recentReportsOptions.length > 0
  ) {
  ...
  useEffect(() => {
    ...
    autocompleteQueryWithoutFilters,
    debounceHandleSearch,
    handleSearch,
    hasUpstreamDebounce,
  ]);
  ...
  const { sections, styledRecentReports, suggestionsCount } = useMemo(() => {
    ...
    const nextStyledRecentReports = recentReportsOptions.map((option) => {
      const report = getReportOrDraftReport(
        option.reportID,
        undefined,
        undefined,
        undefined,
        reports?.[`${ONYXKEYS.COLLECTION.REPORT}${option.reportID}`],
      );
      ...
      const keyForList =
        option.keyForList ??
        option.reportID ??
        (option.accountID ? String(option.accountID) : undefined);
      return {
        ...option,
        keyForList,
    ...
    if (!hasActiveSearchResults) {
      ...
      localRows.sort(
        (a, b) =>
          (frozenLocalRank.get(getStableRankKey(a) ?? "") ?? 0) -
          (frozenLocalRank.get(getStableRankKey(b) ?? "") ?? 0),
      );
    ...
    if (autocompleteSuggestions.length > 0) {
      const autocompleteData: AutocompleteListItem[] =
        autocompleteSuggestions.map(
          ({ filterKey, text, autocompleteID, mapKey, workspaceIcon }) => {
            return {
  ...
  useEffect(() => {
    if (
      trimmedAutocompleteQueryValue !== "" ||
      isLoadingOptions ||
      recentReportsOptions.length >=
        CONST.AUTO_COMPLETE_SUGGESTER.MAX_AMOUNT_OF_SUGGESTIONS ||
      !hasMoreRecentReports
    ) {
      return;
    }
  ...
  const shouldAnnounceNoResults =
    !isLoading && suggestionsCount === 0 && !!trimmedAutocompleteQueryValue;
  useDebouncedAccessibilityAnnouncement(
    noResultsFoundText,
  ...
  const recentReportKeys = new Set(
    styledRecentReports.map((report) => report.keyForList),
  );
  const {
    ...
    defaultFocusedKey,
    defaultFocusedFlatIndex,
  } = getAutocompleteInitialFocus(sections, recentReportKeys);
  ...
  useEffect(() => {
    ...
    const isDebounceSettled =
      autocompleteQueryValue.trim() === effectiveInputQueryValue.trim();
    if (
      isDebounceSettled &&
      shouldHighlightFirstItem &&
      firstRecentReportFlatIndex !== -1 &&
      shouldHighlight(normalizedReferenceText, autocompleteQueryValue)
    ) {
      lastProgrammaticFocusKeyRef.current = firstRecentReportKey;
      innerListRef.current?.updateAndScrollToFocusedIndex(
        firstRecentReportFlatIndex,
        true,
      );
      return;
  ...
  useEffect(() => {
    if (
      ...
      isLoadingOptions ||
      hasSetInitialFocusRef.current ||
      defaultFocusedFlatIndex === -1
    ) {
      return;
    }
  ...
  useEffect(() => {
    ...
    const currentFocusedKey =
      innerListRef.current?.getFocusedOption?.()?.keyForList;
    const isOnProgrammaticTarget =
      lastProgrammaticFocusKeyRef.current === undefined ||
      currentFocusedKey === lastProgrammaticFocusKeyRef.current;
  ...
  return (
    <SelectionListWithSections<AutocompleteListItem>
      ...
      initiallyFocusedItemKey={
        !shouldUseNarrowLayout ? defaultFocusedKey : undefined
      }
