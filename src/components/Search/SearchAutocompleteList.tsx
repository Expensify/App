...
function SearchAutocompleteList({
    ...
    const {sections, styledRecentReports, suggestionsCount} = useMemo(() => {
        ...
        if (!hasActiveSearchResults) {
            ...
            // Active search: keep locally available rows fixed while server-only rows arrive separately. Every row
            // lands in one section or the other — Auth's list orders the results, it never decides which are shown.
