            ...
            if (isSearchQueryItem(item)) {
                ...
                backHistory(() => {
                    if (item?.reportID) {
                        ...
                        navigateToAndOpenReport(item.login ? [item.login] : [], allPersonalDetails, currentUserAccountID, introSelected, false);
                    }
                });
                onRouterClose();
            }
        },
        [autocompleteSubstitutions, onRouterClose, onSearchQueryChange, policies, reports, submitSearch, textInputValue, currentUserAccountID, introSelected, allPersonalDetails],
    );
