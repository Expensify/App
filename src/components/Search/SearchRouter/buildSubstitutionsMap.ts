...
import type {CardFeeds, CardList, PersonalDetailsList, Policy, PolicyTagLists, Report} from '@src/types/onyx';
...
function buildSubstitutionsMap(
    ...
    const substitutionsMap = searchAutocompleteQueryRanges.reduce((map, range) => {
        ...
        if (filterKey === CONST.SEARCH.SYNTAX_FILTER_KEYS.TAX_RATE) {
            ...
            const displayValue = getFilterDisplayValue(filterKey, filterValue, personalDetails, reports, cardList, cardFeeds, policies, policyTags, currentUserAccountID);
