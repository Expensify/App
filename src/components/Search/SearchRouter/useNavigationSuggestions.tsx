/**
 * Builds the top-level, Spend, and Workspace navigation suggestions shown in the Search Router.
 */
...
function buildWorkspaceNavigationItems({
    ...
    return Object.values(policies ?? {})
        .filter((policy): policy is OnyxTypes.Policy => !!policy?.id && shouldShowPolicy(policy, isOffline, currentUserLogin))
        .flatMap((policy) => {
...
function useNavigationSuggestions(query: string, shouldWatchForApprovals = true): SearchQueryItem[] {
    ...
    return buildNavigationSuggestions(query, [topLevelItems, spendItems, workspaceItems], localeCompare);
}
