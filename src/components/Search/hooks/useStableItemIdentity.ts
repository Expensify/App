 ...
 */
function reconcileItemIdentity<T extends KeyedItem>(previous: T[], next: T[]): T[] {
    ...
    const result = next.map((item, index) => {
        ...
        const resolved = previousItem !== undefined && (previousItem === item || deepEqual(previousItem, item)) ? previousItem : item;
 ...
 * by `keyForList` and compared deeply; an unchanged row gets its previous object back, and an array whose every row is
 ...
 */
function useStableItemIdentity<T extends KeyedItem>(items: T[]): T[] {
    ...
    const reconciledItems = reconcileItemIdentity(stableItems, items);
