...
function ImageTransformer({ canvasWidth, canvasHeight, imageWidth, imageHeight, isActive, onSwipe, onSwipeSuccess, renderImage, renderFallback }) {
...
function Page({ isActive, item, onSwipe, onSwipeSuccess, canvasWidth, canvasHeight }) {
    console.log
...
export default function Pager({ items, initialIndex = 0, itemExtractor, onSwipe = noopWorklet, onSwipeSuccess = () => { } }) {
        ...
        <View style={{ flex: 1 }}>
            <View style={{ flex: 1 }}>
                ...
                >
                    ...
                    >
                        {items
                            .map((item, index) => itemExtractor({ item, index }))
                            .map((item, index) => (
                                <View
                                    key={item.key}
                                    style={{ flex: 1 }}
