...
class DisplayNames extends PureComponent {
    ...
    render() {
        if (!this.props.tooltipEnabled) {
            ...
            return (
                <Text style={this.props.textStyles} numberOfLines={this.props.numberOfLines}>
        ...
        return (
            ...
            <Text style={[...this.props.textStyles, styles.pRelative]} onLayout={this.setContainerLayout} numberOfLines={1} ref={(el) => (this.containerRef = el)}>
                {this.props.shouldUseFullTitle
                    ? this.props.fullTitle
                    : _.map(this.props.displayNamesWithTooltips, ({displayName, tooltip}, index) => (
                          <Fragment key={index}>
                              <Tooltip key={index} text={tooltip} containerStyles={[styles.dInline]} shiftHorizontal={() => this.getTooltipShiftX(index)}>
