...
function useReportActionAvatars({
    ...
    const [policyChatReport] = useOnyx(`${ONYXKEYS.COLLECTION.REPORT}${Number(chatReportIDAnnounce) ? chatReportIDAnnounce?.toString() : chatReportIDAdmins?.toString()}`, {canBeMissing: true});
