...
function SelectionList<TItem extends ListItem>({onScroll, ...props}: BaseSelectionListProps<TItem>, ref: ForwardedRef<SelectionListHandle>) {
    ...
    const touchStart = useCallback(() => setIsScreenTouched(true), []);
    const touchEnd = useCallback(() => setIsScreenTouched(false), []);
    useEffect(() => {
    ...
    const checkShouldDebounceScrolling = useCallback((event: KeyboardEvent) => {
        if (!event) {
            return;
        }
