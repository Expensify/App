...
function DistanceRequest({transactionID, report, transaction, route, isEditingRequest, isEditingNewRequest, onSubmit}) {
    ...
    useEffect(() => {
        isTransactionLoading.current = lodashGet(transaction, 'isLoading', false);
    ...
    useEffect(() => {
        ...
        return () => {
            // If the user cancels out of the modal without without saving changes and transaction isn't loading, then the original transaction
