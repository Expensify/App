...
/**
 * The main props for the useTable hook, which controls the table
 */
...
type TableValue<DataType, ColumnKey extends string = string, FilterKey extends string = string> = {
    /** The title for the table when shown on smaller screens */
    title: string | undefined;

    /** The processed data array after applying sorting, filtering, and searching. */
    processedData: DataType[];

    /** The original unprocessed data array passed to the table. */
    ...
    /** Optional filter configuration for dropdown filters. */
    filterConfig?: FilterConfig<FilterKey>;

    /** The current active filters applied to the table. */
    activeFilters: Record<FilterKey, unknown>;

    /** The current active sorting configuration for the table. */
    activeSorting: ActiveSorting<ColumnKey>;

    /** The current active search string applied to the table. */
    activeSearchString: string;

    /** Methods for controlling the table programmatically (sorting, filtering, searching). */
    tableMethods: TableMethods<ColumnKey, FilterKey>;

    /** Indicates if the table is currently applying any filters. */
    hasActiveFilters: boolean;

    /** Indicates if the table is currently applying a search string. */
    hasSearchString: boolean;

    /** Indicates if the current search or filter results in an empty data set. */
    isEmptyResult: boolean;

    /** Determines if the table should use a narrow layout (e.g., on smaller screens). */
