 ...
 */
type FilterConfig<FilterKey extends string = string> = Record<
  FilterKey,
  FilterConfigEntry
>;
 ...
 */
type IsItemInFilterCallback<DataType extends TableData> = (
  item: DataType,
  values: string[],
) => boolean;
...
type UseFilteringResult<
  DataType extends TableData,
  FilterKey extends string = string,
> = MiddlewareHookResult<DataType, FilteringMethods<FilterKey>> & {
...
>({
  filters,
  isItemInFilter,
}: UseFilteringProps<DataType, FilterKey>): UseFilteringResult<
  DataType,
  FilterKey
> {
  ...
  const updateFilter: FilteringMethods<FilterKey>["updateFilter"] = ({
    key,
    value,
  }) => {
  ...
  const middleware: Middleware<DataType> = (data) =>
    filter({ data, filters, currentFilters, isItemInFilter });
  ...
  const hasActiveFilters = getObjectValues(currentFilters).some(
...
type FilteringMiddlewareParams<
  DataType extends TableData,
  FilterKey extends string = string,
> = {
  data: DataType[];
...
function filter<DataType extends TableData, FilterKey extends string = string>({
  data,
  filters,
  currentFilters,
  isItemInFilter,
}: FilteringMiddlewareParams<DataType, FilterKey>): DataType[] {
  if (!filters) {
