...
function DoubleLineButtonText({
  primaryText,
  primaryTextStyle,
  secondLineText,
}: DoubleLineButtonTextProps) {
...
function ButtonWithDropdownMenu<IValueType>({
  ...
  const defaultPopoverAnchorPosition =
    process.env.NODE_ENV === "test" ? { horizontal: 100, vertical: 100 } : null;
  const [popoverAnchorPosition, setPopoverAnchorPosition] =
    useState<AnchorPosition | null>(defaultPopoverAnchorPosition);
  const dropdownAnchor = useRef<View | null>(null);
  // We need to use isSmallScreenWidth instead of shouldUseNarrowLayout to apply correct popover styles
  // eslint-disable-next-line rulesdir/prefer-shouldUseNarrowLayout-instead-of-isSmallScreenWidth
  const { isSmallScreenWidth } = useResponsiveLayout();
  const dropdownButtonRef = isSplitButton
    ? buttonRef
    : mergeRefs(buttonRef, dropdownAnchor);
  ...
  useEffect(() => {
    ...
    calculatePopoverPosition(dropdownAnchor, anchorAlignment).then(
  ...
  const splitButtonWrapperStyle = isSplitButton
  ...
  const nonSplitButtonStyle = buttonStyle
    ? [styles.w100, buttonStyle]
    : defaultStyle;
      ...
      {shouldAlwaysShowDropdownMenu || options.length > 1 ? (
        <View style={dropdownButtonStyle}>
          <Button
            ...
            onPress={handlePress}
            accessibilityState={
              !isSplitButton ? { expanded: isMenuVisible } : undefined
            }
            ...
            style={
              isSplitButton
                ? [styles.pr0, styles.flexGrow1, styles.flexShrink1]
                : nonSplitButtonStyle
            }
            size={buttonSize}
            innerStyles={[
              innerStyleDropButton,
              !isSplitButton && styles.dropDownButtonCartIconView,
              isTextTooLong &&
                shouldUseShortForm && { ...styles.pl2, ...styles.pr1 },
            ]}
            testID={testID}
          ...
          >
            ...
            {secondLineText ? (
              <DoubleLineButtonText
                ...
                primaryTextStyle={[
                  isTextTooLong && shouldUseShortForm
          ...
          {isSplitButton && (
            <Button
              ...
              size={buttonSize}
              innerStyles={[
                styles.dropDownButtonCartIconContainerPadding,
                innerStyleDropButton,
                isButtonSizeSmall && styles.dropDownButtonCartIcon,
              ]}
              sentryLabel={sentryLabel}
            >
              <View
              ...
              >
                ...
                <View
                  style={[
                    ...
                    isButtonSizeSmall && shouldUseShortForm
                      ? styles.dropDownSmallButtonArrowContain
                      : styles.dropDownMediumButtonArrowContain,
                  ]}
                >
                  <Icon
                    ...
                    additionalStyles={[
                      ...(shouldUseShortForm
                        ? [styles.pRelative, styles.t0]
                        : []),
                      isMenuVisible ? styles.flipUpsideDown : undefined,
                    ]}
        ...
        >
          ...
          {!!singleOptionButtonIcon && (
            <Button.Icon src={singleOptionButtonIcon} />
          )}
          {secondLineText ? (
            <DoubleLineButtonText
              primaryText={selectedItem?.text ?? ""}
              primaryTextStyle={[
                !!singleOptionButtonIcon && styles.textAlignLeft,
                styles.noPaddingBottom,
              ]}
              secondLineText={secondLineText}
            />
          ...
          {shouldShowButtonRightIcon && !!rightOptionIcon && (
            <Button.Icon src={rightOptionIcon} style={styles.ml2} />
          )}
        </Button>
      )}
      {(shouldAlwaysShowDropdownMenu || options.length > 1) &&
        !!popoverAnchorPosition && (
          <PopoverMenu
            ...
            scrollContainerStyle={
              ...
              isSmallScreenWidth && { ...styles.pt4, paddingBottom }
            }
