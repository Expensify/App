...
function commitEndPeek() {
    ...
    isPeeking = false;
    peekProgress.set(withTiming(0, ANIMATION));
    notify();
}
...
function endPeek() {
    ...
    // Small delay before committing end-peek to avoid flicker when moving between items
    setTimeout(commitEndPeek, 80);
}

function useSearchSidebarCollapse() {
    const collapsed = useSyncExternalStore(subscribe, getCollapseSnapshot);
    const peeking = useSyncExternalStore(subscribe, getPeekSnapshot);
...
export {collapseProgress, peekProgress, useSearchSidebarCollapse};
