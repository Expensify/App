...
type PromotedActionsType = Record<BasePromotedActions, (report: OnyxReport) => PromotedAction> & {
    ...
    [CONST.PROMOTED_ACTIONS.MESSAGE]: (params: {reportNameValuePairs: OnyxCollection<ReportNameValuePairs>; reportID?: string; accountID?: number; login?: string}) => PromotedAction;
};
...
const PromotedActions = {
    ...
    message: ({reportNameValuePairs, reportID, accountID, login}) => ({
        ...
        onSelected: () => {
            ...
            if (login) {
                navigateToAndOpenReport([login], reportNameValuePairs, false);
                return;
            }
            if (accountID) {
                navigateToAndOpenReportWithAccountIDs([accountID], reportNameValuePairs);
            }
        },
    }),
