...
type UseFooterDerivedFlagsParams = {
  ...
  iouType: Exclude<
    IOUType,
    typeof CONST.IOU.TYPE.REQUEST | typeof CONST.IOU.TYPE.SEND
  >;
...
function useFooterDerivedFlags({
  ...
  const {
    ...
    shouldSelectPolicy,
    shouldNavigateToUpgradePath: canNavigateToUpgradePath,
  ...
  const isCreatingTrackExpense =
    action === CONST.IOU.ACTION.CREATE && iouType === CONST.IOU.TYPE.TRACK;

  const shouldShowTags =
    (isPolicyExpenseChat || isUnreported || isCreatingTrackExpense) &&
    hasEnabledTags(policyTagLists);
  const shouldShowAttendees = shouldShowAttendeesTransactionUtils(
    iouType,
    policy,
  );
  ...
  const shouldShowDate = shouldShowConfirmationDate(
    shouldShowSmartScanFields,
    isDistanceRequest,
  );
  ...
  const canModifyTaxFields =
    !isReadOnly && !isDistanceRequest && !isPerDiemRequest;
  ...
  const shouldNavigateToUpgradePath =
    !isSubmitWorkspace && canNavigateToUpgradePath;
  const shouldShowTimeRequestFields =
    isTimeRequest && action === CONST.IOU.ACTION.CREATE;
