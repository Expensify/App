...
function MoneyReportHeader({reportID: reportIDProp, shouldDisplayBackButton = false, onBackButtonPress}: MoneyReportHeaderProps) {
    ...
    const dismissModalAndUpdateUseHold = () => {
        ...
        if (requestParentReportAction) {
            changeMoneyRequestHoldStatus(formatPhoneNumber, requestParentReportAction, transaction, isOffline);
        }
    };
                ...
                onPress={() => {
                    ...
                    if (IOUActions.length) {
                        for (const action of IOUActions) {
                            changeMoneyRequestHoldStatus(formatPhoneNumber, action, getLinkedIOUTransaction(action, transactions), isOffline);
                        }
                        return;
                    }
                    ...
                    changeMoneyRequestHoldStatus(formatPhoneNumber, moneyRequestAction, getLinkedIOUTransaction(moneyRequestAction, transactions), isOffline);
                }}
            />
        ),
