...
} from "@components/DelegateNoAccessModalProvider";
...
import type {
  KYCFlowEvent,
  TriggerKYCFlow,
  WorkspacePolicyPaymentOption,
} from "@libs/PaymentUtils";
import { selectPaymentType } from "@libs/PaymentUtils";
import { sortPoliciesByName } from "@libs/PolicyUtils";
import {
  REPORT_MORE_MENU_SECTIONS,
  sortAndSectionPopoverMenuItems,
} from "@libs/PopoverMenuSections";
import {
...
} from "@userActions/IOU/ReportWorkflow";
...
function MoneyReportHeaderSecondaryActionsInner({
  reportID,
  primaryAction,
  isReportInSearch,
  backTo,
  ...
  const {
    ...
    openHoldEducational,
    openRejectModal,
  } = useMoneyReportHeaderModals();

  const { translate, localeCompare } = useLocalize();
  const kycWallRef = useContext(KYCWallContext);

  const [moneyRequestReport] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT}${reportID}`,
  );
  const [policy] = useOnyx(
    `${ONYXKEYS.COLLECTION.POLICY}${getNonEmptyStringOnyxID(moneyRequestReport?.policyID)}`,
  );
  const [chatReport] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT}${getNonEmptyStringOnyxID(moneyRequestReport?.chatReportID)}`,
  );
  const [submitterLogin] = useOnyx(
    ONYXKEYS.PERSONAL_DETAILS_LIST,
    {
      selector: personalDetailsLoginSelector(
  ...
  const [nextStep] = useOnyx(
    `${ONYXKEYS.COLLECTION.NEXT_STEP}${moneyRequestReport?.reportID}`,
  );
  const [reportNameValuePairs] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT_NAME_VALUE_PAIRS}${moneyRequestReport?.reportID}`,
  );
  const [reportMetadata] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT_METADATA}${moneyRequestReport?.reportID}`,
  );
  const [outstandingReportsByPolicyID] = useOnyx(
    ONYXKEYS.DERIVED.OUTSTANDING_REPORTS_BY_POLICY_ID,
  );
  ...
  const [ownerBillingGracePeriodEnd] = useOnyx(
    ONYXKEYS.NVP_PRIVATE_OWNER_BILLING_GRACE_PERIOD_END,
  );
  const [userBillingGracePeriodEnds] = useOnyx(
  ...
  const [allTransactionViolations] = useOnyx(
  ...
  const { transactions: reportTransactions, violations } =
    useTransactionsAndViolationsForReport(moneyRequestReport?.reportID);
  const nonPendingDeleteTransactions = Object.values(reportTransactions).filter(
    (t) => t.pendingAction !== CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE,
  );
  const allTransactions = Object.values(reportTransactions);
  const singleTransaction =
    nonPendingDeleteTransactions.length === 1
      ? nonPendingDeleteTransactions.at(0)
      : undefined;
  const [originalTransaction] = useOnyx(
    `${ONYXKEYS.COLLECTION.TRANSACTION}${getNonEmptyStringOnyxID(singleTransaction?.comment?.originalTransactionID)}`,
  );

  const { reportActions: unfilteredReportActions } = usePaginatedReportActions(
    moneyRequestReport?.reportID,
  );
  const reportActions = getFilteredReportActionsForReportView(
  ...
  const shouldCalculateTotals = useSearchShouldCalculateTotals(
    currentSearchKey,
    currentSearchQueryJSON?.hash,
    true,
  );

  const isInvoiceReport = isInvoiceReportUtil(moneyRequestReport);
  const isAnyTransactionOnHold = hasHeldExpensesReportUtils(allTransactions);
  const existingB2BInvoiceReport = useParticipantsInvoiceReport(
    activePolicyID,
    CONST.REPORT.INVOICE_RECEIVER_TYPE.BUSINESS,
    chatReport?.policyID,
  );
  const getChatReportActions = usePayChatReportActions(
    chatReport,
  ...
  const confirmPayment = ({
    ...
    if (isDelegateAccessRestricted) {
      ...
      payInvoice({
        ...
        betas,
        isSelfTourViewed,
        defaultWorkspaceName: generateDefaultWorkspaceName(
          email ?? "",
          lastWorkspaceNumber,
          translate,
        ),
  ...
  const shouldShowPayButton =
    isPaidAnimationRunning || canIOUBePaid || onlyShowPayElsewhere;
  const hasOnlyPendingTransactions =
    allTransactions.length > 0 &&
    allTransactions.every((t) => isExpensifyCardTransaction(t) && isPending(t));
  ...
  const isApproveDisabled =
    ...
    !isAllowedToApproveExpenseReport(moneyRequestReport);
  ...
  const hasPersonalPaymentOption = paymentButtonOptions.some(
    (opt) => opt.value === CONST.IOU.PAYMENT_TYPE.EXPENSIFY,
  );
  ...
  const workspacePolicyOptions =
    isIOUReportUtil(moneyRequestReport) &&
    hasPersonalPaymentOption &&
    activeAdminPolicies.length &&
  ...
  // Domain hooks
  const { actions: lifecycleActionEntries, confirmApproval } =
    useLifecycleActions({
      reportID,
      startApprovedAnimation,
      startAnimation,
  ...
  const secondaryActionsImplementation: Record<
    string,
    (typeof lifecycleActionEntries)[string]
  > = {
  ...
  const applicableSecondaryActions = sortAndSectionPopoverMenuItems(
    secondaryActions
      ...
      .filter(
        (action) =>
          action?.shouldShow !== false && action?.value !== primaryAction,
      ),
    REPORT_MORE_MENU_SECTIONS,
  );
...
function MoneyReportHeaderSecondaryActionsPlaceholder({
  primaryAction,
}: {
  primaryAction: ValueOf<typeof CONST.REPORT.PRIMARY_ACTIONS> | "";
}) {
  ...
  const { shouldUseNarrowLayout, isMediumScreenWidth, isInLandscapeMode } =
    useResponsiveLayout();
  const icons = useMemoizedLazyExpensifyIcons(["DownArrow"]);
  const shouldTakeRemainingWidth =
    (shouldUseNarrowLayout || isMediumScreenWidth) &&
    !primaryAction &&
    !isInLandscapeMode;
  const wrapperStyle = shouldTakeRemainingWidth
    ? [styles.w100, styles.mnw0]
    : undefined;
  ...
  const innerStyles = [
    StyleUtils.getDropDownButtonHeight(CONST.DROPDOWN_BUTTON_SIZE.MEDIUM),
    styles.dropDownButtonCartIconView,
  ];
  return (
