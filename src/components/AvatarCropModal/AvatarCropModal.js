...
const AvatarCropModal = (props) => {
    ...
    return (
        <Modal onClose={props.onClose} isVisible={props.isVisible} type={CONST.MODAL.MODAL_TYPE.RIGHT_DOCKED} onModalHide={resetState} statusBarTranslucent={false}>
            {props.isSmallScreenWidth && <HeaderGap />}
            <HeaderWithCloseButton title={props.translate('avatarCropModal.title')} onCloseButtonPress={props.onClose} />
                ...
                {!isImageInitialized || !isImageContainerInitialized ? (
                    ...
                    <>
                        ...
                        <View style={[styles.mt5, styles.justifyContentBetween, styles.alignItemsCenter, styles.flexRow, StyleUtils.getWidthStyle(imageContainerSize)]}>
                            ...
                            <Pressable style={[styles.mh5, styles.flex1]} onLayout={initializeSliderContainer} onPressIn={(e) => runOnUI(sliderOnPress)(e.nativeEvent.locationX)}>
                                <Slider sliderValue={translateSlider} onGesture={panSliderGestureEventHandler} />
                            </Pressable>
                            <Button medium icon={Expensicons.Rotate} iconFill={themeColors.inverse} iconStyles={[styles.mr0]} style={[styles.imageCropRotateButton]} onPress={rotateImage} />
                        </View>
                    </>
                )}
            </GestureHandlerRootView>
            <Button success style={[styles.m5]} onPress={cropAndSaveImage} pressOnEnter text={props.translate('common.save')} />
        </Modal>
    );
};
