...
import {getFileName, isLocalFile, splitExtensionFromFileName} from '@libs/fileDownload/FileUtils';
...
function extractAttachments(
    ...
    const htmlParser = new HtmlParser({
        onopentag: (name, attribs) => {
            ...
            if (name === 'video') {
                const source = attribs[CONST.ATTACHMENT_SOURCE_ATTRIBUTE];
                const isLocalSource = isLocalFile(source);
                const finalProcessedSource = !isLocalSource ? tryResolveUrlFromApiRoot(source) : source;
                ...
                attachments.unshift({
                    ...
                    source: finalProcessedSource,
                    isAuthTokenRequired: !isLocalSource ? !!attribs[CONST.ATTACHMENT_SOURCE_ATTRIBUTE] : false,
                    file: {name: fileName},
            ...
            if (name === 'img' && attribs.src) {
                ...
                const source = expensifySource || attribs.src;
                const isLocalSource = isLocalFile(source);
                const finalProcessedSource = !isLocalSource ? tryResolveUrlFromApiRoot(source) : source;
                ...
                attachments.unshift({
                    ...
                    source: finalProcessedSource,
                    previewSource,
                    isAuthTokenRequired: !isLocalSource ? !!expensifySource : false,
