 ...
 */
function parseVictoryAxisNode(
  tnode: TNode,
  typeface: SkTypeface | null,
  rootProcessedResult: ProcessNodeResult | null,
): PartialProcessNodeResult {
  const isHorizontal = rootProcessedResult?.isHorizontal;
  const isDependentAxis =
    "dependentaxis" in tnode.attributes &&
    tnode.attributes.dependentaxis !== "false";
  ...
  const rawTickValues = parseAttributeAsNumberArray(
    tnode.attributes.tickvalues,
  );
  const rawTickFormat = parseAttributeAsStringArray(
    tnode.attributes.tickformat,
  );
  const tickFormat = Array.isArray(rawTickFormat) ? rawTickFormat : undefined;
  const hasExplicitTickValues =
    Array.isArray(rawTickValues) && rawTickValues.length > 0;
  ...
  const formatLabel = (label: string | number) => {
    ...
    if (
      ...
      numericLabel >= 0 &&
      numericLabel < tickFormat.length
    ) {
  ...
  const lineWidth =
    style?.grid?.strokeWidth !== undefined ? Number(style.grid.strokeWidth) : 0;
  const labelColor =
    style?.tickLabels?.fill !== undefined
      ? String(style.tickLabels.fill)
      : undefined;
  const labelOffset =
    style?.tickLabels?.padding !== undefined
      ? Number(style.tickLabels.padding)
      : undefined;
  const fontSize =
    style?.tickLabels?.fontSize !== undefined
      ? Number(style.tickLabels.fontSize)
      : undefined;
