...
import {
  ...
  LABEL_KEY,
  VALUE_KEY,
} from "@components/HTMLEngineProvider/HTMLRenderers/VictoryChartRenderer/constants";
...
function VictoryChartPolar({ explicitSize, headless }: VictoryChartPolarProps) {
  const { tnode, data, labelItems, legendItems, chartContentStyles } =
    useVictoryChartContext();
  const theme = useTheme();
  const timezone = useCurrentTimezone();
  const designWidth = getChartDesignWidth(
    explicitSize,
    chartContentStyles.width,
  );
