...
function VictoryChartCartesian({
  ...
  const resolvedXAxis = xAxis
    ? {
        ...xAxis,
        lineColor:
          typeof xAxis.lineColor === "string"
            ? resolveChartThemeColor(xAxis.lineColor, theme)
            : xAxis.lineColor,
        labelColor:
          typeof xAxis.labelColor === "string"
            ? resolveChartThemeColor(xAxis.labelColor, theme)
            : xAxis.labelColor,
      }
    : xAxis;
  const resolvedYAxis = yAxis?.map((axis) => ({
    ...axis,
    lineColor:
      typeof axis.lineColor === "string"
        ? resolveChartThemeColor(axis.lineColor, theme)
        : axis.lineColor,
    labelColor:
      typeof axis.labelColor === "string"
        ? resolveChartThemeColor(axis.labelColor, theme)
        : axis.labelColor,
  }));
          ...
          <VictoryChartRenderArgsProvider value={renderArgs}>
            {labelItems.map((labelItem) => (
              <VictoryChartLabel
                ...
                {...labelItem}
                barWidth={barSeriesCount === 1 ? barWidth : undefined}
                timezone={timezone}
              />
            ))}
