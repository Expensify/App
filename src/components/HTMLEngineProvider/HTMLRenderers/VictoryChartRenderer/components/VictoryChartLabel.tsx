...
function VictoryChartLabel({
  ...
  const processedLines = displayText.split("\n").reduce(
    (acc, line, index) => {
      ...
      const lineFont =
        typeface && lineFontSize ? Skia.Font(typeface, lineFontSize) : null;
      const { ascent, lineHeight: metricsLineHeight } =
        getSkiaLineMetrics(lineFont);
      const lineWidth =
        lineFont
          ?.getGlyphWidths(lineFont.getGlyphIDs(line))
          .reduce((totalWidth, width) => totalWidth + width, 0) ?? 0;
      const customLineHeight = lineLineHeight
        ? lineLineHeight * (lineFontSize ?? 0)
        : 0;
      const lineX = x;
  ...
  if (angleRad === 0) {
    return processedLines.lines.map(
      ({ lineX, lineY, line, lineFont, lineColor, lineWidth }) => {
        const anchoredX = computeTextAnchorPosition(
          lineX,
          lineWidth,
          textAnchor,
        );
        const anchoredY = computeTextAnchorPosition(
          lineY,
          processedLines.y - y,
          verticalAnchor,
        );
  ...
  const tickX =
    x - (textAnchor === "end" && barWidth !== undefined ? barWidth / 2 : 0);
  const labelY = y;
