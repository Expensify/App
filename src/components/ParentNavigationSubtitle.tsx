...
function ParentNavigationSubtitle({
    ...
    const isReportInRHP = currentRoute.name === SCREENS.RIGHT_MODAL.DYNAMIC_SEARCH_REPORT;
    ...
    const onPress = () => {
        ...
        if (openParentReportInCurrentTab && currentFocusedNavigator?.name === NAVIGATORS.RIGHT_MODAL_NAVIGATOR) {
            ...
            if (lastRoute?.name === SCREENS.RIGHT_MODAL.SEARCH_MONEY_REQUEST_REPORT) {
                Navigation.navigate(createDynamicRoute(DYNAMIC_ROUTES.SEARCH_REPORT.getRoute({reportID: parentReportID, reportActionID: parentReportActionID})));
                return;
            }
