...
function OptionsListContextProvider({children}: OptionsListProviderProps) {
    ...
    const loadOptions = useCallback(() => {
        const optionLists = createOptionList(personalDetails, privateIsArchivedMap, reports, allPolicies, formatPhoneNumber, reportAttributes?.reports);
        setOptions({
            reports: optionLists.reports,
            personalDetails: optionLists.personalDetails,
        });
    }, [personalDetails, privateIsArchivedMap, reports, allPolicies, reportAttributes?.reports, formatPhoneNumber]);
    ...
    useEffect(() => {
        ...
        setOptions((prevOptions) => {
            ...
            for (const reportKey of changedReportKeys) {
                ...
                const {reportOption} = processReport(report, personalDetails, privateIsArchived, policy, formatPhoneNumber, reportAttributes?.reports);
    ...
    }, [changedReportsEntries, personalDetails, reports, allPolicies, reportAttributes?.reports, privateIsArchivedMap, formatPhoneNumber]);
    ...
    useEffect(() => {
        ...
        setOptions((prevOptions) => {
            ...
            for (const [key, reportAction] of changedReportActionsEntries) {
                ...
                const {reportOption} = processReport(reportItem, personalDetails, privateIsArchived, policy, formatPhoneNumber, reportAttributes?.reports);
    ...
    }, [changedReportActions, personalDetails, reports, allPolicies, reportAttributes?.reports, privateIsArchivedMap, formatPhoneNumber]);
    ...
    useEffect(() => {
        ...
        for (const accountID of Object.keys(personalDetails)) {
            ...
            for (const report of Object.values(reports ?? {})) {
                ...
                const policy = allPolicies?.[`${ONYXKEYS.COLLECTION.POLICY}${report.policyID}`];
                const newReportOption = createOptionFromReport(report, personalDetails, privateIsArchived, policy, formatPhoneNumber, reportAttributes?.reports, {
        ...
        const newPersonalDetailsOptions = createOptionList(personalDetails, privateIsArchivedMap, reports, allPolicies, formatPhoneNumber, reportAttributes?.reports).personalDetails;
