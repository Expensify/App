    ...
    const dismissReceiptError = () => {
        ...
        if (transaction?.pendingAction === CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD) {
            if (chatReport?.reportID && getCreationReportErrors(chatReport)) {
                navigateToConciergeChatAndDeleteReport(chatReport.reportID, conciergeReportID, currentUserAccountID, introSelected, isSelfTourViewed, true, true);
                return;
            }
            if (parentReportAction) {
                cleanUpMoneyRequest(
        ...
        if (!isEmptyObject(reportCreationError)) {
            ...
            navigateToConciergeChatAndDeleteReport(report.reportID, conciergeReportID, currentUserAccountID, introSelected, isSelfTourViewed, true, true);
        }
    };
