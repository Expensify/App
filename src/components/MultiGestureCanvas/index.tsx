...
function MultiGestureCanvas({canvasSize, isActive = true, onScaleChanged, children, ...props}: MultiGestureCanvasProps) {
    ...
    const defaultContext: AttachmentCarouselPagerContextType = {
        onTap: () => undefined,
        onSwipe: () => undefined,
        onSwipeSuccess: () => undefined,
        onPinchGestureChange: () => undefined,
        pagerRef: pagerRefFallback,
        shouldPagerScroll: useSharedValue(false),
        isScrolling: useSharedValue(false),
        ...props,
    };
    const {onTap, onSwipe, onSwipeSuccess, pagerRef, shouldPagerScroll, isScrolling, onPinchGestureChange} = attachmentCarouselPagerContext ?? defaultContext;
    ...
    const previousTouch = useSharedValue<{
        x: number;
        y: number;
    } | null>(null);
    ...
    useAnimatedReaction(
        () => ({scale: zoomScale.value, isRunning: pinchGestureRunning.value}),
        ({scale, isRunning}) => {
            const newIsPinchGestureInUse = scale !== 1 || isRunning;
