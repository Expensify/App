    ...
    const panGesture = Gesture.Pan()
        ...
        .onTouchesMove((evt, state) => {
            ...
            if (!shouldDisableTransformationGestures.value && zoomScale.value === 1 && previousTouch.value !== null) {
                const velocityX = Math.abs(evt.allTouches.at(0).x - previousTouch.value.x);
                const velocityY = evt.allTouches.at(0).y - previousTouch.value.y;
            ...
            if (previousTouch.value === null) {
                previousTouch.value = {
                    x: evt.allTouches.at(0).x,
                    y: evt.allTouches.at(0).y,
                };
            }
        })
        .onStart(() => {
