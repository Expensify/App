...
/**
 * Array order === z-order (upsert keeps each entry's slot), so the last entry is topmost. selectTopModal skips
 * popovers because a popover can sit above a modal, so the top entry isn't necessarily the top modal.
 */
