...
function haversineDistance(coordinate1: number[], coordinate2: number[]) {
    ...
    const R = 6371e3;
    const lat1 = (coordinate1.at(0) * Math.PI) / 180;
    const lat2 = (coordinate2.at(0) * Math.PI) / 180;
    const deltaLat = ((coordinate2.at(0) - coordinate1.at(0)) * Math.PI) / 180;
    const deltaLon = ((coordinate2.at(1) - coordinate1.at(1)) * Math.PI) / 180;
