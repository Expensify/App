...
function useConfirmationValidation({
    ...
    const validate = (paymentType?: PaymentMethodType): ValidationResult | null => {
        ...
        // invoice, ...). Block confirmation when the user cleared it. Gating on the same `shouldShowDate && !isReadOnly`
        // condition that renders the inline picker keeps validation and UI in sync, and skips read-only/scan flows
        // where the date is populated server-side.
