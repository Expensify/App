...
function WorkspaceCompanyCardsTableHeaderButtons({
  ...
  const formattedFeedName = feedName
    ? getCustomOrFormattedFeedName(
        translate,
        feedName,
        cardFeeds?.[feedName]?.customFeedName,
      )
    : undefined;
  const isCommercialFeed = isCustomFeed(feedName);
  const companyFeeds = getCompanyFeeds(cardFeeds);
  const currentFeedData = feedName ? companyFeeds?.[feedName] : undefined;
  const [domain] = useOnyx(
    `${ONYXKEYS.COLLECTION.DOMAIN}${currentFeedData?.domainID}`,
  );
  const [countryByIp] = useOnyx(ONYXKEYS.COUNTRY);
  const [allPolicies] = useOnyx(ONYXKEYS.COLLECTION.POLICY);

  const { cardFeedErrors, shouldShowRbrForFeedNameWithDomainID } =
    useCardFeedErrors();
  const feedErrors = cardFeedErrors[feedName];
  const hasOtherFeedWithRBR = Object.keys(companyFeeds ?? {}).some(
    (feed) => feed !== feedName && shouldShowRbrForFeedNameWithDomainID[feed],
  );
  const shouldShowFeedSelectorRBR =
    hasOtherFeedWithRBR || !!feedErrors?.hasWorkspaceErrors;
  const shouldShowBrokenConnectionError = getShouldShowBrokenConnectionError(
    feedName,
    feedErrors,
  );
  ...
  const openBankConnection = () => {
    ...
    const initialStep = institutionId
    ...
    Navigation.setNavigationActionToMicrotaskQueue(() => {
      Navigation.navigate(
        ROUTES.WORKSPACE_COMPANY_CARDS_BROKEN_CARD_FEED_CONNECTION.getRoute(
          policyID ?? String(CONST.DEFAULT_NUMBER_ID),
          feedName,
        ),
      );
    });
  ...
  const secondaryActions = [
    {
      ...
      onSelected: () =>
        Navigation.navigate(
          ROUTES.WORKSPACE_COMPANY_CARDS_SETTINGS.getRoute(
            policyID ?? String(CONST.DEFAULT_NUMBER_ID),
          ),
        ),
  ...
  const firstPart = translate(
    isCommercialFeed
      ? "workspace.companyCards.commercialFeed"
      : "workspace.companyCards.directFeed",
  );
  const domainName = domain?.email
    ? Str.extractEmailDomain(domain.email)
    : undefined;
  const policyName = getLinkedPolicyName(
    allPolicies,
    currentFeedData?.preferredPolicy,
    policyID,
    policy?.name,
  );
  const secondPart = ` (${domainName ?? policyName})`;
  const supportingText = isCsvFeed
  ...
  return (
    <View>
      ...
      >
        {isLoading ? (
          <AccountSwitcherSkeletonView
            ...
            style={[
              shouldShowNarrowLayout
                ? [styles.mb2, styles.mt2]
                : [styles.mb11, styles.mt2],
              styles.mw100,
            ]}
          ...
          <FeedSelector
            onFeedSelect={() =>
              Navigation.navigate(
                ROUTES.WORKSPACE_COMPANY_CARDS_SELECT_FEED.getRoute(
                  policyID ?? String(CONST.DEFAULT_NUMBER_ID),
                ),
              )
            }
          ...
          <View style={[styles.flexRow, styles.gap3]}>
            {!isLoading && canWriteCompanyCards && (
              <ButtonWithDropdownMenu
                ...
                isSplitButton={false}
                wrapperStyle={
                  shouldShowNarrowLayout ? styles.flex1 : styles.flexGrow0
                }
                sentryLabel={
                  CONST.SENTRY_LABEL.WORKSPACE.COMPANY_CARDS.MORE_DROPDOWN
                }
              />
            )}
          </View>
      ...
      {!isLoading &&
        canWriteCompanyCards &&
        shouldShowBrokenConnectionError && (
