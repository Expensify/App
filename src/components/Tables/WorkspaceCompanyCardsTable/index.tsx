...
type CompanyCardsTableColumnKey =
  | "member"
  | "card"
  | "customCardName"
  | "actions";
...
function WorkspaceCompanyCardsTable({
  ...
  const tableRef =
    useRef<
      TableHandle<WorkspaceCompanyCardTableItemData, CompanyCardsTableColumnKey>
    >(null);
  ...
  const isFeedConnectionBroken = feedName
    ? cardFeedErrors[feedName]?.isFeedConnectionBroken
    : false;

  const [countryByIp] = useOnyx(ONYXKEYS.COUNTRY);
  const [customCardNames] = useOnyx(
    ONYXKEYS.NVP_EXPENSIFY_COMPANY_CARDS_CUSTOM_NAMES,
  );
  const [personalDetails, personalDetailsMetadata] = useOnyx(
    ONYXKEYS.PERSONAL_DETAILS_LIST,
  );
  const [sharedCardCustomNames] = useOnyx(
    `${ONYXKEYS.COLLECTION.SHARED_NVP_PRIVATE_DOMAIN_MEMBER}${domainOrWorkspaceAccountID}`,
    { selector: companyCardCustomNamesSelector },
  );

  const hasNoAssignedCard = Object.keys(assignedCards ?? {}).length === 0;
  const areWorkspaceCardFeedsLoading =
    !!workspaceCardFeedsStatus?.[domainOrWorkspaceAccountID]?.isLoading;

  // Synthesize error locally since Onyx discards writes to collection keys with member ID '0'.
  const shouldShowWorkspaceFeedsLoadError =
    domainOrWorkspaceAccountID === CONST.DEFAULT_NUMBER_ID &&
    isPolicyLoaded &&
    !isOffline;
  const workspaceCardFeedsErrors = shouldShowWorkspaceFeedsLoadError
    ? {
        [CONST.COMPANY_CARDS.WORKSPACE_FEEDS_LOAD_ERROR]: translate(
          "workspace.companyCards.error.workspaceFeedsCouldNotBeLoadedMessage",
        ),
      }
  ...
  const [feedErrorKey, feedErrorMessage] =
    Object.entries(workspaceCardFeedsErrors ?? selectedFeedErrors ?? {}).at(
      0,
    ) ?? [];
  ...
  if (feedErrorKey === CONST.COMPANY_CARDS.WORKSPACE_FEEDS_LOAD_ERROR) {
    feedErrorTitle = translate(
      "workspace.companyCards.error.workspaceFeedsCouldNotBeLoadedTitle",
    );
    feedErrorReloadAction = onReloadPage;
  } else if (feedErrorKey === CONST.COMPANY_CARDS.FEED_LOAD_ERROR) {
    feedErrorTitle = translate(
      "workspace.companyCards.error.feedCouldNotBeLoadedTitle",
    );
  ...
  const isLoadingPage =
    ...
    !hasCards &&
    (isLoadingFeed ||
      isLoadingOnyxValue(personalDetailsMetadata) ||
  ...
  const showTableControls =
    showCards && !!selectedFeed && !isLoadingCards && !hasFeedErrors;
  const showTableHeaderButtons =
    (showTableControls ||
      isLoadingPage ||
      isFeedPending ||
      feedErrorKey === CONST.COMPANY_CARDS.FEED_LOAD_ERROR) &&
    !!feedName;
  ...
  const cardsData: WorkspaceCompanyCardTableItemData[] = isLoadingCards
    ...
    : (companyCardEntries ?? [])
        .map(({ cardName, encryptedCardNumber, isAssigned, assignedCard }) => {
          const cardholder = assignedCard?.accountID
            ? personalDetails?.[assignedCard.accountID]
            : undefined;
          ...
          return {
            ...
            customCardName:
              getCompanyCardCustomName(
                assignedCard?.cardID,
                sharedCardCustomNames,
                customCardNames,
              ) ?? getDefaultCardName(cardholder?.displayName ?? ""),
            isCardDeleted:
              assignedCard?.pendingAction ===
              CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE,
            isAssigned,
            assignedCard,
            cardholder,
            errors:
              isFeedConnectionBroken || assignedCard?.pendingFields?.lastScrape
                ? undefined
                : assignedCard?.errors,
  ...
  const keyExtractor = (
    ...
    index: number,
  ) => `${item.cardName}_${index}`;

  const compareItems: CompareItemsCallback<
    WorkspaceCompanyCardTableItemData,
    CompanyCardsTableColumnKey
  > = (a, b, activeSorting) => {
    ...
    const cardNameSortingResult =
      localeCompare(a.cardName, b.cardName) * orderMultiplier;
    ...
    if (activeSorting.columnKey === "member") {
      const aMemberString =
        a.cardholder?.displayName ?? a.cardholder?.login ?? "";
      const bMemberString =
        b.cardholder?.displayName ?? b.cardholder?.login ?? "";
    ...
    if (activeSorting.columnKey === "customCardName") {
      return (
        localeCompare(a.customCardName ?? "", b.customCardName ?? "") *
        orderMultiplier
      );
    }
  ...
  const assignedKeyword = translate(
    "workspace.moreFeatures.companyCards.assignedCards",
  ).toLowerCase();
  const unassignedKeyword = translate(
    "workspace.moreFeatures.companyCards.unassignedCards",
  ).toLowerCase();
  ...
  > = (item, searchString) => {
    ...
    const isAssignedCardMatch =
      assignedKeyword.startsWith(searchLower) && item.isAssigned;
    const isUnassignedCardMatch =
      unassignedKeyword.startsWith(searchLower) && !item.isAssigned;
    ...
    return (
      matchingItems.length > 0 || isAssignedCardMatch || isUnassignedCardMatch
    );
  };

  const isItemInFilter: IsItemInFilterCallback<
    WorkspaceCompanyCardTableItemData
  > = (item, filterValues) => {
  ...
  const cardFeedIcon = (
    <CardFeedIcon
      ...
      iconProps={{
        ...
        width: shouldUseNarrowTableLayout
          ? variables.cardIconWidth
          : variables.cardIconSmallWidth,
        height: shouldUseNarrowTableLayout
          ? variables.cardIconHeight
          : variables.cardIconSmallHeight,
      }}
  ...
  const renderItem = ({
    item,
    index,
  }: ListRenderItemInfo<WorkspaceCompanyCardTableItemData>) => (
        ...
        >
          <View style={[styles.alignItemsCenter]}>
            <BlockingView
              ...
              containerStyle={[
                styles.companyCardsBlockingErrorViewContainer,
                styles.pb4,
              ]}
