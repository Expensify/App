...
type WorkspaceListTableProps = {
  ref?:
    | React.Ref<TableHandle<WorkspaceRowData, WorkspaceTableColumnKey, string>>
    | undefined;
...
export default function WorkspaceListTable({
  ...
  > = (item1, item2, activeSorting) => {
    ...
    if (activeSorting.columnKey === "owner") {
      return (
        orderMultiplier *
        localeCompare(item1.ownerName ?? "", item2.ownerName ?? "")
      );
    }
    ...
    if (activeSorting.columnKey === "type") {
      return (
        orderMultiplier *
        localeCompare(
          getUserFriendlyWorkspaceType(item1.type, translate),
          getUserFriendlyWorkspaceType(item2.type, translate),
        )
      );
    }
  ...
  const isTableItemInSearch: IsItemInSearchCallback<WorkspaceRowData> = (
    item,
    searchValue,
  ) => {
  ...
  const renderTableItem = ({
    item,
    index,
  }: ListRenderItemInfo<WorkspaceRowData>) => {
    return (
      <WorkspaceRow
        item={item}
        rowIndex={index}
