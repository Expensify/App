...
type WorkspaceCategoryTableColumnKey =
  | "name"
  | "glCode"
  | "approver"
  | "enabled"
...
type WorkspaceCategoriesTableProps = {
  ref?:
    | React.Ref<
        TableHandle<
          WorkspaceCategoryTableRowData,
          WorkspaceCategoryTableColumnKey,
          string
        >
      >
    | undefined;
...
export default function WorkspaceCategoriesTable({
  ...
  const categoryTableColumns: Array<
    TableColumn<WorkspaceCategoryTableColumnKey>
  > = [
    {
      key: "name",
  ...
  const compareItems: CompareItemsCallback<WorkspaceCategoryTableRowData> = (
    item1,
    item2,
    activeSorting,
  ) => {
  ...
  const isItemInSearch: IsItemInSearchCallback<
    WorkspaceCategoryTableRowData
  > = (item, searchValue) => {
    const searchLower = searchValue.toLowerCase();
    const results = tokenizedSearch([item], searchLower, (option) => [
      option.name,
      option.glCode ?? "",
    ]);
  ...
  const renderCategoryItem = ({
    item,
    index,
  }: ListRenderItemInfo<WorkspaceCategoryTableRowData>) => (
