...
function WorkspaceExpenseDefaultsTable({
  rulesData,
  selectionEnabled,
  selectedKeys,
  onRowSelectionChange,
  emptyStateContent,
}: WorkspaceExpenseDefaultsTableProps) {
  ...
  const columns: Array<TableColumn<ExpenseDefaultsTableColumnKey>> = [
    ...
    },
    {
      key: "condition",
      label: translate(
        "workspace.rules.expenseDefaultsTable.tableColumnCondition",
      ),
      sortable: true,
    },
    {
  ...
  > = (a, b, activeSorting) => {
    ...
    if (activeSorting.columnKey === "rule") {
      return (
        localeCompare(a.ruleDescription, b.ruleDescription) * orderMultiplier
      );
    }
  ...
  const isItemInSearch: IsItemInSearchCallback<ExpenseDefaultTableItem> = (
    item,
    searchString,
  ) => {
    const matchingItems = tokenizedSearch(
      [item],
      searchString,
      (i) => i.searchTokens,
    );
  ...
  const renderItem = ({
    item,
    index,
  }: ListRenderItemInfo<ExpenseDefaultTableItem>) => (
  ...
  return (
    ...
    >
      ...
      {(!isEmpty || !emptyStateContent) && (
        <>
          <Table.FilterBar
            label={translate("workspace.rules.expenseDefaultsTable.findRule")}
          />
          <Table.Header />
          <Table.Body />
        </>
