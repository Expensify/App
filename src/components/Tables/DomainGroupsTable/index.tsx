...
export default function DomainGroupsTable({ groups }: DomainGroupsTableProps) {
  ...
  const shouldUseNarrowTableLayout =
    shouldUseNarrowLayout || isMediumScreenWidth;

  const domainGroupsTableColumns: Array<
    TableColumn<DomainGroupsTableColumnKey>
  > = [
    {
      key: "name",
  ...
  const compareTableItems: CompareItemsCallback<
    DomainGroupRowData,
    DomainGroupsTableColumnKey
  > = (item1, item2, activeSorting) => {
  ...
  const isTableItemInSearch: IsItemInSearchCallback<DomainGroupRowData> = (
    item,
    searchValue,
  ) => {
    const results = tokenizedSearch([item], searchValue, (option) => [
      option.name,
    ]);
  ...
  const renderTableItem = ({
    item,
    index,
  }: ListRenderItemInfo<DomainGroupRowData>) => (
