...
function WorkspaceSpendRulesTable({
  rulesData,
  selectionEnabled,
  selectedKeys,
  onRowSelectionChange,
  emptyStateContent,
}: WorkspaceSpendRulesTableProps) {
  ...
  const isItemInSearch: IsItemInSearchCallback<SpendRuleTableItem> = (
    item,
    searchString,
  ) => {
  ...
  const renderItem = ({
    item,
    index,
  }: ListRenderItemInfo<SpendRuleTableItem>) => (
  ...
  return (
    ...
    >
      ...
      {!isEmpty && (
        <>
          <Table.FilterBar
            label={translate("workspace.rules.spendRules.findRule")}
          />
          <Table.Header />
          <Table.Body />
        </>
