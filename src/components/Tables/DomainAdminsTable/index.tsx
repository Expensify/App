...
export default function DomainAdminsTable({ admins }: DomainAdminsTableProps) {
  ...
  const domainAdminsTableColumns: Array<
    TableColumn<DomainAdminsTableColumnKey>
  > = [
    {
      key: "admin",
  ...
  const compareTableItems: CompareItemsCallback<DomainAdminRowData> = (
    item1,
    item2,
    activeSorting,
  ) => {
  ...
  const isTableItemInSearch: IsItemInSearchCallback<DomainAdminRowData> = (
    item,
    searchValue,
  ) => {
  ...
  const renderTableItem = ({
    item,
    index,
  }: ListRenderItemInfo<DomainAdminRowData>) => (
