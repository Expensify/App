...
export default function DomainListTable({ domains }: DomainListTableProps) {
  ...
  const compareTableItems: CompareItemsCallback<DomainRowData> = (
    item1,
    item2,
    activeSorting,
  ) => {
  ...
  const isTableItemInSearch: IsItemInSearchCallback<DomainRowData> = (
    item,
    searchValue,
  ) => {
  ...
  const renderTableItem = ({
    item,
    index,
  }: ListRenderItemInfo<DomainRowData>) => {
    return (
      <DomainListTableRow
        item={item}
        rowIndex={index}
