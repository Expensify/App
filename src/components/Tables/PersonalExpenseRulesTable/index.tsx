...
export default function PersonalExpenseRulesTable({
  ...
  const personalExpenseRulesTableColumns: Array<
    TableColumn<PersonalExpenseRulesTableColumnKey>
  > = [
    {
      key: "merchant",
  ...
  const compareItems: CompareItemsCallback<PersonalExpenseRuleRowData> = (
    item1,
    item2,
    activeSorting,
  ) => {
  ...
  const isItemInSearch: IsItemInSearchCallback<PersonalExpenseRuleRowData> = (
    item,
    searchValue,
  ) => {
  ...
  const renderPersonalExpenseRuleItem = ({
    item,
    index,
  }: ListRenderItemInfo<PersonalExpenseRuleRowData>) => (
