...
type WorkspaceTagTableColumnKey =
  | "name"
  | "glCode"
  | "approver"
  | "tagCount"
...
export default function WorkspaceTagsTable({
  ...
  const shouldShowTagCountColumn =
    isMultiLevelTags && !shouldUseNarrowTableLayout;
  ...
  const compareItems: CompareItemsCallback<WorkspaceTagTableRowData> = (
    item1,
    item2,
    activeSorting,
  ) => {
    ...
    if (hasDependentTags) {
      return (
        ((item1.orderWeight ?? 0) - (item2.orderWeight ?? 0)) * orderMultiplier
      );
    }
  ...
  const isItemInSearch: IsItemInSearchCallback<WorkspaceTagTableRowData> = (
    item,
    searchValue,
  ) => {
  ...
  const renderTagItem = ({
    item,
    index,
  }: ListRenderItemInfo<WorkspaceTagTableRowData>) => (
