 ...
 */
function DomainMembersGroupFilterSync({
  shouldShowGroupFilter,
  groupOptionValuesKey,
}: {
  shouldShowGroupFilter: boolean;
  groupOptionValuesKey: string;
}) {
  const { activeFilters, tableMethods } = useTableContext<
    DomainMemberRowData,
    DomainMembersTableColumnKey
  >();
  const groupFilterValue = activeFilters.group;

  useEffect(() => {
    const activeGroupFilter = groupFilterValue.at(0);
    const groupOptionValues = groupOptionValuesKey
      ? groupOptionValuesKey.split(",")
      : [];
...
export default function DomainMembersTable({
  ...
  const groupOptionValuesKey =
    filterConfig?.group.options.map((option) => option.value).join(",") ?? "";
  const isEmpty = members.length === 0;

  const domainMembersTableColumns: Array<
    TableColumn<DomainMembersTableColumnKey>
  > = [
    {
      key: "member",
  ...
  const compareTableItems: CompareItemsCallback<DomainMemberRowData> = (
    item1,
    item2,
    activeSorting,
  ) => {
  ...
  const isTableItemInSearch: IsItemInSearchCallback<DomainMemberRowData> = (
    item,
    searchValue,
  ) => {
  ...
  const renderTableItem = ({
    item,
    index,
  }: ListRenderItemInfo<DomainMemberRowData>) => (
