...
type WorkspaceMembersTableColumnKey =
  | "member"
  | "role"
  | "actions"
  | "customField1"
...
type WorkspaceMembersTableProps = {
  ref?:
    | React.Ref<
        TableHandle<
          WorkspaceMemberRowData,
          WorkspaceMembersTableColumnKey,
          string
        >
      >
    | undefined;
...
export default function WorkspaceMembersTable({
  ...
  const workspaceMembersColumns: Array<
    TableColumn<WorkspaceMembersTableColumnKey>
  > = [
    {
      key: "member",
  ...
  > = (item1, item2, activeSorting) => {
    ...
    const memberNameComparison =
      localeCompare(item1.name, item2.name) * orderMultiplier;
    ...
    if (activeSorting.columnKey === "role") {
      ...
      const roleComparison = localeCompare(
        translate("workspace.common.roleName", item1.role),
        translate("workspace.common.roleName", item2.role),
      );
    ...
    if (activeSorting.columnKey === "customField1") {
      ...
      const employeeIdComparison = localeCompare(
        item1CustomField1Value,
        item2CustomField1Value,
      );
    ...
    if (activeSorting.columnKey === "customField2") {
      ...
      const payrollIdComparison = localeCompare(
        item1CustomField2Value,
        item2CustomField2Value,
      );
  ...
  const isTableItemInSearch: IsItemInSearchCallback<WorkspaceMemberRowData> = (
    item,
    searchValue,
  ) => {
  ...
  const isItemInFilter: IsItemInFilterCallback<WorkspaceMemberRowData> = (
    item,
    filterValues,
  ) => {
    ...
    const isAdmin =
      ...
      item.role === CONST.POLICY.ROLE.OWNER;
    if (
      filterValues.includes(WORKSPACE_MEMBER_FILTER_VALUES.ADMINS) &&
      isAdmin
    ) {
      return true;
    }
    ...
    if (
      filterValues.includes(WORKSPACE_MEMBER_FILTER_VALUES.APPROVERS) &&
      isApprover
    ) {
      return true;
    }
    ...
    if (
      filterValues.includes(WORKSPACE_MEMBER_FILTER_VALUES.AUDITORS) &&
      isAuditor
    ) {
      return true;
    }
    ...
    if (
      filterValues.includes(WORKSPACE_MEMBER_FILTER_VALUES.CARD_ADMINS) &&
      isCardAdmin
    ) {
      return true;
    }
    ...
    if (
      filterValues.includes(WORKSPACE_MEMBER_FILTER_VALUES.PEOPLE_ADMINS) &&
      isPeopleAdmin
    ) {
      return true;
    }
    ...
    if (
      filterValues.includes(WORKSPACE_MEMBER_FILTER_VALUES.EDITORS) &&
      isEditor
    ) {
      return true;
    }
    ...
    if (
      filterValues.includes(WORKSPACE_MEMBER_FILTER_VALUES.MEMBERS) &&
      isMember
    ) {
      return true;
    }
  ...
  const renderTableItem = ({
    item,
    index,
  }: ListRenderItemInfo<WorkspaceMemberRowData>) => {
    return (
