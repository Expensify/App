...
} from "./chartFontAssets";

const EMPTY_CHART_FONTS: ChartFontsValue = {
  typefaces: Object.fromEntries(
    (Object.keys(CHART_SKIA_TYPEFACE_ASSETS) as ChartSkiaTypefaceKey[]).map(
      (key) => [key, null],
    ),
...
function loadTypefaceFromAsset(
  source: DataModule | string,
...
function loadChartSkiaTypefaces(): Promise<ChartDefaultTypeface> {
  const typefaceKeys = Object.keys(
    CHART_SKIA_TYPEFACE_ASSETS,
  ) as ChartSkiaTypefaceKey[];

  return Promise.all(
    typefaceKeys.map(async (typefaceKey) => {
      const typeface = await loadTypefaceFromAsset(
        CHART_SKIA_TYPEFACE_ASSETS[typefaceKey],
      );
...
function buildChartFontsValue(
  typefaces: ChartDefaultTypeface,
): Promise<ChartFontsValue> {
  const fontMgr = buildSkiaFontManager(typefaces);

  return Promise.all(
    Object.entries(CHART_FONT_MGR_SUPPLEMENTAL_ASSETS).map(
      async ([familyName, asset]) => {
