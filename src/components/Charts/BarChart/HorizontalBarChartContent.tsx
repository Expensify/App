...
import BAR_INNER_PADDING, {
  MAX_HORIZONTAL_CHART_HEIGHT,
  MIN_BAR_ROW_HEIGHT,
} from "@components/Charts/barChartConstants";
...
function HorizontalBarChartContent({
  ...
  const measurements = useChartLabelMeasurements(
    data,
    fontManager,
  ...
  const checkIsOverHorizontalBar = (args: HitTestArgs) => {
    ...
    return (
      args.cursorX >= barLeft &&
      args.cursorX <= barRight &&
      args.cursorY >= barTop &&
      args.cursorY <= barBottom
    );
  };
  ...
  const handleChartBoundsChange = (bounds: ChartBounds) => {
    ...
    const calculatedBarHeight =
      ((1 - BAR_INNER_PADDING) * domainHeight) / data.length;
  ...
  const truncatedCategoryLabels = truncateCategoryLabels(
    originalLabels,
    measurements.labelWidths,
    measurements.ellipsisWidth,
  );
  const categoryLabelWidth = getCategoryLabelWidth(
  ...
  const renderOutside = (
    ...
    const valueLabelWidths = valueLabels.map((label) =>
      measureTextWidth(label, fontManager, variables.iconSizeExtraSmall),
    );
    ...
    return (
      <>
        ...
        <ChartXAxisLabels
          ...
          ellipsisWidth={measureTextWidth(
            "...",
            fontManager,
            variables.iconSizeExtraSmall,
          )}
  ...
  const valueLabelWidth = getYAxisLabelWidth(
    data,
    formatValue,
    fontManager,
    variables.iconSizeExtraSmall,
    VALUE_AXIS_DOMAIN_PADDING,
  );
  const { ascent, descent } = fontManager
    ? getFontLineMetrics(fontManager, variables.iconSizeExtraSmall)
    : { ascent: 0, descent: 0 };
  const xAxisLabelHeight = ascent + descent + VictoryTheme.axis.labelGap;

  const contentHeight = Math.max(
    CHART_CONTENT_MIN_HEIGHT,
    data.length * MIN_BAR_ROW_HEIGHT +
      VictoryTheme.axis.padding.top +
      VictoryTheme.axis.padding.bottom +
      xAxisLabelHeight,
  );
  ...
  if (isLoading || !fontManager) {
    ...
    return (
      <View style={styles.chartActivityIndicator}>
        <ActivityIndicator size="large" reasonAttributes={reasonAttributes} />
      </View>
    );
  }
