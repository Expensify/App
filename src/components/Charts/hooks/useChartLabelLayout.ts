...
type LabelLayoutConfig = {
    ...
    /** When true, prefer a horizontal bar chart over 90° rotated labels if 45° still won't fit. */
    fallbackToHorizontalBar?: boolean;
};
...
function useChartLabelLayout({
    ...
    // Pick rotation (prefer 0° → 45° → 90°, or horizontal bar chart when enabled)
    let rotation: LabelRotation = LABEL_ROTATIONS.VERTICAL;
    let shouldUseHorizontalBarChart = false;
    ...
    if (hFitsInTicks && hEdgeFits) {
        ...
        } else if (fallbackToHorizontalBar) {
            shouldUseHorizontalBarChart = true;
        }
    }
