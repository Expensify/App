...
function useChartInteractions({
  ...
  const getNearestIndex = (cursorX: number, cursorY: number) => {
    ...
    return nearestPointAxis === "y"
      ? findClosestPointAnyOrder(positions, coordinate)
      : findClosestPoint(positions, coordinate);
  };
  ...
  const isCursorOverTarget = useDerivedValue(() => {
    ...
    return (
      isCursorOverLabel?.(
        { cursorX, cursorY, targetX, targetY, chartBottom: currentChartBottom },
   ...
   */
  const isTooltipActive = useDerivedValue(
    () => isCursorOverTarget.get() && chartInteractionState.isActive.get(),
  );
    ...
    Gesture.Hover()
      .onBegin((e) => {
        ...
        if (nearestPointAxis === "y") {
          ...
          const touchX =
            e.y >= bottom && resolveLabelTouchX
              ? resolveLabelTouchX(e.x, e.y)
              : e.x;
          const ox = pointOX.get();
  ...
  const tapGesture = () =>
    Gesture.Tap().onEnd((e) => {
      ...
      const idx =
        nearestPointAxis === "y"
          ? findClosestPointAnyOrder(oy, e.y)
          : findClosestPoint(ox, e.x);
      if (idx < 0) {
        return;
      }
...
export {
  ...
  findClosestPoint,
  findClosestPointAnyOrder,
  getRightTooltipAnchorX,
  TOOLTIP_BAR_GAP,
};
