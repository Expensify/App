...
class BaseOptionsList extends Component {
     ...
     *
     *                         For example, given a list with two sections, two items in each section, no header, no footer, and no section headers,
     *                         the flat array might look something like this:
     *
    ...
    getItemLayout(data, index) {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            // To correct it, we need to account for the section headers of all previous sections, and the list header.
            ...
            if (correctedIndexOffset === index) {
                // This means that the item we're looking for a section header
                length = sectionHeaderHeight;
            }

            offset += sectionHeaderHeight;
            offset += (Math.min(correctedIndexOffset + section.data.length, index) - correctedIndexOffset) * optionHeight;
        }
