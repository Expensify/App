...
${amount} para ${merchant} - ${date}`,
    ...
    expenseRulesPage: {
        ...
        changes: {
            updateBillable: (value: boolean) => `Actualiza el gasto a ${value ? 'facturable' : 'no facturable'}`,
            updateCategory: (value: string) => `Actualiza la categoría a "${value}"`,
            updateComment: (value: string) => `Cambia la descripción a "${value}"`,
            updateMerchant: (value: string) => `Actualiza el comercio a "${value}"`,
            updateReimbursable: (value: boolean) => `Actualiza el gasto a ${value ? 'reembolsable' : 'no reembolsable'}`,
            updateTag: (value: string) => `Actualiza la etiqueta a "${value}"`,
            updateTax: (value: string) => `Actualiza la tasa de impuesto a ${value}`,
