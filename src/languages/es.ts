...
const translations = {
    ...
    newRoomPage: {
        ...
        renamedRoomAction: ({oldName, newName, isExpenseReport, actorName}: RenamedRoomActionParams) =>
            isExpenseReport
                ? `${actorName ? `${actorName} ` : ''}cambió el nombre a "${newName}" (previamente "${oldName}")`
                : `${actorName ? `${actorName} ` : ''}cambió el nombre de la sala a "${newName}" (previamente "${oldName}")`,
