...
你可以要求报销包含收据和说明等详细信息，设置限额和默认值，并将审批和付款流程自动化——全部在一个地方完成。`,
        ...
        rules: {
            ...
            merchantRules: {
                ...
                confirmErrorCondition: '请选择商家或类别',
                confirmErrorConditionAndDefault: '选择商家或类别，并至少应用一个默认设置',
                oneConditionPerRuleTitle: '每条规则只能有一个条件',
                alreadyMatchesMerchantPrompt: '此规则已基于商家匹配。重置规则以改为基于类别匹配。',
                alreadyMatchesCategoryPrompt: '此规则已匹配到一个类别。请重置规则以改为匹配商家。',
                turnOnTaxesFirstTitle: '请先开启税费',
                turnOnTaxesFirstPrompt: '类别规则会设定默认税率。请在工作区设置中启用税务功能以使用它们。',
                onlyTaxForCategoryRulesTitle: '类别规则中仅可使用税费',
                onlyTaxForCategoryRulesPrompt: '类别规则可以设置默认税率。若要设置其他默认值，请改为匹配商家。',
                clearIncompatibleDefaultsTitle: '清除其他默认设置？',
                clearIncompatibleDefaultsPrompt: '这些字段与类别不兼容。要清除所有字段吗？',
                clearFields: '清空字段',
...
添加更多消费规则以保护公司现金流。`,
            ...
            newRule: {
                ...
                createAgentRuleDescription: '创建可在需要时运行的灵活规则。',
            },
