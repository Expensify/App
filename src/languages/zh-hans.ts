...
在一个地方即可要求报销明细（如收据和描述）、设置限额和默认值，并自动化审批和付款流程。`,
        ...
        rules: {
            ...
            merchantRules: {
                ...
                addRule: '添加商家规则',
                ruleSummaryTitleContains: (merchantName: string) => `如果商户包含“${merchantName}”`,
                ruleSummaryTitleExact: (merchantName: string) => \`如果商户恰好是"\${merchantName}"\`,
                matchType: '匹配类型',
                matchTypeContains: '包含',
                matchTypeContainsDescription: '匹配商户名称包含指定文本的费用（不区分大小写）',
                matchTypeExact: '完全匹配',
                matchTypeExactDescription: '匹配商户名称与指定文本完全相同的费用',
