...
（例如：${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER}）`,
    ...
    spreadsheet: {
        ...
        fieldNotMapped: (fieldName: string) => `哎呀！一个必填字段（“${fieldName}”）尚未映射。请检查后重试。`,
        singleFieldMultipleColumns: (fieldName: string) => `糟糕！你已将单个字段（“${fieldName}”）映射到多个列。请检查后重试。`,
        ...
        importPerDiemRatesSuccessfulDescription: (rates: number) => (rates > 1 ? `已添加 ${rates} 条每日补贴标准。` : '已添加 1 条每日补贴标准。'),
    ...
    iou: {
        ...
        movedUnreportedTransaction: ({reportUrl}: MovedTransactionParams) => `已将此费用从你的<a href="${reportUrl}">个人空间</a>中移出`,
