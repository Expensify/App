...
（例如：${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER}）`,
    ...
    iou: {
        ...
        conciergeAutoSelectedDistanceRate: ({rate, policyName}: {rate: string; policyName: string}) => `新工作区（${policyName}）的里程费率已更新为 ${rate}`,
        conciergeAutoSelectedDistanceRateForMovedReport: ({rate, policyName}: {rate: string; policyName: string}) => `已为新报销单的工作区（${policyName}）将里程报销费率更新为 ${rate}`,
    },
    transactionMerge: {
        listPage: {
            header: '合并费用',
