...
${amount}，商户：${merchant} - 日期：${date}`,
    ...
    statusPage: {
        ...
        vacationDelegate: {
            notAMemberAdminsWillBeAsked: (email: string) => `<strong>${email}</strong> 不是以下工作区的成员。系统会请求你未管理的工作区的管理员将其添加进来。`,
            notAMemberInviteThemNow: (email: string) => `<strong>${email}</strong> 不是以下工作区的成员。你要现在邀请他们吗？`,
            notAMemberMixed: (email: string) => `<strong>${email}</strong> 不是以下工作区的成员。系统会请求你未管理的工作区的管理员将其添加进来。你要现在邀请他们加入你管理的工作区吗？`,
