    ...
    workspace: {
        ...
        rules: {
            ...
            spendRules: {
                ...
                summaryMerchants: ({merchants, count, action}: {merchants: string; count: number; action: ValueOf<typeof CONST.SPEND_RULES.ACTION>}) => `${action === CONST.SPEND_RULES.ACTION.BLOCK ? 'Blocked' : 'Allowed'} ${count > 1 ? 'merchants' : 'merchant'}: ${merchants}, +${count} more`,
                summaryCategories: ({categories, count, action}: {categories: string; count: number; action: ValueOf<typeof CONST.SPEND_RULES.ACTION>}) => `${action === CONST.SPEND_RULES.ACTION.BLOCK ? 'Blocked' : 'Allowed'} ${count > 1 ? 'categories' : 'category'}: ${categories}, +${count} more`,
