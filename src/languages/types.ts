 ...
 */
type TranslationLeafValue<TStringOrFunction> = TStringOrFunction extends (
  ...args: infer Args
) => any
...
type TranslationDeepObject<TTranslations = any> = {
  [Path in keyof TTranslations]: TTranslations[Path] extends
    | string
    | ((...args: any[]) => any)
    ? TranslationLeafValue<TTranslations[Path]>
    : TTranslations[Path] extends
          | number
          | boolean
          | null
          | undefined
 ...
 */
type TranslationParameters<TKey extends TranslationPaths> =
  FlatTranslationsObject[TKey] extends (...args: infer Args) => infer Return
