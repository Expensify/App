...
領収書や説明などの経費詳細を必須にしたり、上限やデフォルトを設定したり、承認や支払いを自動化したりと、すべてを 1 か所で管理できます。`,
        ...
        rules: {
            ...
            merchantRules: {
                ...
                addRule: '店舗ルールを追加',
                ruleSummaryTitleContains: (merchantName: string) => `もし取引先に「${merchantName}」が含まれている場合`,
                ruleSummaryTitleExact: (merchantName: string) => `もし取引先が「${merchantName}」と完全に一致する場合`,
                ruleSummarySubtitleMerchant: (merchantName: string) => `支払先名を「${merchantName}」に変更`,
                matchType: 'マッチタイプ',
                matchTypeContains: '含む',
                matchTypeContainsDescription: '取引先名に指定したテキストが含まれる経費とマッチ（大文字小文字を区別しない）',
                matchTypeExact: '完全一致',
                matchTypeExactDescription: '取引先名が指定したテキストと完全に一致する経費とマッチ',
