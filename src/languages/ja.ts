...
会社のキャッシュフローを守るために、支出ルールをさらに追加しましょう。`,
    ...
    workspaceActions: {
        ...
        changedItemizedReceiptRequiredAmount: (oldValue: string, newValue: string) => `明細付き領収書の必須金額を「${newValue}」に変更しました（以前は「${oldValue}」）`,
        removedItemizedReceiptRequiredAmount: (oldValue: string) => `明細付き領収書の必須金額を削除しました（以前の値：「${oldValue}」）`,
