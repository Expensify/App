    ...
    initialSettingsPage: {
        ...
        troubleshoot: {
            ...
            betaOverridesDescription:
                'オーバーライドはこの端末にのみ適用され、フロントエンドのチェックにのみ影響します。切り替えるとベータはその値に固定され、リセットするまで維持されます。「すべてのオーバーライドをリセット」はアカウントの値に戻します。一部のベータはバックエンドでも制御されているため、リクエストレベルで失敗する場合があります。',
