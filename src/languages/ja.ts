...
（例：${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER}）`,
    ...
    passwordPage: {
        ...
        newPasswordPrompt: '新しいパスワードは、以前のパスワードと異なるものにし、8文字以上で、1つの大文字、1つの小文字、1つの数字を含める必要があります。',
    },
    twoFactorAuth: {
        headerTitle: '二要素認証',
...
${merchant} への ${amount}（${date}）`,
    ...
    expenseRulesPage: {
        ...
        changes: {
            ...
            report: (value: string) => `「${value}」という名前のレポートを追加`,
...
${
    ...
    setPasswordPage: {
        ...
        newPasswordPrompt: 'パスワードには、8文字以上で、大文字1文字、小文字1文字、数字1文字を含める必要があります。',
        passwordFormTitle: 'New Expensify へお帰りなさい！パスワードを設定してください。',
        passwordNotSet: '新しいパスワードを設定できませんでした。再試行用の新しいパスワードリンクを送信しました。',
        setPasswordLinkInvalid: 'このパスワード設定リンクは無効か、有効期限が切れています。新しいリンクがお使いのメール受信箱に届いています！',
        ...
        moreFeatures: {
            ...
            companyCards: {
                ...
                changeTransactionStartDateWarning:
                    '開始日を変更すると、未報告／下書きレポートの取引はすべて削除され、新しい開始日からすべての取引を再インポートします。これにより、取引が重複する可能性があります。',
                updateCard: 'カードを更新',
