...
const translations = {
    ...
    onboarding: {
        ...
        tasks: {
            testDriveAdminTask: {
                title: ({testDriveURL}) => `[試乗する](${testDriveURL})`,
                description: ({testDriveURL}) => `[Expensifyが経費処理を最速で行う理由を知るために、プロダクトツアーをお試しください](${testDriveURL})。`,
            },
            testDriveEmployeeTask: {
                title: ({testDriveURL}) => `[試乗する](${testDriveURL})`,
                description: ({testDriveURL}) => `[試乗](${testDriveURL})して、チームに*Expensifyを3か月無料*でお試しください！`,
            },
            createTestDriveAdminWorkspaceTask: {
                title: ({workspaceConfirmationLink}) => `[ワークスペースを作成](${workspaceConfirmationLink})`,
                description: 'セットアップスペシャリストの助けを借りて、ワークスペースを作成し、設定を構成しましょう！',
            },
            createWorkspaceTask: {
            ...
            setupCategoriesTask: {
                title: ({workspaceCategoriesLink}) => `[カテゴリ](${workspaceCategoriesLink})を設定する`,
            ...
            addAccountingIntegrationTask: {
                ...
                description: ({integrationName, workspaceAccountingLink}) =>
                    `Connect ${integrationName === CONST.ONBOARDING_ACCOUNTING_MAPPING.other ? 'あなたの' : 'に'} ${integrationName} for automatic expense coding and syncing that makes month-end close a breeze.\n` +
                    '\n' +
                    '1. Click *Settings*.\n' +
                    '2. Go to *Workspaces*.\n' +
                    '3. Select your workspace.\n' +
                    ...
                    `${
                        integrationName && CONST.connectionsVideoPaths[integrationName]
                            ? `[会計に連れて行って](${workspaceAccountingLink})。

![${integrationName}に接続](${CONST.CLOUDFRONT_URL}/${CONST.connectionsVideoPaths[integrationName]})`
                            : `[会計に連れて行ってください](${workspaceAccountingLink}).`
                    }`,
            },
            connectCorporateCardTask: {
                title: ({corporateCardLink}) => `[あなたの法人カード](${corporateCardLink})を接続`,
        ...
        testDrive: {
            name: ({testDriveURL}: {testDriveURL?: string}) => (testDriveURL ? `[試乗する](${testDriveURL})` : '試してみる'),
            embeddedDemoIframeTitle: '試乗',
            employeeFakeReceipt: {
                description: '私の試乗レシート！',
            },
        },
        messages: {
            onboardingEmployerOrSubmitMessage: '返金を受け取るのはメッセージを送るのと同じくらい簡単です。基本を確認しましょう。',
            ...
            onboardingManageTeamMessage: ({hasIntroSelected}: {hasIntroSelected: boolean}) =>
                hasIntroSelected
                    ? '# 無料トライアルが始まりました！セットアップを始めましょう。\n👋 こんにちは、私はあなたのExpensifyセットアップスペシャリストです。ワークスペースを作成したので、以下のステップに従って30日間の無料トライアルを最大限に活用しましょう！'
                    : '# 無料トライアルが開始されました！設定を始めましょう。\n👋 こんにちは、私はあなたのExpensify設定スペシャリストです。チームの領収書と経費を管理するためのワークスペースをすでに作成しました。30日間の無料トライアルを最大限に活用するために、以下の残りの設定手順に従ってください！',
            onboardingTrackWorkspaceMessage:
                '# セットアップを始めましょう\n👋 お手伝いします！まず、個人事業主や類似のビジネス向けにワークスペース設定を調整しました。以下のリンクをクリックしてワークスペースを調整できます！\n\n数クリックで支出を追跡する方法は次の通りです。',
            onboardingChatSplitMessage: '友達と請求書を分割するのは、メッセージを送るのと同じくらい簡単です。方法はこちら。',
            onboardingAdminMessage: '管理者としてチームのワークスペースを管理し、自分の経費を提出する方法を学びましょう。',
            onboardingLookingAroundMessage:
                'Expensifyは経費、旅行、企業カード管理で最もよく知られていますが、それ以上のことを行っています。興味のあることを教えていただければ、始めるお手伝いをします。',
            onboardingTestDriveReceiverMessage: '*3ヶ月無料です！以下から始めましょう。*',
        },
        workspace: {
