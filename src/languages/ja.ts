...
（例：${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER}）`,
    ...
    multifactorAuthentication: {
        ...
        revoke: {
            title: '顔／指紋認証とパスキー',
            explanation:
                '1 台以上のデバイスで、顔認証／指紋認証またはパスキーによる認証が有効になっています。アクセスを取り消すと、そのデバイスで次回認証する際にマジックコードが必要になります。',
            confirmationPrompt: 'よろしいですか？そのデバイスで次回の認証を行うには、マジックコードが必要になります。',
            cta: 'アクセスを取り消す',
            noDevices: '顔／指紋またはパスキー認証用に登録されているデバイスがありません。デバイスを登録すると、ここからそのアクセスを取り消せるようになります。',
            dismiss: '了解しました',
            error: 'リクエストに失敗しました。後でもう一度お試しください。',
            revoke: '取り消す',
            confirmationPromptAll: '本当によろしいですか？今後どのデバイスでも次回の認証にはマジックコードが必要になります。',
            ctaAll: 'すべて取り消す',
            thisDevice: 'このデバイス',
