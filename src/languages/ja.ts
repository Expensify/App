...
${merchant} への ${amount}（${date}）`,
    ...
    expenseRulesPage: {
        ...
        changes: {
            updateBillable: (value: boolean) => `経費 ${value ? '請求対象' : '請求不可'} を更新`,
            updateCategory: (value: string) => `カテゴリを「${value}」に更新`,
            updateComment: (value: string) => `説明を「${value}」に変更`,
            updateMerchant: (value: string) => `支払先を「${value}」に更新`,
            updateReimbursable: (value: boolean) => `経費 ${value ? '精算対象' : '非精算'} を更新`,
            updateTag: (value: string) => `タグを「${value}」に更新`,
            updateTax: (value: string) => `税率を${value}に更新`,
