...
(p. ex. ${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER})`,
    ...
    iou: {
        ...
        markedAsReimbursable: 'a marqué la dépense comme « remboursable »',
        markedAsNonReimbursable: 'a marqué la dépense comme « non remboursable »',
        markedAsBillable: 'a marqué la dépense comme « facturable »',
        markedAsNonBillable: 'a marqué la dépense comme « non facturable »',
        updatedFieldTo: ({key, value, first}: {key: string; value: string | boolean; first: boolean}) => `${first ? 'définir le' : ''}${key} vers "${value}"`,
        viaWorkspaceRules: ({route}: {route: string}) => `via les <a href="${route}">règles de l’espace de travail</a>`,
    },
    transactionMerge: {
        listPage: {
