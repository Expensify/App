...
Aggiungi altre regole di spesa per proteggere il flusso di cassa aziendale.`,
    ...
    workspaceActions: {
        ...
        updatedRequiresCategory: ({enabled}: {enabled: boolean}) => `${enabled ? 'abilitato' : 'disattivato'} il requisito di categorizzazione della spesa`,
        updatedRequiresTag: ({enabled}: {enabled: boolean}) => `${enabled ? 'abilitato' : 'disattivato'} il requisito di etichettatura delle spese`,
    },
    roomMembersPage: {
