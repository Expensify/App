...
(bijv. ${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER})`,
    ...
    iou: {
        ...
        markedAsReimbursable: 'heeft de uitgave gemarkeerd als „terugbetaalbaar”',
        markedAsNonReimbursable: 'heeft de uitgave gemarkeerd als “niet-vergoedbaar”',
        markedAsBillable: 'heeft de uitgave als ‘factureerbaar’ gemarkeerd',
        markedAsNonBillable: 'heeft de uitgave als ‘niet-factureerbaar’ gemarkeerd',
        updatedFieldTo: ({key, value, first}: {key: string; value: string | boolean; first: boolean}) => `${first ? 'stel de' : ''}${key} naar "${value}"`,
        viaWorkspaceRules: ({route}: {route: string}) => `via <a href="${route}">werkruimte­regels</a>`,
    },
    transactionMerge: {
        listPage: {
...
Hier is een *testbon* om je te laten zien hoe het werkt:`,
    ...
    gps: {
        tooltip: 'GPS-tracking bezig! Als je klaar bent, stop dan hieronder met volgen.',
