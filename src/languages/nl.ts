...
(bijv. ${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER})`,
    ...
    iou: {
        ...
        business: 'Zakelijk',
        settleExpensify: (formattedAmount: string) => (formattedAmount ? `Betaal ${formattedAmount} met Expensify` : `Betalen met Expensify`),
    ...
    report: {
        ...
        actions: {
            type: {
                ...
                leftTheChatWithName: ({nameOrEmail}: LeftWorkspaceParams) => `${nameOrEmail ? `${nameOrEmail}: ` : ''}heeft de chat verlaten`,
            },
            error: {
