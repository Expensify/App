...
Vereis onkostendetails zoals bonnen en beschrijvingen, stel limieten en standaardwaarden in, en automatiseer goedkeuringen en betalingen – allemaal op één plek.`,
    ...
    search: {
        ...
        suggestionsAvailable: ({count}: {count?: number} = {}) => {
            const countText = count ? `${count} ${count === 1 ? 'resultaat' : 'resultaten'}. ` : '';
            return `Suggesties beschikbaar. ${countText}Gebruik de pijltoetsen omhoog en omlaag om te navigeren.`;
        },
        exportSearchResults: {
            title: 'Export maken',
