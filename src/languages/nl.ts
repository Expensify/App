...
Vraag verplichte uitgavedetails zoals bonnetjes en beschrijvingen, stel limieten en standaardwaarden in en automatiseer goedkeuringen en betalingen – allemaal op één plek.`,
    ...
    workspaceActions: {
        ...
        updateCurrencyDefaultTax: ({oldName, newName}: UpdatedPolicyCurrencyDefaultTaxParams) =>
            `heeft de standaardbelasting voor de workspacevaluta gewijzigd in ‘${newName}’ (voorheen ‘${oldName}’)`,
        updateForeignCurrencyDefaultTax: ({oldName, newName}: UpdatedPolicyForeignCurrencyDefaultTaxParams) =>
            `heeft de standaardbelasting voor vreemde valuta gewijzigd in "${newName}" (voorheen "${oldName}")`,
    },
    roomMembersPage: {
