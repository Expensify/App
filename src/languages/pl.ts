...
(np. ${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER})`,
    ...
    iou: {
        ...
        individual: 'Osoba',
        business: 'Firma',
        settleExpensify: (formattedAmount: string) => (formattedAmount ? `Zapłać ${formattedAmount} z Expensify` : `Zapłać przez Expensify`),
    ...
    report: {
        ...
        actions: {
            type: {
                ...
                leftTheChatWithName: ({nameOrEmail}: LeftWorkspaceParams) => `${nameOrEmail ? `${nameOrEmail}: ` : ''} opuścił czat`,
            },
            error: {
