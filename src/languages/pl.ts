...
(np. ${CONST.FORMATTED_EXAMPLE_PHONE_NUMBER})`,
    ...
    multifactorAuthentication: {
        ...
        statusMessage: {
            successMessage: ({authorization, because}: {authorization?: boolean; because?: string}) =>
                `Pomyślnie ${authorization ? 'autoryzowano wyzwanie' : 'uwierzytelniono'}${because ? ` za pomocą ${because}` : ''}`,
            failedMessage: ({authorization, because}: {authorization?: boolean; because?: string}) =>
                `Twoja ${authorization ? 'autoryzacja' : 'autentykacja'} ${because ? `nie powiodła się: ${because}` : 'zakończyła się niepowodzeniem'}`,
            successTitle: ({authorization}: {authorization?: boolean}) => `${authorization ? 'Autoryzacja' : 'Uwierzytelnianie'} zakończone pomyślnie`,
            failedTitle: ({authorization}: {authorization?: boolean}) => `${authorization ? 'Autoryzacja' : 'Uwierzytelnianie'} nie powiodła się`,
            successTitleGeneral: 'Operacja zakończona pomyślnie',
            failedTitleGeneral: 'Operacja nie powiodła się',
        },
        /** Title indicating multifactorial authentication registration status */
        title: ({registered = true}: {registered?: boolean}) => `Uwierzytelnianie wieloskładnikowe (${registered ? 'Zarejestrowane' : 'Niezarejestrowane'})`,
        reason: {
            /** Success messages for multifactorial authentication operations */
            success: {
                keySavedInSecureStore: 'Klucz został pomyślnie zapisany w SecureStore',
                keyRetrievedFromSecureStore: 'Klucz został pomyślnie pobrany z SecureStore',
                keyNotInSecureStore: 'Nie znaleziono klucza w SecureStore',
                keyPairGenerated: 'Para kluczy została pomyślnie wygenerowana',
                tokenReceived: 'Token został pomyślnie odebrany',
                tokenSigned: 'Token został pomyślnie podpisany',
                verificationSuccess: 'Weryfikacja zakończona pomyślnie',
                keyDeletedFromSecureStore: 'Klucz został pomyślnie usunięty z SecureStore',
            },
            /** Error messages for multifactorial authentication operation failures */
            error: {
                unableToSaveKey: 'Nie udało się zapisać klucza w SecureStore',
                unableToRetrieve: 'Nie udało się pobrać klucza z SecureStore',
                unableToDelete: 'Nie udało się usunąć klucza z SecureStore',
                badToken: 'Nieprawidłowy lub brakujący token',
            ...
            expoErrors: {
                notInForeground: 'Aplikacja musi być na pierwszym planie',
                alreadyInProgress: 'Uwierzytelnianie jest już w toku',
                canceled: 'Uwierzytelnianie anulowane przez użytkownika',
                generic: 'Wystąpił błąd',
                keyExists: 'Taki klucz już istnieje',
                noAuthentication: 'Brak dostępnych metod uwierzytelniania',
                oldAndroid: 'Ta funkcja nie jest obsługiwana na Twoim urządzeniu',
            },
            /** Generic status messages */
            generic: {
                notRequested: 'Nie wysłano jeszcze żadnego żądania',
                apiError: 'Wystąpił błąd API',
                authFactorsError: 'Błąd czynników uwierzytelniających',
                authFactorsSufficient: 'Czynniki uwierzytelniające zostały zweryfikowane',
            },
        },
        /** API response messages */
        apiResponse: {
            registrationRequired: 'Wymagana rejestracja',
            challengeGenerated: 'Wyzwanie zostało pomyślnie wygenerowane',
            noPublicKey: 'Nie podano klucza publicznego',
            ...
            unableToAuthorize: 'Nie udało się autoryzować przy użyciu podanych danych',
            userAuthorized: 'Użytkownik pomyślnie autoryzowany',
            badRequest: 'Nieprawidłowe żądanie',
            signatureInvalid: 'Podpis jest nieprawidłowy',
            unknownResponse: 'Nierozpoznany typ odpowiedzi',
            noPublicKeysRegistered: 'Brak zarejestrowanych kluczy publicznych',
            revokedAccess: 'Pomyślnie cofnięto dostęp na wszystkich urządzeniach',
        },
    ...
    iou: {
        ...
        settleInvoicePersonal: ({amount, last4Digits}: BusinessBankAccountParams) => (amount ? `zapłacono ${amount} z konta osobistego ${last4Digits}` : `Opłacone z konta prywatnego`),
    ...
    nextStep: {
        message: {
            ...
            [CONST.NEXT_STEP.MESSAGE_KEY.WAITING_FOR_AUTOMATIC_SUBMIT]: ({actor, actorType, eta, etaType}: NextStepParams) => {
                let formattedETA = '';
                if (eta) {
                    formattedETA = etaType === CONST.NEXT_STEP.ETA_TYPE.DATE_TIME ? `${eta}` : ` ${eta}`;
                }
                // Disabling the default-case lint rule here is actually safer as this forces us to make the switch cases exhaustive
                // eslint-disable-next-line default-case
                switch (actorType) {
                    case CONST.NEXT_STEP.ACTOR_TYPE.CURRENT_USER:
                        return `Oczekiwanie na automatyczne złożenie <strong>Twoich</strong> wydatków${formattedETA}.`;
                    case CONST.NEXT_STEP.ACTOR_TYPE.OTHER_USER:
            ...
            [CONST.NEXT_STEP.MESSAGE_KEY.WAITING_FOR_POLICY_BANK_ACCOUNT]: ({actor, actorType}: NextStepParams) => {
                ...
                switch (actorType) {
                    ...
                    case CONST.NEXT_STEP.ACTOR_TYPE.UNSPECIFIED_ADMIN:
                        return `Oczekiwanie na zakończenie przez administratora konfiguracji firmowego konta bankowego.`;
                }
            },
    ...
    contacts: {
        ...
        enterMagicCode: ({contactMethod}: EnterMagicCodeParams) => `Wprowadź magiczny kod wysłany na ${contactMethod}. Powinien dotrzeć w ciągu minuty lub dwóch.`,
    ...
    updateRequiredView: {
        ...
        toGetLatestChanges: 'Na urządzeniu mobilnym lub komputerze pobierz i zainstaluj najnowszą wersję. W przeglądarce odśwież stronę.',
...
${
    ...
    beneficialOwnerInfoStep: {
        ...
        areThereMoreIndividualsWhoOwn25percent: ({companyName}: CompanyNameParams) => `Czy są inne osoby, które posiadają 25% lub więcej udziałów w ${companyName}?`,
    ...
    signerInfoStep: {
        ...
        enterOneEmail: ({companyName}: CompanyNameParams) => `Wpisz adres e-mail dyrektora w ${companyName}`,
        ...
        enterTwoEmails: ({companyName}: CompanyNameParams) => `Wpisz adresy e‑mail dwóch dyrektorów w ${companyName}`,
    ...
    travel: {
        ...
        updates: {
            ...
            flightScheduleChangeClosed: ({airlineCode, startDate}: AirlineParams) => `Zmiana planu potwierdzona: lot ${airlineCode} odlot teraz o ${startDate}.`,
            ...
            flightSeatConfirmed: ({airlineCode}: AirlineParams) => `Twoje miejsce na locie ${airlineCode} zostało potwierdzone.`,
    ...
    workspace: {
        ...
        perDiem: {
            ...
            editDestinationSubtitle: ({destination}: EditDestinationSubtitleParams) => `Zaktualizowanie tego miejsca docelowego zmieni je dla wszystkich podstawek diet ${destination}.`,
            editCurrencySubtitle: ({destination}: EditDestinationSubtitleParams) => `Aktualizacja tej waluty zmieni ją dla wszystkich stawek dziennych ${destination}.`,
        },
        qbd: {
        ...
        xero: {
            ...
            mapTrackingCategoryTo: ({categoryName}: CategoryNameParams) => `Mapuj ${categoryName} w Xero do`,
...
3. Gdy kanał zostanie włączony, kliknij poniżej Submit, a my zajmiemy się jego dodaniem.`,
            ...
            assignedCard: ({assignee, link}: AssignedCardParams) => `przydzielił(-a) ${assignee} ${link}! Zaimportowane transakcje pojawią się w tym czacie.`,
        ...
        expensifyCard: {
            ...
            issuedCard: ({assignee}: AssigneeParams) => `przyznał(a) ${assignee} kartę Expensify! Karta dotrze w ciągu 2–3 dni roboczych.`,
            issuedCardNoShippingDetails: ({assignee}: AssigneeParams) => `przyznano ${assignee} kartę Expensify! Karta zostanie wysłana po potwierdzeniu danych wysyłkowych.`,
            ...
            addedShippingDetails: ({assignee}: AssigneeParams) => `${assignee} dodał dane wysyłki. Karta Expensify dotrze w ciągu 2–3 dni roboczych.`,
        ...
        moreFeatures: {
            ...
            companyCards: {
                ...
                pendingBankDescription: ({bankName}: CompanyCardBankName) => `Połącz się z ${bankName} w oknie przeglądarki, które właśnie zostało otwarte. Jeśli się nie otworzyło,`,
        ...
        editor: {
            ...
            currencyInputDisabledText: ({currency}: CurrencyInputDisabledTextParams) =>
                `Domyślna waluta nie może zostać zmieniona, ponieważ ten workspace jest połączony z kontem bankowym w ${currency}.`,
            save: 'Zapisz',
...
Wymagaj szczegółów wydatków, takich jak paragony i opisy, ustawiaj limity i wartości domyślne oraz automatyzuj akceptacje i płatności – wszystko w jednym miejscu.`,
    ...
    checkForUpdatesModal: {
        ...
        error: {
            ...
            message: 'Nie udało się sprawdzić aktualizacji. Spróbuj ponownie za chwilę.',
        },
    },
    settlement: {
        status: {
    ...
    report: {
        ...
        actions: {
            type: {
                ...
                addEmployee: ({email, role}: AddEmployeeParams) => `dodano ${email} jako ${role === 'member' ? 'a' : 'jeden'} ${role}`,
    ...
    subscription: {
        ...
        billingBanner: {
            ...
            policyOwnerUnderInvoicing: {
                title: 'Twoje informacje dotyczące płatności są nieaktualne',
                subtitle: ({date}: BillingBannerSubtitleWithDateParams) => `Twoja płatność jest zaległa. Prosimy opłacić fakturę do ${date}, aby uniknąć przerwy w świadczeniu usług.`,
            },
            ...
            cardAuthenticationRequired: {
                ...
                subtitle: ({cardEnding}: BillingBannerCardAuthenticationRequiredParams) =>
                    `Proszę dokończyć proces uwierzytelniania, aby aktywować kartę płatniczą z numerem kończącym się na ${cardEnding}.`,
            },
            insufficientFunds: {
                title: 'Nie można było obciążyć Twojej karty',
    ...
    delegate: {
        ...
        enterMagicCode: ({contactMethod}: EnterMagicCodeParams) => `Wprowadź magiczny kod wysłany na ${contactMethod}, aby dodać współpilota. Powinien dotrzeć w ciągu minuty lub dwóch.`,
    ...
    testDrive: {
        ...
        employeeInviteMessage: ({name}: EmployeeInviteMessageParams) => `# ${name} zaprosił Cię do wypróbowania Expensify
