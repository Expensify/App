...
const translations = {
    ...
    termsOfUse: {
        terms: ({termsURL, privacyURL}: TermsOfUseRouteParams) =>
            `<comment center><mtm issmall>Logując się, zgadzasz się na <a href="${termsURL}">Warunki korzystania z usługi</a> i <a href="${privacyURL}">Prywatność</a>.</mtm></comment>`,
        licenses: ({licensesURL}: LicensesParams) =>
            `<comment center><mtm issmall>Przekazy pieniężne są świadczone przez ${CONST.WALLET.PROGRAM_ISSUERS.EXPENSIFY_PAYMENTS} (NMLS ID:2017010) zgodnie z jego <a href="${licensesURL}">licencje</a>.</mtm></comment>`,
    },
    validateCodeForm: {
