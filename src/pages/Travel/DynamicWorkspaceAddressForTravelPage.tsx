...
type DynamicWorkspaceAddressForTravelPageProps = PlatformStackScreenProps<
  TravelNavigatorParamList,
  typeof SCREENS.TRAVEL.WORKSPACE_ADDRESS
>;

function DynamicWorkspaceAddressForTravelPage({
  route,
}: DynamicWorkspaceAddressForTravelPageProps) {
  const { translate } = useLocalize();
  const { policyID, domain } = route.params;
  const backPath = useDynamicBackPath(
    DYNAMIC_ROUTES.TRAVEL_WORKSPACE_ADDRESS.path,
  );
  ...
  const updatePolicyAddress = (
    ...
    if (!isUserValidated) {
      ...
      const currentRoute = createDynamicRoute(
        DYNAMIC_ROUTES.TRAVEL_WORKSPACE_ADDRESS.getRoute(domain, policyID),
      );
      setTravelProvisioningNextStep(currentRoute);
      Navigation.navigate(
        createDynamicRoute(
          DYNAMIC_ROUTES.TRAVEL_VERIFY_ACCOUNT.getRoute(domain, policyID),
        ),
      );
      return;
    }
    ...
    Navigation.navigate(
      createDynamicRoute(
        DYNAMIC_ROUTES.TRAVEL_TCS.getRoute(
          domain ?? CONST.TRAVEL.DEFAULT_DOMAIN,
          policyID,
        ),
      ),
      { forceReplace: true },
