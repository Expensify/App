...
type DynamicTripSummaryPageProps = StackScreenProps<
  TravelNavigatorParamList,
  typeof SCREENS.TRAVEL.TRIP_SUMMARY
>;
...
function DynamicTripSummaryPage({ route }: DynamicTripSummaryPageProps) {
  ...
  const [transaction] = useOnyx(
    `${ONYXKEYS.COLLECTION.TRANSACTION}${getNonEmptyStringOnyxID(route.params.transactionID)}`,
  );
  const reservationsData: TripReservationUtils.ReservationData[] =
    TripReservationUtils.getReservationsFromTripReport(
      report,
      ...
      >
        ...
        <ScrollView>
          {reservationsData.map(
            ({ reservation, transactionID, sequenceIndex, isCancelled }) => {
              return (
