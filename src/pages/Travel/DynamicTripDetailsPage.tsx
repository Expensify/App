...
function pickTravelerPersonalDetails(
  personalDetails: OnyxEntry<PersonalDetailsList>,
  reservation: Reservation | undefined,
) {
  return Object.values(personalDetails ?? {})?.find(
    (personalDetail) =>
      personalDetail?.login === reservation?.travelerPersonalInfo?.email,
  );
}

type DynamicTripDetailsPageProps = StackScreenProps<
  TravelNavigatorParamList,
  typeof SCREENS.TRAVEL.DYNAMIC_TRIP_DETAILS
>;
...
function DynamicTripDetailsPage({ route }: DynamicTripDetailsPageProps) {
  ...
  const isBlockedFromSpotnanaTravel = isBetaEnabled(
  ...
  const [report] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT}${getNonEmptyStringOnyxID(transaction?.reportID)}`,
  );
  const [parentReport] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT}${report?.parentReportID ?? reportID}`,
  );
  ...
  const tripReservations = getReservationsFromTripReport(
    !Number(pnr) && transaction ? undefined : parentReport,
  ...
  const travelerPersonalDetailsSelector = (
    personalDetails: OnyxEntry<PersonalDetailsList>,
  ) => pickTravelerPersonalDetails(personalDetails, reservation);

  const [travelerPersonalDetails] = useOnyx(
    ONYXKEYS.PERSONAL_DETAILS_LIST,
    { selector: travelerPersonalDetailsSelector },
    [travelerPersonalDetailsSelector],
  );
  ...
  return (
    ...
    >
      <FullPageNotFoundView
        ...
        shouldShow={
          !reservation || (!CONFIG.IS_HYBRID_APP && isBlockedFromSpotnanaTravel)
        }
      >
        <HeaderWithBackButton
          title={formatCancelledDescription(
            ...
            reservationType
              ? `${translate(`travel.${reservationType}`)} ${translate("common.details").toLowerCase()}`
          ...
          iconWidth={20}
          iconStyles={[
            StyleUtils.getTripReservationIconContainer(false),
            styles.mr3,
          ]}
          iconFill={theme.icon}
        />
