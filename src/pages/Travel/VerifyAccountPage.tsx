...
function VerifyAccountPage({route}: VerifyAccountPageProps) {
    ...
    const travelTCSRoute = activeRouteParams.has('policyID')
        ? DYNAMIC_ROUTES.TRAVEL_TCS.getRoute(domain ?? CONST.TRAVEL.DEFAULT_DOMAIN)
        : DYNAMIC_ROUTES.TRAVEL_TCS.getRoute(domain ?? CONST.TRAVEL.DEFAULT_DOMAIN, policyID);

    // Determine where to navigate after successful OTP validation
    const navigateForwardTo = isTravelVerifiedBetaEnabled ? (travelProvisioning?.nextStepRoute ?? createDynamicRoute(travelTCSRoute)) : undefined;
