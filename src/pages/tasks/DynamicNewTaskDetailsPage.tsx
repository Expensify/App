...
type NewTaskDetailsPageProps = PlatformStackScreenProps<
  NewTaskNavigatorParamList,
  typeof SCREENS.NEW_TASK.DETAILS
>;
...
function DynamicNewTaskDetailsPage({ route }: NewTaskDetailsPageProps) {
  ...
  const [parentReport] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT}${task?.parentReportID}`,
    undefined,
    [task?.parentReportID],
  );
  ...
  const taskTitle =
    localTitle ?? Parser.htmlToMarkdown(Parser.replace(task?.title ?? ""));
  const taskDescription =
    localDescription ??
    Parser.htmlToMarkdown(Parser.replace(task?.description ?? ""));

  const titleDefaultValue = Parser.htmlToMarkdown(Parser.replace(taskTitle));
  const descriptionDefaultValue = Parser.htmlToMarkdown(
    Parser.replace(taskDescription),
  );
  const { inputCallbackRef } = useAutoFocusInput();

  const backPath = useDynamicBackPath(DYNAMIC_ROUTES.NEW_TASK_DETAILS.path);
  const skipConfirmation =
    task?.skipConfirmation && task?.assigneeAccountID && task?.parentReportID;
  const buttonText = skipConfirmation
    ? translate("newTaskPage.assignTask")
