...
function DynamicTaskAssigneeSelectorModal() {
  ...
  const route = useRoute<
    | PlatformStackRouteProp<
        TaskDetailsNavigatorParamList,
        typeof SCREENS.DYNAMIC_TASK_ASSIGNEE
      >
    | PlatformStackRouteProp<
        NewTaskNavigatorParamList,
        typeof SCREENS.NEW_TASK.DYNAMIC_TASK_ASSIGNEE
      >
  >();
  ...
  const backPath = useDynamicBackPath(
    isNewTaskFlow
      ? DYNAMIC_ROUTES.NEW_TASK_ASSIGNEE.path
      : DYNAMIC_ROUTES.TASK_ASSIGNEE.path,
  );
  const reportID =
    !isNewTaskFlow && route.params && "reportID" in route.params
  ...
  const parentReport =
    reports?.[`${ONYXKEYS.COLLECTION.REPORT}${report?.parentReportID}`];
  ...
  const sections = sectionsList.map((section) => ({
    ...
    data: section.data.map((option) => ({
      ...
      isSelected:
        task?.assigneeAccountID === option.accountID ||
        task?.report?.managerID === option.accountID,
    })),
  }));

  const initiallyFocusedOptionKey = sections
    .flatMap((section) => section.data)
    .find((mode) => mode.isSelected === true)?.keyForList;
  ...
  const selectReport = (option: ListItem) => {
    ...
    if (report) {
      if (option.accountID !== report.managerID) {
        const { report: assigneeChatReport, isOptimisticReport } =
          setAssigneeValue(
  ...
  const isTaskModifiable = canModifyTask(
    report,
    currentUserPersonalDetails.accountID,
    isParentReportArchived,
  );
  const isTaskNonEditable =
    isTaskReport(report) && (!isTaskModifiable || !isOpen);
