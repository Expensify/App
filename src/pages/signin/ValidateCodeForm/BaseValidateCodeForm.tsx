...
import {setIsSigningIn, setReadyToShowAuthScreens} from '@userActions/HybridApp';
...
function BaseValidateCodeForm({autoComplete, isUsingRecoveryCode, setIsUsingRecoveryCode, isVisible}: BaseValidateCodeFormProps, forwardedRef: ForwardedRef<BaseValidateCodeFormRef>) {
     ...
     */
    useEffect(() => {
        if (!NativeModules.HybridAppModule || !useNewDotSignInPage) {
            return;
        }
    ...
    }, [newDotSignInState, isValidateCodeFormSubmitting, session?.authToken, sessionMetadata, useNewDotSignInPage]);
    ...
    }, [hybridApp?.isSigningIn, oldDotSignInState, useNewDotSignInPage]);
    ...
    useEffect(() => {
        ...
        if (newDotSignInState === CONST.NEW_DOT_SIGN_IN_STATE.FINISHED && oldDotSignInState === CONST.OLD_DOT_SIGN_IN_STATE.NOT_STARTED) {
    ...
    }, [account?.isLoading, credentials?.autoGeneratedLogin, credentials?.autoGeneratedPassword, newDotSignInState, oldDotSignInState, tryNewDot, useNewDotSignInPage]);
