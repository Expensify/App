...
function BaseValidateCodeForm({autoComplete, isUsingRecoveryCode, setIsUsingRecoveryCode, isVisible}: BaseValidateCodeFormProps, forwardedRef: ForwardedRef<BaseValidateCodeFormRef>) {
    ...
    useEffect(() => {
        ...
        if (tryNewDot?.classicRedirect.dismissed === true || tryNewDot?.classicRedirect.dismissed === 'true') {
            // This method:
            // 1. Performs sign in on OldDot side with credentials taken from NewDot
            // 2. Closes React Native app and displays OldDot's sliding menu page
            if (credentials?.autoGeneratedLogin && credentials?.autoGeneratedPassword) {
                Log.info('Calling yapl signInToOldDot from RN');
                NativeModules.HybridAppModule.signInToOldDot(credentials?.autoGeneratedLogin, credentials?.autoGeneratedPassword);
                Log.info('Called yapl signInToOldDot from RN');
            }
        }
