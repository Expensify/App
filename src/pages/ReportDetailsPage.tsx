...
function ReportDetailsPage({policy, report, route, reportMetadata}: ReportDetailsPageProps) {
    ...
    const parentNavigationSubtitleData = getParentNavigationSubtitle(report, formatPhoneNumber);
    ...
    const leaveChat = useCallback(() => {
        ...
        Navigation.isNavigationReady().then(() => {
            if (isRootGroupChat) {
                leaveGroupChat(report.reportID, formatPhoneNumber);
                return;
            }
            ...
            leaveRoom(report.reportID, formatPhoneNumber, isWorkspaceMemberLeavingWorkspaceRoom);
        });
    }, [isPolicyEmployee, isPolicyExpenseChat, isRootGroupChat, report.reportID, report.visibility, formatPhoneNumber]);
    ...
    const reportName = Parser.htmlToText(getReportName(report, formatPhoneNumber));
    ...
    const displayNamesWithTooltips = useMemo(() => {
        ...
        return getDisplayNamesWithTooltips(getPersonalDetailsForAccountIDs(participants, personalDetails), hasMultipleParticipants, formatPhoneNumber);
    }, [participants, personalDetails, formatPhoneNumber]);

    const icons = useMemo(() => getIcons(report, formatPhoneNumber, personalDetails, null, '', -1, policy), [report, personalDetails, policy, formatPhoneNumber]);
