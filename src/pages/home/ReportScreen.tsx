...
function ReportScreen({route, navigation}: ReportScreenProps) {
    ...
    useEffect(() => {
        if (!(transactionThreadReportID === CONST.FAKE_REPORT_ID && !transactionThreadReportID && reportMetadata.hasOnceLoadedReportActions) && !(visibleTransactions.length === 1 && !transactionThreadReportID)) {
            return;
        }

        createOneTransactionThreadReport();
    }, [reportMetadata.hasOnceLoadedReportActions, transactionThreadReportID, createOneTransactionThreadReport, visibleTransactions.length]);
