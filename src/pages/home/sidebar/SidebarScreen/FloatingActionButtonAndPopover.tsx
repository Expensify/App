...
function FloatingActionButtonAndPopover(
    ...
    const quickActionTitle = useMemo(() => {
        ...
        if (quickAction?.action === CONST.QUICK_ACTIONS.SEND_MONEY && quickActionAvatars.length > 0) {
            const name: string = ReportUtils.getDisplayNameForParticipant(+quickActionAvatars.at(0)?.id, true) ?? '';
    ...
    const hideQABSubtitle = useMemo(() => {
        ...
        const displayName = personalDetails?.[quickActionAvatars.at(0)?.id]?.firstName ?? '';
    ...
    const navigateToQuickAction = () => {
        ...
        switch (quickAction?.action) {
            ...
            case CONST.QUICK_ACTIONS.SEND_MONEY:
                ...
                return;
            case CONST.QUICK_ACTIONS.ASSIGN_TASK:
                selectOption(() => Task.startOutCreateTaskQuickAction(isValidReport ? quickActionReportID : '', quickAction.targetAccountID), false);
                break;
            case CONST.QUICK_ACTIONS.TRACK_MANUAL:
