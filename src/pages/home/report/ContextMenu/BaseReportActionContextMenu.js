    ...
    render() {
        ...
        return (
            (this.props.isVisible || this.state.shouldKeepOpen) && (
                <View ref={this.props.contentRef} style={this.wrapperStyle}>
                    {_.map(
                        _.filter(ContextMenuActions, shouldShowFilter),
                        (contextAction) => {
                            ...
                            const payload = {
                                ...
                                close: () =>
                                    this.setState({shouldKeepOpen: false}),
                                openContextMenu: () =>
                                    this.setState({shouldKeepOpen: true}),
                            };
                            ...
                            if (contextAction.renderContent) {
                                ...
                                if (
                                    __DEV__ &&
                                    (contextAction.text != null ||
                                        contextAction.icon != null)
                                ) {
                                    throw new Error(
                                        'Dev error: renderContent() and text/icon cannot be used together.',
                                    );
                                }

                                return contextAction.renderContent(
                                    closePopup,
                                    payload,
                                );
                            }
                            ...
                            return (
                                <ContextMenuItem
                                    ...
                                    text={this.props.translate(
                                        contextAction.textTranslateKey,
                                    )}
                                    successIcon={contextAction.successIcon}
                                    successText={
                                        contextAction.successTextTranslateKey
