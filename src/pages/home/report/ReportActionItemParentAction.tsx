    ...
    return (
        <View style={[styles.pRelative]}>
            ...
            {ancestors.map((ancestor) => {
                ...
                const isAncestorReportArchived = isArchivedReport(
                ...
                const reportDraftMessages = originalReportID
                    ? allDraftMessages?.[`${ONYXKEYS.COLLECTION.REPORT_ACTIONS_DRAFTS}${originalReportID}`]
                    : undefined;
                const matchingDraftMessage = reportDraftMessages?.[ancestorReportAction.reportActionID];
                const matchingDraftMessageString =
                    typeof matchingDraftMessage === 'string' ? matchingDraftMessage : matchingDraftMessage?.message;
                const actionEmojiReactions =
                    allEmojiReactions?.[`${ONYXKEYS.COLLECTION.REPORT_ACTIONS_REACTIONS}${ancestorReportAction.reportActionID}`];
                ...
                return (
                    <OfflineWithFeedback
                        ...
                        pendingAction={
                            ancestorReport?.pendingFields?.addWorkspaceRoom ?? ancestorReport?.pendingFields?.createChat
                        }
