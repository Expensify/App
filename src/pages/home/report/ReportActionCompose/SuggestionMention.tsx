...
import usePreSortedPersonalDetails from '@hooks/usePreSortedPersonalDetails';
...
function SuggestionMention(
    ...
    const preSortedPersonalDetails = usePreSortedPersonalDetails();

    console.log('preSortedPersonalDetails: ', preSortedPersonalDetails);
    ...
    const getUserMentionOptions = useCallback(
        (personalDetailsParam: PersonalDetailsList[], searchValue = ''): Mention[] => {
            ...
            const filteredPersonalDetails = personalDetailsParam.filter((detail) => {
            ...
            filteredPersonalDetails.slice(0, CONST.AUTO_COMPLETE_SUGGESTER.MAX_AMOUNT_OF_SUGGESTIONS - suggestions.length).forEach((detail) => {
                suggestions.push({
    ...
    const calculateMentionSuggestion = useCallback(
        (newValue: string, selectionStart?: number, selectionEnd?: number) => {
            ...
            if (isMentionCode(suggestionWord) && prefixType === '@') {
                const suggestions = getUserMentionOptions(preSortedPersonalDetails, prefix);
        ...
        [isComposerFocused, isGroupPolicyReport, setHighlightedMentionIndex, resetSuggestions, getUserMentionOptions, preSortedPersonalDetails, getRoomMentionOptions, reports],
    );
