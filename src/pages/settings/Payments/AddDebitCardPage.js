...
class DebitCardPage extends Component {
    ...
    validate(values) {
        ...
        if (!values.nameOnCard || !ValidationUtils.isValidCardName(values.nameOnCard)) {
            errors.push({nameOnCard: this.props.translate('addDebitCardPage.error.invalidName')});
        }

        if (!values.cardNumber || !ValidationUtils.isValidDebitCard(values.cardNumber.replace(/ /g, ''))) {
            errors.push({cardNumber: this.props.translate('addDebitCardPage.error.debitCardNumber')});
        }

        if (!values.expirationDate || !ValidationUtils.isValidExpirationDate(values.expirationDate)) {
            errors.push({expirationDate: this.props.translate('addDebitCardPage.error.expirationDate')});
        }

        if (!values.securityCode || !ValidationUtils.isValidSecurityCode(values.securityCode)) {
            errors.push({securityCode: this.props.translate('addDebitCardPage.error.securityCode')});
        }

        if (!values.addressStreet || !ValidationUtils.isValidAddress(values.addressStreet)) {
            errors.push({addressStreet: this.props.translate('addDebitCardPage.error.addressStreet')});
        }

        if (!values.addressZipCode || !ValidationUtils.isValidZipCode(values.addressZipCode)) {
            errors.push({addressZipCode: this.props.translate('addDebitCardPage.error.addressZipCode')});
        }

        if (!values.addressState || !values.addressState) {
            errors.push({addressState: this.props.translate('addDebitCardPage.error.addressState')});
        }

        if (!Permissions.canUsePasswordlessLogins(this.props.betas) && (!values.password || _.isEmpty(values.password.trim()))) {
            errors.push({password: this.props.translate('addDebitCardPage.error.password')});
        }

        if (!values.acceptTerms) {
            errors.push({acceptTerms: this.props.translate('common.error.acceptTerms')});
        }
