...
function DelegateMagicCodeModal({login, role, onClose, isValidateCodeActionModalVisible, shouldHandleNavigationBack, disableAnimation}: DelegateMagicCodeModalProps) {
    ...
    const clearError = () => {
        ...
        clearDelegateErrorsByField(currentDelegate?.email ?? '', 'addDelegate', account?.delegatedAccess);
    };
    ...
    return (
        <ValidateCodeActionModal
            ...
            handleSubmitForm={(validateCode) => addDelegate(login, role, validateCode, account?.delegatedAccess)}
