...
function DynamicNewContactMethodPage() {
    ...
    const contactMethodsBackPath = useDynamicBackPath(DYNAMIC_ROUTES.NEW_CONTACT_METHOD.path);
    ...
    }, [contactMethodsBackPath]);

    useEffect(() => {
        if (!pendingContactAction?.actionVerified || !pendingContactAction?.contactMethod) {
            return;
        }
        Navigation.navigate(createDynamicRoute(DYNAMIC_ROUTES.CONTACT_METHOD_DETAILS.getRoute(addSMSDomainIfPhoneNumber(pendingContactAction?.contactMethod), true), contactMethodsBackPath));
        clearUnvalidatedNewContactMethodAction();
    }, [pendingContactAction?.actionVerified, pendingContactAction?.contactMethod, contactMethodsBackPath]);
