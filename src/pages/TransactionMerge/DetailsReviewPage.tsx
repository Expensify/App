...
function DetailsReviewPage({route}: DetailsReviewPageProps) {
    ...
    useEffect(() => {
        ...
        const {conflictFields: detectedConflictFields, mergeableData} = getMergeableDataAndConflictFields(targetTransaction, sourceTransaction, originalSourceTransaction, localeCompare);

        setMergeTransactionKey(transactionID, mergeableData);
        setConflictFields(detectedConflictFields as MergeFieldKey[]);
    }, [targetTransaction, sourceTransaction, originalSourceTransaction, transactionID, localeCompare]);
