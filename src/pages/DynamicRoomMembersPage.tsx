...
import type {
  RoomMemberRowData,
  RoomMembersTableColumnKey,
} from "@components/Tables/RoomMembersTable";
...
import {
  ...
  updateUserSearchPhrase,
} from "@libs/actions/RoomMembersUserSearchPhrase";
...
import {
  isPolicyAdmin,
  isPolicyEmployee as isPolicyEmployeeUtils,
...
type DynamicRoomMembersPageProps = WithReportOrNotFoundProps &
  PlatformStackScreenProps<
    RoomMembersNavigatorParamList,
    typeof SCREENS.ROOM_MEMBERS.DYNAMIC_ROOT
  >;

function DynamicRoomMembersPage({
  report,
  policy,
}: DynamicRoomMembersPageProps) {
  ...
  const [reportMetadata] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT_METADATA}${report?.reportID}`,
  );
  ...
  const isPolicyExpenseChat = useMemo(
    () => isPolicyExpenseChatUtils(report),
    [report],
  );
  const tableRef =
    useRef<TableHandle<RoomMemberRowData, RoomMembersTableColumnKey, string>>(
      null,
    );
  ...
  const { chatParticipants: participants, personalDetailsParticipants } =
    useMemo(
      () =>
        getReportPersonalDetailsParticipants(
          report,
  ...
  const shouldIncludeMember = useCallback(
    (participant?: PersonalDetails) => {
      ...
      const pendingChatMember = reportMetadata?.pendingChatMembers?.find(
        (member) => member.accountID === participant.accountID.toString(),
      );
  ...
  const [selectedMembers, setSelectedMembers] = useFilteredSelection(
    personalDetailsParticipants,
    shouldIncludeMember,
  );
  ...
  const clearTableSelection = useCallback(() => {
    setSelectedMembers((prevSelectedMembers) =>
      prevSelectedMembers.length > 0 ? [] : prevSelectedMembers,
    );
  }, [setSelectedMembers]);
  ...
  const canSelectMultiple = isSmallScreenWidth
    ? isMobileSelectionModeEnabled
    : true;
  const isLoading =
    !isPersonalDetailsReady(personalDetails) || !didLoadRoomMembers;
  ...
  useEffect(() => {
    ...
    const currentSearchString =
      tableRef.current?.getActiveSearchString?.() ?? "";
  ...
  const openRoomMemberDetails = useCallback((accountID: number) => {
    Navigation.navigate(
      createDynamicRoute(
        DYNAMIC_ROUTES.ROOM_MEMBER_DETAILS.getRoute(accountID),
      ),
    );
  }, []);
  ...
  const members = useMemo<RoomMemberRowData[]>(() => {
    ...
    for (const accountID of participants) {
      ...
      result.push({
        ...
        dismissError: () =>
          clearAddRoomMemberError(report.reportID, String(accountID)),
      });
    }

    return result.sort((value1, value2) =>
      localeCompare(value1.name.toLowerCase(), value2.name.toLowerCase()),
    );
  }, [
    formatPhoneNumber,
    isPolicyExpenseChat,
  ...
  const isPolicyEmployee = useMemo(
    () => isPolicyEmployeeUtils(report.policyID, policy),
  ...
  if (!isEmptyObject(report)) {
    subtitleKey = isReportArchived
      ? "roomMembersPage.roomArchived"
      : "roomMembersPage.notAuthorized";
  }
    ...
    >
      <FullPageNotFoundView
        shouldShow={
          ...
          isReportArchived ||
          (!isChatThread(report) &&
            ((isUserCreatedPolicyRoom(report) && !isPolicyEmployee) ||
      ...
      >
        <HeaderWithBackButton
          ...
          subtitle={StringUtils.lineBreaksToSpaces(
            shouldParserToHTML
              ? Parser.htmlToText(
                  getReportName(reportForSubtitle, reportAttributes),
                )
              : getReportName(reportForSubtitle, reportAttributes),
          )}
          onBackButtonPress={() => {
