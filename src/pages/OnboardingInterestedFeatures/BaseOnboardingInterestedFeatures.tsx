...
import {setOnboardingAdminsChatReportID, setOnboardingInterestedFeaturesMap, setOnboardingPolicyID, setOnboardingUserReportedIntegration} from '@libs/actions/Welcome';
...
function BaseOnboardingInterestedFeatures({shouldUseNativeStyles, route}: BaseOnboardingInterestedFeaturesProps) {
    ...
    // We need to use isSmallScreenWidth for the feature grid layout
    ...
    const handleContinue = useCallback(async () => {
        ...
        if (isAccountingEnabled) {
            // Persist features map for the Accounting step to use when completing onboarding
            setOnboardingInterestedFeaturesMap(featuresMap);
            // Clear any previous integration selection since we're going to the accounting step
            setOnboardingUserReportedIntegration(null);
            Navigation.navigate(ROUTES.ONBOARDING_ACCOUNTING.getRoute(route.params?.backTo));
            return;
        }

        // Accounting is disabled — complete onboarding directly
        await completeOnboardingFlow({featuresMap, userReportedIntegration: undefined});
    }, [features, selectedFeatures, completeOnboardingFlow, route.params?.backTo]);
