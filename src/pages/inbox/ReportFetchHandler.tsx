...
function ReportFetchHandler() {
    ...
    // because the wipe can land while the screen is blurred or offline, and because this screen can be
    // re-parameterized to a different report without unmounting.
    ...
    useEffect(() => {
        ...
        if (reportIDWithLostLoadedStampRef.current !== reportIDFromRoute || !isFocused || isOffline) {
            return;
        }
        reportIDWithLostLoadedStampRef.current = undefined;
        fetchReport();
    }, [reportIDFromRoute, reportLoadingState.hasOnceLoadedReportActions, prevHasOnceLoadedReportActions, isFocused, isOffline]);
