...
function PureReportActionItem({
    ...
    const renderItemContent = (hovered = false, isWhisper = false, hasErrors = false): React.JSX.Element => {
        ...
        } else if (isPolicyChangeLogAction(action) || isActionOfType(action, CONST.REPORT.ACTIONS.TYPE.ROOM_CHANGE_LOG.LEAVE_ROOM)) {
            children = (
        ...
        } else if (isActionableWhisperAction(action)) {
            children = (
                <ActionableWhisperContent
                    action={action}
                    report={report}
                    originalReport={originalReport}
                    policy={policy}
