...
function useActiveDraftReportAction({reportID, effectiveTransactionThreadReportID}: UseActiveDraftReportActionArgs): ResolvedActiveDraftEdit | null {
    ...
    const reportActionsSelector = useCallback(
        (allReportActions: OnyxCollection<OnyxTypes.ReportActions> | undefined) => {
            if (!allReportActions) {
                return {};
            }
    ...
    const reportActionsDraftsSelector = useCallback(
        (allDrafts: OnyxCollection<OnyxTypes.ReportActionsDrafts> | undefined) => {
            if (!allDrafts) {
                return {};
            }
    ...
    const [reportActionsDrafts] = useOnyx(ONYXKEYS.COLLECTION.REPORT_ACTIONS_DRAFTS, {
        selector: reportActionsDraftsSelector,
    });
