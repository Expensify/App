...
function ReportActionItemCreated({reportID, policyID}: ReportActionItemCreatedProps) {
    ...
    const [participantPersonalDetail] = useOnyx(ONYXKEYS.PERSONAL_DETAILS_LIST, {selector: personalDetailsSelector(otherParticipantAccountID)});

    if (!isChatReport(report)) {
        return null;
    }

    const shouldDisableDetailPage = shouldDisableDetailPageReportUtils(report, participantPersonalDetail);
