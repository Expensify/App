...
class ReimbursementAccountPage extends React.Component {
    ...
    render() {
        ...
        if (this.props.reimbursementAccount.isLoading && !(this.props.network.isOffline && currentStep === CONST.BANK_ACCOUNT.STEP.BANK_ACCOUNT)) {
            ...
            return <ReimbursementAccountLoadingIndicator isSubmittingVerificationsData={isSubmittingVerificationsData} onBackButtonPress={this.goBack} />;
        }
        ...
        if (
            ...
            (_.contains([CONST.BANK_ACCOUNT.STEP.BANK_ACCOUNT, ''], this.getStepToOpenFromRouteParams()) || achData.state === BankAccount.STATE.PENDING)
        ) {
            return (
        ...
        if (errorComponent) {
            return (
                <ScreenWrapper>
                    <HeaderWithCloseButton title={this.props.translate('workspace.common.bankAccount')} onCloseButtonPress={Navigation.dismissModal} />
                    {errorComponent}
                </ScreenWrapper>
            );
        }
        ...
        if (currentStep === CONST.BANK_ACCOUNT.STEP.VALIDATION) {
            return <ValidationStep reimbursementAccount={this.props.reimbursementAccount} onBackButtonPress={this.goBack} />;
        }

        if (currentStep === CONST.BANK_ACCOUNT.STEP.ENABLE) {
            return <EnableStep reimbursementAccount={this.props.reimbursementAccount} />;
        }
    }
}
