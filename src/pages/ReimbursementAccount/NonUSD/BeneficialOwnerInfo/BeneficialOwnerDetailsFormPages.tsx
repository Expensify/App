...
const { OWNERSHIP_PERCENTAGE, NATIONALITY, COUNTRY, PREFIX } =
  BENEFICIAL_OWNER_INFO_STEP.BENEFICIAL_OWNER_DATA;
...
function BeneficialOwnerDetailsFormPages({
  ...
  const [reimbursementAccountDraft] = useOnyx(
    ONYXKEYS.FORMS.REIMBURSEMENT_ACCOUNT_FORM_DRAFT,
  );
  const [reimbursementAccount] = useOnyx(ONYXKEYS.REIMBURSEMENT_ACCOUNT);
  const [policy] = useOnyx(`${ONYXKEYS.COLLECTION.POLICY}${policyID}`);

  const ownerBeingModifiedID =
    reimbursementAccountDraft?.ownerBeingModifiedID ??
    CONST.NON_USD_BANK_ACCOUNT.CURRENT_USER_KEY;
  const isUserEnteringHisOwnData =
    ownerBeingModifiedID === CONST.NON_USD_BANK_ACCOUNT.CURRENT_USER_KEY;
  const isEditingCreatedOwner =
    reimbursementAccountDraft?.isEditingCreatedOwner ?? false;
  const ownerKeys = reimbursementAccountDraft?.beneficialOwnerKeys ?? [];

  const beneficialOwnerNationalityInputID =
    `${PREFIX}_${ownerBeingModifiedID}_${NATIONALITY}` as const;
  const beneficialOwnerNationality = SafeString(
    reimbursementAccountDraft?.[beneficialOwnerNationalityInputID],
  );
  const { country: countryStepCountryValue, currency } =
    getCurrencyForNonUSDBankAccount(
      policy,
      reimbursementAccountDraft,
      reimbursementAccount,
    );
  const beneficialOwnerAddressCountryInputID =
    `${PREFIX}_${ownerBeingModifiedID}_${COUNTRY}` as const;
  const beneficialOwnerAddressCountry = SafeString(
    reimbursementAccountDraft?.[beneficialOwnerAddressCountryInputID],
  );
  ...
  const totalOwnedPercentage = Object.fromEntries(
    ownerKeys.map((key) => {
      ...
      return [
        key,
        Number(SafeString(reimbursementAccountDraft?.[percentageKey])) || 0,
      ];
    }),
  );
  ...
  const skipPages = useMemo(() => {
    ...
    const documentsStatus = getNeededDocumentsStatusForBeneficialOwner(
      currency,
      countryStepCountryValue,
      beneficialOwnerAddressCountry,
    );
    if (
      !documentsStatus.isProofOfOwnershipNeeded &&
      !documentsStatus.isCopyOfIDNeeded &&
      !documentsStatus.isProofOfAddressNeeded &&
      !documentsStatus.isCodiceFiscaleNeeded
    ) {
  ...
  const {
    ...
    moveTo,
    isRedirecting,
  } = useSubPage<BeneficialOwnerSubPageProps>({
    pages,
    startFrom: 0,
    onFinished,
    buildRoute,
  ...
  const handleBackButtonPress = useCallback(() => {
    ...
    while (
      prevIndex >= 0 &&
      skipPages.includes(pages.at(prevIndex)?.pageName ?? "")
    ) {
      prevIndex -= 1;
    }
