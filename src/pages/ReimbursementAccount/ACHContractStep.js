...
class ACHContractStep extends React.Component {
    constructor(props) {
        ...
        this.state = {
            ...
            beneficialOwners: props.getDefaultStateForField(
                'beneficialOwners',
                [],
            ),
        };
    ...
    validate(values) {
        ...
        if (values.hasOtherBeneficialOwners) {
            _.each(this.state.beneficialOwners, (ownerKey) => {
                ...
                _.each(requiredFields, (inputKey) => {
                    if (
                        !ValidationUtils.isRequiredFulfilled(
                            values[`beneficialOwner_${ownerKey}_${inputKey}`],
                        )
                    ) {
                        const errorKey = errorKeys[inputKey] || inputKey;
                        errors[`beneficialOwner_${ownerKey}_${inputKey}`] =
                            this.props.translate(
                                `bankAccount.error.${errorKey}`,
                            );
                    }
                });

                if (
                    values[`beneficialOwner_${ownerKey}_dob`] &&
                    !ValidationUtils.meetsAgeRequirements(
                        values[`beneficialOwner_${ownerKey}_dob`],
                    )
                ) {
                    errors[`beneficialOwner_${ownerKey}_dob`] =
                        this.props.translate('bankAccount.error.age');
                }

                if (
                    values[`beneficialOwner_${ownerKey}_ssnLast4`] &&
                    !ValidationUtils.isValidSSNLastFour(
                        values[`beneficialOwner_${ownerKey}_ssnLast4`],
                    )
                ) {
                    errors[`beneficialOwner_${ownerKey}_ssnLast4`] =
                        this.props.translate('bankAccount.error.ssnLast4');
                }

                if (
                    values[`beneficialOwner_${ownerKey}_street`] &&
                ...
                if (
                    values[`beneficialOwner_${ownerKey}_zipCode`] &&
                    !ValidationUtils.isValidZipCode(
                        values[`beneficialOwner_${ownerKey}_zipCode`],
                    )
                ) {
                    errors[`beneficialOwner_${ownerKey}_zipCode`] =
        ...
        if (
            ...
            errors.acceptTermsAndConditions = this.props.translate(
                'common.error.acceptTerms',
            );
        }

        if (
            !ValidationUtils.isRequiredFulfilled(values.certifyTrueInformation)
        ) {
            errors.certifyTrueInformation = this.props.translate(
                'beneficialOwnersStep.error.certify',
            );
        }
    ...
    removeBeneficialOwner(ownerKey) {
        this.setState((prevState) => {
            const beneficialOwners = _.without(
                prevState.beneficialOwners,
                ownerKey,
            );

            FormActions.setDraftValues(ONYXKEYS.REIMBURSEMENT_ACCOUNT, {
                beneficialOwners,
            });
    ...
    canAddMoreBeneficialOwners(ownsMoreThan25Percent) {
        return (
            _.size(this.state.beneficialOwners) < 3 ||
            (_.size(this.state.beneficialOwners) === 3 &&
    ...
    submit(values) {
        ...
        const beneficialOwners = !values.hasOtherBeneficialOwners
            ...
            : _.map(this.state.beneficialOwners, (ownerKey) => ({
                  firstName: lodashGet(
                      values,
                      `beneficialOwner_${ownerKey}_firstName`,
                  ),
                  lastName: lodashGet(
                      values,
                      `beneficialOwner_${ownerKey}_lastName`,
                  ),
                  dob: lodashGet(values, `beneficialOwner_${ownerKey}_dob`),
                  ssnLast4: lodashGet(
                      values,
                      `beneficialOwner_${ownerKey}_ssnLast4`,
                  ),
                  street: lodashGet(
                      values,
                  ...
                  zipCode: lodashGet(
                      values,
                      `beneficialOwner_${ownerKey}_zipCode`,
                  ),
              }));
            ...
            <ScreenWrapper includeSafeAreaPaddingBottom={false}>
                <HeaderWithCloseButton
                    title={this.props.translate(
                        'beneficialOwnersStep.additionalInformation',
                    )}
                    ...
                    {({inputValues}) => (
                        <>
                            <Text style={[styles.mb5]}>
                                <Text>
                                    {this.props.translate(
                                        'beneficialOwnersStep.checkAllThatApply',
                                    )}
                                </Text>
                            </Text>
                            <CheckboxWithLabel
                                ...
                                LabelComponent={() => (
                                    <Text>
                                        {this.props.translate(
                                            'beneficialOwnersStep.iOwnMoreThan25Percent',
                                        )}
                                ...
                                onValueChange={(ownsMoreThan25Percent) => {
                                    if (
                                        ownsMoreThan25Percent &&
                                        this.state.beneficialOwners.length > 3
                                    ) {
                                        ...
                                        this.setState((prevState) => ({
                                            beneficialOwners:
                                                prevState.beneficialOwners.slice(
                                                    0,
                                                    -1,
                                                ),
                                        }));
                            ...
                            <CheckboxWithLabel
                                ...
                                LabelComponent={() => (
                                    <Text>
                                        {this.props.translate(
                                            'beneficialOwnersStep.someoneOwnsMoreThan25Percent',
                                        )}
                                        <Text style={[styles.textStrong]}>
                                            {this.props.companyName}
                                        </Text>
                            ...
                            {inputValues.hasOtherBeneficialOwners && (
                                <View style={[styles.mb2]}>
                                    {_.map(
                                        ...
                                        (ownerKey, index) => (
                                            ...
                                            >
                                                ...
                                                >
                                                    {this.props.translate(
                                                        'beneficialOwnersStep.additionalOwner',
                                                    )}
                                                </Text>
                                                <IdentityForm
                                                    translate={
                                                    ...
                                                    defaultValues={{
                                                        ...
                                                        city: this.props.getDefaultStateForField(
                                                            `beneficialOwner_${ownerKey}_city`,
                                                            '',
                                                        ),
                                                        state: this.props.getDefaultStateForField(
                                                            `beneficialOwner_${ownerKey}_state`,
                                                            '',
                                                        ),
                                                        zipCode:
                                                            this.props.getDefaultStateForField(
                                                        ...
                                                        dob: this.props.getDefaultStateForField(
                                                            `beneficialOwner_${ownerKey}_dob`,
                                                            '',
                                                        ),
                                                        ssnLast4:
                                                            this.props.getDefaultStateForField(
                                                ...
                                                {this.state.beneficialOwners
                                                    ...
                                                    >
                                                        {this.props.translate(
                                                            'beneficialOwnersStep.removeOwner',
                                                        )}
                                                    </TextLink>
                                                )}
                                            </View>
                                    ...
                                    {this.canAddMoreBeneficialOwners(
                                        inputValues.ownsMoreThan25Percent,
                                    ) && (
                                        <TextLink
                                            onPress={this.addBeneficialOwner}
                                        >
                                            {this.props.translate(
                                                'beneficialOwnersStep.addAnotherIndividual',
                                            )}
                                            <Text
                                                style={[
                                                    styles.textStrong,
                            ...
                            <Text style={[styles.mv5]}>
                                {this.props.translate(
                                    'beneficialOwnersStep.agreement',
                                )}
                            </Text>
                            <CheckboxWithLabel
                                ...
                                LabelComponent={() => (
                                    <Text>
                                        ...
                                        <TextLink href="https://use.expensify.com/achterms">
                                            {`${this.props.translate(
                                                'beneficialOwnersStep.termsAndConditions',
                                            )}`}
                                        </TextLink>
                                    </Text>
                                )}
                            ...
                            <CheckboxWithLabel
                                ...
                                LabelComponent={() => (
                                    <Text>
                                        {this.props.translate(
                                            'beneficialOwnersStep.certifyTrueAndAccurate',
                                        )}
                                    </Text>
                                )}
                                defaultValue={this.props.getDefaultStateForField(
