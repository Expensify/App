...
function getInitialSubstepForBankInfo(data, setupType) {
    if (setupType === CONST.BANK_ACCOUNT.SETUP_TYPE.MANUAL) {
        if (data[CONST.BANK_ACCOUNT.BANK_INFO_STEP.INPUT_KEY.ACCOUNT_NUMBER].length === 0 || data[CONST.BANK_ACCOUNT.BANK_INFO_STEP.INPUT_KEY.ROUTING_NUMBER].length === 0) {
            return 0;
        }
    ...
    if (setupType === CONST.BANK_ACCOUNT.SETUP_TYPE.PLAID) {
        if (data[CONST.BANK_ACCOUNT.BANK_INFO_STEP.INPUT_KEY.BANK_NAME].length === 0) {
            return 0;
        }
