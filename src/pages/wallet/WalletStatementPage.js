...
function WalletStatementPage(props) {
    const givenYearMonth = lodashGet(props.route.params, 'yearMonth', null);

    useEffect(() => {
        const currentYearMonth = moment().format('YYYYMM');
        if (!givenYearMonth || givenYearMonth.length !== 6 || givenYearMonth > currentYearMonth) {
    ...
    const processDownload = (yearMonth) => {
    ...
    const year = givenYearMonth.substring(0, 4) || moment().year();
    const month = givenYearMonth.substring(4) || moment().month();
    const monthName = moment(month, 'M').format('MMMM');
    const title = `${monthName} ${year} statement`;
    const url = `${CONFIG.EXPENSIFY.EXPENSIFY_URL}statement.php?period=${givenYearMonth}`;
    ...
    return (
        <ScreenWrapper includeSafeAreaPaddingBottom={false}>
            <HeaderWithBackButton
                ...
                onDownloadButtonPress={() => processDownload(givenYearMonth)}
            />
