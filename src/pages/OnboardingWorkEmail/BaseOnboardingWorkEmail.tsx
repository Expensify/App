...
function BaseOnboardingWorkEmail({shouldUseNativeStyles}: BaseOnboardingWorkEmailProps) {
    ...
    useEffect(() => {
        ...
        if (isVsb) {
            Navigation.navigate(createDynamicRoute(DYNAMIC_ROUTES.ONBOARDING_ACCOUNTING.path, ROUTES.ONBOARDING_WORK_EMAIL.route), {forceReplace: true});
            return;
        }

        if (isSmb) {
            Navigation.navigate(createDynamicRoute(DYNAMIC_ROUTES.ONBOARDING_EMPLOYEES.path, ROUTES.ONBOARDING_WORK_EMAIL.route), {forceReplace: true});
            return;
        }

        if (!onboardingValues?.isMergeAccountStepSkipped) {
            Navigation.navigate(createDynamicRoute(DYNAMIC_ROUTES.ONBOARDING_PRIVATE_DOMAIN.path, ROUTES.ONBOARDING_WORK_EMAIL.route), {forceReplace: true});
            return;
        }
