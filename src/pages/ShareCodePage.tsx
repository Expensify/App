...
function ShareCodePage({report, policy, backTo}: ShareCodePageProps) {
    ...
    const subtitle = useMemo(() => {
        if (isReport) {
            ...
            return getParentNavigationSubtitle(report, translate, isParentReportArchived, undefined).workspaceName ?? getChatRoomSubtitle(report, false, isReportArchived);
        }

        return currentUserPersonalDetails.login;
    }, [report, currentUserPersonalDetails.login, isReport, isReportArchived, isParentReportArchived, formatPhoneNumber, translate]);
