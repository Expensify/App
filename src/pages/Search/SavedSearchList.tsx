...
type SavedSearchMenuItemBuilderParams = {
  ...
  getOverflowMenu: (
    ...
    itemHash: number,
    itemQuery: string,
  ) => ReturnType<typeof getOverflowMenuUtil>;
...
function SavedSearchList({ hash }: SavedSearchListProps) {
  ...
  const [hasSeededMyExpensesSearch] = useOnyx(
  ...
  useEffect(() => {
    ...
    for (const policy of Object.values(allPolicies ?? {})) {
      ...
      shouldShowSubmitSuggestion =
        shouldShowSubmitSuggestion || isGroupPolicy(policy);

      const isPaidPolicy = isPaidGroupPolicy(policy);
      const isSubmittedTo = Object.values(policy.employeeList ?? {}).some(
        (employee) =>
          employee.submitsTo === currentUserEmail ||
          employee.forwardsTo === currentUserEmail,
      );
      const isUserApprover = isPolicyApprover(policy, currentUserEmail);
      const isApprovalEnabled = policy.approvalMode
        ? policy.approvalMode !== CONST.POLICY.APPROVAL_MODE.OPTIONAL
        : false;
      const isPolicyEligibleForApprove =
        isPaidPolicy && isApprovalEnabled && (isUserApprover || isSubmittedTo);
      shouldShowApproveSuggestion =
        shouldShowApproveSuggestion || isPolicyEligibleForApprove;
    }

    if (shouldShowSubmitSuggestion && shouldShowApproveSuggestion) {
      seedMyExpensesSearch(
        currentUserAccountID,
        translate("search.mySavedSearch"),
      );
    }
  }, [
    hasSeededMyExpensesSearch,
  ...
  const cardsForSavedSearchDisplay = mergeCardListWithWorkspaceFeeds(
    workspaceCardList ?? CONST.EMPTY_OBJECT,
    cardList,
  );
        ...
        buildSavedSearchMenuItem({
          ...
          title:
            item.name === item.query
              ? (savedSearchTitles.get(item.query) ?? item.name)
              : item.name,
          getOverflowMenu,
          itemStyle,
