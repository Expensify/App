...
function SearchPage({ route }: SearchPageProps) {
  ...
  const { lastSearchType, currentSearchResults, shouldUseLiveData } =
    useSearchResultsContext();
  ...
  const [hasFilterBars = false] = useOnyx(
    ONYXKEYS.FORMS.SEARCH_ADVANCED_FILTERS_FORM,
    { selector: hasFilterBarsSelector },
  );

  const [lastNonEmptySearchResults, setLastNonEmptySearchResults] = useState<
  ...
  if (
    ...
    !shouldUseLiveData &&
    currentSearchResults !== lastNonEmptySearchResults
  ) {
  ...
  useEffect(() => {
    ...
    setLastSearchType,
    currentSearchResults?.search?.type,
  ]);
  ...
  const [searchRequestResponseStatusCode, setSearchRequestResponseStatusCode] =
  ...
  const handleSearchAction = useCallback((value: SearchParams | string) => {
    if (typeof value === "string") {
      ...
      search(value)
        ...
        .catch(() => {
          // Handle rejected promise to prevent unhandled rejection crashes
          setSearchRequestResponseStatusCode(null);
        });
    }
  }, []);
  ...
  const overlayContentContainerStyle = !isMobileSelectionModeEnabled
    ? styles.searchListContentContainerStyles(!!hasFilterBars)
    : undefined;
  const overlayEndSubmitSpans = useEndSubmitNavigationSpans();
  const { searchOverlayContent, onSearchContentReady, isOverlayActive } =
    useSearchOverlay({
      searchResults,
