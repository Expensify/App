...
import filterConfig from './SearchAdvancedFiltersPage/FiltersConfig';
...
function SearchAdvancedFiltersPage() {
    ...
    // Create sections dynamically from filterConfig
    const sectionMap = new Map<string, SectionType['items']>();
    
    filterConfig.forEach((filter) => {
        filter.section.forEach((sectionKey) => {
            if (!sectionMap.has(sectionKey)) {
                sectionMap.set(sectionKey, []);
            }
            sectionMap.get(sectionKey)?.push({
                key: filter.key,
                title: translate(filter.displayText),
                description: filter.displayText,
                onPress: () => {
