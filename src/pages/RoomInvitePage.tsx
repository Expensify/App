...
import {isPolicyEmployee} from '@libs/PolicyUtils';
...
function RoomInvitePage({
    ...
    const isPolicyEmployeeValue = useMemo(() => (report?.policyID ? isPolicyEmployee(report.policyID, policies as Record<string, Policy>) : false), [report?.policyID, policies]);
    const backRoute = useMemo(() => {
        if (role === CONST.IOU.SHARE.ROLE.ACCOUNTANT) {
            return ROUTES.REPORT_WITH_ID.getRoute(reportID);
        }
        return reportID && (isPolicyEmployeeValue ? ROUTES.ROOM_MEMBERS.getRoute(reportID, backTo) : ROUTES.REPORT_WITH_ID_DETAILS.getRoute(reportID, backTo));
    }, [isPolicyEmployeeValue, reportID, role, backTo]);
