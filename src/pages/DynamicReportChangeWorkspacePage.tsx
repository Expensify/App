...
function DynamicReportChangeWorkspacePage({report}: DynamicReportChangeWorkspacePageProps) {
    ...
    const submitterLoginSelector = useCallback(personalDetailsLoginSelector(report?.ownerAccountID), [report?.ownerAccountID]);
    const [submitterLogin] = useOnyx(ONYXKEYS.PERSONAL_DETAILS_LIST, {selector: submitterLoginSelector});
    const managerLoginSelector = useCallback(personalDetailsLoginSelector(report?.managerID), [report?.managerID]);
    const [managerLogin] = useOnyx(ONYXKEYS.PERSONAL_DETAILS_LIST, {selector: managerLoginSelector});
