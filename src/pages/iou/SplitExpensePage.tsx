...
function SplitExpensePage({route}: SplitExpensePageProps) {
    ...
    const draftTransactionReport = getReportOrDraftReport(allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${draftTransaction?.reportID}`]);
    const parentTransactionReport = getReportOrDraftReport(allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${draftTransactionReport?.parentReportID}`]);
