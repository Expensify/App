...
type SplitExpensePageProps = PlatformStackScreenProps<
  ...
  typeof SCREENS.MONEY_REQUEST.SPLIT_EXPENSE
>;
...
function SplitExpenseEditPage({ route }: SplitExpensePageProps) {
  ...
  const currentPolicy = Object.keys(policy?.employeeList ?? {}).length
    ? policy
    : currentSearchResults?.data?.[
        `${ONYXKEYS.COLLECTION.POLICY}${getNonEmptyStringOnyxID(currentReport?.policyID)}`
      ];

  const [policyCategories] = useOnyx(
    `${ONYXKEYS.COLLECTION.POLICY_CATEGORIES}${currentReport?.policyID}`,
    { canBeMissing: false },
  );

  const [policyTags] = useOnyx(
    `${ONYXKEYS.COLLECTION.POLICY_TAGS}${currentReport?.policyID}`,
    { canBeMissing: false },
  );
  ...
  const fetchData = useCallback(() => {
    if (!policyCategories) {
      openPolicyCategoriesPage(
        currentReport?.policyID ?? String(CONST.DEFAULT_NUMBER_ID),
      );
    }
    if (!policyTags) {
      openPolicyTagsPage(
  ...
  const currentAmount =
    transactionDetailsAmount >= 0
      ? Math.abs(Number(splitExpenseDraftTransactionDetails?.amount))
      : Number(splitExpenseDraftTransactionDetails?.amount);
  const currentDescription = getParsedComment(
    Parser.htmlToMarkdown(splitExpenseDraftTransactionDetails?.comment ?? ""),
  );

  const shouldShowCategory =
    !!currentPolicy?.areCategoriesEnabled && !!policyCategories;
  ...
  const isDescriptionRequired = isCategoryDescriptionRequired(
    ...
    currentPolicy?.areRulesEnabled,
  );

  const shouldShowTags =
    !!currentPolicy?.areTagsEnabled &&
  ...
  return (
    <ScreenWrapper testID="SplitExpenseEditPage">
      <FullPageNotFoundView
        shouldShow={!reportID || isEmptyObject(splitExpenseDraftTransaction)}
      >
        <View style={[styles.flex1]}>
          <HeaderWithBackButton
            title={translate("iou.splitExpenseEditTitle", {
              amount: convertToDisplayString(
                currentAmount,
