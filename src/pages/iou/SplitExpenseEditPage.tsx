...
function SplitExpenseEditPage({route}: SplitExpensePageProps) {
    ...
    const isTaxEnabled = isTaxTrackingEnabled(true, effectivePolicy, isDistance);
    ...
    const shouldShowBillable = !(effectivePolicy?.disabledFields?.defaultBillable ?? true);
    const shouldShowReimbursable = effectivePolicy?.disabledFields?.reimbursable !== true;
    ...
    const toggleReimbursable = (value: boolean) => {
        setDraftSplitTransaction(CONST.IOU.OPTIMISTIC_TRANSACTION_ID, splitExpenseDraftTransaction, {reimbursable: value}, effectivePolicy);
    };

    const toggleBillable = (value: boolean) => {
        setDraftSplitTransaction(CONST.IOU.OPTIMISTIC_TRANSACTION_ID, splitExpenseDraftTransaction, {billable: value}, effectivePolicy);
    };
                        ...
                        {shouldShowTax && (
                            <>
                                <MenuItemWithTopDescription
                                    ...
                                    description={effectivePolicy?.taxRates?.name ?? translate('common.tax')}
                                    title={taxRateTitle}
                                    numberOfLinesTitle={2}
                                    onPress={() => {
                                        Navigation.navigate(
