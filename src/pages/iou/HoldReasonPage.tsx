...
import {isMoneyRequestAction as isMoneyRequestActionUtils} from '@libs/ReportActionsUtils';
import {canEditMoneyRequest as canEditMoneyRequestUtils, isReportInGroupPolicy as isReportInGroupPolicyUtils} from '@libs/ReportUtils';
...
function HoldReasonPage({route}: HoldReasonPageProps) {
    ...
    const isWorkspaceRequest = isReportInGroupPolicyUtils(report);
    const parentReportAction = ancestors.at(-1)?.reportAction;
    ...
    const onSubmit = (values: FormOnyxValues<typeof ONYXKEYS.FORMS.MONEY_REQUEST_HOLD_FORM>) => {
        ...
        if (isMoneyRequestActionUtils(parentReportAction) && !canEditMoneyRequestUtils(parentReportAction) && isWorkspaceRequest) {
            return;
        }

        putOnHold(transactionID, values.comment, ancestors, reportID, searchHash);
