...
type IOURequestStepConfirmationIncomingRouteName =
  | typeof SCREENS.MONEY_REQUEST.STEP_CONFIRMATION
  | typeof SCREENS.MONEY_REQUEST.CREATE;

type StepConfirmationParams =
  MoneyRequestNavigatorParamList[typeof SCREENS.MONEY_REQUEST.STEP_CONFIRMATION];

type IOURequestStepConfirmationProps =
  WithWritableReportOrNotFoundProps<IOURequestStepConfirmationIncomingRouteName> &
...
function IOURequestStepConfirmation({
  ...
  const participantsAutoAssignedFromRoute =
    route.name === SCREENS.MONEY_REQUEST.STEP_CONFIRMATION
      ? (params as StepConfirmationParams).participantsAutoAssigned
      : undefined;
  ...
  const [existingTransaction, existingTransactionResult] = useOnyx(
    `${ONYXKEYS.COLLECTION.TRANSACTION}${getNonEmptyStringOnyxID(currentTransactionID)}`,
  );
  const [optimisticTransaction, optimisticTransactionResult] = useOnyx(
    `${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${getNonEmptyStringOnyxID(currentTransactionID)}`,
  );
  const isLoadingCurrentTransaction = isLoadingOnyxValue(
  ...
  const transaction = useMemo(
    () =>
      !isLoadingCurrentTransaction
        ? (optimisticTransaction ?? existingTransaction)
        : undefined,
  ...
  const [policyDraft] = useOnyx(
    `${ONYXKEYS.COLLECTION.POLICY_DRAFTS}${draftPolicyID}`,
  );
  ...
  const reportWithDraftFallback = useMemo(
    () => reportReal ?? reportDraft,
    [reportDraft, reportReal],
  );
  const shouldHideToSection = useMemo(
    () => isMoneyRequestReport(reportWithDraftFallback),
    [reportWithDraftFallback],
  );
  ...
  const [policyCategoriesDraft] = useOnyx(
    `${ONYXKEYS.COLLECTION.POLICY_CATEGORIES_DRAFT}${draftPolicyID}`,
  );
  ...
  const isNewManualExpenseFlowEnabled = isBetaEnabled(
    CONST.BETAS.NEW_MANUAL_EXPENSE_FLOW,
  );
  ...
  const [lastLocationPermissionPrompt] = useOnyx(
  ...
  const isTestTransaction = transaction?.participants?.some((participant) =>
  ...
  const gpsRequired =
    ...
    Object.values(receiptFiles).length &&
    !isTestTransaction &&
      ...
      transaction?.participants?.map((participant) => {
        ...
        const privateIsArchived =
          privateIsArchivedMap[
            `${ONYXKEYS.COLLECTION.REPORT_NAME_VALUE_PAIRS}${participant.reportID}`
          ];
  ...
  const defaultParticipants = useMemo(() => {
    const hasSelectedParticipants = (transaction?.participants ?? []).some(
    ...
    let defaultParticipantsOptions = getMoneyRequestParticipantsFromReport(
      sourceReport,
      currentUserPersonalDetails.accountID,
    ).filter((participant) => participant.selected);

    const isGlobalCreateFlow =
    ...
    if (!defaultParticipantsOptions.length && isGlobalCreateFlow) {
      ...
      if (canUseDefaultPolicy) {
        ...
        const defaultTargetReport = shouldAutoReport
          ? getPolicyExpenseChat(
              currentUserPersonalDetails.accountID,
              defaultExpensePolicy?.id,
            )
          : selfDMReport;
        defaultParticipantsOptions = getMoneyRequestParticipantsFromReport(
          defaultTargetReport,
          currentUserPersonalDetails.accountID,
        ).filter((participant) => participant.selected);
      }
  ...
  const shouldAutoOpenParticipantPicker = useMemo(() => {
    ...
    return (
      !hasTransactionParticipants &&
      !hasDefaultParticipants &&
  ...
  const [
    manuallyOpenedParticipantPickerForTransactionID,
    setManuallyOpenedParticipantPickerForTransactionID,
  ] = useState<string | undefined>();
  const [
    dismissedAutoOpenParticipantPickerForTransactionID,
    setDismissedAutoOpenParticipantPickerForTransactionID,
  ] = useState<string | undefined>();
  const participantPickerIOUType =
  ...
  const isParticipantPickerVisible =
    ...
    (manuallyOpenedParticipantPickerForTransactionID === activeTransactionID ||
      (shouldAutoOpenParticipantPicker &&
        dismissedAutoOpenParticipantPickerForTransactionID !==
          activeTransactionID));
  ...
  const isPolicyExpenseChat = useMemo(() => {
    const hasPolicyExpenseChat = (
      participantList: typeof defaultParticipants,
    ) =>
      participantList.some((participant) => {
        ...
        return (
          ...
          isPolicyExpenseChatUtils(getReportOrDraftReport(participant.reportID))
        );
      });
  ...
  const isFromGlobalCreate =
    transaction?.isFromGlobalCreate === true ||
    transaction?.isFromFloatingActionButton === true;
  ...
  useFetchRoute(
    ...
    shouldUseTransactionDraft(action, iouType)
      ? CONST.TRANSACTION.STATE.DRAFT
      : CONST.TRANSACTION.STATE.CURRENT,
  );

  const policyExpenseChatPolicyID =
    transaction?.participants?.find(
      (participant) => participant?.isPolicyExpenseChat,
    )?.policyID ??
    defaultParticipants.find((participant) => participant?.isPolicyExpenseChat)
      ?.policyID ??
    (isPolicyExpenseChatUtils(report) ? report?.policyID : undefined);

  const senderPolicyID = transaction?.participants?.find(
    (participant) =>
      !!participant && "isSender" in participant && participant.isSender,
  )?.policyID;
  ...
  const { hasVerifiedBlobs } = useRestartOnOdometerImagesFailure(
    isOdometerDistanceRequest ? transaction : undefined,
    reportID,
    iouType,
    backToReport,
  );
  ...
  const canPreInsertSearch =
    iouType !== CONST.IOU.TYPE.PAY &&
    iouType !== CONST.IOU.TYPE.SPLIT &&
    !(isPerDiemRequest && iouType === CONST.IOU.TYPE.TRACK);
  ...
  // other tabs.
  const canDismissFromSearch =
    iouType !== CONST.IOU.TYPE.PAY && iouType !== CONST.IOU.TYPE.SPLIT;

  const hasPreInsertFired = useRef(false);
  const isTransactionReady = !!transaction;
  const selfDMReportID =
    iouType === CONST.IOU.TYPE.TRACK ? findSelfDMReportID() : undefined;
  const destinationReportID =
    backToReport ?? report?.reportID ?? selfDMReportID;
  ...
  useEffect(() => {
    ...
    const isSearchOnTopWithDifferentType =
      isSearchTopmostFullScreenRoute() &&
      getCurrentSearchQueryJSON()?.type !== searchType;
    ...
    const canUseReportPreInsert =
      ...
      (isReportTopmostSplitNavigator() ||
        (!isFromGlobalCreate && !isSearchTopmostFullScreenRoute()));
    ...
    return () => {
      ...
      if (
        !Navigation.getIsFullscreenPreInsertedUnderRHP() ||
        formHasBeenSubmitted.current
      ) {
        return;
      }
  ...
  const handleSendMoney = useCallback(
    (paymentMethod: PaymentMethodType | undefined) => {
      ...
      const resolvedReportIDs = resolveOptimisticChatReportID(
        [
      ...
      const payDestinationReportID =
        destinationReportID ?? resolvedReportIDs.chatReportID;
      if (
        !payDestinationReportID ||
        Navigation.getTopmostReportId() === payDestinationReportID
      ) {
  ...
  const navigateBack = useCallback(() => {
    ...
    if (isPerDiemRequest) {
      ...
      Navigation.goBack(
        ROUTES.MONEY_REQUEST_STEP_SUBRATE.getRoute(
          action,
          iouType,
    ...
    if (
      ...
      Navigation.goBack(
        ROUTES.TEST_DRIVE_MODAL_ROOT.getRoute(
          transaction.participants?.at(0)?.login,
        ),
      );
      return;
    }
  ...
  const { validateFiles, PDFValidationComponent, ErrorModal } =
    useFilesValidation(setReceiptOnDrop);
  ...
  const showReceiptEmptyState = shouldShowReceiptEmptyState(
    iouType,
    action,
    policy,
    isPerDiemRequest,
  ...
  return (
    ...
    >
      ...
      />
      <DragAndDropProvider
        isDisabled={!showReceiptEmptyState || isOdometerDistanceRequest}
      >
        <View style={styles.flex1}>
            ...
            <HeaderWithBackButton
              ...
              subtitle={
                hasMultipleTransactions
                  ? `${currentTransactionIndex + 1} ${translate("common.of")} ${transactions.length}`
                  : undefined
              }
          ...
          <DragAndDropConsumer onDrop={handleDroppingReceipt}>
            <DropZoneUI
              icon={
                isEditingReceipt
                  ? expensifyIcons.ReplaceReceipt
                  : expensifyIcons.SmartScan
              }
              dropStyles={styles.receiptDropOverlay(true)}
              dropTitle={translate(
                isEditingReceipt
                  ? "dropzone.replaceReceipt"
                  : "quickAction.scanReceipt",
              )}
              dropTextStyles={styles.receiptDropText}
              dashedBorderStyles={[
                styles.dropzoneArea,
                styles.easeInOpacityTransition,
                styles.activeDropzoneDashedBorder(
                  theme.receiptDropBorderColorActive,
                  true,
                ),
              ]}
            />
          ...
          >
            {({ onConfirm, isConfirming }) => (
              <MoneyRequestConfirmationList
                ...
                isLoadingReceipt={
                  isStitchingReceipt ||
                  (isOdometerDistanceRequest && !hasVerifiedBlobs)
                }
                isPerDiemRequest={isPerDiemRequest}
...
const IOURequestStepConfirmationWithFullTransactionOrNotFound =
  withFullTransactionOrNotFound(IOURequestStepConfirmation);

const IOURequestStepConfirmationWithWritableReportOrNotFound =
  withWritableReportOrNotFound(
    IOURequestStepConfirmationWithFullTransactionOrNotFound,
  );

type IOURequestStepConfirmationPublicProps = ComponentProps<
  typeof IOURequestStepConfirmationWithWritableReportOrNotFound
>;
