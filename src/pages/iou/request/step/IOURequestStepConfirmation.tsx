...
import MoneyRequestConfirmationList, {type MoneyRequestConfirmationListProps} from '@components/MoneyRequestConfirmationList';
...
import {getParticipantsOption, getReportOption, getReportOptionForTransactionReport} from '@libs/OptionsListUtils';
import Performance from '@libs/Performance';
import {isPaidGroupPolicy} from '@libs/PolicyUtils';
import {generateReportID, getReport, getReportOrDraftReport, isProcessingReport, isReportOutstanding, isSelectedManagerMcTest} from '@libs/ReportUtils';
import {
    getAttendees,
    getDefaultTaxCode,
    getRateID,
...
import {openDraftWorkspaceRequest, clearDraftWorkspaceRequest} from '@userActions/Policy/Policy';
import {removeDraftTransaction, removeDraftTransactions, replaceDefaultDraftTransaction, updateDraftTransaction} from '@userActions/TransactionEdit';
...
import type {WithWritableReportOrNotFoundProps} from '@components/withWritableReportOrNotFound';
import withWritableReportOrNotFound from '@components/withWritableReportOrNotFound';
    checkIfScanFileCanBeRead,
    createDistanceRequest as createDistanceRequestIOUActions,
    getIOURequestPolicyID,
    getReceiverType,
...
const isPerDiemRequest = requestType === CONST.IOU.REQUEST_TYPE.PER_DIEM;
    ...
    useEffect(() => {
        ...
        const isCurrentReportID = transaction?.isFromGlobalCreate
            ? transaction?.participants?.at(0)?.reportID === reportID
    ...
    const navigateBack = useCallback(() => {
        ...
        if (transaction?.isFromGlobalCreate && !transaction.receipt?.isTestReceipt) {
            ...
            if (!transaction?.participantsAutoAssigned && participantsAutoAssignedFromRoute !== 'true') {
                ...
                Navigation.goBack(ROUTES.MONEY_REQUEST_STEP_PARTICIPANTS.getRoute(iouType, initialTransactionID, reportID, undefined, action), {
                    compareParams: false,
                });
                return;
            }
        ...
        (selectedParticipants: Participant[], gpsPoints?: GpsPoint) => {
            ...
            transactions.forEach((item, index) => {
                ...
                trackExpenseIOUActions({
                    report: item.reportID ? ReportConnection.getAllReports()?.[`${ONYXKEYS.COLLECTION.REPORT}${item.reportID}`] : report,
                    isDraftPolicy,
                    action,
                    participantParams: {
                        payeeEmail: currentUserPersonalDetails.login,
...
(selectedParticipants: Participant[], trimmedComment: string) => {
            ...
            createDistanceRequestIOUActions({
                report: reportID ? reports?.[`${ONYXKEYS.COLLECTION.REPORT}${reportID}`] : report,
            ...
            if (iouType === CONST.IOU.TYPE.SPLIT && !transaction?.isFromGlobalCreate) {
                if (currentUserPersonalDetails.login && !!transaction) {
                    splitBill({
                        ...
                        category: transaction.category,
                        tag: transaction.tag,
                        existingSplitChatReportID: transaction.reportID ?? report?.reportID,
                        billable: transaction.billable,
                        reimbursable: transaction.reimbursable,
...
{hasMultipleTransactions ? (
                ...
                />
                        ...
                        receiptPath={receiptPath}
                        receiptFilename={receiptFilename}
I need to see more context of the file to understand what modifications are needed. However, based on the issue description, the problem is that when submitting an expense to a workspace chat from self DM, the selected report is not being obeyed - it submits to the wrong report.

Without seeing the full file context and the withWritableReportOrNotFound HOC implementation, I cannot make a surgical modification to just the export statement. The export line itself (`export default IOURequestStepConfirmationWithWritableReportOrNotFound;`) is unlikely to be the source of the bug.

The issue likely lies in:
1. How the report selection is being handled in the component
2. How the reportID is being passed or stored when the user changes the report
3. The logic in the confirmation step that determines which report to submit to

Could you provide:
1. More context from the file (the component implementation)
2. The actual chunk that needs modification (the export line alone cannot fix this issue)
3. Or clarification on what specific change is needed to this export statement

Based solely on what's provided:
