...
// Submit-only Onyx values cached at module scope via Onyx.connectWithoutView.
// These are consumed exclusively inside `submitAmount` and `getIsP2PForAmount`, never during
// render. useOnyx would register a subscriber on every IOURequestStepAmount mount inside the
// ManualOpenCreateExpense span window for no render benefit; connectWithoutView registers once
// at module load instead.
// If React components ever need any of these values, switch them back to useOnyx.
...
function submitAmount({
    ...
    const navigateToNextPage = () => {
        ...
        if (report?.reportID && !isReportArchived && iouType !== CONST.IOU.TYPE.CREATE) {
            ...
            if (shouldSkipConfirmation) {
                ...
                const draftTransactionIDsList = Object.keys(validTransactionDraftsSelector(allTransactionDrafts) ?? {});
    ...
    // Duplicate-tx state is needed only when editing the amount of an existing transaction.
    // Compute on demand from cached collections, avoiding two full-collection subscriptions on
    // the create path.
