...
import {getRequestType, isCorporateCardTransaction, isPerDiemRequest, isSplitChildTransaction, isTimeRequest as isTimeRequestUtil} from '@libs/TransactionUtils';
...
function IOURequestStepParticipants({
    ...
    const isSubmittingSplitExpense = action === CONST.IOU.ACTION.SUBMIT && isSplitChildTransaction(initialTransaction);
    // In new flow - the amount step is skipped, so we need to include the recents for all the cases.
    const isWorkspacesOnly = isSubmittingSplitExpense || (!isNewManualExpenseFlowEnabled && getIsWorkspacesOnlyForTransaction(initialTransaction, iouRequestType));
