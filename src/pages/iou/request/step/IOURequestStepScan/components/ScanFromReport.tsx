...
} from "@libs/telemetry/ReceiptObservability";
...
function ScanFromReport({
  ...
  const { validateFiles, PDFValidationComponent, ErrorModal } =
    useFilesValidation((files: FileObject[]) => {
    ...
    <>
      ...
      <Camera
        onCapture={(file, source) => {
          ...
          precacheReceiptImage(source).then(() =>
            processReceipts([file], "camera"),
          );
        }}
