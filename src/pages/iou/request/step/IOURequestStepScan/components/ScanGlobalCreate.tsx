...
function ScanGlobalCreate({
  ...
  const navigateGlobalCreate = (transactionIDs: string[]) => {
    ...
    if (
      ...
      const targetReport = shouldAutoReport
        ? getPolicyExpenseChat(
            currentUserPersonalDetails.accountID,
            defaultExpensePolicy?.id,
          )
        : selfDMReport;
      const transactionReportID = isSelfDM(targetReport)
        ? CONST.REPORT.UNREPORTED_REPORT_ID
        : targetReport?.reportID;
      const iouTypeTrackOrSubmit =
        transactionReportID === CONST.REPORT.UNREPORTED_REPORT_ID
      ...
      if (
        ...
        transaction.participants.at(0)?.reportID !== targetReport?.reportID
      ) {
        const isTrackExpense =
          transaction.participants.at(0)?.reportID === selfDMReport?.reportID;

        const setParticipantsPromises = transactionIDs.map((tid) =>
          setMoneyRequestParticipants(tid, transaction.participants),
        );
