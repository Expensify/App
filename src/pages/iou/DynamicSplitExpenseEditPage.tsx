...
type DynamicSplitExpenseEditPageProps = PlatformStackScreenProps<
  SplitExpenseParamList,
  typeof SCREENS.MONEY_REQUEST.DYNAMIC_SPLIT_EXPENSE_EDIT
>;

function DynamicSplitExpenseEditPage({
  route,
}: DynamicSplitExpenseEditPageProps) {
  const styles = useThemeStyles();
  const backPath = useDynamicBackPath(DYNAMIC_ROUTES.SPLIT_EXPENSE_EDIT.path);
  const { isOffline } = useNetwork();
  const { translate, toLocaleDigit } = useLocalize();
  const { convertToDisplayString, getCurrencySymbol } =
  ...
  const [splitExpenseDraftTransaction] = useOnyx(
    `${ONYXKEYS.COLLECTION.SPLIT_TRANSACTION_DRAFT}${CONST.IOU.OPTIMISTIC_TRANSACTION_ID}`,
  );
  const [originalTransactionDraft] = useOnyx(
    `${ONYXKEYS.COLLECTION.SPLIT_TRANSACTION_DRAFT}${splitExpenseDraftTransaction?.comment?.originalTransactionID}`,
    undefined,
    [splitExpenseDraftTransaction?.comment?.originalTransactionID],
  );

  const splitExpenseDraftTransactionDetails = useMemo<
    Partial<TransactionDetails>
  >(
    () => getTransactionDetails(splitExpenseDraftTransaction) ?? {},
  ...
  const transaction =
    allTransactions?.[
      `${ONYXKEYS.COLLECTION.TRANSACTION}${getNonEmptyStringOnyxID(transactionID)}`
    ];
  const originalTransaction =
    allTransactions?.[
      `${ONYXKEYS.COLLECTION.TRANSACTION}${getNonEmptyStringOnyxID(transaction?.comment?.originalTransactionID)}`
    ];

  const report = useReportOrReportDraft(reportID);
  const parentReport =
    allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${report?.parentReportID}`];
  const currentReport =
    report ??
    currentSearchResults?.data?.[
      `${ONYXKEYS.COLLECTION.REPORT}${getNonEmptyStringOnyxID(reportID)}`
    ];

  const personalPolicy = usePersonalPolicy();
  const effectivePolicy = useSplitEffectivePolicy(
    currentReport,
    splitExpenseDraftTransaction,
    transaction,
  );
  ...
  const { policyForMovingExpenses, shouldSelectPolicy } =
    usePolicyForMovingExpenses();
  const shouldNavigateToUpgradePath =
    !policyForMovingExpenses && !shouldSelectPolicy;

  const effectivePolicyID = effectivePolicy?.id;

  const [policyCategories] = useOnyx(
    `${ONYXKEYS.COLLECTION.POLICY_CATEGORIES}${effectivePolicyID}`,
  );

  const [policyTags] = useOnyx(
    `${ONYXKEYS.COLLECTION.POLICY_TAGS}${effectivePolicyID}`,
  );
  const { login, accountID: currentUserAccountID } =
    useCurrentUserPersonalDetails();

  const fetchData = useCallback(() => {
    if (!policyCategories) {
      openPolicyCategoriesPage(
        effectivePolicyID ?? String(CONST.DEFAULT_NUMBER_ID),
      );
    }
    if (!policyTags) {
  ...
  const [draftTransactionWithSplitExpenses] = useOnyx(
    `${ONYXKEYS.COLLECTION.SPLIT_TRANSACTION_DRAFT}${transactionID}`,
  );
  const splitExpensesList =
    draftTransactionWithSplitExpenses?.comment?.splitExpenses;

  const splitExpenseItem = splitExpensesList?.find(
    (item) => item.transactionID === splitExpenseTransactionID,
  );
  const originalSign = (splitExpenseItem?.amount ?? 0) < 0 ? -1 : 1;
  const currentDescription = getParsedComment(
    Parser.htmlToMarkdown(splitExpenseDraftTransactionDetails?.comment ?? ""),
  );

  const draftTransactionReport = getReportOrDraftReport(
    splitExpenseDraftTransaction?.reportID,
  );
  const isSelfDMSplit = isSelfDM(draftTransactionReport);
  const isExpenseUnreported = isSelfDMSplit;
  const [draftTransactionPolicyType] = useOnyx(
    `${ONYXKEYS.COLLECTION.POLICY}${draftTransactionReport?.policyID}`,
    {
  ...
  const categoryForDisplay = isCategoryMissing(transactionCategory)
    ? ""
    : transactionCategory;
  const hasOriginalCategory =
    !!originalTransactionCategory &&
    !isCategoryMissing(originalTransactionCategory);
  const shouldShowCategory =
    hasOriginalCategory ||
    (isPolicyExpenseChat &&
      (!!categoryForDisplay || hasEnabledOptions(policyCategories ?? {}))) ||
    (isExpenseUnreported &&
      (!effectivePolicy || hasEnabledOptions(policyCategories ?? {})));
  ...
  const isCategoryRequired =
    !!effectivePolicy?.requiresCategory && !isSelfDMSplit;
  const reportAttributes = useReportAttributes();
  const reportName =
    getReportName(currentReport, reportAttributes) || parentReport?.reportName;
  ...
  const shouldShowTags =
    !!originalTransactionTag ||
    ((isPolicyExpenseChat || isExpenseUnreported) &&
      !!(transactionTag || hasEnabledTags(policyTagLists)));
  ...
  const previousTagsVisibility =
    usePrevious(tagVisibility.map((v) => v.shouldShow)) ?? [];

  const isDistance = isDistanceRequest(splitExpenseDraftTransaction);
  const isManualDistance = isManualDistanceRequest(
    splitExpenseDraftTransaction,
  );
  const isOdometerDistance = isOdometerDistanceRequest(
    splitExpenseDraftTransaction,
  );
  const {
    unit,
  ...
  const currentAmount =
    isDistance && distance && rate
      ? DistanceRequestUtils.getDistanceRequestAmount(distance, unit, rate) *
        originalSign
      : Math.abs(Number(splitExpenseDraftTransaction?.amount)) * originalSign;
  const distanceToDisplay = DistanceRequestUtils.getDistanceForDisplay(
    true,
    distance,
    unit,
    rate,
  ...
  const rates = DistanceRequestUtils.getMileageRates(
    effectivePolicy,
    false,
    currentRateID,
  );
  ...
  const currency =
    splitExpenseDraftTransactionDetails.currency ?? CONST.CURRENCY.USD;
  ...
  const merchantToDisplay =
    isDistance && distance && rate && unit
      ? DistanceRequestUtils.getDistanceMerchant(
          true,
          distance,
          unit,
          rate,
  ...
  const rawPolicyRate =
    !isP2PRate && currentRateID && effectivePolicy
      ? getDistanceRateCustomUnitRate(effectivePolicy, currentRateID)
      : undefined;
  const isRateBroken =
    isDistance &&
    !isP2PRate &&
    (!rates[currentRateID] ||
      !rate ||
      rawPolicyRate?.pendingAction ===
        CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE ||
      rawPolicyRate?.enabled === false);
  const hasAvailableEnabledRates =
    Object.keys(DistanceRequestUtils.getMileageRates(effectivePolicy)).length >
    0;
  ...
  return (
    <ScreenWrapper testID="DynamicSplitExpenseEditPage">
      <FullPageNotFoundView
        shouldShow={
          !reportID ||
          isEmptyObject(splitExpenseDraftTransaction) ||
          !isSplitAvailable
        }
      >
        <View style={[styles.flex1]}>
          <HeaderWithBackButton
            title={translate(
              ...
              convertToDisplayString(
                currentAmount,
                splitExpenseDraftTransactionDetails?.currency,
              ),
              merchantToDisplay,
            )}
          ...
          <ScrollView>
            <MenuItemWithTopDescription
              ...
              rightLabel={
                isDescriptionRequired ? translate("common.required") : ""
              }
            />
            ...
            {shouldShowCategory && (
              <MenuItemWithTopDescription
                ...
                title={getDecodedLeafCategoryName(
                  splitExpenseDraftTransactionDetails?.category ?? "",
                )}
                numberOfLinesTitle={2}
                rightLabel={
                  isCategoryRequired ? translate("common.required") : ""
                }
                onPress={() => {
                  const categoryRoute =
                  ...
                  if (!effectivePolicy && shouldSelectPolicy) {
                    Navigation.navigate(
                      ROUTES.SET_DEFAULT_WORKSPACE.getRoute(categoryRoute),
                    );
                    return;
                  }
            ...
            {shouldShowTags &&
              policyTagLists.map(({ name }, index) => {
                ...
                const prevShouldShow =
                  previousTagsVisibility.at(index) ?? false;
                ...
                return (
                  <MenuItemWithTopDescription
                    ...
                    key={name}
                    highlighted={
                      !getTagForDisplay(splitExpenseDraftTransaction, index) &&
                      !prevShouldShow
                    }
                    title={getTagForDisplay(
                      splitExpenseDraftTransaction,
                      index,
                    )}
                    description={name}
                    ...
                    numberOfLinesTitle={2}
                    rightLabel={
                      isTagRequired ? translate("common.required") : ""
                    }
                    onPress={() => {
                      Navigation.navigate(
          ...
          <FixedFooter style={styles.mtAuto}>
            {Number(splitExpensesList?.length) > 1 && (
              <Button
                ...
                sentryLabel={
                  CONST.SENTRY_LABEL.SPLIT_EXPENSE.REMOVE_SPLIT_BUTTON
                }
              />
            )}
            <Button
