...
function TermsStep({onNext}: SubPageProps) {
    ...
    useEffect(() => {
        ...
        /* oxlint-disable-next-line rh/set-state-in-effect */ // eslint-disable-next-line react-hooks/set-state-in-effect -- clears the validation error immediately when both checkboxes become checked, so the error doesn't persist until the next submit attempt
        setError(false);
