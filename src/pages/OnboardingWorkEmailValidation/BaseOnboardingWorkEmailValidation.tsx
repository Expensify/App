...
function BaseOnboardingWorkEmailValidation({shouldUseNativeStyles}: BaseOnboardingWorkEmailValidationProps) {
    ...
    useEffect(() => {
        ...
        if (isVsb) {
            Navigation.navigate(createDynamicRoute(DYNAMIC_ROUTES.ONBOARDING_ACCOUNTING.path, ROUTES.ONBOARDING_WORK_EMAIL_VALIDATION.route), {forceReplace: true});
            return;
        }

        if (isSmb) {
            Navigation.navigate(createDynamicRoute(DYNAMIC_ROUTES.ONBOARDING_EMPLOYEES.path, ROUTES.ONBOARDING_WORK_EMAIL_VALIDATION.route), {forceReplace: true});
            return;
        }
