...
function WorkspaceMemberDetailsPage({personalDetails, policy, route}: WorkspaceMemberDetailsPageProps) {
    ...
    const navigateToDetails = (card: MemberCard) => {
        if (card.bank === CONST.EXPENSIFY_CARD.BANK) {
            Navigation.navigate(
                createDynamicRoute(DYNAMIC_ROUTES.WORKSPACE_EXPENSIFY_CARD_DETAILS.getRoute(card.cardID.toString()), ROUTES.WORKSPACE_MEMBER_DETAILS.getRoute(policyID, accountID)),
            );
            return;
        }
        if (!card.fundID) {
