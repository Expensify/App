...
type DynamicWorkspaceOwnerChangeWrapperPageProps = WithPolicyOnyxProps &
  PlatformStackScreenProps<
    SettingsNavigatorParamList,
    typeof SCREENS.WORKSPACE.DYNAMIC_OWNER_CHANGE_CHECK
  >;

function DynamicWorkspaceOwnerChangeWrapperPage({
  route,
  policy,
  isLoadingPolicy,
}: DynamicWorkspaceOwnerChangeWrapperPageProps) {
  const styles = useThemeStyles();
  const { translate } = useLocalize();
  const backPath = useDynamicBackPath(
    DYNAMIC_ROUTES.WORKSPACE_OWNER_CHANGE_CHECK.path,
  );
  const [privateStripeCustomerID] = useOnyx(
  ...
  const isAuthRequired =
    privateStripeCustomerID?.status ===
    CONST.STRIPE_SCA_AUTH_STATUSES.CARD_AUTHENTICATION_REQUIRED;
  const shouldShowPaymentCardForm =
    error === CONST.POLICY.OWNERSHIP_ERRORS.NO_BILLING_CARD || isAuthRequired;
  ...
  useEffect(() => {
    ...
    if (!policy.errorFields && policy.isChangeOwnerFailed) {
      Navigation.navigate(
        createDynamicRoute(DYNAMIC_ROUTES.WORKSPACE_OWNER_CHANGE_ERROR.path),
        { forceReplace: true },
      );
      return;
    }
    ...
    if (!policy?.errorFields?.changeOwner && policy?.isChangeOwnerSuccessful) {
      Navigation.navigate(
        createDynamicRoute(DYNAMIC_ROUTES.WORKSPACE_OWNER_CHANGE_SUCCESS.path),
        { forceReplace: true },
      );
      return;
    }

    const changeOwnerErrors = Object.keys(
