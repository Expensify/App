...
import {SUBSCRIPTION_PRICES} from '@hooks/useSubscriptionPrice';
...
function UpgradeIntro({feature, onUpgrade, buttonDisabled, loading, isCategorizing}: Props) {
    ...
    const formattedPrice = React.useMemo(() => {
        const upgradeCurrency = Object.keys(SUBSCRIPTION_PRICES).includes(preferredCurrency) ? preferredCurrency : CONST.PAYMENT_CARD_CURRENCY.USD;
        const upgradePlan = !isCategorizing || subscriptionPlan === CONST.POLICY.TYPE.CORPORATE ? CONST.POLICY.TYPE.CORPORATE : CONST.POLICY.TYPE.TEAM;
        return `${convertToShortDisplayString(SUBSCRIPTION_PRICES[upgradeCurrency][upgradePlan][CONST.SUBSCRIPTION.TYPE.ANNUAL], upgradeCurrency)} `;
