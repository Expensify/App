...
import {
  ...
  useLockedAccountState,
} from "@components/LockedAccountModalProvider";
...
function WorkspaceExpensifyCardListPage({
  route,
  cardsList,
  fundID,
}: WorkspaceExpensifyCardListPageProps) {
  ...
  const [cardOnWaitlist] = useOnyx(
    `${ONYXKEYS.COLLECTION.NVP_EXPENSIFY_ON_CARD_WAITLIST}${policyID}`,
  );
  const [cardSettings] = useOnyx(
    `${ONYXKEYS.COLLECTION.PRIVATE_EXPENSIFY_CARD_SETTINGS}${fundID}`,
  );
  const [allPolicies] = useOnyx(ONYXKEYS.COLLECTION.POLICY);
  const [domains] = useOnyx(ONYXKEYS.COLLECTION.DOMAIN);
  const [cardList] = useOnyx(ONYXKEYS.CARD_LIST);
  const settings = getCardSettings(cardSettings);
  const { allFeeds: allAdminExpensifyCardFeeds } =
    useExpensifyCardFeedsForFeedSelector(policyID);
  ...
  const { canWrite: canWriteExpensifyCard, showReadOnlyModal } =
    usePolicyFeatureWriteAccess(
      policy,
      CONST.POLICY.POLICY_FEATURE.EXPENSIFY_CARD,
    );
  const headerHeight = useEmptyViewHeaderHeight(
  ...
  const shouldShowEuUkDisclaimer =
    isCurrencySupportedForECards(settlementCurrency);
  const allCards = useMemo(() => {
    const policyMembersAccountIDs = Object.values(
      getMemberAccountIDsForWorkspace(policy?.employeeList),
    );
  ...
  const selectableCardKeySet = useMemo(
    () => new Set(allCards.map((card) => String(card.cardID))),
    [allCards],
  );
  const validatedSelectedCardKeys = useMemo(
    () => selectedCardKeys.filter((key) => selectableCardKeySet.has(key)),
    [selectedCardKeys, selectableCardKeySet],
  );
  const selectedCardIDs = useMemo(
    () => validatedSelectedCardKeys.map((key) => Number(key)),
    [validatedSelectedCardKeys],
  );

  const clearTableSelection = useCallback(() => {
    setSelectedCardKeys((prevSelectedCardKeys) =>
      prevSelectedCardKeys.length > 0 ? [] : prevSelectedCardKeys,
    );
  }, []);
    ...
    () =>
      allCards.map((card) => {
        const frozenByDisplayName = card.nameValuePairs?.frozen?.byAccountID
          ? temporaryGetDisplayNameOrDefault({
              passedPersonalDetails:
                personalDetails?.[card.nameValuePairs.frozen.byAccountID],
              defaultValue: "",
        ...
        return {
          ...
          cardholder:
            personalDetails?.[card.accountID ?? CONST.DEFAULT_NUMBER_ID],
          ...
          action: () =>
            Navigation.navigate(
              createDynamicRoute(
                DYNAMIC_ROUTES.WORKSPACE_EXPENSIFY_CARD_DETAILS.getRoute(
          ...
          onClose: () =>
            clearDeletePaymentMethodError(
              `${ONYXKEYS.COLLECTION.WORKSPACE_CARDS_LIST}${defaultFundID}_${CONST.EXPENSIFY_CARD.BANK}`,
              card.cardID,
            ),
        };
      }),
  ...
  const bulkExportOptions: Array<
    DropdownOption<typeof CONST.EXPENSIFY_CARD.BULK_ACTIONS.EXPORT_CSV>
  > = [
    {
      icon: icons.Export,
      ...
      onSelected: () => {
        const selectedCards = cardRows
          .filter((row) => validatedSelectedCardKeys.includes(row.keyForList))
          .map((row) => row.card);
        exportExpensifyCardListToCSV({
          policyID,
          cards: selectedCards,
  ...
  const handleIssueCardPress = () => {
    ...
    Navigation.navigate(
      createDynamicRoute(
        DYNAMIC_ROUTES.WORKSPACE_EXPENSIFY_CARD_ISSUE_NEW.path,
        ROUTES.WORKSPACE_EXPENSIFY_CARD.getRoute(policyID),
      ),
    );
  };
    ...
    ? [
        {
          ...
          onSelected: () =>
            Navigation.navigate(
              ROUTES.WORKSPACE_EXPENSIFY_CARD_SETTINGS.getRoute(policyID),
            ),
  ...
  const getHeaderButtons = () => {
    ...
    const shouldShowBulkSelectionDropdown = shouldUseNarrowLayout
      ...
      return (
        <View style={headerButtonsRowStyle}>
          <ButtonWithDropdownMenu<
            typeof CONST.EXPENSIFY_CARD.BULK_ACTIONS.EXPORT_CSV
          >
            success
            onPress={() => {}}
            ...
            sentryLabel={
              CONST.SENTRY_LABEL.WORKSPACE_EXPENSIFY_CARD.BULK_ACTIONS_DROPDOWN
            }
            wrapperStyle={[
              !isInLandscapeMode && styles.flexGrow1,
              shouldDisplayButtonsInSeparateLine && styles.flexShrink1,
            ]}
          />
        </View>
      );
      ...
      <View style={headerButtonsRowStyle}>
        {!isCardListEmpty && (
          <Button
            ...
            innerStyles={
              !canWriteExpensifyCard ? styles.buttonOpacityDisabled : undefined
            }
            hoverStyles={
              !canWriteExpensifyCard ? styles.buttonOpacityDisabled : undefined
            }
            sentryLabel={
              CONST.SENTRY_LABEL.WORKSPACE.EXPENSIFY_CARD.ISSUE_CARD_BUTTON
            }
          />
        )}
        {secondaryActions.length > 0 && (
          <ButtonWithDropdownMenu
            ...
            shouldUseOptionIcon
            wrapperStyle={
              isCardListEmpty && !isInLandscapeMode
                ? styles.flexGrow1
                : styles.flexGrow0
            }
            sentryLabel={
              CONST.SENTRY_LABEL.WORKSPACE.EXPENSIFY_CARD.MORE_DROPDOWN
            }
          />
        )}
      </View>
  ...
  const selectionModeHeader =
    isMobileSelectionModeEnabled && shouldUseNarrowLayout;
  ...
  const disclaimerFooter = (
    <Text
      style={[
        ...
        styles.p5,
        footerHeight === 0 && { opacity: 0 },
      ]}
      onLayout={(event) => setFooterHeight(event.nativeEvent.layout.height)}
    >
      {translate(
        shouldShowEuUkDisclaimer
          ? "workspace.expensifyCard.euUkDisclaimer"
          : "workspace.expensifyCard.disclaimer",
      )}
    </Text>
  );
  ...
  return (
    ...
    >
      <HeaderWithBackButton
        ...
        title={
          selectionModeHeader
            ...
            : translate("workspace.common.expensifyCard")
        }
      ...
      {!shouldShowSelector &&
        ...
        isBankAccountVerified &&
        shouldShowHeaderButtons && (
          <View style={styles.ph5}>{getHeaderButtons()}</View>
        )}
      {shouldShowSelector && (
        <View
          style={[
            ...
            styles.ph5,
            styles.pb3,
            (!shouldChangeLayout || isInLandscapeMode) && [
              styles.flexRow,
        ...
        >
          <FeedSelector
            ...
            onFeedSelect={() =>
              Navigation.navigate(
                createDynamicRoute(
                  DYNAMIC_ROUTES.WORKSPACE_EXPENSIFY_CARD_SELECT_FEED.path,
                ),
              )
            }
            CardFeedIcon={cardFeedIcon}
            feedName={translate("workspace.common.expensifyCard")}
            supportingText={getExpensifyCardFeedDescription(
              cardSettings,
              allPolicies,
              domains,
              fundID,
          ...
          buttons={[
            {
              ...
              success: true,
              innerStyles: !canWriteExpensifyCard
                ? styles.buttonOpacityDisabled
                : undefined,
              hoverStyles: !canWriteExpensifyCard
                ? styles.buttonOpacityDisabled
                : undefined,
            },
          ]}
