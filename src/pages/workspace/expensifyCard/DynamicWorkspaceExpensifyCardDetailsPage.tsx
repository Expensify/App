...
import {
  getSpendRuleByCardID,
  getSpendRuleSummaryText,
} from "@libs/SpendRulesUtils";

import Navigation from "@navigation/Navigation";

import NotFoundPage from "@pages/ErrorPage/NotFoundPage";
import CardDetailsActionButtons, {
  CardDetailsActionButton,
} from "@pages/settings/Wallet/CardDetailsActionButtons";
...
import {
  ...
  freezeCard as freezeCardAction,
  openCardDetailsPage,
  unfreezeCard as unfreezeCardAction,
} from "@userActions/Card";
...
type DynamicWorkspaceExpensifyCardDetailsPageProps = PlatformStackScreenProps<
  ...
  | typeof SCREENS.WORKSPACE.DYNAMIC_EXPENSIFY_CARD_DETAILS
  | typeof SCREENS.EXPENSIFY_CARD.DYNAMIC_EXPENSIFY_CARD_DETAILS
>;

type LimitHintTranslationKey =
  | "cardPage.smartLimit.title"
  | "cardPage.monthlyLimit.title"
  | "cardPage.fixedLimit.title";

function getLimitHintTranslationKey(
  limitType?: string,
): LimitHintTranslationKey | undefined {
  switch (limitType) {
...
function DynamicWorkspaceExpensifyCardDetailsPage({
  route,
}: DynamicWorkspaceExpensifyCardDetailsPageProps) {
  const navigation =
    useNavigation<NavigationProp<SettingsNavigatorParamList>>();
  const { policyID, cardID } = route.params;
  const quickSettingsBackPath = useDynamicBackPath(
    DYNAMIC_ROUTES.EXPENSIFY_CARD_DETAILS.path,
  );
  const workspaceBackPath = useDynamicBackPath(
    DYNAMIC_ROUTES.WORKSPACE_EXPENSIFY_CARD_DETAILS.path,
  );
  const isQuickSettingsFlow =
    route.name === SCREENS.EXPENSIFY_CARD.DYNAMIC_EXPENSIFY_CARD_DETAILS;
  const backPath = isQuickSettingsFlow
    ? quickSettingsBackPath
    : workspaceBackPath;
  ...
  const [fundCardSettings] = useOnyx(
    `${ONYXKEYS.COLLECTION.PRIVATE_EXPENSIFY_CARD_SETTINGS}${defaultFundID}`,
  );
  const [allFeedsCards, allFeedsCardsResult] = useOnyx(
    ONYXKEYS.COLLECTION.WORKSPACE_CARDS_LIST,
  );
  const workspaceCards = getAllCardsForWorkspace(
    defaultFundID,
    allFeedsCards,
    cardFeeds,
  ...
  const formattedAvailableSpendAmount = convertToDisplayString(
    card?.availableSpend,
    currency,
  );
  const formattedLimit = convertToDisplayString(
  ...
  const translationForLimitType = getTranslationKeyForLimitType(
    card?.nameValuePairs?.limitType,
  );
  const remainingLimitHintTranslationKey = getLimitHintTranslationKey(
    card?.nameValuePairs?.limitType,
  );
  const remainingLimitHint = remainingLimitHintTranslationKey
    ? translate(remainingLimitHintTranslationKey, formattedAvailableSpendAmount)
    : undefined;
  const canWriteExpensifyCard = canMemberWrite(
    policy,
    currentUserLogin,
    CONST.POLICY.POLICY_FEATURE.EXPENSIFY_CARD,
  );
  const canEditSpendRules =
    canWriteExpensifyCard && arePolicyRulesEnabled(policy, policyCategories);
  const isDeactivated =
  ...
  useEffect(() => {
    ...
    defaultFundID,
    fundCardSettings?.hasOnceLoaded,
    fundCardSettings?.isLoading,
    policyID,
  ]);
  ...
  const deactivateCard = () => {
    showConfirmModal({
      ...
      prompt: translate(
        "workspace.card.deactivateCardModal.deactivateConfirmation",
      ),
  ...
  const handleFreezePress = () => {
    showConfirmModal({
      ...
      freezeCardAction(
        Number(card?.fundID ?? defaultFundID ?? CONST.DEFAULT_NUMBER_ID),
        card,
  ...
  const spendRulesSummary = spendRule
    ? getSpendRuleSummaryText(
        spendRule.formValues,
        currency,
        translate,
    ...
    () => (
      <View>
        {spendRulesSummary.map((summary) => (
          <Text key={summary} numberOfLines={2}>
            {summary}
          </Text>
        ))}
      </View>
  ...
  const workspaceCardImage = (
    <>
      ...
      <Text
        style={[
          styles.walletCardHolder,
          { color: getCardFeedTextColor(CONST.EXPENSIFY_CARD.BANK) },
        ]}
        numberOfLines={1}
        ellipsizeMode="tail"
      >
      ...
      <Badge
        ...
        text={translate(
          isVirtual
            ? "workspace.expensifyCard.virtual"
            : "workspace.expensifyCard.physical",
        )}
      />
    </>
  );
      ...
      >
        ...
        <ScrollView addBottomSafeAreaPadding>
          {canManageCardFreeze && isCardFrozen(card) ? (
            ...
            <View style={[styles.walletCard, styles.mb3, styles.mt8]}>
              {workspaceCardImage}
            </View>
          )}
          ...
          >
            <MenuItemWithTopDescription
              ...
              onPress={() =>
                Navigation.navigate(
                  createDynamicRoute(DYNAMIC_ROUTES.EXPENSIFY_CARD_NAME.path),
                )
              }
          ...
          <MenuItemWithTopDescription
            description={translate(
              isVirtual
                ? "cardPage.virtualCardNumber"
                : "cardPage.physicalCardNumber",
            )}
          ...
          <OfflineWithFeedback
            pendingAction={card?.pendingFields?.availableSpend}
          >
            <MenuItemWithTopDescription
          ...
          <OfflineWithFeedback
            pendingAction={card?.nameValuePairs?.pendingFields?.limitType}
          >
            <MenuItemWithTopDescription
              description={translate("workspace.card.issueNewCard.limitType")}
              title={
                translationForLimitType
                  ? translate(translationForLimitType)
                  : ""
              }
              shouldShowRightIcon={canWriteExpensifyCard}
              onPress={() =>
                Navigation.navigate(
                  createDynamicRoute(
                    DYNAMIC_ROUTES.EXPENSIFY_CARD_LIMIT_TYPE.path,
                  ),
                )
              }
              interactive={canWriteExpensifyCard}
          ...
          <OfflineWithFeedback
            pendingAction={
              card?.nameValuePairs?.pendingFields?.unapprovedExpenseLimit
            }
          >
            <MenuItemWithTopDescription
              ...
              onPress={() =>
                Navigation.navigate(
                  createDynamicRoute(DYNAMIC_ROUTES.EXPENSIFY_CARD_LIMIT.path),
                )
              }
              interactive={canWriteExpensifyCard}
            />
          ...
          <View style={styles.mt6}>
            ...
            {canWriteExpensifyCard && !isDeactivated && (
              <MenuItem
                ...
                style={styles.mb1}
                onPress={() =>
                  isOffline ? setIsOfflineModalVisible(true) : deactivateCard()
                }
              />
            )}
          </View>
