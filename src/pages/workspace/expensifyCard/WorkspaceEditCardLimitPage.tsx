...
function WorkspaceEditCardLimitPage({route}: WorkspaceEditCardLimitPageProps) {
    ...
    const updateCardLimit = (newLimit: number) => {
        ...
        updateExpensifyCardLimit(workspaceAccountID, Number(cardID), newLimit, newAvailableSpend, card?.nameValuePairs?.unapprovedExpenseLimit, card?.availableSpend, card?.nameValuePairs?.isVirtual);
