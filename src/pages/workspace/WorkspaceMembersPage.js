...
class WorkspaceMembersPage extends React.Component {
    ...
    toggleAllUsers() {
        ...
        this.setState(
            (prevState) => ({
                selectedEmployees: !_.every(removableMembers, (member) => _.contains(prevState.selectedEmployees, member)) ? removableMembers : [],
            }),
            () => this.validate(),
        );
    }
    ...
    renderItem({item}) {
        return (
            <OfflineWithFeedback errorRowStyles={[styles.peopleRowBorderBottom]} onClose={() => this.dismissError(item)} pendingAction={item.pendingAction} errors={item.errors}>
                ...
                {!_.isEmpty(this.state.errors[item.login]) && <FormHelpMessage isError message={this.state.errors[item.login]} />}
            </OfflineWithFeedback>
        );
    }
    ...
    render() {
        ...
        return (
            <ScreenWrapper includeSafeAreaPaddingBottom={false} style={[styles.defaultModalContainer]}>
                    ...
                    <FullPageNotFoundView shouldShow={_.isEmpty(this.props.policy)} onBackButtonPress={() => Navigation.navigate(ROUTES.SETTINGS_WORKSPACES)}>
                        ...
                        <View style={[styles.w100, styles.flex1]}>
                            <View style={[styles.w100, styles.flexRow, styles.pt3, styles.ph5]}>
                                <Button medium success text={this.props.translate('common.invite')} onPress={this.inviteUser} />
                                <Button
                                    medium
                                    danger
                                    style={[styles.ml2]}
