...
import OnyxTabNavigator, {
  TabScreenWithFocusTrapWrapper,
  TopTab,
} from "@libs/Navigation/OnyxTabNavigator";
...
type DynamicEditInviteReceiptPartnerPolicyPageProps = PlatformStackScreenProps<
  WorkspaceSplitNavigatorParamList,
  typeof SCREENS.WORKSPACE.DYNAMIC_RECEIPT_PARTNERS_INVITE_EDIT
>;
...
type UberEmployeeStatus = ValueOf<
  typeof CONST.POLICY.RECEIPT_PARTNERS.UBER_EMPLOYEE_STATUS
>;
function DynamicEditInviteReceiptPartnerPolicyPage({
  route,
}: DynamicEditInviteReceiptPartnerPolicyPageProps) {
  ...
  const backPath = useDynamicBackPath(
    DYNAMIC_ROUTES.WORKSPACE_RECEIPT_PARTNERS_INVITE_EDIT.path,
  );
  ...
  const [allSearchTerm, allDebouncedSearchTerm, setAllSearchTerm] =
    useDebouncedState("");
  const [linkedSearchTerm, linkedDebouncedSearchTerm, setLinkedSearchTerm] =
    useDebouncedState("");
  const [
  ...
  const members = useMemo<Array<MemberForList & ListItem>>(() => {
    ...
    const buttonStyles = [
      ...
      StyleUtils.getButtonStyleWithIcon(
        styles,
        CONST.DROPDOWN_BUTTON_SIZE.EXTRA_SMALL,
        true,
        false,
      ),
      styles.ml3,
    ...
    for (const [email, policyEmployee] of Object.entries(employees)) {
      ...
      if (isResend || isInvite) {
        const text = isResend
          ? translate("workspace.receiptPartners.uber.status.resend")
          : translate("workspace.receiptPartners.uber.status.invite");
        const textChecked = translate(
          "workspace.receiptPartners.uber.status.resend",
        );
        rightElement = (
          <PressableWithDelayToggle
            ...
            styles={[
              ...buttonStyles,
              isInvite ? styles.buttonSuccess : undefined,
            ]}
            textStyles={[
              ...buttonTextStyles,
              isInvite ? styles.buttonSuccessText : undefined,
            ]}
            iconChecked={icons.Checkmark}
            inline={false}
            sentryLabel={
              CONST.SENTRY_LABEL.RECEIPT_PARTNERS.INVITE_RESEND_BUTTON
            }
            accessible={false}
          />
        );
        ...
        const badgeText = translate(
          `workspace.receiptPartners.uber.status.${status}`,
        );
        const isSuccess =
  ...
  const getMembersForTabWithoutSearch = useCallback(
    (tab: ReceiptPartnersTab) => applyTabStatusFilter(tab, members),
    [applyTabStatusFilter, members],
  );
  ...
  const filterMembers = useCallback(
    (tab: ReceiptPartnersTab) => {
      ...
      let data = members;
      if (search) {
        data = tokenizedSearch(members, search, (m) => [
          m.text ?? "",
          m.alternateText ?? "",
        ]);
      }
  ...
  const listEmptyContent = useMemo(
    () => (
      <BlockingView
        ...
        subtitle={translate(
          "workspace.receiptPartners.uber.emptyContent.subtitle",
        )}
      ...
      <ScreenWrapper testID="DynamicEditInviteReceiptPartnerPolicyPage">
        ...
        >
          {TAB_NAMES.map((tabName) => (
            <TopTab.Screen key={tabName} name={tabName}>
              {() => {
                const { searchTerm, debouncedSearchTerm, setSearchTerm } =
                  getSearchStateForTab(tabName);
                const filteredMembers = filterMembers(tabName);
                const baseTabMembersCount =
                  getMembersForTabWithoutSearch(tabName).length;
                const shouldShowListEmptyContent =
                  baseTabMembersCount < CONST.STANDARD_LIST_ITEM_LIMIT;
                const shouldShowTextInput = !shouldShowListEmptyContent;

                // Determine header message for search results
                const searchValue = debouncedSearchTerm.trim().toLowerCase();
                let currentHeaderMessage = getHeaderMessage(
                  members.length !== 0,
                  false,
                  searchValue,
                  countryCode,
