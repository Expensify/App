...
type WorkspaceReceiptPartnersPageProps = PlatformStackScreenProps<
  WorkspaceSplitNavigatorParamList,
  typeof SCREENS.WORKSPACE.RECEIPT_PARTNERS
>;

function WorkspaceReceiptPartnersPage({
  route,
}: WorkspaceReceiptPartnersPageProps) {
  ...
  const {
    ...
    isUberConnected,
  } = useGetReceiptPartnersIntegrationData(policyID);
    ...
    ({ name }: { name: string }) => {
      switch (name) {
        case CONST.POLICY.RECEIPT_PARTNERS.NAME.UBER: {
          openExternalLink(
            `${CONST.UBER_CONNECT_URL}?${integrations?.uber?.connectFormData}`,
          );
          break;
        }
        default: {
  ...
  useEffect(() => {
    ...
    Navigation.navigate(
      createDynamicRoute(
        DYNAMIC_ROUTES.WORKSPACE_RECEIPT_PARTNERS_INVITE.getRoute(
  ...
  const calculateAndSetThreeDotsMenuPosition = useCallback(() => {
    ...
    return new Promise<AnchorPosition>((resolve) => {
      threeDotsMenuContainerRef.current?.measureInWindow(
        (x, y, width, height) => {
          resolve({
            horizontal: x + width,
            vertical: y + height,
          ...
          return [
            {
              ...
              onSelected: () => {
                showConfirmModal({
                  title: translate(
                    "workspace.moreFeatures.receiptPartnersWarningModal.featureEnabledTitle",
                  ),
                  prompt: translate(
                    "workspace.moreFeatures.receiptPartnersWarningModal.description",
                  ),
      ...
      return receiptPartnerIntegrations
        .map((integration) => {
          const integrationData =
            getReceiptPartnersIntegrationData(integration);
          if (!integrationData) {
            return undefined;
          }
          const overflowMenu = canWriteMoreFeatures
            ? getOverflowMenu(integration)
            : [];
          ...
          const isUber =
            integration === CONST.POLICY.RECEIPT_PARTNERS.NAME.UBER;
          ...
          return {
            ...
            ...integrationData,
            interactive: false,
            errorText: shouldShowEnterCredentialsError
            ...
            badgeText: isUber
              ? translate("workspace.accounting.claimOffer.badgeText")
              : undefined,
            onBadgePress:
              isUber && canWriteMoreFeatures
                ? () => {
                    Navigation.navigate(
                      ROUTES.POLICY_ACCOUNTING_CLAIM_OFFER.getRoute(
                        policyID,
                        CONST.POLICY.RECEIPT_PARTNERS.NAME.UBER,
                      ),
                    );
            ...
            brickRoadIndicator:
              !!integrationData?.errorFields || shouldShowEnterCredentialsError
              ...
              >
                ...
                {isUberConnected && (
                  <>
                    <OfflineWithFeedback
                      pendingAction={
                        integrations?.uber?.pendingFields?.autoInvite
                      }
                    >
                      <View style={styles.mt5}>
                        <ToggleSettingOptionRow
                          ...
                          title={translate(
                            "workspace.receiptPartners.uber.autoInvite",
                          )}
                          switchAccessibilityLabel={translate(
                            "workspace.receiptPartners.uber.autoInvite",
                          )}
                    ...
                    <OfflineWithFeedback
                      pendingAction={
                        integrations?.uber?.pendingFields?.autoRemove
                      }
                    >
                      <View style={styles.mt5}>
                        <ToggleSettingOptionRow
                          titleStyle={styles.pr3}
                          title={translate(
                            "workspace.receiptPartners.uber.autoRemove",
                          )}
                          switchAccessibilityLabel={translate(
                            "workspace.receiptPartners.uber.autoRemove",
                          )}
                    ...
                    {centralBillingAccountEmail && (
                      <OfflineWithFeedback
                        pendingAction={
                          integrations?.uber?.pendingFields
                            ?.centralBillingAccountEmail
                        }
                      >
                        <MenuItemWithTopDescription
                          description={translate(
                            "workspace.receiptPartners.uber.centralBillingAccount",
                          )}
                    ...
                    {canWriteMoreFeatures && (
                      <MenuItem
                        title={translate(
                          "workspace.receiptPartners.uber.manageInvites",
                        )}
                        shouldShowRightIcon
                        icon={icons.Mail}
                        style={[
                          styles.sectionMenuItemTopDescription,
                          styles.mbn3,
                          !centralBillingAccountEmail && styles.mt6,
                        ]}
                        onPress={() =>
                          Navigation.navigate(
                            createDynamicRoute(
                              DYNAMIC_ROUTES.WORKSPACE_RECEIPT_PARTNERS_INVITE_EDIT.getRoute(
                                CONST.POLICY.RECEIPT_PARTNERS.NAME.UBER,
                              ),
                            ),
                          )
