...
import {connectToDualEntry} from '@libs/actions/connections/DualEntry';
...
import INPUT_IDS from '@src/types/form/DualEntryCredentialsForm';

import React from 'react';
import {View} from 'react-native';

type DualEntrySetupPageProps = PlatformStackScreenProps<SettingsNavigatorParamList, typeof SCREENS.WORKSPACE.ACCOUNTING.DUALENTRY_SETUP>;

function DualEntrySetupPage({route}: DualEntrySetupPageProps) {
    ...
    const confirmCredentials = (values: FormOnyxValues<typeof ONYXKEYS.FORMS.DUALENTRY_CREDENTIALS_FORM>) => {
        connectToDualEntry(policyID, values[INPUT_IDS.API_KEY]);
        Navigation.dismissModal();
    };

    const formItems = Object.values(INPUT_IDS);
    const validate = (values: FormOnyxValues<typeof ONYXKEYS.FORMS.DUALENTRY_CREDENTIALS_FORM>) => {
        const errors: FormInputErrors<typeof ONYXKEYS.FORMS.DUALENTRY_CREDENTIALS_FORM> = {};
    ...
    return (
        ...
        >
            <FormProvider
                ...
                formID={ONYXKEYS.FORMS.DUALENTRY_CREDENTIALS_FORM}
                validate={validate}
...
export default DualEntrySetupPage;
