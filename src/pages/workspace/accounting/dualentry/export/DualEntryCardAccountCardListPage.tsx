...
type DualEntryCardAccountCardListPageProps = WithPolicyConnectionsProps &
    PlatformStackScreenProps<SettingsNavigatorParamList, typeof SCREENS.WORKSPACE.ACCOUNTING.DUALENTRY_CARD_ACCOUNT_CARD_LIST>;

function DualEntryCardAccountCardListPage({
    policy,
    route: {
        params: {feed: feedWithDomainID},
    },
}: DualEntryCardAccountCardListPageProps) {
    ...
    const backPath = policyID ? ROUTES.POLICY_ACCOUNTING_DUALENTRY_CARD_ACCOUNT.getRoute(policyID) : undefined;

    return (
        <ConnectionLayout
            displayName="DualEntryCardAccountCardListPage"
        ...
        >
            <View>
                <Text style={[styles.ph5, styles.pb5]}>{translate('workspace.dualEntry.cardAccount.descriptionLevel2')}</Text>
            </View>
            {Object.values(cardList ?? {})
                .filter(isCard)
                .map((card) => {
                    const cardID = card.cardID;
                    const isUsingCustomAccount = typeof card.nameValuePairs === 'object' && CONST.COMPANY_CARDS.EXPORT_CARD_TYPES.NVP_DUALENTRY_EXPORT_ACCOUNT in card.nameValuePairs;
                    const cardAccountID =
                        (typeof card.nameValuePairs === 'object' ? card.nameValuePairs[CONST.COMPANY_CARDS.EXPORT_CARD_TYPES.NVP_DUALENTRY_EXPORT_ACCOUNT] : undefined) ??
                    ...
                    const cardAccountDisplayName = cardAccount
                        ? `${cardAccount.id} ${cardAccount.name}${isUsingCustomAccount ? '' : ` (${translate('common.default').toLocaleLowerCase()})`}`
                        : '';
                    return (
                        <OfflineWithFeedback
                            key={cardID}
...
export default withPolicyConnections(DualEntryCardAccountCardListPage);
