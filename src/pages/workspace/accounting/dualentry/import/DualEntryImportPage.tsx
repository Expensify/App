...
function DualEntryImportPage({policy}: WithPolicyConnectionsProps) {
    ...
    const data = policy?.connections?.dualEntry?.data;
    const enableNewCategories = config?.enableNewCategories ?? false;
    const syncTaxRates = config?.coding?.syncTaxRates ?? false;
    const classifications = data?.classifications?.filter(({isActive}) => isActive) ?? [];
    ...
    return (
        ...
        >
            ...
            <ToggleSettingOptionRow
                ...
                pendingAction={settingsPendingAction([CONST.DUALENTRY_CONFIG.ENABLE_NEW_CATEGORIES], config?.pendingFields)}
                errors={getLatestErrorField(config ?? {}, CONST.DUALENTRY_CONFIG.ENABLE_NEW_CATEGORIES)}
            ...
            <Text style={[styles.mv3, styles.mh5]}>{translate('workspace.dualEntry.dimensionsImport')}</Text>
            {classifications.map((classification) => {
                const mapping = config?.coding?.fieldMappings?.[classification.id];
                const isImported = mapping === CONST.DUALENTRY_MAPPING_VALUE.TAG;
                const feedbackKey = `${CONST.DUALENTRY_CONFIG.FIELD_MAPPING_PREFIX}${classification.id}`;
                return (
                    <ToggleSettingOptionRow
                        ...
                        pendingAction={settingsPendingAction([feedbackKey], config?.pendingFields)}
                        errors={getLatestErrorField(config ?? {}, feedbackKey)}
                        onCloseError={() => policyID && clearDualEntryErrorField(policyID, feedbackKey)}
                    />
                );
            })}
            {!!data?.taxRates?.length && (
                <>
                    ...
                    <ToggleSettingOptionRow
                        ...
                        pendingAction={settingsPendingAction([CONST.DUALENTRY_CONFIG.SYNC_TAX_RATES], config?.pendingFields)}
                        errors={getLatestErrorField(config ?? {}, CONST.DUALENTRY_CONFIG.SYNC_TAX_RATES)}
