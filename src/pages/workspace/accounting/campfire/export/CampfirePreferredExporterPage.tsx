...
import {clearCampfireErrorField, updateCampfireExporter} from '@libs/actions/connections/Campfire';
...
function CampfirePreferredExporterPage({policy}: WithPolicyConnectionsProps) {
    ...
    const selectExporter = (item: ExporterListItem) => {
        if (item.value !== exporter && policyID) {
            updateCampfireExporter(policyID, item.value, exporter);
        }
    ...
    return (
        <SelectionScreen
            ...
            displayName="CampfirePreferredExporterPage"
            ...
            pendingAction={settingsPendingAction([CONST.CAMPFIRE_CONFIG.EXPORTER], campfireConfig?.pendingFields)}
            errors={getLatestErrorField(campfireConfig, CONST.CAMPFIRE_CONFIG.EXPORTER)}
            errorRowStyles={[styles.ph5, styles.pv3]}
            onClose={() => policyID && clearCampfireErrorField(policyID, CONST.CAMPFIRE_CONFIG.EXPORTER)}
        />
    );
}

export default withPolicyConnections(CampfirePreferredExporterPage);
