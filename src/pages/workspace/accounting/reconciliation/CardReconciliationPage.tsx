...
function CardReconciliationPage({policy, route}: CardReconciliationPageProps) {
    ...
    const [cardContinuousReconciliation] = useOnyx(`${ONYXKEYS.COLLECTION.EXPENSIFY_CARD_USE_CONTINUOUS_RECONCILIATION}${effectiveDomainID}`, {canBeMissing: true});
    ...
    useEffect(() => {
        if (cardContinuousReconciliation?.value !== undefined) {
            return;
        }
        fetchPolicyAccountingData();
    }, [cardContinuousReconciliation?.value, fetchPolicyAccountingData]);
            ...
            >
                ...
                >
                    <ToggleSettingOptionRow
                        ...
                        disabled={!autoSync}
                        isActive={!!cardContinuousReconciliation?.value}
                        onToggle={handleToggleContinuousReconciliation}
                        wrapperStyle={styles.ph5}
                        pendingAction={cardContinuousReconciliation?.pendingAction}
                    />
                    {!autoSync && (
                    ...
                    {!!paymentBankAccountID && !!cardContinuousReconciliation?.value && (
