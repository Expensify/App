...
import {clearBusinessCentralErrorField, updateBusinessCentralExporter} from '@libs/actions/connections/BusinessCentral';
...
function BusinessCentralPreferredExporterSelectPage({policy}: WithPolicyConnectionsProps) {
    ...
    const selectExporter = (item: ExporterListItem) => {
        if (item.value !== exporter && policyID) {
            updateBusinessCentralExporter(policyID, item.value, savedExporter);
        }
    ...
    return (
        <SelectionScreen
            ...
            displayName="BusinessCentralPreferredExporterSelectPage"
            ...
            pendingAction={settingsPendingAction([CONST.BUSINESS_CENTRAL_CONFIG.EXPORTER], businessCentralConfig?.pendingFields)}
            errors={getLatestErrorField(businessCentralConfig, CONST.BUSINESS_CENTRAL_CONFIG.EXPORTER)}
            errorRowStyles={[styles.ph5, styles.pv3]}
            onClose={() => policyID && clearBusinessCentralErrorField(policyID, CONST.BUSINESS_CENTRAL_CONFIG.EXPORTER)}
        />
    );
}

export default withPolicyConnections(BusinessCentralPreferredExporterSelectPage);
