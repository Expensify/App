...
import {clearRilletErrorField, updateRilletTravelBillingSettlementsAccount} from '@libs/actions/connections/Rillet';
...
function RilletTravelBillingSettlementAccountPage({policy}: WithPolicyConnectionsProps) {
    ...
    const setTravelBillingSettlementsAccount = (item: BankAccountListItem) => {
        if (item.value !== travelInvoicingSettlementsBankAccountID && policyID) {
            updateRilletTravelBillingSettlementsAccount(policyID, item.value, travelInvoicingSettlementsBankAccountID);
        }
    ...
    return (
        <SelectionScreen
            ...
            displayName="RilletTravelBillingSettlementAccountPage"
            title="workspace.rillet.travelInvoicingSettlementAccount.label"
            data={filteredData}
            textInputOptions={textInputOptions}
            headerContent={headerContent}
            listEmptyContent={listEmptyContent}
            onSelectRow={setTravelBillingSettlementsAccount}
            ...
            pendingAction={settingsPendingAction([CONST.RILLET_CONFIG.TRAVEL_BILLING_SETTLEMENTS_BANK_ACCOUNT_ID], rilletConfig?.pendingFields)}
            errors={getLatestErrorField(rilletConfig, CONST.RILLET_CONFIG.TRAVEL_BILLING_SETTLEMENTS_BANK_ACCOUNT_ID)}
            errorRowStyles={[styles.ph5, styles.pv3]}
            onClose={() => policyID && clearRilletErrorField(policyID, CONST.RILLET_CONFIG.TRAVEL_BILLING_SETTLEMENTS_BANK_ACCOUNT_ID)}
        />
    );
}

export default withPolicyConnections(RilletTravelBillingSettlementAccountPage);
