...
type RilletSetupPageProps = PlatformStackScreenProps<
	SettingsNavigatorParamList,
	typeof SCREENS.WORKSPACE.ACCOUNTING.RILLET_SETUP
>;
...
function RilletSetupPage({ route }: RilletSetupPageProps) {
	...
	const shouldBeBlocked =
		!!config?.isConfigured &&
		!isAuthenticationError(policy, CONST.POLICY.CONNECTIONS.NAME.RILLET);
	...
	const validate = (
		...
		const errors: FormInputErrors<
			typeof ONYXKEYS.FORMS.RILLET_CREDENTIALS_FORM
		> = {};
		...
		for (const formItem of formItems) {
			...
			addErrorMessage(
				errors,
				formItem,
				translate("common.error.fieldRequired"),
			);
		}
		return errors;
	};
	...
	return (
		<ConnectionLayout
			...
			accessVariants={[
				...
				CONST.POLICY.ACCESS_VARIANTS.CONTROL,
			]}
			policyID={policyID}
		...
		>
			...
			>
				<Text style={[styles.textHeadlineH1, styles.pb5, styles.pt3]}>
					{translate("workspace.rillet.enterCredentials")}
				</Text>
				<View style={[styles.renderHTML, styles.mb5]}>
					<RenderHTML
						html={translate("workspace.rillet.howToFindAPIKey")}
					/>
				</View>

				{formItems.map((formItem, index) => (
					<View style={styles.mb4} key={formItem}>
						<InputWrapper
