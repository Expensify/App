...
function XeroTravelBillingPayableAccountSelectPage({policy}: WithPolicyConnectionsProps) {
    ...
    const backPath = useDynamicBackPath(DYNAMIC_ROUTES.POLICY_ACCOUNTING_XERO_TRAVEL_BILLING_PAYABLE_ACCOUNT_SELECT.path);
    ...
    const selectAccount = (row: SelectorType<string>) => {
        if (row.value !== selectedAccountID) {
            updateXeroTravelBillingPayableAccount(policyID, row.value, selectedAccountID);
        }
    ...
    return (
        <TravelBillingPayableAccountSelectPage
            policyID={policyID}
            displayName="XeroTravelBillingPayableAccountSelectPage"
            data={data}
            ...
            pendingAction={settingsPendingAction([CONST.XERO_CONFIG.TRAVEL_BILLING_PAYABLE_ACCOUNT], config?.pendingFields)}
            errors={getLatestErrorField(config, CONST.XERO_CONFIG.TRAVEL_BILLING_PAYABLE_ACCOUNT)}
            onClose={() => clearXeroErrorField(policyID, CONST.XERO_CONFIG.TRAVEL_BILLING_PAYABLE_ACCOUNT)}
        />
    );
}

export default withPolicyConnections(XeroTravelBillingPayableAccountSelectPage);
