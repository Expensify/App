...
function DynamicSageIntacctTravelBillingPayableAccountSelectPage({policy}: WithPolicyConnectionsProps) {
    const policyID = policy?.id ?? String(CONST.DEFAULT_NUMBER_ID);
    const config = policy?.connections?.intacct?.config;
    const selectedAccountID = config?.export?.travelInvoicingPayableAccountID;
    const backPath = useDynamicBackPath(DYNAMIC_ROUTES.POLICY_ACCOUNTING_SAGE_INTACCT_TRAVEL_BILLING_PAYABLE_ACCOUNT_SELECT.path);
    ...
    const selectAccount = (row: SelectorType<string>) => {
        if (row.value !== selectedAccountID) {
            updateSageIntacctTravelBillingPayableAccount(policyID, row.value, selectedAccountID);
        }
    ...
    return (
        <TravelBillingPayableAccountSelectPage
            ...
            pendingAction={settingsPendingAction([CONST.SAGE_INTACCT_CONFIG.TRAVEL_BILLING_PAYABLE_ACCOUNT], config?.pendingFields)}
            errors={getLatestErrorField(config, CONST.SAGE_INTACCT_CONFIG.TRAVEL_BILLING_PAYABLE_ACCOUNT)}
            onClose={() => clearSageIntacctErrorField(policyID, CONST.SAGE_INTACCT_CONFIG.TRAVEL_BILLING_PAYABLE_ACCOUNT)}
        />
    );
}

export default withPolicyConnections(DynamicSageIntacctTravelBillingPayableAccountSelectPage);
