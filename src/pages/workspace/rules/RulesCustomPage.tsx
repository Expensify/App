...
function RulesCustomPage({
    ...
    const validate = useCallback(
        (values: FormOnyxValues<typeof ONYXKEYS.FORMS.RULES_CUSTOM_FORM>) => {
            const errors = {};

            if (values[INPUT_IDS.CUSTOM_RULES].length > CONST.DESCRIPTION_LIMIT) {
                addErrorMessage(
                    errors,
                    INPUT_IDS.CUSTOM_RULES,
                    translate('common.error.characterLimitExceedCounter', {length: values[INPUT_IDS.CUSTOM_RULES].length, limit: CONST.DESCRIPTION_LIMIT}),
                );
            }
            return errors;
        },
