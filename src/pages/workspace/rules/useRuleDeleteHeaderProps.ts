...
/**
 * The header icon-button props that put a rule's delete on a trashcan in the top right, and the confirmation
 * behind it, which every rule editor shares so the gesture is the same whichever kind of rule is open.
 *
 * `canDelete` decides whether the button appears at all: an unsaved rule has nothing to delete, a member without
 * write access can't, and a rule already on its way out would otherwise be deleted twice.
 *
...
function useRuleDeleteHeaderProps({canDelete, onDelete, sentryLabel, backTo, titleKey, promptKey}: RuleDeleteHeaderPropsParams) {
    ...
    return {
        /** Spread onto `Header.IconButton`; `undefined` (render nothing) when there's nothing to delete. */
        deleteIconButtonProps: canDelete
            ? {
