...
import {isAnyHRConnected, isMergeHRConnected, isMergeHRSetupComplete} from '@libs/HRUtils';
...
function WorkspaceInitialPage({policyDraft, policy: policyProp, route}: WorkspaceInitialPageProps) {
    ...
    if (isGroupPolicy(policy) && shouldShowProtectedItems) {
        ...
        if (policyFeatureStates?.[CONST.POLICY.MORE_FEATURES.IS_HR_ENABLED]) {
            const isMergeHRSetupIncomplete = isMergeHRConnected(policy) && !isMergeHRSetupComplete(policy);
            workspaceMenuItems.push({
                translationKey: 'workspace.common.hr',
                brickRoadIndicator: isMergeHRSetupIncomplete ? CONST.BRICK_ROAD_INDICATOR_STATUS.INFO : undefined,
                icon: expensifyIcons.Users,
