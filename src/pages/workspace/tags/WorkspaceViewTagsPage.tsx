...
type WorkspaceViewTagsProps =
  | PlatformStackScreenProps<
      ...
      typeof SCREENS.WORKSPACE.TAG_LIST_VIEW
    >
  | PlatformStackScreenProps<
      SettingsNavigatorParamList,
      typeof SCREENS.SETTINGS_TAGS.DYNAMIC_SETTINGS_TAG_LIST_VIEW
    >;
...
function WorkspaceViewTagsPage({ route }: WorkspaceViewTagsProps) {
  ...
  const shouldDisplayButtonsInSeparateLine =
    useShouldDisplayButtonsInSeparateLine();
  ...
  const currentTagListName = useMemo(
    () => getTagListName(policyTags, orderWeight),
    [policyTags, orderWeight],
  );
  const hasDependentTags = useMemo(
    () => hasDependentTagsPolicyUtils(policy, policyTags),
    [policy, policyTags],
  );
  ...
  } = usePolicyFeatureWriteAccess(policy, CONST.POLICY.POLICY_FEATURE.TAGS);
  const isQuickSettingsFlow =
    route.name === SCREENS.SETTINGS_TAGS.DYNAMIC_SETTINGS_TAG_LIST_VIEW;
  const backPath = useDynamicBackPath(
    DYNAMIC_ROUTES.SETTINGS_TAG_LIST_VIEW.path,
  );
  ...
  const filterFunction = useCallback(
    (tag: PolicyTag | undefined) =>
      !!tag && tag.pendingAction !== CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE,
    [],
  );

  const [selectedTags, setSelectedTags] = useFilteredSelection(
    currentPolicyTag?.tags,
    filterFunction,
  );
  ...
  const updateWorkspaceTagEnabled = useCallback(
    (value: boolean, tagName: string) => {
      ...
      setWorkspaceTagEnabled(
        policyData,
        { [tagName]: { name: tagName, enabled: value } },
        orderWeight,
      );
    },
  ...
  const navigateToTagSettings = useCallback(
    (tag: PolicyTag) => {
      ...
      Navigation.navigate(
        isQuickSettingsFlow
          ? createDynamicRoute(
              DYNAMIC_ROUTES.SETTINGS_TAG_SETTINGS.getRoute(
                orderWeight,
                tag.name,
              ),
            )
          : createDynamicRoute(workspaceTagSettingsSuffix),
      );
    },
  ...
  const tagRows = useMemo<WorkspaceTagTableRowData[]>(() => {
    const enabledTagsCount = getCountOfEnabledTagsOfList(
      currentPolicyTag?.tags,
    );

    return Object.values(currentPolicyTag?.tags ?? {}).reduce<
      WorkspaceTagTableRowData[]
    >((acc, tag) => {
      const isDisabled =
        tag.pendingAction === CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE;

      if (!isOffline && isDisabled) {
        return acc;
      }

      const isDisablingLastEnabledTag = isDisablingOrDeletingLastEnabledTag(
        currentPolicyTag,
        [tag],
        enabledTagsCount,
      );
      ...
      acc.push({
        keyForList: hasDependentTags
          ? `${tag.name}-${tag.rules?.parentTagsFilter ?? ""}`
          : tag.name,
        value: tag.name,
        ...
        onToggleEnabled: (enabled: boolean) => {
          if (isDisablingLastEnabledTag) {
            showConfirmModal({
              title: translate(
                "workspace.tags.cannotDeleteOrDisableAllTags.title",
              ),
              prompt: translate(
                "workspace.tags.cannotDeleteOrDisableAllTags.description",
              ),
  ...
  const getHeaderButtons = () => {
    if (
      !canWriteTags ||
      (!isSmallScreenWidth && selectedTags.length === 0) ||
    ...
    const options: Array<
      DropdownOption<DeepValueOf<typeof CONST.POLICY.BULK_ACTION_TYPES>>
    > = [];
      ...
      options.push({
        ...
        onSelected: async () => {
          const { action } = await showConfirmModal({
            ...
            prompt: translate(
              selectedTags.length === 1
                ? "workspace.tags.deleteTagConfirmation"
                : "workspace.tags.deleteTagsConfirmation",
            ),
    ...
    if (enabledTagCount > 0) {
      const selectedTagsObject = selectedTags.map(
        (key) => currentPolicyTag?.tags[key],
      );
      options.push({
        icon: icons.Close,
        text: translate(
    ...
    return (
      <ButtonWithDropdownMenu
        ...
        options={options}
        style={[
          shouldDisplayButtonsInSeparateLine && styles.flexGrow1,
          shouldDisplayButtonsInSeparateLine && styles.mb3,
        ]}
  ...
  if (
    canWriteTags &&
    !!currentPolicyTag?.required &&
    !Object.values(currentPolicyTag?.tags ?? {}).some((tag) => tag.enabled)
  ) {
  ...
  const navigateToEditTag = () => {
    ...
    Navigation.navigate(
      isQuickSettingsFlow
        ? createDynamicRoute(
            DYNAMIC_ROUTES.SETTINGS_TAGS_EDIT.getRoute(
              currentPolicyTag?.orderWeight ?? 0,
            ),
          )
        : createDynamicRoute(
            DYNAMIC_ROUTES.WORKSPACE_EDIT_TAGS.getRoute(
              currentPolicyTag?.orderWeight ?? 0,
            ),
          ),
    );
  ...
  const selectionModeHeader =
    isMobileSelectionModeEnabled && isSmallScreenWidth;
            ...
            <ToggleSettingOptionRow
              ...
              onToggle={(on) => {
                ...
                if (!isMultiLevelTags) {
                  showConfirmModal({
                    title: translate(
                      "workspace.tags.cannotMakeTagListRequired.title",
                    ),
                    prompt: translate(
                      "workspace.tags.cannotMakeTagListRequired.description",
                    ),
                    confirmText: translate("common.buttonConfirm"),
                    shouldShowCancelButton: false,
                  });
                ...
                if (
                  isMakingLastRequiredTagListOptional(policy, policyTags, [
                    currentPolicyTag,
                  ])
                ) {
                  showConfirmModal({
                    title: translate(
                      "workspace.tags.cannotMakeAllTagsOptional.title",
                    ),
                    prompt: translate(
                      "workspace.tags.cannotMakeAllTagsOptional.description",
                    ),
                    confirmText: translate("common.buttonConfirm"),
                    shouldShowCancelButton: false,
                  });
              ...
              disabled={
                !canWriteTags ||
                (!currentPolicyTag?.required &&
                  !Object.values(currentPolicyTag?.tags ?? {}).some(
                    (tag) => tag.enabled,
                  ))
              }
