...
type DynamicWorkspaceWorkflowsApprovalsExpensesFromPageProps =
...
function DynamicWorkspaceWorkflowsApprovalsExpensesFromPage({
  policy,
  isLoadingReportData = true,
  route,
}: DynamicWorkspaceWorkflowsApprovalsExpensesFromPageProps) {
  const styles = useThemeStyles();
  const { translate } = useLocalize();
  const backPath = useDynamicBackPath(
    DYNAMIC_ROUTES.WORKSPACE_WORKFLOWS_APPROVALS_EXPENSES_FROM.path,
  );
  const [approvalWorkflow, approvalWorkflowResults] = useOnyx(
    ONYXKEYS.APPROVAL_WORKFLOW,
  );
  const [personalDetails] = useOnyx(ONYXKEYS.PERSONAL_DETAILS_LIST);
  const [invitedEmailsToAccountIDsDraft] = useOnyx(
    `${ONYXKEYS.COLLECTION.WORKSPACE_INVITE_MEMBERS_DRAFT}${route.params.policyID}`,
  );
  const icons = useMemoizedLazyExpensifyIcons(["FallbackAvatar"]);
  const { showConfirmModal } = useConfirmModal();
  const { login: currentUserLogin = "" } = useCurrentUserPersonalDetails();

  const personalDetailLogins = useMemo(
    () =>
      Object.fromEntries(
        Object.entries(personalDetails ?? {}).map(([id, details]) => [
          id,
          details?.login,
        ]),
      ),
  ...
  >([]);
  const canWriteApprovals = canMemberWrite(
    policy,
    currentUserLogin,
    CONST.POLICY.POLICY_FEATURE.WORKFLOWS_APPROVALS,
  );
  ...
  const isInitialCreationFlow =
    approvalWorkflow?.action === CONST.APPROVAL_WORKFLOW.ACTION.CREATE &&
    approvalWorkflow?.isInitialFlow;
  const hasAnyEligibleMember = Object.values(policy?.employeeList ?? {}).some(
    (employee) =>
      !!employee.email &&
      employee.pendingAction !== CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE,
  );
  const shouldShowListEmptyContent =
    !isLoadingApprovalWorkflow && !hasAnyEligibleMember;
  ...
  const membersInExistingWorkflows = (() => {
    ...
    for (const employee of Object.values(employees)) {
      if (
        !employee.email ||
        !employee.submitsTo ||
        employee.submitsTo === defaultApprover
      ) {
        continue;
      }
  ...
  useEffect(() => {
    ...
    setSelectedMembers(
      approvalWorkflow.members
        .filter((member) => {
          const isPolicyMember =
            !!policy?.employeeList?.[normalizeLogin(member.email)];
          ...
          return (
            isPolicyMember ||
            invitedEmailsToAccountIDsDraft?.[member.email] != null
          );
        })
        .map((member) => {
          let accountID = Number(
            policyMemberEmailsToAccountIDs[normalizeLogin(member.email)] ?? "",
          );
          const isPolicyMember =
            !!policy?.employeeList?.[normalizeLogin(member.email)];
          ...
          if (!accountID) {
            const draftAccountID =
              invitedEmailsToAccountIDsDraft?.[member.email];
          ...
          const displayName =
            member.displayName ?? personalDetail?.displayName ?? member.email;
          ...
          return {
            ...
            rightElement: isPolicyMember ? (
              <MemberRightIcon
                role={
                  policy?.employeeList?.[normalizeLogin(member.email)]?.role
                }
                owner={policy?.owner}
                login={login}
              />
    ...
    const availableMembers = liveAvailableMembers
      .map((member) => {
        const accountID = Number(
          policyMemberEmailsToAccountIDs[member.email] ?? "",
        );
      ...
      .filter(
        (member) =>
          (!policy?.preventSelfApproval ||
            !approversEmail?.includes(member.login)) &&
          !selectedMembers.some(
            (selectedOption) =>
              normalizeLogin(selectedOption.login) ===
              normalizeLogin(member.login),
          ),
      );
    ...
    if (debouncedSearchTerm && areOptionsInitialized) {
      ...
      if (availableOptions.userToInvite) {
        ...
        const isAlreadySelected = selectedMembers.some(
          (selectedOption) => selectedOption.login === userToInvite.login,
        );
      ...
      const searchResults = availableOptions.personalDetails.filter(
        (option) =>
          !members.some(
            (m) => normalizeLogin(m.login) === normalizeLogin(option.login),
          ),
      );
  ...
  const dropUnconfirmedStagedMembers = useCallback(() => {
    ...
    const confirmedMembers = stagedMembers.filter(
      (m) => !!policy?.employeeList?.[normalizeLogin(m.email)],
    );
  ...
  const nextStep = useCallback(() => {
    ...
    const normalizedExistingMembers: Member[] = existingMembers.map(
      (member) => ({
        ...member,
    ...
    if (usersToInvite.length > 0) {
      ...
      for (const user of usersToInvite) {
        ...
        invitedEmailsToAccountIDs[user.email] =
          user.accountID ?? CONST.DEFAULT_NUMBER_ID;
      }
      ...
      Navigation.navigate(
        createDynamicRoute(DYNAMIC_ROUTES.WORKSPACE_INVITE_MESSAGE.path),
      );
      return;
    }
  ...
  const button = useMemo(() => {
    let buttonText = isInitialCreationFlow
    ...
    return (
      <FormAlertWithSubmitButton
        ...
        onSubmit={
          shouldShowListEmptyContent ? () => Navigation.goBack() : nextStep
        }
        containerStyles={[
          styles.flexReset,
          styles.flexGrow0,
    ...
    (members: SelectionListApprover[]) => {
      const applySelection = (nextMembers: SelectionListApprover[]) => {
        ...
        for (const member of nextMembers) {
          ...
          const iconId = member.icons?.at(0)?.id;
          nextDraft[member.login] =
            typeof iconId === "number"
              ? iconId
              : (invitedEmailsToAccountIDsDraft?.[member.login] ??
      ...
      if (isCreateAction && members.length > selectedMembers.length) {
        const newMember = members.find(
          (m) => !selectedMembers.some((s) => s.login === m.login),
        );
        const existingApproverEmail = newMember?.login
          ? membersInExistingWorkflows.get(newMember.login)
          : undefined;

        if (newMember && existingApproverEmail) {
          const memberName = Str.removeSMSDomain(
            newMember.text ?? newMember.login ?? "",
          );
          const approverDetails = getPersonalDetailByEmail(
            existingApproverEmail,
          );
          const approverName = Str.removeSMSDomain(
            approverDetails?.displayName ?? existingApproverEmail,
          );

          showConfirmModal({
            title: translate(
              "workflowsExpensesFromPage.memberAlreadyInWorkflowTitle",
            ),
            prompt: translate(
              "workflowsExpensesFromPage.memberAlreadyInWorkflowPrompt",
  ...
  return (
    ...
    >
      <ApproverSelectionList
        ...
        listEmptyContentSubtitle={translate(
          "workflowsPage.emptyContent.expensesFromSubtitle",
        )}
