...
function WorkflowsPaymentsTab({policyID}: WorkflowsPaymentsTabProps) {
    ...
    const shouldShowContinueModal = useMemo(() => hasInProgressVBBA(achData, isNonUSDWorkspace, policy?.id), [achData, isNonUSDWorkspace, policy?.id]);
    ...
    const workflowsBackTo = ROUTES.WORKSPACE_WORKFLOWS.getRoute(policyID, CONST.TAB.WORKFLOWS.PAYMENTS);
