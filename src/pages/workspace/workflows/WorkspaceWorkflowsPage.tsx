...
type WorkspaceWorkflowsPageProps = WithPolicyProps &
...
function WorkflowNoResultsView({
  message,
  shouldShow,
  searchValue,
}: {
  ...
  return (
    <View style={[styles.pt3, styles.pb5]}>
      <Text style={[styles.textNormal, styles.colorMuted]} aria-hidden>
        {message}
      </Text>
    </View>
  );
...
function WorkflowsLoadMoreCard({
  count,
  onPress,
}: {
  count: number;
  ...
  return (
    <PressableWithFeedback
      ...
      style={[
        styles.border,
        shouldUseNarrowLayout ? styles.ph3 : styles.ph4,
        styles.pv3,
        styles.mt6,
        styles.mbn3,
    ...
    >
      <View
        style={[
          ...
          styles.alignItemsCenter,
          styles.justifyContentCenter,
          styles.minHeightComponentSizeSmall,
        ]}
      >
        <Icon
        ...
        />
        <Text style={[styles.buttonSmallText, styles.textSupporting]}>
          {label}
        </Text>
      </View>
    </PressableWithFeedback>
...
function WorkspaceWorkflowsPage({
  ...
  const isUserReimburser =
    account?.primaryLogin !== undefined &&
    (policy?.achAccount?.reimburser ?? policy?.owner) === account?.primaryLogin;
  ...
  const canAccessSubmit2026Features = canAccessSubmitWorkspaceFeatures(
    policy,
    isSubmit2026BetaEnabled,
  );
  const hasValidExistingAccounts =
    getEligibleExistingBusinessBankAccounts(
      bankAccountList,
  ...
  const displayNameForAuthorizedPayer = useMemo(
    () =>
      temporaryGetDisplayNameOrDefault({
        passedPersonalDetails: getPersonalDetailByEmail(
          policyReimburserEmail ?? "",
        ),
  ...
  const onPressAutoReportingFrequency = useCallback(
    () =>
      Navigation.navigate(
        ROUTES.WORKSPACE_WORKFLOWS_AUTOREPORTING_FREQUENCY.getRoute(
  ...
  const confirmCurrencyChangeAndHideModal = useCallback(() => {
    ...
    Navigation.navigate(
      ROUTES.WORKSPACE_OVERVIEW_CURRENCY.getRoute(policy.id, true),
    );
  }, [policy]);

  const { isOffline } = useNetwork({ onReconnect: fetchData });
  const isPolicyAdmin = isPolicyAdminUtil(policy);
  const canReadWorkflows = canMemberRead(
    policy,
    currentUserLogin,
  ...
  const {
    ...
    showReadOnlyModal,
    withReadOnlyFallback: withWorkflowsReadOnlyFallback,
  ...
  const promptConfigureApprovalsInHR = useCallback(async () => {
    const { action } = await showConfirmModal({
      title: translate(
        "workspace.moreFeatures.connectionsWarningModal.featureEnabledTitle",
      ),
  ...
  const filterWorkflow = (workflow: ApprovalWorkflow, searchInput: string) => {
    ...
    return (
      tokenizedSearch([workflow], searchInput, () => searchableTexts).length > 0
    );
  };

  const [workflowSearchInput, setWorkflowSearchInput, searchFilteredWorkflows] =
    useSearchResults(filteredApprovalWorkflows, filterWorkflow);
  ...
  if (
    ...
    searchFilteredWorkflows.length <= CONST.WORKFLOW_APPROVALS_INITIAL_BATCH
  ) {
  ...
  const approvalSubtitle = useMemo(() => {
    ...
    return (
      <Text
        style={[
          styles.textLabelSupportingEmptyValue,
          styles.lh20,
          styles.mt1,
          styles.mr5,
        ]}
  ...
  const optionItems: ToggleSettingOptionRowProps[] = useMemo(() => {
    const isBankAccountFullySetup =
      policy?.achAccount &&
      (policy?.achAccount.state === CONST.BANK_ACCOUNT.STATE.OPEN ||
        policy?.achAccount.state === CONST.BANK_ACCOUNT.STATE.LOCKED);
    const bankAccountConnectedToWorkspace = Object.values(
      bankAccountList ?? {},
    ).find(
      (bankAccount) =>
        bankAccount?.accountData?.additionalData?.policyID === policy?.id,
    );
    const bankName = isBankAccountFullySetup
      ? (policy?.achAccount?.bankName ?? "")
      : (bankAccountConnectedToWorkspace?.accountData?.additionalData
          ?.bankName ?? "");
    const addressName = isBankAccountFullySetup
      ? (policy?.achAccount?.addressName ?? "")
      : (bankAccountConnectedToWorkspace?.accountData?.addressName ?? "");
    const accountData = isBankAccountFullySetup
      ? policy?.achAccount
      : bankAccountConnectedToWorkspace?.accountData;
    const bankTitle = addressName.includes(CONST.MASKED_PAN_PREFIX)
      ? bankName
      : addressName;
    const bankAccountID = isBankAccountFullySetup
      ? policy?.achAccount?.bankAccountID
      : bankAccountConnectedToWorkspace?.methodID;
    const state = isBankAccountFullySetup
      ? (policy?.achAccount?.state ?? "")
      : (bankAccountConnectedToWorkspace?.accountData?.state ?? "");
    const isAccountInSetupState = isBankAccountPartiallySetup(state);
    const isBusinessBankAccountLocked =
    ...
    const bankAccountPendingAction =
      bankAccountConnectedToWorkspace?.pendingAction;
    const isBankAccountPendingDelete =
      bankAccountPendingAction === CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE;
    ...
    const hasDelayedSubmissionError = !!(
      policy?.errorFields?.autoReporting ??
      policy?.errorFields?.autoReportingFrequency
    );
    ...
    const approvalOptionSubtitle =
      isHRConnected || !isSmartLimitEnabled
        ? approvalSubtitle
    ...
    const shouldShowSubmissionFrequency =
      canWriteWorkflows || !canWriteApprovals;
    const shouldShowPayments = canMemberRead(
      policy,
      currentUserLogin,
      CONST.POLICY.POLICY_FEATURE.WORKFLOWS_PAYMENTS,
    );
    ...
    return [
        ...
        ? [
            {
              ...
              subtitle: translate(
                "workflowsPage.submissionFrequencyDescription",
              ),
                ...
                <MenuItemWithTopDescription
                  title={
                    getAutoReportingFrequencyDisplayNames(translate)[
                      getCorrectedAutoReportingFrequency(policy) ??
                  ...
                  sentryLabel={
                    CONST.SENTRY_LABEL.WORKSPACE.WORKFLOWS
                      .AUTO_REPORTING_FREQUENCY
                  }
              ...
              isActive:
                (policy?.autoReporting && !hasDelayedSubmissionError) ?? false,
              pendingAction:
                policy?.pendingFields?.autoReporting ??
                policy?.pendingFields?.autoReportingFrequency,
              errors: getLatestErrorField(
                policy ?? {},
                CONST.POLICY.COLLECTION_KEYS.AUTOREPORTING,
              ),
              onCloseError: () =>
                clearPolicyErrorField(
                  route.params.policyID,
        ...
        : []),
      {
        ...
        switchAccessibilityLabel: isSmartLimitEnabled
          ? translate("workspace.moreFeatures.workflows.disableApprovalPrompt")
          : translate("workflowsPage.addApprovalsDescription"),
        onToggle: (isEnabled: boolean) => {
          ...
          if (!isEnabled) {
            showConfirmModal({
              ...
              prompt: translate(
                "workflowsPage.disableApprovalPromptDescription",
              ),
          ...
          <>
            {isDEWEnabled && (
              <View
                style={[
                  styles.border,
                  shouldUseNarrowLayout ? styles.p3 : styles.p4,
                  styles.mt6,
                  styles.mbn3,
                  styles.flexRow,
                  styles.alignItemsCenter,
                  ...
                  <RenderHTML
                    html={
                      accountManagerReportID
                        ? translate(
                            "workflowsPage.customApprovalWorkflowEnabled",
                          )
                        : translate(
                            "workflowsPage.customApprovalWorkflowEnabledConciergeOnly",
                          )
                    }
                  />
                </View>
            ...
            <WorkflowNoResultsView
              ...
              shouldShow={
                searchFilteredWorkflows.length === 0 &&
                workflowSearchInput.length > 0
              }
                ...
                >
                  <ApprovalWorkflowSection
                    ...
                    hrFinalApproverEmail={
                      isHRAdvancedModeEnabled ? hrFinalApproverEmail : undefined
                    }
                  />
                </OfflineWithFeedback>
              );
            ...
            {!shouldBlockApprovalWorkflowEditing && canWriteApprovals && (
              <MenuItem
                ...
                sentryLabel={
                  CONST.SENTRY_LABEL.WORKSPACE.WORKFLOWS.ADD_APPROVAL
                }
              />
            )}
          </>
        ...
        disabledAction: withApprovalsReadOnlyFallback(
          getAddApprovalsToggleDisabledAction(),
        ),
        ...
        isActive:
          ...
          (isHRConnected ||
            ...
            (([
              CONST.POLICY.APPROVAL_MODE.BASIC,
              CONST.POLICY.APPROVAL_MODE.ADVANCED,
            ].some((approvalMode) => approvalMode === policy?.approvalMode) &&
              !hasApprovalError) ??
              false)),
        pendingAction: policy?.pendingFields?.approvalMode,
        errors: getLatestErrorField(
          policy ?? {},
          CONST.POLICY.COLLECTION_KEYS.APPROVAL_MODE,
        ),
        onCloseError: () =>
          clearPolicyErrorField(
            route.params.policyID,
        ...
        ? [
            {
              ...
              subtitle: translate(
                "workflowsPage.makeOrTrackPaymentsDescription",
              ),
              switchAccessibilityLabel: translate(
                "workflowsPage.makeOrTrackPaymentsDescription",
              ),
              onToggle: (isEnabled: boolean) => {
                ...
                if (isEnabled && canAccessSubmit2026Features) {
                  Navigation.navigate(
                    ROUTES.WORKSPACE_UPGRADE.getRoute(
                      ...
                      ROUTES.WORKSPACE_WORKFLOWS.getRoute(
                        route.params.policyID,
                      ),
                    ),
                  );
                ...
                const newReimburserEmail =
                  policy?.achAccount?.reimburser ?? policy?.owner;
                setWorkspaceReimbursement({
              ...
              subMenuItems: (
                <>
                  {shouldShowBankAccount ? (
                    <OfflineWithFeedback
                      pendingAction={bankAccountPendingAction}
                    >
                      <View
                        style={[
                          styles.sectionMenuItemTopDescription,
                      ...
                      >
                        <Text
                          style={[
                            styles.textLabelSupportingNormal,
                            styles.colorMuted,
                          ]}
                        >
                      ...
                      <MenuItem
                          ...
                          canWritePayments
                            ? () => {
                                ...
                                navigateToBankAccountRoute({
                                  policyID: route.params.policyID,
                                  backTo: ROUTES.WORKSPACE_WORKFLOWS.getRoute(
                                    route.params.policyID,
                                  ),
                                });
                              }
                        ...
                        sentryLabel={
                          CONST.SENTRY_LABEL.WORKSPACE.WORKFLOWS.BANK_ACCOUNT
                        }
                        badgeIcon={
                        ...
                        shouldGreyOutWhenDisabled={
                          !policy?.pendingFields?.reimbursementChoice
                        }
                        wrapperStyle={[
                          styles.sectionMenuItemTopDescription,
                          styles.mt3,
                      ...
                      <MenuItem
                        ...
                        onPress={() => {
                          ...
                          if (
                            ...
                            showConfirmModal({
                              title: translate(
                                "workspace.bankAccount.workspaceCurrencyNotSupported",
                              ),
                              prompt: updateWorkspaceCurrencyPrompt,
                              confirmText: translate(
                                "workspace.bankAccount.updateWorkspaceCurrency",
                              ),
                              cancelText: translate("common.cancel"),
                            }).then((result) => {
                              if (result.action !== ModalActions.CONFIRM) {
                  ...
                  {shouldShowPayer && (
                    <OfflineWithFeedback
                      ...
                      errors={getLatestErrorField(
                        policy ?? {},
                        CONST.POLICY.COLLECTION_KEYS.REIMBURSER,
                      )}
                      onClose={() =>
                        clearPolicyErrorField(
                          policy?.id,
                    ...
                    >
                      <MenuItemWithTopDescription
                        ...
                        onPress={() =>
                          Navigation.navigate(
                            ROUTES.WORKSPACE_WORKFLOWS_PAYER.getRoute(
                              route.params.policyID,
                            ),
                          )
                        }
              ...
              errors: getLatestErrorField(
                policy ?? {},
                CONST.POLICY.COLLECTION_KEYS.REIMBURSEMENT_CHOICE,
              ),
              onCloseError: () =>
                clearPolicyErrorField(
                  route.params.policyID,
  ...
  const renderOptionItem = (
    item: ToggleSettingOptionRowProps,
    index: number,
  ) => (
    <Section
  ...
  const isLoading = !!(
    policy?.isLoading && policy?.reimbursementChoice === undefined
  );
