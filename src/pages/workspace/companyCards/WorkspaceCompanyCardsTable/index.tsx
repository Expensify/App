...
function WorkspaceCompanyCardsTable({policyID, isPolicyLoaded, domainOrWorkspaceAccountID, companyCards, onAssignCard, isAssigningCardDisabled}: WorkspaceCompanyCardsTableProps) {
    ...
    const isDomainLevelFeed = selectedFeed?.preferredPolicy === policyID;
    let isLoadingCards;
    if (isDomainLevelFeed) {
        isLoadingCards = isLoadingOnyxValue(cardListMetadata);
    } else if (cardFeedType === 'directFeed') {
        isLoadingCards = selectedFeed?.accountList === undefined;
    } else {
        isLoadingCards = isLoadingOnyxValue(cardListMetadata) || cardList === undefined;
    }
