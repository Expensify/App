...
type ReportFieldsListValuesPageProps = WithPolicyAndFullscreenLoadingProps &
...
function ReportFieldsListValuesPage({
  ...
  const [formDraft] = useOnyx(
    ONYXKEYS.FORMS.WORKSPACE_REPORT_FIELDS_FORM_DRAFT,
  );
  const isMobileSelectionModeEnabled = useMobileSelectionMode();
  const illustrations = useMemoizedLazyIllustrations(["FolderWithPapers"]);
  const { showConfirmModal } = useConfirmModal();
  const { canWrite: canWriteReportFields, withReadOnlyFallback } =
  ...
  const [listValues, disabledListValues] = useMemo(() => {
    ...
    if (reportFieldID) {
      ...
      reportFieldValues = Object.values(
        policy?.fieldList?.[reportFieldKey]?.values ?? {},
      );
      reportFieldDisabledValues = Object.values(
        policy?.fieldList?.[reportFieldKey]?.disabledOptions ?? {},
      );
    } else {
    ...
    formDraft?.disabledListValues,
    formDraft?.listValues,
    policy?.fieldList,
    reportFieldID,
  ]);
  ...
  const openListValuePage = useCallback(
    (valueIndex: number) => {
      Navigation.navigate(
        ROUTES.WORKSPACE_REPORT_FIELDS_VALUE_SETTINGS.getRoute(
          policyID,
          valueIndex,
          reportFieldID,
        ),
    ...
    () =>
      listValues.map((value, index) => ({
        ...
        onToggleEnabled: (enabled: boolean) =>
          updateReportFieldListValueEnabled(enabled, index),
  ...
  const listValueRowsKeyed = useMemo(
    () =>
      listValueRows.reduce<Record<string, ReportFieldListValueRowData>>(
        (acc, row) => {
  ...
  const filterListValueRow = useCallback(
    (row?: ReportFieldListValueRowData) => !!row,
    [],
  );

  const [selectedKeys, setSelectedKeys] = useFilteredSelection(
    listValueRowsKeyed,
    filterListValueRow,
  );
  ...
  const clearTableSelection = useCallback(() => {
    setSelectedKeys((prevSelectedKeys) =>
      prevSelectedKeys.length > 0 ? [] : prevSelectedKeys,
    );
  }, [setSelectedKeys]);
  ...
  const handleDeleteValues = useCallback(() => {
    const valuesToDelete = selectedKeys
      .map((value) => listValues.indexOf(value))
    ...
    if (
        ...
        options.push({
          ...
          text: translate(
            selectedKeys.length === 1
              ? "workspace.reportFields.deleteValue"
              : "workspace.reportFields.deleteValues",
          ),
          ...
          onSelected: () => {
            showConfirmModal({
              ...
              prompt: translate(
                selectedKeys.length === 1
                  ? "workspace.reportFields.deleteValuePrompt"
                  : "workspace.reportFields.deleteValuesPrompt",
              ),
        ...
        options.push({
          ...
          text: translate(
            enabledValues.length === 1
              ? "workspace.reportFields.disableValue"
              : "workspace.reportFields.disableValues",
          ),
      ...
      if (disabledValues.length > 0) {
        ...
        options.push({
          ...
          text: translate(
            disabledValues.length === 1
              ? "workspace.reportFields.enableValue"
              : "workspace.reportFields.enableValues",
          ),
    ...
    if (canWriteReportFields && !hasAccountingConnections) {
      return (
        <Button
          ...
          onPress={() =>
            Navigation.navigate(
              ROUTES.WORKSPACE_REPORT_FIELDS_ADD_VALUE.getRoute(
                policyID,
                reportFieldID,
              ),
            )
  ...
  const emptyStateContent = (
    <ScrollView contentContainerStyle={[styles.flexGrow1, styles.flexShrink0]}>
      ...
      <EmptyStateComponent
        title={translate(
          "workspace.reportFields.emptyReportFieldsValues.title",
        )}
        subtitle={translate(
          "workspace.reportFields.emptyReportFieldsValues.subtitle",
        )}
    ...
    >
      ...
      >
        <HeaderWithBackButton
          title={translate(
            selectionModeHeader
              ? "common.selectMultiple"
              : "workspace.reportFields.listValues",
          )}
          onBackButtonPress={() => {
