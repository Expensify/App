...
function DynamicPayAndDowngradePage() {
  ...
  const backPath = useDynamicBackPath(
    DYNAMIC_ROUTES.WORKSPACE_PAY_AND_DOWNGRADE.path,
  );
  ...
  const items: BillingItem[] = useMemo(() => {
    ...
    const results = [
      ...billingDetails.receiptsWithoutDiscount,
      ...billingDetails.discounts,
    ].map((item) => {
      return {
  ...
  if (isLoadingOnyxValue(metadata)) {
    return (
      <FullScreenLoadingIndicator
        reasonAttributes={{ context: "DynamicPayAndDowngradePage" }}
      />
    );
  }
    ...
    >
      <FullPageNotFoundView shouldShow={isEmptyObject(billingDetails)}>
        ...
        <FullPageOfflineBlockingView>
          <ScrollView
            contentContainerStyle={[styles.flexGrow1, styles.ph5, styles.pt3]}
          >
            <Text style={[styles.textHeadlineH1, styles.mb5]}>
              {translate("workspace.payAndDowngrade.headline")}
            </Text>
            <View style={[styles.renderHTML]}>
              <RenderHTML
                html={translate(
                  "workspace.payAndDowngrade.description1",
                  billingDetails?.formattedSubtotal ?? "",
                )}
              />
            </View>
            <Text style={[styles.mb5]}>
              {translate(
                "workspace.payAndDowngrade.description2",
              ...
              {items.map((item) => (
                <View
                  ...
                  style={[
                    ...
                    styles.gap8,
                    styles.pv3,
                    !item.isTotal ? styles.borderBottom : undefined,
                  ]}
                >
                  {!item.isTotal ? (
                    <RenderHTML html={item.key} />
                  ) : (
                    <Text style={styles.textBold}>{item.key}</Text>
                  )}
                  <Text style={item.isTotal ? styles.textBold : undefined}>
                    {item.value}
                  </Text>
                </View>
              ))}
            </View>
            <Text style={[styles.textLabelSupportingNormal]}>
              {translate("workspace.payAndDowngrade.subscription")}
            </Text>
          </ScrollView>
          <FixedFooter style={[styles.mtAuto, styles.pt5]}>
            {!!errorMessage && (
              <View style={[styles.mb3]}>
                <FormHelpMessage isError message={errorMessage} />
              </View>
            )}
            <Button
              large
