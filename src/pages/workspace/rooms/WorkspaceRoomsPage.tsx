...
function WorkspaceRoomsPage({route}: WorkspaceRoomsPageProps) {
    ...
    const rooms: WorkspaceRoomRowData[] = (policyReports ?? []).map((report) => {
        ...
        return {
            ...
            action: () => {
                const targetRoute = isPolicyAdmin(policy)
                    ? createDynamicRoute(DYNAMIC_ROUTES.REPORT_DETAILS.getRoute(report.reportID), ROUTES.WORKSPACE_ROOMS.getRoute(policyID))
                    : ROUTES.REPORT_WITH_ID.getRoute(report.reportID);
