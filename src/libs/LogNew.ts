...
function serverLoggingCallback(logger: Logger, params: ServerLoggingCallbackOptions): Promise<{requestID: string}> {
    ...
    setTimeout(() => logger.info('Flushing logs older than 10 minutes', true, {}, true), 10 * 60 * 1000);
...
const createLogger = (shouldCollectLogs: boolean) =>
    new Logger({
        serverLoggingCallback,
        clientLoggingCallback: (message: string, extraData: string | Record<string, unknown> | Error | Array<Record<string, unknown>>) => {
