...
function hasRouteMatchingPolicyID(route: NavigationPartialRoute<CentralPaneScreen>, policyID?: string) {
    if (!route?.params) {
        return false;
    }
...
function getAlreadyOpenedSettingsScreen(rootState?: State, policyID?: string): keyof AuthScreensParamList | undefined {
    ...
    const alreadyOpenedSettingsTab = rootState.routes
        .filter((item) => item.params && 'screen' in item.params && TAB_TO_CENTRAL_PANE_MAPPING[SCREENS.SETTINGS.ROOT].includes(item.name as CentralPaneScreen))
        .at(-1);

    if (!hasRouteMatchingPolicyID(alreadyOpenedSettingsTab as NavigationPartialRoute<CentralPaneScreen>, policyID)) {
        return undefined;
    }
...
function getMatchingCentralPaneRouteForState(state: State<RootStackParamList>, rootState?: State): NavigationPartialRoute<CentralPaneScreen> | undefined {
    ...
    if (topmostBottomTabRoute.name === SCREENS.SETTINGS.ROOT) {
        ...
        return {name: screen as CentralPaneScreen, params: topmostBottomTabRoute.params};
    }
