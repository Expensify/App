...
function getMatchingRootRouteForRHPRoute(
...
): NavigationPartialRoute<typeof NAVIGATORS.CENTRAL_PANE_NAVIGATOR | typeof NAVIGATORS.WORKSPACE_NAVIGATOR> | undefined {
    ...
    if (route.params && 'backTo' in route.params && typeof route.params.backTo === 'string') {
        ...
        if (stateForBackTo) {
            ...
            const centralPaneOrFullScreenNavigator = stateForBackTo.routes.find(
                // eslint-disable-next-line @typescript-eslint/no-shadow
                (route) => route.name === NAVIGATORS.CENTRAL_PANE_NAVIGATOR || route.name === NAVIGATORS.WORKSPACE_NAVIGATOR,
            );
            ...
            if (centralPaneOrFullScreenNavigator && centralPaneOrFullScreenNavigator.state) {
                return centralPaneOrFullScreenNavigator as NavigationPartialRoute<typeof NAVIGATORS.CENTRAL_PANE_NAVIGATOR | typeof NAVIGATORS.WORKSPACE_NAVIGATOR>;
            }
        }
    }
    ...
    for (const [WorkspaceScreenName, RHPNames] of Object.entries(WORKSPACE_SCREEN_TO_RHP_MAPPING)) {
        if (RHPNames && RHPNames.includes(route.name)) {
            return createWorkspaceNavigator({name: WorkspaceScreenName as WorkspaceScreenName, params: route.params});
        }
    }
}
