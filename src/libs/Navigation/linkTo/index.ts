...
export default function linkTo(navigation: NavigationContainerRef<RootStackParamList> | null, path: Route, type?: string, isActiveRoute?: boolean) {
    ...
    const isWorkspaceScreenOnTop = rootState.routes?.at(-1)?.name === NAVIGATORS.WORKSPACE_NAVIGATOR;
    ...
    if (action?.type === CONST.NAVIGATION.ACTION_TYPE.NAVIGATE) {
        ...
        if (action.payload.name === NAVIGATORS.CENTRAL_PANE_NAVIGATOR && (isTargetScreenDifferentThanCurrent || areParamsDifferent)) {
            ...
            // All actions related to WorkspaceNavigator on wide screen are pushed when comparing differences between rootState and adaptedState.
            if (action.payload.name === NAVIGATORS.WORKSPACE_NAVIGATOR && !isNarrowLayout) {
                return;
            }
            action.type = CONST.NAVIGATION.ACTION_TYPE.PUSH;
