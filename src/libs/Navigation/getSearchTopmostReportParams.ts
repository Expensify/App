...
function getReportRHPParams(state: State): RightModalNavigatorParamList[typeof SCREENS.RIGHT_MODAL.SEARCH_REPORT] | undefined {
    ...
    const topmostRHPReport = lastRoute.state?.routes?.findLast((route) => route?.name === SCREENS.RIGHT_MODAL.SEARCH_REPORT || route?.name === SCREENS.RIGHT_MODAL.EXPENSE_REPORT);
    if (!topmostRHPReport) {
        return;
    }
