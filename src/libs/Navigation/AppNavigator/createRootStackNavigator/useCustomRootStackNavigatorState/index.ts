...
import reuseNavigatorKey, { cleanNavigationState } from './reuseNavigatorKey';
...
export default function useCustomRootStackNavigatorState({state}: CustomStateHookProps) {
    ...
    const cleanState = cleanNavigationState(state)
    const lastSplitIndex = cleanState.routes.findLastIndex((route) => isFullScreenName(route.name));
    let indexToSlice = Math.max(0, lastSplitIndex);
    const hasPrevRoute = lastSplitIndex > 0;
    const isPrevFullScreen = isFullScreenName(cleanState.routes.at(lastSplitIndex - 1)?.name);
    ...
    const routesToRender = cleanState.routes.slice(indexToSlice, cleanState.routes.length);
    return {...cleanState, routes: routesToRender, index: routesToRender.length - 1};
}
