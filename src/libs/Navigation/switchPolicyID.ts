...
export default function switchPolicyID(navigation: NavigationContainerRef<RootStackParamList> | null, policyID?: string, path?: Route) {
    if (!navigation) {
    ...
    const newPath = path ?? getPathFromState({routes: rootState.routes} as State, linkingConfig.config);
