    ...
    return new Promise((resolve) => {
        const connectionID = Onyx.connect({
            ...
            callback: (allReportActions) => {
                ...
                const allReportActionsEntires =
                    Object.entries(allReportActions);
                for (let i = 0; i < allReportActionsEntires.length; i++) {
                    const [onyxKey, reportActionsForReport] =
                        allReportActionsEntires[i];
                    if (reportActionsForReport) {
                        const newReportActionsForReport = {};
                        const reportActionsForReportEntries = Object.entries(
                            reportActionsForReport,
                        );
                        for (
                            let j = 0;
                            j < reportActionsForReportEntries.length;
                            j++
                        ) {
                            const [reportActionKey, reportAction] =
                                reportActionsForReportEntries[j];
                            if (
                                !_.isNaN(Number(reportActionKey)) &&
                                Number(reportActionKey) ===
                                    Number(reportAction.reportActionID) &&
                                Number(reportActionKey) !==
                                    Number(reportAction.sequenceNumber)
                            ) {
                                Log.info(
                ...
                Log.info(
                    `[Migrate Onyx] Re-keying reportActions by reportActionID for ${
                        _.keys(newReportActions).length
                    } reports`,
                );
