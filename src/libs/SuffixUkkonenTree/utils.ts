...
function stringToNumeric(
    ...
    options?: {
        ...
        out?: {
            outArray: DynamicArrayBuffer<Uint8Array>;
            ...
            outOccurrenceToIndex?: DynamicArrayBuffer<Uint32Array>;
    ...
    const outArray = options?.out?.outArray ?? new DynamicArrayBuffer(input.length * 6, Uint8Array);
    const offset = options?.out?.offset ?? {value: 0};
    const occurrenceToIndex = options?.out?.outOccurrenceToIndex ?? new DynamicArrayBuffer(input.length * 16 * 4, Uint32Array);
    ...
    for (let i = 0; i < input.length; i++) {
        ...
        if (char >= 'a' && char <= 'z') {
            ...
            outArray.set(offset.value++, char.charCodeAt(0) - CHAR_CODE_A);
        } else {
            const charCode = input.charCodeAt(i);
            occurrenceToIndex.set(offset.value, index);
            outArray.set(offset.value++, SPECIAL_CHAR_CODE);
            const asBase26Numeric = convertToBase26(charCode);
            // eslint-disable-next-line @typescript-eslint/prefer-for-of
            for (let j = 0; j < asBase26Numeric.length; j++) {
                occurrenceToIndex.set(offset.value, index);
                outArray.set(offset.value++, asBase26Numeric[j]);
            }
        }
    }
