...
//    Every ONYXKEYS entry (top-level + COLLECTION.*) that is not in
//    ONYX_KEY_EXPORT_RULES, safeOnyxKeys, or onyxKeysToRemove falls back here
//    and is processed by maskFragileData. This is the same treatment the
//    `main` branch applies to any key without an explicit rule, so populating
//    this bucket does not change `main` behavior.
// ============================================================
const allOnyxKeysToCategorize = new Set<string>([
    ...(Object.values(ONYXKEYS) as unknown[]).filter((value): value is string => typeof value === 'string'),
    ...Object.values(ONYXKEYS.COLLECTION),
]);

const explicitlyCategorizedKeys = new Set<string>([...Object.keys(ONYX_KEY_EXPORT_RULES), ...safeOnyxKeys, ...(Array.from(onyxKeysToRemove) as string[])]);

const onyxKeysToMaskFragileData = new Set<string>([...allOnyxKeysToCategorize].filter((key) => !explicitlyCategorizedKeys.has(key)));
