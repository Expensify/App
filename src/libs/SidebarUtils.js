...
function getOptionData(reportID) {
    ...
    if (result.isChatRoom || result.isPolicyExpenseChat) {
        ...
        result.alternateText = isPolicyChatAdmin ? subtitle : (lastMessageTextFromReport.length > 0 ? lastMessageText : Localize.translate(preferredLocale, 'report.noActivityYet'));
    } else {
        if (!lastMessageText) {
