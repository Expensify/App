...
function getOptionData(reportID) {
    ...
    let lastMessageText = '';
    if (report && lastMessageTextFromReport) {
        if (hasMultipleParticipants && lastActorDetails && lastActorDetails.accountID && Number(lastActorDetails.accountID) !== currentUserAccountID) {
            lastMessageText = `${lastActorDetails.displayName}: `;
        }
