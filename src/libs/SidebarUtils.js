...
function getOrderedReportIDs(reportIDFromRoute) {
    ...
    const reportsToDisplay = _.filter({...chatReports, ...iouReports},
        report => ReportUtils.shouldReportBeInOptionList(report, reportIDFromRoute, isInGSDMode, currentUserLogin, iouReports, betas, policies));
...
function getOptionData(reportID) {
    ...
    result.shouldShowSubscript = (result.isPolicyExpenseChat && !report.isOwnPolicyExpenseChat && !result.isArchivedRoom) || ReportUtils.isExpenseReport(report);
