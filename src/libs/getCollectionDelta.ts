...
/**
 * Computes the subset of collection members that changed between two snapshots — the replacement for
 * Onyx's removed `sourceValue`. Relies on Onyx structural sharing: unchanged members keep the same
 ...
 * reference-equal or no member differs) — mirroring the old `sourceValue === undefined` signal that
 * consumers branch on to mean "no delta to process".
 */
