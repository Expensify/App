 ...
 * `isVendorMatchingBetaEnabled` is consulted on every branch but QBO and Sage Intacct:
...
function hasVendorFeature(policy: OnyxEntry<Policy>, isVendorMatchingBetaEnabled: boolean): boolean {
    ...
    return (
        ...
        (isXeroVendorMatchingActive(policy) || isRilletVendorMatchingActive(policy) || isDualEntryVendorMatchingActive(policy) || isBusinessCentralVendorMatchingActive(policy))
    );
}
