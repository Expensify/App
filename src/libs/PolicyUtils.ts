 ...
 */
const isUserPolicyAdmin = (policy: OnyxInputOrEntry<Policy>, login?: string) => (getPolicyRole(policy, login) === CONST.POLICY.ROLE.ADMIN) || !!(policy && policy.employeeList && login && policy.employeeList[login]?.role === CONST.POLICY.ROLE.ADMIN);
