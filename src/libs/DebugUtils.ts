...
type ObjectType<T extends Record<string, unknown>> = Record<
  keyof T,
  "string" | "number" | "object" | "array" | "boolean" | ConstantEnum
>;
...
type ArrayTypeFromOnyxDefinition<T> = T extends unknown[]
  ? NonNullable<T[number]>
  : never;

type ArrayElement<
  TOnyx extends Record<string, unknown>,
  K extends keyof TOnyx,
> = ArrayTypeFromOnyxDefinition<Required<TOnyx>[K]>;
...
type ObjectElement<
  TOnyx,
  K extends keyof TOnyx,
  TCollectionKey extends string | number | undefined = undefined,
> =
  Required<TOnyx>[K] extends Record<string | number, infer ValueType>
    ? TCollectionKey extends string | number
      ...
      : {
          [ElementKey in KeysOfUnion<Required<TOnyx>[K]>]: Required<
            Required<TOnyx>[K]
          >[ElementKey];
        }
    : never;
...
const REPORT_REQUIRED_PROPERTIES: Array<
  keyof Report | keyof ReportNameValuePairs
> = ["reportID"] satisfies Array<keyof Report | keyof ReportNameValuePairs>;

const REPORT_ACTION_REQUIRED_PROPERTIES: Array<keyof ReportAction> = [
  "reportActionID",
  "created",
  "actionName",
...
const TRANSACTION_REQUIRED_PROPERTIES: Array<keyof Transaction> = [
  "transactionID",
  "reportID",
  "amount",
  "created",
...
const TRANSACTION_VIOLATION_REQUIRED_PROPERTIES: Array<
  keyof TransactionViolation
> = ["type", "name"] satisfies Array<keyof TransactionViolation>;
...
type OnyxData<T extends OnyxDataType> =
  | (T extends "number"
      ? number
      : T extends "object"
 ...
 */
function stringToOnyxData<T extends OnyxDataType = "string">(
  data: string,
  type?: T,
): OnyxData<T> {
...
function validateDate(value: string) {
  if (
    isEmptyValue(value) ||
    ((isMatch(value, CONST.DATE.FNS_DB_FORMAT_STRING) ||
      isMatch(value, CONST.DATE.FNS_FORMAT_STRING)) &&
      ...
      for (const [key, val] of Object.entries(element)) {
        ...
        if (
          Array.isArray(expectedType)
            ? !expectedType.includes(typeof val as TupleToUnion<PropertyTypes>)
...
function validateObject<T extends Record<string, unknown>>(
  value: string,
  type: ObjectType<T>,
  collectionIndexType?: "string" | "number",
) {
  if (isEmptyValue(value)) {
    return;
  }
  ...
  const tests = collectionIndexType
    ? (Object.values(object) as unknown as Array<
...
function validateReportDraftProperty(
  key: keyof Report | keyof ReportNameValuePairs,
  value: string,
) {
  ...
  switch (key) {
    ...
    case "errorFields":
      return validateObject<ObjectElement<Report, "errorFields", string>>(
        value,
        {},
        "string",
      );
    ...
    case "calendlySchedule":
      return validateObject<
        ObjectElement<ReportNameValuePairs, "calendlySchedule">
      >(value, {
        isLoading: "boolean",
        data: "object",
        errors: "object",
    ...
    case "calendlyCalls":
      return validateArray<ArrayElement<ReportNameValuePairs, "calendlyCalls">>(
        value,
        {
          status: "string",
          host: "number",
    ...
    case "pendingFields":
      return validateObject<
        ObjectElement<Report | ReportNameValuePairs, "pendingFields">
      >(value, {
    ...
    case "expensify_text_title":
      return validateObject<
        ObjectElement<ReportNameValuePairs, "expensify_text_title">
      >(value, {
        fieldID: "string",
        type: "string",
        name: "string",
 ...
 * @param value - value provided by the user
 */
function validateReportActionDraftProperty(
  key: keyof ReportAction,
  value: string,
) {
  ...
  switch (key) {
    ...
    case "pendingAction":
      return validateConstantEnum(value, CONST.RED_BRICK_ROAD_PENDING_ACTION);
    case "pendingFields":
      return validateObject<ObjectElement<ReportAction, "pendingFields">>(
        value,
        {
          reportID: CONST.RED_BRICK_ROAD_PENDING_ACTION,
    ...
    case "childRecentReceiptTransactionIDs":
      return validateObject<
        ObjectElement<ReportAction, "childRecentReceiptTransactionIDs">
      >(value, {}, "string");
    case "linkMetadata":
    ...
    case "originalMessage":
      return validateObject<ObjectElement<ReportAction, "originalMessage">>(
        value,
        {},
      );
    case "previousMessage":
      return unionValidation(
        () =>
          validateObject<ObjectElement<ReportAction, "previousMessage">>(
            value,
            {
              html: "string",
              text: "string",
 ...
 * @param value - value provided by the user
 */
function validateTransactionDraftProperty(
    ...
    case "pendingFields":
      return validateObject<ObjectElement<Transaction, "pendingFields">>(
        value,
        {
          ...
          isSingleTransactionMode: CONST.RED_BRICK_ROAD_PENDING_ACTION,
        },
        "string",
      );
    case "receipt":
...
function validateTransactionViolationDraftProperty(
  key: keyof TransactionViolation,
  value: string,
) {
  if (
    TRANSACTION_VIOLATION_REQUIRED_PROPERTIES.includes(key) &&
    isEmptyValue(value)
  ) {
    throw SyntaxError("debug.missingValue");
  }
  switch (key) {
    ...
    case "data":
      return validateObject<ObjectElement<TransactionViolation, "data">>(
        value,
        {
          rejectedBy: "string",
          rejectReason: "string",
...
function validateReportActionJSON(json: string) {
  ...
  for (const [key, val] of Object.entries(parsedReportAction)) {
    try {
      if (
        !isEmptyValue(val as string) &&
        REPORT_ACTION_NUMBER_PROPERTIES.includes(key as keyof ReportAction) &&
        typeof val !== "number"
      ) {
        throw new NumberError();
      }
      validateReportActionDraftProperty(
        key as keyof ReportAction,
        onyxDataToString(val),
      );
    } catch (e) {
  ...
  for (const [key, val] of Object.entries(parsedTransactionViolation)) {
    try {
      validateTransactionViolationDraftProperty(
        key as keyof TransactionViolation,
        onyxDataToString(val),
      );
    } catch (e) {
      const { cause } = e as SyntaxError & {
...
function getReasonForShowingRowInLHN({
  ...
  if (
    !(
      [
        CONST.REPORT_IN_LHN_REASONS.HAS_ADD_WORKSPACE_ROOM_ERRORS,
        CONST.REPORT_IN_LHN_REASONS.HAS_IOU_VIOLATIONS,
      ] as Array<typeof reason>
    ).includes(reason) &&
    hasRBR
  ) {
