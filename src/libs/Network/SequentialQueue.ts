...
function saveQueueFlushedData<TKey extends OnyxKey>(
  ...onyxUpdates: Array<OnyxUpdate<TKey>>
) {
    ...
    .catch((error: RequestError) => {
      ...
      if (
        ...
        if (shouldFailAllRequests) {
          const onyxUpdates = [
            ...(requestToProcess.failureData ?? []),
            ...(requestToProcess.finallyData ?? []),
          ] as AnyOnyxUpdate[];
          Log.info(
            "[SequentialQueue] Applying failure and finally data because shouldFailAllRequests",
            false,
            {
              command: requestToProcess.command,
      ...
      if (error.message === CONST.ERROR.ALREADY_CREATED) {
        const onyxUpdates = [
          ...(requestToProcess.successData ?? []),
          ...(requestToProcess.finallyData ?? []),
        ] as AnyOnyxUpdate[];
        Log.info(
          "[SequentialQueue] Applying success and finally data on ALREADY_CREATED — resource already exists server-side",
          false,
          {
            command: requestToProcess.command,
            updatesCount: onyxUpdates.length,
        ...
        Onyx.update(onyxUpdates);
        Log.info(
          "[SequentialQueue] Removing persisted request because the resource was already created server-side.",
          false,
          {
            command: requestToProcess.command,
            errorMessage: error.message,
      ...
      if (
        ...
        requestToProcess.command === WRITE_COMMANDS.RESEND_VALIDATE_CODE
      ) {
        Log.info(
          "[SequentialQueue] RESEND_VALIDATE_CODE throttled, not retrying",
          false,
          {
            command: requestToProcess.command,
          },
      ...
      Log.info(
        "[SequentialQueue] Will retry request after rollback and throttle delay",
        false,
        {
          command: requestToProcess.command,
          errorMessage: error.message,
      ...
      return sequentialQueueRequestThrottle
        ...
        .then(() => {
          Log.info(
            "[SequentialQueue] Throttle delay completed, retrying request",
            false,
            {
              command: requestToProcess.command,
            },
        ...
        .catch(() => {
          Log.info(
            "[SequentialQueue] Request failed too many times, giving up",
            false,
            {
              command: requestToProcess.command,
              errorMessage: error.message,
...
function flush(shouldResetPromise = true) {
  ...
  if (
    persistedRequestsLength === 0 &&
    !currentOngoingRequest &&
    !hasOnyxUpdates
  ) {
    Log.info(
  ...
  if (!isClientTheLeader()) {
    Log.info(
      "[SequentialQueue] Unable to flush. Client is not the leader.",
      false,
      {
        persistedRequestsLength,
        hasOngoingRequest: !!currentOngoingRequest,
  ...
  const connection = Onyx.connectWithoutView({
    ...
    callback: () => {
      Log.info(
        "[SequentialQueue] PERSISTED_REQUESTS loaded, starting process()",
        false,
        {
      ...
      process().finally(() => {
        ...
        const hasRemainingRequests =
          remainingPersistedRequests > 0 || hasOngoingRequest;
        ...
        if (!hasRemainingRequests) {
          ...
          flushOnyxUpdatesQueue()?.then(() => {
            ...
            Onyx.update(queueFlushedData).then(() => {
              Log.info(
                "[SequentialQueue] QueueFlushedData has been applied and stored",
                false,
                {
                  queueFlushedDataLength: queueFlushedData.length,
                },
          ...
          Log.info(
            "[SequentialQueue] Queue still has requests, NOT flushing Onyx updates",
            false,
            {
              remainingRequests: remainingPersistedRequests,
              hasOngoingRequest,
...
function unpause() {
  ...
  if (numberOfPersistedRequests === 0 && !currentOngoingRequest) {
    Log.info(
      "[SequentialQueue] No persisted requests, flushing Onyx updates queue",
    );
  ...
  Log.info(
    "[SequentialQueue] Calling flush(false) to start processing",
    false,
    {
      numberOfPersistedRequests,
    },
...
async function handleConflictActions<TKey extends OnyxKey>(
  conflictAction: ConflictData,
  newRequest: OnyxRequest<TKey>,
): Promise<void> {
  ...
  if (conflictAction.type === "push") {
    ...
    await updatePersistedRequest(
      conflictAction.index,
      conflictAction.request ?? (newRequest as AnyRequest),
    );
...
async function push<TKey extends OnyxKey>(
  newRequest: OnyxRequest<TKey>,
): Promise<void> {
  ...
  if (newRequest.checkAndFixConflictingRequest) {
    const { conflictAction } = newRequest.checkAndFixConflictingRequest(
      currentRequests as Array<OnyxRequest<TKey>>,
    );
  ...
  if (isOfflineNetwork()) {
    Log.info(
      "[SequentialQueue] Request persisted but not flushing — we are offline",
      false,
      {
        command: newRequest.command,
  ...
  if (isOfflineNetwork()) {
    Log.info(
      "[SequentialQueue] Went offline during persist — resolving isReadyPromise without flushing",
      false,
      {
        command: newRequest.command,
      },
  ...
  if (isSequentialQueueRunning) {
    Log.info(
      "[SequentialQueue] Queue is running. Will flush when the current request is finished.",
      false,
      {
        command: newRequest.command,
      },
  ...
  Log.info(
    "[SequentialQueue] Queue is not running. Flushing the queue.",
    false,
    {
      command: newRequest.command,
    },
