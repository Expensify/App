 ...
 * 3. Throw an "already authenticating" error if we try to authenticate and we are already in the process of authenticating
 * 4. Throw an "unable to reauthenticate" error if the call the API.reauthenticate() fails
 * 5. Resolve a special jsonCode response that tells the queues that reauthentication was successful so they can replay the original requests.
 *
 * @param {Object} request
 * @param {Object} failedResponse
 * @param {Boolean} isFromSequentialQueue
 * @returns {Promise}
 */
function reauthenticate(request, failedResponse) {
    ...
    if (unableToReauthenticate) {
        return Promise.reject(new Error(CONST.ERROR.MISSING_CREDENTIALS));
    }
    ...
    // If we are already authenticating let each queue handle this reauthentication attempt
    if (NetworkStore.getIsAuthenticating()) {
        return Promise.reject(new Error(CONST.ERROR.ALREADY_AUTHENTICATING));
    }

    // Create new promise and then call API.reauthenticate() via NetworkEvents.triggerAuthTokenExpired()
    return new Promise((resolve, reject) => {
        NetworkEvents.triggerAuthTokenExpired(request.command, resolve, reject);
    })
        .then(() => ({jsonCode: CONST.JSON_CODE.REAUTHENTICATED}))
        .catch(() => {
...
export default function processRequest(request) {
    ...
    return HttpUtils.xhr(request.command, finalParameters, request.type, request.shouldUseSecure)
        .then((response) => {
            if (request.command !== 'Log') {
                NetworkEvents.getLogger().info('Finished API request', false, {
                    command: request.command,
                    type: request.type,
                    shouldUseSecure: request.shouldUseSecure,
            ...
            if (response.jsonCode === CONST.JSON_CODE.NOT_AUTHENTICATED) {
                return reauthenticate(request, response);
            }

            return response;
        })
        .catch((error) => {
            // If we were not able to reauthenticate re-throw this error so that the caller of processRequest() can handle it
            if (_.contains([CONST.ERROR.UNABLE_TO_REAUTHENTICATE, CONST.ERROR.ALREADY_AUTHENTICATING, CONST.ERROR.MISSING_CREDENTIALS], error.message)) {
                throw error;
            }
