 ...
 * @returns {Promise}
 */
function reauthenticateAndRetryRequest(request, failedResponse, isFromSequentialQueue) {
    ...
    return new Promise((resolve, reject) => {
        NetworkEvents.triggerAuthTokenExpired(request, resolve, reject, isFromSequentialQueue);
    })
        .then((response) => {
            if (request.resolve) {
 ...
 * @returns {Promise}
 */
export default function processRequest(request, isFromSequentialQueue) {
    ...
    return HttpUtils.xhr(request.command, finalParameters, request.type, request.shouldUseSecure)
        .then((response) => {
            ...
            if (response.jsonCode === CONST.JSON_CODE.NOT_AUTHENTICATED) {
                return reauthenticateAndRetryRequest(request, response, isFromSequentialQueue);
            }

            // Persisted requests do not have a resolve method so we check for that here.
        ...
        .catch((error) => {
            // If we were not able to reauthenticate then re-throw this error so that the SequentialQueue can handle it.
            // Note: We should not see this error for requests in the main queue as they are retried asynchronously.
            if (error.message === CONST.ERROR.UNABLE_TO_REAUTHENTICATE) {
                throw error;
            }
