...
function reauthenticate(request, failedResponse) {
    ...
    if (unableToReauthenticate) {
        return Promise.reject(new Error(CONST.ERROR.UNABLE_TO_REAUTHENTICATE));
    }
...
export default function processRequest(request) {
    ...
    return HttpUtils.xhr(request.command, finalParameters, request.type, request.shouldUseSecure)
        ...
        .catch((error) => {
            // If we were not able to reauthenticate or we are already authenticating then re-throw this error so that the caller of processRequest() can handle it
