...
import type {ChangeLog, OriginalMessageRenamed} from '@src/types/onyx/OriginalMessage';
...
function getOptionData({
    ...
    if ((result.isChatRoom || result.isPolicyExpenseChat || result.isThread || result.isTaskReport || isThreadMessage) && !result.isArchivedRoom) {
        ...
        if (lastAction?.actionName === CONST.REPORT.ACTIONS.TYPE.RENAMED) {
            ...
            const lastActionOriginalMessage = lastAction?.actionName ? ReportActionsUtils.getReportActionOriginalMessage<ChangeLog>(lastAction) : null;
