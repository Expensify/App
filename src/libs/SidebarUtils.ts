...
function getOptionData({
    ...
    if (!lastMessageTextFromReport) {
        lastMessageTextFromReport = getLastMessageTextForReport({report, lastActorDetails, movedFromReport, movedToReport, policy, isReportArchived, policyTags});
    }
    ...
    if ((result.isChatRoom || result.isPolicyExpenseChat || result.isThread || result.isTaskReport || isThreadMessage || isGroupChat) && !isReportArchived) {
        ...
        if (isRenamedAction(lastAction)) {
            ...
            const roomName = getReportName({report: lastActionReport, policyTags}) || lastActionOriginalMessage?.roomName;
            if (roomName) {
                const preposition =
