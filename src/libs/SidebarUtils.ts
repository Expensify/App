...
function getOptionData({
    ...
    result.isUnread = isUnread(report, oneTransactionThreadReport, isReportArchived, reportActions) && !!report.lastActorAccountID;
    ...
    if ((result.isChatRoom || result.isPolicyExpenseChat || result.isThread || result.isTaskReport || isThreadMessage || isGroupChat) && !isReportArchived) {
        ...
        if (isRenamedAction(lastAction)) {
            ...
            result.alternateText = Parser.htmlToText(getUnreportedTransactionMessage(translate, lastAction, formatPhoneNumber));
            ...
            result.alternateText = Parser.htmlToText(getMovedTransactionMessage(translate, lastAction, formatPhoneNumber));
...
function getWelcomeMessage(params: WelcomeMessageParams): WelcomeMessage {
    ...
    if (isChatRoom(report)) {
        return getRoomWelcomeMessage(translate, report, invoiceReceiverPolicy, formatPhoneNumber, reportAttributes, isReportArchived, reportDetailsLink);
    }
