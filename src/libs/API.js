...
function addAuthTokenToParameters(command, parameters) {
    ...
    if (isAuthTokenRequired(command) && !parameters.authToken) {
        ...
        // In this case, we should cancel the current request and clear the queue.
        if (!authToken) {
            redirectToSignIn();
        ...
        networkPromise
            .then((response) => {
                ...
                if (response.jsonCode === 407) {
                    ...
                    if (!credentials || !credentials.autoGeneratedLogin || !credentials.autoGeneratedPassword) {
                        // If we are here with invalid credentials it means we are trying to make an API request
                        // before we are signed in. In this case, we should not continue.
                        console.debug('A request tried to renew an authToken without credentials.', {
                            command,
                            parameters,
                        });

                        throw new Error('A request tried to renew an authToken without credentials.');
                    }
