...
function createOption(
    ...
    if (report) {
        ...
        // eslint-disable-next-line @typescript-eslint/no-deprecated
        result.lastMessageText = getLastMessageTextForReport({translate: translateLocal, report, lastActorDetails, isReportArchived: !!result.private_isArchived, chatReport});
...
function processReport(
    ...
    return {
        ...
        reportOption: {
            item: report,
            ...createOption(accountIDs, personalDetails, report, undefined, reportAttributesDerived, undefined, reports),
        },
    };
}
...
function createFilteredOptionList(
    ...
    const personalDetailsOptions = includeP2P
        ? Object.values(personalDetails ?? {}).map((personalDetail) => {
              ...
              return {
                  item: personalDetail,
                  ...createOption([accountID], personalDetails, mappedReport, {showPersonalDetails: true}, reportAttributesDerived, undefined, reports),
              };
          })
        : [];
...
function createOptionFromReport(
    ...
    return {
        item: report,
        ...createOption(accountIDs, personalDetails, report, config, reportAttributesDerived, undefined, reports),
    };
}
