 ...
 */
function getLastActorDisplayName(lastActorDetails: Partial<PersonalDetails> | null, currentUserAccountID: number | undefined) {
    if (!lastActorDetails) {
        return '';
    }
...
function getAlternateText(
    ...
    const formattedLastMessageText =
        formatReportLastMessageText(Parser.htmlToText(option.lastMessageText ?? '')) || getLastMessageTextForReport({report, lastActorDetails, isReportArchived, currentUserAccountID});
...
function getLastActorDisplayNameFromLastVisibleActions(report: OnyxEntry<Report>, lastActorDetails: Partial<PersonalDetails> | null, currentUserAccountID: number | undefined): string {
    const reportID = report?.reportID;
...
function createOption(
    ...
    if (report) {
        ...
        result.lastMessageText = getLastMessageTextForReport({report, lastActorDetails, isReportArchived: !!result.private_isArchived, currentUserAccountID});
 ...
 */
function isCurrentUser(userDetails: PersonalDetails, loginList: OnyxEntry<Login>, currentUserEmail: string | undefined): boolean {
    if (!userDetails) {
        return false;
    }
 ...
 */
function prepareReportOptionsForDisplay(options: Array<SearchOption<Report>>, policiesCollection: OnyxCollection<Policy>, config: GetValidReportsConfig): Array<SearchOption<Report>> {
    const {
        showChatPreviewLine = false,
        forcePolicyNamePreview = false,
        action,
...
function filterUserToInvite(
    ...
    const {canInviteUser = true, excludeLogins = {}, currentUserEmail, currentUserAccountID} = config ?? {};
    if (!canInviteUser) {
        return null;
    }
...
function getManagerMcTestParticipant(currentUserAccountID: number | undefined): Participant | undefined {
