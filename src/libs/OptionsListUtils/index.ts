...
function getLastMessageTextForReport({
    ...
    if (isArchivedNonExpenseReport(report, isReportArchived)) {
        ...
        if (reportUtilsIsPolicyExpenseChat(report) && !isEmptyObject(iouReport)) {
            const reportName = computeReportNameWithoutFormula(iouReport);
...
function createOption(
    ...
    if (report) {
        ...
        const computedReportName = computeReportNameWithoutFormula(report, allReports, allPolicies, undefined, allReportNameValuePairs, personalDetailsForCompute, allReportActions);
...
function getReportOption(participant: Participant, reportAttributesDerived?: ReportAttributesDerivedValue['reports'], reportDrafts?: OnyxCollection<Report>): OptionData {
    ...
    if (option.isSelfDM) {
        ...
        option.text = computeReportNameWithoutFormula(report, undefined, undefined, undefined, allReportNameValuePairs, allPersonalDetails, undefined);
        // eslint-disable-next-line @typescript-eslint/no-deprecated
