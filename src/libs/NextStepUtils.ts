 ...
 * @private
 */
function getNextApproverDisplayName(report: OnyxEntry<Report>, translateFn: LocaleContextProps['translate'] | undefined, isUnapprove?: boolean) {
    const approverAccountID = getNextApproverAccountID(report, isUnapprove);

    return (
        getDisplayNameForParticipant({accountID: approverAccountID, formatPhoneNumber: formatPhoneNumberPhoneUtils, translate: translateFn}) ??
        getPersonalDetailsForAccountID(approverAccountID).login
    );
}
...
function buildNextStepNew(params: BuildNextStepNewParams): ReportNextStepDeprecated | null {
    ...
    const nextApproverDisplayName = bypassNextApproverID
        ...
        : getNextApproverDisplayName(report, translate, isUnapprove);
    const approverAccountID = bypassNextApproverID ?? getNextApproverAccountID(report, isUnapprove);
    // eslint-disable-next-line @typescript-eslint/no-deprecated
    ...
    switch (predictedNextStatus) {
        ...
        case CONST.REPORT.STATUS_NUM.APPROVED: {
            ...
            if (isPayer(currentUserAccountIDParam, currentUserEmailParam, report, undefined)) {
                ...
                payerMessage = {
                    text: getDisplayNameForParticipant({accountID: reimburserAccountID, formatPhoneNumber: formatPhoneNumberPhoneUtils, translate}),
                    type: 'strong',
                };
            }
