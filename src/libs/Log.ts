...
function serverLoggingCallback(
  ...
  return Promise.allSettled(requests).then((results) => {
    const fulfilled = results.filter(
      (r): r is PromiseFulfilledResult<{ requestID: string }> =>
        r.status === "fulfilled",
    );
    ...
    if (fulfilled.length > 0) {
      ...
      if (rejected.length > 0) {
        ...
        console.error(
          `[Log] ${rejected.length} of ${results.length} log groups failed to upload and will not be retried`,
        );
      }
...
const appGroupName = HybridAppModule.isHybridApp()
  ? "group.com.expensify"
