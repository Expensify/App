...
export default function getAttachmentName(html) {
    ...
    const PREVIEW_SOURCE_REGEX = new RegExp(
        `${CONST.ATTACHMENT_PREVIEW_ATTRIBUTE}*=*"(.+?)"`,
        'i',
    );
    ...
    const ORIGINAL_FILENAME_REGEX = IS_IMAGE_TAG
        ? new RegExp(
              `${CONST.ATTACHMENT_ORIGINAL_FILENAME_ATTRIBUTE}*=*"(.+?)"`,
              'i',
          )
        : new RegExp('<a[^>]*>([^<]+)</a>', 'i');
    if (!html) {
    ...
    const imageURL =
        IS_IMAGE_TAG &&
        tryResolveUrlFromApiRoot(html.match(PREVIEW_SOURCE_REGEX)[1]);
