...
function processReportActionEntry(ctx: PreprocessingContext, key: string, actions: Record<string, OnyxTypes.ReportAction>): void {
    ...
    let earliestApprovedTime = Infinity;
    let earliestApprovedAction: OnyxTypes.ReportAction | undefined;
    ...
    for (const action of Object.values(actions)) {
        ...
        if (isApprovedAction(action)) {
            const currentTime = new Date(action.created).getTime();
            if (currentTime < earliestApprovedTime || (currentTime === earliestApprovedTime && earliestApprovedAction && action.reportActionID < earliestApprovedAction.reportActionID)) {
                earliestApprovedTime = currentTime;
                earliestApprovedAction = action;
            }
        }
    ...
    if (earliestApprovedAction) {
        ctx.firstApprovedActionByReportID.set(reportID, earliestApprovedAction);
        if (!ctx.shouldShowYearFirstApprovedReport && DateUtils.doesDateBelongToAPastYear(earliestApprovedAction.created)) {
    ...
    for (const key of orderedKeys) {
        if (isReportEntry(key) && (data[key].type === CONST.REPORT.TYPE.IOU || data[key].type === CONST.REPORT.TYPE.EXPENSE || data[key].type === CONST.REPORT.TYPE.INVOICE)) {
            ...
            if (shouldShow) {
                ...
                const firstApproverAccountID = firstApprovedAction?.actorAccountID ?? reportItem.managerID;
                ...
                reportIDToTransactions[reportKey] = {
                    ...
                    firstApproved: firstApprovedAction?.created ?? reportItem.approved ?? '',
