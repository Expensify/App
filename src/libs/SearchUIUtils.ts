...
/**
 * Whether the edited filters can be saved as a view. The edit footer is only actionable when the edited query isn't
 * already some saved view's query: an unchanged query has nothing new to save, and one matching another view would just
 * duplicate it (the LHN resolves a query to the first view with that hash). Shared by the wide popover and the narrow
 * fullscreen footer.
 */
function canSaveEditedView(savedSearches: OnyxEntry<SaveSearch>, editedQueryHash: number | undefined): boolean {
    if (editedQueryHash === undefined) {
        return false;
    }
    return !Object.values(savedSearches ?? {}).some((savedSearch) => buildSearchQueryJSON(savedSearch.query)?.hash === editedQueryHash);
}
...
function getOverflowMenu(
    ...
    return [
        ...
        // Only offered when the caller can open the edit-filters UI.
        ...(onEditFilters
            ? [
                  {
