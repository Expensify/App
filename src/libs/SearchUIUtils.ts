...
function getSuggestedSearches(defaultFeedID: string | undefined, accountID: number = CONST.DEFAULT_NUMBER_ID): Record<ValueOf<typeof CONST.SEARCH.SEARCH_KEYS>, SearchTypeMenuItem> {
    return {
        ...
        [CONST.SEARCH.SEARCH_KEYS.RECONCILIATION]: {
            ...
            searchQuery: buildQueryStringFromFilterFormValues({
                type: CONST.SEARCH.DATA_TYPES.EXPENSE,
                groupBy: CONST.SEARCH.GROUP_BY.BANK_WITHDRAWAL,
                withdrawalType: CONST.SEARCH.WITHDRAWAL_TYPE.BANK_WITHDRAWAL,
            }),
            get hash() {
                return buildSearchQueryJSON(this.searchQuery)?.hash ?? CONST.DEFAULT_NUMBER_ID;
            },
...
function getSuggestedSearchesVisibility(
    ...
    Object.values(policies ?? {}).some((policy) => {
        ...
        const hasVBBA = !!policy.achAccount?.bankAccountID;
        ...
        const isEligibleForReconciliationSuggestion = isPaidPolicy && isAdmin && ((isPaymentEnabled && hasReimburser && hasVBBA) || isECardEnabled);
