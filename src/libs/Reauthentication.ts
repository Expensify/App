...
function reauthenticate(command = ''): Promise<boolean> {
    ...
    return hasReadRequiredDataFromStorage().then(() => {
        ...
        if (account?.isSAMLRequired && (!credentials?.autoGeneratedLogin || !credentials?.autoGeneratedPassword)) {
            Log.info(`[Reauthenticate] Redirecting to Sign In because SAML is required and no auto-generated credentials are available`);
