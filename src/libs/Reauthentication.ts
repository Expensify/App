...
Onyx.connectWithoutView({
    ...
    callback: (value) => {
        ...
        // Copilot and supportal sessions had no Sentry field that identified them. The session already carries
        // authTokenType, an existing enum, so tag it where the user context is set. Passing null when the session
        // clears stops a signed-out event from keeping a stale tag.
        Sentry.setTag(CONST.TELEMETRY.TAGS.AUTH_TOKEN_TYPE, value?.authTokenType ?? null);
    },
});
...
function reauthenticate(command = ''): Promise<ReauthenticationResult> {
    ...
    return hasReadRequiredDataFromStorage().then(() => {
        ...
        if (!credentials?.autoGeneratedLogin || !credentials?.autoGeneratedPassword) {
            const errorMessage = 'No credentials available';
            Log.info('[Reauthenticate] No credentials available, redirecting to sign in');
            // Report through trackAuthenticationError, as the two branches below do. This exit only ever logged, so a
            // session that had lost its credentials left no Sentry event and the 407s that followed looked
            // unexplained.
            trackAuthenticationError(new Error(errorMessage), {
                errorType: 'missing_params',
            ...
            setIsAuthenticating(false);
            redirectToSignIn(CONST.SIGN_OUT_REASON.NO_CREDENTIALS, errorMessage);
            return {wasSuccessful: false};
        }
