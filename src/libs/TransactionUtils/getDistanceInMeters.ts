...
import type {OnyxInputOrEntry, ReadonlyOnyx, Transaction} from '@src/types/onyx';
...
function getDistanceInMeters(transaction: ReadonlyOnyx<OnyxInputOrEntry<Transaction>>, unit: Unit | undefined) {
    ...
    // Routes are cleared once the request is saved, leaving `routeDistanceMeters` (already in meters) as the only distance until the backend writes `quantity` back.
