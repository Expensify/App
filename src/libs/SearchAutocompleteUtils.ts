...
function filterOutRangesWithCorrectValue(
    ...
    const billableTypeList = Object.values(CONST.SEARCH.BILLABLE) as string[];
    const reimbursableTypeList = Object.values(CONST.SEARCH.REIMBURSABLE) as string[];
    ...
    switch (range.key) {
        ...
        case CONST.SEARCH.SYNTAX_FILTER_KEYS.BILLABLE:
            return billableTypeList.includes(range.value);
        case CONST.SEARCH.SYNTAX_FILTER_KEYS.REIMBURSABLE:
            return reimbursableTypeList.includes(range.value);
        default:
            return false;
    }
}
