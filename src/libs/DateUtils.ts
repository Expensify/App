 ...
 * param datetime
 * param dateFormat
 * param dateFnsLocale - language to resolve locale-sensitive tokens such as MMM in, falling back to the date-fns global default when omitted.
 * returns If the date is valid, returns the formatted date with the UTC timezone, otherwise returns an empty string.
 */
function formatWithUTCTimeZone(datetime: string, dateFormat: string = CONST.DATE.FNS_FORMAT_STRING, dateFnsLocale?: DateFnsLocale) {
