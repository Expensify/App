...
import {getCleanedTagName, getCommaSeparatedTagNameWithSanitizedColons, getMatchingVendorByID, getSortedTagKeys, isPolicyAdmin} from './PolicyUtils';
...
function getForReportAction({
    ...
    if (hasModifiedVendor) {
        ...
        // the display name from the policy's matching vendor list (QBO or Intacct); if the vendor
        // has since been removed the name is unrecoverable, so fall back to the externalID so the
        // fragment still identifies which vendor was set rather than rendering `set vendor ""`.
        const resolveVendorName = (entry: typeof reportActionOriginalMessage.vendor): string => {
            if (!entry?.externalID) {
                return '';
            }
            return getMatchingVendorByID(policy, entry.externalID)?.name ?? entry.externalID;
        };
        buildMessageFragmentForValue(
            translate,
