...
const routeFocusProvenanceMap = new Map<string, InteractionProvenance>();
...
function updateRouteFocusEntry(routeKey: string, update: RouteFocusEntryUpdate): void {
    ...
    if (update.provenance !== undefined) {
        if (update.provenance) {
            routeFocusProvenanceMap.set(routeKey, update.provenance);
        } else {
            routeFocusProvenanceMap.delete(routeKey);
        }
    }
}

function clearRouteFocusEntry(routeKey: string): void {
    updateRouteFocusEntry(routeKey, {element: null, identifier: null, provenance: null});
}
...
function isKeyboardRouteRestoreEligible(provenance: InteractionProvenance | null | undefined): boolean {
    return provenance?.interactionType === 'keyboard' && provenance.interactionTrigger === 'enterOrSpace';
}
...
function captureForRoute(routeKey: string): void {
    ...
    if (elementToStore) {
        updateRouteFocusEntry(routeKey, {
            ...
            identifier: extractElementIdentifier(elementToStore),
            provenance: lastInteractionProvenance ?? routeFocusProvenanceMap.get(routeKey) ?? null,
        });
        logFocusDebug('[NavigationFocusManager] Stored focus for route', {
            ...
            interactionType: (lastInteractionProvenance ?? routeFocusProvenanceMap.get(routeKey))?.interactionType,
            interactionTrigger: (lastInteractionProvenance ?? routeFocusProvenanceMap.get(routeKey))?.interactionTrigger,
        });
    } else {
 ...
 */
function consumeStoredFocusForRoute(routeKey: string): HTMLElement | null {
...
function retrieveForRoute(routeKey: string): HTMLElement | null {
    return consumeStoredFocusForRoute(routeKey);
}

function retrieveKeyboardFocusForRoute(routeKey: string): HTMLElement | null {
    const provenance = routeFocusProvenanceMap.get(routeKey);
    if (!isKeyboardRouteRestoreEligible(provenance)) {
        clearRouteFocusEntry(routeKey);
        logFocusDebug('[NavigationFocusManager] No keyboard-eligible focus stored for route', {
            routeKey,
            interactionType: provenance?.interactionType,
            interactionTrigger: provenance?.interactionTrigger,
        });
        return null;
    }
...
function hasKeyboardStoredFocus(routeKey: string): boolean {
    return hasStoredFocus(routeKey) && isKeyboardRouteRestoreEligible(routeFocusProvenanceMap.get(routeKey));
}
...
function cleanupRemovedRoutes(state: State): void {
    ...
    const knownRouteKeys = new Set<string>([...routeFocusMap.keys(), ...routeElementIdentifierMap.keys(), ...routeFocusProvenanceMap.keys()]);
...
const NavigationFocusManager: NavigationFocusManagerModule = {
    ...
    captureForRoute,
    retrieveForRoute,
    retrieveKeyboardFocusForRoute,
    clearForRoute,
    hasStoredFocus,
    hasKeyboardStoredFocus,
