...
function signInToOldDotAndChooseExperience(
    ...
    // Disconnect Pusher before handing the auth token to OldDot. Once signInToOldDot() is called the
    // token belongs to OldDot and any in-flight or subsequent Pusher auth requests will receive
    // NOT_AUTHENTICATED, which previously caused a SIGSEGV crash in PusherSwift (fetchAuthValue
    // receiving a null dict from an undefined JS Promise result).
    Pusher.disconnect();
