    ...
    response
        .then((data) => {
            ...
            if (data.jsonCode === CONST.JSON_CODE.NOT_AUTHENTICATED) {
                ...
                Log.info(`@51888 middleware reauthentication has reauthenticate at ${new Date().toISOString()}`);
        ...
        .catch((error) => {
            Log.info(`@51888 middleware reauthenttication failed reauthenticate`);
