 ...
 *
 * A field imported by an accounting integration carries that integration's origin in `origin`
 * (e.g. `"netsuite"`), so we treat a field as integration-imported only when `origin` matches one of the
 * known integration origins. Those origins are not the connection names — the backend stamps `qbo` for
 * `quickbooksOnline`, `qbd` for `quickbooksDesktop` and `dualentry` for `dualEntry` — so we match against
 * `CONST.POLICY.CONNECTIONS.REPORT_FIELD_ORIGIN` rather than `ACCOUNTING_CONNECTION_NAMES`.
 *
 * Checking merely that `origin`/`externalIDs` are present is not reliable: per the `PolicyReportField` type
 * both can also be populated by non-integration automated actions (`origin` is "Automated action or
 * integration…" and `externalIDs` can be "auto generated by us"), which would produce false positives.
 *
 * Manually-created fields have no `origin` (see `src/libs/actions/Policy/ReportField.ts`) and the default
 * `text_title` field has no `origin` either, so neither is ever treated as imported.
 */
