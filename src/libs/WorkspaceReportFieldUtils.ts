 ...
 *
 * A field imported by an accounting integration carries that integration's name in `origin`
 * (e.g. `"netsuite"`), so we treat a field as integration-imported only when `origin` matches one of the
 * known accounting connection names. Checking merely that `origin`/`externalIDs` are present is not
 * reliable: per the `PolicyReportField` type both can also be populated by non-integration automated
 * actions (`origin` is "Automated action or integration…" and `externalIDs` can be "auto generated by us"),
...
function isReportFieldImportedFromIntegration(reportField: PolicyReportField | undefined | null): boolean {
    ...
    return (CONST.POLICY.CONNECTIONS.ACCOUNTING_CONNECTION_NAMES as readonly string[]).includes(reportField.origin);
}
