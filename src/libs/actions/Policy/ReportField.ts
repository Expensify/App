 ...
 */
function createReportFieldsListValue(valueName: string, listValues: string[], disabledListValues: boolean[]) {
 ...
 */
function renameReportFieldsListValue(valueIndex: number, newValueName: string, listValues: string[]) {
 ...
 */
function setReportFieldsListValueEnabled(valueIndexes: number[], enabled: boolean, disabledListValues: boolean[]) {
 ...
 */
function deleteReportFieldsListValue(valueIndexes: number[], listValues: string[], disabledListValues: boolean[]) {
    const listValuesCopy = [...listValues];
    const disabledListValuesCopy = [...disabledListValues];
...
function createReportField(
    policyID: string,
    {name, type, initialValue}: CreateReportFieldArguments,
    listValues: string[],
    disabledListValues: boolean[],
    policy: OnyxEntry<Policy>,
    policyExpenseReports: Array<OnyxEntry<Report>>,
) {
...
function deleteReportFields(policyID: string, reportFieldsToUpdate: string[], policy: OnyxEntry<Policy>) {
 ...
 */
function updateReportFieldInitialValue(policyID: string, reportFieldID: string, newInitialValue: string, policy: OnyxEntry<Policy>) {
    const previousFieldList = policy?.fieldList ?? {};
...
function updateReportFieldListValueEnabled(policyID: string, reportFieldID: string, valueIndexes: number[], enabled: boolean, policy: OnyxEntry<Policy>) {
    const previousFieldList = policy?.fieldList ?? {};
    const fieldKey = ReportUtils.getReportFieldKey(reportFieldID);
 ...
 */
function addReportFieldListValue(policyID: string, reportFieldID: string, valueName: string, policy: OnyxEntry<Policy>) {
    const previousFieldList = policy?.fieldList ?? {};
 ...
 */
function removeReportFieldListValue(policyID: string, reportFieldID: string, valueIndexes: number[], policy: OnyxEntry<Policy>) {
    const previousFieldList = policy?.fieldList ?? {};
    const reportFieldKey = ReportUtils.getReportFieldKey(reportFieldID);
    const reportField = previousFieldList[reportFieldKey];
    const updatedReportField = cloneDeep(reportField);
