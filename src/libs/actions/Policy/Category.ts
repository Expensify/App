...
function appendSetupCategoriesOnboardingData(
  ...
  onyxData.optimisticData?.push(
    ...(finishOnboardingTaskData.optimisticData ?? []),
  );
...
function buildOptimisticPolicyWithExistingCategories(
  policyID: string,
  categories: PolicyCategories,
) {
  const categoriesValues = Object.values(categories);
  const optimisticCategoryMap = categoriesValues.reduce<
  ...
  const successCategoryMap = categoriesValues.reduce<
    Record<string, Partial<PolicyCategory>>
  >((acc, category) => {
    acc[category.name] = {
      errors: null,
  ...
  const failureCategoryMap = categoriesValues.reduce<
    Record<string, Partial<PolicyCategory>>
  >((acc, category) => {
    acc[category.name] = {
...
function buildOptimisticMccGroup() {
  ...
  const successMccGroup: Record<
    "mccGroup",
    Record<string, Partial<MccGroup>>
  > = { mccGroup: {} };
...
function setWorkspaceCategoryEnabled({
    ...
    successData: [
      {
        ...
        key: `${ONYXKEYS.COLLECTION.POLICY_CATEGORIES}${policyID}`,
        value: {
          ...Object.keys(categoriesToUpdate).reduce<
            PartialDeep<PolicyCategories>
          >((acc, key) => {
            acc[key] = {
              errors: null,
              pendingFields: {
    ...
    failureData: [
      {
        ...
        key: `${ONYXKEYS.COLLECTION.POLICY_CATEGORIES}${policyID}`,
        value: {
          ...Object.keys(categoriesToUpdate).reduce<PolicyCategories>(
            (acc, key) => {
              acc[key] = {
                ...policyData.categories[key],
                errors: ErrorUtils.getMicroSecondOnyxErrorWithTranslationKey(
                  "workspace.categories.updateFailureMessage",
                ),
                pendingFields: {
                  enabled: null,
                },
  ...
  const parameters = {
    ...
    categories: JSON.stringify(
      Object.keys(categoriesToUpdate).map((key) => categoriesToUpdate[key]),
    ),
  };
...
function setPolicyCategoryDescriptionRequired(
  ...
  const originalAreCommentsRequired =
    policyCategoryToUpdate?.areCommentsRequired;
...
function setPolicyCategoryReceiptsRequired(
  ...
  const originalMaxAmountNoReceipt =
    policyData.categories[categoryName]?.maxAmountNoReceipt;
  const policyCategoriesOptimisticData = {
    [categoryName]: {
      pendingAction: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
      pendingFields: {
...
function removePolicyCategoryReceiptsRequired(
  policyData: PolicyData,
  categoryName: string,
) {
  const policyID = policyData.policy?.id;
...
function setPolicyCategoryItemizedReceiptsRequired(
  policyData: PolicyData,
  categoryName: string,
  maxAmountNoItemizedReceipt: number,
) {
  const policyID = policyData.policy?.id;
  const originalMaxAmountNoItemizedReceipt =
    policyData.categories[categoryName]?.maxAmountNoItemizedReceipt;
  const policyCategoriesOptimisticData = {
    [categoryName]: {
      pendingAction: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
      pendingFields: {
  ...
  API.write(
    WRITE_COMMANDS.SET_POLICY_CATEGORY_ITEMIZED_RECEIPTS_REQUIRED,
    parameters,
    onyxData,
  );
}

function removePolicyCategoryItemizedReceiptsRequired(
  policyData: PolicyData,
  categoryName: string,
) {
  const policyID = policyData.policy?.id;
  ...
  API.write(
    WRITE_COMMANDS.REMOVE_POLICY_CATEGORY_ITEMIZED_RECEIPTS_REQUIRED,
    parameters,
    onyxData,
  );
}
...
function renamePolicyCategory(
  policyData: PolicyData,
  policyCategory: { oldName: string; newName: string },
) {
  ...
  const updatedMccGroup = CategoryUtils.updateCategoryInMccGroup(
    clonedMccGroup,
    policyCategory.oldName,
    policyCategory.newName,
  );
  const updatedMccGroupWithClearedPendingAction =
  ...
  if (policyCategoryExpenseRule) {
    const ruleIndex = updatedExpenseRules.findIndex(
      (rule) => rule.id === policyCategoryExpenseRule.id,
    );
    policyCategoryExpenseRule.applyWhen =
      policyCategoryExpenseRule.applyWhen.map((applyWhen) => ({
        ...applyWhen,
  ...
  if (policyCategoryApproverRule) {
    const indexToUpdate = updatedApprovalRules.findIndex(
      (rule) => rule.id === policyCategoryApproverRule.id,
    );
    policyCategoryApproverRule.applyWhen =
      policyCategoryApproverRule.applyWhen.map((ruleCondition) => {
        const { value, field, condition } = ruleCondition;

        if (
          value === policyCategory.oldName &&
  ...
  const policyCategories = Object.values(
    policyData.categories ?? {},
  ).reduce<PolicyCategories>((acc, category) => {
  ...
  pushTransactionViolationsOnyxData(
    onyxData,
    { ...policyData, categories: policyCategories },
    policyOptimisticData,
    policyCategoriesOptimisticData,
  );
...
function setPolicyCategoryPayrollCode(
  policyID: string,
  categoryName: string,
  payrollCode: string,
  policyCategories: PolicyCategories = {},
    ...
    failureData: [
      {
        ...
        value: {
          [categoryName]: {
            ...policyCategoryToUpdate,
            errors: ErrorUtils.getMicroSecondOnyxErrorWithTranslationKey(
              "workspace.categories.updatePayrollCodeFailureMessage",
            ),
            pendingAction: null,
            pendingFields: {
              // eslint-disable-next-line @typescript-eslint/naming-convention
...
function setPolicyCategoryGLCode(
  policyID: string,
  categoryName: string,
  glCode: string,
  policyCategories: PolicyCategories = {},
    ...
    failureData: [
      {
        ...
        value: {
          [categoryName]: {
            ...policyCategoryToUpdate,
            errors: ErrorUtils.getMicroSecondOnyxErrorWithTranslationKey(
              "workspace.categories.updateGLCodeFailureMessage",
            ),
            pendingAction: null,
            pendingFields: {
              // eslint-disable-next-line @typescript-eslint/naming-convention
...
function deleteWorkspaceCategories(
  ...
  const policyID = policyData.policy?.id;
  const optimisticPolicyCategoriesData = categoryNamesToDelete.reduce<
    Record<string, Partial<PolicyCategory>>
  >((acc, categoryName) => {
    acc[categoryName] = {
  ...
  const shouldDisableRequiresCategory = !hasEnabledOptions(
    Object.values(policyData.categories).filter(
      (category) =>
        !categoryNamesToDelete.includes(category.name) &&
        category.pendingAction !== CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE,
    ),
  );
    ...
    successData: [
      {
        onyxMethod: Onyx.METHOD.MERGE,
        key: `${ONYXKEYS.COLLECTION.POLICY_CATEGORIES}${policyID}`,
        value: categoryNamesToDelete.reduce<Record<string, null>>(
          (acc, categoryName) => {
    ...
    failureData: [
      {
        onyxMethod: Onyx.METHOD.MERGE,
        key: `${ONYXKEYS.COLLECTION.POLICY_CATEGORIES}${policyID}`,
        value: categoryNamesToDelete.reduce<
          Record<string, Partial<PolicyCategory>>
        >((acc, categoryName) => {
          acc[categoryName] = {
            pendingAction: null,
            errors: ErrorUtils.getMicroSecondOnyxErrorWithTranslationKey(
              "workspace.categories.deleteFailureMessage",
            ),
  ...
  pushTransactionViolationsOnyxData(
    onyxData,
    policyData,
    optimisticPolicyData,
    optimisticPolicyCategoriesData,
  );
  appendSetupCategoriesOnboardingData(
    onyxData,
    setupCategoryTaskReport,
...
function enablePolicyCategories(
  ...
  const policyCategoriesUpdate: Record<
    string,
    Partial<PolicyCategory>
  > = Object.fromEntries(
...
function setPolicyCustomUnitDefaultCategory(
  policyID: string,
  customUnitID: string,
...
function downloadCategoriesCSV(
  ...
  fileDownload(
    translate,
    ApiUtils.getCommandURL({ command: WRITE_COMMANDS.EXPORT_CATEGORIES_CSV }),
    fileName,
    "",
    false,
    formData,
...
function setPolicyCategoryMaxAmount(
  ...
  const parsedMaxExpenseAmount =
    maxExpenseAmount === ""
      ? null
      : CurrencyUtils.convertToBackendAmount(parseFloat(maxExpenseAmount));
...
function setPolicyCategoryApprover(
  ...
  const existingCategoryApproverRule = CategoryUtils.getCategoryApproverRule(
    updatedApprovalRules,
    categoryName,
  );
  ...
  if (!existingCategoryApproverRule) {
    ...
    updatedApprovalRules = updatedApprovalRules.filter(
      (rule) => rule.approver !== approver,
    );
    newApprover = "";
  } else {
    const indexToUpdate = updatedApprovalRules.indexOf(
      existingCategoryApproverRule,
    );
...
function setPolicyCategoryTax(
  ...
  const expenseRules = policy?.rules?.expenseRules ?? [];
  const updatedExpenseRules: ExpenseRule[] = lodashCloneDeep(expenseRules);
  const existingCategoryExpenseRule = updatedExpenseRules.find((rule) =>
    rule.applyWhen.some((when) => when.value === categoryName),
  );
  ...
  if (!existingCategoryExpenseRule) {
    ...
    const indexToUpdate = updatedExpenseRules.indexOf(
      existingCategoryExpenseRule,
    );
...
function setPolicyCategoryAttendeesRequired(
  policyID: string,
  categoryName: string,
  ...
  const policyCategoryToUpdate = policyCategories?.[categoryName];
  const originalAreAttendeesRequired =
    policyCategoryToUpdate?.areAttendeesRequired;
  ...
  API.write(
    WRITE_COMMANDS.SET_POLICY_CATEGORY_ATTENDEES_REQUIRED,
    parameters,
    onyxData,
  );
}
