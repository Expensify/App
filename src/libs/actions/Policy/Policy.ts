...
import type { CustomFieldType, PolicyEmployeeList } from '@src/types/onyx/PolicyEmployee';
...
function setWorkspaceApprovalMode(policyID: string, approver: string, approvalMode: ValueOf<typeof CONST.POLICY.APPROVAL_MODE>) {
    ...
    // When enabling approvals (switching from OPTIONAL to BASIC/ADVANCED),
    // we need to update all employees' submitsTo to point to the new approver
    // to prevent duplicate workflows from being created (fixes #76719)
    const previousApprovalMode = policy?.approvalMode;
    const isEnablingApprovals = previousApprovalMode === CONST.POLICY.APPROVAL_MODE.OPTIONAL && approvalMode !== CONST.POLICY.APPROVAL_MODE.OPTIONAL;

    let updatedEmployeeList: PolicyEmployeeList | undefined;
    if (isEnablingApprovals && policy?.employeeList) {
        updatedEmployeeList = {};
        for (const [email, employee] of Object.entries(policy.employeeList)) {
            // Only update if the employee has a submitsTo that is different from the new approver
            if (employee.submitsTo && employee.submitsTo !== approver) {
        ...
        // If no employees need updating, set to undefined to avoid unnecessary updates
        if (Object.keys(updatedEmployeeList).length === 0) {
            updatedEmployeeList = undefined;
        }
    }
    ...
    const optimisticData: OnyxUpdate[] = [
        {
            ...
            value: {
                ...
                ...(updatedEmployeeList ? { employeeList: updatedEmployeeList } : {}),
            },
        },
    ];
                ...
                ...(updatedEmployeeList
                    ? {
                        employeeList: Object.fromEntries(
                            Object.keys(updatedEmployeeList).map((email) => [email, { pendingAction: null, pendingFields: { submitsTo: null } }]),
                        ),
                    }
                    : {}),
            },
