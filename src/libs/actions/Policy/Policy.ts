...
function deleteWorkspace(params: DeleteWorkspaceActionParams) {
    ...
    // Optimistically remove the policy from Onyx. This will prevent the reports from being filtered out
    // by `policy?.pendingAction === CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE` in ReportUtils.getDisplayReports.
    ...
    > = [
        {
            onyxMethod: Onyx.METHOD.SET, // Changed from MERGE to SET
            key: `${ONYXKEYS.COLLECTION.POLICY}${policyID}`,
            value: null, // Policy is optimistically removed
        },
        {
    ...
    // Restore the old report stateNum and statusNum, and the entire policy object on failure
    ...
    > = [
        ...
        },
        {
            onyxMethod: Onyx.METHOD.SET, // Changed from MERGE to SET
            key: `${ONYXKEYS.COLLECTION.POLICY}${policyID}`,
            value: policy, // Restore the entire original policy object
        },
    ...
    if (policyID === activePolicyID) {
        const mostRecentlyCreatedGroupPolicy = Object.values(policies ?? {})
            .filter((p) => p && p.id !== policyID && p.type !== CONST.POLICY.TYPE.PERSONAL && p.pendingAction !== CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE)
    ...
    for (const report of reportsToArchive) {
        ...
        // Determine the base name for the archived report title.
        // For workspace chats (admin/expense), this is typically the policy name.
        // For policy expense DMs, it's typically "Chat with X in Y Workspace".
        // The existing `report?.reportDisplayName` or `report?.chatReportName` or `report?.reportName` should already contain the desired base.
        const originalReportDisplayName = report?.reportDisplayName || report?.chatReportName || report?.reportName || policyName;
        const archivedReportDisplayName = `${originalReportDisplayName} (archived)`;
        ...
        optimisticData.push({
            ...
            value: {
                ...(!isInvoiceReceiverReport && {
                    // Retain oldPolicyName and policyName to be able to revert on failure
                    // and allow other parts of the app to still resolve the policy name if needed,
                    // but the display name will show (archived)
                    oldPolicyName: policy?.name,
                    policyName: policy?.name,
                }),
                isPinned: false,
                stateNum: CONST.REPORT.STATE_NUM.ARCHIVED,
                statusNum: CONST.REPORT.STATUS_NUM.CLOSED,
                reportDisplayName: archivedReportDisplayName,
                chatReportName: archivedReportDisplayName,
                reportName: archivedReportDisplayName,
                // New additions to push to bottom of LHN and clear active state
                lastVisibleActionCreated: 0, // Set to 0 to push to bottom
                lastReadTime: 0, // Clear last read time
                lastMessageText: '', // Clear last message
                lastActorAccountID: 0, // Clear last actor
                lastMessageTranslationKey: '',
                lastMessageType: '',
                lastMessageHtml: '',
                lastMessageModificationDate: '',
                oldestFourParticipantAccountIDs: '', // Clear for sorting/display purposes
                pendingFields: {
                    reportDisplayName: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    chatReportName: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    reportName: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    stateNum: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    statusNum: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    // New pending fields
                    lastVisibleActionCreated: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    lastReadTime: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    lastMessageText: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    lastActorAccountID: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    lastMessageTranslationKey: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    lastMessageType: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    lastMessageHtml: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    lastMessageModificationDate: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                    oldestFourParticipantAccountIDs: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
                },
            },
        });
        ...
        optimisticData.push({
            ...
            value: {
                ...
                report_title: archivedReportDisplayName,
            },
        });
        ...
        failureData.push({
            ...
            value: {
                ...
                chatReportName: report?.chatReportName,
                ...
                lastMessageType: report?.lastMessageType,
                lastMessageHtml: report?.lastMessageHtml,
                lastMessageModificationDate: report?.lastMessageModificationDate,
                oldestFourParticipantAccountIDs: report?.oldestFourParticipantAccountIDs,
                pendingFields: {
                    ...
                    stateNum: null,
                    statusNum: null,
                    // Clear pending fields on failure
                    lastVisibleActionCreated: null,
                    lastReadTime: null,
                    lastMessageText: null,
                    lastActorAccountID: null,
                    lastMessageTranslationKey: null,
                    lastMessageType: null,
                    lastMessageHtml: null,
                    lastMessageModificationDate: null,
                    oldestFourParticipantAccountIDs: null,
                },
            },
        });
        ...
        failureData.push({
            ...
            value: {
                ...
                report_title: report?.reportName,
            },
        });
...
function enablePolicyRules(policyID: string, enabled: boolean, shouldGoBack = true, policyData?: PolicyData) {
    ...
    if (policyData) {
        ReportUtils.pushTransactionViolationsOnyxData(onyxData, policyData, {
            ...
            ...(!enabled ? DISABLED_MAX_EXPENSE_VALUES : {}),
            pendingFields: {
                areRulesEnabled: enabled ? null : CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE, // Clear pending fields if enabling, otherwise keep current pending if disabled
            },
        });
    }
        ...
        failureData: [
            {
                ...
                value: {
                    customUnits: {
                        [customUnitID]: policy?.customUnits ? policy?.customUnits[customUnitID].attributes : null,
                        errorFields: {
