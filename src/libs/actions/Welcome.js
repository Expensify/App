...
function show({routes, showCreateMenu}) {
    isReadyPromise.then(() => {
        ...
        NameValuePair.set(
            CONST.NVP.IS_FIRST_TIME_NEW_EXPENSIFY_USER,
            false,
        ...
        const isWorkspaceRoute =
            topRoute.name === 'Settings' &&
            topRoute.params.path.includes('workspace');
        const transitionRoute = _.find(
            routes,
            (route) => route.name === SCREENS.TRANSITION_FROM_OLD_DOT,
        );
        const exitingToWorkspaceRoute =
            lodashGet(transitionRoute, 'params.exitTo', '') === 'workspace/new';
        const isDisplayingWorkspaceRoute =
            isWorkspaceRoute || exitingToWorkspaceRoute;
        ...
        const workspaceChatReport = _.find(
            ...
            (report) =>
                ...
                report.ownerEmail === email,
        );
        if (workspaceChatReport && !isDisplayingWorkspaceRoute) {
            Navigation.navigate(
                ROUTES.getReportRoute(workspaceChatReport.reportID),
            );
            return;
        }
        ...
        if (
            !Policy.isAdminOfFreePolicy(allPolicies) &&
