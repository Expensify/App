...
function signOutAndRedirectToSignIn(shouldReplaceCurrentScreen?: boolean, stashSession?: boolean) {
    ...
    if (!isAnonymousUser()) {
        // We'll only call signOut if we're not stashing the session, otherwise we'll call the API to invalidate
        // the autogenerated credentials used for infinite sessions
        const isSupportal = isSupportalToken();
        if (!isSupportal && !stashSession) {
            signOut();
        }
        ...
        const shouldStashSession = !isSupportal && stashSession;
        ...
        if (shouldStashSession) {
            redirectPromise.then(() => {
                Onyx.multiSet({
                    ...
                    [ONYXKEYS.STASHED_SESSION]: sessionCopy,
                });
            });
        }
        if (shouldRestoreStashedSession) {
            ...
            redirectPromise.then(() => {
                Onyx.multiSet({
                    ...
                    [ONYXKEYS.SESSION]: stashedSessionCopy,
                });
            });
        }
    } else {
