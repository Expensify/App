...
function signInWithSupportAuthToken(authToken: string) {
    ...
    API.read(READ_COMMANDS.SIGN_IN_WITH_SUPPORT_AUTH_TOKEN, {authToken}, {optimisticData, finallyData, failureData: []});
}
...
function signOut(params: {autoGeneratedLogin?: string; signedInWithSAML?: boolean; authToken?: string} = {}): Promise<void | Response<never>> {
    ...
    return API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT, logOutParams, {failureData: []});
}
...
function callSAMLSignOut(params: LogOutParams, authToken: string): Promise<void | Response<never>> {
    ...
    return openAuthSessionAsync(`${CONFIG.EXPENSIFY.SAML_URL}/logout?${queryString}`, CONST.SAML_REDIRECT_URL)
        ...
        .then((result) => {
            ...
            // eslint-disable-next-line rulesdir/no-api-side-effects-method
            return API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT, params, {failureData: []});
        });
}
...
function resendValidateCode(login = credentials.login) {
    ...
    API.write(WRITE_COMMANDS.REQUEST_NEW_VALIDATE_CODE, params, {optimisticData, finallyData, failureData: []});
}
...
function authenticatePusher(socketID: string, channelName: string, callback?: ChannelAuthorizationCallback) {
    ...
    // eslint-disable-next-line rulesdir/no-api-side-effects-method
    return API.makeRequestWithSideEffects(SIDE_EFFECT_REQUEST_COMMANDS.AUTHENTICATE_PUSHER, params, {failureData: []})
        .then((response) => {
