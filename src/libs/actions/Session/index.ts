...
function signInAfterTransitionFromOldDot(transitionURL: string) {
    ...
    const {email, authToken, encryptedAuthToken, accountID, autoGeneratedLogin, autoGeneratedPassword, clearOnyxOnStart, completedHybridAppOnboarding, isSingleNewDotEntry, primaryLogin, shouldRemoveDelegatedAccess} =
        Object.fromEntries(
        ...
        clearOnyxForNewAccount()
            ...
            .then(() => {
                if (shouldRemoveDelegatedAccess) {
                    Onyx.clear(KEYS_TO_PRESERVE_DELEGATE_ACCESS).then(App.openApp);
                } else {
                    App.openApp();
                }
            })
