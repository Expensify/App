...
function signInAfterTransitionFromOldDot(transitionURL: string) {
    const searchParams = new URL(transitionURL, CONFIG.EXPENSIFY.EXPENSIFY_URL).searchParams;
    const searchParamsKeys = Array.from(searchParams.keys());

    const {email, authToken, encryptedAuthToken, accountID, autoGeneratedLogin, autoGeneratedPassword} = Object.fromEntries(
        searchParamsKeys.map((param: string) => [param, searchParams.get(param)]),
    );

    if (email && authToken && encryptedAuthToken && accountID && autoGeneratedLogin && autoGeneratedPassword) {
        Log.info('[HybridApp] Saving data from transition in Onyx');
        Onyx.set(ONYXKEYS.SESSION, {email, authToken, encryptedAuthToken, accountID: Number(accountID)});
        Onyx.set(ONYXKEYS.CREDENTIALS, {autoGeneratedLogin, autoGeneratedPassword});
    }
}
