...
function signOutAndRedirectToSignIn(shouldReplaceCurrentScreen?: boolean, stashSession?: boolean) {
    ...
    if (!isAnonymousUser()) {
        ...
        // If someone is logged as a support user, let's check if there's any credentials stashed, and
        // if there is, let's set that session as the main one instead of completely logging out.
        if(isSupportalToken() && stashedCredentials.autoGeneratedLogin && stashedCredentials.autoGeneratedLogin !== '') {
            Onyx.multiSet({
