...
function setupNewDotAfterTransitionFromOldDot(
    ...
    return clearOnyxIfSigningIn()
        ...
        .then(() => {
            ...
            // OldDot bumps stagingToggleGeneration only when the user deliberately flips its staging toggle,
            // so a generation we haven't accepted yet marks a deliberate change rather than the ambient value
            // OldDot sends on every transition.
            ...
            // OldDot wipes its whole state on sign-out, which restarts the generation counter. A generation lower
            // than the one we accepted means such a reset happened; without re-baselining, every flip counted from
            // the new base would stay below the stored value and be ignored forever.
            ...
            for (const [key, value] of Object.entries(newDotOnyxValues)) {
                ...
                // merge over it would reset the toggle on every cold start. We take OldDot's value only while NewDot
                // has none (seeding the key on the first transition so both apps start out agreeing), or when it comes
                // from a deliberate OldDot toggle change we haven't applied yet.
