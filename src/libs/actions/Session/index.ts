...
import type {HybridApp, TryNewDot} from '@src/types/onyx';
...
async function handleAccountSwitch(hybridApp: HybridApp) {
    // User switched account - clear Onyx and apply delegate data
    ...
    await Onyx.clear(KEYS_TO_PRESERVE_DELEGATE_ACCESS);
    ...
    await Onyx.merge(ONYXKEYS.ACCOUNT, {primaryLogin: hybridApp?.delegateAccessData?.oldDotCurrentUserEmail});
...
async function setupNewDotAfterTransitionFromOldDot(hybridAppSettings: HybridAppSettings, tryNewDot?: TryNewDot): Promise<void> {
    const {hybridApp, ...newDotOnyxValues} = hybridAppSettings;

    try {
        // Clear Onyx if signing in with new sign-in page
        ...
        if (hybridApp?.delegateAccessData?.oldDotCurrentUserEmail === currentUserEmail) {
            ...
            await handleAccountSwitch(hybridApp);
        }

        // Prepare hybrid app after transition
        await HybridAppActions.prepareHybridAppAfterTransitionToNewDot({
            ...hybridApp,
            closingReactNativeApp: false,
        });

        // Reset did user login during session if needed
        ...
        // Merge new dot Onyx values
        await Promise.all(Object.entries(newDotOnyxValues).map(([key, value]) => Onyx.merge(key as OnyxKey, value ?? {})));
