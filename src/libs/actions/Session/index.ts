...
function signInAfterTransitionFromOldDot(transitionURL: string) {
    ...
    const {email, authToken, accountID, autoGeneratedLogin, autoGeneratedPassword} = Object.fromEntries(
    ...
    if (email && authToken && accountID && autoGeneratedLogin && autoGeneratedPassword) {
        Log.info('[HybridApp] Saving data from transition in Onyx');
        Onyx.set(ONYXKEYS.SESSION, {email, authToken, accountID: Number(accountID)});
