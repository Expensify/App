...
function enterSavedViewEditMode({id, name, query}: EditingSavedSearch) {
    hasPendingOpenEditFilters = true;
    Onyx.set(ONYXKEYS.RAM_ONLY_SEARCH_EDITING_SAVED_VIEW, {id, name, query});
...
/** Saves the edited filters back onto the view. SaveSearch is keyed by savedSearchID, so the same id updates it in place. */
function saveSavedViewEdits({queryJSON, editingSavedView}: {queryJSON: Readonly<SearchQueryJSON>; editingSavedView: EditingSavedSearch}) {
    ...
    saveSearch({id: editingSavedView.id, queryJSON, newName: wasAutoNamed ? undefined : editingSavedView.name});
    Onyx.set(ONYXKEYS.RAM_ONLY_SEARCH_EDITING_SAVED_VIEW, null);
}
