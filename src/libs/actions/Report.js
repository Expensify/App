...
function getUnreadActionCount(report) {
  ...
  const unreadActionCount =
    report.reportActionList.length - lastReadSequenceNumber;
...
function getSimplifiedReportObject(report) {
  return {
    ...
    lastVisitedTimestamp: lodashGet(
      report,
      ['reportNameValuePairs', `lastRead_${currentUserAccountID}`, 'timestamp'],
      0,
    ),
  };
}
...
function updateReportWithNewAction(reportID, reportAction) {
  ...
  Onyx.merge(`${ONYXKEYS.COLLECTION.REPORT}${reportID}`, {
    reportID,
    unreadActionCount:
      newMaxSequenceNumber - (lastReadSequenceNumbers[reportID] || 0),
...
function fetchActions(reportID) {
  API.Report_GetHistory({reportID}).then((data) => {
...
function fetchAll(
  ...
  fetchChatReports().then((reportIDs) => {
    if (
      shouldRedirectToReport &&
      (currentURL === ROUTES.ROOT || currentURL === ROUTES.HOME)
    ) {
    ...
    if (shouldFetchActions) {
      _.each(reportIDs, (reportID) => {
        console.debug(
          `[RECONNECT] Fetching report actions for report ${reportID}`,
        );
...
function fetchOrCreateChatReport(participants) {
  ...
  if (participants.length < 2) {
    throw new Error(
      'fetchOrCreateChatReport() must have at least two participants',
    );
  }
      ...
      message: [
        {
          ...
          text: isAttachment
            ? '[Attachment]'
            : htmlComment.replace(/<[^>]*>?/gm, '').replace(/\&nbsp;/g, ' '),
        },
      ],
      isFirstItem: false,
      isAttachment,
