...
function getNonOptimisticPolicyIDs(
...
function setLocale(locale: Locale, currentPreferredLocale: Locale | undefined) {
  ...
  const optimisticData: Array<
    OnyxUpdate<typeof ONYXKEYS.NVP_PREFERRED_LOCALE>
  > = [
    {
...
AppState.addEventListener("change", (nextAppState) => {
  ...
  if (nextAppState.match(/inactive|background/)) {
    Log.info(
      "Cancelling telemetry spans as app is going inactive/background",
      false,
      { previousState: appState, nextState: nextAppState },
    );
    cancelAllSpans();
  }
  ...
  if (nextAppState === "active" && appState?.match(/inactive|background/)) {
    ...
    Log.info(
      "Cancelling telemetry spans as app is coming to foreground",
      false,
      { previousState: appState, nextState: nextAppState },
    );
...
function getOnyxDataForOpenOrReconnect(
  ...
  if (shouldKeepPublicRooms) {
    const publicReports = Object.values(allReports ?? {}).filter(
      (report) => isPublicRoom(report) && isValidReport(report),
    );
    if (publicReports) {
  ...
  const reportsWithDraftComments = Object.entries(
    allReportsWithDraftComments ?? {},
  )
    .filter(([, value]) => value !== null)
    .map(([key]) => key.replace(ONYXKEYS.COLLECTION.REPORT_DRAFT_COMMENT, ""))
    .map(
      (reportID) => allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${reportID}`],
    );
...
function triggerFullReconnect(cutoff: string) {
  Onyx.merge(
    ...
    getLastFullReconnectTimeToRecord(cutoff),
  ).then(() => reconnectApp());
}
...
function finalReconnectAppAfterActivatingReliableUpdates(): Promise<void | OnyxTypes.Response<OnyxDataForOpenOrReconnectKeys>> {
  ...
  return API.makeRequestWithSideEffects(
    ...
    params,
    getOnyxDataForOpenOrReconnect(false, true),
  );
}
...
): Promise<void | OnyxTypes.Response<OnyxDataForOpenOrReconnectKeys>> {
  console.debug(
...
type PolicyType =
  | typeof CONST.POLICY.TYPE.TEAM
  | typeof CONST.POLICY.TYPE.CORPORATE;
...
function createWorkspaceWithPolicyDraftAndNavigateToIt(
  params: CreateWorkspaceWithPolicyDraftParams,
) {
  const {
    introSelected,
    policyOwnerEmail = "",
  ...
  Navigation.isNavigationReady().then(() => {
    ...
    const routeToNavigate =
      routeToNavigateAfterCreate ??
      ROUTES.WORKSPACE_INITIAL.getRoute(policyIDWithDefault, backTo);
...
function createWorkspaceWithPolicyDraft(
  params: CreateWorkspaceWithPolicyDraftParams,
) {
  const {
    introSelected,
...
function savePolicyDraftByNewWorkspace({
  ...
  createWorkspace({
    ...
    policyName,
    policyID,
    engagementChoice: isTrackOnboardingChoice(introSelected?.choice)
...
function setUpPoliciesAndNavigate(
  ...
  const isLoggingInAsNewUser =
    !!session.email &&
    isLoggingInAsNewUserSessionUtils(currentUrl, session.email);
  const url = new URL(currentUrl);
  const exitTo = url.searchParams.get("exitTo") as Route | null;

  // Approved Accountants and Guides can enter a flow where they make a workspace for other users,
  // and those are passed as a search parameter when using transition links
  const policyOwnerEmail =
    url.searchParams.get("ownerEmail") ?? session.email ?? "";
  ...
  if (shouldCreateFreePolicy) {
    createWorkspaceWithPolicyDraftAndNavigateToIt({
      ...
      activePolicy,
      currentUserAccountIDParam:
        currentSessionData.accountID ?? CONST.DEFAULT_NUMBER_ID,
    ...
    clearOnyxAndSeedFullReconnect(KEYS_TO_PRESERVE)
      .then(() => {
        ...
        if (isStateImported) {
          ...
          Log.info(
            "[ImportedState] Exiting imported state mode, restoring original session",
          );
        }
 ...
 */
function showSupportalPermissionDenied(
  payload: OnyxTypes.SupportalPermissionDenied,
) {
