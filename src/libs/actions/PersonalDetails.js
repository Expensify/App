 ...
 * @returns {String}
 */
function getDisplayName(accountId, personalDetail) {
    // If we have a number like +15857527441@expensify.sms then let's remove @expensify.sms and format it
    // so that the option looks cleaner in our UI.
    const userDetails = personalDetail || lodashGet(personalDetails, accountId);
    const userLogin = LocalePhoneNumber.formatPhoneNumber(userDetails.login);
            ...
            optimisticData: [
                {
                    ...
                    value: {
                        [currentUserAccountID]: {
                            firstName,
                            lastName,
                            displayName: getDisplayName(currentUserAccountID, {
                                firstName,
                                lastName,
                            }),
                        },
