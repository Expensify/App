...
function getDeleteTrackExpenseInformation(
    ...
    const reportLastMessageText = ReportActionsUtils.getLastVisibleMessage(chatReport?.reportID ?? '-1', updatedReportAction).lastMessageText;
...
function startSplitBill({
    ...
    Report.notifyNewAction(splitChatReport.chatReportID ?? '-1', currentUserAccountID);
}
...
function deleteTrackExpense(chatReportID: string, transactionID: string, reportAction: OnyxTypes.ReportAction, isSingleTransactionView = false) {
    ...
    if (isSingleTransactionView && shouldDeleteTransactionThread) {
        // Pop the deleted report screen before navigating. This prevents navigating to the Concierge chat due to the missing report.
        Navigation.goBack(ROUTES.REPORT_WITH_ID.getRoute(chatReport?.reportID ?? '-1'));
    }
}
