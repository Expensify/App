...
function approveMoneyRequest(
    ...
    const hasDuplicates = hasDuplicateTransactions(currentUserEmailParam, expenseReport.reportID);
    ...
    if (hasDuplicates) {
        const transactions = getReportTransactions(expenseReport.reportID).filter((transaction) => isDuplicate(transaction, currentUserEmailParam));
        if (!full) {
