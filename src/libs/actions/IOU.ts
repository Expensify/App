...
function getPerDiemExpenseInformation(perDiemExpenseInformation: PerDiemExpenseInformationParams): MoneyRequestInformation {
    ...
    const predictedNextStatus = policy?.reimbursementChoice === CONST.POLICY.REIMBURSEMENT_CHOICES.REIMBURSEMENT_NO ? CONST.REPORT.STATUS_NUM.CLOSED : CONST.REPORT.STATUS_NUM.OPEN;
    const optimisticNextStep = buildNextStep({report: iouReport, predictedNextStatus, currentUserAccountIDParam, currentUserEmailParam, hasViolations, isASAPSubmitBetaEnabled, policy});
...
type UpdateMoneyRequestCategoryParams = {
    transactionID: string;
    transactionThreadReportID: string;
    category: string;
    policy: OnyxEntry<OnyxTypes.Policy>;
...
function updateMoneyRequestCategory({
    ...
    currentUserEmailParam,
    isASAPSubmitBetaEnabled,
}: UpdateMoneyRequestCategoryParams) {
    const transactionChanges: TransactionChanges = {
        category,
    };
...
type UpdateMoneyRequestDistanceRateParams = {
    transactionID: string;
    transactionThreadReportID: string;
    rateID: string;
    policy: OnyxEntry<OnyxTypes.Policy>;
...
function updateMoneyRequestDistanceRate({
    ...
    updatedTaxAmount,
    updatedTaxCode,
}: UpdateMoneyRequestDistanceRateParams) {
    const transactionChanges: TransactionChanges = {
...
function getPayMoneyRequestParams({
    ...
    if (!isInvoiceReport) {
        ...
        optimisticNextStepDeprecated = buildNextStep({report: iouReport, predictedNextStatus: CONST.REPORT.STATUS_NUM.REIMBURSED});
