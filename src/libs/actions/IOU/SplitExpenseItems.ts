...
function initSplitExpenseItemData(
    ...
    return {
        ...
        created: created ?? transactionDetails?.created ?? DateUtils.formatUnlocalizedWithUTCTimeZone(DateUtils.getDBTime(), CONST.DATE.FNS_FORMAT_STRING),
    ...
    const splitExpenses = originalTransactionDraft?.comment?.splitExpenses?.map((item) => {
        if (item.transactionID === splitExpenseTransactionID) {
            ...
            const updatedItem: SplitExpense = {
                ...
                created: transactionDetails?.created ?? DateUtils.formatUnlocalizedWithUTCTimeZone(DateUtils.getDBTime(), CONST.DATE.FNS_FORMAT_STRING),
