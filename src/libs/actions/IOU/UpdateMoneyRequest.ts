...
function getUpdateMoneyRequestParams(params: GetUpdateMoneyRequestParamsType): UpdateMoneyRequestData<UpdateMoneyRequestDataKeys> {
    ...
    if (
        ...
        if (hash) {
            ...
            optimisticSnapshotData[`${ONYXKEYS.COLLECTION.TRANSACTION_VIOLATIONS}${transactionID}`] = Array.isArray(finalViolationsOnyxData.value) ? finalViolationsOnyxData.value : [];
            optimisticData.push({
                onyxMethod: Onyx.METHOD.MERGE,
