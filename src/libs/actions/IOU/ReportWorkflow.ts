...
function canApproveIOU(
  ...
  if (
    ...
    !policy ||
    !(isPaidGroupPolicy(policy) || isSubmitWorkspace)
  ) {
    return false;
  }
...
function canIOUBePaid(
  ...
  const { reimbursableSpend, nonReimbursableSpend } =
    getMoneyRequestSpendBreakdown(iouReport);
  ...
  return (
    ...
    !iouSettled &&
    (reimbursableSpend > 0 ||
      canShowMarkedAsPaidForNegativeAmount ||
...
function canSubmitReport(
  ...
  const hasNoSubmittableTransaction =
    ...
    transactions.every(
      (t) =>
        isScanningTransaction(t) ||
        hasSmartScanFailedWithMissingFields([t], report),
    );
...
function getBadgeFromIOUReport(
  ...
  const canBePaidNow = canIOUBePaid(
    iouReport,
    chatReport,
    policy,
    undefined,
  ...
  const canBePaidElsewhere = canIOUBePaid(
    iouReport,
    chatReport,
    policy,
    undefined,
  ...
  if (canBePaidElsewhere) {
    return hasOnlyNonReimbursableTransactions(iouReport?.reportID)
      ? undefined
 ...
 */
function canPayIOUFromReportAction(
  action: ReportAction,
  chatReport: OnyxEntry<OnyxTypes.Report>,
  currentUserAccountID: number,
): boolean {
...
function getIOUReportActionWithBadge(
  ...
  for (const action of Object.values(chatReportActions ?? {})) {
    ...
    if (!iouReport) {
      ...
      if (
        chatReport?.hasOutstandingChildRequest &&
        canPayIOUFromReportAction(action, chatReport, currentUserAccountID)
      ) {
...
function approveMoneyRequest(params: ApproveMoneyRequestFunctionParams) {
  ...
  if (isSubmitPolicy(expenseReportPolicy) && expenseReportPolicy?.id) {
    ...
    const backTo =
      ...
      ROUTES.REPORT_WITH_ID.getRoute(expenseReport.reportID);
...
function retractReport(
  ...
  const optimisticRetractReportAction =
    buildOptimisticRetractedReportAction(delegateEmail);
  const predictedNextState = CONST.REPORT.STATE_NUM.OPEN;
  const predictedNextStatus = CONST.REPORT.STATUS_NUM.OPEN;
...
function submitReport({
  ...
  const managerAccountIDFromEmail = trimmedManagerEmail
    ...
    : undefined;
  const resolvedManagerAccountIDFromEmail =
    managerAccountIDFromPopover ?? managerAccountIDFromEmail;
  ...
  const managerID = trimmedManagerEmail
    ? (resolvedManagerAccountIDFromEmail ??
      managerIDFromChain ??
      expenseReport.managerID)
    : submitReportManagerAccountID;
  const optimisticNextStepApproverID =
    !isSubmitAndClosePolicy &&
    managerID !== undefined &&
    isValidAccountRoute(managerID)
      ? managerID
      : undefined;
