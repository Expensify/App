...
function updateMultipleMoneyRequests({
    ...
    for (const transactionID of transactionIDs) {
        ...
        if (!transactionThreadReportID && iouReport?.reportID) {
            const optimisticTransactionThread = createTransactionThreadReport(introSelected, deprecatedCurrentUserEmail, deprecatedUserAccountID, betas, iouReport, reportAction, transaction);
