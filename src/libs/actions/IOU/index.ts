...
} from "@userActions/TransactionEdit";
...
import type {
  ...
  Participant,
  Split,
} from "@src/types/onyx/IOU";
import type {
  ErrorFields,
  Errors,
  PendingAction,
  PendingFields,
} from "@src/types/onyx/OnyxCommon";
...
type InitMoneyRequestParams = {
  ...
  personalPolicy:
    | Pick<OnyxTypes.Policy, "id" | "type" | "autoReporting" | "outputCurrency">
    | undefined;
  isFromGlobalCreate?: boolean;
...
let allTransactionViolations: NonNullable<
  OnyxCollection<OnyxTypes.TransactionViolations>
> = {};
Onyx.connect({
...
function getAllTransactions(): NonNullable<
  OnyxCollection<OnyxTypes.Transaction>
> {
  return allTransactions;
}

function getAllTransactionViolations(): NonNullable<
  OnyxCollection<OnyxTypes.TransactionViolations>
> {
 ...
 */
function dismissModalAndOpenReportInInboxTab(
  reportID?: string,
  isInvoice?: boolean,
) {
    ...
    if (rhpKey) {
      const hasMultipleTransactions =
        Object.values(allTransactions).filter(
          (transaction) => transaction?.reportID === reportID,
        ).length > 0;
  ...
  return (
    Object.values(reportActions).find(
      (
        ...
        typeof CONST.REPORT.ACTIONS.TYPE.REPORT_PREVIEW
      > =>
        reportAction &&
        isReportPreviewAction(reportAction) &&
...
function initMoneyRequest({
  ...
  const comment: Comment = {
    attendees: formatCurrentUserToAttendee(
      currentUserPersonalDetails,
      reportID,
    ),
  };
  ...
  if (
    ...
    if (!isFromGlobalCreate) {
      const isPolicyExpenseChat =
        isPolicyExpenseChatReportUtil(report) ||
        isPolicyExpenseChatReportUtil(parentReport);
  ...
  if (newIouRequestType === CONST.IOU.REQUEST_TYPE.PER_DIEM) {
    ...
    if (!isFromGlobalCreate) {
      const { customUnitID, category } = getCustomUnitID(
        report,
        parentReport,
        policy,
      );
  ...
  Onyx.set(
    `${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${newTransactionID}`,
    newTransaction,
  );
...
function createDraftTransaction(transaction: OnyxTypes.Transaction) {
  ...
  Onyx.set(
    `${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${transaction.transactionID}`,
    newTransaction,
  );
}
...
function clearMoneyRequest(
  ...
  Onyx.set(
    `${ONYXKEYS.COLLECTION.SKIP_CONFIRMATION}${transactionID}`,
    skipConfirmation,
  );
}
...
function startMoneyRequest(
  ...
  switch (requestType) {
    case CONST.IOU.REQUEST_TYPE.MANUAL:
      Navigation.navigate(
        ROUTES.MONEY_REQUEST_CREATE_TAB_MANUAL.getRoute(
    ...
    case CONST.IOU.REQUEST_TYPE.SCAN:
      Navigation.navigate(
        ROUTES.MONEY_REQUEST_CREATE_TAB_SCAN.getRoute(
          CONST.IOU.ACTION.CREATE,
          iouType,
          CONST.IOU.OPTIMISTIC_TRANSACTION_ID,
          reportID,
    ...
    case CONST.IOU.REQUEST_TYPE.DISTANCE:
      Navigation.navigate(
        ROUTES.MONEY_REQUEST_CREATE_TAB_DISTANCE.getRoute(
          CONST.IOU.ACTION.CREATE,
          iouType,
          CONST.IOU.OPTIMISTIC_TRANSACTION_ID,
          reportID,
    ...
    case CONST.IOU.REQUEST_TYPE.TIME:
      Navigation.navigate(
        ROUTES.MONEY_REQUEST_CREATE_TAB_TIME.getRoute(
          CONST.IOU.ACTION.CREATE,
          iouType,
          CONST.IOU.OPTIMISTIC_TRANSACTION_ID,
          reportID,
...
function startDistanceRequest(
  ...
  switch (requestType) {
    case CONST.IOU.REQUEST_TYPE.DISTANCE_MAP:
      Navigation.navigate(
        ROUTES.DISTANCE_REQUEST_CREATE_TAB_MAP.getRoute(
          CONST.IOU.ACTION.CREATE,
          iouType,
          CONST.IOU.OPTIMISTIC_TRANSACTION_ID,
          reportID,
    ...
    case CONST.IOU.REQUEST_TYPE.DISTANCE_MANUAL:
      Navigation.navigate(
        ROUTES.DISTANCE_REQUEST_CREATE_TAB_MANUAL.getRoute(
          CONST.IOU.ACTION.CREATE,
          iouType,
          CONST.IOU.OPTIMISTIC_TRANSACTION_ID,
          reportID,
    ...
    case CONST.IOU.REQUEST_TYPE.DISTANCE_GPS:
      Navigation.navigate(
        ROUTES.DISTANCE_REQUEST_CREATE_TAB_GPS.getRoute(
          CONST.IOU.ACTION.CREATE,
          iouType,
          CONST.IOU.OPTIMISTIC_TRANSACTION_ID,
          reportID,
    ...
    default:
      Navigation.navigate(
        ROUTES.DISTANCE_REQUEST_CREATE.getRoute(
          CONST.IOU.ACTION.CREATE,
          iouType,
          CONST.IOU.OPTIMISTIC_TRANSACTION_ID,
          reportID,
...
function setMoneyRequestAmount(
  transactionID: string,
  amount: number,
  currency: string,
...
function setMoneyRequestCreated(
  transactionID: string,
  created: string,
  isDraft: boolean,
) {
...
function setMoneyRequestDateAttribute(
  transactionID: string,
  start: string,
  end: string,
) {
...
function setMoneyRequestCurrency(
  transactionID: string,
  currency: string,
  isEditing = false,
) {
...
function setMoneyRequestDescription(
  transactionID: string,
  comment: string,
  isDraft: boolean,
) {
...
function setMoneyRequestMerchant(
  transactionID: string,
  merchant: string,
  isDraft: boolean,
) {
...
function setMoneyRequestAttendees(
  transactionID: string,
  attendees: Attendee[],
  isDraft: boolean,
) {
...
function setMoneyRequestAccountant(
  transactionID: string,
  accountant: Accountant,
  isDraft: boolean,
) {
...
function setMoneyRequestPendingFields(
  transactionID: string,
...
function setMoneyRequestTimeRate(
  transactionID: string,
  rate: number,
  isDraft: boolean,
) {
...
function setMoneyRequestTimeCount(
  transactionID: string,
  count: number,
  isDraft: boolean,
) {
...
function setMoneyRequestCategory(
  ...
  const transaction =
    allTransactionDrafts[
      `${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${transactionID}`
    ];
  const { categoryTaxCode, categoryTaxAmount } = getCategoryTaxCodeAndAmount(
    category,
    transaction,
    policy,
  );
...
function setMoneyRequestReimbursable(
  transactionID: string,
  reimbursable: boolean,
) {
  Onyx.merge(`${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${transactionID}`, {
...
function setGPSTransactionDraftData(
  transactionID: string,
  gpsDraftDetails: OnyxTypes.GpsDraftDetails | undefined,
  distance: number,
) {
...
function setMoneyRequestDistance(
  transactionID: string,
  distanceAsFloat: number,
  isDraft: boolean,
  distanceUnit: Unit,
) {
  Onyx.merge(
    `${isDraft ? ONYXKEYS.COLLECTION.TRANSACTION_DRAFT : ONYXKEYS.COLLECTION.TRANSACTION}${transactionID}`,
    { comment: { customUnit: { quantity: distanceAsFloat, distanceUnit } } },
  );
}
 ...
 */
function setMoneyRequestOdometerReading(
  transactionID: string,
  startReading: number,
  endReading: number,
  isDraft: boolean,
...
function revokeOdometerImageUri(
  image: FileObject | string | null | undefined,
  nextImage?: FileObject | string | null,
): void {
...
function setMoneyRequestOdometerImage(
  ...
  const imageKey =
    imageType === CONST.IOU.ODOMETER_IMAGE_TYPE.START
      ? "odometerStartImage"
      : "odometerEndImage";
 ...
 * @param isDraft - Whether this is a draft transaction
 */
function removeMoneyRequestOdometerImage(
  transactionID: string,
  imageType: OdometerImageType,
  isDraft: boolean,
) {
...
function setMoneyRequestDistanceRate(
  ...
  if (
    newDistanceUnit &&
    newDistanceUnit !== transaction?.comment?.customUnit?.distanceUnit
  ) {
    newDistance = DistanceRequestUtils.convertDistanceUnit(
      getDistanceInMeters(
        transaction,
        transaction?.comment?.customUnit?.distanceUnit,
      ),
...
function getReceiptError(
  ...
  const formattedRetryParams =
    typeof retryParams === "string" ? retryParams : JSON.stringify(retryParams);
...
function getFieldViolationsOnyxData(
  iouReport: OnyxTypes.Report,
): OnyxData<typeof ONYXKEYS.COLLECTION.REPORT_VIOLATIONS> {
  const missingFields: OnyxTypes.ReportFieldsViolations = {};
  const excludedFields = Object.values(
    CONST.REPORT_VIOLATIONS_EXCLUDED_FIELDS,
  ) as string[];

  for (const field of Object.values(iouReport.fieldList ?? {})) {
    if (
      excludedFields.includes(field.fieldID) ||
      !!field.value ||
      !!field.defaultValue
    ) {
      continue;
...
> {
  ...
  const optimisticIOUReportAction = buildOptimisticIOUReportAction({
    ...
    reportActionID:
      testDriveIOUParams.iouOptimisticParams.action.reportActionID,
  });
  // eslint-disable-next-line @typescript-eslint/no-deprecated
  const text = Localize.translateLocal(
    "testDrive.employeeInviteMessage",
    personalDetailsList?.[userAccountID]?.firstName ?? "",
  );
  const textComment = buildOptimisticAddCommentReportAction(
    ...
    undefined,
    undefined,
    testDriveIOUParams.testDriveCommentReportActionID,
  );
  textComment.reportAction.created = DateUtils.subtractMillisecondsFromDateTime(
    testDriveIOUParams.iouOptimisticParams.createdAction.created,
    1,
  );
...
function buildOnyxDataForMoneyRequest(
...
): OnyxData<BuildOnyxDataForMoneyRequestKeys> {
  const {
    isNewChatReport,
  ...
  const clearedPendingFields = Object.fromEntries(
    Object.keys(transaction.pendingFields ?? {}).map((key) => [key, null]),
  );
  ...
  const existingTransactionThreadReport =
    allReports?.[
      `${ONYXKEYS.COLLECTION.REPORT}${existingTransactionThreadReportID}`
    ] ?? null;
  ...
  if (chat.report) {
    onyxData.optimisticData?.push({
      ...
      value: {
        ...
        ...(shouldCreateNewMoneyRequestReport
          ? { lastVisibleActionCreated: chat.reportPreviewAction.created }
          : {}),
        // do not update iouReportID if auto submit beta is enabled and it is a scan request
        ...(isASAPSubmitBetaEnabled && isScanRequest
          ? {}
  ...
  onyxData.optimisticData?.push(
    {
      onyxMethod: shouldCreateNewMoneyRequestReport
        ? Onyx.METHOD.SET
        : Onyx.METHOD.MERGE,
      ...
      ? {
          ...
          value: {
            [iou.createdAction.reportActionID]:
              iou.createdAction as OnyxTypes.ReportAction,
  ...
  if (
    ...
    !isEmptyObject(transactionThreadCreatedReportAction)
  ) {
    onyxData.optimisticData?.push({
      onyxMethod: Onyx.METHOD.MERGE,
  ...
  if (isMoneyRequestToManagerMcTest) {
    ...
    const managerMcTestParticipant =
      getManagerMcTestParticipant(currentUserAccountIDParam) ?? {};
    const optimisticIOUReportAction = buildOptimisticIOUReportAction({
      type:
    ...
    onyxData.optimisticData?.push(
      {
        ...
        value: {
          [CONST.PRODUCT_TRAINING_TOOLTIP_NAMES.SCAN_TEST_TOOLTIP]:
            DateUtils.getDBTime(date.valueOf()),
        },
      },
      {
        onyxMethod: Onyx.METHOD.MERGE,
  ...
  onyxData.failureData?.push(
    {
      ...
      value: {
        ...
        ...(isNewChatReport
          ? {
              errorFields: {
                createChat: getMicroSecondOnyxErrorWithTranslationKey(
                  "report.genericCreateReportFailureMessage",
                ),
              },
            }
          : {}),
  ...
  if (violationsOnyxData) {
    const shouldFixViolations =
      Array.isArray(violationsOnyxData.value) &&
      violationsOnyxData.value.length > 0;
    ...
    onyxData.optimisticData?.push(violationsOnyxData, {
      ...
      value: buildNextStepNew({
        report: iou.report,
        predictedNextStatus:
          iou.report.statusNum ?? CONST.REPORT.STATE_NUM.OPEN,
        shouldFixViolations,
        policy,
        currentUserAccountIDParam,
        currentUserEmailParam,
...
type BuildOnyxDataForTrackExpenseParams = {
  ...
  policyParams: {
    ...
    tagList?: OnyxInputValue<OnyxTypes.PolicyTagLists>;
    categories?: OnyxInputValue<OnyxTypes.PolicyCategories>;
  };
  shouldCreateNewMoneyRequestReport: boolean;
  existingTransactionThreadReportID?: string;
...
function buildOnyxDataForTrackExpense({
  ...
  const {
    report: iouReport,
    createdAction: iouCreatedAction,
    action: iouAction,
  } = iou;
  const {
    transaction,
    threadReport: transactionThreadReport,
    threadCreatedReportAction: transactionThreadCreatedReportAction,
  } = transactionParams;
  const {
    policy,
    tagList: policyTagList,
  ...
  let newQuickAction: QuickActionName = isSelfDMReport
    ? CONST.QUICK_ACTIONS.TRACK_MANUAL
    : CONST.QUICK_ACTIONS.REQUEST_MANUAL;
  if (isScanRequest) {
    newQuickAction = isSelfDMReport
      ? CONST.QUICK_ACTIONS.TRACK_SCAN
      : CONST.QUICK_ACTIONS.REQUEST_SCAN;
  } else if (isDistanceRequest) {
    newQuickAction = isSelfDMReport
      ? CONST.QUICK_ACTIONS.TRACK_DISTANCE
      : CONST.QUICK_ACTIONS.REQUEST_DISTANCE;
  }
  const existingTransactionThreadReport =
    allReports?.[
      `${ONYXKEYS.COLLECTION.REPORT}${existingTransactionThreadReportID}`
        ...
        value: {
          ...
          iouReportID:
            isASAPSubmitBetaEnabled && isScanRequest
              ? null
              : iouReport?.reportID,
          lastVisibleActionCreated: shouldCreateNewMoneyRequestReport
            ? reportPreviewAction?.created
            : chatReport.lastVisibleActionCreated,
        },
      },
      {
        onyxMethod: Onyx.METHOD.SET,
        ...
        ? {
            ...
            value: {
              [iouCreatedAction.reportActionID]:
                iouCreatedAction as OnyxTypes.ReportAction,
  ...
  if (isControlPolicy(policy) && iouReport) {
    const {
      optimisticData: fieldViolationsOptimisticData,
      failureData: fieldViolationsFailureData,
    } = getFieldViolationsOnyxData(iouReport);
...
function getDeleteTrackExpenseInformation(
  ...
  const shouldDeleteTransactionThread =
    !isMovingTransactionFromTrackExpense && !!transactionThreadID;

  const shouldShowDeletedRequestMessage =
  ...
  const updatedReportAction = {
    [reportAction.reportActionID]: {
      pendingAction: shouldShowDeletedRequestMessage
  ...
  if (chatReport) {
    canUserPerformWriteAction = !!canUserPerformWriteActionReportUtils(
      chatReport,
      isChatReportArchived,
    );
  }
  const lastVisibleAction = getLastVisibleAction(
    chatReport?.reportID,
  ...
  const cleanUpTransactionThreadReportOnyxData =
  ...
  if (actionableWhisperReportActionID) {
    const actionableWhisperReportAction = getReportAction(
      chatReport?.reportID,
      actionableWhisperReportActionID,
    );
    failureData.push({
      ...
      value: {
        [actionableWhisperReportActionID]: {
          originalMessage: {
            resolution: isActionableTrackExpense(actionableWhisperReportAction)
              ? (getOriginalMessage(actionableWhisperReportAction)
                  ?.resolution ?? null)
              : null,
          },
        },
  ...
  failureData.push(
    {
      ...
      value: {
        [reportAction.reportActionID]: {
          ...
          pendingAction: null,
          errors: getMicroSecondOnyxErrorWithTranslationKey(
            "iou.error.genericDeleteFailureMessage",
          ),
        },
      },
    },
...
function recalculateOptimisticReportName(
  ...
  const titleFormula =
    policy.fieldList[CONST.POLICY.FIELDS.FIELD_LIST_TITLE]?.defaultValue ?? "";
  if (!titleFormula) {
    return undefined;
  }
  return populateOptimisticReportFormula(
    titleFormula,
    iouReport as Parameters<typeof populateOptimisticReportFormula>[1],
    policy,
  );
}
...
function maybeUpdateReportNameForFormulaTitle(
  ...
  const reportNameValuePairs =
    allReportNameValuePairs?.[
      `${ONYXKEYS.COLLECTION.REPORT_NAME_VALUE_PAIRS}${iouReport.reportID}`
    ];
...
function getMoneyRequestInformation(
  moneyRequestInformation: MoneyRequestInformationParams,
  ...
  let chatReport =
    !isEmptyObject(parentChatReport) && parentChatReport?.reportID
      ? parentChatReport
      : null;
  ...
  if (
    chatReport &&
    !isPolicyExpenseChat &&
    !isPolicyExpenseChatReportUtil(chatReport) &&
    !isSelfDM(chatReport) &&
    !isGroupChat(chatReport) &&
    !isDeprecatedGroupDM(chatReport)
  ) {
    const parentChatReportParticipants = Object.keys(
      chatReport.participants ?? {},
    ).map(Number);
    const expectedParticipants = [payerAccountID, payeeAccountID].sort();
    const sortedParentChatReportParticipants =
      parentChatReportParticipants.sort();

    const participantsMatch =
      expectedParticipants.length ===
        sortedParentChatReportParticipants.length &&
      expectedParticipants.every(
        (id, index) => id === sortedParentChatReportParticipants.at(index),
      );
  ...
  if (!chatReport && isPolicyExpenseChat) {
    chatReport =
      allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${participant.reportID}`] ??
      null;
  }

  if (!chatReport) {
    chatReport =
      getChatByParticipants([payerAccountID, payeeAccountID]) ?? null;
  }
  ...
  if (existingIOUReport) {
    ...
    iouReport =
      allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${moneyRequestReportID}`] ??
      null;
  } else if (
    !allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${chatReport.iouReportID}`]
  ...
  if (!iouReport || shouldCreateNewMoneyRequestReport) {
      ...
      if (!Number.isNaN(iouReport.total) && iouReport.total !== undefined) {
        ...
        if (!reimbursable) {
          if (newNonReimbursableTotal !== undefined) {
            ...
            iouReport.nonReimbursableTotal =
              (iouReport.nonReimbursableTotal ?? 0) - amount;
          }
        }
    ...
    iouReport = updateIOUOwnerAndTotal(
      iouReport,
      payeeAccountID,
      amount,
      currency,
  ...
  const isDistanceRequest =
    existingTransaction &&
    isDistanceRequestTransactionUtils(existingTransaction);
  const isManualDistanceRequest =
    existingTransaction &&
  ...
  let optimisticTransaction = buildOptimisticTransaction({
    ...
    transactionParams: {
      ...
      ...(modifiedAmount !== undefined && {
        modifiedAmount: isExpenseReport(iouReport)
          ? -modifiedAmount
          : modifiedAmount,
      }),
      currency,
    ...
    isDemoTransactionParam:
      isSelectedManagerMcTest(participant.login) ||
      transactionParams.receipt?.isTestDriveReceipt,
  });

  iouReport.transactionCount = (iouReport.transactionCount ?? 0) + 1;

  const optimisticPolicyRecentlyUsedCategories =
    mergePolicyRecentlyUsedCategories(category, policyRecentlyUsedCategories);
  ...
  const optimisticPolicyRecentlyUsedCurrencies =
    mergePolicyRecentlyUsedCurrencies(currency, policyRecentlyUsedCurrencies);
  ...
  if (isDistanceRequest && existingTransaction) {
    optimisticTransaction = fastMerge(
      existingTransaction,
  ...
  if (isSplitExpense && existingTransaction) {
    const {
      ...
      ...existingTransactionWithoutConvertedAmount
    } = existingTransaction;
    optimisticTransaction = fastMerge(
      existingTransactionWithoutConvertedAmount,
      optimisticTransaction,
    ...
    const originalAmount =
      Number(existingTransaction.modifiedAmount) || existingTransaction.amount;
    // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing -- modifiedAmount can be empty string
    const splitAmount =
      Number(optimisticTransaction.modifiedAmount) ||
      optimisticTransaction.amount;
    if (originalConvertedAmount && originalAmount && splitAmount) {
      optimisticTransaction.convertedAmount = Math.round(
        (originalConvertedAmount * splitAmount) / originalAmount,
      );
    }
  }
  ...
  let reportPreviewAction = shouldCreateNewMoneyRequestReport
    ? null
    : getReportPreviewAction(chatReport.reportID, iouReport.reportID);
  ...
  const shouldCreateOptimisticPersonalDetails =
    isNewChatReport && !allPersonalDetails[payerAccountID];
  // Add optimistic personal details for participant
  const optimisticPersonalDetailListAction =
    shouldCreateOptimisticPersonalDetails
      ? {
          [payerAccountID]: {
  ...
  const { optimisticData, successData, failureData } =
  ...
  return {
    ...
    transaction: optimisticTransaction,
    iouAction,
    createdChatReportActionID: isNewChatReport
      ? optimisticCreatedActionForChat.reportActionID
      : undefined,
    createdIOUReportActionID: shouldCreateNewMoneyRequestReport
...
function computePerDiemExpenseAmount(customUnit: TransactionCustomUnit) {
  ...
  return subRates.reduce(
    (total, subRate) => total + subRate.quantity * subRate.rate,
    0,
  );
}

function computePerDiemExpenseMerchant(
  customUnit: TransactionCustomUnit,
  policy: OnyxEntry<OnyxTypes.Policy>,
) {
  ...
  const formattedTime = DateUtils.getFormattedDateRangeForPerDiem(
    new Date(startDate),
    new Date(endDate),
  );
...
function mergePolicyRecentlyUsedCategories(
  category: string | undefined,
  policyRecentlyUsedCategories: OnyxEntry<OnyxTypes.RecentlyUsedCategories>,
) {
  let mergedCategories: string[];
  if (category) {
    const categoriesArray = Array.isArray(policyRecentlyUsedCategories)
      ? policyRecentlyUsedCategories
      : [];
...
function mergePolicyRecentlyUsedCurrencies(
 ...
 * it creates optimistic versions of them and uses those instead
 */
function getPerDiemExpenseInformation(
  perDiemExpenseInformation: PerDiemExpenseInformationParams,
): MoneyRequestInformation {
  const {
    parentChatReport,
    transactionParams,
  ...
  const defaultComment = computeDefaultPerDiemExpenseComment(
    customUnit,
    currency,
  );
  ...
  const shouldCreateNewMoneyRequestReport =
    shouldCreateNewMoneyRequestReportReportUtils(iouReport, chatReport, false);
  ...
  const optimisticPolicyRecentlyUsedCurrencies =
    mergePolicyRecentlyUsedCurrencies(currency, policyRecentlyUsedCurrencies);
  const optimisticPolicyRecentlyUsedDestinations = customUnit.customUnitRateID
    ? [
        ...new Set([
...
function getTrackExpenseInformation(
  params: GetTrackExpenseInformationParams,
): TrackExpenseInformation {
  const {
    parentChatReport,
    moneyRequestReportID = "",
    existingTransactionID,
  ...
  if (!chatReport) {
    chatReport =
      allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${findSelfDMReportID()}`] ??
      null;
  }
  ...
  if (!chatReport) {
    ...
    const selfDMCreatedReportAction = buildOptimisticCreatedReportAction(
      currentUserEmail ?? "",
      currentTime,
    );
  ...
  const shouldUseMoneyReport =
    !!isPolicyExpenseChat &&
    chatReport.chatType !== CONST.REPORT.CHAT_TYPE.SELF_DM;
  ...
  if (shouldUseMoneyReport) {
    ...
    if (!iouReport || shouldCreateNewMoneyRequestReport) {
      ...
      if (iouReport?.currency === currency) {
        if (
          !Number.isNaN(iouReport.total) &&
          iouReport.total !== undefined &&
          typeof iouReport.nonReimbursableTotal === "number"
        ) {
          iouReport.total -= amount;
        ...
        if (
          typeof iouReport.unheldTotal === "number" &&
          typeof iouReport.unheldNonReimbursableTotal === "number"
        ) {
          iouReport.unheldTotal -= amount;
  ...
  const existingTransaction =
    allTransactionDrafts[
      `${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${existingTransactionID ?? CONST.IOU.OPTIMISTIC_TRANSACTION_ID}`
    ];
  const isDistanceRequest =
    existingTransaction &&
    isDistanceRequestTransactionUtils(existingTransaction);
  const isManualDistanceRequest =
    existingTransaction &&
    isManualDistanceRequestTransactionUtils(existingTransaction);
  const isOdometerDistanceRequest =
    existingTransaction &&
    isOdometerDistanceRequestTransactionUtils(existingTransaction);
  const isGPSDistanceRequest =
    existingTransaction &&
    isGPSDistanceRequestTransactionUtils(existingTransaction);
  let optimisticTransaction = buildOptimisticTransaction({
    existingTransactionID: optimisticTransactionID,
    existingTransaction,
    policy,
    transactionParams: {
      ...
      currency,
      reportID:
        shouldUseMoneyReport && iouReport
          ? iouReport.reportID
  ...
  let reportPreviewAction: OnyxInputValue<
    OnyxTypes.ReportAction<typeof CONST.REPORT.ACTIONS.TYPE.REPORT_PREVIEW>
  > = null;
  ...
  let actionableTrackExpenseWhisper: OnyxInputValue<OnyxTypes.ReportAction> =
    null;
...
function calculateDiffAmount(
  ...
  if (
    updatedCurrency === iouReport.currency &&
    currentCurrency === iouReport.currency
  ) {
...
function getUpdateMoneyRequestParams(
  ...
  const pendingFields: OnyxTypes.Transaction["pendingFields"] =
    Object.fromEntries(
      Object.keys(transactionChanges).map((key) => [
        key,
        CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE,
      ]),
    );
  ...
  const errorFields = Object.fromEntries(
    Object.keys(pendingFields).map((key) => [
      key,
      getMicroSecondOnyxErrorWithTranslationKey(
        "iou.error.genericEditFailureMessage",
      ),
    ]),
  );
  ...
  const dataToIncludeInParams: Partial<TransactionDetails> = Object.fromEntries(
    Object.entries(transactionDetails ?? {}).filter(
      ([key]) => key in transactionChanges,
    ),
  );
  ...
  if (
    transaction &&
    updatedTransaction &&
    (hasPendingWaypoints || hasModifiedDistanceRate)
  ) {
  ...
  if (
    ...
    const isDistanceTransaction =
      transaction && isDistanceRequestTransactionUtils(transaction);
  ...
  const calculatedDiffAmount = calculateDiffAmount(
    iouReport,
    updatedTransaction,
    transaction,
  );
  ...
  if (!iouReport) {
    ...
    (isExpenseReport(iouReport) || isInvoiceReportReportUtils(iouReport)) &&
    !Number.isNaN(iouReport.total) &&
    iouReport.total !== undefined
  ) {
    ...
    if (
      ...
      typeof updatedMoneyRequestReport.nonReimbursableTotal === "number"
    ) {
    ...
    if (
      ...
      updatedMoneyRequestReport.nonReimbursableTotal +=
        updatedTransaction.reimbursable
    ...
    if (!isTransactionOnHold) {
      ...
      if (
        !transaction?.reimbursable &&
        typeof updatedMoneyRequestReport.unheldNonReimbursableTotal === "number"
      ) {
      ...
      if (
        ...
        updatedMoneyRequestReport.unheldNonReimbursableTotal +=
          updatedTransaction.reimbursable
            ? -updatedTransaction.amount
            : updatedTransaction.amount;
      }
    ...
    if ("reimbursable" in transactionChanges) {
      updatedMoneyRequestReport = maybeUpdateReportNameForFormulaTitle(
        updatedMoneyRequestReport,
        policy,
      );
    }
  ...
  optimisticData.push(
    {
      ...
      value: {
        ...
        ...(isTotalIndeterminate && {
          pendingFields: { total: CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE },
        }),
      },
    },
    {
  ...
  if (
    ...
    ("amount" in transactionChanges || "currency" in transactionChanges)
  ) {
  ...
  if ("currency" in transactionChanges) {
    const optimisticRecentlyUsedCurrencies = mergePolicyRecentlyUsedCurrencies(
      transactionChanges.currency,
      policyRecentlyUsedCurrencies ?? [],
    );
  ...
  if ("attendees" in transactionChanges) {
    ...
    const overLimitViolation = violations?.find(
      (violation) => violation.name === "overLimit",
    );
    if (overLimitViolation) {
      const limitForSingleAttendee =
        ViolationsUtils.getViolationAmountLimit(overLimitViolation);
      if (
        limitForSingleAttendee * (transactionChanges?.attendees?.length ?? 1) >
        ...
        optimisticData.push({
          ...
          key: `${ONYXKEYS.COLLECTION.TRANSACTION_VIOLATIONS}${transactionID}`,
          value:
            violations?.filter((violation) => violation.name !== "overLimit") ??
            [],
        });
      }
    }
    optimisticData.push({
      ...
      key: ONYXKEYS.NVP_RECENT_ATTENDEES,
      value: lodashUnionBy(
        transactionChanges.attendees?.map(
  ...
  if (
    ...
    let optimisticViolations =
      hasModifiedAmount || hasModifiedDate || hasModifiedCurrency
        ? currentTransactionViolations.filter(
            (violation) =>
    ...
    optimisticViolations =
      hasModifiedCategory && transactionChanges.category === ""
        ? optimisticViolations.filter(
            (violation) =>
              violation.name !== CONST.VIOLATIONS.CATEGORY_OUT_OF_POLICY,
          )
        : optimisticViolations;
    if (hasPendingWaypoints) {
      optimisticViolations = optimisticViolations.filter(
        (violation) => violation.name !== CONST.VIOLATIONS.NO_ROUTE,
      );
    }
    ...
    if (
      violationsOnyxData &&
      ((iouReport?.statusNum ?? CONST.REPORT.STATUS_NUM.OPEN) ===
        CONST.REPORT.STATUS_NUM.OPEN ||
        (hasModifiedReimbursable &&
      ...
      const moneyRequestReport =
        updatedMoneyRequestReport ?? iouReport ?? undefined;
      const hasViolations = hasViolationsReportUtils(
...
> {
  ...
  const chatReport =
    allReports?.[
      `${ONYXKEYS.COLLECTION.REPORT}${transactionThread?.parentReportID}`
    ] ?? null;
...
function updateMoneyRequestDistance({
  ...
  let data: UpdateMoneyRequestData<
    UpdateMoneyRequestDataKeys | typeof ONYXKEYS.NVP_RECENT_WAYPOINTS
  >;
  // eslint-disable-next-line @typescript-eslint/no-deprecated
  if (isTrackExpenseReport(transactionThreadReport) && isSelfDM(parentReport)) {
    data = getUpdateTrackExpenseParams(
  ...
  if (!distance) {
    const recentServerValidatedWaypoints = recentWaypoints.filter(
  ...
  if (transactionBackup) {
    ...
    const allWaypointKeys = [
      ...new Set([
        ...Object.keys(transactionBackup.comment?.waypoints ?? {}),
        ...Object.keys(transaction?.comment?.waypoints ?? {}),
      ]),
    ];
    const onyxWaypoints = allWaypointKeys.reduce(
      (acc: NullishDeep<WaypointCollection>, key) => {
        acc[key] = transactionBackup.comment?.waypoints?.[key]
          ? { ...transactionBackup.comment?.waypoints?.[key] }
          : null;
        return acc;
      },
      {},
    );
    const allModifiedWaypointsKeys = [
      ...new Set([
        ...Object.keys(waypoints ?? {}),
        ...Object.keys(transaction?.modifiedWaypoints ?? {}),
      ]),
    ];
    const onyxModifiedWaypoints = allModifiedWaypointsKeys.reduce(
      (acc: NullishDeep<WaypointCollection>, key) => {
        acc[key] = transactionBackup.modifiedWaypoints?.[key]
          ? { ...transactionBackup.modifiedWaypoints?.[key] }
          : null;
        return acc;
      },
      {},
...
function updateMoneyRequestDistanceRate({
  ...
  const transactionChanges: TransactionChanges = {
    ...
    ...(typeof updatedTaxAmount === "number"
  ...
  if (transaction) {
    ...
    if (
      ...
      newDistanceUnit &&
      newDistanceUnit !== existingDistanceUnit
    ) {
...
const getConvertTrackedExpenseInformation = (
  ...
  const {
    ...
    allReports?.[
      `${ONYXKEYS.COLLECTION.REPORT}${linkedTrackedExpenseReportID}`
    ],
    transactionID,
    linkedTrackedExpenseReportAction,
  ...
  const modifiedExpenseReportAction = buildOptimisticMovedTransactionAction(
    transactionThreadReportID,
    linkedTrackedExpenseReportID ?? CONST.REPORT.UNREPORTED_REPORT_ID,
  );
...
function addTrackedExpenseToPolicy(
  parameters: AddTrackedExpenseToPolicyParam,
  onyxData: OnyxData<BuildOnyxDataForMoneyRequestKeys>,
) {
...
function convertTrackedExpenseToRequest(
  convertTrackedExpenseParams: ConvertTrackedExpenseToRequestParams,
) {
  const {
    payerParams,
    transactionParams,
  ...
  optimisticData?.push(
    ...(convertTrackedExpenseInformation.optimisticData ?? []),
  );
  ...
  if (workspaceParams) {
    const params = {
      ...
      modifiedExpenseReportActionID:
        convertTrackedExpenseInformation.modifiedExpenseReportActionID,
...
function convertBulkTrackedExpensesToIOU(
  ...
  const iouReport =
    allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${targetReportID}`];
  ...
  const chatReport =
    allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${chatReportID}`];
  if (!chatReport) {
    Log.warn("[convertBulkTrackedExpensesToIOU] Chat report not found", {
      chatReportID,
    });
    return;
  }

  const participantAccountIDs = getReportRecipientAccountIDs(
    iouReport,
    userAccountID,
  );
  ...
  for (const transactionID of transactionIDs) {
    ...
    const linkedTrackedExpenseReportAction = Object.values(
      selfDMReportActions,
    ).find((action) => {
    ...
    if (!linkedTrackedExpenseReportAction) {
      Log.warn(
        "[convertBulkTrackedExpensesToIOU] Tracked expense IOU action not found",
        { transactionID },
      );
      continue;
    }

    const actionableWhisperReportActionID = getTrackExpenseActionableWhisper(
      transactionID,
      selfDMReportID,
    )?.reportActionID;
    ...
    const transactionThreadReportID = (
      linkedTrackedExpenseReportAction as OnyxTypes.ReportAction
    ).childReportID;

    if (!transactionThreadReportID) {
      Log.warn(
        "[convertBulkTrackedExpensesToIOU] No transaction thread found for tracked expense, skipping",
        {
          transactionID,
          actionReportActionID: (
            linkedTrackedExpenseReportAction as OnyxTypes.ReportAction
          ).reportActionID,
        },
      );
...
function categorizeTrackedExpense(trackedExpenseParams: TrackedExpenseParams) {
  ...
  const parameters: CategorizeTrackedExpenseApiParams = {
    ...
    policyExpenseCreatedReportActionID:
      createdWorkspaceParams?.expenseCreatedReportActionID,
    adminsChatReportID: createdWorkspaceParams?.adminsChatReportID,
    adminsCreatedReportActionID:
      createdWorkspaceParams?.adminsCreatedReportActionID,
    ...
    customUnitRateID:
      createdWorkspaceParams?.customUnitRateID ??
      transactionParams.customUnitRateID,
    attendees: transactionParams.attendees
      ? JSON.stringify(transactionParams.attendees)
      : undefined,
  };
...
function shareTrackedExpense(trackedExpenseParams: TrackedExpenseParams) {
  ...
  const accountantEmail = addSMSDomainIfPhoneNumber(
    accountantParams?.accountant?.login,
  );
  ...
  const {
    optimisticData: shareTrackedExpenseOptimisticData = [],
    successData: shareTrackedExpenseSuccessData = [],
    failureData: shareTrackedExpenseFailureData = [],
  } = onyxData ?? {};
  ...
  if (policyParams.policy && !policyEmployeeList?.[accountantEmail]) {
    const policyMemberAccountIDs = Object.values(
      getMemberAccountIDsForWorkspace(policyEmployeeList, false, false),
    );
    const {
    ...
    policyEmployeeList?.[accountantEmail].role !== CONST.POLICY.ROLE.ADMIN
  ) {
    const {
      optimisticData: addAccountantToWorkspaceOptimisticData,
      successData: addAccountantToWorkspaceSuccessData,
  ...
  API.write(WRITE_COMMANDS.SHARE_TRACKED_EXPENSE, parameters, {
    optimisticData,
    successData,
    failureData,
  });
...
function requestMoney(requestMoneyInformation: RequestMoneyInformation): {
  ...
  const testDriveCommentReportActionID = isTestDrive
    ? NumberUtils.rand64()
    : undefined;

  const sanitizedWaypoints = waypoints
    ? JSON.stringify(sanitizeRecentWaypoints(waypoints))
    : undefined;
  ...
  const isMovingTransactionFromTrackExpense =
    isMovingTransactionFromTrackExpenseIOUUtils(action);
  const existingTransactionID =
    isMovingTransactionFromTrackExpense &&
    linkedTrackedExpenseReportAction &&
    isMoneyRequestAction(linkedTrackedExpenseReportAction)
      ? getOriginalMessage(linkedTrackedExpenseReportAction)?.IOUTransactionID
      : undefined;
  const existingTransaction =
    action === CONST.IOU.ACTION.SUBMIT
      ? allTransactionDrafts[
          `${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${existingTransactionID}`
        ]
      : allTransactions[
          `${ONYXKEYS.COLLECTION.TRANSACTION}${existingTransactionID}`
        ];
  ...
  const {
    ...
    parentChatReport: isMovingTransactionFromTrackExpense
      ? undefined
      : currentChatReport,
    existingIOUReport,
    participantParams,
    ...
    moneyRequestReportID,
    existingTransactionID,
    existingTransaction: isDistanceRequestTransactionUtils(existingTransaction)
      ? existingTransaction
      : undefined,
    retryParams,
    testDriveCommentReportActionID,
  ...
  const activeReportID = isMoneyRequestReport
    ? report?.reportID
    : chatReport.reportID;
  ...
  switch (action) {
    case CONST.IOU.ACTION.SUBMIT: {
      ...
      const customUnitParams = isDistanceRequestTransactionUtils(transaction)
        ? {
            customUnitID: getDistanceRateCustomUnit(policyParams?.policy)
              ?.customUnitID,
            customUnitRateID,
          }
        : {};
    ...
    default: {
      ...
      const guidedSetupData: GuidedSetupData | undefined = isTestDrive
        ? prepareOnboardingOnyxData({
            ...
            onboardingMessage:
              getOnboardingMessages().onboardingMessages[
      ...
      const parameters: RequestMoneyParams = {
        ...
        attendees: attendees ? JSON.stringify(attendees) : undefined,
        isTestDrive,
        guidedSetupData: guidedSetupData
          ? JSON.stringify(guidedSetupData)
          : undefined,
        testDriveCommentReportActionID,
  ...
  if (shouldHandleNavigation) {
    ...
    const trackReport = Navigation.getReportRouteByID(
      linkedTrackedExpenseReportAction?.childReportID,
    );
    if (trackReport?.key) {
...
function submitPerDiemExpense(
  submitPerDiemExpenseInformation: PerDiemExpenseInformation,
) {
  const {
    report,
    participantParams,
  ...
  const activeReportID =
    isMoneyRequestReport && Navigation.getTopmostReportId() === report?.reportID
      ? report?.reportID
      : chatReport.reportID;

  const customUnitRate = getPerDiemRateCustomUnitRate(
    policyParams.policy,
    customUnit.customUnitRateID,
  );
 ...
 */
function getPerDiemExpenseInformationForSelfDM(
  perDiemExpenseInformation: PerDiemExpenseInformationForSelfDM,
): PerDiemExpenseInformationForSelfDMResult {
  const {
    selfDMReport,
    transactionParams,
    policy,
 ...
 */
function submitPerDiemExpenseForSelfDM(
  submitPerDiemExpenseInformation: PerDiemExpenseInformationForSelfDM,
) {
  const {
    selfDMReport,
    policy,
  ...
  const {
    ...
    createdReportActionIDForThread,
    onyxData,
  } = getPerDiemExpenseInformationForSelfDM({
    selfDMReport,
    policy,
    transactionParams,
    currentUserAccountIDParam,
...
function trackExpense(params: CreateTrackExpenseParams) {
  ...
  const trackedReceipt = validWaypoints
    ...
    : receipt;
  const sanitizedWaypoints = validWaypoints
    ? JSON.stringify(sanitizeRecentWaypoints(validWaypoints))
    : undefined;
  ...
  const activeReportID = isMoneyRequestReport
    ? report?.reportID
    : chatReport?.reportID;
  const onyxData: TrackedExpenseParams["onyxData"] =
    trackExpenseInformationOnyxData;
  ...
  const isGPSDistanceRequest =
    isGPSDistanceRequestTransactionUtils(transaction);
  ...
  switch (action) {
    case CONST.IOU.ACTION.CATEGORIZE: {
      ...
      const reportInformation: TrackedExpenseReportInformation = {
        ...
        chatReportID: chatReport?.reportID,
        isLinkedTrackedExpenseReportArchived:
          transactionData.isLinkedTrackedExpenseReportArchived,
      };
    ...
    default: {
      ...
      const parameters: TrackExpenseParams = {
        ...
        distance:
          distance !== undefined
            ? roundToTwoDecimalPlaces(distance)
            : undefined,
        created,
        merchant,
        iouReportID: iouReport?.reportID,
      ...
      if (actionableWhisperReportActionIDParam) {
        parameters.actionableWhisperReportActionID =
          actionableWhisperReportActionIDParam;
      }
...
function getOrCreateOptimisticSplitChatReport(
  ...
  const existingChatReportID =
    existingSplitChatReportID ?? participants.at(0)?.reportID;

  // Check if the report is available locally if we do have one
  const existingSplitChatOnyxData =
    allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${existingChatReportID}`];
  let existingSplitChatReport =
    existingChatReportID && existingSplitChatOnyxData
      ? { ...existingSplitChatOnyxData }
      : undefined;
  ...
  if (!existingSplitChatReport) {
    existingSplitChatReport = getChatByParticipants(
  ...
  if (existingSplitChatReport) {
    ...
    return {
      ...
      splitChatReport: existingSplitChatReport,
    };
  }
...
function createSplitsAndOnyxData({
  ...
  const currentUserEmailForIOUSplit =
    addSMSDomainIfPhoneNumber(currentUserLogin);
  const participantAccountIDs = participants.map((participant) =>
    Number(participant.accountID),
  );

  const { splitChatReport, existingSplitChatReport } =
  ...
  let splitTransaction = buildOptimisticTransaction({
    ...
    transactionParams: {
      ...
      billable,
      reimbursable,
      pendingFields: isDistanceRequest
        ? { waypoints: CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD }
        : undefined,
      attendees,
    },
  ...
  const splitCreatedReportAction = buildOptimisticCreatedReportAction(
  ...
  if (
    ...
    getReportNotificationPreference(splitChatReport) ===
      CONST.REPORT.NOTIFICATION_PREFERENCE.HIDDEN
  ) {
    splitChatReport.participants[currentUserAccountID] = {
      notificationPreference: CONST.REPORT.NOTIFICATION_PREFERENCE.ALWAYS,
    };
  }
  ...
  const optimisticData: Array<OnyxUpdate<BuildOnyxDataForMoneyRequestKeys>> = [
    ...
    },
    {
      ...
      value: {
        action:
          iouRequestType === CONST.IOU.REQUEST_TYPE.DISTANCE
            ? CONST.QUICK_ACTIONS.SPLIT_DISTANCE
            : CONST.QUICK_ACTIONS.SPLIT_MANUAL,
      ...
      ? {
          ...
          value: {
            [splitIOUReportAction.reportActionID]:
              splitIOUReportAction as OnyxTypes.ReportAction,
          },
        }
      : {
          onyxMethod: Onyx.METHOD.SET,
          ...
          value: {
            [splitCreatedReportAction.reportActionID]:
              splitCreatedReportAction as OnyxTypes.ReportAction,
            [splitIOUReportAction.reportActionID]:
              splitIOUReportAction as OnyxTypes.ReportAction,
          },
        },
  ...
  const currentUserAmount =
    splitShares?.[currentUserAccountID]?.amount ??
    calculateIOUAmount(participants.length, amount, currency, true);
  const currentUserTaxAmount = calculateIOUAmount(
    participants.length,
    taxAmount,
    currency,
    true,
  );
  const isPendingDistanceSplitBill =
    isDistanceRequest && isEmptyObject(splitShares);

  const splits: Split[] = [
    {
      email: currentUserEmailForIOUSplit,
      accountID: currentUserAccountID,
      ...(!isPendingDistanceSplitBill && {
        amount: currentUserAmount,
        taxAmount: currentUserTaxAmount,
      }),
    },
  ];
  ...
  for (const participant of participants) {
    ...
    const splitAmount =
      splitShares?.[participant.accountID ?? CONST.DEFAULT_NUMBER_ID]?.amount ??
      calculateIOUAmount(participants.length, amount, currency, false);
    const splitTaxAmount = calculateIOUAmount(
      participants.length,
      taxAmount,
      currency,
      false,
    ...
    const email =
      isOwnPolicyExpenseChat || isPolicyExpenseChat
        ? ""
        : addSMSDomainIfPhoneNumber(participant.login ?? "").toLowerCase();
    const accountID =
      isOwnPolicyExpenseChat || isPolicyExpenseChat
        ? 0
        : Number(participant.accountID);
    ...
    if (
      (!hasMultipleParticipants && !existingSplitChatReportID) ||
      isOwnPolicyExpenseChat ||
      isOneOnOneChat(splitChatReport)
    ) {
      oneOnOneChatReport = splitChatReport;
      shouldCreateOptimisticPersonalDetails =
        !existingSplitChatReport && isOptimisticPersonalDetail(accountID);
    } else {
      const existingChatReport = getChatByParticipants([
        accountID,
        currentUserAccountID,
      ]);
      isNewOneOnOneChatReport = !existingChatReport;
      shouldCreateOptimisticPersonalDetails =
        isNewOneOnOneChatReport && isOptimisticPersonalDetail(accountID);
      oneOnOneChatReport =
        existingChatReport ??
        buildOptimisticChatReport({
    ...
    if (!oneOnOneIOUReport || shouldCreateNewOneOnOneIOUReport) {
      ...
      oneOnOneIOUReport = updateIOUOwnerAndTotal(
        oneOnOneIOUReport,
        currentUserAccountID,
        splitAmount,
        currency,
    ...
    let oneOnOneTransaction = buildOptimisticTransaction({
      ...
      transactionParams: {
        ...
        tag,
        taxCode,
        taxAmount: isExpenseReport(oneOnOneIOUReport)
          ? -splitTaxAmount
          : splitTaxAmount,
        billable,
    ...
    oneOnOneIOUReport.transactionCount =
      (oneOnOneIOUReport.transactionCount ?? 0) + 1;

    if (isDistanceRequest) {
      oneOnOneTransaction = fastMerge(
        existingTransaction,
        oneOnOneTransaction,
        false,
      );
    ...
    const oneOnOnePersonalDetailListAction: OnyxTypes.PersonalDetailsList =
      shouldCreateOptimisticPersonalDetails
        ? {
            [accountID]: {
              accountID,
    ...
    if (oneOnOneReportPreviewAction) {
      oneOnOneReportPreviewAction = updateReportPreview(
        oneOnOneIOUReport,
...
function createDistanceRequest(
  distanceRequestInformation: CreateDistanceRequestInformation,
) {
  const {
    report,
    participants,
  ...
  let onyxData: OnyxData<
    ...
    | typeof ONYXKEYS.GPS_DRAFT_DETAILS
  >;
  const sanitizedWaypoints = !isManualDistanceRequest
  ...
  if (iouType === CONST.IOU.TYPE.SPLIT) {
    ...
    parameters = {
      ...
      waypoints: JSON.stringify(sanitizedWaypoints),
      distance:
        distance !== undefined ? roundToTwoDecimalPlaces(distance) : undefined,
      receipt,
      odometerStart,
      odometerEnd,
      created,
  ...
  const activeReportID =
    isMoneyRequestReport && report?.reportID
      ? report.reportID
      : parameters.chatReportID;
...
function prepareToCleanUpMoneyRequest(
  ...
  if (transactionThreadID) {
    transactionThread =
      allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${transactionThreadID}`] ??
      null;
  }
  ...
  const shouldDeleteIOUReport =
    getReportTransactions(iouReportID).filter(
      (trans) => !transactionIDsPendingDeletion?.includes(trans.transactionID),
    ).length === 1;

  const iouReportActions =
    allReportActions?.[`${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${iouReportID}`];
  if (shouldDeleteIOUReport) {
    for (const [reportActionID, reportActionData] of Object.entries(
      ...
      if (
        ...
        reportActionData.pendingAction ===
          CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE ||
  ...
  const updatedReportPreviewAction: Partial<
    OnyxTypes.ReportAction<typeof CONST.REPORT.ACTIONS.TYPE.REPORT_PREVIEW>
  > = cloneDeep(reportPreviewAction ?? {});
  updatedReportPreviewAction.pendingAction = shouldDeleteIOUReport
    ? CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE
    : CONST.RED_BRICK_ROAD_PENDING_ACTION.UPDATE;

  const transactionPendingDelete = transactionIDsPendingDeletion?.map(
    (id) => allTransactions[`${ONYXKEYS.COLLECTION.TRANSACTION}${id}`],
  );
  const selectedTransactions = selectedTransactionIDs?.map(
    (id) => allTransactions[`${ONYXKEYS.COLLECTION.TRANSACTION}${id}`],
  );
  const canEditTotal = !selectedTransactions?.some(
    (trans) => getCurrency(trans) !== iouReport?.currency,
  );
  const isExpenseReportType = isExpenseReport(iouReport);
  const amountDiff =
    getAmount(transaction, isExpenseReportType) +
    (transactionPendingDelete?.reduce(
      (prev, curr) => prev + getAmount(curr, isExpenseReportType),
      0,
    ) ?? 0);
  const unheldAmountDiff =
    ...
    (transactionPendingDelete?.reduce(
      (prev, curr) =>
        prev + (!isOnHold(curr) ? getAmount(curr, isExpenseReportType) : 0),
      0,
    ) ?? 0);
  ...
  if (iouReport && isExpenseReportType) {
    ...
    if (
      typeof updatedIOUReport.total === "number" &&
      ...
      if (
        !transaction?.reimbursable &&
        typeof updatedIOUReport.nonReimbursableTotal === "number"
      ) {
        const nonReimbursableAmountDiff =
          getAmount(transaction, true) +
          (transactionPendingDelete?.reduce(
            (prev, curr) =>
              prev + (!curr?.reimbursable ? getAmount(curr, true) : 0),
            0,
          ) ?? 0);
      ...
      if (!isTransactionOnHold) {
        ...
        if (
          !transaction?.reimbursable &&
          typeof updatedIOUReport.unheldNonReimbursableTotal === "number"
        ) {
          const unheldNonReimbursableAmountDiff =
            ...
            (transactionPendingDelete?.reduce(
              (prev, curr) =>
                prev +
                (!isOnHold(curr) && !curr?.reimbursable
                  ? getAmount(curr, true)
                  : 0),
              0,
            ) ?? 0);
          updatedIOUReport.unheldNonReimbursableTotal +=
            unheldNonReimbursableAmountDiff;
        }
      }
    }
  } else {
  ...
  if (updatedIOUReport) {
    ...
    const iouReportLastMessageText = getLastVisibleMessage(
      iouReport?.reportID,
      canUserPerformWriteAction,
      updatedReportAction,
    ).lastMessageText;
  ...
  const hasNonReimbursableTransactions =
    hasNonReimbursableTransactionsReportUtils(iouReport?.reportID);
  // eslint-disable-next-line @typescript-eslint/no-deprecated
  const messageText = Localize.translateLocal(
    ...
    getPersonalDetailsForAccountID(
      updatedIOUReport?.managerID ?? CONST.DEFAULT_NUMBER_ID,
    ).login ?? "",
  );
  ...
  if (getReportActionMessage(updatedReportPreviewAction)) {
    if (Array.isArray(updatedReportPreviewAction?.message)) {
      ...
      !Array.isArray(updatedReportPreviewAction.message) &&
      updatedReportPreviewAction.message
    ) {
      updatedReportPreviewAction.message.text = messageText;
      updatedReportPreviewAction.message.deleted = shouldDeleteIOUReport
        ? DateUtils.getDBTime()
        : "";
    }
  }
  ...
  if (
    ...
    reportPreviewAction?.childMoneyRequestCount &&
    reportPreviewAction?.childMoneyRequestCount > 0
  ) {
    updatedReportPreviewAction.childMoneyRequestCount =
...
function getNavigationUrlOnMoneyRequestDelete(
  ...
  const { shouldDeleteTransactionThread, shouldDeleteIOUReport } =
...
function cleanUpMoneyRequest(
  ...
  const reportActionsOnyxUpdates: Array<
    OnyxUpdate<typeof ONYXKEYS.COLLECTION.REPORT_ACTIONS>
  > = [];
...
function getCleanUpTransactionThreadReportOnyxData({
  ...
  if (shouldDeleteTransactionThread) {
    ...
    if (transactionThreadID) {
      ...
      transactionThreadReportActions =
        allReportActions?.[
          `${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${transactionThreadID}`
        ] ?? null;
    }
  ...
  const chatReport =
    allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${iouReport?.chatReportID}`];
  const originalReportPreviewAction =
    getReportPreviewAction(chatReport?.reportID, iouReport?.reportID) ??
    undefined;
  let reportPreviewAction =
    updatedReportPreviewAction ?? originalReportPreviewAction;
  if (
    ...
    if (chatReport) {
      ...
      canUserPerformWriteAction = !!canUserPerformWriteActionReportUtils(
        chatReport,
        isChatIOUReportArchived ?? isArchivedExpenseReport,
      );
    }
    const lastVisibleAction = getLastVisibleAction(
      iouReportID,
    ...
    const {
      ...
      reportPreviewAction,
      lastVisibleAction?.childLastVisibleActionCreated ??
        lastVisibleAction?.created ??
        "",
      CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE,
      reportAction.childVisibleActionCount,
    );
    ...
    if (shouldAddUpdatedReportPreviewActionToOnyxData) {
      ...
      failureData.push({
        ...
        value: {
          [reportPreviewAction.reportActionID]: {
            childVisibleActionCount:
              originalReportPreviewAction.childVisibleActionCount,
            childCommenterCount:
              originalReportPreviewAction.childCommenterCount,
            childLastVisibleActionCreated:
              originalReportPreviewAction.childLastVisibleActionCreated,
            childOldestFourAccountIDs:
              originalReportPreviewAction.childOldestFourAccountIDs,
          },
        },
      });
    }
...
function deleteMoneyRequest({
  ...
  if (shouldDeleteIOUReport) {
    ...
    const optimisticReportActions = reportPreviewAction?.reportActionID
      ? { [reportPreviewAction.reportActionID]: null }
      : {};
  ...
  const originalReportActionsUpdate = {} as Record<
    string,
    Partial<OnyxTypes.ReportAction>
  >;
  if (shouldDeleteIOUReport) {
...
function deleteTrackExpense({
  ...
  const whisperAction = getTrackExpenseActionableWhisper(
    transactionID,
    chatReportID,
  );
  const actionableWhisperReportActionID = whisperAction?.reportActionID;
  const { parameters, optimisticData, successData, failureData } =
    getDeleteTrackExpenseInformation(
      chatReport,
      transactionID,
      reportAction,
...
function getHoldReportActionsAndTransactions(reportID: string | undefined) {
  ...
  const holdReportActions: Array<
    OnyxTypes.ReportAction<typeof CONST.REPORT.ACTIONS.TYPE.IOU>
  > = [];
  const holdTransactions: OnyxTypes.Transaction[] = [];

  for (const action of Object.values(iouReportActions)) {
    const transactionID = isMoneyRequestAction(action)
      ? getOriginalMessage(action)?.IOUTransactionID
      : undefined;
    const transaction =
      allTransactions?.[`${ONYXKEYS.COLLECTION.TRANSACTION}${transactionID}`];

    if (transaction?.comment?.hold) {
      holdReportActions.push(
        action as OnyxTypes.ReportAction<typeof CONST.REPORT.ACTIONS.TYPE.IOU>,
      );
...
function getDuplicateActionsForPartialReport(
  ...
  for (const action of Object.values(sourceReportActions)) {
    if (
      action &&
      (workflowActionTypes as readonly string[]).includes(action.actionName)
    ) {
...
function getReportFromHoldRequestsOnyxData({
  ...
  const { holdReportActions, holdTransactions } =
    getHoldReportActionsAndTransactions(iouReport?.reportID);
  ...
  const holdAmount =
    ((iouReport?.total ?? 0) - (iouReport?.unheldTotal ?? 0)) * coefficient;
  const holdNonReimbursableAmount =
    ((iouReport?.nonReimbursableTotal ?? 0) -
      (iouReport?.unheldNonReimbursableTotal ?? 0)) *
    coefficient;
  ...
  if (isApprovalFlow) {
    optimisticCreatedReportForUnapprovedAction =
      buildOptimisticCreatedReportForUnapprovedAction(
        optimisticExpenseReport.reportID,
        iouReport?.reportID,
      );
  ...
  const updateHeldReports: Record<
    string,
    Pick<
      OnyxTypes.Report,
      "parentReportActionID" | "parentReportID" | "chatReportID"
    >
  > = {};
  const addHoldReportActions: OnyxTypes.ReportActions = {};
  const addHoldReportActionsSuccess: OnyxCollection<NullishDeep<ReportAction>> =
    {};
  const deleteHoldReportActions: Record<
    string,
    Pick<OnyxTypes.ReportAction, "message">
  > = {};
  ...
  for (const holdReportAction of holdReportActions) {
    ...
    const originalMessage = getOriginalMessage(
      holdReportAction,
    ) as OnyxTypes.OriginalMessageIOU;
    ...
    optimisticHoldReportExpenseActionIDs.push({
      optimisticReportActionID: reportActionID,
      oldReportActionID: holdReportAction.reportActionID,
    });
  ...
  const updateHeldTransactions: Record<
    string,
    Pick<OnyxTypes.Transaction, "reportID">
  > = {};
  for (const transaction of holdTransactions) {
    updateHeldTransactions[
  ...
  const isApprovalEnabled = policy
    ? policy.approvalMode &&
  ...
  const [
    copiedActionsOptimistic,
    copiedActionsSuccess,
    copiedActionsFailure,
    optimisticReportActionCopyIDs,
...
function getPayMoneyRequestParams({
  ...
  const shouldCreatePolicy =
    !activePolicy ||
    !isPolicyAdmin(activePolicy) ||
    !isPaidGroupPolicy(activePolicy);

  if (
    isIndividualInvoiceRoom(chatReport) &&
    payAsBusiness &&
    shouldCreatePolicy
  ) {
    ...
    onyxData.optimisticData?.push(...(policyOptimisticData ?? []), {
      onyxMethod: Onyx.METHOD.MERGE,
      key: ONYXKEYS.NVP_ACTIVE_POLICY_ID,
      value: payerPolicyID,
    });
    onyxData.successData?.push(...(policySuccessData ?? []));
    onyxData.failureData?.push(...(policyFailureData ?? []), {
      onyxMethod: Onyx.METHOD.MERGE,
      key: ONYXKEYS.NVP_ACTIVE_POLICY_ID,
  ...
  if (
    hasHeldExpensesReportUtils(iouReport?.reportID) &&
    !full &&
    !!iouReport?.unheldTotal
  ) {
    total =
      iouReport.unheldTotal - (iouReport?.unheldNonReimbursableTotal ?? 0);
  }
  ...
  if (!isInvoiceReport) {
    ...
    // buildOptimisticNextStep is used in parallel
    // eslint-disable-next-line @typescript-eslint/no-deprecated
    optimisticNextStepDeprecated = buildNextStepNew({
      report: iouReport,
      predictedNextStatus: CONST.REPORT.STATUS_NUM.REIMBURSED,
    });
    optimisticNextStep = buildOptimisticNextStep({
      report: iouReport,
      predictedNextStatus: CONST.REPORT.STATUS_NUM.REIMBURSED,
    });
  }
  ...
  if (iouReport?.policyID) {
    ...
    const optimisticLastPaymentMethod = {
      [iouReport.policyID]: {
        ...(iouReport.type
          ? { [iouReport.type]: { name: usedPaymentOption } }
          : {}),
        ...(isInvoiceReport
          ? { invoice: { name: paymentMethodType, bankAccountID } }
          : {}),
        lastUsed: {
          name:
            prevLastUsedPaymentMethod !== usedPaymentOption &&
            !!prevLastUsedPaymentMethod
              ? prevLastUsedPaymentMethod
              : usedPaymentOption,
        },
      },
  ...
  if (full) {
    ...
    const optimisticTransactionViolations: Array<
      OnyxUpdate<typeof ONYXKEYS.COLLECTION.TRANSACTION_VIOLATIONS>
    > = reportTransactions.map(({ transactionID }) => {
      return {
        onyxMethod: Onyx.METHOD.MERGE,
        key: `${ONYXKEYS.COLLECTION.TRANSACTION_VIOLATIONS}${transactionID}`,
        value: null,
    ...
    const failureTransactionViolations: Array<
      OnyxUpdate<typeof ONYXKEYS.COLLECTION.TRANSACTION_VIOLATIONS>
    > = reportTransactions.map(({ transactionID }) => {
      const violations =
        allTransactionViolations[
  ...
  if (!full) {
    ...
    optimisticHoldReportExpenseActionIDs = JSON.stringify(
      holdReportOnyxData.optimisticHoldReportExpenseActionIDs,
    );
  }
...
function canApproveIOU(
  ...
  const reportTransactions =
    iouTransactions ?? getReportTransactions(iouReport?.reportID);
  const hasOnlyPendingCardOrScanningTransactions =
    reportTransactions.length > 0 &&
    reportTransactions.every(
      (transaction) => isScanning(transaction) || isPending(transaction),
    );
  if (hasOnlyPendingCardOrScanningTransactions) {
    return false;
  }
  const isPayAtEndExpenseReport = isPayAtEndExpenseReportReportUtils(
    iouReport ?? undefined,
    reportTransactions,
  );
...
function canIOUBePaid(
  ...
  const reportNameValuePairs =
    chatReportRNVP ??
    allReportNameValuePairs?.[
      `${ONYXKEYS.COLLECTION.REPORT_NAME_VALUE_PAIRS}${chatReport?.reportID}`
    ];
  ...
  if (isInvoiceReportReportUtils(iouReport)) {
    if (
      isChatReportArchived ||
      iouSettled ||
      isOpenInvoiceReportReportUtils(iouReport)
    ) {
      return false;
    }
    if (
      chatReport?.invoiceReceiver?.type ===
    ...
    return (
      (
        ...
        getPolicy(chatReport?.invoiceReceiver?.policyID)
  ...
  const isAutoReimbursable =
    ...
    CONST.POLICY.REIMBURSEMENT_CHOICES.REIMBURSEMENT_YES
      ? false
      : canBeAutoReimbursed(iouReport, policy);
  const isPayAtEndExpenseReport = isPayAtEndExpenseReportReportUtils(
    iouReport ?? undefined,
    transactions,
  );
  ...
  const isApproved =
    isReportApproved({ report: iouReport }) ||
    isSubmittedWithoutApprovalsEnabled;
  const isClosed = isClosedReportUtil(iouReport);
  const isReportFinished =
    (isApproved || isClosed) && !iouReport?.isWaitingOnBankAccount;
  const isIOU = isIOUReport(iouReport);
  const canShowMarkedAsPaidForNegativeAmount =
    onlyShowPayElsewhere && reimbursableSpend < 0;
  ...
  return (
    ...
    !isPayAtEndExpenseReport &&
    (!isExpenseReport(iouReport) ||
      arePaymentsEnabled(policy as OnyxEntry<OnyxTypes.Policy>))
  );
}
...
function canSubmitReport(
  ...
  const hasAllPendingRTERViolations = allHavePendingRTERViolation(
    transactions,
    allViolations,
    currentUserEmailParam,
    currentUserAccountID,
  ...
  const hasTransactionWithoutRTERViolation =
    hasAnyTransactionWithoutRTERViolation(
      transactions,
      allViolations,
      currentUserEmailParam,
  ...
  const hasOnlyPendingCardOrScanFailTransactions =
    ...
    transactions.every((t) => isPendingCardOrScanningTransaction(t));
...
function getIOUReportActionToApproveOrPay(
  ...
  return Object.values(chatReportActions).find((action) => {
    ...
    const iouReport =
      updatedIouReport?.reportID === action.childReportID
        ? updatedIouReport
        : getReportOrDraftReport(action.childReportID);
    // This will be fixed as part of https://github.com/Expensify/Expensify/issues/507850
    // eslint-disable-next-line @typescript-eslint/no-deprecated
    const policy = getPolicy(iouReport?.policyID);
    // Only show to the actual payer, exclude admins with bank account access
    const shouldShowSettlementButton =
      canIOUBePaid(iouReport, chatReport, policy, undefined) ||
      canApproveIOU(iouReport, policy, reportMetadata);
    return (
 ...
 */
function getReportOriginalCreationTimestamp(
  ...
  const createdAction = Object.values(expenseReportActions ?? {}).find(
    (action) => isCreatedAction(action),
  );
...
function approveMoneyRequest(
  ...
  if (
    ...
    shouldRestrictUserBillableActions(expenseReport.policyID)
  ) {
    Navigation.navigate(
      ROUTES.RESTRICTED_ACTION.getRoute(expenseReport.policyID),
    );
    return;
  }
  ...
  const hasDuplicates = hasDuplicateTransactions(
    currentUserEmailParam,
    currentUserAccountIDParam,
    expenseReport,
    policy,
  ...
  const predictedNextStatus = !nextApproverAccountID
    ? CONST.REPORT.STATUS_NUM.APPROVED
    : CONST.REPORT.STATUS_NUM.SUBMITTED;
  const predictedNextState = !nextApproverAccountID
    ? CONST.REPORT.STATE_NUM.APPROVED
    : CONST.REPORT.STATE_NUM.SUBMITTED;
  const managerID = !nextApproverAccountID
    ...
    if (isDEWPolicy) {
      ...
      failureData.push({
        ...
        value: {
          [optimisticApprovedReportAction.reportActionID]: {
            errors:
              getMicroSecondOnyxErrorWithTranslationKey("iou.error.other"),
          },
        },
      });
    }
  ...
  if (full && hasHeldExpenses) {
    const heldTransactions = getAllHeldTransactionsReportUtils(
      expenseReport.reportID,
    );
  ...
  if (!full && !!chatReport && !!expenseReport) {
    ...
    optimisticHoldActionID = holdReportOnyxData.optimisticHoldActionID;
    optimisticCreatedReportForUnapprovedTransactionsActionID =
      holdReportOnyxData.optimisticCreatedReportForUnapprovedTransactionsActionID;
    optimisticHoldReportExpenseActionIDs = JSON.stringify(
      holdReportOnyxData.optimisticHoldReportExpenseActionIDs,
    );
    optimisticReportActionCopyIDs = JSON.stringify(
      holdReportOnyxData.optimisticReportActionCopyIDs,
    );
  }

  // Remove duplicates violations if we approve the report
  if (hasDuplicates) {
    let transactions = getReportTransactions(expenseReport.reportID).filter(
      (transaction) =>
        isDuplicate(
          transaction,
          currentUserEmailParam,
    ...
    if (!full) {
      transactions = transactions.filter(
        (transaction) => !isOnHold(transaction),
      );
    }
...
function determineIouReportID(
  ...
  const expenseReportActions = getAllReportActions(expenseReport?.reportID);
  const iouCreatedAction = Object.values(iouReportActions).find((action) =>
    isCreatedAction(action),
  );
  const expenseCreatedAction = Object.values(expenseReportActions).find(
    (action) => isCreatedAction(action),
  );

  // The report created later will become the iouReportID of the chat report
  return (iouCreatedAction?.created ?? "") >
    (expenseCreatedAction?.created ?? "")
...
function unapproveExpenseReport(
  ...
  const optimisticUnapprovedReportAction =
    buildOptimisticUnapprovedReportAction(
...
function submitReport(
  ...
  const isCurrentUserManager =
    currentUserAccountIDParam === expenseReport.managerID;
  ...
  const optimisticNextStepDeprecated = isDEWPolicy
    ...
    : // eslint-disable-next-line @typescript-eslint/no-deprecated
      buildNextStepNew({
        report: expenseReport,
        predictedNextStatus: isSubmitAndClosePolicy
          ? CONST.REPORT.STATUS_NUM.CLOSED
          : CONST.REPORT.STATUS_NUM.SUBMITTED,
        policy,
        currentUserAccountIDParam,
      ...
      value: {
        ...
        ...(shouldAddOptimisticSubmitAction
          ? {
              ...
              lastMessageHtml: getReportActionHtml(
                optimisticSubmittedReportAction,
              ),
            }
          : {}),
  ...
  Log.info(
    "[submitReport] Submitting report with approval chain diagnostic",
    false,
    {
      reportID: expenseReport.reportID,
...
function cancelPayment(
  ...
  const stateNum: ValueOf<typeof CONST.REPORT.STATE_NUM> =
    CONST.REPORT.STATE_NUM.APPROVED;
  const statusNum: ValueOf<typeof CONST.REPORT.STATUS_NUM> =
...
function completePaymentOnboarding(
  ...
  if (
    introSelected?.inviteType === CONST.ONBOARDING_INVITE_TYPES.IOU &&
    paymentSelected === CONST.IOU.PAYMENT_SELECTED.BBA
  ) {
    onboardingPurpose = CONST.ONBOARDING_CHOICES.MANAGE_TEAM;
  }

  if (
    introSelected?.inviteType === CONST.ONBOARDING_INVITE_TYPES.INVOICE &&
    paymentSelected !== CONST.IOU.PAYMENT_SELECTED.BBA
  ) {
...
function payMoneyRequest(params: PayMoneyRequestFunctionParams) {
  ...
  if (
    chatReport.policyID &&
    shouldRestrictUserBillableActions(chatReport.policyID)
  ) {
  ...
  const paymentSelected =
    paymentType === CONST.IOU.PAYMENT_TYPE.VBBA
      ? CONST.IOU.PAYMENT_SELECTED.BBA
      : CONST.IOU.PAYMENT_SELECTED.PBA;
  completePaymentOnboarding(paymentSelected, introSelected);

  const recipient = {
    accountID: iouReport?.ownerAccountID ?? CONST.DEFAULT_NUMBER_ID,
  };
  ...
  notifyNewAction(
    !full
      ? (Navigation.getTopmostReportId() ?? iouReport?.reportID)
      : iouReport?.reportID,
    undefined,
    true,
  );
...
function payInvoice({
  ...
  const recipient = {
    accountID: invoiceReport?.ownerAccountID ?? CONST.DEFAULT_NUMBER_ID,
  };
  const {
    onyxData,
    params: {
...
function detachReceipt(
  ...
  > = [
    {
      ...
      value: {
        ...(transaction ?? null),
        errors: getMicroSecondOnyxErrorWithTranslationKey(
          "iou.error.receiptDeleteFailureError",
        ),
        pendingFields: {
          receipt: null,
        },
  ...
  if (
    transactionPolicy &&
    isPaidGroupPolicy(transactionPolicy) &&
    newTransaction
  ) {
    // TODO: Replace getPolicyTagsData (https://github.com/Expensify/App/issues/72721) and getPolicyRecentlyUsedTagsData (https://github.com/Expensify/App/issues/71491) with useOnyx hook
    // eslint-disable-next-line @typescript-eslint/no-deprecated
  ...
  const updatedReportAction = buildOptimisticDetachReceipt(
    expenseReport?.reportID,
    transactionID,
  ...
  API.write(WRITE_COMMANDS.DETACH_RECEIPT, parameters, {
    optimisticData,
    successData,
    failureData,
  });
...
function replaceReceipt({
  ...
  API.write(WRITE_COMMANDS.REPLACE_RECEIPT, parameters, {
    optimisticData,
    successData,
    failureData,
  });
 ...
 */
function getMoneyRequestParticipantsFromReport(
  report: OnyxEntry<OnyxTypes.Report>,
  currentUserAccountID?: number,
): Participant[] {
  // If the report is iou or expense report, we should get the chat report to set participant for request money
  const chatReport = isMoneyRequestReportReportUtils(report)
    ? getReportOrDraftReport(report?.chatReportID)
    : report;
  ...
  if (isPolicyExpenseChat || shouldAddAsReport) {
    ...
    const chatReportOtherParticipants = Object.keys(
      chatReport?.participants ?? {},
    )
      .map(Number)
...
function setMoneyRequestParticipantsFromReport(
  ...
  const participants = getMoneyRequestParticipantsFromReport(
    report,
    currentUserAccountID,
  );
  return Onyx.merge(
...
function setMoneyRequestTaxRate(
  transactionID: string,
  taxCode: string | null,
  isDraft = true,
) {
...
function setMoneyRequestTaxAmount(
  transactionID: string,
  ...
  Onyx.merge(`${ONYXKEYS.NVP_LAST_PAYMENT_METHOD}`, {
    [policyID]: type
      ? {
          ...
          [CONST.LAST_PAYMENT_METHOD.LAST_USED]: {
            name:
              typeof prevPaymentMethod === "string"
                ? prevPaymentMethod
                : (prevPaymentMethod?.lastUsed?.name ?? paymentMethod),
          },
        }
      : paymentMethod,
  });
...
function getIOURequestPolicyID(
  transaction: OnyxEntry<OnyxTypes.Transaction>,
  report: OnyxEntry<OnyxTypes.Report>,
): string | undefined {
  // Workspace sender will exist for invoices
  const workspaceSender = transaction?.participants?.find(
    (participant) => participant.isSender,
  );
...
function setMultipleMoneyRequestParticipantsFromReport(
  transactionIDs: string[],
  reportValue: OnyxEntry<OnyxTypes.Report>,
  currentUserAccountID: number,
) {
  const participants = getMoneyRequestParticipantsFromReport(
    reportValue,
  ...
  const updatedTransactions: Record<
    `${typeof ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${string}`,
    NullishDeep<OnyxTypes.Transaction>
  > = {};
  for (const transactionID of transactionIDs) {
    updatedTransactions[
      `${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${transactionID}`
...
const expenseReportStatusFilterMapping = {
  ...
  [CONST.SEARCH.STATUS.EXPENSE.OUTSTANDING]: (
    ...
    expenseReport?.statusNum === CONST.REPORT.STATUS_NUM.SUBMITTED,
  [CONST.SEARCH.STATUS.EXPENSE.APPROVED]: (
    expenseReport: OnyxEntry<OnyxTypes.Report>,
  ) =>
    expenseReport?.stateNum === CONST.REPORT.STATE_NUM.APPROVED &&
    expenseReport?.statusNum === CONST.REPORT.STATUS_NUM.APPROVED,
  [CONST.SEARCH.STATUS.EXPENSE.PAID]: (
    expenseReport: OnyxEntry<OnyxTypes.Report>,
  ) =>
    (expenseReport?.stateNum ?? 0) >= CONST.REPORT.STATE_NUM.APPROVED &&
    expenseReport?.statusNum === CONST.REPORT.STATUS_NUM.REIMBURSED,
  [CONST.SEARCH.STATUS.EXPENSE.DONE]: (
    expenseReport: OnyxEntry<OnyxTypes.Report>,
  ) =>
    expenseReport?.stateNum === CONST.REPORT.STATE_NUM.APPROVED &&
    expenseReport?.statusNum === CONST.REPORT.STATUS_NUM.CLOSED,
...
function shouldOptimisticallyUpdateSearch(
  ...
  if (Array.isArray(status)) {
    ...
    shouldOptimisticallyUpdateByStatus =
      expenseReportStatusFilterMapping[expenseStatus](iouReport);
  }
  ...
  const submitQueryJSON =
    suggestedSearches[CONST.SEARCH.SEARCH_KEYS.SUBMIT].searchQueryJSON;
  const approveQueryJSON =
    suggestedSearches[CONST.SEARCH.SEARCH_KEYS.APPROVE].searchQueryJSON;
  ...
  const validSearchTypes =
    ...
    (isInvoice &&
      currentSearchQueryJSON.type === CONST.SEARCH.DATA_TYPES.INVOICE) ||
    (iouReport?.type === CONST.REPORT.TYPE.EXPENSE &&
      currentSearchQueryJSON.type === CONST.SEARCH.DATA_TYPES.EXPENSE_REPORT);
  ...
  return (
    ...
    (currentSearchQueryJSON.flatFilters.length === 0 ||
      (submitQueryJSON?.similarSearchHash ===
        currentSearchQueryJSON.similarSearchHash &&
        expenseReportStatusFilterMapping[CONST.SEARCH.STATUS.EXPENSE.DRAFTS](
          iouReport,
        )) ||
      (approveQueryJSON?.similarSearchHash ===
        currentSearchQueryJSON.similarSearchHash &&
        expenseReportStatusFilterMapping[
          CONST.SEARCH.STATUS.EXPENSE.OUTSTANDING
        ](iouReport)) ||
      (unapprovedCashSimilarSearchHash ===
        currentSearchQueryJSON.similarSearchHash &&
        isExpenseReport(iouReport) &&
        (expenseReportStatusFilterMapping[CONST.SEARCH.STATUS.EXPENSE.DRAFTS](
          iouReport,
              ...
              data: {
                [ONYXKEYS.PERSONAL_DETAILS_LIST]: {
                  ...
                  [fromAccountID]: {
                    ...
                    displayName:
                      deprecatedCurrentUserPersonalDetails?.displayName,
...
function dismissRejectUseExplanation() {
  ...
  const optimisticData: Array<
    OnyxUpdate<typeof ONYXKEYS.NVP_DISMISSED_REJECT_USE_EXPLANATION>
  > = [
    {
      onyxMethod: Onyx.METHOD.MERGE,
...
function prepareRejectMoneyRequestData(
  ...
  const isUserOnSearchPage =
    isSearchTopmostFullScreenRoute() && lastRoute?.name === SCREENS.SEARCH.ROOT;
  const isUserOnSearchMoneyRequestReport =
    isSearchTopmostFullScreenRoute() &&
    lastRoute?.name === SCREENS.SEARCH.MONEY_REQUEST_REPORT;
  ...
  const transactionCommentCleanup = (() => {
    ...
    return {
      comment: {
        ...(transaction.comment ?? {}),
        dismissedViolations: isEmptyObject(dismissedViolations)
          ? null
          : dismissedViolations,
      },
    };
  ...
  const optimisticRejectReportAction =
    buildOptimisticRejectReportAction(baseTimestamp);
  const optimisticRejectReportActionComment =
    buildOptimisticRejectReportActionComment(
      comment,
  ...
  if ((!isPolicyDelayedSubmissionEnabled || isIOU) && !shouldUseBulkAction) {
    if (hasMultipleExpenses) {
      ...
      if (isUserOnSearchPage) {
        ...
        urlToNavigateBack =
          lastRouteParams && "backTo" in lastRouteParams
            ? lastRouteParams?.backTo
            : undefined;
      } else {
    ...
    if (existingOpenReport) {
      ...
      const [, createdActionForExpenseReport, iouAction, ,] =
        buildOptimisticMoneyRequestEntities({
      ...
      movedTransactionAction = buildOptimisticMovedTransactionAction(
        childReportID,
        newExpenseReport.reportID,
      );
      createdIOUReportActionID = iouAction.reportActionID;
      expenseMovedReportActionID = movedTransactionAction.reportActionID;
      expenseCreatedReportActionID =
        createdActionForExpenseReport.reportActionID;
      newExpenseReport.parentReportActionID =
        reportPreviewAction.reportActionID;
      optimisticData.push(
        {
          onyxMethod: Onyx.METHOD.MERGE,
      ...
      successData.push(
        ...
        },
        {
          ...
          value: {
            ...
            [iouAction.reportActionID]: { pendingAction: null },
          },
        },
        {
          onyxMethod: Onyx.METHOD.MERGE,
      ...
      failureData.push(
        {
          ...
          value: {
            lastVisibleActionCreated:
              policyExpenseChat?.lastVisibleActionCreated,
          },
        },
        {
          onyxMethod: Onyx.METHOD.SET,
  ...
  if (policyExpenseChat) {
    ...
    if (
      policyExpenseChat.hasOutstandingChildRequest !==
      shouldHaveOutstandingChildRequest
    ) {
      optimisticData.push({
        onyxMethod: Onyx.METHOD.MERGE,
        key: `${ONYXKEYS.COLLECTION.REPORT}${policyExpenseChat.reportID}`,
  ...
  const lastReadTime = DateUtils.subtractMillisecondsFromDateTime(
    optimisticRejectReportAction.created,
    1,
  );
...
function markRejectViolationAsResolved(
  transactionID: string,
  reportID?: string,
) {
  if (!reportID) {
  ...
  const optimisticMarkedAsResolvedReportAction =
    buildOptimisticMarkedAsResolvedReportAction();
...
function initSplitExpense(
  ...
  const { isExpenseSplit } = getOriginalTransactionWithSplitInfo(
    transaction,
    originalTransaction,
  );
  ...
  if (isExpenseSplit) {
    ...
    const splitExpenses = singleTransactionMode
      ...
      : getChildTransactions(transactions, reports, originalTransactionID).map(
          (currentTransaction) => initSplitExpenseItemData(currentTransaction),
        );

    const transactionDetails = singleTransactionMode
      ? getTransactionDetails(transaction)
      : getTransactionDetails(originalTransaction);
    const draftTransaction = buildOptimisticTransaction({
      originalTransactionID,
      transactionParams: {
        splitExpenses,
        splitExpensesTotal: splitExpenses.reduce(
    ...
    if (isSearchTopmostFullScreenRoute()) {
      Navigation.navigate(
        ROUTES.SPLIT_EXPENSE_SEARCH.getRoute(
          reportID,
          originalTransactionID,
          transaction.transactionID,
  ...
  Onyx.set(
    `${ONYXKEYS.COLLECTION.SPLIT_TRANSACTION_DRAFT}${transaction?.transactionID}`,
    draftTransaction,
  );
 ...
 */
function initDraftSplitExpenseDataForEdit(
  ...
  const splitTransactionData = draftTransaction?.comment?.splitExpenses?.find(
  ...
  Navigation.navigate(
    ROUTES.SPLIT_EXPENSE_EDIT.getRoute(
      reportID,
      originalTransactionID,
      splitTransactionData?.transactionID,
      Navigation.getActiveRoute(),
    ),
  );
}
...
function removeSplitExpenseField(
  ...
  const splitExpenses = draftTransaction.comment?.splitExpenses?.filter(
    (item) => item.transactionID !== splitExpenseTransactionID,
  );
...
function updateSplitExpenseField(
  ...
  const splitExpenses = originalTransactionDraft?.comment?.splitExpenses?.map(
    (item) => {
      if (item.transactionID === splitExpenseTransactionID) {
        ...
        return {
          ...
          description: transactionDetails?.comment,
          category: transactionDetails?.category,
          tags: splitExpenseDraftTransaction?.tag
            ? [splitExpenseDraftTransaction?.tag]
            : [],
          created:
            transactionDetails?.created ??
            DateUtils.formatWithUTCTimeZone(
  ...
  Onyx.merge(
    `${ONYXKEYS.COLLECTION.SPLIT_TRANSACTION_DRAFT}${originalTransactionID}`,
    {
      comment: {
        ...
        splitsStartDate: shouldResetDateRange
          ? null
          : originalTransactionDraft?.comment?.splitsStartDate,
        splitsEndDate: shouldResetDateRange
          ? null
          : originalTransactionDraft?.comment?.splitsEndDate,
      },
    },
  );
}
...
function updateSplitExpenseAmountField(
  ...
  const updatedSplitExpenses = draftTransaction.comment?.splitExpenses?.map(
    (splitExpense) => {
  ...
  Onyx.merge(
    `${ONYXKEYS.COLLECTION.SPLIT_TRANSACTION_DRAFT}${draftTransaction?.comment?.originalTransactionID}`,
    {
      comment: {
        splitExpenses: updatedSplitExpenses,
      },
...
function assignReportToMe(
  ...
  const takeControlReportAction = buildOptimisticChangeApproverReportAction(
    accountID,
    accountID,
  );
