...
function buildSnapshotTotalUpdatesForHash(
    snapshotSearches: YourSpendPatchData['snapshotSearches'],
    ...
    const search = snapshotSearches[snapshotKey];
...
function getSnapshotSearchResults(snapshotSearches: YourSpendPatchData['snapshotSearches'], snapshotHash: number | undefined) {
    if (!snapshotHash) {
        return undefined;
    }
    const snapshotKey = `${ONYXKEYS.COLLECTION.SNAPSHOT}${snapshotHash}` as const;
    return snapshotSearches[snapshotKey];
}
...
function getReimbursableTransactionAmountInCurrency(policies: OnyxCollection<Policy>, transaction: Transaction, iouReport: OnyxEntry<Report>, targetCurrency: string): number | null {
    ...
    const policyOutputCurrency = iouReport?.policyID ? policies?.[`${ONYXKEYS.COLLECTION.POLICY}${iouReport.policyID}`]?.outputCurrency : undefined;
...
function getReportReimbursableTotal(
    ...
    for (const reportTransaction of reportTransactions) {
        ...
        const amount = getReimbursableTransactionAmountInCurrency(policies, reportTransaction, iouReport, targetCurrency);
        if (amount === null) {
            return null;
        }
        total += amount;
...
function buildSnapshotDataUpdatesForHash(
    snapshotSearches: YourSpendPatchData['snapshotSearches'],
    snapshotHash: number | undefined,
    transactions: Transaction[],
    enters: boolean,
    ...
    const snapshotKey = `${ONYXKEYS.COLLECTION.SNAPSHOT}${snapshotHash}` as const;
    if (!snapshotSearches[snapshotKey]) {
        return {optimisticData: [], successData: [], failureData: []};
    }
...
function getYourSpendSnapshotTotalUpdates({
    transaction,
    updatedTransaction,
    iouReport,
    currentUserAccountID,
    context = EMPTY_YOUR_SPEND_PATCH_DATA,
}: GetYourSpendSnapshotTotalUpdatesParams): YourSpendSnapshotOnyxData {
    ...
    if (transactionMatchesAwaitingApprovalQuery(iouReport, transaction, currentUserAccountID, paidGroupPolicyIDs)) {
        const approvalQueryJSON = buildSearchQueryJSON(buildAwaitingApprovalQuery(currentUserAccountID, paidGroupPolicyIDs));
        mergeYourSpendSnapshotOnyxData(result, buildSnapshotTotalUpdatesForHash(snapshotSearches, approvalQueryJSON?.hash, diff, currency));
    }

    if (transactionMatchesRepaidLast30DaysQuery(iouReport, transaction, currentUserAccountID)) {
        const paymentQueryJSON = buildSearchQueryJSON(buildRepaidLast30DaysQuery(currentUserAccountID));
        mergeYourSpendSnapshotOnyxData(result, buildSnapshotTotalUpdatesForHash(snapshotSearches, paymentQueryJSON?.hash, diff, currency));
    }
...
function getYourSpendSnapshotSplitUpdates({
    ...
    const approvalQueryJSON = buildSearchQueryJSON(buildAwaitingApprovalQuery(currentUserAccountID, paidGroupPolicyIDs));
    mergeYourSpendSnapshotOnyxData(result, buildSnapshotTotalUpdatesForHash(snapshotSearches, approvalQueryJSON?.hash, reimbursableDiff, currency, reimbursableCountDiff));
    return result;
}
