...
function createTransaction({
  ...
  for (const [index, receiptFile] of files.entries()) {
    const transaction = transactions.find(
      (item) => item.transactionID === receiptFile.transactionID,
    );
    ...
    const submittedCommand =
      iouType === CONST.IOU.TYPE.TRACK && report
    ...
    if (iouType === CONST.IOU.TYPE.TRACK && report) {
      ...
      const existingTransactionDraft = existingTransactionID
        ? allTransactionDrafts?.[existingTransactionID]
        : undefined;
...
function getMoneyRequestParticipantOptions(
  ...
  const selectedParticipants = getMoneyRequestParticipantsFromReport(
    report,
    currentUserAccountID,
  );
...
type InitMoneyRequestParams = {
  ...
  policy?: OnyxEntry<Policy>;
  personalPolicy:
    | Pick<Policy, "id" | "type" | "autoReporting" | "outputCurrency">
    | undefined;
...
function initMoneyRequest({
  ...
  const newTransaction = {
    ...
    ...(defaultParticipants && defaultParticipants.length > 0
      ? { participants: defaultParticipants, participantsAutoAssigned: true }
      : {}),
  };
...
function setMoneyRequestTaxValue(
  transactionID: string,
...
function setMoneyRequestTaxRateValues(
  transactionID: string,
  taxRateValues: TaxRateValues,
  isDraft = true,
) {
  Onyx.merge(
    `${isDraft ? ONYXKEYS.COLLECTION.TRANSACTION_DRAFT : ONYXKEYS.COLLECTION.TRANSACTION}${transactionID}`,
...
function setMoneyRequestCategory(
  ...
  const { categoryTaxCode, categoryTaxAmount, categoryTaxValue } =
    getCategoryTaxDetails(category, transaction, policy);
 ...
 */
function setLastSelectedDistanceRate(
  policy: OnyxEntry<Policy>,
...
function setMoneyRequestDistanceRate(
  ...
  if (!currentTransaction) {
    Log.warn(
      "setMoneyRequestDistanceRate is called without a valid transaction, skipping setting distance rate.",
    );
    return;
  }
...
function setMoneyRequestReceiptState(
  transactionID: string,
  isDraft: boolean,
...
function setMoneyRequestDescription(
  ...
  Onyx.merge(
    `${isDraft ? ONYXKEYS.COLLECTION.TRANSACTION_DRAFT : ONYXKEYS.COLLECTION.TRANSACTION}${transactionID}`,
    { comment: { comment: isDraft ? comment : comment.trim() } },
  );
  setMoneyRequestReceiptState(transactionID, isDraft, shouldStopSmartscan);
}
...
function updateDistanceRateOnExpenseDateChange({
  ...
  if (
    ...
    rateID &&
    isTaxTrackingEnabled(
      isPolicyExpenseChat || isTrackExpense || isExpenseUnreported(transaction),
      effectivePolicy,
      isDistanceRequest(transaction),
    )
  ) {
    const mileageRates = DistanceRequestUtils.getMileageRates(effectivePolicy);
    const distanceUnit = mileageRates[rateID]
      ? DistanceRequestUtils.getDistanceUnit(transaction, mileageRates[rateID])
      : transaction?.comment?.customUnit?.distanceUnit;
    const { taxAmount, taxCode, taxValue } = getDistanceRateTaxUpdates(
      effectivePolicy,
      transaction,
      rateID,
      distanceUnit,
