...
import {createOpenReportMatcher, createUpdateCommentMatcher, resolveCommentDeletionConflicts, resolveDuplicationConflictAction} from './RequestConflictUtils';
...
function openReport(
    ...
    if (isFromDeepLink) {
        ...
        API.paginate(CONST.API_REQUEST_TYPE.WRITE, WRITE_COMMANDS.OPEN_REPORT, parameters, {optimisticData, successData, failureData}, paginationConfig, {
            checkAndFixConflictingRequest: (persistedRequests) => resolveDuplicationConflictAction(persistedRequests, createOpenReportMatcher(reportID)),
        });
    }
}
