...
function addComment(reportID: string, text: string, shouldPlaySound?: boolean) {
    ...
    // If we are sending a comment on an expense report that only has a single transaction thread child report, we need to add the action to the transaction thread instead.
