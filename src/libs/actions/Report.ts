...
function navigateToAndOpenReport(
    ...
    if (isEmptyObject(chat)) {
        ...
        openReport(newChat?.reportID, formatPhoneNumber, '', userLogins, newChat, undefined, undefined, undefined, avatarFile);
    }
 ...
 */
function navigateToAndOpenReportWithAccountIDs(participantAccountIDs: number[], formatPhoneNumber: FormatPhoneNumberType) {
    let newChat: OptimisticChatReport | undefined;
    ...
    if (!chat) {
        ...
        // We want to pass newChat here because if anything is passed in that param (even an existing chat), we will try to create a chat on the server
        openReport(newChat?.reportID, formatPhoneNumber, '', [], newChat, '0', false, participantAccountIDs);
    }
 ...
 */
function navigateToAndOpenChildReport(childReportID: string | undefined, formatPhoneNumber: FormatPhoneNumberType, parentReportAction: Partial<ReportAction> = {}, parentReportID?: string) {
    ...
    if (childReport?.reportID) {
        ...
        if (!childReportID) {
            ...
            openReport(newChat.reportID, formatPhoneNumber, '', participantLogins, newChat, parentReportAction.reportActionID);
        } else {
...
function toggleSubscribeToChildReport(
    ...
    if (childReportID) {
        openReport(childReportID, formatPhoneNumber);
...
function navigateToConciergeChat(
    ...
    if (!conciergeReportID) {
        ...
        onServerDataReady().then(() => {
            ...
            navigateToAndOpenReport([CONST.EMAIL.CONCIERGE], formatPhoneNumber, shouldDismissModal);
        });
...
function buildNewReportOptimisticData(
    ...
    const message = getReportPreviewMessage(optimisticReportData, formatPhoneNumber);
    ...
    const optimisticNextStep = buildNextStep(optimisticReportData, CONST.REPORT.STATUS_NUM.OPEN, formatPhoneNumber);
...
function createNewReport(creatorPersonalDetails: PersonalDetails, formatPhoneNumber: FormatPhoneNumberType, policyID?: string, shouldNotifyNewAction = false) {
 ...
 */
function navigateToConciergeChatAndDeleteReport(reportID: string | undefined, formatPhoneNumber: FormatPhoneNumberType, shouldPopToTop = false, shouldDeleteChildReports = false) {
...
function clearCreateChatError(report: OnyxEntry<Report>, formatPhoneNumber: FormatPhoneNumberType) {
    ...
    navigateToConciergeChatAndDeleteReport(report?.reportID, formatPhoneNumber, undefined, true);
}
...
function showReportActionNotification(reportID: string, reportAction: ReportAction, formatPhoneNumber: FormatPhoneNumberType) {
    ...
    if (reportAction.actionName === CONST.REPORT.ACTIONS.TYPE.MODIFIED_EXPENSE) {
        LocalNotification.showModifiedExpenseNotification(report, reportAction, onClick, formatPhoneNumber);
    } else {
        LocalNotification.showCommentNotification(report, reportAction, onClick, formatPhoneNumber);
    }
...
function openReportFromDeepLink(url: string, formatPhoneNumber: FormatPhoneNumberType) {
    const reportID = getReportIDFromLink(url);
    const isAuthenticated = hasAuthToken();

    if (reportID && !isAuthenticated) {
        // Call the OpenReport command to check in the server if it's a public room. If so, we'll open it as an anonymous user
        openReport(reportID, formatPhoneNumber, '', [], undefined, '0', true);
                    ...
                    Navigation.waitForProtectedRoutes().then(() => {
                        ...
                        const handleDeeplinkNavigation = () => {
                            ...
                            if (reportID && !report) {
                                ...
                                navigateToConciergeChat(formatPhoneNumber, false, () => true);
                                return;
                            }
...
function navigateToMostRecentReport(currentReport: OnyxEntry<Report>, formatPhoneNumber: FormatPhoneNumberType) {
    ...
    if (lastAccessedReportID) {
        ...
        navigateToConciergeChat(formatPhoneNumber, false, () => true, {forceReplace: true});
    }
}
...
function leaveGroupChat(reportID: string, formatPhoneNumber: FormatPhoneNumberType) {
    const report = allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${reportID}`];
    if (!report) {
    ...
    navigateToMostRecentReport(report, formatPhoneNumber);
    API.write(WRITE_COMMANDS.LEAVE_GROUP_CHAT, {reportID}, {optimisticData, successData, failureData});
}

/** Leave a report by setting the state to submitted and closed */
function leaveRoom(reportID: string, formatPhoneNumber: FormatPhoneNumberType, isWorkspaceMemberLeavingWorkspaceRoom = false) {
    const report = allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${reportID}`];
...
function buildInviteToRoomOnyxData(reportID: string, inviteeEmailsToAccountIDs: InvitedEmailsToAccountIDs, formatPhoneNumber: FormatPhoneNumberType) {
    const report = allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${reportID}`];
...
function inviteToRoom(reportID: string, inviteeEmailsToAccountIDs: InvitedEmailsToAccountIDs, formatPhoneNumber: FormatPhoneNumberType) {
...
function inviteToGroupChat(reportID: string, inviteeEmailsToAccountIDs: InvitedEmailsToAccountIDs, formatPhoneNumber: FormatPhoneNumberType) {
 ...
 * @param policyID - The ID of the policy to move the report to
 */
function moveIOUReportToPolicyAndInviteSubmitter(reportID: string, policyID: string, formatPhoneNumber: FormatPhoneNumberType) {
    const iouReport = allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${reportID}`];
    // This will be fixed as part of https://github.com/Expensify/Expensify/issues/507850
    // eslint-disable-next-line deprecation/deprecation
    const policy = getPolicy(policyID);
...
function buildOptimisticChangePolicyData(report: Report, policyID: string, formatPhoneNumber: FormatPhoneNumberType, reportNextStep?: ReportNextStep) {
    const optimisticData: OnyxUpdate[] = [];
    const successData: OnyxUpdate[] = [];
    const failureData: OnyxUpdate[] = [];
    ...
    if (newStatusNum) {
        optimisticData.push({
            onyxMethod: Onyx.METHOD.MERGE,
            key: `${ONYXKEYS.COLLECTION.NEXT_STEP}${reportID}`,
            value: buildNextStep({...report, policyID}, newStatusNum, formatPhoneNumber),
        });
    ...
    const optimisticReportPreviewAction = buildOptimisticReportPreview(policyExpenseChat, report, formatPhoneNumber);
 ...
 */
function changeReportPolicy(report: Report, policyID: string, formatPhoneNumber: FormatPhoneNumberType, reportNextStep?: ReportNextStep) {
 ...
 */
function changeReportPolicyAndInviteSubmitter(report: Report, policyID: string, employeeList: PolicyEmployeeList | undefined, formatPhoneNumber: FormatPhoneNumberType) {
    ...
    } = buildOptimisticChangePolicyData(report, policyID, formatPhoneNumber);
