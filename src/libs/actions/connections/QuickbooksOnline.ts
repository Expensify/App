...
function buildOnyxDataForQuickbooksConfiguration<TSettingName extends keyof Connections['quickbooksOnline']['config']>(
    ...
    oldValue?: Partial<Connections['quickbooksOnline']['config'][TSettingName]>,
) {
    const optimisticData: OnyxUpdate[] = [
        {
            onyxMethod: Onyx.METHOD.MERGE,
...
function updateQuickbooksOnlineReimbursableExpensesAccount<TConnectionName extends ConnectionNameExceptNetSuite, TSettingName extends keyof Connections[TConnectionName]['config']>(
    ...
    oldValue: Partial<Connections[TConnectionName]['config'][TSettingName]>,
) {
    const onyxData = buildOnyxDataForQuickbooksConfiguration(policyID, CONST.QUICKBOOKS_CONFIG.REIMBURSABLE_EXPENSES_ACCOUNT, settingValue, oldValue);
...
function updateQuickbooksOnlineSyncLocations(policyID: string, settingValue: IntegrationEntityMap, oldValue?: IntegrationEntityMap) {
    const onyxData = buildOnyxDataForQuickbooksConfiguration(policyID, CONST.QUICKBOOKS_CONFIG.SYNC_LOCATIONS, settingValue, oldValue);
...
function updateQuickbooksOnlineSyncCustomers(policyID: string, settingValue: IntegrationEntityMap, oldValue?: IntegrationEntityMap) {
    const onyxData = buildOnyxDataForQuickbooksConfiguration(policyID, CONST.QUICKBOOKS_CONFIG.SYNC_CUSTOMERS, settingValue, oldValue);
...
function updateQuickbooksOnlineSyncClasses(policyID: string, settingValue: IntegrationEntityMap, oldValue?: IntegrationEntityMap) {
    const onyxData = buildOnyxDataForQuickbooksConfiguration(policyID, CONST.QUICKBOOKS_CONFIG.SYNC_CLASSES, settingValue, oldValue);
    const parameters: UpdateQuickbooksOnlineGenericTypeParams = {
        policyID,
        settingValue: JSON.stringify(settingValue),
...
function updateQuickbooksOnlineNonReimbursableBillDefaultVendor(policyID: string, settingValue: string, oldValue?: string) {
    const onyxData = buildOnyxDataForQuickbooksConfiguration(policyID, CONST.QUICKBOOKS_CONFIG.NON_REIMBURSABLE_BILL_DEFAULT_VENDOR, settingValue, oldValue);
