	...
	> = [
		{
			...
			value: {
				stageInProgress:
					CONST.POLICY.CONNECTIONS.SYNC_STAGE_NAME
						.RILLET_SYNC_CONNECTION,
...
function prepareRilletOptimisticData<
	TSettingName extends keyof Connections["rillet"]["config"],
>(
	policyID: string,
	settingName: TSettingName,
	settingValue: Partial<Connections["rillet"]["config"][TSettingName]>,
	oldSettingValue: Partial<
		Connections["rillet"]["config"][TSettingName]
	> | null,
) {
...
function prepareRilletCodingOptimisticData<
	TSettingName extends keyof Connections["rillet"]["config"]["coding"],
>(
	policyID: string,
	settingName: TSettingName,
	settingValue: Partial<
		Connections["rillet"]["config"]["coding"][TSettingName]
	>,
	oldSettingValue: Partial<
		Connections["rillet"]["config"]["coding"][TSettingName]
	> | null,
...
function prepareRilletFieldMappingOptimisticData(
	...
	mapping: ValueOf<
		Connections["rillet"]["config"]["coding"]["fieldMappings"]
	>,
	oldMapping: ValueOf<
		Connections["rillet"]["config"]["coding"]["fieldMappings"]
	> | null,
...
function updateRilletSubsidiary(
	policyID: string,
	subsidiaryID: Connections["rillet"]["config"]["subsidiaryID"],
	oldSubsidiaryID?: Connections["rillet"]["config"]["subsidiaryID"],
) {
	const onyxData = prepareRilletOptimisticData(
		policyID,
		CONST.RILLET_CONFIG.SUBSIDIARY_ID,
		subsidiaryID,
		oldSubsidiaryID ?? null,
	);
...
function updateRilletEnableNewCategories(
	...
	write(
		WRITE_COMMANDS.UPDATE_RILLET_ENABLE_NEW_CATEGORIES,
		parameters,
		onyxData,
	);
}
...
function updateRilletSyncTaxRates(
	...
	const onyxData = prepareRilletCodingOptimisticData(
		policyID,
...
function updateRilletFieldMapping(
	...
	const onyxData = prepareRilletFieldMappingOptimisticData(
		policyID,
		fieldID,
		mapping,
		oldMapping ?? null,
