...
import {findLastAccessedReport, getReportIDFromLink, getRouteFromLink, isReportArchivedByID} from '@libs/ReportUtils';
                        ...
                        const handleDeeplinkNavigation = () => {
                            ...
                            const navigateHandler = (reportParam?: OnyxEntry<Report>) => {
                                ...
                                if (reportID && (!report?.reportID || report.errorFields?.notFound)) {
                                    const isReportArchived = (currentReportID?: string) => isReportArchivedByID(archivedReportsIDSet, currentReportID);
