...
function formatSectionsFromSearchTerm(
    ...
    const selectedParticipantsWithoutDetails = selectedOptions.filter((participant) => {
        ...
        const isPartOfSearchTerm = getPersonalDetailSearchTerms(participant, participant.accountID === currentUserAccountID && includeCurrentUserSearchTerm)
            .join(' ')
            .toLowerCase()
...
function getPersonalDetailSearchTerms(item: Partial<OptionData>, includeCurrentUser = false) {
    const currentUserSearchTerm = includeCurrentUser ? [translateLocal('common.you'), translateLocal('common.me')] : [];
    return [item.participantsList?.[0]?.displayName ?? item?.displayName ?? '', item.login ?? '', item.login?.replace(CONST.EMAIL_SEARCH_REGEX, '') ?? '', ...currentUserSearchTerm];
}
