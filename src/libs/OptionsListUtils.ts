...
function formatSectionsFromSearchTerm(
    ...
    const selectedParticipantsWithoutDetails = tokenizedSearch(selectedOptions, cleanSearchTerm, (participant) =>
        getPersonalDetailSearchTerms(participant).map((term) => term.toLowerCase()),
    ).filter((participant) => {
        const accountID = participant.accountID ?? null;
        const isReportInRecentReports = filteredRecentReports.some((report) => report.accountID === accountID);
        const isReportInWorkspaceChats = filteredWorkspaceChats.some((report) => report.accountID === accountID);
        const isReportInPersonalDetails = filteredPersonalDetails.some((personalDetail) => personalDetail.accountID === accountID);

        return !isReportInRecentReports && !isReportInWorkspaceChats && !isReportInPersonalDetails;
    });
