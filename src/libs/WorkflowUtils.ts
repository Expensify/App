...
function convertPolicyEmployeesToApprovalWorkflows({policy, personalDetails, firstApprover, localeCompare, currentUserLogin}: PolicyConversionParams): PolicyConversionResult {
    ...
    for (const employee of Object.values(employees)) {
        ...
        // Skip if the effective approver doesn't exist in the employee list
        if (!employees[effectiveSubmitsTo]) {
            continue;
        }

        if (!approvalWorkflows[effectiveSubmitsTo]) {
            const approvers = calculateApprovers({employees, firstEmail: effectiveSubmitsTo, personalDetailsByEmail});
            // Filter Expensify team members from the approver chain
            const filteredApprovers = shouldFilterExpensifyTeam ? approvers.filter((approver) => !isExpensifyTeam(approver.email)) : approvers;
