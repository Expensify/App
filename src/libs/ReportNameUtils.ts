...
function computeReportNameBasedOnReportAction(
    ...
    if (isActionOfType(parentReportAction, CONST.REPORT.ACTIONS.TYPE.UNREPORTED_TRANSACTION)) {
        return Parser.htmlToText(getUnreportedTransactionMessage(translate, parentReportAction, formatPhoneNumber));
    }

    if (isActionOfType(parentReportAction, CONST.REPORT.ACTIONS.TYPE.MOVED_TRANSACTION)) {
        return Parser.htmlToText(getMovedTransactionMessage(translate, parentReportAction, formatPhoneNumber));
    }
...
function computeReportName({
    ...
    if (isInvoiceRoom(report)) {
        ...
        formattedName = getInvoicesChatName({report, receiverPolicy: invoiceReceiverPolicy, personalDetails: personalDetailsList, currentUserAccountID, policy, formatPhoneNumber});
    }
