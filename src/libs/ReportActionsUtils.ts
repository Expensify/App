...
function getPaymentMessageWithExpectedDate(translate: LocalizedTranslate, preferredLocale: Locale, paymentMessage: string, expectedDate: string | undefined): string {
    if (!expectedDate) {
        return paymentMessage;
    }

    // A due date is a calendar day, so it renders UTC-anchored, the way the expense date beside it does.
    const formattedExpectedDate = DateUtils.formatInUTCToMedium(expectedDate, preferredLocale);
