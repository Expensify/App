...
function reauthenticate(command = ''): Promise<void> {
    ...
    const postReauthenticate = (response: Response) => {
    ...
    // OldDot and NewDot share the same authToken
    if (NativeModules.HybridAppModule) {
        return NativeModules.HybridAppModule.reauthenticate()
            .then(postReauthenticate)
            .catch((error) => {
                throw new Error('Unable to retry Authenticate request');
            });
    }
