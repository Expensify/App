...
function getTranslatedPhrase<TKey extends TranslationPaths>(
    ...
    if (translatedPhrase) {
        if (typeof translatedPhrase === 'function') {
            const result = translatedPhrase(...phraseParameters) as string | PluralFormPhrase;

            if (typeof result === 'object' && 'other' in result) {
                // Handle plural form
                const count = phraseParameters[0] as number;
                const pluralForm = new Intl.PluralRules(language, {type: 'ordinal'}).select(count);
                const pluralFormPhrase = result[pluralForm];

                if (!pluralFormPhrase) {
                    Log.alert(`Plural form ${pluralForm} is not defined for ${phraseKey}`);
                    return result.other;
                }

                // Store the translated value in the cache
                cacheForLocale?.set(phraseKey, pluralFormPhrase);
                return pluralFormPhrase;
            }
        ...
        if (typeof translatedPhrase !== 'string') {
            Log.alert(`Invalid translation for ${phraseKey}`);
            return null;
        }
