import type {
  LocaleContextProps,
  LocalizedTranslate,
} from "@components/LocaleContextProvider";
...
import {
  getAddAgentRuleMessage,
  getDeleteAgentRuleMessage,
  getUpdateAgentRuleMessage,
} from "./AgentRuleChangeLogUtils";
...
/** @deprecated Use sortedReportActionsData from ONYXKEYS.DERIVED.RAM_ONLY_SORTED_REPORT_ACTIONS instead. Will be removed once all flows are migrated. */
const deprecatedCachedOneTransactionThreadReportIDs: Record<
  string,
  string | undefined
> = {};
/** @deprecated Use sortedReportActionsData from ONYXKEYS.DERIVED.RAM_ONLY_SORTED_REPORT_ACTIONS instead. Will be removed once all flows are migrated. */
...
Onyx.connectWithoutView({
  ...
  callback: (actions) => {
    ...
    for (const reportActions of Object.entries(deprecatedAllReportActions)) {
      ...
      let sortedReportActions = getSortedReportActions(
        withDEWRoutedActionsArray(reportActionsArray),
        true,
      );
      // eslint-disable-next-line @typescript-eslint/no-deprecated
      ...
      const transactionThreadReportID = getOneTransactionThreadReportID(
        report,
        chatReport,
        actions[reportActions[0]],
        getIsOffline(),
      );
      // eslint-disable-next-line @typescript-eslint/no-deprecated
      deprecatedCachedOneTransactionThreadReportIDs[reportID] =
      ...
      if (transactionThreadReportID) {
        ...
        const isSelfDMReport =
          report?.chatType === CONST.REPORT.CHAT_TYPE.SELF_DM;
        sortedReportActions = getCombinedReportActions(
          sortedReportActions,
          transactionThreadReportID,
          transactionThreadReportActionsArray,
          isSelfDMReport,
        );
      }
...
function getLastActorDisplayName(
  ...
  return lastActorDetails.accountID !== currentUserAccountID
    ? // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
      lastActorDetails.firstName ||
        temporaryGetDisplayNameOrDefault({
          passedPersonalDetails: lastActorDetails,
          translate,
...
function shouldShowLastActorDisplayName(
  ...
  if (
    ...
    (isDM(report) &&
      lastActorDetails.accountID !== currentUserAccountIDParam) ||
    lastActionName === CONST.REPORT.ACTIONS.TYPE.IOU
  ) {
    return false;
  }

  const lastActorDisplayName = getLastActorDisplayName(
    lastActorDetails,
...
function getLatestVisibleMoneyRequestAction(
  ...
  return sortedReportActions.find(
    (
      reportAction,
    ): reportAction is ReportAction<typeof CONST.REPORT.ACTIONS.TYPE.IOU> =>
...
function getExpenseReportPreviewText(
  ...
  const originalMessage = moneyRequestAction
    ? getOriginalMessage(moneyRequestAction)
    : undefined;
  const linkedTransaction = transactions.find(
    (transaction) =>
      transaction.transactionID === originalMessage?.IOUTransactionID,
  );
  const amount = linkedTransaction
    ? getTransactionAmount(linkedTransaction, true)
    : originalMessage?.amount;
  const currency = linkedTransaction
    ? getTransactionCurrency(linkedTransaction)
    : (originalMessage?.currency ?? report?.currency);
  ...
  const description = linkedTransaction
    ? getDescription(linkedTransaction)
    : "";
  // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
  const comment = Parser.htmlToText(
    description || originalMessage?.comment || "",
  ).trim();

  return formatReportLastMessageText(
    translate("iou.expenseAmount", formattedAmount, comment || undefined),
  );
}
...
function resolveLastActionContext(
  ...
  const canUserPerformWrite = canUserPerformWriteAction(
    report,
    isReportArchived,
  );
  // eslint-disable-next-line @typescript-eslint/no-deprecated
  const resolvedOneTransactionThreadReportID =
  ...
  if (isInviteOrRemovedAction(lastAction)) {
    ...
    lastActionReport = lastActionOriginalMessage?.reportID
      ? getReportOrDraftReport(String(lastActionOriginalMessage.reportID))
      : undefined;
  }
  return {
    lastAction,
    lastActionReport,
    movedFromReport: getReportOrDraftReport(
      getMovedReportID(lastAction, CONST.REPORT.MOVE_TYPE.FROM),
    ),
    movedToReport: getReportOrDraftReport(
      getMovedReportID(lastAction, CONST.REPORT.MOVE_TYPE.TO),
    ),
  };
}
...
function getLastActorDisplayNameFromLastVisibleActions(
  ...
  if (lastReportAction) {
    ...
    const lastActorAccountID =
      getReportActionActorAccountID(lastReportAction, undefined, undefined) ||
      report?.lastActorAccountID;
    let actorDetails: Partial<PersonalDetails> | null = lastActorAccountID
      ? (personalDetails?.[lastActorAccountID] ?? null)
      : null;
    ...
    if (actorDetails) {
      return getLastActorDisplayName(
        actorDetails,
        currentUserAccountIDParam,
        translate,
      );
...
function getLastMessageTextForReport({
  ...
  if (
    transactionThreadReportID &&
    lastReportAction &&
    isMoneyRequestAction(lastReportAction)
  ) {
    const actionType = getOriginalMessage(lastReportAction)?.type ?? "";
    const isSelfDMReport = reportUtilsIsSelfDM(report);
    if (
      actionType === CONST.IOU.REPORT_ACTION_TYPE.CREATE ||
      (!isSelfDMReport && actionType === CONST.IOU.REPORT_ACTION_TYPE.TRACK)
    ) {
      const parentLastAction = getLastVisibleAction(
        reportID,
        canUserPerformWrite,
        {},
        undefined,
      ...
      if (parentLastAction && isMoneyRequestAction(parentLastAction)) {
        const parentActionType =
          getOriginalMessage(parentLastAction)?.type ?? "";
        if (
          parentActionType === CONST.IOU.REPORT_ACTION_TYPE.CREATE ||
          (!isSelfDMReport &&
            parentActionType === CONST.IOU.REPORT_ACTION_TYPE.TRACK)
        ) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-deprecated
  const lastOriginalReportAction =
    lastOriginalAction ??
    (reportID ? deprecatedLastReportActions[reportID] : undefined);
  let lastMessageTextFromReport = "";

  if (isArchivedNonExpenseReport(report, isReportArchived)) {
    const archiveReason =
      // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
      (isClosedAction(lastOriginalReportAction) &&
        getOriginalMessage(lastOriginalReportAction)?.reason) ||
    ...
    lastMessageTextFromReport = formatReportLastMessageText(
      Parser.htmlToText(properSchemaForMoneyRequestMessage),
    );
  } else if (isReportPreviewAction(lastReportAction)) {
    const iouReport = getReportOrDraftReport(
      getIOUReportIDFromReportActionPreview(lastReportAction),
    );
    const iouReportID = iouReport?.reportID;
    const reportCache = iouReportID
      ? visibleReportActionsDataParam?.[iouReportID]
      : undefined;
    const visibleReportActionsForIOUReport =
      reportCache && Object.keys(reportCache).length > 0
        ? visibleReportActionsDataParam
        : undefined;
    const iouReportActions = iouReportID
      ? sortedActions?.[iouReportID]
      : undefined;
    ...
    const lastIOUMoneyReportAction =
      iouReportID && iouReportActions
        ? getLatestVisibleMoneyRequestAction(
            ...
            canPerformWrite,
            iouReportActions,
            visibleReportActionsForIOUReport,
          )
        : undefined;
    ...
    if (reportUtilsIsPolicyExpenseChat(report) && !isEmptyObject(iouReport)) {
      const reportName =
        reportAttributesDerived?.[iouReport.reportID]?.reportName ?? "";
      ...
      lastMessageTextFromReport = formatReportLastMessageText(
        Parser.htmlToText(reportPreviewMessage),
      );
    }
  } else if (isReimbursementQueuedAction(lastReportAction)) {
    lastMessageTextFromReport = getReimbursementQueuedActionMessage({
    ...
    isDeletedParentAction(lastReportAction) &&
    reportUtilsIsChatReport(report)
  ) {
    lastMessageTextFromReport =
      getDeletedParentActionMessageForChatReport(lastReportAction);
  } else if (
    isPendingRemove(lastReportAction) &&
    report?.reportID &&
    isThreadParentMessage(lastReportAction, report.reportID)
  ) {
    ...
    lastMessageTextFromReport = getMarkedReimbursedMessage(
      translate,
      lastReportAction,
    );
  } else if (
    isActionOfType(lastReportAction, CONST.REPORT.ACTIONS.TYPE.REIMBURSED)
  ) {
    ...
    const properSchemaForModifiedExpenseMessage = Parser.htmlToText(
      properSchemaForModifiedExpenseMessageWithHTML,
    );
    lastMessageTextFromReport = formatReportLastMessageText(
      properSchemaForModifiedExpenseMessage,
      true,
    );
  } else if (isMovedTransactionAction(lastReportAction)) {
    const { fromReportID, toReportID, displayReportID } =
      parseMovedTransactionReportIDs(lastReportAction);
    lastMessageTextFromReport = Parser.htmlToText(
      getMovedTransactionMessage({
        translate,
        fromReportID,
        toReportID,
        derivedReportName: displayReportID
          ? reportAttributesDerived?.[displayReportID]?.reportName
          : undefined,
      }),
    );
  } else if (isTaskAction(lastReportAction)) {
    lastMessageTextFromReport = formatReportLastMessageText(
      getTaskReportActionMessage(translate, lastReportAction).text,
    );
  } else if (isCreatedTaskReportAction(lastReportAction)) {
    lastMessageTextFromReport = getTaskCreatedMessage(
      translate,
      lastReportAction,
      getReportOrDraftReport(lastReportAction?.childReportID),
    );
  } else if (
    ...
    const wasSubmittedViaHarvesting = !isMarkAsClosedAction(lastReportAction)
      ? (getOriginalMessage(lastReportAction)?.harvesting ?? false)
      : false;
    ...
    if (wasSubmittedViaHarvesting) {
      lastMessageTextFromReport = Parser.htmlToText(
        translate("iou.automaticallySubmitted"),
      );
    } else if (
      hasPendingDEWSubmit(reportMetadata, isDEWPolicy) &&
      isPendingAdd
    ) {
      ...
      lastMessageTextFromReport = shouldShowMarkAsDone({
        ...
        ? translate(
            "iou.markedAsDone",
            getOriginalMessage(lastReportAction)?.message,
          )
        : translate(
            "iou.submitted",
            getOriginalMessage(lastReportAction)?.message,
    ...
    isActionOfType(lastReportAction, CONST.REPORT.ACTIONS.TYPE.APPROVED)
  ) {
    ...
    if (automaticAction) {
      lastMessageTextFromReport = Parser.htmlToText(
        translate("iou.automaticallyApproved"),
      );
    } else if (
      hasPendingDEWApprove(reportMetadata, isDEWPolicy) &&
      isPendingAdd
    ) {
    ...
    isDynamicExternalWorkflowSubmitFailedAction(lastReportAction) ||
    isDynamicExternalWorkflowApproveFailedAction(lastReportAction)
  ) {
    lastMessageTextFromReport =
      getOriginalMessage(lastReportAction)?.message ??
      translate("iou.error.genericCreateFailureMessage");
  } else if (isUnapprovedAction(lastReportAction)) {
    lastMessageTextFromReport = translate("iou.unapproved");
  } else if (
    isActionOfType(lastReportAction, CONST.REPORT.ACTIONS.TYPE.FORWARDED)
  ) {
    const { automaticAction } = getOriginalMessage(lastReportAction) ?? {};
    if (automaticAction) {
      lastMessageTextFromReport = Parser.htmlToText(
        translate("iou.automaticallyForwarded"),
      );
    } else {
      lastMessageTextFromReport = getForwardedReportActionMessage(
        lastReportAction,
        translate,
      );
    }
  } else if (
    lastReportAction?.actionName === CONST.REPORT.ACTIONS.TYPE.REJECTED
  ) {
    ...
    lastMessageTextFromReport = Parser.htmlToText(
      translate("workspaceActions.forcedCorporateUpgrade"),
    );
  } else if (
    lastReportAction?.actionName ===
    ...
    lastMessageTextFromReport = translate(
      "workspaceActions.downgradedWorkspace",
    );
  } else if (
    lastReportAction?.actionName ===
    ...
    lastMessageTextFromReport = getExportIntegrationLastMessageText(
      translate,
      lastReportAction,
      integrationName,
    );
    ...
    const iouAction = getReportAction(
      report?.parentReportID,
      report?.parentReportActionID,
    );
    const missingFields = getOriginalMessage(lastReportAction)?.missingFields;
    lastMessageTextFromReport = translate("violations.smartscanFailed", {
      canEdit: wasActionTakenByCurrentUser(iouAction),
      missingFields,
    });
  } else if (
    lastReportAction?.actionName &&
    isOldDotReportAction(lastReportAction)
  ) {
    lastMessageTextFromReport = getMessageOfOldDotReportAction(
      translate,
      lastReportAction,
      false,
    );
    ...
    lastMessageTextFromReport = getJoinRequestMessage(
      ...
      lastReportAction,
      getPersonalDetailsByID(
        getOriginalMessage(lastReportAction)?.accountID,
        personalDetails,
      ),
    );
  } else if (
    ...
    lastMessageTextFromReport = Parser.htmlToText(
      getUpdateRoomDescriptionMessage(translate, lastReportAction),
    );
  } else if (
    isActionOfType(
      lastReportAction,
    ...
    lastMessageTextFromReport = getRoomAvatarUpdatedMessage(
      translate,
      lastReportAction,
    );
  } else if (
    isActionOfType(lastReportAction, CONST.REPORT.ACTIONS.TYPE.RETRACTED)
  ) {
    lastMessageTextFromReport = translate("iou.retracted");
  } else if (
    isActionOfType(lastReportAction, CONST.REPORT.ACTIONS.TYPE.REOPENED)
  ) {
    lastMessageTextFromReport = translate("iou.reopened");
  } else if (
    isActionOfType(lastReportAction, CONST.REPORT.ACTIONS.TYPE.CHANGE_POLICY)
  ) {
    lastMessageTextFromReport = getPolicyChangeMessage(
      translate,
      lastReportAction,
    );
  } else if (
    isActionOfType(lastReportAction, CONST.REPORT.ACTIONS.TYPE.TRAVEL_UPDATE)
  ) {
    lastMessageTextFromReport = getTravelUpdateMessage(
      translate,
      lastReportAction,
    );
  } else if (isInviteOrRemovedAction(lastReportAction)) {
    lastMessageTextFromReport = getRoomChangeLogMessage(
      translate,
      lastReportAction,
    );
  } else if (isRenamedAction(lastReportAction)) {
    lastMessageTextFromReport = getRenamedAction(
      translate,
      lastReportAction,
    ...
    lastMessageTextFromReport = getDeletedTransactionMessage(
      translate,
      lastReportAction,
      convertToDisplayString,
    );
    ...
    lastMessageTextFromReport = Parser.htmlToText(
      getChangedApproverActionMessage(translate, lastReportAction),
    );
  } else if (isMovedAction(lastReportAction)) {
    lastMessageTextFromReport = Parser.htmlToText(
      getMovedActionMessage(translate, lastReportAction, report),
    );
  } else if (
    isActionOfType(
      lastReportAction,
    ...
    lastMessageTextFromReport = Parser.htmlToText(
      getUnreportedTransactionMessage({
        translate,
        fromReportID,
        derivedReportName: fromReportID
          ? reportAttributesDerived?.[fromReportID]?.reportName
          : undefined,
      }),
    );
  } else if (isActionableMentionWhisper(lastReportAction)) {
    const targetAccountIDs =
      getOriginalMessage(lastReportAction)?.inviteeAccountIDs;
    lastMessageTextFromReport = Parser.htmlToText(
      getActionableMentionWhisperMessage(
        translate,
        lastReportAction,
    ...
    lastMessageTextFromReport = getDynamicExternalWorkflowRoutedMessage(
      lastReportAction,
      translate,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getPolicyChangeLogMaxExpenseAmountMessage(
      translate,
      lastReportAction,
      convertToDisplayString,
    );
  ...
  if (
    ...
    lastMessageTextFromReport = getPolicyChangeLogMaxExpenseAgeMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getUpdateACHAccountMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getInvoiceCompanyNameUpdateMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    isActionOfType(
      lastReportAction,
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_INVOICE_COMPANY_WEBSITE,
    )
  ) {
    lastMessageTextFromReport = getInvoiceCompanyWebsiteUpdateMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    isActionOfType(
      lastReportAction,
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_AUTO_PAY_APPROVED_REPORTS_ENABLED,
    )
  ) {
    lastMessageTextFromReport = getAutoPayApprovedReportsEnabledMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    isActionOfType(
      lastReportAction,
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_REQUIRE_COMPANY_CARDS_ENABLED,
    )
  ) {
    lastMessageTextFromReport = getRequireCompanyCardsEnabledMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getRequiresCategoryMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getRequiresTagMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    isActionOfType(
      lastReportAction,
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_GLOBAL_REIMBURSEMENTS_FX_PREFERENCE,
    )
  ) {
    lastMessageTextFromReport = getCurrencyConversionFeeMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getUpdatedAutoHarvestingMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getAutoReimbursementMessage(
      translate,
      lastReportAction,
      convertToDisplayString,
    );
  ...
  if (
    ...
    lastMessageTextFromReport = getCategoryTaxRateMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getMccGroupCategoryMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getWorkspaceCustomUnitRateUpdatedMessage(
      translate,
      dateFnsLocale,
      lastReportAction,
    );
  ...
  if (
    lastReportAction?.actionName &&
    isCategoryModificationAction(lastReportAction.actionName)
  ) {
    lastMessageTextFromReport = getWorkspaceCategoryUpdateMessage(
      translate,
      lastReportAction,
      policy,
    );
  ...
  if (
    ...
    lastMessageTextFromReport = getWorkspaceTaxUpdateMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getCustomTaxNameUpdateMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getCurrencyDefaultTaxUpdateMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    isActionOfType(
      lastReportAction,
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_FOREIGN_CURRENCY_DEFAULT_TAX,
    )
  ) {
    lastMessageTextFromReport = getForeignCurrencyDefaultTaxUpdateMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getAddedCardFeedMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getRemovedCardFeedMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getRenamedCardFeedMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getAssignedCompanyCardMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getUnassignedCompanyCardMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getUpdatedCardFeedLiabilityMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    isActionOfType(
      lastReportAction,
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_CARD_FEED_STATEMENT_PERIOD,
    )
  ) {
    lastMessageTextFromReport = getUpdatedCardFeedStatementPeriodMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getWorkspaceFeatureEnabledMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getAddAgentRuleMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getUpdateAgentRuleMessage(
      translate,
      lastReportAction,
    );
  }
  if (
    ...
    lastMessageTextFromReport = getDeleteAgentRuleMessage(
      translate,
      lastReportAction,
    );
  }
  if (isPolicyCopyReportAction(lastReportAction)) {
    lastMessageTextFromReport = Parser.htmlToText(
      getPolicyChangeLogCopyMessage(translate, lastReportAction),
    );
  }
  ...
  if (
    reportID &&
    !isReportArchived &&
    report.lastActionType === CONST.REPORT.ACTIONS.TYPE.CLOSED
  ) {
    return (
      lastMessageTextFromReport ||
      (getReportLastMessage(reportID, isReportArchived, undefined)
        .lastMessageText ??
        "")
    );
  }
  ...
  if (
    reportID &&
    !lastMessageTextFromReport &&
    isPendingRemove(lastOriginalReportAction)
  ) {
    lastMessageTextFromReport = getReportActionMessageText(lastReportAction);
  }
  ...
  if (
    reportID &&
    !lastMessageTextFromReport &&
    isAddCommentAction(lastReportAction)
  ) {
  ...
  if (
    reportID &&
    !lastMessageTextFromReport &&
    reportUtilsIsMoneyRequestReport(report)
  ) {
    const transactions = getReportTransactions(reportID);
    const scanningTransactions = transactions.filter((transaction) =>
      isScanning(transaction),
    );

    if (scanningTransactions.length > 0) {
      lastMessageTextFromReport = translate("iou.receiptScanning", {
      ...
      const latestVisibleMoneyRequestAction =
        getLatestVisibleMoneyRequestAction(
          reportID,
          canUserPerformWrite,
      ...
      if (isExpenseReport(report) && latestVisibleMoneyRequestAction) {
        lastMessageTextFromReport = getExpenseReportPreviewText(
          report,
          latestVisibleMoneyRequestAction,
          translate,
          transactions,
          convertToDisplayString,
        );
...
function getWelcomeMessage(params: WelcomeMessageParams): WelcomeMessage {
  ...
  if (isPolicyExpenseChat(report)) {
    if (policy?.description) {
      ...
      welcomeMessage.messageText = Parser.htmlToText(
      ...
      welcomeMessage.messageHtml = translate(
        "reportActionsView.beginningOfChatHistoryPolicyExpenseChatTrack",
      );
      welcomeMessage.messageText = Parser.htmlToText(
        welcomeMessage.messageHtml,
      );
  ...
  if (isSelfDM(report)) {
    welcomeMessage.messageText = translate(
      "reportActionsView.beginningOfChatHistorySelfDM",
    );
    return welcomeMessage;
  }
  ...
  if (isSystemChatUtil(report)) {
    welcomeMessage.messageText = translate(
      "reportActionsView.beginningOfChatHistorySystemDM",
    );
    return welcomeMessage;
  }
  ...
  const userTags = displayNamesWithTooltips.map(
    ({ displayName, accountID }) =>
      `<user-details accountid="${accountID}">${displayName ?? ""}</user-details>`,
  );
  const usersHtml = formatList(userTags);

  let messageHtml = translate(
...
function getRoomWelcomeMessage({
  ...
  const welcomeMessage: WelcomeMessage = {};
  const workspaceName = getPolicyName({
    report,
    unavailableTranslation: translate("workspace.common.unavailable"),
  });
  ...
  if (isReportArchived) {
    welcomeMessage.messageHtml = translate(
      "reportActionsView.beginningOfArchivedRoom",
      reportName,
      reportDetailsLink,
    );
  } else if (isDomainRoom(report)) {
    welcomeMessage.messageHtml = translate(
      "reportActionsView.beginningOfChatHistoryDomainRoom",
      report?.reportName ?? "",
    );
  } else if (isAdminRoom(report)) {
    welcomeMessage.messageHtml = translate(
      "reportActionsView.beginningOfChatHistoryAdminRoom",
      workspaceName,
    );
  } else if (isAnnounceRoom(report)) {
    welcomeMessage.messageHtml = translate(
      "reportActionsView.beginningOfChatHistoryAnnounceRoom",
      workspaceName,
    );
  } else if (isInvoiceRoom(report)) {
    const payer =
      report?.invoiceReceiver?.type ===
      CONST.REPORT.INVOICE_RECEIVER_TYPE.INDIVIDUAL
        ? getDisplayNameForParticipant({
            accountID: report?.invoiceReceiver?.accountID,
            formatPhoneNumber,
            translate,
          })
        : invoiceReceiverPolicy?.name;
    const receiver = getPolicyName({
      report,
      unavailableTranslation: translate("workspace.common.unavailable"),
    });
    welcomeMessage.messageHtml = translate(
      "reportActionsView.beginningOfChatHistoryInvoiceRoom",
      payer ?? "",
      receiver,
    );
  } else {
    ...
    welcomeMessage.messageHtml = translate(
      "reportActionsView.beginningOfChatHistoryUserRoom",
      reportName,
      reportDetailsLink,
    );
  }
...
function getReportAlternateText({
  ...
  participantPersonalDetailListExcludeCurrentUser:
    participantPersonalDetailListExcludeCurrentUserParam,
  policyTags,
  isReportArchived,
  privateIsArchived,
  conciergeReportID,
  ...
  const isAllowedToComment = canUserPerformWriteAction(
    report,
    isReportArchived,
  );
  const isExpense = isExpenseReport(report);
  const reportMetadata =
    reportMetadataParam ?? getReportMetadata(report.reportID);
  // eslint-disable-next-line @typescript-eslint/no-deprecated
  const resolvedOneTransactionThreadReportID =
    oneTransactionThreadReportID ??
    deprecatedCachedOneTransactionThreadReportIDs[report.reportID];
  const getParticipantPersonalDetailListExcludeCurrentUser = () => {
    ...
    const participantAccountIDsExcludeCurrentUser =
      excludeParticipantsForDisplay(
  ...
  // eslint-disable-next-line @typescript-eslint/prefer-nullish-coalescing
  const lastActorAccountID =
    getReportActionActorAccountID(lastAction, undefined, undefined) ||
    report.lastActorAccountID;
  // If the last actor's details are not currently saved in Onyx Collection,
  // then try to get that from the last report action if that action is valid
  // to get data from.
  let lastActorDetails: Partial<PersonalDetails> | null = lastActorAccountID
    ? (personalDetails?.[lastActorAccountID] ?? null)
    : null;
  ...
  if (
    ...
    !!getDelegateSubmitMessage(translate, lastAction, currentUserLogin)
  ) {
    lastMessageText = Parser.htmlToText(
      getDelegateSubmitMessage(translate, lastAction, currentUserLogin),
    );
  }

  const isGroupChat =
    isGroupChatUtil(report) || isDeprecatedGroupDM(report, isReportArchived);

  const isThreadMessage =
    isThread(report) &&
    lastAction?.actionName === CONST.REPORT.ACTIONS.TYPE.ADD_COMMENT &&
    lastAction?.pendingAction !== CONST.RED_BRICK_ROAD_PENDING_ACTION.DELETE;
  if (
    (isChatRoomReport ||
      isPolicyExpenseChatReport ||
      isThreadReport ||
      isTaskReportFlag ||
      isThreadMessage ||
      isGroupChat) &&
    ...
    if (isRenamedAction(lastAction)) {
      alternateText = getRenamedAction(
        translate,
        lastAction,
        isExpense,
        lastActorDisplayName,
      ...
      alternateText = formatReportLastMessageText(
        getTaskReportActionMessage(translate, lastAction).text,
      );
    } else if (
      ...
      alternateText = actionMessage
        ? `${lastActorDisplayName}: ${actionMessage}`
        : "";
    } else if (
      lastAction?.actionName ===
      CONST.REPORT.ACTIONS.TYPE.ROOM_CHANGE_LOG.LEAVE_ROOM
      ...
      alternateText = translate(
        "report.actions.type.leftTheChatWithName",
        lastActorDisplayName,
      );
      ...
      actorDisplayName = actorDetails
        ? getLastActorDisplayName(actorDetails, currentUserAccountID, translate)
        : undefined;
      const lastActionOriginalMessage = lastAction?.actionName
        ? getOriginalMessage(lastAction)
        : null;
      const targetAccountIDs =
        lastActionOriginalMessage?.targetAccountIDs ?? [];
      const targetAccountIDsLength =
        targetAccountIDs.length !== 0
          ? targetAccountIDs.length
          : (report.lastMessageHtml?.match(
              /<mention-user[^>]*><\/mention-user>/g,
            )?.length ?? 0);
      const verb =
        lastActionName ===
      ...
      const users = translate(
        targetAccountIDsLength > 1 ? "common.members" : "common.member",
      )?.toLocaleLowerCase();
      alternateText = formatReportLastMessageText(
        `${actorDisplayName ?? lastActorDisplayName}: ${verb} ${targetAccountIDsLength} ${users}`,
      );
      // eslint-disable-next-line @typescript-eslint/no-deprecated
      const roomName =
      ...
      getOriginalMessage(lastAction)?.resolution
    ) {
      alternateText = getActionableCardFraudAlertResolutionMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = StringUtils.lineBreaksToSpaces(
        Parser.htmlToText(
          getWorkspaceDescriptionUpdatedMessage(translate, lastAction),
        ),
      );
    } else if (
      isActionOfType(
      ...
      isActionOfType(
        lastAction,
        CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
          .UPDATE_AUTO_REPORTING_FREQUENCY,
      )
    ) {
      ...
      alternateText = Parser.htmlToText(
        getCompanyCardConnectionBrokenMessage(translate, lastAction),
      );
    } else if (
      isActionOfType(
        lastAction,
      ...
      alternateText = Parser.htmlToText(
        getPlaidBalanceFailureMessage(translate, lastAction),
      );
    } else if (
      lastAction?.actionName &&
      isCategoryModificationAction(lastAction.actionName)
    ) {
      ...
      alternateText = getWorkspaceCategoriesUpdatedMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getForeignCurrencyDefaultTaxUpdateMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getCleanedTagName(
        getTagListNameUpdatedMessage(translate, lastAction) ?? "",
      );
    } else if (isTagModificationAction(lastAction?.actionName ?? "")) {
      alternateText = getCleanedTagName(
        getWorkspaceTagUpdateMessage(translate, lastAction) ?? "",
      );
    } else if (
      isActionOfType(
        lastAction,
      ...
      alternateText = getWorkspaceCustomUnitUpdatedMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getWorkspaceCustomUnitRateImportedMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getWorkspaceCustomUnitRateAddedMessage(
        translate,
        dateFnsLocale,
        lastAction,
      );
      ...
      alternateText = getWorkspaceCustomUnitRateUpdatedMessage(
        translate,
        dateFnsLocale,
        lastAction,
      );
      ...
      alternateText = getWorkspaceCustomUnitRateDeletedMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getWorkspaceCustomUnitSubRateUpdatedMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getWorkspaceCustomUnitSubRateDeletedMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getWorkspaceReportFieldUpdateMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getWorkspaceReportFieldDeleteMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      lastAction?.actionName ===
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_IS_ATTENDEE_TRACKING_ENABLED
    ) {
      alternateText = getWorkspaceAttendeeTrackingUpdateMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getRequireCompanyCardsEnabledMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getAutoPayApprovedReportsEnabledMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getAutoReimbursementMessage(
        translate,
        lastAction,
        convertToDisplayString,
      );
      ...
      alternateText = getInvoiceCompanyWebsiteUpdateMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getWorkspaceReimbursementUpdateMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getPolicyChangeLogMaxExpenseAmountNoReceiptMessage(
        translate,
        lastAction,
        convertToDisplayString,
      );
      ...
      lastAction?.actionName ===
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_MAX_EXPENSE_AMOUNT_NO_ITEMIZED_RECEIPT
    ) {
      alternateText =
        getPolicyChangeLogMaxExpenseAmountNoItemizedReceiptMessage(
          translate,
      ...
      alternateText = getPolicyChangeLogMaxExpenseAmountMessage(
        translate,
        lastAction,
        convertToDisplayString,
      );
      ...
      alternateText = getPolicyChangeLogMaxExpenseAgeMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getPolicyChangeLogDefaultBillableMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getPolicyChangeLogDefaultReimbursableMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getPolicyChangeLogDefaultTitleEnforcedMessage(
        translate,
        lastAction,
      );
    } else if (isLeavePolicyAction(lastAction)) {
      alternateText = getPolicyChangeLogEmployeeLeftMessage(
        translate,
        lastAction,
        getPersonalDetailsByID(lastAction.actorAccountID, personalDetails),
        true,
      );
    } else if (isCardIssuedAction(lastAction)) {
      alternateText = getCardIssuedMessage({
      ...
      alternateText = StringUtils.lineBreaksToSpaces(
        Parser.htmlToText(
          getUpdateRoomDescriptionMessage(translate, lastAction),
        ),
      );
    } else if (
      lastAction?.actionName ===
      ...
      alternateText = getPolicyChangeLogAddEmployeeMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getPolicyChangeLogDeleteMemberMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getUpdatedCardFeedStatementPeriodMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = StringUtils.lineBreaksToSpaces(
        getAddAgentRuleMessage(translate, lastAction),
      );
    } else if (
      lastAction?.actionName ===
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG.UPDATE_AGENT_RULE
      ...
      alternateText = StringUtils.lineBreaksToSpaces(
        getUpdateAgentRuleMessage(translate, lastAction),
      );
    } else if (
      lastAction?.actionName ===
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG.DELETE_AGENT_RULE
      ...
      alternateText = StringUtils.lineBreaksToSpaces(
        getDeleteAgentRuleMessage(translate, lastAction),
      );
    } else if (
      lastAction?.actionName ===
      CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG
        .UPDATE_MANUAL_APPROVAL_THRESHOLD
    ) {
      alternateText = getUpdatedManualApprovalThresholdMessage(
        translate,
        lastAction,
        convertToDisplayString,
      );
      ...
      alternateText = getUpdatedTimeRateMessage(
        translate,
        lastAction,
        convertToDisplayString,
      );
      ...
      alternateText = getUpdatedProhibitedExpensesMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getUpdatedReimbursementChoiceMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getUpdatedIndividualBudgetNotificationMessage(
        translate,
        lastAction,
      );
    } else if (
      ...
      alternateText = getUpdatedSharedBudgetNotificationMessage(
        translate,
        lastAction,
      );
    } else if (isPolicyCopyReportAction(lastAction)) {
      alternateText = Parser.htmlToText(
        getPolicyChangeLogCopyMessage(translate, lastAction),
      );
      ...
      isActionOfType(lastAction, CONST.REPORT.ACTIONS.TYPE.TRAVEL_UPDATE)
    ) {
      ...
      alternateText = Parser.htmlToText(
        getChangedApproverActionMessage(translate, lastAction),
      );
    } else if (
      lastAction?.actionName ===
      ...
      alternateText = Parser.htmlToText(
        getUpdatedOwnershipMessage(translate, lastAction, policy),
      );
    } else if (
      isActionOfType(lastAction, CONST.REPORT.ACTIONS.TYPE.MOVED_TRANSACTION)
    ) {
      const { fromReportID, toReportID, displayReportID } =
        parseMovedTransactionReportIDs(lastAction);
      alternateText = Parser.htmlToText(
        getMovedTransactionMessage({
          translate,
          fromReportID,
      ...
      alternateText = Parser.htmlToText(
        getSettlementAccountLockedMessage(translate, lastAction),
      );
    } else if (
      ...
      alternateText = formatReportLastMessageText(
        `${getLastActorDisplayNamePrefix()}: ${lastMessageText}`,
      );
    } else {
      alternateText =
        lastMessageTextFromReport.length > 0
      ...
      if (!alternateText) {
        alternateText = formatReportLastMessageText(
          getWelcomeMessage({
            ...
            invoiceReceiverPolicy,
            participantPersonalDetailList:
              getParticipantPersonalDetailListExcludeCurrentUser(),
            translate,
            localeCompare,
            conciergeReportID,
            derivedReportName:
              reportAttributesDerived?.[report.reportID]?.reportName,
            isReportArchived,
            isTrackIntentUser,
            currentUserAccountID,
            formatPhoneNumber,
...
function getExpensifyCardFromReportAction({
  ...
  const cardIssuedActionOriginalMessage = isCardIssuedAction(reportAction)
    ? getOriginalMessage(reportAction)
    : undefined;
  const cardID =
    cardIssuedActionOriginalMessage?.cardID ?? CONST.DEFAULT_NUMBER_ID;
  ...
  return (
    ...
    workspaceCardList?.[
      `${ONYXKEYS.COLLECTION.WORKSPACE_CARDS_LIST}${getTravelBillingFeedID(workspaceAccountID)}`
    ]?.[cardID] ??
    cardList?.[cardID]
  );
}
