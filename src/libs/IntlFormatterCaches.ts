 ...
 */
/** `Intl.DateTimeFormat` holds 10-50 KB of ICU state per entry, so this one is bounded. The relative-time key space is the shipped locales in each numeric style. */
const INTL_FORMAT_CACHE_MAX_SIZE = 256;

const intlDateTimeFormatCache = new Map<string, Intl.DateTimeFormat | null>();
const relativeTimeFormatCache = new Map<string, Intl.RelativeTimeFormat | null>();
