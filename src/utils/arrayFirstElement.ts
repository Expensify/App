function arrayFirstElement<T>(array: T[] | undefined | null, comparator?: (el1: T, el2: T) => number, filter: (el: T) => boolean = () => true): T | undefined {
    if (!array?.length) {
        return undefined;
    }
    if (array.length === 1) {
...
export default arrayFirstElement;
