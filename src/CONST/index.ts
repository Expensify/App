...
// Scrape result codes where the connection is broken specifically because the user needs to
// re-authenticate with their bank (438 = account refresh required, 532 = session expired).
// Explicit type annotation is required so `.includes(number)` type-checks under `as const`.
...
const CONST = {
    ...
    COMPANY_CARDS: {
        ...
        // Scrape result codes where the connection is broken specifically because the user needs to
        // re-authenticate with their bank (438 = account refresh required, 532 = session expired), as
        // opposed to a generic broken connection. Used to surface a clearer "needs re-auth" message.
