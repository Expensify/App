...
const lodashGet = (config, key, defaultValue) =>
...
const newExpensifyURL = Url.addTrailingForwardSlash(
    lodashGet(Config, 'NEW_EXPENSIFY_URL', 'https://new.expensify.com/'),
);
const expensifyURL = Url.addTrailingForwardSlash(
    lodashGet(Config, 'EXPENSIFY_URL', 'https://www.expensify.com/'),
);
const stagingExpensifyURL = Url.addTrailingForwardSlash(
    lodashGet(
        Config,
...
const stagingSecureExpensifyUrl = Url.addTrailingForwardSlash(
    lodashGet(
        Config,
        'STAGING_SECURE_EXPENSIFY_URL',
        'https://staging-secure.expensify.com/',
    ),
);
const ngrokURL = Url.addTrailingForwardSlash(
    lodashGet(Config, 'NGROK_URL', ''),
);
const secureNgrokURL = Url.addTrailingForwardSlash(
    lodashGet(Config, 'SECURE_NGROK_URL', ''),
);
const secureExpensifyUrl = Url.addTrailingForwardSlash(
    lodashGet(Config, 'SECURE_EXPENSIFY_URL', 'https://secure.expensify.com/'),
);
const useNgrok = lodashGet(Config, 'USE_NGROK', 'false') === 'true';
const useWebProxy = lodashGet(Config, 'USE_WEB_PROXY', 'true') === 'true';
const expensifyComWithProxy =
    getPlatform() === 'web' && useWebProxy ? '/' : expensifyURL;

// Throw errors on dev if config variables are not set correctly
if (ENVIRONMENT === CONST.ENVIRONMENT.DEV) {
    if (
        !useNgrok &&
        expensifyURL.includes('dev') &&
        !secureExpensifyUrl.includes('dev')
    ) {
        throw new Error(
            'SECURE_EXPENSIFY_URL must end with .dev when EXPENSIFY_URL ends with .dev',
        );
    }
...
const secureURLRoot =
    useNgrok && secureNgrokURL ? secureNgrokURL : secureExpensifyUrl;
...
const expensifyURLRoot =
    useNgrok && ngrokURL ? ngrokURL : expensifyComWithProxy;
...
export default {
    ...
    EXPENSIFY: {
        ...
        PARTNER_NAME: lodashGet(
            Config,
            'EXPENSIFY_PARTNER_NAME',
            'chat-expensify-com',
        ),
        PARTNER_PASSWORD: lodashGet(
            Config,
    ...
    SEND_CRASH_REPORTS:
        lodashGet(Config, 'SEND_CRASH_REPORTS', 'false') === 'true',
};
