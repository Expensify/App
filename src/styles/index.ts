...
const getReceiptDropZoneViewStyle = (
  theme: ThemeColors,
  margin: number,
...
const touchCalloutNone: Pick<ViewStyle, "WebkitTouchCallout"> = isMobileSafari()
  ? { WebkitTouchCallout: "none" }
  : {};
// to prevent vertical text offset in Safari for badges, new lineHeight values have been added
const lineHeightBadge: Pick<TextStyle, "lineHeight"> = isSafari()
...
const staticStyles = (theme: ThemeColors) =>
  StyleSheet.create({
    ...
    moneyRequestAmountContainer: {
      minHeight:
        variables.inputHeight + 2 * (variables.formErrorLineHeight + 8),
    },
    ...
    workspaceSection: {
      maxWidth:
        variables.workspaceSectionMaxWidth + variables.sectionMargin * 2,
    },
    ...
    uploadFileView: getReceiptDropZoneViewStyle(
      theme,
      variables.uploadViewMargin,
      40,
    ),
    ...
    paymentMethodErrorRow: {
      paddingHorizontal:
        variables.iconSizeMenuItem + variables.iconSizeNormal / 2,
    },
...
const dynamicStyles = (theme: ThemeColors) =>
  ({
    ...
    chooseFilesView: (isSmallScreenWidth: boolean) =>
      ({
        ...getReceiptDropZoneViewStyle(
          theme,
          variables.chooseFilesViewMargin,
          20,
        ),
        borderWidth: isSmallScreenWidth ? 0 : 2,
      }) satisfies ViewStyle,
      ...
      if (isForTextComponent) {
        return {
          fontSize: interpolate(
            scale.get(),
            [0, ACTIVE_LABEL_SCALE],
            [0, variables.fontSizeLabel],
          ),
        } satisfies TextStyle;
      }
    ...
    createMenuPositionSearchBar: (windowHeight: number) =>
      ({
        ...
        vertical:
          windowHeight -
          (variables.fabBottom + variables.componentSizeNormal + 12),
      }) satisfies AnchorPosition,
    ...
    twoFactorAuthCodesBoxPadding: ({
      isExtraSmallScreenWidth,
      isSmallScreenWidth,
    }: TwoFactorAuthCodesBoxParams) => {
    ...
    workspaceUpgradeIntroBox: ({
      isExtraSmallScreenWidth,
    }: WorkspaceUpgradeIntroBoxParams): ViewStyle => {
    ...
    activeDropzoneDashedBorder: (borderColor: string, isActive: boolean) => {
      ...
      const isSafariOrChromeBrowser =
        getPlatform() === CONST.PLATFORM.WEB &&
        (browser === CONST.BROWSER.SAFARI || browser === CONST.BROWSER.CHROME);
    ...
    attachmentDropOverlay: (isActive?: boolean) => ({
      backgroundColor: isActive
        ? theme.attachmentDropUIBGActive
        : theme.attachmentDropUIBG,
      transition: "background-color 0.2s ease-in",
    }),

    receiptDropOverlay: (isActive?: boolean) => ({
      backgroundColor: isActive
        ? theme.receiptDropUIBGActive
        : theme.receiptDropUIBG,
      transition: "background-color 0.2s ease-in",
    }),
    ...
    tabBackground: (
      ...
      isFocused: boolean,
      isDisabled: boolean,
      background: string | Animated.AnimatedInterpolation<string>,
    ) => {
      if (isDisabled) {
      ...
      return {
        backgroundColor:
          hovered && !isFocused ? theme.highlightBG : (background as string),
      };
    },
    ...
    emojiHeaderContainerWidth: (
      ...
      ({
        width: isSmallScreenWidth
          ? windowWidth - 32
          : CONST.EMOJI_PICKER_SIZE.WIDTH - 32,
      }) satisfies ViewStyle,
    ...
    getForYouSectionContainerStyle: (
    ...
    getSafeAreaPressablePosition: (
      safeAreaPosition: "right" | "left",
      width: number,
    ): ViewStyle => ({
      position: "absolute",
      [safeAreaPosition]: 0,
    ...
    getSelectionListPopoverHeight: ({
      ...
      const ESTIMATED_LIST_HEIGHT =
        itemCount * itemHeight + SEARCHBAR_HEIGHT + extraHeight;
      const ESTIMATED_NON_LIST_HEIGHT =
        BUTTON_HEIGHT + HEADER_HEIGHT + TITLE_HEIGHT + MODAL_VERTICAL_PADDING;

      const heightRatio = isInLandscapeMode
        ? CONST.MODAL_MAX_HEIGHT_TO_WINDOW_HEIGHT_RATIO_LANDSCAPE_MODE
        : CONST.MODAL_MAX_HEIGHT_TO_WINDOW_HEIGHT_RATIO;
      ...
      const height = Math.min(
        ...
        CONST.POPOVER_DROPDOWN_MAX_HEIGHT - ESTIMATED_NON_LIST_HEIGHT,
        windowHeight * heightRatio - ESTIMATED_NON_LIST_HEIGHT,
      );
      return { height };
    },
    ...
    getCenteredModalInnerView: (
      ...
      const borderBottomRadius = shouldUseNarrowLayout
        ? 0
    ...
    getTestToolsNavigatorInnerView: (
      shouldUseNarrowLayout: boolean,
    ...
    getEmptyStateCompanyCardsIllustrationContainer: (
      shouldUseNarrowLayout: boolean,
    ) => (shouldUseNarrowLayout ? { height: 220 } : { aspectRatio: 680 / 220 }),

    getEmptyStateCompanyCardsIllustration: (shouldUseNarrowLayout: boolean) =>
      shouldUseNarrowLayout ? { width: 680, height: 220 } : {},

    searchListContentContainerStyles: (hasFilterBars: boolean) => ({
      paddingTop: hasFilterBars
        ? variables.searchListContentWithFiltersMarginTop
        : variables.searchListContentMarginTop,
    }),
    ...
    sectionMenuItem: (shouldUseNarrowLayout: boolean) => ({
      ...
      height: shouldUseNarrowLayout
        ? variables.sectionMenuItemHeight
        : variables.sectionMenuItemHeightCompact,
      alignItems: "center",
    }),
    ...
    searchActionsBar: (shouldUseNarrowLayout: boolean) => ({
      ...
      width: shouldUseNarrowLayout
        ? variables.componentSizeNormal
        : variables.h28,
      backgroundColor: shouldUseNarrowLayout
        ? undefined
...
const plainStyles = (theme: ThemeColors) =>
  ({
    ...
    pickerSmall: (
      disabled = false,
      backgroundColor: string = theme.highlightBG,
    ) =>
      ({
        inputIOS: {
          ...FontUtils.fontFamily.platform.EXP_NEUE,
