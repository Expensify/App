    ...
    constructor() {
        ...
        this.rules = [
             ...
             */
            {
                ...
                // We're using a function here to perform an additional replace on the content inside the backticks
                // because Android is not able to use <pre> tags and does not respect whitespace characters at all
                // like HTML does. We do not want to mess with the new lines here since they need to be converted into
                // <br>. And we don't want to do this anywhere else since that would break HTML.
                replacement: (match, firstCapturedGroup) => {
                    const capturedGroup = firstCapturedGroup.replace(/(?:(?![\n\r])\s)/g, '&nbsp;');
                    return `<pre>${capturedGroup}</pre>`;
                },
            },

            /**
             * Converts markdown style links to anchor tags e.g. [Expensify](https://www.expensify.com)
             * We need to convert before the autolink rule since it will not try to create a link from
            ...
            },
            {
                 ...
                 * `\b\_([^<>]*?)\_\b` doesn't work because it won't replace `_https://www.test.com_`
                 */
                name: 'italic',
            ...
            {
                // Use \B in this case because \b doesn't match * or ~. \B
                // will match everything that \b doesn't, so it works for * and ~:
                // https://www.rexegg.com/regex-boundaries.html#notb
                name: 'bold',
                regex: /\B\*(.*?)\*\B(?![^<]*<\/pre>)/g,
