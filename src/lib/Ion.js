...
/**
 * Perform a depth-first search of all the IONKEYS, and collect all the keys whose values are NOT null or undefined
 *
...
function getCurrentKeySet() {
    const currentKeySet = (
        function dfs(keyObj) {
            return _.reduce(keyObj, (keySet, key) => ((typeof key === 'object')
                ? _.union(keySet, dfs(key))
                : _.union(keySet, [get(key)
                    .then((ionVal) => {
                        if (ionVal === null || ionVal === undefined) {
    ...
    // Wait for all promises generated by the recursive function to finish, then filter and extract the results
    return promiseAllSettled(currentKeySet)
        .then(keySet => _.filter(keySet, key => key.status === 'fulfilled'))
        .then(keySet => _.flatten(_.map(keySet, key => key.value)));
}
