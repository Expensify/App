...
export default function useSearchTypeMenu(queryJSON: SearchQueryJSON) {
    ...
    const {savedSearchesMenuItems, isSavedSearchActive} = useMemo(() => {
        ...
        const menuItems: PopoverMenuItem[] = Object.entries(savedSearches).map(([key, item], index) => {
            ...
            if (savedSearchTitle === item.query) {
                ...
                savedSearchTitle = buildUserReadableQueryString(jsonQuery, personalDetails, reports, taxRates, allCards, allFeeds, allPolicies, currentUserAccountID, undefined, translate);
            }
    ...
    }, [savedSearches, hash, getOverflowMenu, expensifyIcons.Bookmark, personalDetails, reports, taxRates, allCards, allFeeds, allPolicies, currentUserAccountID, translate]);
