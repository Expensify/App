...
} from "@userActions/IOU/PayMoneyRequest";
import {
  approveMoneyRequest,
  reopenReport,
  retractReport,
...
function useLifecycleActions({
  ...
  onHoldMenuOpen,
}: UseLifecycleActionsParams): UseLifecycleActionsResult {
  ...
  const [chatReportActions] = useOnyx(
    `${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${getNonEmptyStringOnyxID(moneyRequestReport?.chatReportID)}`,
  );
  ...
  const isSubmitterSameAsNextApprover =
    isReportOwner(moneyRequestReport) &&
    (nextApproverAccountID === moneyRequestReport?.ownerAccountID ||
      moneyRequestReport?.managerID === moneyRequestReport?.ownerAccountID);
  const isBlockSubmitDueToPreventSelfApproval = !!(
    isSubmitterSameAsNextApprover && policy?.preventSelfApproval
  );

  const isBlockSubmitDueToStrictPolicyRules =
    shouldBlockSubmitDueToStrictPolicyRules(
      moneyRequestReport?.reportID,
      violations,
  ...
  const selectedTransactions = transactions.filter((transaction) =>
    selectedTransactionIDs.includes(transaction.transactionID),
  );

  const isBlockSubmitDueToSelectedTransactionsOnSubmitPolicy =
    isSubmitPolicy(policy) && selectedTransactions.length > 1;

  const shouldBlockSubmit =
    isBlockSubmitDueToStrictPolicyRules ||
    isBlockSubmitDueToPreventSelfApproval ||
    isBlockSubmitDueToSelectedTransactionsOnSubmitPolicy;
  ...
  const integrationNameFromExportMessage = isExported
    ? getIntegrationNameFromExportMessageUtils(reportActions)
    : null;
  ...
  const confirmPendingRTERAndProceed = useConfirmPendingRTERAndProceed(
  ...
  const confirmApproval = (skipAnimation = false) => {
    ...
    if (isAnyTransactionOnHold) {
      onHoldMenuOpen(
        ...
        skipAnimation ? undefined : () => startApprovedAnimation(),
      );
      return;
    }
      ...
      onSelected: async () => {
        ...
        if (isExported) {
          const unapproveWarningText = (
            <Text>
              <Text style={[styles.textStrong, styles.noWrap]}>
                {translate("iou.headsUp")}
              </Text>{" "}
              <Text>
                {translate(
                  "iou.unapproveWithIntegrationWarning",
