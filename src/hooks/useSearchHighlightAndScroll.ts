...
function useSearchHighlightAndScroll({searchResults, queryJSON}: UseSearchHighlightAndScroll) {
    ...
    const handleSelectionListScroll = useCallback(
        (data: SearchListItem[]) => {
            // Return a function that takes a ref and performs scrolling if needed
            // This function is used in a ref={} attribute in the component
            return (ref: SelectionListHandle | null) => {
                // If the ref is null or there's no new item to scroll to, just return the ref
