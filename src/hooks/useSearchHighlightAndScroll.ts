...
import type {ReportActionListItemType, ReportListItemType, SelectionListHandle, TaskListItemType, TransactionListItemType} from '@components/SelectionList/types';
...
function useSearchHighlightAndScroll({searchResults, transactions, previousTransactions, reportActions, previousReportActions, queryJSON, offset}: UseSearchHighlightAndScroll) {
    ...
    const handleSelectionListScroll = useCallback(
        (data: Array<TransactionListItemType | ReportActionListItemType | ReportListItemType | TaskListItemType>) => (ref: SelectionListHandle | null) => {
