...
function useChangeTransactionsReportReports(transactionIDs: string[], allTransactions: OnyxCollection<Transaction>, newReport: OnyxEntry<Report>): OnyxCollection<Report> {
    ...
    const reportsSelector = useCallback(
        ...
        [transactionIDs, allTransactions, newReportID],
    );

    const [reports = getEmptyObject<NonNullable<OnyxCollection<Report>>>()] = useOnyx(ONYXKEYS.COLLECTION.REPORT, {selector: reportsSelector}, [reportsSelector]);
