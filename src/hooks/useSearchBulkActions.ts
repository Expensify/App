...
import {useSearchBulkActionsModal} from '@components/Search/SearchBulkActionsModalContext';
...
import {canIOUBePaid} from '@userActions/IOU';
...
function useSearchBulkActions(): UseSearchBulkActionsResult {
    ...
    const {currentSearchResults, currentSearchQueryJSON: queryJSON} = useSearchContext();
    ...
    const modal = useSearchBulkActionsModal();
    ...
    const {bulkPayButtonOptions} = useBulkPayOptions({
    ...
    const beginExportWithTemplate = useCallback(
        async (templateName: string, templateType: string, policyID: string | undefined) => {
            ...
            if (hasOnlyEmptyReports) {
                modal.setEmptyReportsCount(emptyReports.length);
                modal.setIsDownloadErrorModalVisible(true);
                return;
            }
            if (isOffline) {
                modal.setIsOfflineModalVisible(true);
                return;
            }
    ...
    const handleBasicExport = useCallback(async () => {
        ...
        if (areAllMatchingItemsSelected) {
            ...
            const reportIDList = selectedReports?.map((r) => r?.reportID).filter((id): id is string => id !== undefined) ?? [];
            queueExportSearchItemsToCSV({
                query: status,
                jsonQuery: JSON.stringify(queryJSON),
                reportIDList,
    ...
    const handleApproveWithDEWCheck = useCallback(async () => {
        ...
        const selectedPolicyIDList = selectedReports.length ? selectedReports.map((r) => r.policyID) : Object.values(selectedTransactions).map((t) => t.policyID);
        ...
        const reportIDList = !selectedReports.length ? Object.values(selectedTransactions).map((t) => t.reportID) : (selectedReports?.filter((r) => !!r).map((r) => r.reportID) ?? []);
        approveMoneyRequestOnSearch(
            hash,
            reportIDList.filter((id): id is string => id !== undefined),
        );
        ...
        (paymentMethod?: PaymentMethodType, additionalData?: Record<string, unknown>) => {
            ...
            for (const item of selectedOptions) {
                ...
                if (!lastPolicyPaymentMethod) {
                    Navigation.navigate(ROUTES.SEARCH_REPORT.getRoute({reportID: itemReportID, backTo: activeRoute}));
                    return;
                }
                ...
                if (isPolicyPaymentMethod && isIOUReport) {
                    ...
                    const reportTransactions = Object.values(allTransactions ?? {}).filter((tx): tx is NonNullable<typeof tx> => !!tx && tx.reportID === itemReportID);
    ...
    const searchResults: SearchResults | undefined = currentSearchResults;
    const areAllTransactionsFromSubmitter = useMemo(() => {
        ...
        const reports: Report[] = searchData
            ? Object.keys(searchData)
                  ...
                  .filter((r): r is Report => r != null && 'reportID' in r)
            : [];
        return selectedTransactionReportIDs.length > 0 && selectedTransactionReportIDs.every((id) => isCurrentUserSubmitter(getReportOrDraftReport(id, reports)));
    }, [selectedTransactionReportIDs, currentUserPersonalDetails?.accountID, currentSearchResults?.data]);

    const headerButtonsOptions = useMemo(() => {
        if (selectedTransactionsKeys.length === 0 || status == null || !hash) {
            return [] as Array<DropdownOption<SearchHeaderOptionValue>>;
        }
        const options: Array<DropdownOption<SearchHeaderOptionValue>> = [];
        const isAnyTransactionOnHold = Object.values(selectedTransactions).some((t) => t.isHeld);
        ...
        const getExportOptions = () => {
            ...
            const isAllOneTransactionReport = Object.values(selectedTransactions).every((t) => t.isFromOneTransactionReport);
            ...
            for (const template of exportTemplates) {
                exportOptions.push({
                    ...
                    onSelected: () => {
                        beginExportWithTemplate(template.templateName, template.type, template.policyID).catch(() => {});
                    },
                    shouldCloseModalOnSelect: true,
                    shouldCallAfterModalHide: true,
                });
        ...
        } as DropdownOption<SearchHeaderOptionValue> & Pick<PopoverMenuItem, 'rightIcon'>;
        ...
        const shouldShowApproveOption =
            ...
            (selectedReports.length
                ? selectedReports.every((r) => r.allActions.includes(CONST.SEARCH.ACTION_TYPES.APPROVE))
        ...
        if (shouldShowApproveOption) {
            options.push({
                ...
                shouldCloseModalOnSelect: true,
                onSelected: () => handleApproveWithDEWCheck(),
            });
        }
            ...
            options.push({
                ...
                onSelected: () => {
                    ...
                    if (dismissedRejectUseExplanation) {
                        ...
                        modal.setRejectModalAction(CONST.REPORT.TRANSACTION_SECONDARY_ACTIONS.REJECT);
                    }
                },
            });
        }
        ...
        const shouldShowSubmitOption =
            ...
            areSelectedTransactionsIncludedInReports &&
            (selectedReports.length
                ? selectedReports.every((r) => r.allActions.includes(CONST.SEARCH.ACTION_TYPES.SUBMIT))
        ...
        if (shouldShowSubmitOption) {
            options.push({
                ...
                onSelected: () => {
                    ...
                    const itemList = !selectedReports.length ? Object.values(selectedTransactions).map((t) => t) : (selectedReports?.filter((r) => !!r) ?? []);
        ...
        if (shouldShowPayOption) {
            ...
            } as DropdownOption<SearchHeaderOptionValue>);
        }
            ...
            options.push({
                ...
                onSelected: () => {
                    ...
                    if (isDismissed) {
                        ...
                        modal.setIsHoldEducationalModalVisible(true);
                    } else {
                        modal.setRejectModalAction(CONST.REPORT.TRANSACTION_SECONDARY_ACTIONS.HOLD);
                    }
                },
            });
        }
        ...
        if (shouldShowUnholdOption) {
            options.push({
                ...
                onSelected: () => {
                    ...
                    InteractionManager.runAfterInteractions(() => clearSelectedTransactions());
                },
            });
        }
        ...
        for (const id of selectedTransactionsKeys) {
            const entry = selectedTransactions[id];
            if (!entry) {
                continue;
            }
            const ownerAccountID = entry.ownerAccountID ?? getReportOrDraftReport(entry.reportID)?.ownerAccountID;
        ...
        if (canSplitTransaction) {
            options.push({
                ...
                onSelected: () => initSplitExpense(allTransactions, allReports, firstTransaction),
            });
        }
