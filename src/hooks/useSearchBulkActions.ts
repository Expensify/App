...
function useSearchBulkActions({queryJSON}: UseSearchBulkActionsParams) {
    ...
    const headerButtonsOptions = useMemo(() => {
        ...
        return options;
    }, [selectedTransactionsKeys, status, hash, selectedTransactions, queryJSON?.type, expensifyIcons, translate, areAllMatchingItemsSelected, isOffline, selectedReports, lastPaymentMethods, selectedReportIDs, personalPolicyID, searchResults, currentSearchResults?.data, selectedTransactionReportIDs, selectedPolicyIDs, policies, allReportActions, integrationsExportTemplates, csvExportLayouts, allReports, accountID, currentUserLogin, bankAccountList, styles.integrationIcon, showConfirmModal, clearSelectedTransactions, handleBasicExport, handleExportCurrentView, beginExportWithTemplate, handleApproveWithDEWCheck, allTransactionViolations, isDelegateAccessRestricted, dismissedRejectUseExplanation, showDelegateNoAccessModal, bulkPayButtonOptions, onBulkPaySelected, areAllTransactionsFromSubmitter, dismissedHoldUseExplanation, localeCompare, firstTransaction, isDuplicateOptionVisible, invokeDuplicateHandler, isDuplicateReportOptionVisible, invokeDuplicateReportHandler, isExpenseReportType, handleDeleteSelectedTransactions, undeleteTransactions, theme.icon, styles.colorMuted, styles.fontWeightNormal, userBillingGracePeriodEnds, ownerBillingGracePeriodEnd, currentSearchKey, isTrackIntentUser, getCurrencyDecimals, amountOwed, allTransactions, transactions, isBetaEnabled, defaultExpensePolicy, personalDetails, selfDMReportID, splitEffectivePolicy, restrictedActionPolicyID, doSelectedItemsBelongToSubmitPolicy, openSearchReportSubmitToPopover, deleteTransactionsFromHook, duplicateTransactionViolations, duplicateTransactions, firstTransactionReport, isProduction, shouldOpenSplitExpenseEditFlowOnDelete, styles.textWrap, trackExport, allReportsShouldMarkAsDone, noReportsShouldMarkAsDone, queryJSON?.groupBy]);
