...
function useFrozenSplitTransactionIDs(
    ...
    for (const item of splitExpenses) {
        const itemTransaction = allTransactions?.[`${ONYXKEYS.COLLECTION.TRANSACTION}${item.transactionID}`];
        const itemReport = allReports?.[`${ONYXKEYS.COLLECTION.REPORT}${itemTransaction?.reportID}`] ?? fallbackReport;
