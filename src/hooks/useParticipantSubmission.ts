...
function useParticipantSubmission({
    ...
    const goToNextStep = (_value?: string, nextParticipants?: Participant[]) => {
        ...
        if ((isCategorizing || isShareAction) && numberOfParticipants.current === 0) {
            ...
            Navigation.setNavigationActionToMicrotaskQueue(() => {
                if (isCategorizing) {
                    const confirmationRoute = ROUTES.MONEY_REQUEST_STEP_CONFIRMATION.getRoute(action, CONST.IOU.TYPE.SUBMIT, initialTransactionID, expenseChatReportID);
                    Navigation.navigate(
        ...
        const categoryRoute = createDynamicRoute(
            DYNAMIC_ROUTES.MONEY_REQUEST_STEP_CATEGORY.getRoute(action, iouType, initialTransactionID, selectedReportID.current || reportID),
            iouConfirmationPageRoute,
        );
        const route = isCategorizing ? categoryRoute : iouConfirmationPageRoute;
