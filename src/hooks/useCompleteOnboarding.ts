...
type CompleteOnboardingParams = {
    featuresMap: FeaturesMapItem[];
    userReportedIntegration: string | undefined | null;
};
...
function useCompleteOnboarding() {
    ...
    const completeOnboardingFlow = async ({featuresMap, userReportedIntegration}: CompleteOnboardingParams) => {
        ...
        const resolvedIntegration = featuresMap.some((feature) => feature.id === CONST.POLICY.MORE_FEATURES.ARE_CONNECTIONS_ENABLED && feature.enabled) ? userReportedIntegration : undefined;
