...
// settleable, because it only passes through and cannot end a walk.
//
// Auto-advance is a static approximation here. An unguarded `always` transition must fire, so it always
// disqualifies the leaf; a guarded one may keep the machine in place, so it never does. An invoked actor
// is treated as routing onward once it registers `onDone` or `onError`, which holds for an actor that
// completes immediately (the mocked promise actors this harness expects) but not for one that stays
// pending. Revisit this predicate when the first real invoke state lands in the machine.
