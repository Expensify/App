...
describe('incrementVersion', () => {
    it('should increment major', () => {
        expect(functions.incrementVersion(version, functions.semanticVersionLevels.major)).toStrictEqual('v3.0.0');
    });
    it('should increment major even above max level', () => {
        expect(functions.incrementVersion(`v${functions.maxIncrements}.5.1-80`, functions.semanticVersionLevels.major)).toStrictEqual(`v${functions.maxIncrements + 1}.0.0`);
    });
    it('should increment build number', () => {
        expect(functions.incrementVersion(version, functions.semanticVersionLevels.build)).toStrictEqual('v2.3.9-81');
    });
    it('should add build number if there is no build number', () => {
        expect(functions.incrementVersion(version.split('-')[0], functions.semanticVersionLevels.build)).toStrictEqual('v2.3.9-1');
    });
    it('should increment patch if minor is above max level', () => {
        expect(functions.incrementVersion(`v2.3.9-${functions.maxIncrements}`, functions.semanticVersionLevels.build)).toStrictEqual('v2.3.10');
    });
});
