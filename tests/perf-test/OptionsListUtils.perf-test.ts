...
const options = createFilteredOptionList(personalDetails, reports, formatPhoneNumber, undefined, EMPTY_PRIVATE_IS_ARCHIVED_MAP, undefined, {conciergeReportID: undefined, isSearching: true});
...
describe('OptionsListUtils', () => {
    ...
    test('[OptionsListUtils] getFilteredOptions with search value', async () => {
        ...
        await measureFunction(() => {
            filterAndOrderOptions(formattedOptions, SEARCH_VALUE, COUNTRY_CODE, loginList, MOCK_CURRENT_USER_EMAIL, MOCK_CURRENT_USER_ACCOUNT_ID, personalDetails, formatPhoneNumber);
        });
    });
    test('[OptionsListUtils] getFilteredOptions with empty search value', async () => {
        ...
        await measureFunction(() => {
            filterAndOrderOptions(formattedOptions, '', COUNTRY_CODE, loginList, MOCK_CURRENT_USER_EMAIL, MOCK_CURRENT_USER_ACCOUNT_ID, personalDetails, formatPhoneNumber);
        });
    });
    ...
    test('[OptionsListUtils] createFilteredOptionList', async () => {
        ...
        await measureFunction(() => {
            ...
            return createFilteredOptionList(personalDetails, mockedReportsMap, formatPhoneNumber, undefined, EMPTY_PRIVATE_IS_ARCHIVED_MAP, undefined, {
                conciergeReportID: undefined,
    ...
    test('[OptionsListUtils] createFilteredOptionList with isSearching is true', async () => {
        await waitForBatchedUpdates();
        await measureFunction(() =>
            createFilteredOptionList(personalDetails, mockedReportsMap, formatPhoneNumber, undefined, EMPTY_PRIVATE_IS_ARCHIVED_MAP, undefined, {
                conciergeReportID: undefined,
                maxRecentReports: 500,
                isSearching: true,
            }),
    ...
    test('[OptionsListUtils] getSearchOptions with isSearching is true', async () => {
        await waitForBatchedUpdates();
        const optionLists = createFilteredOptionList(personalDetails, mockedReportsMap, formatPhoneNumber, undefined, EMPTY_PRIVATE_IS_ARCHIVED_MAP, undefined, {
            conciergeReportID: undefined,
            maxRecentReports: 500,
            isSearching: true,
        });
