...
import {createOptionList, getValidOptions} from '@libs/PersonalDetailsOptionsListUtils';
...
const reports = createCollection<Report>(
    ...
    (index) => ({
        ...
        participants: {
            [index]: {notificationPreference: CONST.REPORT.NOTIFICATION_PREFERENCE.ALWAYS},
            [REPORTS_COUNT + 1]: {notificationPreference: CONST.REPORT.NOTIFICATION_PREFERENCE.ALWAYS},
        },
...
describe('PersonalDetailsOptionsListUtils', () => {
    ...
    test('[PersonalDetailsOptionsListUtils] getValidOptions with includeRecentReports=false', async () => {
        ...
        await measureFunction(() => getValidOptions(options.options, currentLogin, {includeRecentReports: false}));
    });

    test('[PersonalDetailsOptionsListUtils] getValidOptions with includeRecentReports=true', async () => {
        await waitForBatchedUpdates();
        await measureFunction(() => getValidOptions(options.options, currentLogin, {includeRecentReports: true}));
    });

    test('[PersonalDetailsOptionsListUtils] getValidOptions with includeRecentReports=false and searchValue', async () => {
        await waitForBatchedUpdates();
        await measureFunction(() => getValidOptions(options.options, currentLogin, {includeRecentReports: false, searchString: SEARCH_VALUE}));
    });

    test('[PersonalDetailsOptionsListUtils] getValidOptions with includeRecentReports=true and searchValue', async () => {
        await waitForBatchedUpdates();
        await measureFunction(() => getValidOptions(options.options, currentLogin, {includeRecentReports: true, searchString: SEARCH_VALUE}));
    });

    test('[PersonalDetailsOptionsListUtils] getValidOptions with includeRecentReports=false on selected options', async () => {
        await waitForBatchedUpdates();
        await measureFunction(() => getValidOptions(selectedOptions, currentLogin, {includeRecentReports: false}));
    });

    test('[PersonalDetailsOptionsListUtils] getValidOptions with includeRecentReports=true on selected options', async () => {
        await waitForBatchedUpdates();
        await measureFunction(() => getValidOptions(selectedOptions, currentLogin, {includeRecentReports: true}));
    });
});
