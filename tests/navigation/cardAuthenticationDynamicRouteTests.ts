...
/**
 * The 3DS card-authentication screen is layered on the dynamic owner-change route. It never receives an explicit
 * `policyID` at navigation time; instead `getStateFromPath` recursively merges the owner-change route's params onto
 * the card-authentication leaf. These tests pin that behavior so the owner-change flow keeps its `policyID` (and
 * subscription keeps none), which is what routes `DynamicCardAuthenticationPage.onSuccess` to the correct verify
 * action.
 */
...
// `createDynamicRoute` is exactly how production layers card authentication on the owner-change route and returns a
// real `Route`, so no `as Route` cast is needed here.
const ownerChangeAuthenticationPath = createDynamicRoute(DYNAMIC_ROUTES.CARD_AUTHENTICATION.path, ownerChangePath);
...
describe('card-authentication dynamic route policyID inheritance', () => {
    it('inherits policyID from the dynamic owner-change path onto the dynamic auth screen (parse layer)', () => {
        const focused = findFocusedRoute(getStateFromPath(ownerChangeAuthenticationPath));
    ...
    it('carries policyID into the dispatched navigation action for the owner-change flow', () => {
        const action = getActionFromState(getStateFromPath(ownerChangeAuthenticationPath), linkingConfig.config);
