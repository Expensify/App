...
describe('actions/IOU/BulkEdit', () => {
    describe('updateMultipleMoneyRequests', () => {
        ...
        it('clears the tag when the only tag is deselected, instead of dropping the empty edit (independent single tag list)', () => {
            const transactionID = 'transaction-clear-1';
            const iouReportID = 'iou-clear-1';
            const policy = {
            ...
            const transaction: Transaction = {
                ...
                transactionID,
                reportID: iouReportID,
                transactionThreadReportID: 'thread-clear-1',
                tag: 'CostCenterA',
            };
            const transactions = {
                [`${ONYXKEYS.COLLECTION.TRANSACTION}${transactionID}`]: transaction,
            ...
            updateMultipleMoneyRequests({
                personalDetailsList: undefined,
                transactionIDs: [transactionID],
                // Deselecting the only tag resolves the display string to empty...
                changes: {tag: ''},
                // ...with an empty per-level clear intent that must still reach the transaction. Built
                // programmatically because numeric-string object-literal keys trip the naming-convention lint rule.
                bulkEditTagChanges: Object.fromEntries([[0, '']]),
                policy,
                reports,
                transactions,
                reportActions: {},
            ...
            // The empty clear reaches the transaction (tag set to '') rather than the update being dropped.
            expect(getBulkEditUpdates(writeSpy, 0).tag).toBe('');
