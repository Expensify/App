...
jest.mock("@hooks/useCardFeedsForDisplay", () =>
  jest.fn(() => ({ defaultCardFeed: null, cardFeedsByPolicy: {} })),
);
...
jest.mock("@src/libs/SearchQueryUtils", () => {
  ...
  return {
    ...
    getCurrentSearchQueryJSON: jest.fn().mockImplementation(() => ({
      ...
      flatFilters: [
        { key: "reimbursable", filters: [{ operator: "eq", value: "yes" }] },
      ],
      inputQuery: "",
      recentSearchHash: 89,
...
jest.mock("@libs/PolicyUtils", () => ({
  ...
  isPolicyOwner: jest
    .fn()
    .mockImplementation(
      (policy?: OnyxEntry<Policy>, currentUserAccountID?: number) =>
        !!currentUserAccountID &&
        policy?.ownerAccountID === currentUserAccountID,
    ),
}));
...
describe("actions/IOU", () => {
  ...
  describe("requestMoney", () => {
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allReports) => {
                  ...
                  const chatReports = Object.values(allReports ?? {}).filter(
                    (report) => report?.type === CONST.REPORT.TYPE.CHAT,
                  );
                  const iouReports = Object.values(allReports ?? {}).filter(
                    (report) => report?.type === CONST.REPORT.TYPE.IOU,
                  );
                ...
                callback: (iouReportMetadata) => {
                  ...
                  const loadingStateConnection = Onyx.connect({
                    ...
                    callback: (iouReportLoadingState) => {
                      ...
                      expect(
                        iouReportLoadingState?.hasOnceLoadedReportActions,
                      ).toBe(true);
                      resolve();
                    },
                  });
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (reportActionsForIOUReport) => {
                  ...
                  expect(
                    Object.values(reportActionsForIOUReport ?? {}).length,
                  ).toBe(2);
                  const createdActions = Object.values(
                  ...
                  const iouActions = Object.values(
                    reportActionsForIOUReport ?? {},
                  ).filter(
                    (
                      reportAction,
                    ): reportAction is ReportAction<
                      typeof CONST.REPORT.ACTIONS.TYPE.IOU
                  ...
                  const originalMessage = isMoneyRequestAction(iouAction)
                    ? getOriginalMessage(iouAction)
                    : undefined;

                  // The CREATED action should not be created after the IOU action
                  expect(Date.parse(createdAction?.created ?? "")).toBeLessThan(
                    Date.parse(iouAction?.created ?? ""),
                  );
                  ...
                  expect(originalMessage?.type).toBe(
                    CONST.IOU.REPORT_ACTION_TYPE.CREATE,
                  );

                  // Both actions should be pending
                  expect(createdAction?.pendingAction).toBe(
                    CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD,
                  );
                  expect(iouAction?.pendingAction).toBe(
                    CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD,
                  );
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (reportActionsForTransactionThread) => {
                  ...
                  const createdActions = Object.values(
                    reportActionsForTransactionThread ?? {},
                  ).filter(
                    (reportAction) =>
                      reportAction?.actionName ===
                      CONST.REPORT.ACTIONS.TYPE.CREATED,
                  ...
                  expect(transactionThreadCreatedAction?.pendingAction).toBe(
                    CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD,
                  );
                  resolve();
                },
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allTransactions) => {
                  ...
                  const transaction = Object.values(allTransactions ?? []).find(
                    (t) => !isEmptyObject(t),
                  );
                  ...
                  expect(transaction?.pendingAction).toBe(
                    CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD,
                  );
                  ...
                  expect(
                    iouAction &&
                      getOriginalMessage(iouAction)?.IOUTransactionID,
                  ).toBe(transactionID);
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (reportActionsForIOUReport) => {
                  ...
                  for (const reportAction of Object.values(
                    reportActionsForIOUReport ?? {},
                  )) {
    ...
    it("updates existing chat report if there is one", () => {
      ...
      mockFetch?.pause?.();
      return Onyx.set(
        `${ONYXKEYS.COLLECTION.REPORT}${chatReport.reportID}`,
        chatReport,
      )
        .then(() =>
          Onyx.set(
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allReports) => {
                  ...
                  ).toBe(chatReport.reportID);
                  chatReport =
                    Object.values(allReports ?? {}).find(
                      (report) => report?.type === CONST.REPORT.TYPE.CHAT,
                    ) ?? chatReport;
                  const iouReport = Object.values(allReports ?? {}).find(
                    (report) => report?.type === CONST.REPORT.TYPE.IOU,
                  );
                  iouReportID = iouReport?.reportID;
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allIOUReportActions) => {
                  ...
                  iouCreatedAction = Object.values(
                    allIOUReportActions ?? {},
                  ).find(
                    (reportAction) =>
                      reportAction.actionName ===
                  ...
                  // The CREATED action should not be created after the IOU action
                  expect(
                    Date.parse(iouCreatedAction?.created ?? ""),
                  ).toBeLessThan(Date.parse(iouAction?.created ?? ""));
    ...
    it("updates existing IOU report if there is one", () => {
      ...
      return Onyx.set(
        ...
        .then(() =>
          Onyx.set(
            `${ONYXKEYS.COLLECTION.TRANSACTION}${existingTransaction.transactionID}`,
            existingTransaction,
          ),
        )
        .then(() => {
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allReports) => {
                  ...
                  expect(
                    Object.values(allReports ?? {}).find(
                      (report) => report?.reportID === chatReportID,
                    ),
                  ).toBeTruthy();
                  expect(
                    Object.values(allReports ?? {}).find(
                      (report) => report?.reportID === iouReportID,
                    ),
                  ).toBeTruthy();
                  ...
                  iouReport = Object.values(allReports ?? {}).find(
                    (report) => report?.type === CONST.REPORT.TYPE.IOU,
                  );
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (reportActionsForIOUReport) => {
                  ...
                  const newOriginalMessage = newIOUAction
                    ? getOriginalMessage(newIOUAction)
                    : null;
                  ...
                  expect(newOriginalMessage?.type).toBe(
                    CONST.IOU.REPORT_ACTION_TYPE.CREATE,
                  );

                  // The IOU action should be pending
                  expect(newIOUAction?.pendingAction).toBe(
                    CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD,
                  );
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allTransactions) => {
                  ...
                  newTransaction = Object.values(allTransactions ?? {}).find(
                    (transaction) =>
                  ...
                  expect(newTransaction?.merchant).toBe(
                    CONST.TRANSACTION.DEFAULT_MERCHANT,
                  );
                  expect(newTransaction?.pendingAction).toBe(
                    CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD,
                  );

                  // The transactionID on the iou action should match the one from the transactions collection
                  expect(
                    isMoneyRequestAction(newIOUAction)
                      ? getOriginalMessage(newIOUAction)?.IOUTransactionID
                      : undefined,
                  ).toBe(newTransaction?.transactionID);
              ...
              new Promise<void>((resolve) => {
                const connection = Onyx.connect({
                  ...
                  callback: (reportActionsForIOUReport) => {
                    ...
                    // The CREATED action should not be created after the IOU action
                    expect(
                      Date.parse(createdAction?.created ?? ""),
                    ).toBeLessThan(Date.parse(iouAction?.created ?? ""));
              ...
              new Promise<void>((resolve) => {
                const connection = Onyx.connect({
                  ...
                  callback: (allTransactions) => {
                    ...
                    // There should be one transaction
                    expect(Object.values(allTransactions ?? {}).length).toBe(1);
                    const transaction = Object.values(
                      allTransactions ?? {},
                    ).find((t) => !isEmptyObject(t));
                    transactionID = transaction?.transactionID;
                ...
                const connection = Onyx.connect({
                  ...
                  callback: (reportActionsForTransactionThread) => {
                    ...
                    transactionThreadAction = Object.values(
                      reportActionsForTransactionThread?.[
                        `${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${transactionThreadReport?.reportID}`
                      ] ?? {},
                    ).find(
                      (reportAction) =>
                        reportAction.actionName ===
                    ...
                    expect(transactionThreadAction?.pendingAction).toBe(
                      CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD,
                    );
                    resolve();
                  },
              ...
              new Promise<void>((resolve) => {
                const connection = Onyx.connect({
                  ...
                  callback: (transaction) => {
                    ...
                    expect(
                      Object.values(transaction?.errors ?? {}).at(0),
                    ).toEqual(
                      translateLocal("iou.error.genericCreateFailureMessage"),
                    );
                    resolve();
                  },
                });
    ...
    it("correctly implements RedBrickRoad error handling for ShareTrackedExpense when inviting new user to workspace", async () => {
      ...
      await Promise.all([
        Onyx.set(
          `${ONYXKEYS.COLLECTION.REPORT}${selfDMReport.reportID}`,
          selfDMReport,
        ),
        Onyx.set(
          `${ONYXKEYS.COLLECTION.REPORT}${policyExpenseChat.reportID}`,
          policyExpenseChat,
        ),
      ...
      const recentWaypoints =
        (await getOnyxValue(ONYXKEYS.NVP_RECENT_WAYPOINTS)) ?? [];
      ...
      await getOnyxData({
        ...
        waitForCollectionCallback: true,
        callback: (reports) => {
          const selfDMReportOnyx = Object.values(reports ?? {}).find(
            (report) => report?.reportID === selfDMReport.reportID,
          );
          selfDMReportID = selfDMReportOnyx?.reportID;
        },
      });

      const reportActions = await getOnyxValue(
        `${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${selfDMReportID}`,
      );
      const actions = Object.values(reportActions ?? {});
      const linkedTrackedExpenseReportAction = actions.find(
        (action) => action && isMoneyRequestAction(action),
      );
      const actionableWhisperReportActionID = actions.find(
        (action) => action && isActionableTrackExpense(action),
      )?.reportActionID;
      ...
      const policyData = await getOnyxValue(
      ...
      expect(Object.values(accountantEmployee?.errors ?? {}).at(0)).toEqual(
        translateLocal("workspace.people.error.genericAdd"),
      );
    ...
    it("increase the nonReimbursableTotal only when the expense is not reimbursable", async () => {
      ...
      await Onyx.set(
        `${ONYXKEYS.COLLECTION.REPORT}${workspaceChat.reportID}`,
        workspaceChat,
      );
    ...
    it("should update policyRecentlyUsedTags when tag is provided", async () => {
      ...
      await Onyx.merge(
        `${ONYXKEYS.COLLECTION.POLICY_RECENTLY_USED_TAGS}${policyID}`,
      ...
      const newPolicyRecentlyUsedTags: RecentlyUsedTags = await new Promise(
        (resolve) => {
    ...
    it("should use personalDetails to create expense with participant display name", async () => {
      ...
      const createdIouReport = await getOnyxValue(
    ...
    it("should update the parentReportID and parentReportActionID of the transactionThreadReport of the transaction when submitted to another report", async () => {
      ...
      const allReports = await new Promise<OnyxCollection<Report>>(
        (resolve) => {
          const connection = Onyx.connect({
            key: ONYXKEYS.COLLECTION.REPORT,
            waitForCollectionCallback: true,
      ...
      const transactionThreadReport = Object.values(allReports ?? {}).find(
        (report) =>
          ...
          report?.parentReportID === selfDMReport.reportID,
      );
      ...
      const allReportActions = await new Promise<OnyxCollection<ReportActions>>(
        (resolve) => {
          const connection = Onyx.connect({
            key: ONYXKEYS.COLLECTION.REPORT_ACTIONS,
            waitForCollectionCallback: true,
      ...
      const reportActionsForIOUReport =
        allReportActions?.[
          `${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${selfDMReport.reportID}`
        ];
      const createIOUAction = Object.values(
        reportActionsForIOUReport ?? {},
      ).find(
        (
          reportAction,
      ...
      expect(createIOUAction?.childReportID).toBe(
        transactionThreadReport?.reportID,
      );

      // When fetching all transactions from Onyx
      const allTransactions = await new Promise<OnyxCollection<Transaction>>(
        (resolve) => {
          const connection = Onyx.connect({
            key: ONYXKEYS.COLLECTION.TRANSACTION,
            waitForCollectionCallback: true,
      ...
      expect(
        createIOUAction &&
          getOriginalMessage(createIOUAction)?.IOUTransactionID,
      ...
      const updatedTransactionThreadReport = await new Promise<
        OnyxEntry<Report>
      >((resolve) => {
        const connection = Onyx.connect({
          key: ONYXKEYS.COLLECTION.REPORT,
          ...
          callback: (reports) => {
            ...
            resolve(
              reports?.[
                `${ONYXKEYS.COLLECTION.REPORT}${transactionThreadReport?.reportID}`
              ],
            );
          },
        });
      ...
      const iouReportActionID = getIOUActionForReportID(
        iouReport?.reportID,
        transaction?.transactionID,
      )?.reportActionID;
      expect(updatedTransactionThreadReport).toBeTruthy();
      expect(updatedTransactionThreadReport?.parentReportID).toBe(
        iouReport?.reportID,
      );
      expect(updatedTransactionThreadReport?.parentReportActionID).toBe(
        iouReportActionID,
      );

      // Also, the fromReportID of movedTransactionAction should be CONST.REPORT.UNREPORTED_REPORT_ID
      const updatedTransactionThreadReportActions = getAllReportActions(
        transactionThreadReport?.reportID,
      );
      const movedTransactionAction = Object.values(
        updatedTransactionThreadReportActions ?? {},
      ).find(
        (reportAction) =>
          reportAction?.actionName ===
      ...
      const originalMessage = getOriginalMessage(movedTransactionAction) as
        | OriginalMessageMovedTransaction
        | undefined;
      expect(originalMessage?.fromReportID).toBe(
    ...
    it("creates new chat report when participant does not match existing chat report participants", () => {
      ...
      return Onyx.set(
        `${ONYXKEYS.COLLECTION.REPORT}${existingChatReport.reportID}`,
        existingChatReport,
      )
        .then(() => {
              ...
              const connection = Onyx.connect({
                ...
                callback: (allReports) => {
                  ...
                  const newChatReport = chatReports.find((report) => {
                    ...
                    return (
                      participantKeys.includes(RORY_ACCOUNT_ID) &&
                      participantKeys.includes(CARLOS_ACCOUNT_ID) &&
                  ...
                  expect(newChatReport?.reportID).not.toBe(
                    existingChatReport.reportID,
                  );
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allReports) => {
                  ...
                  // Find the chat report that has RORY and CARLOS as participants
                  const chatReportWithParticipants = chatReports.find(
                    (report) => {
                      const participantKeys = Object.keys(
                        report?.participants ?? {},
                      ).map(Number);
                  ...
                  // The existing chat report should be reused
                  expect(chatReportWithParticipants?.reportID).toBe(
                    existingChatReport.reportID,
                  );
    ...
    it("skips participant validation for policy expense chat participant", () => {
      ...
      return Onyx.set(
        `${ONYXKEYS.COLLECTION.REPORT}${policyExpenseChatReport.reportID}`,
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allReports) => {
                  ...
                  const policyExpenseChats = Object.values(
                    allReports ?? {},
                  ).filter(
                    (report) =>
                      report?.chatType ===
                  ...
                  expect(
                    policyExpenseChats.some(
                      (report) =>
                        report?.reportID === policyExpenseChatReport.reportID,
                    ),
                  ).toBe(true);
            ...
            new Promise<void>((resolve) => {
              const connection = Onyx.connect({
                ...
                callback: (allReports) => {
                  ...
                  const selfDMReports = Object.values(allReports ?? {}).filter(
                    (report) =>
                  ...
                  const chatReportsWithZeroParticipant = Object.values(
                    allReports ?? {},
                  ).filter((report) => {
                    if (
                      report?.type !== CONST.REPORT.TYPE.CHAT ||
  ...
  describe("should have valid parameters", () => {
    ...
    test.each([
      ...
      [
        WRITE_COMMANDS.CONVERT_TRACKED_EXPENSE_TO_REQUEST,
      ...
      for (const value of Object.values(params as Record<string, unknown>)) {
        expect(
          Array.isArray(value) ? value.every(isValid) : isValid(value),
        ).toBe(true);
      }
    });
    ...
    it("propagates the capture-time receiptTraceId into the final request params", async () => {
      ...
      const isFileUploadableSpy = jest
        .spyOn(IsFileUploadable, "default")
    ...
    it("adds grouped from snapshot optimistic data for grouped search queries", async () => {
      const currentSearchQueryJSON = {
        ...
        inputQuery:
          "sortBy:date sortOrder:desc type:expense groupBy:from status:drafts,outstanding",
        flatFilters: [],
        hash: 71801560,
      ...
      const getCurrentSearchQueryJSONSpy = jest
        .spyOn(SearchQueryUtils, "getCurrentSearchQueryJSON")
        .mockReturnValue(currentSearchQueryJSON);
      ...
      expect(writeSpy).toHaveBeenCalledTimes(1);
      const [, , requestData] = writeSpy.mock.calls.at(0) as [
        ApiCommand,
      ...
      expect(
        optimisticData.some((update) => update.key === mainSnapshotKey),
      ).toBeTruthy();

      const newFlatFilters = currentSearchQueryJSON.flatFilters.filter(
        (filter) => filter.key !== CONST.SEARCH.SYNTAX_FILTER_KEYS.FROM,
      );
      newFlatFilters.push({
      ...
      const groupedTransactionsQueryJSON =
        SearchQueryUtils.buildSearchQueryJSON(
      ...
      if (!groupedTransactionsQueryJSON) {
        throw new Error(
          "Expected grouped transactions query JSON to be defined",
        );
      }
      const groupedSnapshotKey = `${ONYXKEYS.COLLECTION.SNAPSHOT}${groupedTransactionsQueryJSON.hash}`;
      expect(
        optimisticData.some((update) => update.key === groupedSnapshotKey),
      ).toBeTruthy();
    ...
    test.each([
      ...
      trackExpense({
        ...
        accountantParams:
          action === CONST.IOU.ACTION.SHARE
            ? { accountant: { accountID: VIT_ACCOUNT_ID, login: VIT_EMAIL } }
            : undefined,
        isASAPSubmitBetaEnabled: false,
        currentUser: { accountID: RORY_ACCOUNT_ID, email: RORY_EMAIL },
        introSelected: undefined,
