...
describe('actions/Passkey', () => {
    ...
    describe('addLocalPasskeyCredential', () => {
        ...
        it('should add credential to existing entry', async () => {
            const existingEntry: LocalPasskeyEntry = {credentialIds: [{id: 'existing', type: CONST.PASSKEY_CREDENTIAL_TYPE, transports: ['internal']}]};

            addLocalPasskeyCredential({userId, rpId, credential, existingEntry});
            await waitForBatchedUpdates();

            const value = await getOnyxValue(getPasskeyOnyxKey(userId, rpId));
            expect(value).toEqual({credentialIds: [{id: 'existing', type: CONST.PASSKEY_CREDENTIAL_TYPE, transports: ['internal']}, credential]});
        });

        it('should update existing credential when id matches', async () => {
            const existingEntry: LocalPasskeyEntry = {credentialIds: [{id: 'cred-1', type: CONST.PASSKEY_CREDENTIAL_TYPE, transports: ['internal']}]};
            const updatedCredential: PasskeyCredential = {id: 'cred-1', type: CONST.PASSKEY_CREDENTIAL_TYPE, transports: ['internal', 'hybrid']};
    ...
    describe('reconcileLocalPasskeysWithBackend', () => {
        ...
        it('should remove local credentials not on backend and update Onyx', async () => {
            ...
            const value = await getOnyxValue(getPasskeyOnyxKey(userId, rpId));
            expect(value).toEqual({credentialIds: [{id: 'cred-1', type: CONST.PASSKEY_CREDENTIAL_TYPE, transports: ['internal']}]});
        });
