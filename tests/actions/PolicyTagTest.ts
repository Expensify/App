...
describe('actions/Policy', () => {
    ...
    describe('RenamePolicyTag', () => {
        ...
        it('should update approval rules when tag is used in approval conditions', async () => {
            ...
            expect(policyData.current.policy?.rules?.approvalRules?.[0]?.applyWhen?.[0]?.value).toBe(newTagName);
    ...
    describe('EnablePolicyTags', () => {
        it('should enable tags and create default tag list if none exists', async () => {
            ...
            const defaultTagList = Object.values(policyData.current?.tagLists ?? {}).at(0);
            expect(defaultTagList?.name).toBe('Tag');
            expect(defaultTagList?.orderWeight).toBe(0);
            expect(defaultTagList?.required).toBe(false);
            expect(defaultTagList?.tags).toEqual({});
            mockFetch.resume();
            await waitForBatchedUpdates();
        ...
        it('should disable tags and update existing tag list', async () => {
            ...
            Object.keys(existingTags).forEach((tagName) => {
                expect(policyData.current?.tagLists?.[tagListName]?.tags[tagName]?.enabled).toBe(false);
            });
        ...
        it('should reset changes when API returns error', async () => {
            ...
            expect(policyData.current?.tagLists).toMatchObject({});
        });
        ...
        it('should work with data from useOnyx hook', async () => {
            ...
            expect(policyData?.current?.tagLists?.Tag).toBeDefined();
        });
    });
    ...
    describe('SetPolicyTagsRequired', () => {
        it('should set tag list as required when requiresTag is true', async () => {
            ...
            let updatedPolicyTags = policyData?.current?.tagLists;
            ...
            updatedPolicyTags = policyData?.current?.tagLists;
        ...
        it('should handle API failure and restore original state with error', async () => {
            ...
            const updatedPolicyTags = policyData?.current?.tagLists;
            expect(updatedPolicyTags?.[tagListName]?.required).toBe(false);
            expect(updatedPolicyTags?.[tagListName]?.pendingFields?.required).toBeUndefined();
