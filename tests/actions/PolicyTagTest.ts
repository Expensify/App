...
describe('actions/Policy', () => {
    ...
    describe('CreatePolicyTag', () => {
        ...
        it('clears tags optimistically when switching tag levels', async () => {
            const fakePolicy = createRandomPolicy(0);
            fakePolicy.areTagsEnabled = true;
            fakePolicy.requiresTag = true;

            const tagListName = 'Tag';
            const existingPolicyTags = createRandomPolicyTags(tagListName, 1);
            existingPolicyTags[tagListName].required = true;
            ...
            cleanPolicyTags({
                ...
                shouldRestoreRequiresTagAfterTagCreate: true,
                requiresTag: fakePolicy.requiresTag,
            });
            await waitForBatchedUpdates();
            ...
            expect(policyTags).toEqual(CONST.POLICY.DEFAULT_TAG_LIST);

            mockFetch.resume();
            await waitForBatchedUpdates();
        });

        it('enables requiresTag when creating a tag offline after required tags were cleared by switching tag levels', async () => {
            const fakePolicy = createRandomPolicy(0);
            fakePolicy.areTagsEnabled = true;
            fakePolicy.requiresTag = true;
            ...
            const tagListName = 'Tag';
            const newTagName = 'new tag';
            const requiredPolicyTags = createRandomPolicyTags(tagListName, 1);
            requiredPolicyTags[tagListName].required = true;

            await Onyx.set(`${ONYXKEYS.COLLECTION.POLICY}${fakePolicy.id}`, fakePolicy);
            await Onyx.set(`${ONYXKEYS.COLLECTION.POLICY_TAGS}${fakePolicy.id}`, requiredPolicyTags);
            ...
            await Onyx.set(`${ONYXKEYS.COLLECTION.POLICY_TAGS}${fakePolicy.id}`, {
                [tagListName]: {
                    name: tagListName,
                    orderWeight: 0,
                    required: false,
            ...
            expect(policyTags?.[tagListName]?.tags?.[newTagName]?.enabled).toBe(true);

            mockFetch.resume();
            await waitForBatchedUpdates();
        });

        it('does not restore requiresTag after the user turns it off before creating a tag offline', async () => {
            const fakePolicy = createRandomPolicy(0);
            fakePolicy.areTagsEnabled = true;
            fakePolicy.requiresTag = true;
            ...
            expect(policy?.requiresTag).toBe(false);
            expect(policyTags?.[tagListName]?.required).toBe(false);
            expect(policyTags?.[tagListName]?.tags?.[newTagName]?.enabled).toBe(true);
