...
jest.mock('@src/libs/API', () => {
    ...
    return {
        ...
        writeWithNoDuplicatesConflictAction: jest.fn(actual.writeWithNoDuplicatesConflictAction),
    };
});
...
jest.mock('@src/libs/actions/Session', () => {
    const actual = jest.requireActual<typeof SessionUtil>('@src/libs/actions/Session');
    return {
        ...actual,
        isSupportAuthToken: jest.fn(actual.isSupportAuthToken),
        hasStashedSession: jest.fn(actual.hasStashedSession),
        signOut: jest.fn(actual.signOut),
    };
});

jest.mock('@libs/Network/NetworkStore', () => {
    const actual = jest.requireActual<typeof NetworkStore>('@libs/Network/NetworkStore');
    return {
        ...actual,
...
describe('Session', () => {
    ...
    test('SignOut should clear native startup prefetch state before LOG_OUT', async () => {
        ...
        const makeRequestSpy = jest.mocked(API.makeRequestWithSideEffects);
    ...
    describe('SignOutAndRedirectToSignIn', () => {
        ...
        test('SignOutAndRedirectToSignIn should restore stashed session and redirect to OldDot supportal of supportal agent', async () => {
            jest.mocked(SessionUtil.isSupportAuthToken).mockReturnValue(true);
            jest.mocked(SessionUtil.hasStashedSession).mockReturnValue(true);
            jest.mocked(SessionUtil.signOut).mockResolvedValue(undefined);
        ...
        test('SignOutAndRedirectToSignIn should preserve SESSION and restore stashed session when shouldForceUseStashedSession is true', async () => {
            jest.mocked(SessionUtil.isSupportAuthToken).mockReturnValue(false);
            jest.mocked(SessionUtil.hasStashedSession).mockReturnValue(true);
            jest.mocked(SessionUtil.signOut).mockResolvedValue(undefined);
            const onyxClearSpy = jest.spyOn(Onyx, 'clear').mockResolvedValue(undefined);
            const onyxMultiSetSpy = jest.spyOn(Onyx, 'multiSet').mockResolvedValue(undefined);
    ...
    describe('validateTwoFactorAuth', () => {
        test('forced onboarding path updates auth token before clearing Onyx without openApp', async () => {
            ...
            const writeWithNoDuplicatesSpy = jest.mocked(API.writeWithNoDuplicatesConflictAction);
            writeWithNoDuplicatesSpy.mockResolvedValue(undefined);
