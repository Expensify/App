...
describe('Session', () => {
    ...
    test('SignOut should clear native startup prefetch state before LOG_OUT', async () => {
        ...
        expect(makeRequestSpy).toHaveBeenCalledWith(SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT, expect.objectContaining({authToken: 'testAuthToken'}), {}, undefined);
    ...
    describe('SignOutAndRedirectToSignIn', () => {
        test('SignOutAndRedirectToSignIn should skip the LogOut but still clear and redirect when there is no auth token', async () => {
            // Given no session — a QA crossing made from the sign-in screen reaches here with no auth token
            const makeRequestSpy = jest.spyOn(API, 'makeRequestWithSideEffects').mockResolvedValue(undefined);
            const redirectToSignInSpy = jest.spyOn(SignInRedirect, 'default').mockImplementation(() => Promise.resolve());
            ...
            // Then nothing is sent to a server that has no token to invalidate, yet the credentials of the
            // database being left are still cleared
            expect(makeRequestSpy.mock.calls.some(([command]) => command === SIDE_EFFECT_REQUEST_COMMANDS.LOG_OUT)).toBe(false);
    ...
    describe('SAML sign out', () => {
        ...
        test('SignOut should call openAuthSessionAsync without appversion when signedInWithSAML is true on web', async () => {
            ...
            expect(makeRequestSpy).toHaveBeenCalledWith(
                ...
                expect.objectContaining({authToken: 'testAuthToken', partnerUserID: 'testLogin'}),
                {},
                undefined,
            );
