...
describe('Session', () => {
    ...
    describe('setupNewDotAfterTransitionFromOldDot', () => {
        ...
        test('writes the passed credentials into CREDENTIALS and STASHED_CREDENTIALS instead of reading the module cache', async () => {
            ...
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettings(true), undefined, credentialsParam, undefined, undefined);
            await waitForBatchedUpdates();
        ...
        test('falls back to the OldDot delegate credentials when the passed credentials are undefined', async () => {
            ...
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettings(true), undefined, undefined, undefined, undefined);
            await waitForBatchedUpdates();
        ...
        test('does not stash the passed credentials when the transition is not a delegate access', async () => {
            ...
            const credentialsParam = {login: 'nd@user.com', autoGeneratedLogin: 'ndAutoLogin', autoGeneratedPassword: 'ndAutoPassword'};
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettings(false), undefined, credentialsParam, undefined, undefined);
            await waitForBatchedUpdates();
        ...
        const buildHybridAppSettingsWithStagingServer = () => ({
            ...buildHybridAppSettings(false),
            // OldDot defaults this to false on every release build, and it sends it on every transition
            [ONYXKEYS.SHOULD_USE_STAGING_SERVER]: false,
        });

        test('keeps the NewDot staging server preference when the user has already picked one', async () => {
            await Onyx.set(ONYXKEYS.IS_USING_IMPORTED_STATE, true);
            await waitForBatchedUpdates();

            const onyxUpdateSpy = jest.spyOn(Onyx, 'update').mockResolvedValue(undefined);

            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingServer(), undefined, undefined, true, undefined);
            await waitForBatchedUpdates();

            const updatedKeys = onyxUpdateSpy.mock.calls
                .at(0)
                ?.at(0)
                ?.map((update) => update.key);
            expect(updatedKeys).not.toContain(ONYXKEYS.SHOULD_USE_STAGING_SERVER);

            onyxUpdateSpy.mockRestore();
        });

        test('takes the OldDot staging server value when NewDot has no preference yet', async () => {
            await Onyx.set(ONYXKEYS.IS_USING_IMPORTED_STATE, true);
            await waitForBatchedUpdates();

            const onyxUpdateSpy = jest.spyOn(Onyx, 'update').mockResolvedValue(undefined);

            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingServer(), undefined, undefined, undefined, undefined);
            await waitForBatchedUpdates();

            expect(onyxUpdateSpy.mock.calls.at(0)?.at(0)).toEqual(expect.arrayContaining([expect.objectContaining({key: ONYXKEYS.SHOULD_USE_STAGING_SERVER, value: false})]));

            onyxUpdateSpy.mockRestore();
        });

        const buildHybridAppSettingsWithStagingToggleGeneration = (generation: number) => {
            const settings = buildHybridAppSettingsWithStagingServer();
            return {
                ...settings,
                [ONYXKEYS.HYBRID_APP]: {
                    ...settings[ONYXKEYS.HYBRID_APP],
                    // OldDot bumps this only when the user deliberately flips its staging toggle
                    stagingToggleGeneration: generation,
                },
            };
        };

        test('takes a deliberate OldDot staging toggle change and records its generation', async () => {
            await Onyx.set(ONYXKEYS.IS_USING_IMPORTED_STATE, true);
            await waitForBatchedUpdates();

            const onyxUpdateSpy = jest.spyOn(Onyx, 'update').mockResolvedValue(undefined);

            // NewDot has staging on, but OldDot's toggle was flipped off since the last accepted generation (1)
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingToggleGeneration(2), undefined, undefined, true, 1);
            await waitForBatchedUpdates();

            expect(onyxUpdateSpy.mock.calls.at(0)?.at(0)).toEqual(
                expect.arrayContaining([
                    expect.objectContaining({key: ONYXKEYS.SHOULD_USE_STAGING_SERVER, value: false}),
                    expect.objectContaining({key: ONYXKEYS.LAST_ACCEPTED_OLD_DOT_STAGING_TOGGLE_GENERATION, value: 2}),
                ]),
            );
        ...
        test('ignores an OldDot staging toggle generation that has already been accepted', async () => {
            await Onyx.set(ONYXKEYS.IS_USING_IMPORTED_STATE, true);
            await waitForBatchedUpdates();

            const onyxUpdateSpy = jest.spyOn(Onyx, 'update').mockResolvedValue(undefined);

            // The same generation arrives again, e.g. redelivered on a later transition
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingToggleGeneration(2), undefined, undefined, true, 2);
            await waitForBatchedUpdates();
            ...
            const updatedKeys = onyxUpdateSpy.mock.calls
                ...
                ?.map((update) => update.key);
            expect(updatedKeys).not.toContain(ONYXKEYS.SHOULD_USE_STAGING_SERVER);
            expect(updatedKeys).not.toContain(ONYXKEYS.LAST_ACCEPTED_OLD_DOT_STAGING_TOGGLE_GENERATION);

            onyxUpdateSpy.mockRestore();
        });

        test('re-baselines the accepted generation when OldDot state was reset, without taking the ambient value', async () => {
            await Onyx.set(ONYXKEYS.IS_USING_IMPORTED_STATE, true);
            await waitForBatchedUpdates();

            const onyxUpdateSpy = jest.spyOn(Onyx, 'update').mockResolvedValue(undefined);

            // OldDot wiped its state (sign-out), so its generation restarted below the accepted one (5)
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingToggleGeneration(0), undefined, undefined, true, 5);
            await waitForBatchedUpdates();

            const updates = onyxUpdateSpy.mock.calls.at(0)?.at(0);
            expect(updates).toEqual(expect.arrayContaining([expect.objectContaining({key: ONYXKEYS.LAST_ACCEPTED_OLD_DOT_STAGING_TOGGLE_GENERATION, value: 0})]));
