...
describe('Session', () => {
    ...
    describe('setupNewDotAfterTransitionFromOldDot', () => {
        ...
        test('writes the passed credentials into CREDENTIALS and STASHED_CREDENTIALS instead of reading the module cache', async () => {
            ...
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettings(true), undefined, credentialsParam, undefined);
            await waitForBatchedUpdates();
        ...
        test('falls back to the OldDot delegate credentials when the passed credentials are undefined', async () => {
            ...
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettings(true), undefined, undefined, undefined);
            await waitForBatchedUpdates();
        ...
        test('does not stash the passed credentials when the transition is not a delegate access', async () => {
            ...
            const credentialsParam = {login: 'nd@user.com', autoGeneratedLogin: 'ndAutoLogin', autoGeneratedPassword: 'ndAutoPassword'};
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettings(false), undefined, credentialsParam, undefined);
            await waitForBatchedUpdates();
        ...
        test('never merges the ambient OldDot staging value, so a fresh install keeps the env-based default', async () => {
            // OldDot's default is production on every release binary; merging it would plant an explicit false
            // that overrides ApiUtils' staging default on staging builds and reset a preserved NewDot preference.
            await Onyx.set(ONYXKEYS.IS_USING_IMPORTED_STATE, true);
            await waitForBatchedUpdates();

            const onyxUpdateSpy = jest.spyOn(Onyx, 'update').mockResolvedValue(undefined);

            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingServer(), undefined, undefined, undefined);
            await waitForBatchedUpdates();
        ...
        test('takes a deliberate OldDot staging toggle change and records its generation', async () => {
            ...
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingToggleGeneration(2), undefined, undefined, 1);
            await waitForBatchedUpdates();
        ...
        test('ignores an OldDot staging toggle generation that has already been accepted', async () => {
            ...
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingToggleGeneration(2), undefined, undefined, 2);
            await waitForBatchedUpdates();
        ...
        test('re-baselines the accepted generation when OldDot state was reset, without taking the ambient value', async () => {
            ...
            await SessionUtil.setupNewDotAfterTransitionFromOldDot(buildHybridAppSettingsWithStagingToggleGeneration(0), undefined, undefined, 5);
            await waitForBatchedUpdates();
