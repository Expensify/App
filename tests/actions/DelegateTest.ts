        ...
        it('keeps IS_LOADING_APP=true after the clear so DelegateAccessHandler does not see HAS_LOADED_APP=true && IS_LOADING_APP=undefined and fire a duplicate openApp', async () => {
            ...
            await Onyx.multiSet({
                ...
                [ONYXKEYS.CREDENTIALS]: {
                    autoGeneratedLogin: 'delegate-login',
                    autoGeneratedPassword: 'delegate-password',
                },
            ...
            // Credentials must be preserved so delegate-token reauthentication can restore the original account.
            await new Promise<void>((resolve) => {
                const conn = Onyx.connect({
                    key: ONYXKEYS.CREDENTIALS,
                    callback: (value) => {
                        expect(value?.autoGeneratedLogin).toBe('delegate-login');
                        expect(value?.autoGeneratedPassword).toBe('delegate-password');
                        Onyx.disconnect(conn);
                        resolve();
                    },
                });
