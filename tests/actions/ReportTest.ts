...
jest.mock('@libs/ReportUtils', () => {
    ...
    return {
        ...
        getPolicyExpenseChat: jest.fn().mockImplementation(() => ({
            reportID: MOCKED_POLICY_EXPENSE_CHAT_REPORT_ID,
...
describe('actions/Report', () => {
    ...
    it('clearCreateChatError should not delete the report if it is not optimistic report', () => {
        ...
        const REPORT_METADATA: OnyxTypes.ReportMetadata = {
    ...
    it('clearCreateChatError should pass introSelected when navigating to concierge for optimistic report', () => {
        ...
        const CONCIERGE_REPORT_ID = '123456';
        const INTRO_SELECTED: OnyxTypes.IntroSelected = {
            choice: CONST.ONBOARDING_CHOICES.MANAGE_TEAM,
        };
    ...
    it('should be updated correctly when new comments are added, deleted or marked as unread', () => {
        ...
        const setPromise = Onyx.set(`${ONYXKEYS.COLLECTION.REPORT}${REPORT_ID}`, {
            reportName: 'Test',
            reportID: REPORT_ID,
        })
    ...
    it('should optimistically mark an unresolved ACTIONABLE_MENTION_WHISPER as deleted when its parent comment is deleted', async () => {
        ...
        await Onyx.mergeCollection(ONYXKEYS.COLLECTION.REPORT_ACTIONS, {
            [`${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${REPORT_ID}`]: {
                ...
                [WHISPER_ACTION_ID]: {
                    ...
                    originalMessage: {
                        inviteeAccountIDs: [123],
                        inviteeEmails: ['test@example.com'],
                        whisperedTo: [1],
                    },
                },
            },
        ...
        expect(getOriginalMessage(whisper)).toMatchObject({
    ...
    it('should only delete the whisper linked to the deleted comment, not other whispers in the same report', async () => {
        ...
        await Onyx.mergeCollection(ONYXKEYS.COLLECTION.REPORT_ACTIONS, {
            [`${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${REPORT_ID}`]: {
                ...
                [WHISPER_B_ID]: {
                    ...
                    originalMessage: {
                        inviteeAccountIDs: [222],
                        inviteeEmails: ['b@example.com'],
                        whisperedTo: [1],
                    },
                },
            },
        ...
        const reportActions = await getOnyxValue(`${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${REPORT_ID}` as const);
        expect(getOriginalMessage(reportActions?.[WHISPER_B_ID])).toMatchObject({
            deleted: expect.any(String),
        });
    ...
    describe('deleteReportComment SEARCH_REPORT navigation', () => {
        ...
        const mockNavigation: {
            isActiveRoute: jest.Mock;
            goBack: jest.Mock;
            getTopmostSearchReportRouteParams: jest.Mock;
        ...
        it('calls goBack when viewing the child thread at search/view/<childReportID>', async () => {
            ...
            mockNavigation.getTopmostSearchReportRouteParams.mockReturnValue({
                reportID: CHILD_REPORT_ID,
            });
    ...
    describe('deleteAppReport', () => {
        ...
        it('should not reset the chatReport hasOutstandingChildRequest if there is another outstanding report', async () => {
            ...
            const transaction: OnyxTypes.Transaction = {
                ...
                reportID: expenseReport2.reportID,
            };
    ...
    describe('changeReportPolicyAndInviteSubmitter', () => {
        ...
        it('should invite the submitter and optimistically add them to the policy employee list with USER role', async () => {
            ...
            await Onyx.merge(ONYXKEYS.PERSONAL_DETAILS_LIST, {
                [ownerAccountID]: {login: ownerEmail, accountID: ownerAccountID},
            });
        ...
        it('should create moved action on the expense report', async () => {
            ...
            const policy: OnyxTypes.Policy = {
                ...
                employeeList: {
                    ...
                    [ownerEmail]: {email: ownerEmail, role: CONST.POLICY.ROLE.USER},
                },
            };
    ...
    describe('navigateToConciergeChat', () => {
        ...
        const mockNavigation: {
            navigate: jest.Mock;
            dismissModalWithReport: jest.Mock;
        } = jest.requireMock('@libs/Navigation/Navigation');
        ...
        const testIntroSelected: OnyxTypes.IntroSelected = {
        ...
        it('should optimistically resolve followups and post comment with pre-generated response when response is provided', async () => {
            ...
            resolveSuggestedFollowup(
                ...
                reportAction,
                {
                    ...
                    response: 'To set up QuickBooks, go to Settings...',
                },
                CONST.DEFAULT_TIME_ZONE,
                TEST_USER_ACCOUNT_ID,
                TEST_USER_EMAIL,
    ...
    describe('navigateToMostRecentReport via leaveRoom', () => {
        ...
        const mockNavigation: {
            goBack: jest.Mock;
            dismissToSuperWideRHP: jest.Mock;
            navigate: jest.Mock;
            getTopmostSuperWideRHPReportID: jest.Mock;
        } = jest.requireMock('@libs/Navigation/Navigation');
        ...
        it('should navigate to last accessed report when one exists', async () => {
            ...
            await Onyx.merge(ONYXKEYS.SESSION, {
                accountID: TEST_CURRENT_USER_ACCOUNT_ID,
            });
            await waitForBatchedUpdates();
    ...
    describe('handleWalletStatementNavigation', () => {
        ...
        beforeEach(() => {
            jest.resetModules();
            handleWalletStatementNavigation = jest.requireActual<{
                default: typeof handleWalletStatementNavigationDefault;
            }>('@components/WalletStatementModal/walletNavigationUtils').default;
        });
        ...
        it('should pass isSelfTourViewed=true to navigateToConciergeChat when selfTour has been viewed', async () => {
            ...
            jest.resetModules();
            const localHandleWalletStatementNavigation = jest.requireActual<{
                default: typeof handleWalletStatementNavigationDefault;
            }>('@components/WalletStatementModal/walletNavigationUtils').default;
    ...
    describe('getGuidedSetupDataForOpenReport', () => {
        ...
        it.each<[boolean, string]>([
            ...
            expect(result).toBeDefined();
            const guidedSetupData = JSON.parse(result?.guidedSetupData ?? '[]') as Array<{
                type: string;
                task?: string;
                completedTaskReportActionID?: string;
            }>;
    ...
    describe('resolveActionableMentionWhisper', () => {
        it('should optimistically add invited users to report.participants when resolution is INVITE', async () => {
            ...
            const whisperAction = createMock<OnyxTypes.ReportAction>({
                ...
                message: [
                    {
                        html: 'Mentioned @user1 and @user2',
                        text: 'Mentioned @user1 and @user2',
                        type: 'COMMENT',
                    },
                ],
                originalMessage: {
        ...
        it('should NOT update participants when resolution is NOTHING', async () => {
            ...
            const whisperAction = createMock<OnyxTypes.ReportAction>({
                ...
                message: [
                    {
                        html: 'Mentioned @user1',
                        text: 'Mentioned @user1',
                        type: 'COMMENT',
                    },
                ],
                originalMessage: {
    ...
    describe('buildOptimisticAddCommentReportAction delegateAccountID forwarding', () => {
        ...
        it('sets delegateAccountID when delegateAccountIDParam is provided', () => {
            const result = ReportUtils.buildOptimisticAddCommentReportAction({
                text: 'test comment',
    ...
    describe('buildOptimisticReportPreview delegateAccountID forwarding', () => {
        ...
        it('sets delegateAccountID when delegateAccountIDParam is provided', () => {
            ...
            const iouReport = createMock<OnyxTypes.Report>({
                reportID: 'iou1',
                ownerAccountID: 1,
                managerID: 2,
            });
