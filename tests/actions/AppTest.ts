...
describe('actions/App', () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(ActiveClientManager, 'isClientTheLeader').mockReturnValue(true);
    ...
    describe('openApp', () => {
        it('should not make api call when client is not leader', async () => {
            // When Open App runs
        ...
        it('should not make api call when using imported state', async () => {
            // When Open App runs
            ...
            // The app should be marked as loaded without an API call
            expect(await getOnyxValue(ONYXKEYS.IS_LOADING_APP)).toBeFalsy();
            expect(await getOnyxValue(ONYXKEYS.HAS_LOADED_APP)).toBeTruthy();

            // The lastFullReconnectTime should NOT be updated
            expect(await getOnyxValue(ONYXKEYS.LAST_FULL_RECONNECT_TIME)).toBeUndefined();
        });

        it('should make an api call successfully and update lastFullReconnectTime', async () => {
            // When Open App runs
            App.openApp();
            App.confirmReadyToOpenApp();
            await waitForBatchedUpdates();
    ...
    describe('reconnectApp', () => {
        it('should update lastFullReconnectTime on full reconnect', async () => {
        ...
        it('should not update lastFullReconnectTime on incremental reconnect', async () => {
        ...
        it('should trigger full reconnect', async () => {
        ...
        it('should not trigger full reconnect', async () => {
            const reconnectApp = jest.spyOn(App, 'reconnectApp');
