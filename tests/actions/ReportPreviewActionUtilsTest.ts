...
describe('getReportPreviewAction', () => {
    ...
    it('getReportPreviewAction should return ADD_EXPENSE action for report preview with no transactions', async () => {
        ...
        expect(getReportPreviewAction(false, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [])).toBe(CONST.REPORT.REPORT_PREVIEW_ACTIONS.ADD_EXPENSE);
    });
    ...
    it('canSubmit should return true for expense preview report with manual submit', async () => {
        ...
        expect(getReportPreviewAction(isReportArchived.current, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [transaction])).toBe(CONST.REPORT.REPORT_PREVIEW_ACTIONS.SUBMIT);
    });
    ...
    it('canSubmit should return false for expense preview report with only pending transactions', async () => {
        ...
        expect(getReportPreviewAction(isReportArchived.current, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [transaction])).toBe(CONST.REPORT.REPORT_PREVIEW_ACTIONS.VIEW);
    });
    ...
    it('canSubmit should return false for expense preview report with smartscan failed violation', async () => {
        ...
        expect(
            getReportPreviewAction(isReportArchived.current, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [transaction], undefined, undefined, undefined, undefined, {
    ...
    describe('canApprove', () => {
        it('should return true for report being processed', async () => {
            ...
            const {result: isReportArchived} = renderHook(() => useReportIsArchived(report?.parentReportID));
            await waitForBatchedUpdatesWithAct();
            expect(getReportPreviewAction(isReportArchived.current, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [transaction])).toBe(
        ...
        it('should return false for report with scanning expenses', async () => {
            ...
            expect(getReportPreviewAction(false, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [transaction])).toBe(CONST.REPORT.REPORT_PREVIEW_ACTIONS.VIEW);
        });
    ...
    it('canPay should return true for expense report with payments enabled', async () => {
        ...
        const {result: isReportArchived} = renderHook(() => useReportIsArchived(report?.parentReportID));
        await waitForBatchedUpdatesWithAct();
        expect(getReportPreviewAction(isReportArchived.current, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [transaction])).toBe(CONST.REPORT.REPORT_PREVIEW_ACTIONS.PAY);
    });
    ...
    it('canPay should return true for submitted invoice', async () => {
        ...
        const {result: isReportArchived} = renderHook(() => useReportIsArchived(report?.parentReportID));
        await waitForBatchedUpdatesWithAct();
        expect(getReportPreviewAction(isReportArchived.current, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [transaction], invoiceReceiverPolicy)).toBe(
    ...
    it('getReportPreviewAction should return VIEW action for invoice when the chat report is archived', async () => {
        ...
        expect(getReportPreviewAction(isChatReportArchived.current, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, report, policy, [transaction], undefined)).toBe(
            CONST.REPORT.REPORT_PREVIEW_ACTIONS.VIEW,
        );
    });
