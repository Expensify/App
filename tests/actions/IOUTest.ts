...
describe('actions/IOU', () => {
    ...
    describe('split expense', () => {
        ...
        it('the description should not be parsed again after completing the scan split bill without changing the description', async () => {
            ...
            completeSplitBill(reportID, iouAction, updatedSplitTransaction, RORY_ACCOUNT_ID, false, {}, [CONST.BETAS.ALL], RORY_EMAIL);
            ...
            return waitForBatchedUpdates()
                ...
                .then(() => {
                    ...
                    if (chatReport && iouReport) {
                        payMoneyRequest(CONST.IOU.PAYMENT_TYPE.ELSEWHERE, chatReport, iouReport, undefined, [CONST.BETAS.ALL]);
                    }
                    return waitForBatchedUpdates();
                })
                .then(
            ...
            return waitForBatchedUpdates()
                ...
                .then(() => {
                    if (chatReport && expenseReport) {
                        payMoneyRequest(CONST.IOU.PAYMENT_TYPE.VBBA, chatReport, expenseReport, undefined, [CONST.BETAS.ALL], undefined);
                    }
                    return waitForBatchedUpdates();
                })
                .then(
        ...
        it('shows an error when paying results in an error', () => {
            ...
            return waitForBatchedUpdates()
                ...
                .then(() => {
                    mockFetch?.fail?.();
                    if (chatReport && expenseReport) {
                        payMoneyRequest('ACH', chatReport, expenseReport, undefined, [CONST.BETAS.ALL], undefined);
                    }
                    return waitForBatchedUpdates();
                })
                .then(
    ...
    describe('payMoneyRequest', () => {
        ...
        it('calls notifyNewAction for the top most report', () => {
            ...
            return waitForBatchedUpdates()
                ...
                .then(() => {
                    ...
                    payMoneyRequest(CONST.IOU.PAYMENT_TYPE.ELSEWHERE, {reportID: topMostReportID}, iouReport, undefined, [CONST.BETAS.ALL], undefined, false);
                    return waitForBatchedUpdates();
                })
                .then(() => {
        ...
        it('new expense report should be a draft report when paying partially and the approval is disabled', async () => {
            ...
            const newExpenseReportID = payMoneyRequest(CONST.IOU.PAYMENT_TYPE.ELSEWHERE, chatReport, expenseReport, undefined, [CONST.BETAS.ALL], undefined, false, undefined, policy);
            await waitForBatchedUpdates();
        ...
        it("has an iouReportID of the cancelled payment's expense report", () => {
            ...
            return waitForBatchedUpdates()
                ...
                .then(() => {
                    ...
                    if (chatReport && expenseReport) {
                        payMoneyRequest(CONST.IOU.PAYMENT_TYPE.ELSEWHERE, chatReport, expenseReport, undefined, [CONST.BETAS.ALL]);
                    }
                    return waitForBatchedUpdates();
                })
                .then(() => {
    ...
    describe('changeTransactionsReport', () => {
        it('should set the correct optimistic onyx data for reporting a tracked expense', async () => {
            ...
            createNewReport(creatorPersonalDetails, true, false, mockPolicy, undefined, [CONST.BETAS.ALL], undefined);
            // Create a tracked expense
            const selfDMReport: Report = {
                ...createRandomReport(1, CONST.REPORT.CHAT_TYPE.SELF_DM),
                reportID: '10',
