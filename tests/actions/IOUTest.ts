...
describe('actions/IOU', () => {
    ...
    describe('requestMoney', () => {
        ...
        it('submits open report automatically when adding first expense to an empty report under Submit & Close policy', () => {
            const amount = 10000;
            const comment = 'Test expense';
            const chatReportID = rand64();
            const iouReportID = rand64();
            const policyID = 'submit_and_close_policy';

            // Create a Submit & Close policy
            const policy: OnyxEntry<Policy> = {
                ...createRandomPolicy(1),
                id: policyID,
                approvalMode: CONST.POLICY.APPROVAL_MODE.OPTIONAL, // Submit & Close (no approvers)
                autoReporting: true,
                autoReportingFrequency: CONST.POLICY.AUTO_REPORTING_FREQUENCIES.INSTANT, // Instant submit
                type: CONST.POLICY.TYPE.CORPORATE,
            };

            // Create a policy expense chat with iouReportID pointing to the expense report
            let chatReport: OnyxEntry<Report> = {
                reportID: chatReportID,
                type: CONST.REPORT.TYPE.CHAT,
                chatType: CONST.REPORT.CHAT_TYPE.POLICY_EXPENSE_CHAT,
                policyID,
                iouReportID, // Link to expense report
                participants: {[RORY_ACCOUNT_ID]: RORY_PARTICIPANT, [CARLOS_ACCOUNT_ID]: CARLOS_PARTICIPANT},
                isOwnPolicyExpenseChat: true,
            };

            // Create an OPEN expense report (simulating empty report created under Submit & Close policy)
            let iouReport: OnyxEntry<Report> = {
                reportID: iouReportID,
                ...
                currency: CONST.CURRENCY.USD,
                total: 0,
                stateNum: CONST.REPORT.STATE_NUM.OPEN, // Report starts as OPEN in Submit & Close
            ...
            mockFetch?.pause?.();
            return Onyx.set(`${ONYXKEYS.COLLECTION.POLICY}${policyID}`, policy)
                .then(() => Onyx.set(`${ONYXKEYS.COLLECTION.REPORT}${chatReportID}`, chatReport ?? null))
                .then(() => Onyx.set(`${ONYXKEYS.COLLECTION.REPORT}${iouReportID}`, iouReport ?? null))
                .then(() =>
                    Onyx.set(`${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${iouReportID}`, {
                        [createdAction.reportActionID]: createdAction,
                ...
                .then(() => {
                    if (chatReport) {
                        // Add the first expense to the OPEN report
                        requestMoney({
                            report: chatReport,
                            participantParams: {
                                payeeEmail: RORY_EMAIL,
                        ...
                        new Promise<void>((resolve) => {
                            const connection = Onyx.connect({
                                ...
                                callback: (allReports) => {
                                    ...
                                    chatReport = Object.values(allReports ?? {}).find((report) => report?.reportID === chatReportID);
                                    iouReport = Object.values(allReports ?? {}).find((report) => report?.reportID === iouReportID);

                                    // The report should be automatically submitted
                                    expect(iouReport?.stateNum).toBe(CONST.REPORT.STATE_NUM.SUBMITTED);
                                    expect(iouReport?.statusNum).toBe(CONST.REPORT.STATUS_NUM.SUBMITTED);
                                    expect(iouReport?.total).toBe(amount);
