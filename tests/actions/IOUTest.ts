...
describe('actions/IOU', () => {
            ...
            return (
                waitForBatchedUpdates()
                    ...
                    .then(
                        () =>
                            new Promise<void>((resolve) => {
                                ReportActions.clearAllRelatedReportActionErrors(iouReportID ?? '-1', iouAction);
                                resolve();
                            }),
                    )
    ...
    describe('deleteMoneyRequest', () => {
        ...
        it('navigate the user correctly to the chat Report when appropriate', () => {
            ...
            expect(Navigation.goBack).toHaveBeenCalledWith(ROUTES.REPORT_WITH_ID.getRoute(chatReport?.reportID ?? '-1'));
        });
    });
