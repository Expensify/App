...
import type {OnyxCollection, OnyxEntry, OnyxKey, OnyxMultiSetInput} from 'react-native-onyx';
...
describe('actions/IOU', () => {
    ...
    describe('createDistanceRequest', () => {
        ...
        it('clears odometer draft in successData for odometer distance request', async () => {
            // eslint-disable-next-line rulesdir/no-multiple-api-calls -- Spying on API.write to assert createDistanceRequest Onyx payload.
            ...
            createDistanceRequest(
                getDefaultDistanceRequestParams(
                    ...
                    {
                        amount: 500,
                        comment: 'Odometer draft clear test',
                        odometerStart: 1200,
                        odometerEnd: 1250,
                    },
                    recentWaypoints,
            ...
            const onyxData = writeSpy.mock.calls.at(-1)?.at(2) as {successData?: Array<{onyxMethod: string; key: string; value: unknown}>} | undefined;
            const shouldClearOdometerDraft = (onyxData?.successData ?? []).some(
                (update) => update.onyxMethod === Onyx.METHOD.SET && update.key === ONYXKEYS.ODOMETER_DRAFT && update.value === null,
            );

            expect(shouldClearOdometerDraft).toBe(true);
            writeSpy.mockRestore();
        });

        it('does not clear odometer draft in successData for non-odometer distance request', async () => {
            // eslint-disable-next-line rulesdir/no-multiple-api-calls -- Spying on API.write to assert createDistanceRequest Onyx payload.
            const writeSpy = jest.spyOn(API, 'write').mockImplementation(jest.fn());
            const testReport = createRandomReport(1, undefined);
            await Onyx.set(`${ONYXKEYS.COLLECTION.REPORT}${testReport.reportID}`, testReport);
            ...
            createDistanceRequest(
                getDefaultDistanceRequestParams(
                    ...
                    {
                        amount: 500,
                        comment: 'Manual distance request',
                        distance: 10,
                        validWaypoints: {},
                    },
                    recentWaypoints,
            ...
            expect(shouldClearOdometerDraft).toBe(false);
            writeSpy.mockRestore();
        });
    ...
    describe('odometer draft helpers', () => {
        const transactionID = 'testOdometerDraftTransaction';
        ...
        it('saveOdometerDraft should persist odometer readings and images', async () => {
            await saveOdometerDraft({
                startReading: 100,
                endReading: 250,
                startImage: 'data:image/png;base64,abc',
                endImage: 'file:///tmp/odometer-end.jpg',
            });
            await waitForBatchedUpdates();

            const odometerDraft = await getOnyxValue(ONYXKEYS.ODOMETER_DRAFT);
            expect(odometerDraft).toEqual({
                odometerStartReading: 100,
                odometerEndReading: 250,
                odometerStartImage: 'data:image/png;base64,abc',
                odometerEndImage: 'file:///tmp/odometer-end.jpg',
            });
        });

        it('saveOdometerDraft should clear draft when no data is provided', async () => {
            await Onyx.set(ONYXKEYS.ODOMETER_DRAFT, {
                odometerStartReading: 5,
            });
            await waitForBatchedUpdates();
            ...
            const odometerDraft = await getOnyxValue(ONYXKEYS.ODOMETER_DRAFT);
            expect(odometerDraft).toBeUndefined();
        });

        it('clearOdometerDraft should clear persisted draft data', async () => {
            await Onyx.set(ONYXKEYS.ODOMETER_DRAFT, {
                odometerStartReading: 500,
                odometerEndReading: 900,
            });
            await waitForBatchedUpdates();
        ...
        it('hydrateOdometerDraftToTransaction should populate transaction draft from persisted odometer draft', async () => {
            await Onyx.set(`${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${transactionID}`, createRandomTransaction(1));
            await waitForBatchedUpdates();

            hydrateOdometerDraftToTransaction(transactionID, true, {
                odometerStartReading: 700,
                odometerEndReading: 760,
                odometerStartImage: 'file:///tmp/odometer-start.jpg',
                odometerEndImage: 'file:///tmp/odometer-end.jpg',
            });
            await waitForBatchedUpdates();

            const updatedDraftTransaction = await getOnyxValue(`${ONYXKEYS.COLLECTION.TRANSACTION_DRAFT}${transactionID}`);
            expect(updatedDraftTransaction?.comment?.odometerStart).toBe(700);
            expect(updatedDraftTransaction?.comment?.odometerEnd).toBe(760);
            expect(updatedDraftTransaction?.comment?.odometerStartImage).toBe('file:///tmp/odometer-start.jpg');
            expect(updatedDraftTransaction?.comment?.odometerEndImage).toBe('file:///tmp/odometer-end.jpg');
        });

        it('hydrateOdometerDraftToTransaction should return hydrated comment data', async () => {
            const hydratedComment = hydrateOdometerDraftToTransaction(transactionID, true, {
                odometerStartReading: 410,
                odometerEndReading: 450,
                odometerStartImage: 'file:///tmp/start.jpg',
                odometerEndImage: 'file:///tmp/end.jpg',
            });

            expect(hydratedComment).toMatchObject({
                odometerStart: 410,
                odometerEnd: 450,
                odometerStartImage: 'file:///tmp/start.jpg',
                odometerEndImage: 'file:///tmp/end.jpg',
