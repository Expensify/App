...
describe('actions/IOU', () => {
    ...
    describe('startSplitBill', () => {
        ...
        it('should set iouRequestType to SCAN on the optimistic transaction when a receipt is provided', async () => {
            const participants: IOUParticipant[] = [{accountID: RORY_ACCOUNT_ID}];
            const participantsPolicyTags = await getParticipantsPolicyTags(participants);
            ...
            const splitTransaction = await new Promise<OnyxEntry<Transaction>>((resolve) => {
                const connection = Onyx.connect({
                    key: `${ONYXKEYS.COLLECTION.TRANSACTION}${splitTransactionID}`,
                    callback: (transaction) => {
                        Onyx.disconnect(connection);
                        resolve(transaction);
                    },
        ...
        it('should set iouRequestType to MANUAL on the optimistic transaction when no receipt is provided', async () => {
            const participants: IOUParticipant[] = [{accountID: RORY_ACCOUNT_ID}];
            const participantsPolicyTags = await getParticipantsPolicyTags(participants);
