...
describe('Split Expense Auto-Adjustment', () => {
    ...
    describe('addSplitExpenseField', () => {
        it('should redistribute evenly when adding a split to 2 unedited splits', async () => {
            ...
            addSplitExpenseField(mockTransaction, mockTransaction, undefined, undefined, false, undefined, getCurrencySymbolLocal, getCurrencyDecimalsLocal);
            await waitForBatchedUpdates();
        ...
        it('should redistribute unedited splits when manual edits exist and totals do not match', async () => {
            // Setup: Total $10.00, edited split at $3.00, unedited split at $3.00 (sum != total)
            const initialSplits = [createSplitExpense('split1', 300, true), createSplitExpense('split2', 300, false)];
            ...
            const editedSplit = splitExpenses.find((split) => split.transactionID === 'split1');
            expect(editedSplit?.amount).toBe(300);
            expect(editedSplit?.isManuallyEdited).toBe(true);

            const uneditedAmounts = splitExpenses
                .filter((split) => !split.isManuallyEdited)
                .map((split) => split.amount)
                .sort((a, b) => a - b);
            expect(uneditedAmounts).toEqual([350, 350]);

            const totalAmount = splitExpenses.reduce((sum, split) => sum + split.amount, 0);
            expect(totalAmount).toBe(TOTAL_AMOUNT);
        });

        it('should clear split date range when adding a split', async () => {
            ...
            addSplitExpenseField(transaction, draftTransaction, undefined, undefined, false, undefined, getCurrencySymbolLocal, getCurrencyDecimalsLocal);
            await waitForBatchedUpdates();
            ...
            expect(updatedDraft?.comment?.splitsStartDate).toBeUndefined();
            expect(updatedDraft?.comment?.splitsEndDate).toBeUndefined();
            expect(updatedDraft?.comment?.splitExpenses?.length).toBe(2);
        });
    ...
    describe('updateSplitExpenseAmountField', () => {
        it('should mark edited split and redistribute remaining to unedited splits', async () => {
            ...
            updateSplitExpenseAmountField(mockTransaction, 'split1', 300, undefined, false, undefined, getCurrencySymbolLocal, getCurrencyDecimalsLocal);
            await waitForBatchedUpdates();
        ...
        it('should not redistribute when all splits are manually edited', async () => {
            ...
            updateSplitExpenseAmountField(mockTransaction, 'split1', 500, undefined, false, undefined, getCurrencySymbolLocal, getCurrencyDecimalsLocal);
            await waitForBatchedUpdates();
        ...
        it('should ignore NaN amount and preserve existing split values', async () => {
            ...
            updateSplitExpenseAmountField(mockTransaction, 'split1', NaN, undefined, false, undefined, getCurrencySymbolLocal, getCurrencyDecimalsLocal);
            await waitForBatchedUpdates();
    ...
    describe('evenlyDistributeSplitExpenseAmounts', () => {
        it('should reset isManuallyEdited and distribute evenly', async () => {
            ...
            evenlyDistributeSplitExpenseAmounts(mockTransaction, undefined, undefined, false, undefined, getCurrencySymbolLocal, getCurrencyDecimalsLocal);
            await waitForBatchedUpdates();
        ...
        it('should recalculate each split tax amount from its own tax rate when distributing evenly', async () => {
            ...
            evenlyDistributeSplitExpenseAmounts(mockTransaction, originalTransaction, undefined, false, undefined, getCurrencySymbolLocal, getCurrencyDecimalsLocal);
            await waitForBatchedUpdates();
