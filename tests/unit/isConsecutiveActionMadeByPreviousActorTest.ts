...
describe('isConsecutiveActionMadeByPreviousActor', () => {
    ...
    it('returns false if current action is missing', () => {
        const result = isConsecutiveActionMadeByPreviousActorUtil([getFakeReportAction(accountID)], 0);
    ...
    it('returns false if actions are more than 5 minutes apart', () => {
        ...
        const result = isConsecutiveActionMadeByPreviousActorUtil(actions, 0);
        expect(result).toBe(false);
    });
    ...
    it('returns false if either action is RENAMED', () => {
        ...
        expect(isConsecutiveActionMadeByPreviousActorUtil(actions, 0)).toBe(false);
        expect(isConsecutiveActionMadeByPreviousActorUtil(actions.toReversed(), 0)).toBe(false);
    });
    ...
    it('returns false if the current action is SUBMITTED and the current action adminID is different than previous action actorID', () => {
        ...
        const isConsecutiveActionMadeByPreviousActor = isConsecutiveActionMadeByPreviousActorUtil(actions, 0);
