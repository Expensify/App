...
describe('getVersionNumberFromString', () => {
    it('should return a list with version levels numbers', () => {
        expect(
            versionUpdater.getVersionNumberFromString(VERSION),
        ).toStrictEqual(VERSION_NUMBER);
    });

    it('should return build as zero if not present in string', () => {
        const versionWithZeroBuild = [
            VERSION_NUMBER[0],
            VERSION_NUMBER[1],
            VERSION_NUMBER[2],
            0,
        ...
        ).toStrictEqual(versionWithZeroBuild);
    });
});
...
describe('getVersionStringFromNumber', () => {
    it(`should return ${VERSION}`, () => {
        expect(
            versionUpdater.getVersionStringFromNumber(...VERSION_NUMBER),
        ).toBe(VERSION);
    });
});
...
describe('incrementMinor', () => {
    ...
    it('should increment major', () => {
        expect(
            versionUpdater.incrementMinor(2, versionUpdater.MAX_INCREMENTS),
...
describe('incrementVersion', () => {
    it('should increment MAJOR', () => {
        expect(
            versionUpdater.incrementVersion(
                VERSION,
                versionUpdater.SEMANTIC_VERSION_LEVELS.MAJOR,
            ),
        ).toStrictEqual('3.0.0-0');
    });
    ...
    it('should increment MAJOR even above max level', () => {
        expect(
            versionUpdater.incrementVersion(
                `${versionUpdater.MAX_INCREMENTS}.5.1-80`,
                versionUpdater.SEMANTIC_VERSION_LEVELS.MAJOR,
            ),
