...
describe('ExpensifyCardStatementUtils', () => {
    ...
    it('hides the export when the backend marks the settlement as not exportable', () => {
        const groupKey = `${CONST.SEARCH.GROUP_PREFIX}123`;
        const selectedTransactions = makeSettlementSelection(groupKey, 1);
        // The statement is admin-only. The backend stamps canExportStatement=false for a user who is not an admin of
        // the feed (e.g. a cardholder who can still see the settlement in search), so the action must stay hidden.
        const searchData = makeSearchData({[groupKey]: makeSettlementGroup({entryID: 123, canExportStatement: false})});

        expect(getExpensifyCardStatementSelection(expensifyCardStatementQueryJSON, selectedTransactions, searchData)).toBeUndefined();
    });

    it('includes the settlement when the backend marks it as exportable', () => {
        const groupKey = `${CONST.SEARCH.GROUP_PREFIX}123`;
        const selectedTransactions = makeSettlementSelection(groupKey, 1);
