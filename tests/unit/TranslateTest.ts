...
describe('TranslateTest', () => {
    describe('translate', () => {
        ...
        it('uses the accounting integration display name in connection messages', () => {
            ...
            expect(translate(CONST.LOCALES.EN, 'workspace.accounting.syncError', {connectionName})).toBe("Can't connect to Intuit Enterprise Suite");
            expect(translate(CONST.LOCALES.EN, 'workspace.accounting.disconnectTitle', {connectionName})).toBe('Disconnect Intuit Enterprise Suite');
            expect(translate(CONST.LOCALES.EN, 'workspace.accounting.disconnectPrompt', {connectionName})).toBe('Are you sure you want to disconnect Intuit Enterprise Suite?');
        });
