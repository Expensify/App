...
function getNavigationFocusManager(): NavigationFocusManagerModule {
    // eslint-disable-next-line @typescript-eslint/no-require-imports, @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-unsafe-member-access
    return require('@libs/NavigationFocusManager/index.web').default as NavigationFocusManagerModule;
}
...
describe('FocusTrapForScreen', () => {
    beforeEach(() => {
        ...
        resetMocks();
        const NavigationFocusManager = getNavigationFocusManager();
    ...
    describe('P0-2: Navigation back - focus restored to trigger element', () => {
        it('should restore a keyboard-captured route target through initialFocus on a fresh remount', () => {
            const NavigationFocusManager = getNavigationFocusManager();
            const triggerButton = createMockElement('button', 'keyboard-trigger-button');
            triggerButton.setAttribute('aria-label', 'Keyboard Trigger');
            triggerButton.focus();

            NavigationFocusManager.registerFocusedRoute('test-route');
            ...
            NavigationFocusManager.unregisterFocusedRoute('test-route');

            expect(NavigationFocusManager.hasKeyboardStoredFocus('test-route')).toBe(true);
            ...
            const initialFocusFn = capturedFocusTrapOptions.initialFocus as () => HTMLElement | false;
            expect(initialFocusFn()).toBe(triggerButton);
        });

        it('should not restore a pointer-captured route target through initialFocus on a fresh remount', () => {
            const NavigationFocusManager = getNavigationFocusManager();
            const triggerButton = createMockElement('button', 'pointer-trigger-button');
            triggerButton.setAttribute('aria-label', 'Pointer Trigger');
            ...
            Object.defineProperty(pointerEvent, 'target', {value: triggerButton});
            document.dispatchEvent(pointerEvent);
            NavigationFocusManager.unregisterFocusedRoute('test-route');

            expect(NavigationFocusManager.hasStoredFocus('test-route')).toBe(true);
            expect(NavigationFocusManager.hasKeyboardStoredFocus('test-route')).toBe(false);
        ...
        it('should mark the restored keyboard target with the programmatic-focus marker', () => {
            const NavigationFocusManager = getNavigationFocusManager();
            const triggerButton = createMockElement('button', 'styled-keyboard-trigger');
            triggerButton.setAttribute('aria-label', 'Styled Keyboard Trigger');
            triggerButton.focus();
            ...
            expect(initialFocusFn()).toBe(triggerButton);
            expect(triggerButton.getAttribute('data-programmatic-focus')).toBe('true');
        });
