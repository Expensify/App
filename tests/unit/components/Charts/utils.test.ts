...
describe("effectiveWidth", () => {
  it("returns labelWidth at 0°", () => {
    expect(effectiveWidth(100, LINE_HEIGHT, LABEL_ROTATIONS.HORIZONTAL)).toBe(
      100,
    );
  });
  ...
  it("returns labelWidth * sin(45°) at 45°", () => {
    expect(
      effectiveWidth(100, LINE_HEIGHT, LABEL_ROTATIONS.DIAGONAL),
    ).toBeCloseTo(100 * SIN_45);
  });

  it("returns lineHeight at 90°", () => {
    expect(effectiveWidth(100, LINE_HEIGHT, LABEL_ROTATIONS.VERTICAL)).toBe(
      LINE_HEIGHT,
    );
  });
});
...
describe("effectiveHeight", () => {
  it("returns lineHeight at 0°", () => {
    expect(effectiveHeight(100, LINE_HEIGHT, LABEL_ROTATIONS.HORIZONTAL)).toBe(
      LINE_HEIGHT,
    );
  });
  ...
  it("returns (labelWidth + lineHeight) * sin(45°) at 45°", () => {
    expect(
      effectiveHeight(100, LINE_HEIGHT, LABEL_ROTATIONS.DIAGONAL),
    ).toBeCloseTo((100 + LINE_HEIGHT) * SIN_45);
  });

  it("returns labelWidth at 90°", () => {
    expect(effectiveHeight(100, LINE_HEIGHT, LABEL_ROTATIONS.VERTICAL)).toBe(
      100,
    );
  });
});
...
describe("edgeMaxLabelWidth", () => {
  it("returns 2 * edgeSpace at 0°", () => {
    expect(
      edgeMaxLabelWidth(50, LINE_HEIGHT, LABEL_ROTATIONS.HORIZONTAL, "first"),
    ).toBe(100);
  });

  it("returns Infinity at 90° (overhang is constant)", () => {
    expect(
      edgeMaxLabelWidth(50, LINE_HEIGHT, LABEL_ROTATIONS.VERTICAL, "first"),
    ).toBe(Infinity);
  });

  it("returns finite value at 45° for first label", () => {
    const result = edgeMaxLabelWidth(
      50,
      LINE_HEIGHT,
      LABEL_ROTATIONS.DIAGONAL,
      "first",
    );
  ...
  it("returns Infinity at 45° for last label (right-aligned labels never overhang right)", () => {
    expect(
      edgeMaxLabelWidth(50, LINE_HEIGHT, LABEL_ROTATIONS.DIAGONAL, "last"),
    ).toBe(Infinity);
  });
  ...
  it("returns 0 when edgeSpace is too small at 45°", () => {
    ...
    expect(
      edgeMaxLabelWidth(1, LINE_HEIGHT, LABEL_ROTATIONS.DIAGONAL, "first"),
    ).toBe(0);
  });
});
...
describe("findSliceAtPosition", () => {
  ...
  it("returns -1 when cursor is outside the pie radius", () => {
    ...
    expect(
      findSliceAtPosition(250, 100, center, center, radius, 0, slices),
    ).toBe(-1);
  });
  ...
  it("returns -1 when cursor is inside the inner radius (donut hole)", () => {
    const slices = makeSlices();
    expect(
      findSliceAtPosition(100, 100, center, center, radius, 50, slices),
    ).toBe(-1);
  });
  ...
  it("returns the correct slice index for the largest slice", () => {
    ...
    expect(
      findSliceAtPosition(150, 100, center, center, radius, 0, slices),
    ).toBe(0);
  });
  ...
  it("returns the correct slice index for a smaller slice", () => {
    ...
    expect(findSliceAtPosition(50, 50, center, center, radius, 0, slices)).toBe(
      1,
    );
  });
  ...
  it("returns matching slice when cursor is exactly at center with zero inner radius", () => {
    ...
    expect(
      findSliceAtPosition(100, 100, center, center, radius, 0, slices),
    ).toBe(0);
  });
});
...
describe("processDataIntoSlices", () => {
  ...
  it("creates a single slice covering 360 degrees for one data point", () => {
    ...
    const slices = processDataIntoSlices(
      data,
      { centerX: 0, centerY: 0, radius: 0, innerRadius: 0 },
      -90,
    );
  ...
  it("produces angles that sum to 360 degrees", () => {
    ...
    const totalSweep = slices.reduce(
  ...
  it("chains consecutive angles without gaps", () => {
    ...
    for (let i = 1; i < slices.length; i++) {
      expect(slices.at(i)?.startAngle).toBeCloseTo(
        slices.at(i - 1)?.endAngle ?? 0,
        10,
      );
    }
  });
  ...
  it("places tooltipPosition at the ring midpoint along each slice mid-angle", () => {
    ...
    const slices = processDataIntoSlices(
      data,
      { centerX: 200, centerY: 150, radius: 100, innerRadius: 60 },
      -90,
    );
...
describe("isCursorOverChartLabel", () => {
  ...
  describe("0° (horizontal label)", () => {
    ...
    it("returns true when cursor is inside the horizontal label box", () => {
      ...
      expect(
        isCursorOverChartLabel({ ...params(), cursorX: 12, cursorY: 21 }),
      ).toBe(true);
    });
    ...
    it("returns true when cursor is on the horizontal boundary", () => {
      expect(
        isCursorOverChartLabel({ ...params(), cursorX: 5, cursorY: 20 }),
      ).toBe(true);
      expect(
        isCursorOverChartLabel({ ...params(), cursorX: 15, cursorY: 20 }),
      ).toBe(true);
    });

    it("returns true when cursor is on the vertical boundary", () => {
      expect(
        isCursorOverChartLabel({ ...params(), cursorX: 10, cursorY: 18 }),
      ).toBe(true);
      expect(
        isCursorOverChartLabel({ ...params(), cursorX: 10, cursorY: 22 }),
      ).toBe(true);
    });
  });
...
describe("getAdditionalOffset", () => {
  ...
  it("returns iconSizeExtraSmall / 1.5 for diagonal angles (0 < angle < DIAGONAL_ANGLE_RADIAN_THRESHOLD)", () => {
    ...
    expect(
      getAdditionalOffset(DIAGONAL_ANGLE_RADIAN_THRESHOLD - 0.001),
    ).toBeCloseTo(12 / 1.5);
  });
  ...
  it("returns iconSizeExtraSmall / 3 for 90° (vertical, angle >= DIAGONAL_ANGLE_RADIAN_THRESHOLD)", () => {
    ...
    expect(getAdditionalOffset(DIAGONAL_ANGLE_RADIAN_THRESHOLD)).toBeCloseTo(
      12 / 3,
    );
  });
});
...
describe("rotatedLabelCenterCorrection", () => {
  ...
  it("returns (ascent - descent) / 2 at 90° because sin(90°) = 1", () => {
    expect(
      rotatedLabelCenterCorrection(ascent, descent, Math.PI / 2),
    ).toBeCloseTo((ascent - descent) / 2);
  });
...
describe("rotatedLabelYOffset", () => {
  ...
  it("returns ascent * cos(angle) for intermediate angles", () => {
    expect(rotatedLabelYOffset(ascent, descent, Math.PI / 4)).toBeCloseTo(
      ascent * Math.cos(Math.PI / 4),
    );
  });

  it("is always smaller at 45° than at 0° for positive ascent", () => {
    expect(rotatedLabelYOffset(ascent, descent, Math.PI / 4)).toBeLessThan(
      rotatedLabelYOffset(ascent, descent, 0),
    );
  });
});
...
describe("calculateMinDomainPadding", () => {
  ...
  it("uses innerPadding=0 as default", () => {
    expect(calculateMinDomainPadding(400, 5)).toBe(
      calculateMinDomainPadding(400, 5, 0),
    );
  });
...
describe("getNiceYAxisTicks", () => {
  ...
  it("returns correct ticks for a high positive-only range with bar chart padding", () => {
    ...
    ).toEqual([0, 2000, 4000, 6000, 8000, 10000]);
  });
  ...
  it("rounds intermediate ticks to eliminate floating-point noise", () => {
    expect(getNiceYAxisTicks(0, -1.11, 5)).toEqual([
...
describe("truncateCategoryLabels", () => {
  ...
  it("truncates each label to the Y-axis max width", () => {
    ...
    const truncated = truncateCategoryLabels(
      labels,
      labelWidths,
      ellipsisWidth,
    );
...
describe("estimateVerticalBarChartGeometry", () => {
  ...
  it("returns zero geometry when chartWidth is 0", () => {
    const geometry = estimateVerticalBarChartGeometry(
      0,
      data,
      null,
      12,
  ...
  it("returns wider barAreaWidth for wider chartWidth", () => {
    const narrow = estimateVerticalBarChartGeometry(
      200,
      data,
      null,
      12,
    ...
    const wide = estimateVerticalBarChartGeometry(
      400,
      data,
      null,
      12,
