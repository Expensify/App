...
jest.mock(
  "@components/RequireTwoFactorAuthenticationModal",
  () =>
    ({ isVisible, onSubmit, onCancel }: Require2FAProps) => {
...
describe("XeroSetupPage", () => {
  ...
  describe("when 2FA is enabled", () => {
    ...
    it("opens a WebView pointing at the Xero setup link for the given policy", () => {
      ...
      expect(mockWebViewProps.current?.source?.uri).toBe(
        `https://xero-setup.example/${POLICY_ID}`,
      );
    });
    ...
    it("passes the session auth token to the WebView as a cookie", () => {
      ...
      expect(mockWebViewProps.current?.source?.headers?.Cookie).toBe(
