...
jest.mock('@components/MultifactorAuthentication/helpers', () => ({
    ...
    getNotificationPaths: jest.fn(() => {
        // Always return valid notification paths that exist in the config mock
        return {
...
jest.mock('@components/MultifactorAuthentication/config', () => {
    ...
    const notificationMap = {
        'biometrics-test-success': successNotification,
        'biometrics-test-failure': failureNotification,
    };
    ...
    return {
        ...
        MULTIFACTOR_AUTHENTICATION_NOTIFICATION_MAP: notificationMap,
    };
});
...
describe('useMultifactorAuthenticationStatus hook', () => {
    ...
    it('should have notification paths', () => {
        const {result} = renderHook(() => useMultifactorAuthenticationStatus(null));
        const [status] = result.current;

        expect(status.notificationPaths).toBeDefined();
        expect(status.notificationPaths.successNotification).toBeDefined();
        expect(status.notificationPaths.failureNotification).toBeDefined();
    });
    ...
    it('should accept valid scenario and update notification paths', () => {
        const {result} = renderHook(() => useMultifactorAuthenticationStatus(null));
        ...
        expect(updatedStatus.notificationPaths).toBeDefined();
    });

    it('should update state and notification paths on multiple calls', () => {
        const {result} = renderHook(() => useMultifactorAuthenticationStatus(false));
