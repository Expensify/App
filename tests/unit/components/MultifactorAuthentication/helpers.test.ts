...
describe('MultifactorAuthentication helpers', () => {
    ...
    describe('Status.createEmptyStatus', () => {
        ...
        it('should have default notification paths', () => {
            ...
            expect(status.notificationPaths.successNotification).toBe('biometrics-test-success');
            expect(status.notificationPaths.failureNotification).toBe('biometrics-test-failure');
        });
    ...
    describe('getNotificationPaths', () => {
        it('should return default paths when scenario is undefined', () => {
            const result = getNotificationPaths(undefined);

            expect(result.successNotification).toBe('biometrics-test-success');
            expect(result.failureNotification).toBe('biometrics-test-failure');
        });

        it('should include scenario prefix in notification paths', () => {
            const result = getNotificationPaths(CONST.MULTIFACTOR_AUTHENTICATION.SCENARIO.BIOMETRICS_TEST);

            expect(result.successNotification).toContain('biometrics-test');
            expect(result.successNotification).toContain('success');
            expect(result.failureNotification).toContain('biometrics-test');
            expect(result.failureNotification).toContain('failure');
        });
    ...
    describe('createAuthorizeErrorStatus', () => {
        it('should create error status with failed step', () => {
            const errorStatus: MultifactorAuthenticationPartialStatus<boolean, true> = {
                value: false,
                type: 0,
                reason: 'Authorization failed with provided credentials',
            };
            ...
            expect(result.reason).toBe('Authorization failed with provided credentials');
    ...
    describe('Status.createUnsupportedDeviceStatus', () => {
        it('should clear biometric flags for unsupported devices', () => {
            const prevStatus: MultifactorAuthenticationStatus<BiometricsStatus> = {
                ...
                title: 'Test',
                description: 'Test',
                notificationPaths: {successNotification: 'biometrics-test-success', failureNotification: 'biometrics-test-failure'},
            };
    ...
    describe('Status.createValidateCodeMissingStatus', () => {
        it('should set validate code as required factor', () => {
            ...
            expect(result.reason).toBe(CONST.MULTIFACTOR_AUTHENTICATION.REASON.BACKEND.VALIDATE_CODE_MISSING);
        });
    });
    ...
    describe('getNotificationRoute', () => {
        it('should return not found route when path is undefined', () => {
            const result = getNotificationRoute(undefined);
            expect(typeof result).toBe('string');
        });

        it('should return notification route for valid path', () => {
            const result = getNotificationRoute('biometrics-test-success');
            expect(typeof result).toBe('string');
        });
    });

    describe('getNotificationPath', () => {
        it('should construct notification path with scenario prefix and success suffix', () => {
            const result = getNotificationPath('biometrics-test', 'success');
            expect(result).toContain('success');
            expect(result).toContain('biometrics-test');
        });

        it('should construct notification path with scenario prefix and failure suffix', () => {
            const result = getNotificationPath('biometrics-test', 'failure');
            expect(result).toContain('failure');
            expect(result).toContain('biometrics-test');
        });

        it('should use default prefix when scenario is undefined', () => {
            const result = getNotificationPath(undefined, 'success');
            expect(result).toContain('biometrics-test');
            expect(result).toContain('success');
        });
    });
