...
describe("victoryLegendParser", () => {
  it.each(NON_ARRAY_DATA)(
    "returns empty entries instead of throwing when data is %p",
    (data) => {
...
describe("victorySeriesParser", () => {
  it.each(NON_ARRAY_DATA)(
    "returns empty data instead of throwing when data is %p",
    (data) => {
...
describe("victoryAxisParser", () => {
  it.each(NON_ARRAY_DATA)(
    "does not throw when tickvalues/tickformat are non-array (%p) and the format callback is invoked",
    (data) => {
  ...
  it("derives sequential tick values from tickformat when tickvalues are omitted", () => {
    ...
    const result = parseVictoryAxisNode(
      node,
      null,
  ...
  it("does not derive tickValues from tickformat on dependent value axes", () => {
    const node = createNode("victoryaxis", {
      dependentaxis: "true",
      tickformat: "['$0','$3,000','$6,000']",
    });
    const result = parseVictoryAxisNode(
      node,
      null,
    ...
    expect(result.xAxis?.tickValues).toBeUndefined();
    expect(result.xAxis?.tickCount).toBe(0);
  });
...
describe("processVictoryChartTree", () => {
  it("does not throw when a legend has non-array data (reproduces the reported crash path)", () => {
    const tree = createNode("victorychart", {}, [
      createNode("victorybar", { data: "[{x: 1, y: 2}]" }),
      createNode("victorylegend", { data: "oops" }),
    ]);
  ...
  it("does not throw when a pie has non-array data", () => {
    const tree = createNode("victorychart", {}, [
      createNode("victorypie", { data: "oops" }),
    ]);
    expect(() => processVictoryChartTree(tree, null, null)).not.toThrow();
  });
  ...
  it("does not throw when an axis has non-array tickvalues", () => {
    const tree = createNode("victorychart", {}, [
      createNode("victorybar", { data: "[{x: 1, y: 2}]" }),
      createNode("victoryaxis", { dependentaxis: "true", tickvalues: "oops" }),
    ]);
    expect(() => processVictoryChartTree(tree, null, null)).not.toThrow();
  });
  ...
  it("counts multiple bar series in a grouped chart", () => {
    const tree = createNode("victorychart", {}, [
      createNode("victorygroup", {}, [
        createNode("victorybar", { data: "[{x: 1, y: 2}]" }),
        createNode("victorybar", { data: "[{x: 1, y: 3}]" }),
      ]),
    ]);
