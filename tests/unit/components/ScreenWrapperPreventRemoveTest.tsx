...
import waitForBatchedUpdates from "../../utils/waitForBatchedUpdates.ts";
...
jest.mock("@components/InitialURLContextProvider", () => ({
  ...
  useInitialURLActions: () => ({
    setInitialURL: jest.fn(),
    setIsAuthenticatedAtStartup: jest.fn(),
  }),
}));
...
jest.mock("@hooks/useRootNavigationState", () =>
  jest.fn((selector: (state: undefined) => unknown) => selector(undefined)),
);
...
jest.mock(
  "@components/ScreenWrapper/ScreenWrapperOfflineIndicators",
  () => () => null,
);
...
function getNavigationMock() {
  return jest.requireMock<{
    getActiveRouteWithoutParams: jest.Mock;
    isNavigationReady: jest.Mock;
  }>("@libs/Navigation/Navigation");
}

function getUsePreventRemove() {
  return jest.requireMock<{ usePreventRemove: jest.Mock }>(
...
async function renderScreenWrapper(): Promise<ReactTestRendererType.ReactTestRenderer> {
  ...
  await act(async () => {
    renderer = ReactTestRenderer.create(
      <ScreenWrapper testID="test-screen">{null}</ScreenWrapper>,
    );
...
describe("ScreenWrapper – usePreventRemove condition", () => {
  ...
  it("is false when shouldBlockSingleEntryOldAppExit is true", async () => {
    ...
    const { shouldHideOldAppRedirect } = jest.requireMock<{
      shouldHideOldAppRedirect: jest.Mock;
    }>("@libs/TryNewDotUtils");
