...
jest.mock("@libs/actions/connections/QuickbooksOnline", () => ({
  getQuickbooksOnlineSetupLink: jest.fn(
    (policyID: string) => `https://qbo-setup.example/${policyID}`,
  ),
}));
...
const mockedGetQuickbooksOnlineSetupLink = jest.mocked(
...
describe("QuickbooksOnlineSetupPage", () => {
  ...
  it("opens a WebView pointing at the QuickBooks Online setup link for the given policy", () => {
    ...
    expect(mockWebViewProps.current?.source?.uri).toBe(
      `https://qbo-setup.example/${POLICY_ID}`,
    );
  });
