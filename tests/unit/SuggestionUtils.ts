...
describe('SuggestionUtils', () => {
    describe('getSortedPersonalDetails', () => {
        it('Should sort using the weight if the weight is different', () => {
            ...
            expect(getSortedPersonalDetails([second, first], localeCompare, formatPhoneNumber, translateLocal)).toEqual([first, second]);
        });
        ...
        it('Should sort using the displayName if the weight is the same', () => {
            ...
            expect(getSortedPersonalDetails([second, first, third], localeCompare, formatPhoneNumber, translateLocal)).toEqual([first, second, third]);
            expect(getSortedPersonalDetails([third, second, first], localeCompare, formatPhoneNumber, translateLocal)).toEqual([first, second, third]);
        });
        ...
        it('Should sort using the displayName with different diacritics if the weight is the same', () => {
            ...
            expect(getSortedPersonalDetails([second, first], localeCompare, formatPhoneNumber, translateLocal)).toEqual([second, first]);
        });
    });
});
