...
describe('OptionsListUtils', () => {
    ...
    beforeAll(() => {
        ...
        return waitForPromisesToResolve().then(() =>
            Onyx.set(ONYXKEYS.PERSONAL_DETAILS, PERSONAL_DETAILS),
        );
    });

    it('getSearchOptions()', () => {
        // When we filter in the Search view without providing a searchValue
        let results = OptionsListUtils.getSearchOptions(
            REPORTS,
            PERSONAL_DETAILS,
            '',
        );
        ...
        expect(results.recentReports[0].login).toBe(
            'peterparker@expensify.com',
        );
    ...
    it('getNewChatOptions()', () => {
        ...
        let results = OptionsListUtils.getNewChatOptions(
            REPORTS,
            PERSONAL_DETAILS,
            [],
            '',
        ...
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS) - 1 - MAX_RECENT_REPORTS,
        );
        ...
        const personalDetailWithExistingReport = _.find(
            ...
            (personalDetail) =>
                personalDetail.login === 'peterparker@expensify.com',
        );
        ...
        expect(results.personalDetails[0].login).toBe(
            'natasharomanoff@expensify.com',
        );
        ...
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS_WITH_CONCIERGE) - 1 - MAX_RECENT_REPORTS,
        );
        ...
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS_WITH_CONCIERGE) - 2 - MAX_RECENT_REPORTS,
        );
        ...
        // All the personalDetails should be returned minus the currently logged in user and Concierge
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS_WITH_CHRONOS) - 2 - MAX_RECENT_REPORTS,
        );
        expect(results.personalDetails).not.toEqual(
            expect.arrayContaining([
        ...
        // All the personalDetails should be returned minus the currently logged in user and Concierge
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS_WITH_RECEIPTS) - 2 - MAX_RECENT_REPORTS,
        );
        expect(results.personalDetails).not.toEqual(
            expect.arrayContaining([
    ...
    it('getNewChatOptions() for group Chat', () => {
        ...
        const reportLogins = _.map(
            results.recentReports,
            (reportOption) => reportOption.login,
        );
        const personalDetailsOverlapWithReports = _.every(
            results.personalDetails,
            (personalDetailOption) =>
                _.contains(reportLogins, personalDetailOption.login),
        );
        ...
        expect(
            _.every(
                results.recentReports,
                (option) => option.login !== 'peterparker@expensify.com',
            ),
        ).toBe(true);
        expect(
            _.every(
        ...
        // (minus the 5 that are already showing and the currently logged in user)
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS_WITH_CONCIERGE) - 6,
        );
        expect(results.recentReports).toEqual(
            expect.arrayContaining([
                expect.objectContaining({login: 'concierge@expensify.com'}),
        ...
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS_WITH_CONCIERGE) - 7,
        );
        expect(results.personalDetails).not.toEqual(
            expect.arrayContaining([
                expect.objectContaining({login: 'concierge@expensify.com'}),
        ...
        // the currently logged in user and Concierge)
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS_WITH_CHRONOS) - 7,
        );
        expect(results.personalDetails).not.toEqual(
            expect.arrayContaining([
                expect.objectContaining({login: 'chronos@expensify.com'}),
        ...
        // the currently logged in user and Concierge)
        expect(results.personalDetails.length).toBe(
            _.size(PERSONAL_DETAILS_WITH_RECEIPTS) - 7,
        );
        expect(results.personalDetails).not.toEqual(
            expect.arrayContaining([
                expect.objectContaining({login: 'receipts@expensify.com'}),
    ...
    it('getMemberInviteOptions()', () => {
        // When we only pass personal details
        let results = OptionsListUtils.getMemberInviteOptions(
            PERSONAL_DETAILS,
            [],
            '',
        );
        ...
        // When we provide a search value that does not match any personal details
        results = OptionsListUtils.getMemberInviteOptions(
            PERSONAL_DETAILS,
            [],
            'magneto',
        );
