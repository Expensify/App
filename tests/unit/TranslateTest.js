...
import { translate } from '../../src/libs/translate';

describe('translate', () => {
  it('Test present key in full locale', () => {
    expect(translate('es-ES', 'testkey1')).toBe('Spanish ES');
  });

  it('Test when key is not found in full locale, but present in language', () => {
    expect(translate('es-ES', 'testKey2')).toBe('Spanish Word 2');
    expect(translate('es', 'testKey2')).toBe('Spanish Word 2');
  });

  it('Test when key is not found in full locale and language, but present in default', () => {
    expect(translate('es-ES', 'testKey3')).toBe('Test Word 3');
  });

  test('Test when key is not found in default', () => {
    expect(() => translate('es-ES', 'testKey4')).toThrow(Error);
    expect(() => translate('es-ES', ['a', 'b', 'c'])).toThrow(Error);
  });
  ...
  test('Test when key is not found in default (Production Mode)', () => {
    ...
    expect(translate('es-ES', 'testKey4')).toBe('testKey4');
    expect(translate('es-ES', ['a', 'b', 'c'])).toBe('a.b.c');
  });
  ...
  it('Test when translation value is a function', () => {
    ...
    expect(translate('en', 'testKeyGroup.testFunction', { testVariable })).toBe(expectedValue);
    expect(translate('en', ['testKeyGroup', 'testFunction'], { testVariable })).toBe(expectedValue);
  });
});
