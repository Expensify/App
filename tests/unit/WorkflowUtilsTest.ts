...
describe('WorkflowUtils', () => {
    ...
    describe('convertPolicyEmployeesToApprovalWorkflows', () => {
        ...
        it('Should use HR finalApprover as default approver when Gusto is in basic mode', () => {
            const hrFinalApprover = '5@example.com';
            const employees: PolicyEmployeeList = {
                '1@example.com': {
                    email: '1@example.com',
                    forwardsTo: undefined,
                    submitsTo: hrFinalApprover,
                },
                '2@example.com': {
                    email: '2@example.com',
                    forwardsTo: undefined,
                ...
                [hrFinalApprover]: {
                    email: hrFinalApprover,
                    forwardsTo: undefined,
                    submitsTo: hrFinalApprover,
                },
            };
            const policy = {
                ...createMockPolicy(employees, '3@example.com'),
                connections: {
                    gusto: {config: {approvalMode: 'basic', finalApprover: hrFinalApprover, isConfigured: true}},
                },
            };

            const {approvalWorkflows} = convertPolicyEmployeesToApprovalWorkflows({policy: policy as unknown as Policy, personalDetails, localeCompare});

            const defaultWorkflow = approvalWorkflows.find((w) => w.isDefault);
            expect(defaultWorkflow).toBeDefined();
            expect(defaultWorkflow?.approvers.at(0)?.email).toBe(hrFinalApprover);
        });

        it('Should use policy.approver as default approver when Gusto is in custom mode', () => {
            const policyApprover = '3@example.com';
            const employees: PolicyEmployeeList = {
                [policyApprover]: {
                    email: policyApprover,
                    forwardsTo: undefined,
                    submitsTo: policyApprover,
                },
                '1@example.com': {
                    email: '1@example.com',
                    forwardsTo: undefined,
            ...
            const policy = {
                ...createMockPolicy(employees, policyApprover),
                connections: {
                    gusto: {config: {approvalMode: 'custom', finalApprover: '5@example.com', isConfigured: true}},
                },
            };
            ...
            const defaultWorkflow = approvalWorkflows.find((w) => w.isDefault);
            expect(defaultWorkflow).toBeDefined();
            expect(defaultWorkflow?.approvers.at(0)?.email).toBe(policyApprover);
        });

        it('Should use HR finalApprover as default approver when Zenefits is in manager mode', () => {
            const hrFinalApprover = '5@example.com';
            const employees: PolicyEmployeeList = {
                '1@example.com': {
                    email: '1@example.com',
            ...
            const policy = {
                ...createMockPolicy(employees, '3@example.com'),
                connections: {
                    zenefits: {config: {approvalMode: 'manager', finalApprover: hrFinalApprover, isConfigured: true}},
                },
            };
        ...
        it('Should fall back to policy.approver when HR finalApprover is null in read-only mode', () => {
            const policyApprover = '3@example.com';
            const employees: PolicyEmployeeList = {
                [policyApprover]: {
                    email: policyApprover,
            ...
            const policy = {
                ...createMockPolicy(employees, policyApprover),
                connections: {
                    gusto: {config: {approvalMode: 'basic', finalApprover: null, isConfigured: true}},
                },
            };
