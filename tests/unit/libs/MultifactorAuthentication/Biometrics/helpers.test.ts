...
import {decodeExpoMessage, isChallengeSigned, parseHttpCode, processRegistration} from '@libs/MultifactorAuthentication/Biometrics/helpers';
...
describe('MultifactorAuthentication Biometrics helpers', () => {
    ...
    describe('parseHttpCode', () => {
        it('should parse valid HTTP code and return reason', () => {
            ...
            const result = parseHttpCode(200, responseMap);
        ...
        it('should handle string HTTP codes', () => {
            const responseMap = VALUES.API_RESPONSE_MAP.REQUEST_AUTHENTICATION_CHALLENGE;
            const result = parseHttpCode('401', responseMap);

            expect(result.httpCode).toBe(401);
            expect(result.reason).toBe(VALUES.REASON.BACKEND.REGISTRATION_REQUIRED);
        });

        it('should return unknown reason for unmapped code', () => {
            const responseMap = VALUES.API_RESPONSE_MAP.REQUEST_AUTHENTICATION_CHALLENGE;
            const result = parseHttpCode(999, responseMap);

            expect(result.httpCode).toBe(999);
            expect(result.reason).toBe(VALUES.API_RESPONSE_MAP.UNKNOWN);
        });

        it('should handle undefined HTTP code', () => {
            const responseMap = VALUES.API_RESPONSE_MAP.REQUEST_AUTHENTICATION_CHALLENGE;
            const result = parseHttpCode(undefined, responseMap);
    ...
    describe('processRegistration', () => {
        ...
        it('should handle missing validate code', async () => {
            ...
            expect(result.reason).toBe(VALUES.REASON.BACKEND.VALIDATE_CODE_MISSING);
        ...
        it('should handle invalid validate code length', async () => {
            ...
            expect(result.value).not.toBe(true);
            expect(result.reason).toBe(VALUES.REASON.BACKEND.VALIDATE_CODE_INVALID);
        });

        it('should handle backend registration failure', async () => {
            (registerAuthenticationKey as jest.Mock).mockResolvedValue({
                httpCode: 409,
                reason: VALUES.REASON.BACKEND.KEY_ALREADY_REGISTERED,
            });
            ...
            expect(result.reason).toBe(VALUES.REASON.BACKEND.KEY_ALREADY_REGISTERED);
