...
describe('NavigationFocusManager Gap Tests', () => {
    ...
    describe('Gap 1: Delayed Element Availability', () => {
        it('should store a pointer capture immediately for the registered route without calling captureForRoute', () => {
            const button = document.createElement('button');
            button.setAttribute('aria-label', 'Pointer Trigger');
            document.body.appendChild(button);

            NavigationFocusManager.registerFocusedRoute('pointer-route');
            ...
            NavigationFocusManager.unregisterFocusedRoute('pointer-route');

            expect(NavigationFocusManager.hasStoredFocus('pointer-route')).toBe(true);
            expect(NavigationFocusManager.hasKeyboardStoredFocus('pointer-route')).toBe(false);
            expect(NavigationFocusManager.retrieveKeyboardFocusForRoute('pointer-route')).toBeNull();
        });

        it('should keep pointer capture available for generic retrieval while excluding it from keyboard-only restore', () => {
            const button = document.createElement('button');
            button.setAttribute('aria-label', 'Pointer Trigger');
            document.body.appendChild(button);
            ...
            expect(NavigationFocusManager.retrieveForRoute('pointer-route')).toBe(button);
        });

        it('should store an Enter-key capture immediately for the registered route without calling captureForRoute', () => {
            const button = document.createElement('button');
            button.setAttribute('aria-label', 'Keyboard Trigger');
            document.body.appendChild(button);
            button.focus();

            NavigationFocusManager.registerFocusedRoute('keyboard-route');
            ...
            NavigationFocusManager.unregisterFocusedRoute('keyboard-route');

            expect(NavigationFocusManager.hasStoredFocus('keyboard-route')).toBe(true);
            expect(NavigationFocusManager.hasKeyboardStoredFocus('keyboard-route')).toBe(true);
            expect(NavigationFocusManager.retrieveKeyboardFocusForRoute('keyboard-route')).toBe(button);
        });
