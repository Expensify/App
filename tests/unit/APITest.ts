...
describe('API.write() persistence guarantees', () => {
    ...
    // fire-and-forget. Even if the caller awaits the returned promise, there is
    ...
    test('Issue 2: should return a promise that resolves only after the request is persisted to disk', () => {
        ...
        return Onyx.multiSet({
            ...
            // Mock Onyx.set so that PERSISTED_REQUESTS writes never resolve,
            // simulating stalled disk I/O. If API.write()'s promise were correctly
            // tied to persistence, it would also stall.
            ...
            // Guard: confirm that save() was triggered — Onyx.set was called
            // for PERSISTED_REQUESTS with our request.
    ...
    // the in-memory state of the persisted requests queue. If a pending
    // Onyx.set() callback resolves out of order (Issue 4), the queue can be
