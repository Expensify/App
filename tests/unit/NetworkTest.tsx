...
describe('NetworkTests', () => {
    ...
    test('short-lived token authentication state is not cleared while account loading state is still unknown', async () => {
        const TEST_USER_LOGIN = 'test@testguy.com';
        const TEST_USER_ACCOUNT_ID = 1;
        ...
        // Simulate the startup race where SESSION has hydrated and marked short-lived
        // auth as active, but ACCOUNT has not yet reached its final loading state.
    ...
    test('HTTP 407 auth flow does not start reauthentication while offline', async () => {
        const TEST_USER_LOGIN = 'test@testguy.com';
        const TEST_USER_ACCOUNT_ID = 1;
        ...
        expect(commandNames.filter((commandName) => commandName === 'Authenticate')).toHaveLength(0);
        expect(sessionState?.authToken).toBe(initialAuthToken);
    ...
    test('Authenticate HTTP 403 redirects to sign in without retrying re-authentication', async () => {
        jest.useFakeTimers();
        ...
        expect(commandNames.filter((commandName) => commandName === 'Authenticate')).toHaveLength(1);
        expect(redirectToSignInSpy).toHaveBeenCalledWith('passwordForm.error.invalidLoginOrPassword');
