...
describe('OptionsListUtils', () => {
    ...
    describe('getValidOptions()', () => {
        ...
        it('should include Manager McTest in results by default', () => {
            ...
            const result = getValidOptions(
                {
                    reports: OPTIONS_WITH_MANAGER_MCTEST.reports,
                    personalDetails: OPTIONS_WITH_MANAGER_MCTEST.personalDetails,
                },
                allPolicies,
                {},
                loginList,
    ...
    describe('canCreateOptimisticPersonalDetailOption()', () => {
        ...
        it('should allow to create optimistic option when partial-match contacts exist but none has an exact login match', () => {
            // Regression test for #88730: the old gate returned false whenever any option was present,
            // suppressing userToInvite even when no existing contact matched the typed email exactly.
            const canCreate = canCreateOptimisticPersonalDetailOption({
                searchValue: 'unknown@example.com',
                currentUserOption: {login: 'currentuser@expensify.com'} as OptionData,
                personalDetailsOptions: [{login: 'unknownuser@other.com'} as OptionData],
                recentReportOptions: [{login: 'unknown@other.org'} as OptionData],
            });

            expect(canCreate).toBe(true);
        });

        it('should not allow to create option when a contact with an exact login match exists', () => {
            const canCreate = canCreateOptimisticPersonalDetailOption({
                searchValue: 'known@example.com',
                currentUserOption: {login: 'currentuser@expensify.com'} as OptionData,
                personalDetailsOptions: [{login: 'known@example.com'} as OptionData],
                recentReportOptions: [],
            });
