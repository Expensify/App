...
describe('OptionsListUtils', () => {
    ...
    describe('getValidOptions() with deferContactHydration', () => {
        ...
        it('should leave surviving contacts without hydration so only the caller-rendered page is ever built', () => {
            ...
            const config = {dateFnsLocale: undefined, personalDetails: PERSONAL_DETAILS, deferContactHydration: true as const};
        ...
        it.each([false, true])('should preserve selected and bold marks through deferred hydration ($shouldBoldTitleByDefault)', (shouldBoldTitleByDefault) => {
            ...
            const config = {dateFnsLocale: undefined, personalDetails: PERSONAL_DETAILS, selectedOptions: [{login: selectedLogin}], includeSelectedOptions: true, shouldBoldTitleByDefault};
            const {options: eager} = getValidOptions(buildLazyList(), allPolicies, {}, loginList, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, undefined, config, translateLocal);
            const {options: deferred} = getValidOptions(
                lazyList,
                allPolicies,
        ...
        it('should expose the same current-user option after deferred hydration as the eager path', () => {
            ...
            const config = {dateFnsLocale: undefined, personalDetails: PERSONAL_DETAILS, includeCurrentUser: true};
            const {options: eager} = getValidOptions(buildLazyList(), allPolicies, {}, loginList, CURRENT_USER_ACCOUNT_ID, CURRENT_USER_EMAIL, undefined, config, translateLocal);
            const {options: deferred} = getValidOptions(
                buildLazyList(),
                allPolicies,
        ...
        it('should keep GBR indicators when the caller asks for them', () => {
            ...
            const config = {dateFnsLocale: undefined, personalDetails: PERSONAL_DETAILS, shouldShowGBR: true};
