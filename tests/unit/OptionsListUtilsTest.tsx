...
describe('OptionsListUtils', () => {
    ...
    describe('getSearchOptions()', () => {
        it('should return all options when no search value is provided', () => {
            ...
            const results = getSearchOptions({options: OPTIONS, draftComments: {}, nvpDismissedProductTraining, betas: [CONST.BETAS.ALL]});
    ...
    describe('orderOptions()', () => {
        ...
        it('should sort personal details options alphabetically when only personal details are provided', () => {
            ...
            let results: Pick<Options, 'personalDetails' | 'recentReports'> = getValidOptions({personalDetails: OPTIONS.personalDetails, reports: []}, {}, nvpDismissedProductTraining);
            // When we call orderOptions()
            results = orderOptions(results);
    ...
    describe('getValidOptions()', () => {
        it('should return empty options when no reports or personal details are provided', () => {
            ...
            // When we call getValidOptions()
            const results = getValidOptions({reports: [], personalDetails: []}, {}, nvpDismissedProductTraining);
        ...
        it('should include Concierge by default in results', () => {
            ...
            // When we call getValidOptions()
            const results = getValidOptions({reports: OPTIONS_WITH_CONCIERGE.reports, personalDetails: OPTIONS_WITH_CONCIERGE.personalDetails}, {}, nvpDismissedProductTraining);
        ...
        it('should exclude Chronos when excludedLogins is specified', () => {
            ...
            // When we call getValidOptions()
            const results = getValidOptions({reports: OPTIONS_WITH_CHRONOS.reports, personalDetails: OPTIONS_WITH_CHRONOS.personalDetails}, {}, nvpDismissedProductTraining, {
        ...
        it('should keep admin rooms if specified', () => {
            ...
            const results = getValidOptions({reports: [adminRoom], personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining, {
        ...
        it('should include brickRoadIndicator if showRBR is true', () => {
            ...
            const results = getValidOptions({reports: [workspaceChat], personalDetails: []}, {}, nvpDismissedProductTraining, {
                includeMultipleParticipantReports: true,
                showRBR: true,
            });
    ...
    describe('getValidOptions() for chat room', () => {
        it('should include all reports by default', () => {
            ...
            // When we call getValidOptions()
            const results = getValidOptions(OPTIONS_WITH_WORKSPACE_ROOM, {}, nvpDismissedProductTraining, {
                includeRecentReports: true,
                includeMultipleParticipantReports: true,
                includeP2P: true,
    ...
    describe('getValidOptions() for group Chat', () => {
        ...
        it('should exclude the report from selected options if shouldExcludeSelectedByReportID is true', () => {
            const selectedPerson = PERSONAL_DETAILS['3'];
            const selectedReport = REPORTS['3'] as unknown as OptionData;

            // getValidOptions with shouldExcludeSelectedByReportID === false
            let results = getValidOptions({reports: OPTIONS.reports, personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining, {
            ...
            // The report is not excluded because shouldExcludeSelectedByReportID is false
            expect(results.recentReports.some((option) => option.reportID === selectedReport.reportID)).toBe(true);

            // getValidOptions with shouldExcludeSelectedByReportID === true
            results = getValidOptions({reports: OPTIONS.reports, personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining, {
                excludeLogins: CONST.EXPENSIFY_EMAILS_OBJECT,
                selectedOptions: [selectedPerson, selectedReport],
        ...
        it('should show all reports when maxRecentReportElements is not specified', () => {
            ...
            const resultsWithoutLimit = getValidOptions({reports: OPTIONS.reports, personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining);
            const resultsWithLimit = getValidOptions({reports: OPTIONS.reports, personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining, {maxRecentReportElements: 2});
    ...
    describe('getShareDestinationsOptions()', () => {
        it('should exclude archived rooms and hidden threads from share destinations', () => {
            ...
            const results = getValidOptions({reports: filteredReports, personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining, {
                betas: [],
                includeMultipleParticipantReports: true,
        ...
        it('should include DMS, group chats, and workspace rooms in share destinations', () => {
            ...
            // When we call getValidOptions for share destination with an empty search value
            const results = getValidOptions({reports: filteredReportsWithWorkspaceRooms, personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining, {
                betas: [],
                includeMultipleParticipantReports: true,
                showChatPreviewLine: true,
                forcePolicyNamePreview: true,
    ...
    describe('filterAndOrderOptions()', () => {
        ...
        it('should filter options by email if dot is skipped in the email', () => {
            ...
            // When we call getSearchOptions with all betas
            const options = getSearchOptions({options: OPTIONS_WITH_PERIODS, draftComments: {}, nvpDismissedProductTraining, betas: [CONST.BETAS.ALL]});
            // When we pass the returned options to filterAndOrderOptions with a search value and sortByReportTypeInSearch param
            const filteredOptions = filterAndOrderOptions(options, searchText, COUNTRY_CODE, {sortByReportTypeInSearch: true});
        ...
        it('should include workspace rooms in the search results', () => {
            ...
            // When we call getSearchOptions with all betas
            const options = getSearchOptions({options: OPTIONS_WITH_WORKSPACE_ROOM, draftComments: {}, nvpDismissedProductTraining, betas: [CONST.BETAS.ALL]});
            // When we pass the returned options to filterAndOrderOptions with a search value
            const filteredOptions = filterAndOrderOptions(options, searchText, COUNTRY_CODE);
        ...
        it('should prioritize options with matching display name over chat rooms', () => {
            ...
            // When we call getSearchOptions with all betas
            const options = getSearchOptions({options: OPTIONS_WITH_CHAT_ROOMS, draftComments: {}, nvpDismissedProductTraining, betas: [CONST.BETAS.ALL]});
            // When we pass the returned options to filterAndOrderOptions with a search value
        ...
        it('should put the item with latest lastVisibleActionCreated on top when search value match multiple items', () => {
            ...
            // Given a set of options
            const options = getSearchOptions({options: OPTIONS, draftComments: {}, nvpDismissedProductTraining});
        ...
        it('should not show any recent reports if a search value does not match the group chat name (getShareDestinationsOptions)', () => {
            ...
            const options = getValidOptions({reports: filteredReports, personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining, {
                betas: [],
                includeMultipleParticipantReports: true,
                showChatPreviewLine: true,
                forcePolicyNamePreview: true,
        ...
        it('should return a workspace room when we search for a workspace room(getShareDestinationsOptions)', () => {
            ...
            // When we call getValidOptions for share destination with the filteredReports
            const options = getValidOptions({reports: filteredReportsWithWorkspaceRooms, personalDetails: OPTIONS.personalDetails}, {}, nvpDismissedProductTraining, {
                betas: [],
                includeMultipleParticipantReports: true,
                showChatPreviewLine: true,
                forcePolicyNamePreview: true,
        ...
        it('should return latest lastVisibleActionCreated item on top when search value matches multiple items (getSearchOptions)', () => {
            ...
            return waitForBatchedUpdates()
                ...
                .then(() => {
                    ...
                    const results = getSearchOptions({options: OPTIONS_WITH_PERIODS, draftComments: {}, nvpDismissedProductTraining});
                    // When we pass the returned options to filterAndOrderOptions with a search value
        ...
        it('should order self dm always on top if the search matches with the self dm login', () => {
            ...
            const options = getSearchOptions({options: OPTIONS_WITH_SELF_DM, draftComments: {}, nvpDismissedProductTraining, betas: [CONST.BETAS.ALL]});
            // When we call filterAndOrderOptions with a search value
