...
} from "@libs/OptionsListUtils/searchMatchUtils";
import {
...
const renderLocaleContextProvider = () => {
  return render(
    <ComposeProviders
      components={[OnyxListItemProvider, LocaleContextProvider]}
    >
      <View>TEST</View>
    </ComposeProviders>,
  );
...
describe("OptionsListUtils", () => {
   ...
   */
  const createMockReportAttributesDerived = (
    reports: OnyxCollection<Report>,
    personalDetails: PersonalDetailsList,
    currentUserAccountID: number,
  ): Record<string, ReportAttributes> => {
    ...
    for (const report of reportValues) {
      ...
      if (report.type === CONST.REPORT.TYPE.CHAT && report.participants) {
        ...
        if (participantAccountIDs.length > 0) {
          const participantNames = participantAccountIDs
            .map((accountID) => personalDetails[accountID]?.displayName)
            .filter(Boolean);
  ...
  const MOCK_REPORT_ATTRIBUTES_DERIVED_RAW = createMockReportAttributesDerived(
    REPORTS,
    PERSONAL_DETAILS,
  ...
  const MOCK_REPORT_ATTRIBUTES_DERIVED_WITH_CONCIERGE =
    createMockReportAttributesDerived(
      REPORTS_WITH_CONCIERGE,
      PERSONAL_DETAILS_WITH_CONCIERGE,
      CURRENT_USER_ACCOUNT_ID,
    );
  const MOCK_REPORT_ATTRIBUTES_DERIVED_WITH_CHRONOS =
    createMockReportAttributesDerived(
      REPORTS_WITH_CHRONOS,
      PERSONAL_DETAILS_WITH_CHRONOS,
      CURRENT_USER_ACCOUNT_ID,
    );
  const MOCK_REPORT_ATTRIBUTES_DERIVED_WITH_RECEIPTS =
    createMockReportAttributesDerived(
      REPORTS_WITH_RECEIPTS,
      PERSONAL_DETAILS_WITH_RECEIPTS,
      CURRENT_USER_ACCOUNT_ID,
    );
  const MOCK_REPORT_ATTRIBUTES_DERIVED_WITH_WORKSPACE_ROOM =
    createMockReportAttributesDerived(
  ...
  beforeAll(async () => {
    ...
    await Onyx.set(
      `${ONYXKEYS.COLLECTION.REPORT_NAME_VALUE_PAIRS}10`,
      reportNameValuePairs,
    );
  ...
  describe("getSearchOptions()", () => {
    it("should return all options when no search value is provided", () => {
      ...
      expect(results.recentReports.length).toBe(
        Object.values(OPTIONS.reports).length - 1,
      );
    });
    ...
    it("should include a policy expense chat when the current user notification preference is hidden", () => {
      ...
      const { options: results } = getSearchOptions({
        ...
        reportAttributesDerived: createMockReportAttributesDerived(
          memberWorkspaceChat,
          PERSONAL_DETAILS,
          CURRENT_USER_ACCOUNT_ID,
        ),
      ...
      expect(
        results.recentReports.some((report) => report.reportID === "20"),
      ).toBe(true);
    });
    ...
    it("should include current user when includeCurrentUser is true for type:chat from suggestions", () => {
      ...
      const currentUserOption = results.personalDetails.find(
        (option) => option.login === "tonystark@expensify.com",
      );
    ...
    it("should use policyCollection to filter workspace chats correctly", () => {
      ...
      const workspaceRoom = results.recentReports.find(
        (report) => report.subtitle === "Avengers Room",
      );
    ...
    it("should pass conciergeReportID through to results when provided", () => {
      ...
      const conciergeOption = results.recentReports.find(
        (option) => option.login === "concierge@expensify.com",
      );
  ...
  describe("orderOptions()", () => {
    it("should sort options alphabetically and preserves reportID for personal details with existing reports", () => {
      ...
      let results: Pick<Options, "personalDetails" | "recentReports"> =
        validOptions;
      ...
      expect(results.personalDetails.length).toBe(
        Object.values(OPTIONS.personalDetails).length - 1,
      );
      ...
      const personalDetailWithExistingReport = results.personalDetails.find(
        (personalDetail) =>
  ...
  describe("getValidOptions()", () => {
    ...
    it("should include Concierge by default in results", () => {
      ...
      expect(results.personalDetails.length).toBe(
        Object.values(OPTIONS_WITH_CONCIERGE.personalDetails).length - 1,
      );
    ...
    it.each([
      ...
      "should preserve a contact when the canonical matcher finds $description",
      ({ displayName, searchString }) => {
        ...
        expect(preFilteredOptions.personalDetails).toEqual(
          expect.arrayContaining([
            expect.objectContaining({ login: "contact1003@example.com" }),
          ]),
        );
        ...
        expect(filteredOptions.personalDetails).toEqual([
          expect.objectContaining({ login: "contact1003@example.com" }),
        ]);
      },
    );
    ...
    it("should exclude Concierge when excludedLogins is specified", () => {
      ...
      expect(results.personalDetails.length).toBe(
        Object.values(OPTIONS_WITH_CONCIERGE.personalDetails).length - 2,
      );
    ...
    it("should exclude Chronos when excludedLogins is specified", () => {
      ...
      expect(results.personalDetails.length).toBe(
        Object.values(OPTIONS_WITH_CHRONOS.personalDetails).length - 2,
      );
    ...
    it("should exclude Receipts option from results when excludedLogins is specified", () => {
      ...
      expect(results.personalDetails.length).toBe(
        Object.values(OPTIONS_WITH_RECEIPTS.personalDetails).length - 2,
      );
    ...
    it("should keep admin rooms if specified", () => {
      ...
      const adminRoomOption = results.recentReports.find(
        (report) => report.reportID === "1455140530846319",
      );
    ...
    it("should include brickRoadIndicator if showRBR is true", () => {
      ...
      expect(results.recentReports.at(0)?.brickRoadIndicator).toBe(
    ...
    it("should mark a personal detail as selected in place when it matches a selected option and includeSelectedOptions is true", () => {
      ...
      const selectedOptions = [
        { accountID: 3, login: "peterparker@expensify.com" },
      ];
      ...
      const selectedDetail = results.personalDetails.find(
      ...
      const otherDetail = results.personalDetails.find(
        (option) => option.login === "reedrichards@expensify.com",
      );
  ...
  describe("getValidOptions() with lazy contact options", () => {
    ...
    it("should defer the full option build for every contact", () => {
      ...
      expect(
        lazyList.personalDetails.every((option) => !option.isHydrated),
      ).toBe(true);
      expect(
        lazyList.personalDetails.every((option) => !("icons" in option)),
      ).toBe(true);
    });
    ...
    it("should produce results identical to eagerly built options after hydration", () => {
      ...
      expect(
        lazyResults.personalDetails.every(
          (option) => option.icons !== undefined,
        ),
      ).toBe(true);
      expect(
        lazyResults.personalDetails.every((option) => !("hydrate" in option)),
      ).toBe(true);
    ...
    it.each([
      ...
      "should preserve a report match through the pre-filter and final filter for $description",
      ({ reportText, reportLogin, searchText }) => {
        ...
        const searchString = getSearchValueForPhoneOrEmail(
          searchText,
          COUNTRY_CODE,
        );
        ...
        expect(filteredOptions.recentReports).toEqual(
          expect.arrayContaining([
    ...
    it("should keep every shell filter and sort field identical to the hydrated option", () => {
      ...
      expect(lazyList.personalDetails.length).toBe(
        eagerList.personalDetails.length,
      );
    ...
    it("should produce results identical to eagerly built options with custom exclusions", () => {
      ...
      expect(
        lazyResults.personalDetails.some(
          (option) => option.login === "peterparker@expensify.com",
        ),
      ).toBe(false);
      expect(lazyResults.personalDetails).toEqual(eagerResults.personalDetails);
    ...
    it("should hydrate correctly after a filtered option list cache hit", () => {
      ...
      expect(
        cachedLazyList.personalDetails.every((option) => !option.isHydrated),
      ).toBe(true);
      ...
      if (freshShell && cachedShell) {
        expect(hydrateContactOption(cachedShell).icons).toBe(
          hydrateContactOption(freshShell).icons,
        );
      }
      ...
      expect(cachedResults.personalDetails).toEqual(
        firstResults.personalDetails,
      );
      expect(cachedResults.personalDetails).toEqual(
        eagerResults.personalDetails,
      );
    });
    ...
    it("serves the SearchRouter empty-query list from the cache on the second build", () => {
      ...
      const { maxRecentReports, includeP2P, deferContactsUntilSearch } =
    ...
    it("should hydrate with the build-time inputs, keeping brickRoadIndicator without any caller-provided data", () => {
      ...
      const dmReportID = baseList.personalDetails.find(
        (option) => option.reportID,
      )?.reportID;
      ...
      const shell = lazyList.personalDetails.find(
        (option) => option.reportID === dmReportID,
      );
      ...
      expect(hydrated.brickRoadIndicator).toBe(
        CONST.BRICK_ROAD_INDICATOR_STATUS.ERROR,
      );
    });
    ...
    it("should hydrate lazy shells when mixed with fully-built device contacts", () => {
      ...
      expect(
        lazyResults.personalDetails.some(
          (option) => option.login === deviceContactLogin,
        ),
      ).toBe(true);
      expect(
        lazyResults.personalDetails.find(
          (option) => option.login === deviceContactLogin,
        )?.icons,
      ).toBeDefined();
      expect(
  ...
  describe("lazy contact hydration vs. a direct createOption build", () => {
    ...
    const PARITY_REPORTS: OnyxCollection<Report> = {
      [PARITY_REPORT_ID]: PARITY_REPORT,
    };
    ...
    it("should carry the brick road status from reportAttributesDerived into the built option", () => {
      ...
      expect(hydrateContactOption(shell).brickRoadIndicator).toBe(
        CONST.BRICK_ROAD_INDICATOR_STATUS.ERROR,
      );
    });
  ...
  describe("lazy contact options on the warm path", () => {
    ...
    const buildIdentity = (option: PersonalDetailOptionOrShell) =>
      hydrateContactOption(option).icons;
    ...
    it("should not rebuild any contact option when the second option list comes from the entry cache", () => {
      ...
      expect(secondResults.personalDetails).toEqual(
        firstResults.personalDetails,
      );
    });
  ...
  describe("getValidOptions() for chat room", () => {
    it("should include all reports by default", () => {
      ...
      expect(results.recentReports.length).toBe(
        OPTIONS_WITH_WORKSPACE_ROOM.reports.length - 1,
      );
      expect(results.recentReports).toEqual(
        expect.arrayContaining([expect.objectContaining({ reportID: "14" })]),
      );
    });
  ...
  describe("getValidOptions() for group Chat", () => {
    it("should exclude users with recent reports from personalDetails", () => {
      ...
      const reportLogins = new Set(
        results.recentReports.map((reportOption) => reportOption.login),
      );
      const personalDetailsOverlapWithReports = results.personalDetails.every(
        (personalDetailOption) => reportLogins.has(personalDetailOption.login),
      );
    ...
    it("should exclude selected options", () => {
      ...
      expect(
        results.recentReports.every(
          (option) => option.login !== "peterparker@expensify.com",
        ),
      ).toBe(true);
      expect(
        results.personalDetails.every(
          (option) => option.login !== "peterparker@expensify.com",
        ),
      ).toBe(true);
    });
    ...
    it("should limit recent reports when maxRecentReportElements is specified", () => {
      ...
      expect(results.recentReports.length).toBeLessThanOrEqual(
        maxRecentReports,
      );
    });
    ...
    it("should show all reports when maxRecentReportElements is not specified", () => {
      ...
      expect(resultsWithoutLimit.recentReports.length).toBeGreaterThanOrEqual(
        resultsWithLimit.recentReports.length,
      );
    });
    ...
    it("should not affect personalDetails count when maxRecentReportElements is specified", () => {
      ...
      expect(resultsWithLimit.personalDetails.length).toBe(
        resultsWithoutLimit.personalDetails.length,
      );
    });
    ...
    it("should respect maxRecentReportElements when combined with maxElements", () => {
      ...
      expect(
        results.recentReports.length + results.personalDetails.length,
      ).toBeLessThanOrEqual(maxTotalElements);
    });
  });
  ...
  describe("getShareDestinationsOptions()", () => {
    ...
    it("should exclude archived rooms and hidden threads from share destinations", () => {
      ...
      const filteredReports = Object.values(OPTIONS.reports).reduce<
        OptionList["reports"]
      >((filtered, option) => {
        const report = option.item;
        const { result: isReportArchived } = renderHook(() =>
          useReportIsArchived(report.reportID),
        );
        if (
          canUserPerformWriteAction(report, isReportArchived.current) &&
          canCreateTaskInReport(report) &&
          !isCanceledTaskReport(report)
        ) {
      ...
      expect(results.recentReports.length).toBe(
        Object.values(OPTIONS.reports).length - 2,
      );
    });
    ...
    it("should include DMS, group chats, and workspace rooms in share destinations", () => {
      ...
      const filteredReportsWithWorkspaceRooms = Object.values(
        OPTIONS_WITH_WORKSPACE_ROOM.reports,
      ).reduce<OptionList["reports"]>((filtered, option) => {
        const report = option.item;
        const { result: isReportArchived } = renderHook(() =>
          useReportIsArchived(report.reportID),
        );
        ...
        if (
          canUserPerformWriteAction(report, isReportArchived.current) ||
          isExpensifyOnlyParticipantInReport(report)
        ) {
          filtered.push(option);
        }
        return filtered;
      ...
      expect(results.recentReports.length).toBe(
        Object.values(OPTIONS_WITH_WORKSPACE_ROOM.reports).length - 2,
      );
    });
  });
  ...
  describe("formatMemberForList()", () => {
    it("should format members correctly", () => {
      ...
      const formattedMembers = Object.values(PERSONAL_DETAILS).map(
        (personalDetail) => formatMemberForList(personalDetail),
      );
      ...
      expect(
        formattedMembers
          .slice(1)
          .every((personalDetail) => !personalDetail.isSelected),
      ).toBe(true);
      // Then all items should be enabled
      expect(
        formattedMembers.every((personalDetail) => !personalDetail.isDisabled),
      ).toBe(true);
    });
  });
  ...
  describe("filterAndOrderOptions()", () => {
    it("should return all options when search is empty", () => {
      ...
      expect(
        filteredOptions.recentReports.length +
          filteredOptions.personalDetails.length,
      ).toBe(14);
    });
    ...
    it("should return filtered options in correct order", () => {
      ...
      expect(filteredOptions.recentReports.at(3)?.text).toBe(
        "Mister Fantastic, Invisible Woman",
      ); // This again is a report with '2022-11-22 03:26:02.015'
    });
    ...
    it("should find archived chats", () => {
      ...
      expect(filteredOptions.recentReports.length).toBe(1);
      // Then the returned report should match the search text
      expect(!!filteredOptions.recentReports.at(0)?.private_isArchived).toBe(
        true,
      );
    });
    ...
    it("should filter options by email if dot is skipped in the email", () => {
      ...
      expect(filteredOptions.recentReports.length).toBe(1);
      // Then the returned report should match the search text
      expect(filteredOptions.recentReports.at(0)?.login).toBe(
    ...
    it("should include workspace rooms in the search results", () => {
      ...
      expect(filteredOptions.recentReports.length).toBe(1);
      // Then the returned report should match the search text
      expect(filteredOptions.recentReports.at(0)?.subtitle).toBe(
        "Avengers Room",
      );
    });
    ...
    it("should prioritize options with matching display name over chat rooms", () => {
      ...
      const MOCK_REPORT_ATTRIBUTES_DERIVED_WITH_CHAT_ROOM =
        createMockReportAttributesDerived(
    ...
    it("should put the item with latest lastVisibleActionCreated on top when search value match multiple items", () => {
      ...
      expect(filteredOptions.recentReports.at(0)?.text).toBe(
        "Mister Fantastic",
      );
      // Then the second report should match the search text
      expect(filteredOptions.recentReports.at(1)?.text).toBe(
        "Mister Fantastic, Invisible Woman",
      );
    });
    ...
    it("should not show any recent reports if a search value does not match the group chat name (getShareDestinationsOptions)", () => {
      ...
      >((filtered, option) => {
        const report = option.item;
        if (
          canUserPerformWriteAction(report, false) &&
          canCreateTaskInReport(report) &&
          !isCanceledTaskReport(report)
        ) {
          filtered.push(option);
    ...
    it("should show the option from personal details when searching for personal detail with no existing report", () => {
      ...
      expect(filteredOptions.personalDetails.at(0)?.login).toBe(
    ...
    it("should filter out duplicated entries by login", () => {
      ...
      OPTIONS.personalDetails = OPTIONS.personalDetails.flatMap((obj) => [
        obj,
        { ...obj },
      ]);
      ...
      const matchingEntries = filteredOptions.personalDetails.filter(
        (detail) => detail.login === login,
      );
    ...
    it("should return the same matches for normalized multi-word queries with extra spaces", () => {
      ...
      expect(
        multiSpaceQueryResults.recentReports.map((option) => option.reportID),
      ).toEqual(
        spaceSeparatedQueryResults.recentReports.map(
          (option) => option.reportID,
        ),
      );
      expect(
        multiSpaceQueryResults.personalDetails.map(
          (option) => option.accountID,
        ),
      ).toEqual(
        spaceSeparatedQueryResults.personalDetails.map(
          (option) => option.accountID,
        ),
      );
    });
  ...
  describe("Alternative text", () => {
    it("The text should not contain the last actor's name at prefix if the report is archived.", async () => {
      ...
      expect(archivedReport?.lastMessageText).toBe(
  ...
  describe("getAlternateText()", () => {
    ...
    const buildConfig = (
      overrides: Partial<AlternateTextConfig> = {},
    ): AlternateTextConfig => ({
    ...
    const seedActions = async (
      ...
      await Onyx.set(
        ...
        Object.fromEntries(
          actions.map((action) => [action.reportActionID, action]),
        ),
      );
      await waitForBatchedUpdates();
    };
    ...
    it("should show the archive reason without an actor prefix when the report is archived", async () => {
      ...
      const result = getAlternateText(
        option,
        { showChatPreviewLine: true },
        buildConfig({ isReportArchived: true }),
      );
    ...
    it("should keep the actor prefix when currentUserAccountID is undefined", async () => {
      ...
      const result = getAlternateText(
        option,
        { showChatPreviewLine: true },
        buildConfig({ currentUserAccountID: undefined }),
      );
    ...
    it("should replace the preview with the rename message for a RENAMED last action", async () => {
      ...
      expect(result).toBe(
        'Spider-Man renamed this room to "New Room" (previously "Old Room")',
      );
    });
    ...
    it("should replace the preview with the leave message for a room LEAVE_ROOM last action", async () => {
      const reportID = "9358";
      await setReport(
        buildRoomReport({ reportID, lastMessageText: "left the chat" }),
      );
      await seedActions(
        reportID,
        buildAction(CONST.REPORT.ACTIONS.TYPE.ROOM_CHANGE_LOG.LEAVE_ROOM, 3),
      );
      const option: OptionData = {
        reportID,
        keyForList: "",
    ...
    it("should prefix the action message with the actor for a policy LEAVE_ROOM last action", async () => {
      const reportID = "9359";
      await setReport(
        buildRoomReport({ reportID, lastMessageText: "left the workspace" }),
      );
      // The fake action's message text is 'hey'
      await seedActions(
        reportID,
        buildAction(CONST.REPORT.ACTIONS.TYPE.POLICY_CHANGE_LOG.LEAVE_ROOM, 3),
      );
      const option: OptionData = {
        reportID,
        keyForList: "",
    ...
    it("should build the invite message with member count and room name", async () => {
      const reportID = "9360";
      await setReport(
        buildRoomReport({ reportID, lastMessageText: "invited" }),
      );
      await seedActions(
        reportID,
        buildAction(
          ...
          3,
          { targetAccountIDs: [4, 5], roomName: "#galaxy" },
        ),
      );
      const option: OptionData = {
        reportID,
    ...
    it("should build the remove message with a singular member and room name", async () => {
      const reportID = "9361";
      await setReport(
        buildRoomReport({ reportID, lastMessageText: "removed" }),
      );
      await seedActions(
        reportID,
        buildAction(
          ...
          3,
          { targetAccountIDs: [4], roomName: "#galaxy" },
        ),
      );
      const option: OptionData = {
        reportID,
    ...
    it("should count invited members from lastMessageHtml mentions when targetAccountIDs is empty", async () => {
      ...
      await setReport(
        buildRoomReport({
          ...
          lastMessageText: "invited",
          lastMessageHtml:
            '<mention-user accountID="4"></mention-user> <mention-user accountID="5"></mention-user>',
        }),
      );
      await seedActions(
        reportID,
    ...
    it.each([
      [
        ...
        "issued @Hidden an Expensify Card! The card will arrive in 2-3 business days.",
      ],
      ...
      async (actionName, expectedText) => {
        const reportID =
          actionName === CONST.REPORT.ACTIONS.TYPE.CARD_ISSUED
            ? "9363"
            : "9364";
        await setReport(
          buildRoomReport({
    ...
    it("should fall back to type subtitles when showChatPreviewLine is false", async () => {
      ...
      ).toBe(translateLocal("threads.thread"));
    });
    ...
    it("should thread currentUserAccountID through getValidOptions to build the actor prefix", async () => {
      ...
      const roomOption = options.recentReports.find(
        (option) => option.reportID === reportID,
      );
    ...
    it("should match the LHN alternate text from SidebarUtils.getOptionData for the same room and last action", async () => {
      ...
      const searchAlternateText = getAlternateText(
        option,
        { showChatPreviewLine: true },
        buildConfig(),
      );
    ...
    it.each([
      ...
      async (actionName) => {
        ...
        expect(lhnOption?.alternateText).toBe(
          "Spider-Man: updated a custom unit",
        );
        expect(searchAlternateText).toBe(lhnOption?.alternateText);
      },
    );
    ...
    it("should resolve the actor from the transaction thread when its comment is the newest action of a one-transaction report", async () => {
      ...
      await Onyx.mergeCollection(ONYXKEYS.COLLECTION.REPORT_ACTIONS, {
        ...
        [`${ONYXKEYS.COLLECTION.REPORT_ACTIONS}${TRANSACTION_THREAD_REPORT_ID}`]:
          { [threadComment.reportActionID]: threadComment },
      });
      await waitForBatchedUpdates();
  ...
  describe("createFilteredOptionList()", () => {
    it("should set private_isArchived on personal details options when privateIsArchivedMap is provided", () => {
      ...
      const misterFantasticOption = result.personalDetails.find(
        (pd) => pd.item?.accountID === 1,
      );
      const hydratedMisterFantasticOption = misterFantasticOption
        ? hydrateContactOption(misterFantasticOption)
        : undefined;
    ...
    it("should not set private_isArchived on personal details options when privateIsArchivedMap is empty", () => {
      ...
      const optionsWithArchived = result.personalDetails
        .map(hydrateContactOption)
        .filter((pd) => pd.private_isArchived);
    ...
    it("should correctly map multiple archived reports to personal details options", () => {
      ...
      const invisibleWomanOption = result.personalDetails.find(
        (pd) => pd.item?.accountID === 5,
      );
      const hydratedMisterFantasticOption = misterFantasticOption
        ? hydrateContactOption(misterFantasticOption)
        : undefined;
      const hydratedInvisibleWomanOption = invisibleWomanOption
        ? hydrateContactOption(invisibleWomanOption)
        : undefined;
    ...
    it("should set private_isArchived on report options when privateIsArchivedMap is provided", () => {
      ...
      const report3Option = result.reports.find(
        (r) => r.item?.reportID === "3",
      );
      const report5Option = result.reports.find(
        (r) => r.item?.reportID === "5",
      );
      const report1Option = result.reports.find(
        (r) => r.item?.reportID === "1",
      );
    ...
    it("should correctly map multiple archived reports in privateIsArchivedMap", () => {
      ...
      const report7Option = result.reports.find(
        (r) => r.item?.reportID === "7",
      );
      const report2Option = result.reports.find(
        (r) => r.item?.reportID === "2",
      );
  ...
  describe("filterSelfDMChat()", () => {
    ...
    it("should filter report by alternate text", () => {
      ...
      const result = filterSelfDMChat(
        { ...REPORT, alternateText: ALTERNATE_TEXT, isThread: true },
        ["William"],
      );
    ...
    it("should filter report by alternate text if it is not a thread", () => {
      ...
      // When we call filterSelfDMChat with the report and a search term that matches the report's alternate text
      const result = filterSelfDMChat(
        { ...REPORT, alternateText: ALTERNATE_TEXT, isThread: false },
        ["William"],
      );
    ...
    it("should filter report by subtitle", () => {
      ...
      const result = filterSelfDMChat({ ...REPORT, subtitle: SUBTITLE }, [
        "Software",
      ]);
  ...
  describe("filterReports()", () => {
    ...
    it.each([
      ...
      "should match report text containing $description",
    ...
    it("should match a report by its normalized phone number", () => {
      ...
      const filteredReports = filterReports(
        [report],
        [getSearchValueForPhoneOrEmail("+1 (234) 567-8901", COUNTRY_CODE)],
      );
  ...
  describe("getMostRecentOptions()", () => {
    it("returns the most recent options up to the specified limit", () => {
      ...
      const comparator = (option: OptionData) =>
        option.lastVisibleActionCreated ?? "";
    ...
    it("applies filter function if provided", () => {
      ...
      const comparator = (option: OptionData) =>
        option.lastVisibleActionCreated ?? "";
      const result = optionsOrderBy(
        options,
        comparator,
        2,
        (option) => option.isPinned,
      ).options;
      expect(result.length).toBe(2);
      // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  ...
  describe("optionsOrderBy()", () => {
    ...
    it("calls comparator exactly once per option that passes the filter", () => {
      ...
      const comparator = jest.fn(
        (option: OptionData) => option.lastVisibleActionCreated ?? "",
      );
  ...
  describe("optionsOrderAndGroupBy()", () => {
    ...
    const separators = [
      (option: OptionData) => option.isSelfDM,
      (option: OptionData) => option.isPolicyExpenseChat,
    ];
    ...
    it("does not call comparator for options rejected by the filter", () => {
      ...
      optionsOrderAndGroupBy(
        ...
        2,
        (option) =>
          option.isSelfDM === true || option.isPolicyExpenseChat === true,
      );
    ...
    it("keeps the oldest options per group up to the limit when reversed", () => {
      ...
      const result = optionsOrderAndGroupBy(
        separators,
        options,
        comparator,
        2,
      ...
      expect(
        result.options.map((group) => group.map((option) => option.reportID)),
      ).toStrictEqual([
        ["1", "4"],
        ["2", "5"],
        ["3", "6"],
  ...
  describe("sortAlphabetically", () => {
    ...
    it("should handle single option", () => {
      const options: OptionData[] = [
        { text: "Single", reportID: "1", keyForList: "1" },
      ];
      const sortedOptions = sortAlphabetically(options, "text", localeCompare);
  ...
  describe("getSearchValueForPhoneOrEmail", () => {
    ...
    it("should handle SMS domain removal for valid phone number", () => {
      const result = getSearchValueForPhoneOrEmail(
        "+12345678901@expensify.sms",
        1,
      );
      expect(result).toBe("+12345678901");
    });
  ...
  describe("getParticipantsOption", () => {
    it("returns the personal-detail display name for a known Expensify user", () => {
      ...
      const result = getParticipantsOption(
        participant,
        PERSONAL_DETAILS,
        translateLocal,
      );
      ...
      expect(result.text?.replaceAll(String.fromCharCode(0xa0), " ")).toBe(
        "Iron Man",
      );
  ...
  describe("getIOUConfirmationOptionsFromPayeePersonalDetail", () => {
    it("formats payee text and alternate text with the injected phone-number formatter", () => {
      ...
      const result = getIOUConfirmationOptionsFromPayeePersonalDetail(
        personalDetail,
        translateLocal,
        formatPhoneNumber,
      );
  ...
  describe("getPersonalDetailSearchTerms", () => {
    it("should include display name", () => {
      ...
      const searchTerms = getPersonalDetailSearchTerms(
        { displayName },
        CURRENT_USER_ACCOUNT_ID,
      );
      expect(searchTerms.includes(displayName)).toBe(true);
      const searchTerms2 = getPersonalDetailSearchTerms(
        { participantsList: [{ displayName, accountID: 123 }] },
        CURRENT_USER_ACCOUNT_ID,
      );
  ...
  describe("getReportDisplayOption", () => {
    ...
    it("should return option with isSelfDM alternateText when report is a self DM", () => {
      ...
      expect(result.alternateText).toBe(
        translateLocal("reportActionsView.yourSpace"),
      );
    ...
    it("should return option with invoice room text and alternateText when report is an invoice room", () => {
      ...
      expect(result.alternateText).toBe(
        translateLocal("workspace.common.invoices"),
      );
      expect(result.isDisabled).toBe(true);
    });
  ...
  describe("getReportOption", () => {
    it("should return option with correct workspace name when policy is provided", async () => {
      ...
      expect(option.alternateText).toBe(
        translateLocal("workspace.common.workspace"),
      );
    ...
    it('should title a self DM with "<display name> (you)" even when the derived report name is empty', async () => {
      ...
      expect(option.text).toBe(
        `Test (${translateLocal("common.you").toLowerCase()})`,
      );
      expect(option.alternateText).toBe(
        translateLocal("reportActionsView.yourSpace"),
      );
    ...
    it("should show submits to info when policy has approval workflow", async () => {
      ...
      expect(option.alternateText).toBe(
        translateLocal("iou.submitsTo", "Report Owner"),
      );
    });
    ...
    it("should return option with correct text for workspace chat", async () => {
      ...
      expect(
        option.alternateText === translateLocal("workspace.common.workspace") ||
          option.alternateText?.includes("Submits to"),
      ).toBe(true);
    });
  ...
  describe("getReportDisplayOption", () => {
    ...
    it("should not treat report as concierge chat when conciergeReportID does not match reportID", async () => {
      ...
      expect(optionWithConcierge.reportID).toBe(
        optionWithoutConcierge.reportID,
      );
  ...
  describe("getPolicyExpenseReportOption", () => {
    ...
    it("routes the workspace subtitle through the injected translate function", async () => {
      ...
      const translateWithMarker: LocalizedTranslate = (path, ...parameters) =>
        path === "workspace.common.workspace"
          ? "WorkspaceMarker"
          : translateLocal(path, ...parameters);
  ...
  describe("formatSectionsFromSearchTerm", () => {
    ...
    beforeEach(async () => {
      ...
      await Onyx.merge(
        `${ONYXKEYS.COLLECTION.REPORT}${formatReportID1}`,
        report1,
      );
      await Onyx.merge(
        `${ONYXKEYS.COLLECTION.REPORT}${formatReportID2}`,
        report2,
      );
      await Onyx.merge(
        `${ONYXKEYS.COLLECTION.POLICY}${formatTestPolicyID}`,
        formatPolicy,
      );
    ...
    it("should not set private_isArchived when report is not in the archived map", () => {
      ...
      if (!option) {
        throw new Error(
          "Expected the active policy expense option to be defined",
        );
      }
      expect(option.private_isArchived).toBeUndefined();
    });
    ...
    it("should resolve the policy expense report through the getReportByID resolver instead of the module-level Onyx cache", () => {
      ...
      const getReportByID = (reportID: string | undefined) =>
        reportID === resolverReportID ? resolverReport : undefined;
  ...
  describe("createOptionFromReport", () => {
    it("should subtitle the option as Concierge support only when the threaded conciergeReportID matches the report", () => {
      ...
      expect(conciergeOption.subtitle).toBe(
        translateLocal("reportActionsView.conciergeSupport"),
      );
      ...
      expect(regularOption.subtitle).not.toBe(
        translateLocal("reportActionsView.conciergeSupport"),
      );
    });
  ...
  describe("createFilteredOptionList", () => {
    ...
    const reportsToCollection = (reports: Report[]): OnyxCollection<Report> =>
      Object.fromEntries(reports.map((report) => [report.reportID, report]));

    it("returns the most recent reports when maxRecentReports is less than the input size", () => {
      const reports = [
        createChatReport("101", "2022-01-01 00:00:00"),
        createChatReport("102", "2024-01-01 00:00:00"),
        createChatReport("103", "2023-01-01 00:00:00"),
      ];
      ...
      expect(result.reports.map((option) => option.item?.reportID)).toEqual([
        "102",
        "103",
      ]);
    });
    ...
    it("prioritizes self-DM over newer non-self-DM reports", () => {
      const reports = [
        createChatReport("101", "2024-01-01 00:00:00"),
        createChatReport(
          "102",
          "2020-01-01 00:00:00",
    ...
    it("should sort reports by lastVisibleActionCreated (most recent first)", () => {
      ...
      const result = createFilteredOptionList(
        {},
        Object.fromEntries(
          reportsWithDates.map((report) => [report.reportID, report]),
        ),
        undefined,
        {},
        undefined,
    ...
    it("should keep top-level fields of returned options mutable while the cached entry stays pristine", () => {
      ...
      expect(secondOption?.participantsList).toBe(
        firstOption?.participantsList,
      );
    ...
    it("should recompute cached options when only conciergeReportID changes", () => {
      ...
      const getHydrate = (option: PersonalDetailOptionOrShell | undefined) =>
        option && !option.isHydrated ? option.hydrate : undefined;
      expect(getHydrate(first.personalDetails.at(0))).toBeDefined();
      expect(getHydrate(second.personalDetails.at(0))).not.toBe(
        getHydrate(first.personalDetails.at(0)),
      );
    });
  ...
  describe("orderPersonalDetailsOptions()", () => {
    it("sorts options alphabetically using text values", () => {
      ...
      expect(sorted.map((option) => option.text)).toEqual([
        "aaron",
        "Bob",
        "Charlie",
      ]);
  ...
  describe("getValidOptions with sortedActions", () => {
    it("returns lastIOUCreationDate from the latest IOU action linked via REPORT_PREVIEW", async () => {
      ...
      expect(results.recentReports.length).toBe(1);
      expect(results.recentReports.at(0)?.lastIOUCreationDate).toBe(
        expectedDate,
      );
    });
