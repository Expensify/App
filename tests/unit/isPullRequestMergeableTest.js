...
describe('isPullRequestMergeable', () => {
    test('Pull request immediately mergeable', () => {
        ...
        mockListChecks.mockResolvedValue({
            data: {check_runs: [{status: 'completed'}, {status: 'completed'}]},
        });
        return run().then(() => {
    ...
    test('Pull request mergeable after delay', () => {
        ...
        mockListChecks
            .mockResolvedValue({
                data: {
                    check_runs: [{status: 'completed'}, {status: 'completed'}],
                },
            })
            .mockResolvedValueOnce({
                data: {
    ...
    test('Pull request not mergeable after delay', () => {
        mockGetPullRequest
            ...
            .mockResolvedValueOnce({
                data: {mergeable: null, head: {sha: 'abcd'}},
            })
            .mockResolvedValueOnce({
                data: {mergeable: null, head: {sha: 'abcd'}},
            });
        mockListChecks.mockResolvedValue({
            data: {check_runs: [{status: 'completed'}]},
        });
        return run().then(() => {
