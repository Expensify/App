...
import type {Policy, PolicyCategories, Report, TaxRatesWithDefault, Transaction} from '@src/types/onyx';
import type {PendingAction} from '@src/types/onyx/OnyxCommon';
import type {PersonalDetailsList} from '../utils/OptionListUtilsHelper';
...
describe('OptionsListUtils', () => {
    ...
    const optionListUtilsHelper = createOptionsListUtilsHelper();

    beforeAll(() => {
        return optionListUtilsHelper.beforeAll();
    });

    beforeEach(() => {
        const options = optionListUtilsHelper.generateOptions();
        OPTIONS = options.OPTIONS;
        OPTIONS_WITH_CONCIERGE = options.OPTIONS_WITH_CONCIERGE;
        OPTIONS_WITH_CHRONOS = options.OPTIONS_WITH_CHRONOS;
        OPTIONS_WITH_RECEIPTS = options.OPTIONS_WITH_RECEIPTS;
        OPTIONS_WITH_WORKSPACE_ROOM = options.OPTIONS_WITH_WORKSPACE_ROOM;
    });
    ...
    it('formatMemberForList()', () => {
        const formattedMembers = Object.values(optionListUtilsHelper.PERSONAL_DETAILS).map((personalDetail) => OptionsListUtils.formatMemberForList(personalDetail));
    ...
    describe('filterOptions', () => optionListUtilsHelper.createFilterTests());
