...
describe('DateUtils', () => {
        ...
        Onyx.init({
            ...
            initialKeyStates: {
                ...
                [ONYXKEYS.PERSONAL_DETAILS]: {
                    'current@user.com': {timezone: {selected: 'Etc/UTC'}},
                },
            },
        });
    ...
    it('should return a moment object with the formatted datetime when calling getLocalMomentFromDatetime', () => {
        const localMoment = DateUtils.getLocalMomentFromDatetime(
            LOCALE,
            datetime,
            'America/Los_Angeles',
        );
        expect(moment.isMoment(localMoment)).toBe(true);
        expect(moment(localMoment).format()).toEqual(
            '2022-11-06T16:00:00-08:00',
        );
    });
    ...
    describe('getDBTime', () => {
        it('should return the date in the format expected by the database', () => {
            ...
            expect(getDBTime).toBe(
                moment(getDBTime).format('YYYY-MM-DD HH:mm:ss.SSS'),
            );
        });
