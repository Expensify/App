...
describe('TransactionUtils', () => {
    ...
    describe('isSevenDayHoldViolation', () => {
        it('returns true for an RTER violation whose rterType is sevenDayHold', () => {
            const violation: TransactionViolation = {
                type: CONST.VIOLATION_TYPES.VIOLATION,
                name: CONST.VIOLATIONS.RTER,
                data: {pendingPattern: true, rterType: CONST.RTER_VIOLATION_TYPES.SEVEN_DAY_HOLD},
            };
            expect(TransactionUtils.isSevenDayHoldViolation(violation)).toBe(true);
        });

        it('returns false for a generic pending RTER violation with no rterType', () => {
            const violation: TransactionViolation = {type: CONST.VIOLATION_TYPES.VIOLATION, name: CONST.VIOLATIONS.RTER, data: {pendingPattern: true}};
            expect(TransactionUtils.isSevenDayHoldViolation(violation)).toBe(false);
        });

        it('returns false for a broken-card-connection RTER violation', () => {
            const violation: TransactionViolation = {
                type: CONST.VIOLATION_TYPES.VIOLATION,
                name: CONST.VIOLATIONS.RTER,
                data: {pendingPattern: true, rterType: CONST.RTER_VIOLATION_TYPES.BROKEN_CARD_CONNECTION},
            };
            expect(TransactionUtils.isSevenDayHoldViolation(violation)).toBe(false);
        });

        it('returns false for a rejected-expense violation', () => {
            const violation: TransactionViolation = {type: CONST.VIOLATION_TYPES.VIOLATION, name: CONST.VIOLATIONS.AUTO_REPORTED_REJECTED_EXPENSE};
            expect(TransactionUtils.isSevenDayHoldViolation(violation)).toBe(false);
        });
    });
    ...
    describe('getSubmitViolationsSummary', () => {
        ...
        it('returns all-false/empty when there are no violations', () => {
            ...
            const summary = TransactionUtils.getSubmitViolationsSummary([transaction], {}, SUBMITTER_EMAIL, SUBMITTER_ACCOUNT_ID, undefined, REPORT_OWNER_LOGIN, undefined);
        ...
        it('flags hasSevenDayHoldViolation for a seven-day-hold RTER violation and does not double-count it as generic pending', () => {
            ...
            const summary = TransactionUtils.getSubmitViolationsSummary([transaction], violations, SUBMITTER_EMAIL, SUBMITTER_ACCOUNT_ID, undefined, REPORT_OWNER_LOGIN, undefined);
        ...
        it('flags hasReportBeenRejected when the whole report was rejected back to the submitter, even though the transaction itself has no violations', () => {
            ...
            const summary = TransactionUtils.getSubmitViolationsSummary([transaction], {}, SUBMITTER_EMAIL, SUBMITTER_ACCOUNT_ID, report, REPORT_OWNER_LOGIN, undefined);
        ...
        it('collects any other violation of type VIOLATION into otherViolations, deduped by name', () => {
            ...
            const summary = TransactionUtils.getSubmitViolationsSummary(
