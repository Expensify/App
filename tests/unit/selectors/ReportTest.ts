...
describe('policyRoomNamesSelector', () => {
    ...
    it('returns room names matching the given policyID', () => {
        const reports: OnyxCollection<Report> = {
            ...
            '2': makeReport({reportID: '2', policyID: POLICY_A, reportName: '#random', chatType: CONST.REPORT.CHAT_TYPE.POLICY_ROOM}),
        };
    ...
    it('excludes reports with non-room chatType in the same policy', () => {
        const reports: OnyxCollection<Report> = {
            '1': makeReport({reportID: '1', policyID: POLICY_A, reportName: '#general', chatType: CONST.REPORT.CHAT_TYPE.POLICY_ROOM}),
            '2': makeReport({reportID: '2', policyID: POLICY_A, reportName: 'Expense Report', chatType: CONST.REPORT.CHAT_TYPE.POLICY_EXPENSE_CHAT}),
        };
    ...
    it('excludes reports from other policies', () => {
        const reports: OnyxCollection<Report> = {
            '1': makeReport({reportID: '1', policyID: POLICY_A, reportName: '#general', chatType: CONST.REPORT.CHAT_TYPE.POLICY_ROOM}),
            '2': makeReport({reportID: '2', policyID: POLICY_B, reportName: '#other', chatType: CONST.REPORT.CHAT_TYPE.POLICY_ROOM}),
        };
        expect(policyRoomNamesSelector(POLICY_A)(reports)).toEqual(['#general']);
    });
    ...
    it('excludes reports without reportName', () => {
        const reports: OnyxCollection<Report> = {
            '1': makeReport({reportID: '1', policyID: POLICY_A, reportName: '#general', chatType: CONST.REPORT.CHAT_TYPE.POLICY_ROOM}),
            '2': makeReport({reportID: '2', policyID: POLICY_A, chatType: CONST.REPORT.CHAT_TYPE.POLICY_ROOM}),
        };
        expect(policyRoomNamesSelector(POLICY_A)(reports)).toEqual(['#general']);
    });
    ...
    it('returns empty array when no reports match the policyID', () => {
        const reports: OnyxCollection<Report> = {
            '1': makeReport({reportID: '1', policyID: POLICY_B, reportName: '#other'}),
        };
