...
} from "@components/Charts/utils/chartFontsCache";

const mockFromURI = jest.fn();
const mockMakeFreeTypeFaceFromData = jest.fn();
...
jest.mock("@components/Charts/utils/chartFontAssets", () => {
  ...
  return {
    CHART_SKIA_TYPEFACE_ASSETS: Object.fromEntries(
      typefaceKeys.map((key) => [key, makeAsset(key)]),
    ),
...
function setupSuccessfulFontLoading(failingKey?: ChartSkiaTypefaceKey) {
  ...
  mockMakeFreeTypeFaceFromData.mockImplementation((data: { uri: string }) =>
    makeMockTypeface(data.uri.replace("mock://font/", "")),
  );
}
...
describe("chartFontsCache", () => {
  ...
  it("should return empty chart fonts when every asset fails to load", async () => {
    ...
    expect(
      Object.values(fonts.typefaces).every((typeface) => typeface === null),
    ).toBe(true);
