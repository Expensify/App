...
describe('getSecondaryAction', () => {
    ...
    it('includes SUBMIT option when report total is 0 but there are transactions', async () => {
        ...
        const result = getSecondaryReportActions({currentUserEmail: EMPLOYEE_EMAIL, report, chatReport, reportTransactions: [transaction1, transaction2], originalTransaction: {} as Transaction, violations: {}, policy});
        expect(result.includes(CONST.REPORT.SECONDARY_ACTIONS.SUBMIT)).toBe(true);
    });
