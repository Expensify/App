...
describe('Onyx key export coverage', () => {
    ...
    it('removes the Cloudflare QA session from the export entirely', () => {
        ...
        const input = {
            [ONYXKEYS.CF_SESSION]: {accessToken: 'oauth:access-token', refreshToken: 'oauth:refresh-token', expiresAt: 1753600000000},
            [ONYXKEYS.IS_DEBUG_MODE_ENABLED]: true,
        };

        const result = maskOnyxState(input, true);

        expect(result[ONYXKEYS.CF_SESSION]).toBeUndefined();
        expect(Object.keys(result)).not.toContain(ONYXKEYS.CF_SESSION);
    });
