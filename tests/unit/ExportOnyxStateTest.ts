...
describe('maskOnyxState', () => {
    ...
    it('should mask delegate credentials held under hybridApp', () => {
        ...
        // When the state is exported with masking enabled
        const serialized = JSON.stringify(maskOnyxState(input, true));

        // Then none of the credential values may appear verbatim anywhere in the export
        expect(serialized).not.toContain('live-olddot-auth-token');
        expect(serialized).not.toContain('live-olddot-encrypted-token');
        expect(serialized).not.toContain('auto-generated-password');
    });
});
...
describe('Onyx key export coverage', () => {
    ...
    it('credential field names must be in keysToMask so the fallback can never pass them through', () => {
        // maskFragileData exports any field name it does not recognise verbatim. A key that lands in the
        // maskFragileData bucket (e.g. HYBRID_APP, whose delegateAccessData holds live tokens) therefore
        // relies on keysToMask to catch its secrets. Every credential field name below must stay listed.
        const credentialFieldNames = ['authToken', 'encryptedAuthToken', 'supportAuthToken', 'oldDotCurrentAuthToken', 'oldDotCurrentEncryptedAuthToken', 'oldDotAutoGeneratedPassword'];
