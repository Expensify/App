...
describe('CalendarPicker', () => {
    ...
    test('clicking next month arrow updates the displayed month', () => {
        const {getByTestId, getByText} = render(<CalendarPicker onChange={onChangeMock} />);
    ...
    test('should block the back arrow when there is no available dates in the previous month', () => {
        const {getByTestId} = render(<CalendarPicker minDate={new Date('2023-02-01')} value={new Date('2023-02-17')} onChange={onChangeMock} />);
    ...
    test('should block the next arrow when there is no available dates in the next month', () => {
        const {getByTestId} = render(<CalendarPicker maxDate={new Date('2023-02-24')} value={new Date('2023-02-17')} onChange={onChangeMock} />);
    ...
    test('should open the calendar on a month from max date if it is earlier than current month', () => {
        const {getByLabelText} = render(<CalendarPicker maxDate={new Date('2011-03-01')} onChange={onChangeMock} />);
    ...
    test('should open the calendar on a month from min date if it is later than current month', () => {
        ...
        const {getByLabelText} = render(<CalendarPicker minDate={minDate} onChange={onChangeMock} />);
    ...
    test('should not allow to press earlier day than minDate', () => {
        ...
        const {getByLabelText} = render(<CalendarPicker minDate={minDate} value={date} onChange={onChangeMock} />);
    ...
    test('should not allow to press later day than max', () => {
        ...
        const {getByLabelText} = render(<CalendarPicker maxDate={maxDate} value={date} onChange={onChangeMock} />);
