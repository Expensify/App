...
describe('SearchActions', () => {
    ...
    describe('saveSearch', () => {
        ...
        const queryJSON = getQueryJSON('type:expense status:all');
    ...
    describe('getExportTemplates', () => {
        ...
        it('includes the Reconciliation - All Expenses template when the user is a workspace admin of a policy with company cards enabled', () => {
            const {defaultTemplates} = getExportTemplates([], {}, translateForTemplates, localeCompare, {
        ...
        it('includes the Reconciliation - All Expenses template when the user is a card admin of a policy with the Expensify Card enabled', () => {
            const {defaultTemplates} = getExportTemplates([], {}, translateForTemplates, localeCompare, {
                ...createRandomPolicy(1),
        ...
        it('excludes the Reconciliation - All Expenses template when the admin policy has no card product enabled', () => {
            const {defaultTemplates} = getExportTemplates([], {}, translateForTemplates, localeCompare, {
                ...createRandomPolicy(1),
                role: CONST.POLICY.ROLE.ADMIN,
        ...
        it('excludes the Reconciliation - All Expenses template when includeReconciliationAllExpenses is false for an admin policy', () => {
            const {defaultTemplates} = getExportTemplates(
                [],
                {},
                translateForTemplates,
                localeCompare,
                {...createRandomPolicy(1), role: CONST.POLICY.ROLE.ADMIN, areCompanyCardsEnabled: true},
                true,
                false,
                false,
                false,
    ...
    describe('getFooterConvertedAmounts', () => {
        ...
        it('scopes the request to the given transaction and report IDs', () => {
            ...
            expect(mockRead).toHaveBeenCalledWith(
                READ_COMMANDS.GET_TRANSACTIONS_CONVERTED_AMOUNT,
                expect.objectContaining({transactionIDList: '1,2', reportIDList: '3,4'}),
                expect.anything(),
            );
        });
...
describe('saveSavedViewEdits', () => {
    ...
    it('preserves a custom name and saves under the same savedSearchID so the backend updates the view in place', () => {
        const editedQuery = buildSearchQueryJSON('type:invoice');
        if (!editedQuery) {
            throw new Error('failed to build query JSON');
        }

        const savedSearchID = '42';
        saveSavedViewEdits({queryJSON: editedQuery, editingSavedView: {id: savedSearchID, name: 'My view', query: 'type:expense'}});

        const [command, parameters] = mockWrite.mock.calls.at(-1) ?? [];
        expect(command).toBe(WRITE_COMMANDS.SAVE_SEARCH);
        expect(parameters).toEqual(expect.objectContaining({savedSearchID, newName: 'My view'}));
        ...
        expect(savedSearchesUpdate?.value).toEqual({
            [savedSearchID]: expect.objectContaining({pendingAction: CONST.RED_BRICK_ROAD_PENDING_ACTION.ADD, name: 'My view', query: editedQuery.inputQuery}),
        });
    });
    ...
    it('re-auto-names an auto-named view (name === query) to the edited query so it does not show the stale old query', () => {
        ...
        saveSavedViewEdits({queryJSON: editedQuery, editingSavedView: {id: '42', name: originalQuery.inputQuery, query: originalQuery.inputQuery}});
        ...
        expect(parameters).toEqual(expect.objectContaining({savedSearchID: '42', newName: editedQuery.inputQuery}));
    });
});
