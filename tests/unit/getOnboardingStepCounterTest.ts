...
describe('getOnboardingFlow', () => {
    it('returns [ACCOUNTING, INTERESTED_FEATURES] for VSB without domain context', () => {
        expect(getOnboardingFlow({signupQualifier: 'vsb'})).toEqual([O.DYNAMIC_ACCOUNTING, O.DYNAMIC_INTERESTED_FEATURES]);
    });

    it('returns [EMPLOYEES, ACCOUNTING, INTERESTED_FEATURES] for SMB without domain context', () => {
        expect(getOnboardingFlow({signupQualifier: 'smb'})).toEqual([O.DYNAMIC_EMPLOYEES, O.DYNAMIC_ACCOUNTING, O.DYNAMIC_INTERESTED_FEATURES]);
    });
    ...
    it('returns [PURPOSE, PERSONAL_DETAILS] for individual + EMPLOYER', () => {
        expect(getOnboardingFlow({signupQualifier: 'individual', purposeSelected: ONBOARDING_CHOICES.EMPLOYER})).toEqual([O.DYNAMIC_PURPOSE, O.DYNAMIC_PERSONAL_DETAILS]);
    });
    ...
    it('returns [DYNAMIC_WORK_EMAIL, DYNAMIC_WORK_EMAIL_VALIDATION, ACCOUNTING, INTERESTED_FEATURES] for public + VSB', () => {
    ...
    it('returns [DYNAMIC_WORK_EMAIL, DYNAMIC_WORK_EMAIL_VALIDATION, EMPLOYEES, ACCOUNTING, INTERESTED_FEATURES] for public + SMB', () => {
...
describe('getOnboardingStepCounter', () => {
    it('returns correct step/total/percentage for each page in VSB flow', () => {
        ...
        expect(getOnboardingStepCounter(O.DYNAMIC_ACCOUNTING, ctx)).toEqual({stepCounter: {step: 1, total: 2}, progressBarPercentage: 50});
        expect(getOnboardingStepCounter(O.DYNAMIC_INTERESTED_FEATURES, ctx)).toEqual({stepCounter: {step: 2, total: 2}, progressBarPercentage: 100});
    });

    it('returns correct step/total/percentage for each page in SMB flow', () => {
        const ctx: OnboardingFlowContext = {signupQualifier: 'smb'};
        expect(getOnboardingStepCounter(O.DYNAMIC_EMPLOYEES, ctx)).toEqual({stepCounter: {step: 1, total: 3}, progressBarPercentage: 33});
        expect(getOnboardingStepCounter(O.DYNAMIC_ACCOUNTING, ctx)).toEqual({stepCounter: {step: 2, total: 3}, progressBarPercentage: 67});
        expect(getOnboardingStepCounter(O.DYNAMIC_INTERESTED_FEATURES, ctx)).toEqual({stepCounter: {step: 3, total: 3}, progressBarPercentage: 100});
    });
    ...
    it('returns correct step/total/percentage for individual + MANAGE_TEAM', () => {
        ...
        expect(getOnboardingStepCounter(O.DYNAMIC_PURPOSE, ctx)).toEqual({stepCounter: {step: 1, total: 4}, progressBarPercentage: 25});
        expect(getOnboardingStepCounter(O.DYNAMIC_EMPLOYEES, ctx)).toEqual({stepCounter: {step: 2, total: 4}, progressBarPercentage: 50});
        expect(getOnboardingStepCounter(O.DYNAMIC_ACCOUNTING, ctx)).toEqual({stepCounter: {step: 3, total: 4}, progressBarPercentage: 75});
        expect(getOnboardingStepCounter(O.DYNAMIC_INTERESTED_FEATURES, ctx)).toEqual({stepCounter: {step: 4, total: 4}, progressBarPercentage: 100});
    });
    ...
    it('returns correct step/total/percentage for individual + other purpose', () => {
        ...
        expect(getOnboardingStepCounter(O.DYNAMIC_PURPOSE, ctx)).toEqual({stepCounter: {step: 1, total: 2}, progressBarPercentage: 50});
    ...
    it('returns correct step/total/percentage for public + individual + MANAGE_TEAM', () => {
        ...
        expect(getOnboardingStepCounter(O.DYNAMIC_WORK_EMAIL_VALIDATION, ctx)).toEqual({stepCounter: {step: 2, total: 6}, progressBarPercentage: 33});
        ...
        expect(getOnboardingStepCounter(O.DYNAMIC_ACCOUNTING, ctx)).toEqual({stepCounter: {step: 5, total: 6}, progressBarPercentage: 83});
        expect(getOnboardingStepCounter(O.DYNAMIC_INTERESTED_FEATURES, ctx)).toEqual({stepCounter: {step: 6, total: 6}, progressBarPercentage: 100});
    });
    ...
    it('returns correct step/total/percentage for public + merge + individual + MANAGE_TEAM', () => {
        ...
        expect(getOnboardingStepCounter(O.DYNAMIC_WORK_EMAIL_VALIDATION, ctx)).toEqual({stepCounter: {step: 2, total: 7}, progressBarPercentage: 29});
        ...
        expect(getOnboardingStepCounter(O.DYNAMIC_EMPLOYEES, ctx)).toEqual({stepCounter: {step: 5, total: 7}, progressBarPercentage: 71});
        expect(getOnboardingStepCounter(O.DYNAMIC_ACCOUNTING, ctx)).toEqual({stepCounter: {step: 6, total: 7}, progressBarPercentage: 86});
        expect(getOnboardingStepCounter(O.DYNAMIC_INTERESTED_FEATURES, ctx)).toEqual({stepCounter: {step: 7, total: 7}, progressBarPercentage: 100});
    });
    ...
    it('returns correct step/total/percentage for public + skipped + individual + MANAGE_TEAM (no gaps)', () => {
        ...
        expect(getOnboardingStepCounter(O.DYNAMIC_INTERESTED_FEATURES, ctx)).toEqual({stepCounter: {step: 5, total: 5}, progressBarPercentage: 100});
    });
    ...
    describe('sub-page mappings', () => {
        ...
        it('PRIVATE_DOMAIN resolves to DYNAMIC_WORK_EMAIL_VALIDATION step in public domain flows', () => {
            const ctx: OnboardingFlowContext = {signupQualifier: 'individual', isFromPublicDomain: true, isMergeAccountStepSkipped: false, purposeSelected: ONBOARDING_CHOICES.MANAGE_TEAM};
            expect(getOnboardingStepCounter(O.DYNAMIC_PRIVATE_DOMAIN, ctx)).toEqual(getOnboardingStepCounter(O.DYNAMIC_WORK_EMAIL_VALIDATION, ctx));
        });
        ...
        it('PRIVATE_DOMAIN is its own step in private domain flows', () => {
            ...
            expect(getOnboardingStepCounter(O.DYNAMIC_WORK_EMAIL_VALIDATION, ctx)).toBeUndefined();
        });
    });
    ...
    describe('indeterminate flow (no purposeSelected)', () => {
        const domainVariants: Array<{
            ...
            },
            {
                ...
                prefixPages: [
                    ...
                    {page: O.DYNAMIC_WORK_EMAIL_VALIDATION, step: 2, pct: 33},
                ],
            },
            {
            ...
            },
            {
                ...
                prefixPages: [
                    ...
                    {page: O.DYNAMIC_WORK_EMAIL_VALIDATION, step: 2, pct: 29},
        ...
        it.each(domainVariants)('$label: PURPOSE returns step $expectedPurposeStep without total', ({ctx, expectedPurposeStep, expectedPurposePct}) => {
            const result = getOnboardingStepCounter(O.DYNAMIC_PURPOSE, ctx);
        ...
        it('PRIVATE_DOMAIN resolves to DYNAMIC_WORK_EMAIL_VALIDATION step in indeterminate public merge flow', () => {
    ...
    describe('indeterminate percentages never exceed the next page after purpose selection', () => {
        ...
        it.each(domainVariants)('$label: PURPOSE indeterminate % ≤ min next-page % across all purposes', ({ctx}) => {
            ...
            const purposeResult = getOnboardingStepCounter(O.DYNAMIC_PURPOSE, indeterminateCtx);
            ...
            for (const purpose of Object.values(ONBOARDING_CHOICES)) {
                ...
                const purposeIndex = flow.indexOf(O.DYNAMIC_PURPOSE);
    ...
    it('returns undefined for a page that is not in the flow', () => {
        expect(getOnboardingStepCounter(O.DYNAMIC_PURPOSE, {signupQualifier: 'vsb'})).toBeUndefined();
    });
