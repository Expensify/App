...
describe('getOnboardingFlow', () => {
    ...
    // Public domain + VSB
    it('returns [WORK_EMAIL, ACCOUNTING, INTERESTED_FEATURES] for public + VSB', () => {
    ...
    // Public domain + SMB
    it('returns [WORK_EMAIL, EMPLOYEES, ACCOUNTING, INTERESTED_FEATURES] for public + SMB', () => {
        const flow = getOnboardingFlow({signupQualifier: 'smb', isFromPublicDomain: true});
        expect(flow).toEqual(['Onboarding_Work_Email', 'Onboarding_Employees', 'Onboarding_Accounting', 'Onboarding_Interested_Features']);
    });

    // Public domain + individual + MANAGE_TEAM
    it('returns [WORK_EMAIL, PURPOSE, EMPLOYEES, ACCOUNTING, INTERESTED_FEATURES] for public + individual + MANAGE_TEAM', () => {
        ...
        expect(flow).toEqual(['Onboarding_Work_Email', 'Onboarding_Purpose', 'Onboarding_Employees', 'Onboarding_Accounting', 'Onboarding_Interested_Features']);
    });

    // Public domain + individual + PERSONAL_SPEND
    it('returns [WORK_EMAIL, PURPOSE, PERSONAL_DETAILS, WORKSPACE_OPTIONAL] for public + individual + PERSONAL_SPEND', () => {
        ...
        expect(flow).toEqual(['Onboarding_Work_Email', 'Onboarding_Purpose', 'Onboarding_Personal_Details', 'Onboarding_Workspace_Optional']);
    });

    // Public domain + individual + other
    it('returns [WORK_EMAIL, PURPOSE, PERSONAL_DETAILS] for public + individual + LOOKING_AROUND', () => {
        ...
        expect(flow).toEqual(['Onboarding_Work_Email', 'Onboarding_Purpose', 'Onboarding_Personal_Details']);
    });

    // Private domain + VSB
    it('returns [PERSONAL_DETAILS, WORKSPACES, ACCOUNTING, INTERESTED_FEATURES] for private domain + VSB', () => {
    ...
    // Private domain + SMB
    it('returns [PERSONAL_DETAILS, WORKSPACES, EMPLOYEES, ACCOUNTING, INTERESTED_FEATURES] for private domain + SMB', () => {
        const flow = getOnboardingFlow({signupQualifier: 'smb', hasAccessibleDomainPolicies: true});
        expect(flow).toEqual(['Onboarding_Personal_Details', 'Onboarding_Workspaces', 'Onboarding_Employees', 'Onboarding_Accounting', 'Onboarding_Interested_Features']);
    });

    // Private domain + individual + MANAGE_TEAM
    it('returns [PERSONAL_DETAILS, WORKSPACES, PURPOSE, EMPLOYEES, ACCOUNTING, INTERESTED_FEATURES] for private domain + individual + MANAGE_TEAM', () => {
    ...
    it('returns [PERSONAL_DETAILS, WORKSPACES, PURPOSE, WORKSPACE_OPTIONAL] for private domain + individual + PERSONAL_SPEND', () => {
        ...
        expect(flow).toEqual(['Onboarding_Personal_Details', 'Onboarding_Workspaces', 'Onboarding_Purpose', 'Onboarding_Workspace_Optional']);
    });

    // Private domain + individual + other
    it('returns [PERSONAL_DETAILS, WORKSPACES, PURPOSE] for private domain + individual + EMPLOYER', () => {
        ...
        expect(flow).toEqual(['Onboarding_Personal_Details', 'Onboarding_Workspaces', 'Onboarding_Purpose']);
    });
});
...
describe('getOnboardingStepCounter', () => {
    ...
    it('returns correct step/total/percentage for public + individual + MANAGE_TEAM', () => {
        ...
        expect(getOnboardingStepCounter('Onboarding_Work_Email', ctx)).toEqual({stepCounter: {step: 1, total: 5}, progressBarPercentage: 20});
        expect(getOnboardingStepCounter('Onboarding_Purpose', ctx)).toEqual({stepCounter: {step: 2, total: 5}, progressBarPercentage: 40});
        expect(getOnboardingStepCounter('Onboarding_Employees', ctx)).toEqual({stepCounter: {step: 3, total: 5}, progressBarPercentage: 60});
        expect(getOnboardingStepCounter('Onboarding_Accounting', ctx)).toEqual({stepCounter: {step: 4, total: 5}, progressBarPercentage: 80});
        expect(getOnboardingStepCounter('Onboarding_Interested_Features', ctx)).toEqual({stepCounter: {step: 5, total: 5}, progressBarPercentage: 100});
    });

    it('returns correct step/total/percentage for private domain + individual + MANAGE_TEAM (6-step flow)', () => {
        ...
        expect(getOnboardingStepCounter('Onboarding_Personal_Details', ctx)).toEqual({stepCounter: {step: 1, total: 6}, progressBarPercentage: 17});
        expect(getOnboardingStepCounter('Onboarding_Workspaces', ctx)).toEqual({stepCounter: {step: 2, total: 6}, progressBarPercentage: 33});
    ...
    describe('sub-page mappings', () => {
        it('WORK_EMAIL_VALIDATION maps to WORK_EMAIL step', () => {
            ...
            expect(validationResult).toEqual(workEmailResult);
        });

        it('PRIVATE_DOMAIN maps to PERSONAL_DETAILS step', () => {
            ...
            expect(privateDomainResult).toEqual(personalDetailsResult);
        });
    ...
    describe('indeterminate flow (no purposeSelected)', () => {
        ...
        it('returns stepCounter without total for PURPOSE screen when purpose not yet selected (public domain)', () => {
            ...
            expect(result?.stepCounter.total).toBeUndefined();
            expect(result?.progressBarPercentage).toBe(67);
        });
        ...
        it('returns step 2 without total for WORKSPACES when purpose not yet selected (private domain)', () => {
            const ctx: OnboardingFlowContext = {signupQualifier: 'individual', hasAccessibleDomainPolicies: true};
