...
import {canCreateOptimisticPersonalDetailOption, createOption, createOptionList, filterOption, getValidOptions} from '@libs/PersonalDetailsOptionsListUtils';
...
describe('PersonalDetailsOptionsListUtils', () => {
    ...
    beforeEach(() => {
        ...
        OPTIONS = createOptionList(currentUserAccountID, PERSONAL_DETAILS, ACCOUNT_ID_TO_REPORT_ID_MAP, translateReportObjectToOnyxCollection(REPORTS));
        OPTIONS_WITH_SELF_DM = createOptionList(currentUserAccountID, PERSONAL_DETAILS, ACCOUNT_ID_TO_REPORT_ID_MAP, translateReportObjectToOnyxCollection(REPORTS_WITH_SELF_DM));
    ...
    describe('createOption', () => {
        it('should create an option with the given parameters', () => {
            const option = createOption(PERSONAL_DETAILS['1'], REPORTS['3']);
        ...
        it('should create an option with no reportID when report is not provided', () => {
            const option = createOption(PERSONAL_DETAILS['1'], undefined);
            expect(option).toEqual({
                accountID: 1,
        ...
        it('should create an option with isSelected when isSelected is provided', () => {
            const option = createOption(PERSONAL_DETAILS['1'], undefined, {isSelected: true});
            expect(option).toEqual({
                accountID: 1,
                allReportErrors: undefined,
                alternateText: 'reedrichards@expensify.com',
    ...
    describe('getValidOptions', () => {
        it('should return empty options when no options are provided', () => {
            const results = getValidOptions([], currentUserLogin);
        ...
        describe('Default behavior', () => {
            ...
            beforeAll(() => {
                defaultOptions = getValidOptions(OPTIONS.options, currentUserLogin);
            });
            ...
            it('should include Concierge by default in results', () => {
                const results = getValidOptions(OPTIONS_WITH_CONCIERGE.options, currentUserLogin);
        ...
        it('should return empty recent options when no options are provided', () => {
            const results = getValidOptions(OPTIONS.options, currentUserLogin, {includeRecentReports: false});
        ...
        it('should return limited options when searchString is provided', () => {
            const results = getValidOptions(OPTIONS.options, currentUserLogin, {searchString: 'er'});
        ...
        it('should return current user as valid option when includeCurrentUser is provided', () => {
            const results = getValidOptions(OPTIONS.options, currentUserLogin, {includeCurrentUser: true});
        ...
        it('should limit recent options when recentMaxElements is provided', () => {
            const results = getValidOptions(OPTIONS.options, currentUserLogin, {recentMaxElements: 2});
        ...
        it('should limit personal detail options when maxElements is provided', () => {
            const results = getValidOptions(OPTIONS.options, currentUserLogin, {maxElements: 2});
        ...
        describe('excludedLogins', () => {
            ...
            it('should exclude Chronos when excludedLogins is specified', () => {
                const results = getValidOptions(OPTIONS_WITH_CHRONOS.options, currentUserLogin, {
            ...
            it('should exclude Receipts when excludedLogins is specified', () => {
                const results = getValidOptions(OPTIONS_WITH_RECEIPTS.options, currentUserLogin, {
        ...
        describe('Invite user', () => {
            it('should return nothing if includeUserToInvite is false / default behavior', () => {
                const results = getValidOptions(OPTIONS.options, currentUserLogin, {searchString: 'abc@def.com'});
                expect(results.userToInvite).toBe(null);
            });

            it('should return userToInvite if includeUserToInvite is true', () => {
                const results = getValidOptions(OPTIONS.options, currentUserLogin, {searchString: 'abc@def.com', includeUserToInvite: true});
            ...
            it('should return nothing if includeUserToInvite is true and email is not valid', () => {
                const results = getValidOptions(OPTIONS.options, currentUserLogin, {searchString: 'invalid-email', includeUserToInvite: true});
                expect(results.userToInvite).toBe(null);
            });

            it('should return userToInvite with invalid email if includeUserToInvite is true and shouldAcceptName is also true', () => {
                const results = getValidOptions(OPTIONS.options, currentUserLogin, {searchString: 'invalid-email', includeUserToInvite: true, shouldAcceptName: true});
            ...
            it("should return userToInvite if includeUserToInvite is true and it's a valid phone number (without country code)", () => {
                const results = getValidOptions(OPTIONS.options, currentUserLogin, {searchString: '5005550006', includeUserToInvite: true});
            ...
            it("should return userToInvite if includeUserToInvite is true and it's a valid phone number (with country code)", () => {
                const results = getValidOptions(OPTIONS.options, currentUserLogin, {searchString: '+15005550006', includeUserToInvite: true});
                expect(results.userToInvite).toEqual(expect.objectContaining({login: '+15005550006'}));
            });

            it("should return userToInvite if includeUserToInvite is true and it's a valid phone number (with special characters)", () => {
                const results = getValidOptions(OPTIONS.options, currentUserLogin, {searchString: '+1 (800)324-3233', includeUserToInvite: true});
        ...
        describe('Selected options', () => {
            ...
            it('should return selectedOptions when options have isSelected=true', () => {
                const results = getValidOptions(optionsWithSelected, currentUserLogin, {includeCurrentUser: true});
            ...
            it('should filter selectedOptions when search string is present', () => {
                const results = getValidOptions(optionsWithSelected, currentUserLogin, {includeCurrentUser: true, searchString: 'Iron'});
            ...
            it('should exclude current user from selectedOptions when includeCurrentUser is false', () => {
                const results = getValidOptions(optionsWithSelected, currentUserLogin, {includeCurrentUser: false});
            ...
            it('should exclude selectedOptions when excludeLogins is present', () => {
                const results = getValidOptions(optionsWithSelected, currentUserLogin, {includeCurrentUser: true, excludeLogins: {'peterparker@expensify.com': true}});
                expect(results.selectedOptions.length).toBe(2);
                const selectedNames = results.selectedOptions.map((option) => option.text);
