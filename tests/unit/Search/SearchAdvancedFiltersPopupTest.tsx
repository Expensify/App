...
/** Effects only run for a visible `Activity`, so this tracks which content the popover is actually showing. */
const mockOnContentShown = jest.fn<void, [string]>();
...
jest.mock('@components/Search/FilterComponents/AdvancedFilters/SearchAdvancedFiltersContent', () => ({
    ...
    default: ({baseFilterKey}: {baseFilterKey: string}) => {
        ...
        mockUseEffect(() => {
            mockOnContentShown(baseFilterKey);
        }, [baseFilterKey]);
...
describe('SearchAdvancedFiltersPopup', () => {
    ...
    it('reuses the content of a filter that was already visited instead of remounting it', () => {
        ...
        expect(mockOnContentShown).toHaveBeenLastCalledWith(FILTER_KEYS.FROM);
    });
});
