...
jest.mock('@hooks/useOnyx', () => ({
    __esModule: true,
    // Only the filters form is fed; the saved-view edit keys read as unset so these tests stay outside edit mode.
    default: (key: string) => (key === 'searchAdvancedFiltersForm' ? mockUseOnyx() : [undefined]),
}));
...
const mockBuildFilterQueryString = jest.fn(() => '');

jest.mock('@components/Search/hooks/useUpdateFilterQuery', () => ({
    __esModule: true,
    default: () => ({
        getUpdatedFilterFormValues: mockGetUpdatedFilterFormValues,
        setFilterQueryParams: mockSetFilterQueryParams,
        buildFilterQueryString: mockBuildFilterQueryString,
    }),
}));
