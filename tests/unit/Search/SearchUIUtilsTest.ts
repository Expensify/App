...
describe('SearchUIUtils', () => {
    ...
    describe('Test getSections', () => {
        ...
        it('should intersect date preset with additional constraints instead of overwriting', () => {
            ...
            const yearDateRange = DateUtils.getYearDateRange(now.getFullYear());
            const dateFilter = {
                key: CONST.SEARCH.SYNTAX_FILTER_KEYS.DATE,
                filters: [
                    {
                        operator: CONST.SEARCH.SYNTAX_OPERATORS.EQUAL_TO,
                        value: CONST.SEARCH.DATE_PRESETS.LAST_MONTH,
                    },
                    {
                        operator: CONST.SEARCH.SYNTAX_OPERATORS.GREATER_THAN_OR_EQUAL_TO,
            ...
            // Should intersect: max(preset start, constraint start) — preset start wins since it's later
            // The preset start should be preserved, not overwritten by the earlier constraint
            expect(result.start).toBe(lastMonthStart);
            // End should remain the preset end
            expect(result.end).toBe(lastMonthEnd);
        });
        ...
        it('should intersect date preset end limit with additional constraints', () => {
            ...
            const lastMonth = subMonths(now, 1);
            const lastMonthStart = format(startOfMonth(lastMonth), 'yyyy-MM-dd');
            // Use the 15th of last month as the constraint end — earlier than the preset end
            const constraintEnd = format(new Date(lastMonth.getFullYear(), lastMonth.getMonth(), 15), 'yyyy-MM-dd');
            const yearDateRange = DateUtils.getYearDateRange(now.getFullYear());
            const dateFilter = {
                key: CONST.SEARCH.SYNTAX_FILTER_KEYS.DATE,
                filters: [
                    ...
                    {
                        operator: CONST.SEARCH.SYNTAX_OPERATORS.LOWER_THAN_OR_EQUAL_TO,
                        value: constraintEnd, // Earlier than preset end
                    },
                ],
            };
            ...
            // Start should remain the preset start
            expect(result.start).toBe(lastMonthStart);
            // Should intersect: min(preset end, constraint end) — the constraint end should be used (earlier date)
            expect(result.end).toBe(constraintEnd);
        });
