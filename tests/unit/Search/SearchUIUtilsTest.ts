...
describe('SearchUIUtils', () => {
    ...
    describe('Test Empty Report Selection and Deletion', () => {
        it('should identify empty reports correctly', () => {
            // Test with empty report (no transactions)
            const emptyReport = transactionReportGroupListItems.find((item) => item.transactions.length === 0);
            expect(emptyReport).toBeDefined();
            expect(emptyReport?.transactions).toHaveLength(0);
            expect(emptyReport?.total).toBe(0);
        });

        it('should identify non-empty reports correctly', () => {
            // Test with reports that have transactions
            const nonEmptyReports = transactionReportGroupListItems.filter((item) => item.transactions.length > 0);
            expect(nonEmptyReports.length).toBeGreaterThan(0);
            nonEmptyReports.forEach((report) => {
                expect(report.transactions.length).toBeGreaterThan(0);
            });
        });

        it('should allow empty reports to be selectable', () => {
            const emptyReport = transactionReportGroupListItems.find((item) => item.transactions.length === 0);
            expect(emptyReport).toBeDefined();

            // Empty reports should have the same selection capabilities as non-empty reports
            expect(emptyReport?.keyForList).toBeDefined();
            expect(emptyReport?.reportID).toBeDefined();
            expect(emptyReport?.action).toBe(CONST.SEARCH.ACTION_TYPES.VIEW);
        });

        it('should handle mixed list of empty and non-empty reports', () => {
            const emptyReports = transactionReportGroupListItems.filter((item) => item.transactions.length === 0);
            const nonEmptyReports = transactionReportGroupListItems.filter((item) => item.transactions.length > 0);

            expect(emptyReports.length).toBeGreaterThan(0);
            expect(nonEmptyReports.length).toBeGreaterThan(0);

            // Both types should be present in the list
            expect(emptyReports.length + nonEmptyReports.length).toBe(transactionReportGroupListItems.length);
        });

        it('should return correct list item component for grouped reports', () => {
            const listItemComponent = SearchUIUtils.getListItem(CONST.SEARCH.DATA_TYPES.EXPENSE, CONST.SEARCH.STATUS.EXPENSE.ALL, CONST.SEARCH.GROUP_BY.REPORTS);

            expect(listItemComponent).toBe(TransactionGroupListItem);
        });

        it('should handle empty report data gracefully', () => {
            const emptySearchResults: OnyxTypes.SearchResults = {
                data: {
                    personalDetailsList: {},
                },
                search: {
            ...
            expect(() => {
                SearchUIUtils.getSections(CONST.SEARCH.DATA_TYPES.EXPENSE, emptySearchResults.data, adminAccountID, formatPhoneNumber, CONST.SEARCH.GROUP_BY.REPORTS);
            }).not.toThrow();
        });
    });
});
