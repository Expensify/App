...
import ChatSearchView from '@components/Search/ChatSearchView';
...
// Capture each row's tap handler so the mobile-selection-mode toggle can be exercised without the heavy real ChatListItem.
const mockRowSelect: Record<string, () => void> = {};
jest.mock('@components/Search/SearchList/ListItem/ChatListItem', () => {
...
describe('ChatSearchView', () => {
    ...
    it('renders one row per data item', async () => {
        ...
        await waitForBatchedUpdates();
        expect(screen.queryAllByTestId(/^row-chat-/)).toHaveLength(3);
    });
    ...
    it('computes selection counts excluding deleted rows and reflects select-all state', async () => {
        ...
        mockSelectedTransactions.current = selectKeys('chat-0');
        renderView({
            data: createMockData(3, new Set(['chat-2'])),
