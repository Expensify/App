...
import {buildSearchQueryJSON, getPolicyIDFromQuery, getStatusFromQuery} from '@src/libs/SearchQueryUtils';
...
describe('buildAwaitingApprovalQuery', () => {
    ...
    it('produces status:outstanding', () => {
        const queryJSON = buildSearchQueryJSON(queryString);
        expect(getStatusFromQuery(queryJSON)).toEqual([CONST.SEARCH.STATUS.EXPENSE.OUTSTANDING]);
    });
    ...
    it('emits the policyID filter for a single policy', () => {
        ...
        expect(getPolicyIDFromQuery(queryJSON).value).toEqual(['policy_a']);
    ...
    it('emits the policyID filter for multiple policies, preserving the provided order', () => {
        ...
        const queryJSON = buildSearchQueryJSON(scoped);
        expect(getPolicyIDFromQuery(queryJSON).value).toEqual(['policy_a', 'policy_b', 'policy_c']);
...
describe('buildRepaidLast30DaysQuery', () => {
    ...
    it('produces status:paid', () => {
        const queryJSON = buildSearchQueryJSON(queryString);
        expect(getStatusFromQuery(queryJSON)).toEqual([CONST.SEARCH.STATUS.EXPENSE.PAID]);
    });
