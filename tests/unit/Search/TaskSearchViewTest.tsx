...
import TaskSearchView from '@components/Search/TaskSearchView';
...
// Capture each row's tap handler so the mobile-selection-mode toggle can be exercised without the heavy real TaskListItem.
const mockRowSelect: Record<string, () => void> = {};
jest.mock('@components/Search/SearchList/ListItem/TaskListItem', () => {
...
describe('TaskSearchView', () => {
    ...
    it('renders one row per data item', async () => {
        ...
        await waitForBatchedUpdates();
        expect(screen.queryAllByTestId(/^row-task-/)).toHaveLength(3);
    });
    ...
    it('computes selection counts excluding deleted rows and reflects select-all state', async () => {
        ...
        mockSelectedTransactions.current = selectKeys('task-0');
        renderView({
            data: createMockData(3, new Set(['task-2'])),
