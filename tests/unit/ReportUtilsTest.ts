...
describe('ReportUtils', () => {
    ...
    describe('canHoldUnholdReportAction', () => {
        it('should return canUnholdRequest as true for a held duplicate transaction', async () => {
            ...
            putOnHold(expenseTransaction.transactionID, 'hold', transactionThreadReport.reportID, false, undefined, currentUserEmail, currentUserAccountID);
            await waitForBatchedUpdates();
    ...
    describe('changeMoneyRequestHoldStatus', () => {
        ...
        it('should unhold request when transaction is already on hold', async () => {
            ...
            changeMoneyRequestHoldStatus(reportAction, iouTransaction, false, undefined, currentUserEmail, currentUserAccountID);

            // Then unholdRequest should be called with the correct parameters and navigation should not be called
            expect(unholdRequestSpy).toHaveBeenCalledWith(transactionID, childReportID, expect.objectContaining({id: policyID}), false, undefined, currentUserEmail, currentUserAccountID);
