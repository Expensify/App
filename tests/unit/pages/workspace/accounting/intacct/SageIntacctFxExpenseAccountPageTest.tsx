...
const SageIntacctFxExpenseAccountPage = require<{default: ComponentType<{policy: Policy}>}>('@pages/workspace/accounting/intacct/advanced/SageIntacctFxExpenseAccountPage').default;
...
jest.mock('@userActions/connections/SageIntacct', () => ({
    updateSageIntacctFxExpenseAccount: jest.fn(),
}));

jest.mock('@userActions/Policy/Policy', () => ({
    clearSageIntacctErrorField: jest.fn(),
}));
...
function renderPicker(config: Parameters<typeof buildPolicy>[0] = {}) {
    render(<SageIntacctFxExpenseAccountPage policy={buildPolicy(config)} />);
...
describe('SageIntacctFxExpenseAccountPage', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });

    it('puts None at the top, selected, when no account is saved', () => {
        // Given: synced Sage Intacct expense accounts and no saved fee account.
    ...
    it('does not persist until Save is pressed', () => {
        ...
        expect(updateSageIntacctFxExpenseAccount).not.toHaveBeenCalled();
    ...
    it('saves the selected account when Save is pressed', () => {
        ...
        expect(updateSageIntacctFxExpenseAccount).toHaveBeenCalledWith('1', TRAVEL_ACCOUNT.id, undefined);
    ...
    it('clears the saved account when None is saved', () => {
        ...
        expect(updateSageIntacctFxExpenseAccount).toHaveBeenCalledWith('1', '', TRAVEL_ACCOUNT.id);
        expect(Navigation.goBack).toHaveBeenCalled();
    });
});
