...
const QuickbooksFxExpenseAccountSelectPage = require<{default: ComponentType<{policy: Policy}>}>('@pages/workspace/accounting/qbo/advanced/QuickbooksFxExpenseAccountSelectPage').default;
...
jest.mock('@libs/actions/connections/QuickbooksOnline', () => ({
    updateQuickbooksOnlineFxExpenseAccount: jest.fn(),
}));

jest.mock('@userActions/Policy/Policy', () => ({
    clearQBOErrorField: jest.fn(),
}));

jest.mock('@pages/workspace/accounting/utils', () => ({
    getQuickbooksOnlineIntegrationName: () => 'QuickBooks Online',
}));
...
function renderPicker(config: Parameters<typeof buildPolicy>[0] = {}) {
    render(<QuickbooksFxExpenseAccountSelectPage policy={buildPolicy(config)} />);
...
describe('QuickbooksFxExpenseAccountSelectPage', () => {
    beforeEach(() => {
        jest.clearAllMocks();
    });

    it('puts None at the top, selected, when no account is saved', () => {
        // Given: synced QBO expense accounts and no saved fee account.
    ...
    it('does not persist until Save is pressed', () => {
        ...
        expect(updateQuickbooksOnlineFxExpenseAccount).not.toHaveBeenCalled();
    ...
    it('saves the selected account when Save is pressed', () => {
        ...
        expect(updateQuickbooksOnlineFxExpenseAccount).toHaveBeenCalledWith('1', TRAVEL_ACCOUNT.id, undefined);
    ...
    it('clears the saved account when None is saved', () => {
        ...
        expect(updateQuickbooksOnlineFxExpenseAccount).toHaveBeenCalledWith('1', '', TRAVEL_ACCOUNT.id);
        expect(Navigation.goBack).toHaveBeenCalled();
    });
});
