...
jest.mock('@hooks/useDeleteTransactions', () => ({
    ...
    default: () => ({
        ...
        shouldOpenSplitExpenseEditFlowOnDelete: mockShouldOpenSplitExpenseEditFlowOnDelete,
    }),
}));
...
describe('useSelectedTransactionsActions', () => {
    ...
    it('should show edit split action and delete transactions when deleting redirects to split edit', async () => {
        const transactionID = '123';
        const report = createRandomReport(1, undefined);
        const session: Session = {accountID: 1};
        const reportActions: ReportAction[] = [
    ...
    it('should show unhold option and handle unhold action', async () => {
        ...
        expect(unholdRequest).toHaveBeenCalledWith(transactionID, 'child123', undefined, false, CURRENT_USER_LOGIN, CURRENT_USER_ACCOUNT_ID, undefined, false, undefined, undefined);
        expect(mockClearSelectedTransactions).toHaveBeenCalledWith(true);
    });
    ...
    it('should pass isOffline=true to unholdRequest when offline', async () => {
        ...
        expect(unholdRequest).toHaveBeenCalledWith(transactionID, 'child123', undefined, true, CURRENT_USER_LOGIN, CURRENT_USER_ACCOUNT_ID, undefined, false, undefined, undefined);
        expect(mockClearSelectedTransactions).toHaveBeenCalledWith(true);
    });
