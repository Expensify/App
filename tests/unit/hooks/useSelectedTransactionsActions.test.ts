...
describe('useSelectedTransactionsActions', () => {
    ...
    it('should use reportActions childReportID as the merge override when transaction thread metadata is missing', async () => {
        ...
        expect(setupMergeTransactionDataAndNavigate).toHaveBeenCalledWith(transactionID, [transaction], mockLocalCompare, [], false, true, undefined, childReportID);
    });
    ...
    it('should create a thread and use its reportID as the merge override when no thread metadata exists', async () => {
        ...
        expect(setupMergeTransactionDataAndNavigate).toHaveBeenCalledWith(transactionID, [transaction], mockLocalCompare, [], false, true, undefined, createdThreadReportID);
    });
