import {filterRequestByCommand, resolveDuplicationConflictAction} from '@libs/actions/RequestConflictUtils';
...
describe('RequestConflictUtils', () => {
    it.each([['OpenApp'], ['ReconnectApp']])('resolveDuplicationConflictAction when %s do not exist in the queue should push %i', (command) => {
        ...
        const result = resolveDuplicationConflictAction(persistedRequests, filterRequestByCommand(commandToFind));
