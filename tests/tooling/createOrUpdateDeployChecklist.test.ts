...
describe('createOrUpdateDeployChecklist', () => {
    ...
    // A baseCommitDate that predates all PR dates generated by toMergedPRs (which start at
    // 2024-01-01). Using a date in 2023 means the staleness filter is never triggered for
    // existing tests — all generated PR dates are >= baseCommitDate, so nothing is filtered out.
    const DEFAULT_BASE_COMMIT_DATE = '2023-12-31T00:00:00Z';
    ...
    function toMergedPRs(prNumbers: number[]) {
        return {
            ...
            submoduleUpdates: [],
            // A baseline date before all generated PR dates, so the staleness filter is a no-op
            // for standard test fixtures. Override this in tests that specifically exercise the filter.
            baseCommitDate: DEFAULT_BASE_COMMIT_DATE,
        };
    }
    ...
    test('creates new issue when there are no Mobile-Expensify PRs', async () => {
        ...
        mockGetMergedPRsDeployedBetween.mockImplementation(async (fromRef, toRef, repositoryName) => {
            if (fromRef === '1.0.1-0-staging' && toRef === '1.0.2-1-staging') {
                if (repositoryName === CONST.MOBILE_EXPENSIFY_REPO) {
                    return {mergedPRs: [], submoduleUpdates: [], baseCommitDate: DEFAULT_BASE_COMMIT_DATE}; // No Mobile-Expensify PRs
                }
                return toMergedPRs(baseNewPullRequests); // App PRs
            }
            return {mergedPRs: [], submoduleUpdates: [], baseCommitDate: DEFAULT_BASE_COMMIT_DATE};
    ...
    describe('cherry-pick filtering', () => {
        ...
        test('filters out stale PRs whose commit date predates the previous staging tag (cherry-picked in a prior cycle)', async () => {
            // This test covers the specific bug where a PR is cherry-picked to staging in cycle N,
            // deployed to production, and then re-appears in cycle N+1's checklist.
            //
            // How the bug manifests:
            //   1. PR #50 merges to main on 2024-01-01 (before the previous staging tag).
            //   2. It is cherry-picked directly to the staging branch in cycle N.
            //      The cherry-pick commit lands on staging BEFORE the previous staging tag is cut.
            //   3. Production is deployed; staging is synced from production/main.
            //      The ORIGINAL merge commit of PR #50 (dated 2024-01-01) now enters the staging
            //      branch for the first time and appears in compareCommits(previousTag, newTag).
            //   4. The duplicate-detection filter doesn't fire (the cherry-pick commit is before the
            //      range; the original commit appears only once).
            //   5. The previous-checklist filter doesn't fire (the PR was never on the checklist;
            //      it arrived via CP).
            //   Result: PR #50 incorrectly appears in cycle N+1's checklist even though it is
            //   already in production.
            //
            // The fix: filter out any PR whose commit date predates baseCommitDate (the date of
            // the commit that the previous staging tag points to). Such commits are "time-travelers"
            // brought into the range by a post-deploy sync, not by a genuine merge in this cycle.
            ...
            // The previous staging tag was created on 2024-01-10.
            // PR #50 was originally merged on 2024-01-05 — BEFORE the previous tag.
            // PRs #10 and #11 were merged on 2024-01-11 and 2024-01-12 — AFTER the previous tag (genuinely new).
            ...
            mockGetMergedPRsDeployedBetween.mockImplementation(async (fromRef, toRef, repositoryName) => {
                if (fromRef === '1.0.2-1-staging' && toRef === '1.0.3-0-staging') {
                    ...
                    return {
                        mergedPRs: [
                            // Stale: this commit predates the previous staging tag.
                            // It was cherry-picked in the previous cycle, deployed to production,
                            // and re-entered the range via the post-deploy staging sync.
                            {prNumber: 50, date: '2024-01-05T00:00:00Z'},
                            // Genuinely new PRs for this cycle — their commit dates are after the previous tag.
                            {prNumber: 10, date: '2024-01-11T00:00:00Z'},
                            {prNumber: 11, date: '2024-01-12T00:00:00Z'},
                        ],
                        submoduleUpdates: [],
                        // baseCommitDate is the date of the commit that previousStagingTag points to.
                        // Any PR with a commit date before this was already deployed via cherry-pick.
            ...
            // Previous checklist does NOT include PR #50 — it arrived via cherry-pick, not the normal flow.
            const mockGetDeployChecklistData = jest.spyOn(DeployChecklistUtils, 'getDeployChecklistData');
            mockGetDeployChecklistData.mockImplementation(() => ({
                title: 'Previous Checklist',
                url: `https://github.com/${process.env.GITHUB_REPOSITORY}/issues/29`,
