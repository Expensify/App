 ...
 * cache and main's measured Reassure baseline - and neither file references the other, so three
 * agreements are asserted from outside both: one transform key and one baseline key shape, or the
 * restore is a permanent silent miss; and one runner class, or the transform output is not
 * interchangeable and durations are compared across hardware. (test.yml caches the transform path
 * under its own key and policy; out of scope here.)
 */
...
/**
 * The commit expression is the one difference allowed: the seed writes the commit it just measured,
 * the perf job reads its merge ref's base parent. Normalized away so everything else - prefix,
 * separators, runner.os, runner.arch - still has to match byte for byte.
 */
...
describe('Reassure perf caches', () => {
    ...
    it('keys every .reassure/baseline.perf step identically apart from the commit', () => {
        ...
        // The normalization has to have fired on all three, or the assertion above passes on the raw keys.
