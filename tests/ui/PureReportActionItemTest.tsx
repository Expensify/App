  ...
  <followup><followup-text>${followupQuestion}</followup-text></followup>
    ...
    describe('Simple message actions', () => {
        ...
        it('HOLD_COMMENT action renders via ReportActionItemBasicMessage', async () => {
            ...
            expect(screen.getByText('Hold reason text')).toBeOnTheScreen();
            // testID check is needed because HOLD_COMMENT and the default ADD_COMMENT branch both render
            // the same message text. The only difference is the wrapper component: HOLD_COMMENT uses
            // ReportActionItemBasicMessage (muted style), while the default uses ReportActionItemMessage.
            expect(screen.getByTestId('ReportActionItemBasicMessage')).toBeOnTheScreen();
        });
    ...
    describe('Additional policy log actions', () => {
    ...
    describe('Additional simple message actions', () => {
    ...
    describe('Complex rendering branches', () => {
        ...
        it('REIMBURSEMENT_QUEUED with missing bank account shows add bank account button', async () => {
            const reportID = 'reimbursementTestReport';
            await act(async () => {
                        ...
                        <ScreenWrapper testID="test">
                            <PortalProvider>
                                <PureReportActionItem
                                    personalPolicyID={undefined}
                                    currentUserEmail={undefined}
                                    report={{reportID, ownerAccountID: ACTOR_ACCOUNT_ID}}
                                    parentReportAction={undefined}
                                    action={action}
                                    displayAsGroup={false}
                                    isMostRecentIOUReportAction={false}
        ...
        it('REIMBURSEMENT_QUEUED with missing wallet shows enable wallet button', async () => {
            const reportID = 'reimbursementWalletTestReport';
            await act(async () => {
                await Onyx.merge(ONYXKEYS.SESSION, {accountID: ACTOR_ACCOUNT_ID});
                await Onyx.merge(`${ONYXKEYS.COLLECTION.REPORT}${reportID}`, {
                    reportID,
        ...
        it('IOU PAY VBBA manual renders business bank account message with last 4 digits', async () => {
            await act(async () => {
                await Onyx.merge(ONYXKEYS.BANK_ACCOUNT_LIST, {
                    ...
                    12345: {accountData: {accountNumber: '000098765'}} as never,
                });
            });
            await waitForBatchedUpdatesWithAct();
    ...
    describe('Component-delegated branches', () => {
    ...
    describe('Additional policy log actions - remaining branches', () => {
    ...
    describe('Previously untested branches', () => {
