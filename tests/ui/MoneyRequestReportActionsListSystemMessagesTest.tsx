...
describe('MoneyRequestReportActionsList system-message presentation', () => {
    ...
    it('collapses and expands the real list while preserving avatarless members and chat boundaries', async () => {
        ...
        const collapsedControl = screen.getByRole('button', {name: '2 changes were made'});
        ...
        expect(screen.getByRole('button', {name: '2 changes were made'}).props.accessibilityState).toMatchObject({expanded: true});
        ...
        fireEvent.press(screen.getByRole('button', {name: '2 changes were made'}));
        await waitForBatchedUpdatesWithAct();
