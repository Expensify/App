...
describe('ExpensifyCodePage', () => {
    ...
    beforeEach(async () => {
        ...
        await act(async () => {
            await Onyx.clear();
            await Onyx.set(ONYXKEYS.PRIVATE_PROMO_DISCOUNT, {promoCode: null});
        });
        await waitForBatchedUpdatesWithAct();
    });
    ...
    it('show NotFoundPage when subscription already has expensifyCode', async () => {
        await TestHelper.signInWithTestUser();
        await act(async () => {
            await Onyx.merge(ONYXKEYS.PRIVATE_PROMO_DISCOUNT, {promoCode: 'APPLIED_CODE'});
        });
        await waitForBatchedUpdatesWithAct();
    ...
    it('navigate back when expensifyCode is set in subscription after submit', async () => {
        ...
        await act(async () => {
            await Onyx.merge(ONYXKEYS.PRIVATE_PROMO_DISCOUNT, {promoCode: '2026EarlyAdoption50'});
        });
        await waitFor(() => {
