    ...
    return render(
        <ComposeProviders components={[OnyxListItemProvider, LocaleContextProvider, CurrentReportIDContextProvider]}>
            <PortalProvider>
                <ModalProvider>
                    <NavigationContainer>
                        <Stack.Navigator initialRouteName={SCREENS.WORKSPACE.TAGS_IMPORT_OPTIONS}>
                            <Stack.Screen
                                name={SCREENS.WORKSPACE.TAGS_IMPORT_OPTIONS}
                                component={ImportTagsOptionsPage}
...
describe('ImportTagsOptionsPage', () => {
    ...
    it('navigates to import page with forceReplace after confirming "Switch Tag Levels"', async () => {
        await TestHelper.signInWithTestUser();
        ...
        // Regression: the navigation to the import page must happen with forceReplace so
        // the confirm modal's history.back() (when shouldHandleNavigationBack is true)
        // cannot undo the navigation and leave the user stuck on the import-options screen.
        const navigateMock = Navigation.navigate as jest.Mock<void, [string, {forceReplace?: boolean}?]>;
        const importRoute = ROUTES.WORKSPACE_TAGS_IMPORT.getRoute(policy.id);
        const importCall = navigateMock.mock.calls.find(([route]) => route === importRoute);

        expect(importCall).toBeDefined();
        expect(importCall?.[1]).toEqual(expect.objectContaining({forceReplace: true}));
