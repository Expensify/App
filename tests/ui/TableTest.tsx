...
// Captures scrollToOffset calls so the scroll-reset tests can assert on the empty-to-populated behavior
const mockScrollToOffset = jest.fn();

// Mock FlashList so it renders the rows and the empty component like the real list, while exposing scrollToOffset on the
// ref. This lets us assert the scroll-to-top that fires when the data transitions from empty to populated.
jest.mock('@shopify/flash-list', () => {
    ...
    >(({data, renderItem, keyExtractor, ListEmptyComponent, ...scrollViewProps}, ref) => {
        ReactLocal.useImperativeHandle(ref, () => ({scrollToOffset: mockScrollToOffset}));

        if (!data || data.length === 0) {
            return ReactLocal.createElement(RN.ScrollView, scrollViewProps, ListEmptyComponent ?? null);
        }

        return ReactLocal.createElement(
            RN.ScrollView,
            scrollViewProps,
            ...data.map((item, index) => ReactLocal.createElement(ReactLocal.Fragment, {key: keyExtractor?.(item, index) ?? String(index)}, renderItem?.({item, index, target: 'Cell'}))),
        );
    });
...
describe('Table', () => {
    ...
    describe('scroll reset on empty-to-populated transition', () => {
        let requestAnimationFrameSpy: jest.SpyInstance;
        let cancelAnimationFrameSpy: jest.SpyInstance;

        beforeEach(() => {
            // Run the scheduled frame synchronously so the deferred scroll happens within the test.
            requestAnimationFrameSpy = jest.spyOn(global, 'requestAnimationFrame').mockImplementation((callback: FrameRequestCallback) => {
                callback(0);
                return 0;
            });
        ...
        it('should scroll back to the top when the data goes from empty to populated', () => {
            const props = createDefaultProps();
            ...
            // No data has arrived yet, so the list should not have been scrolled.
        ...
        it('should not scroll when the data changes between two populated states', () => {
            const props = createDefaultProps();
            const {rerender} = render(
                <Table<TestItem, TestColumnKey>
            ...
            // Adding rows to an already-populated list must not yank the scroll position to the top.
            expect(mockScrollToOffset).not.toHaveBeenCalled();
        });

        it('should not scroll when the data goes from populated to empty', () => {
            const props = createDefaultProps();
            const {rerender} = render(
                <Table<TestItem, TestColumnKey>
                    data={mockData}
