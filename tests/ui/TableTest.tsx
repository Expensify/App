...
jest.mock("@react-navigation/native", () => {
  const actualNav = jest.requireActual<typeof Navigation>(
  ...
  function MockTextInput(props: {
    ...
    return (
      <RNView>
        ...
        />
        {!!props.onClearInput && (
          <RNTextInput testID="clear-button" onPress={props.onClearInput} />
        )}
      </RNView>
    );
  }
...
function createDefaultProps() {
  ...
  const isItemInSearch: IsItemInSearchCallback<TestItem> = (
    item,
    searchString,
  ) => {
  ...
  const compareItems: CompareItemsCallback<TestItem, TestColumnKey> = (
    a,
    b,
    { columnKey, order },
  ) => {
...
describe("Table", () => {
  ...
  describe("filter functionality", () => {
    it("should apply filter when filter config is provided", () => {
      ...
      const isItemInFilter: IsItemInFilterCallback<TestItem> = (
        item,
        filterValues,
      ) => {
        if (
  ...
  describe("combined search and filter", () => {
    it("should apply both search and filter together", () => {
      const props = createDefaultProps();
      const tableRef =
        React.createRef<TableHandle<TestItem, TestColumnKey, "category">>();
