...
async function navigateToSidebar(renderedApp) {
    ...
    const reportHeaderBackButton = reportHeader.findByType(ReportHeaderViewBackButton);
 ...
 * @return {Promise}
 */
async function navigateToSidebarOption(renderedApp, index) {
...
test('Unread LHN Status, “New Messages” badge, and New Line Indicator', async () => {
    const REPORT_ID = 1;
    ...
    const loginForm = renderedApp.getByTestId('login-form');
    expect(loginForm).toBeTruthy();

    TestHelper.signInWithTestUser(USER_A_ACCOUNT_ID, USER_A_EMAIL, undefined, undefined, 'A');
    await waitForPromisesToResolve();
    ...
    Onyx.merge(`${ONYXKEYS.COLLECTION.REPORT}${REPORT_ID}`, {
        ...
        lastMessageTimestamp: MOMENT_TEN_MINUTES_AGO.utc(), // Ten minutes ago
    ...
    const sidebarLinks = renderedApp.getByTestId('sidebar-links');
    expect(sidebarLinks).toBeTruthy();
