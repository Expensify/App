...
function renderTextInput(onInputChange: jest.Mock, numericInputRef?: React.Ref<NumericFieldRef>, value = '12') {
    return render(
        <ComposeProviders components={[OnyxListItemProvider, LocaleContextProvider]}>
            <NumericField
                ...
                numericInputRef={numericInputRef}
            >
...
describe('NumericField.TextInput native selection guard', () => {
    ...
    it('does not leave the guard set when updateNumber receives the current value', async () => {
        ...
        // Given a TextInput with value "12" and a numericInputRef
        renderTextInput(onInputChange, numericInputRef);
        await waitForBatchedUpdatesWithAct();

        // When updateNumber is called with the same value "12"
        act(() => {
            numericInputRef.current?.updateNumber('12');
        });
        await waitForBatchedUpdatesWithAct();
