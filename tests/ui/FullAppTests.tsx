...
describe('Full app render tests', () => {
    afterEach(() => {
        jest.clearAllMocks();
        Onyx.clear();
    ...
    describe('Unread Indicators', () => {
        it('Display bold in the LHN for unread chat and new line indicator above the chat message when we navigate to it', async () => {
            ...
            await waitFor(() => expect(isNewMessagesBadgeVisible()).toBe(true));
        });
        ...
        it('Shows a browser notification and bold text when a new message arrives for a chat that is read', async () => {
            ...
            let displayNameTexts = screen.queryAllByLabelText(displayNameHintTexts);
            ...
            displayNameTexts = screen.queryAllByLabelText(hintText);
            expect(displayNameTexts).toHaveLength(2);
        ...
        xit('Manually marking a chat message as unread shows the new line indicator and updates the LHN', async () => {
            await signInAndGetAppWithUnreadChat();
            ...
            let hintText = Localize.translateLocal('accessibilityHints.chatUserDisplayNames');
            let displayNameTexts = screen.queryAllByLabelText(hintText);
            ...
            scrollUpToRevealNewMessagesBadge();
            await waitFor(() => expect(isNewMessagesBadgeVisible()).toBe(false));
        });
        ...
        xit('Keeps the new line indicator when the user moves the App to the background', async () => {
            await signInAndGetAppWithUnreadChat();
            // Verify we are on the LHN and that the chat shows as unread in the LHN
            expect(areYouOnChatListScreen()).toBe(true);
        ...
        it('Displays the correct chat message preview in the LHN when a comment is added then deleted', async () => {
            let reportActions: OnyxEntry<ReportActions> = null;
            ...
            let hintText = Localize.translateLocal('accessibilityHints.lastChatMessagePreview');
            let alternateText = screen.queryAllByLabelText(hintText);
            ...
            await waitForBatchedUpdates();
            hintText = Localize.translateLocal('accessibilityHints.lastChatMessagePreview');
