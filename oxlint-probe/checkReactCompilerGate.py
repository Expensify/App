"""Does oxlint's React Compiler gate behave like ESLint's processor?
    ...
    python3 oxlint-probe/checkReactCompilerGate.py
...
processor, so oxlint-probe/reactCompilerGate.mjs does it by wrapping `context.report` inside the
JS plugin. This checks the two agree, on both answers.

The per-rule fixtures in oxlint-probe/port-probe cannot check this. They deliberately opt out of
...
The probe files are written into src/ rather than oxlint-probe/, because the repo's real ESLint
config excludes oxlint-probe from the TypeScript program and its type-aware rules cannot parse a
