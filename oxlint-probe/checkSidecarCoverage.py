"""Every rule oxlint runs in its Node sidecar, and the evidence that it really runs.
    ...
    python3 oxlint-probe/checkSidecarCoverage.py [--json <path>]
...
Evidence is one of three kinds, all of which run both linters and compare:
    ...
    fixture  a file in oxlint-probe/port-probe that violates the rule
...
# The four aliases this repo hand-hosts, rule by rule, in oxlint-probe/*.mjs. Every rule under them is
...
def main():
    ...
    for oxlint_id, alias in sidecar.items():
        ...
        if oxlint_id in replays:
            evidence.append(('replay', 'oxlint-probe/rule-tester'))
