...
#
# Usage:
#   ./oxlint-probe/measureReactCompilerCost.sh [output-dir]   # default /tmp
set -u
cd "$(dirname "$0")/.."
...
python3 oxlint-probe/reactCompilerVariants.py write
...
run() {
    ...
    python3 oxlint-probe/reactCompilerVariants.py record "$json" "$run_id" "$variant" "$SECONDS" "$RESULTS"
}
...
python3 oxlint-probe/compareReactCompilerAggregate.py "$OUT/rc-a-1.json" "$OUT/rc-d-1.json"
