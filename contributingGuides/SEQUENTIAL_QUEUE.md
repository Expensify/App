...
**Problem it solves.** Serialize every WRITE so requests reach the server one at a time, in submission order — deduplicated server-side on the rare resend, not by a client-side exactly-once lock — with automatic retry on transient failure and correct ordering of dependent READs.
