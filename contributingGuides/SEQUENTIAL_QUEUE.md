...
**Sharp edges.**
- The watchdog restores **liveness, not ordering**: it unpauses without closing the update gap first. When no `OpenApp`/`ReconnectApp` is queued, the drained WRITEs run against a client that is still behind, and the next gapped response re-pauses — worst case a pause/unpause oscillation at the watchdog period instead of a permanent hang. The `Log.alert` it emits is the signal for whether a gap-first escalation (an incremental reconnect fired before unpausing) is worth adding.
...
| `MAX_PAUSE_WATCHDOG_TIME_MS` | `60000` (60 s) | Max time the queue may stay paused with no applied-update progress before the pause watchdog force-unpauses |
