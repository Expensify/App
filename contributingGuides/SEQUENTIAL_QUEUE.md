...
**A failed apply holds the watermark, and this same sync is what releases it.** An Onyx write can fail on its own — a full disk, a lost IndexedDB connection — and the update is then lost while the server has already counted it. `apply` therefore advances `ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT` only *after* the apply resolves, and records every `lastUpdateID` whose apply rejected while sitting above the persisted watermark. While any of those is outstanding, `getEffectiveLastUpdateID()` reports the persisted watermark, so `doesClientNeedToBeUpdated` keeps reporting the range and the fetch above keeps running: the hold is what turns a dropped write into a recoverable gap instead of a silent one.

A held failure is released only by a response that proves it re-delivered it, expressed as the range `(coveredFrom, lastUpdateID]`. A full reconnect or `OpenApp` covers from 0; a catch-up covers from its own `updateIDFrom`, which is **exclusive**, so a response starting at the failed ID re-delivers nothing at or below it; a response chained onto the watermark covers from its `previousUpdateID`; and a response that proves nothing (`OpenReport`, no `previousUpdateID`) releases nothing and merely has its own `onyxData` applied. Failures inside the range are released individually, so a partial recovery still advances the watermark exactly as far as it is covered and holds only against the step past the next unrecovered ID. At most `CONST.NETWORK.MAX_TRACKED_FAILED_UPDATE_IDS` (100) IDs are remembered; past that the highest are folded into a remembered band and the watermark freezes until a response covers through the highest failure still remembered, rather than letting a client stuck in a storage failure grow the set without limit. `Onyx.clear()` clears the set along with the watermark.

That hold has two consequences for the machinery above. A refused advance is indistinguishable from a stalled fetch, so the stall guard's stated premise — "the server can't serve that range right now" — is only half the story: the client may be refusing an advance it was just given, and the *incremental* `ReconnectApp` the guard escalates to is exactly what releases a set that has hit its cap. And because the pause watchdog re-arms on an advance of that key, a hold stops re-arming it, so the watchdog fires **sooner** — the safety direction, never a longer pause.
