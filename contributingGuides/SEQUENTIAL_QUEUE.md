...
- **The generic conflict check sees only the queue, never the in-flight `ongoingRequest`.** A new write cannot dedupe against a request that is already being sent. The one exception is the reconnect resolver above, which consults the ongoing request for an incoming `ReconnectApp` only; every other command, `OpenApp` included, still dedupes against the waiting queue alone.
