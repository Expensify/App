...
So `pause()` starts a 60 s timer (`MAX_PAUSE_WATCHDOG_TIME_MS`) that resets whenever the client applies a newer update — a slow-but-healthy catch-up keeps pushing the deadline out, so it only fires when the queue has made **no** progress for a full minute. When it fires, it logs an alert (to keep the root cause visible in telemetry), then **closes the gap before unpausing**: it asks `OnyxUpdateManager` (via a registered callback, since `SequentialQueue` cannot import it directly) to fetch the missing updates with an incremental `ReconnectApp` that runs outside the queue, so the drained writes are not sent against stale data. That recovery is best-effort — it is skipped if the client is already retrying fetches, capped at `MAX_PAUSE_WATCHDOG_ESCALATION_TIME_MS`, and its errors are ignored — because unblocking the queue always wins over closing the gap. `unpause()` and `resetQueue()` cancel the timer.
