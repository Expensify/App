...
An incoming `OpenApp` (wired via `writeWithNoDuplicatesOpenAppConflictAction`) has its own resolver, `resolveOpenAppDuplicationConflictAction`, which also reads `ongoingRequest`. An `OpenApp` that lands while another one is in flight is dropped (`noAction`): it would download the whole account a second time, and the app-load skeleton stays up for both round trips. Two cases still fall through to the queue-only `replace` where the latest one wins. The first is a caller passing `shouldDedupeWithInFlight: false` through `openApp()`, which it does when the in-flight response cannot satisfy it. The params do not describe the whole response: the server picks the report set from the priority-mode NVP as it reads it, so the `PriorityModeHandler` refetch after leaving #focus needs its own round trip, and a caller asking for public-room or draft-comment preservation adds `successData` merges the in-flight request knows nothing about. The second is an `OpenApp` that lands while only a `ReconnectApp` is in flight, since a reconnect does not carry `OpenApp`'s payload.
