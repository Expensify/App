...
| Collaborator | Role |
|---|---|
| **PersistedRequests** | The request store: an in-memory mirror of the on-disk queue (`persistedRequests` array plus a single `ongoingRequest`). |
...
5. **Finally (the drain-end `process().finally`, chained inside `flush`).** The running flag clears, `isReadyPromise` resolves **only when offline or the queue is empty**, and, only when the queue is fully drained, `QueuedOnyxUpdates` is flushed and `queueFlushedData` is applied and cleared.
...
2. **Head dedupe.** The init path strips the array head when it `deepEqual`s the rehydrated `ongoingRequest`. Promotion writes both keys in one atomic `multiSet` (see [PersistedRequests](#persistedrequests-the-store)), so it cannot leave the same request both as `ongoingRequest` and as the array head. This dedupe is a **defensive backstop** for any persisted state where the two keys are out of sync on disk (e.g. a queue whose two key writes did not land together).
...
**Problem it solves.** Serialize every WRITE so requests reach the server one at a time, in submission order, with automatic retry on transient failure and correct ordering of dependent READs. On the rare resend, duplicates are reconciled server-side, not by a client-side exactly-once lock.
...
| Conflict REPLACE | `update()` | `PERSISTED_REQUESTS`, or **nothing** when the target is no longer queued | **Yes**, `handleConflictActions` awaits it |
...
- **`Onyx.set(key, null)`** (e.g. clearing the ongoing key) is routed to a storage **delete** (`remove()`), not a value write. Its returned promise is `Promise.resolve()`, resolved before the delete commits.
- **Subscriber broadcasts** are the synchronous cache/callback loop; they run before the storage write and independently of it.
...
`SequentialQueue` is driven by eight module-level variables plus the shared throttle instance. This table is the densest reference for reasoning about invariants during a refactor.
...
| `pauseWatchdogTimeoutID` | Liveness watchdog for the pause flag | `armPauseWatchdog()`: from `pause()`, re-armed on every `ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT` advance while paused and on the offline→online transition; never armed while `isOfflineNetwork()` | `clearPauseWatchdog()` in `unpause()` / `resetQueue()` / the online→offline transition, or by firing | A paused *and online* queue either applies newer updates or is force-unpaused after `MAX_PAUSE_WATCHDOG_TIME_MS`, and unconditionally by `MAX_PAUSE_WATCHDOG_ABSOLUTE_TIME_MS` from `pauseStartTime` (see [Pausing & Data-Gap Sync](#pausing--data-gap-sync)) |
| `pauseStartTime` | Start of the current pause, for the absolute ceiling | `pause()`, and reset on the offline→online transition so offline time is not counted | (never cleared; only read while `isQueuePaused`) | Re-arming can shorten but never extend the ceiling |
...
**`resolveIsReadyPromise` is called from four sites**, not just the `finally`. Because `push()` arms the pending promise *before* awaiting the disk write, every early-out path *that armed it* has to release parked READs itself, or they would hang. (The offline-at-entry early-out is the exception: it returns before arming, so it has nothing to release.)
...
**Sharp edges.** `isReadyPromise` is a single shared one-shot resolver with no timeout of its own. If a flush round arms it but never reaches any of the four resolve sites (e.g. a persistent, unresolvable `shouldPauseQueue` data gap), subsequent READs blocked on `waitForIdle()` hang — the pause watchdog force-unpauses after `MAX_PAUSE_WATCHDOG_TIME_MS` without applied-update progress so the drain can reach a resolve site (see [Pausing & Data-Gap Sync](#pausing--data-gap-sync)).
...
| Generic (everything else) | (fallthrough) | **Yes**: `rollbackOngoingPersistedRequest()` then `throttle.sleep().then(process)` | (none until resolved/given-up) | No | Transient network/server errors |
...
The pause has a **safety net** so the queue can never stay paused forever. A `pause()` needs a later `unpause()`, but nothing guarantees one arrives: the unpause is the last step of a multi-step chain (a server update arrives, `OnyxUpdateManager` fetches and applies the missing updates, then unpauses), and if any step quietly fails the queue stays paused for good — queued WRITEs (including `OpenApp`) never send, `IS_LOADING_APP` never clears, and the app is stuck on a skeleton until a refresh. This is a real bug we have hit.

So `pause()` starts a 60 s timer (`MAX_PAUSE_WATCHDOG_TIME_MS`) that resets whenever the client applies a newer update — a slow-but-healthy catch-up keeps pushing the deadline out, so it only fires when the queue has made **no** progress for a full minute. When it fires, it logs an alert (to keep the root cause visible in telemetry), then **closes the gap before unpausing**: it asks `OnyxUpdateManager` to fetch the missing updates with an incremental `ReconnectApp` that runs outside the queue, so the drained writes are not sent against stale data. That recovery is best-effort — it is skipped if the client is already retrying fetches or has no `lastUpdateID` to reconnect from, capped at `MAX_PAUSE_WATCHDOG_ESCALATION_TIME_MS`, and its errors are ignored — because unblocking the queue always wins over closing the gap. `unpause()` and `resetQueue()` cancel the timer.

Two bounds keep that timer honest:

- **It does not run while offline.** `isQueuePaused` is orthogonal to connectivity, so a gap pause followed by going offline would otherwise trip the watchdog after 60 s of a stall the client had no way to resolve — a false `Log.alert` in the exact telemetry signal above, a reconnect that cannot succeed, and a force-unpause that drains WRITEs against a client known to be behind. So `armPauseWatchdog()` no-ops while `isOfflineNetwork()`, the online→offline transition clears a running timer, and the offline→online transition restarts both the window and `pauseStartTime`. (Backgrounded native apps are a related but unhandled case: suspended JS timers mean the window can elapse while the app is asleep and fire on foreground.)
- **Re-arming cannot extend the pause indefinitely.** An advance of `ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT` only means *some* update applied, not that *this* gap is closing — commands in `requestsToIgnoreLastUpdateID` route straight to `OnyxUpdates.apply` and advance that key **with the gap still open**. A leader whose unpause is orphaned but which keeps receiving such responses would renew its own deadline forever. `MAX_PAUSE_WATCHDOG_ABSOLUTE_TIME_MS` (2 min), measured from `pauseStartTime`, is the ceiling re-arming cannot push past, so total pause time is genuinely bounded rather than merely "no signal for 60 s". The re-arm subscriber also ignores the key being *cleared*: coercing that to 0 would drop the watermark so the next value — even one below where the client already was — would read as an advance and buy another full window (`Onyx.clear()` via Troubleshoot does exactly this).
...
**Sharp edges.**
- The watchdog always unblocks the queue, but it cannot always close the gap first. When its gap-closing reconnect is skipped (the client is already retrying fetches, or has no `lastUpdateID` yet — at 0 the reconnect would omit `updateIDFrom` and pull the **full** app payload outside the queue, so `handleMissingOnyxUpdates`'s own first-updates flow owns that state instead), fails, or runs past its cap, the watchdog unpauses with the gap still open. The drained WRITEs then run against a client that is behind, and the next gapped response simply pauses again — so the worst case is the queue briefly pausing and unpausing on a ~60 s cycle rather than hanging forever. The `Log.alert` it emits on every fire is how these residual cases show up in telemetry.
...
2. **In `push` (authoritative)** against the live persisted requests. The resolver function is deleted off the request (to keep it serializable), and the `conflictAction` is applied via `handleConflictActions`, which performs the queue mutations. The four action shapes:
   ...
   - **replace**: overwrite a queued request. A top-level replace picks it by array position. A `nextAction` replace picks it by `requestIndex` (see **Index fragility** below) and is refused with a `Log.alert` when it carries none.
   - **delete**: remove requests at computed indices, optionally push a new request and/or hand off to a `nextAction` replace (`pushNewRequest` is a required boolean on the type; the optionality is behavioral).
   - **noAction**: ignore the new request.

An incoming `ReconnectApp` (wired via `writeWithNoDuplicatesReconnectConflictAction`) uses a dedicated resolver, `resolveReconnectDuplicationConflictAction`, that is the **one exception** to the queue-only rule below: it also reads the in-flight `ongoingRequest` (via `getOngoingRequest()`, the same way at both evaluation passes) and decides by **coverage** rather than by command name. Coverage is how far back a reconnect re-fetches (`updateIDFrom`, with a full `ReconnectApp` = everything). An incoming reconnect already covered by one in flight or queued is dropped (`noAction`); a wider one is pushed to run after. A live `OpenApp` counts as covering an incoming `ReconnectApp`, since `OpenApp` re-fetches everything, so a reconnect that lands while an `OpenApp` is live is dropped. This is the durable convergence point for reconnect deduping. Preserve and extend it in any refactor; do not delete it.

An incoming `OpenApp` (wired via `writeWithNoDuplicatesOpenAppConflictAction`) has its own resolver, `resolveOpenAppDuplicationConflictAction`, which also reads `ongoingRequest`. An `OpenApp` that lands while another one is in flight is dropped (`noAction`): it would download the whole account a second time, and the app-load skeleton stays up for both round trips. Two cases still fall through to the queue-only `replace` where the latest one wins. The first is an `OpenApp` the in-flight response cannot satisfy, because the params do not describe the whole response. `openApp()` excludes those two ways: it derives the exclusion when the caller asks for public-room or draft-comment preservation, since that adds `successData` merges the in-flight request knows nothing about, and it takes `shouldDedupeWithInFlight: false` for what the params cannot show, namely that the server picks the report set from the priority-mode NVP as it reads it, so the `PriorityModeHandler` refetch after leaving #focus needs its own round trip. The second is an `OpenApp` that lands while only a `ReconnectApp` is in flight, since a reconnect does not carry `OpenApp`'s payload.

**Sharp edges.**
- **Index fragility.** Every `replace`/`delete` index is computed by the resolver against the queue as it looks at `push` time, while `handleConflictActions` applies actions **asynchronously** (awaiting between positional operations). Meanwhile `processNextRequest` (`slice(1)`), `endRequestAndRemoveFromQueue` (splice), and cross-tab merges can shift positions, so an index measured against one snapshot can point at a different request by the time it is applied — and `update()` `splice`s, so the wrong target is overwritten, not merely skipped. What holds today:
  - The top-level `replace` and the `delete` are the **first statements of their branches**, so they apply synchronously, before any `await`, and no shift can land between measuring and applying. They remain positional.
  - A `nextAction` is applied **after** `deleteRequestsByIndices()` has been awaited — a window as long as a disk commit. It is addressed by `requestIndex`: `PersistedRequests.update()` looks that identity up in the live queue at apply time. When the target is no longer queued (it was promoted to `ongoingRequest`), the replace is **skipped, never appended** — an append would queue a second copy of a request already in flight. Skipping is lossless only because of the in-place mutation noted below; the two are one mechanism. A third load-bearing pair: the refusal in `handleFollowUpConflictAction` and the own-write guard in the `PERSISTED_ONGOING_REQUESTS` callback are one contract — without an identity the refusal fires first, so the skip is never reached, and that guard (the promoted identity is pre-registered in `knownOngoingRequestIDs`) is what stops the callback overwriting `ongoingRequest` with the serialized echo, keeping the object the resolver mutated the one that goes out. A skip whose identity sits in neither the queue nor the ongoing request lost the edit and raises a `Log.alert`; the promoted one stays `Log.info`.
  - A `delete` carries no identity, so a `nextAction` `delete` would still apply stale indices after that same await. `ConflictRequestDelete.nextAction` is typed to a replace and `handleConflictActions` refuses anything else with a `Log.alert`. To add a positional follow-up later, give it an identity first.
  - The identity it keys on is per-tab and the doc's own `requestID` collision caveat under **Multi-Tab & Leader Election** applies to it: tabs seeded in the same millisecond emit the same sequence, and the cross-tab merge appends both into one array. Because a collision now overwrites a request rather than only confusing a dedupe, `update()` refuses instead of guessing: when more than one live request carries the identity it skips the update and raises a `Log.alert`. That is the chosen bound — unique within the live queue, not globally — since making it globally unique would mean reseeding the counter in `makeRequest.ts`.
- **The generic conflict check sees only the queue, never the in-flight `ongoingRequest`.** A new write cannot dedupe against a request that is already being sent. The two exceptions are the resolvers above, which consult the ongoing request for an incoming `ReconnectApp` and an incoming `OpenApp`; every other command still dedupes against the waiting queue alone.
- The resolver is expected to be pure, but production resolvers are not: `resolveCommentDeletionConflicts` performs an `Onyx.update`, and because `prepareRequest` invokes the **same** closure, the side effect fires on **both** evaluations (twice per write; an idempotent merge today). `resolveEditCommentWithNewAddCommentRequest` goes further and mutates the queued request's `data` in place during evaluation — **and that mutation is now load-bearing, not just a smell.** When its `nextAction` replace finds the target already promoted, the replace is skipped and the object in flight is the very one the resolver mutated, so the user's edited text reaches the server *only* through that in-place write. Making the resolver pure here (returning a copy) silently loses edits on exactly that path, and the suite sees it: `SequentialQueue - conflict replace addressing › should keep the queued write when the resolver cannot identify the replace target` fails with `Expected "v3"` / `Received "v1"`, and `› should not replace any request once the queue has promoted the conflict target` fails on the body the queue hands to `fetch`, which carries `reportComment: "v1"`. This mutation and that skip must not be changed independently.
...
| `tests/unit/SequentialQueueTest.ts` | Push/persist counts, conflict resolution (replace/push/noAction, incl. replace-while-ongoing, `async`/awaited to match `push`'s async signature), move-to-ongoing persistence, startup hydration of the ongoing request, `queueFlushedData` lifecycle, `ALREADY_CREATED` treated as success without retry, the persist-before-fire edges (`waitForIdle` resolves without flushing when the network goes offline during persist; a disk-write failure during persist does not strand `isReadyPromise` or block the queue), **and the reconnect coverage collapse wired through the queue: an identical reconnect mid-flight is dropped, a wider one runs after, an incoming incremental does not clobber a waiting full reconnect, the spinner (`IS_LOADING_REPORT_DATA`) still clears after a drop, and an incoming `OpenApp` still runs during an in-flight reconnect, while one that lands during an in-flight `OpenApp` is dropped**, plus the pause-watchdog contract: force-unpause after a stuck window, re-arm on applied-update progress, unpause only after the escalation settles, unpause anyway past the escalation cap or on escalation rejection, and a normal unpause disarms it, **plus conflict replace addressing**: a `nextAction` replace finds its target by `requestIndex` after the queue renumbers under it, a follow-up that cannot be addressed by `requestIndex` (a `replace` without one, or a `delete`) is refused with a `Log.alert` instead of applying its stale position, and a target already promoted to `ongoingRequest` is skipped rather than appended with the edited text still reaching the wire |
| `tests/unit/PersistedRequests.ts` | Save/remove/processNext/update/updateOngoing, File/Blob handling, cross-tab follower reconciliation; the `Issue 3a`/`3c` cases assert the **durable** ongoing-request behavior (`processNextRequest` always persists via `multiSet`); a legacy test title references the vestigial `persistWhenOngoing` flag; **identity-addressed `update`** replaces the request carrying the `requestIndex` rather than the one at the caller's stale index, skips (never appends) a target that is no longer queued or a positional index out of range, and refuses with a `Log.alert` when two queued requests carry the same identity |
| `tests/unit/RequestConflictUtilsTest.ts` | Pure push/replace/delete/noAction resolver logic, including the reconnect coverage table (`resolveReconnectDuplicationConflictAction`): full/incremental drop-vs-keep against an in-flight or queued reconnect, with a live `OpenApp` counting as covering an incoming reconnect, plus `resolveOpenAppDuplicationConflictAction`: drop an incoming `OpenApp` behind an in-flight one, keep it when the caller opted out of the dedupe or when only a reconnect is live, replace a queued one, and the edit-comment resolver stamping `requestIndex` onto the target it returns (plus touching nothing when called with an unresolved add comment index, which pins a defensive branch no production caller can reach) |
...
The suites rely on `resetQueue()` (resets the four core coordinator vars: running flag, `currentRequestPromise`, pause flag, and `isReadyPromise`, but not `shouldFailAllRequests`, `queueFlushedDataToStore`, or the throttle) and `resetPendingWritesForTest()` (resets only the `pendingOnyxWrites` counter). These are test-only reset primitives, the companions to `clear()`.
...
| `MAX_PAUSE_WATCHDOG_TIME_MS` | `60000` (60 s) | Max time the queue may stay paused with no applied-update progress before the pause watchdog recovers |
| `MAX_PAUSE_WATCHDOG_ESCALATION_TIME_MS` | `15000` (15 s) | Cap on the watchdog's gap-closing escalation (incremental `ReconnectApp` outside the queue) before unpausing anyway |
...
A code-location index from concept to file. The behavior lives in the sections above; this is just where to find it.
