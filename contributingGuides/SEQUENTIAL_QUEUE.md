...
| `pauseWatchdogTimeoutID` | Liveness watchdog for the pause flag | `armPauseWatchdog()`: from `pause()`, re-armed on every `ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT` advance while paused | `clearPauseWatchdog()` in `unpause()` / `resetQueue()`, or by firing | A paused queue either applies newer updates or is force-unpaused after `MAX_PAUSE_WATCHDOG_TIME_MS` (see [Pausing & Data-Gap Sync](#pausing--data-gap-sync)) |
...
The pause itself carries a **liveness watchdog**. `pause()` and `unpause()` are not balanced by construction: the unpause depends on a decoupled async chain (the `ONYX_UPDATES_FROM_SERVER` subscription firing, `handleMissingOnyxUpdates` not early-returning, an in-flight fetch promise settling), and a path that dies mid-chain orphans the pause — queued WRITEs (including `OpenApp`) never process, `IS_LOADING_APP` never clears, and READs park on `waitForIdle()` until a hard refresh. So `pause()` arms a watchdog timer (`MAX_PAUSE_WATCHDOG_TIME_MS`, 60 s). While paused, every advance of `ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT` re-arms it, so a slow-but-progressing catch-up never trips it; the timer elapses only when the paused queue has applied **no** newer update for the full window. When it fires it `Log.alert`s (so residual orphaning causes stay visible in telemetry) and recovers in two steps: **close the gap first, then unpause**. The gap-closing escalation — registered by `OnyxUpdateManager` via `registerPauseWatchdogEscalation` (`SequentialQueue` cannot import it, dependency cycle) — fires an incremental `ReconnectApp` **outside the queue** (`reconnectAppWithSideEffects`), whose response applies immediately and moves the client to the server head, so the drained writes don't run against stale data. The escalation shares the stalled-fetch back-off (a client already thrashing on `GetMissingOnyxMessages` is not handed another reconnect; the watchdog then just unpauses) and is capped at `MAX_PAUSE_WATCHDOG_ESCALATION_TIME_MS`, with failures swallowed — liveness wins over ordering when the recovery itself misbehaves. After the escalation settles (or the cap elapses), `unpause()` runs and `flush()` drains the queue. `unpause()` and `resetQueue()` clear the timer.
...
**Sharp edges.**
- The watchdog guarantees **liveness always, ordering best-effort**: when its escalation is skipped (stalled-fetch back-off), fails, or exceeds its cap, it unpauses with the gap still open. The drained WRITEs then run against a client that is behind, and the next gapped response re-pauses — worst case a pause/unpause oscillation at the watchdog period instead of a permanent hang. The `Log.alert` it emits is the telemetry signal for residual orphaning causes.
