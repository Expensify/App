...
| Conflict REPLACE | `update()` | `PERSISTED_REQUESTS`, or **nothing** on a skipped replace | **Yes**, `handleConflictActions` awaits it |
...
**A failed apply holds the watermark, and this same sync is what releases it.** An Onyx write can fail on its own — a full disk, a lost IndexedDB connection — and the update is then lost while the server has already counted it. `apply` therefore advances `ONYX_UPDATES_LAST_UPDATE_ID_APPLIED_TO_CLIENT` only *after* the apply resolves, and records the highest `lastUpdateID` whose apply rejected while sitting above the persisted watermark. While it is outstanding, `getEffectiveLastUpdateID()` reports the persisted watermark, so `doesClientNeedToBeUpdated` keeps reporting the range and the fetch above keeps running: the hold is what turns a dropped write into a recoverable gap instead of a silent one.

Only one number is remembered: the **highest** update ID whose apply failed. While it is set the watermark moves only onto a response that proves it re-delivered every update **from the persisted watermark up** — a full reconnect or `OpenApp`, a catch-up whose `updateIDFrom` is at or below the watermark (`updateIDFrom` is **exclusive**, and a fetch fired from above the watermark proves nothing about the range between), or a response chained onto a non-zero watermark by its `previousUpdateID`. A response that proves nothing (`OpenReport`, no `previousUpdateID`) still has its own `onyxData` applied but cannot move the watermark. Because every advance while the hold is set is contiguous with what the client already has, the watermark can never step over a lower failure, so no lower failed ID needs remembering, and the hold is released as soon as one of those responses reaches the highest one. `Onyx.clear()` clears it along with the watermark.

That hold has two consequences for the machinery above. A refused advance is indistinguishable from a stalled fetch, so the stall guard's stated premise — "the server can't serve that range right now" — is only half the story: the client may be refusing an advance it was just given, and the *incremental* `ReconnectApp` the guard escalates to is one of the responses that releases the hold. And because the pause watchdog re-arms on an advance of that key, a hold stops re-arming it, so the watchdog fires **sooner** — the safety direction, never a longer pause.
...
2. **In `push` (authoritative)** against the live persisted requests. The resolver function is deleted off the request (to keep it serializable), and the `conflictAction` is applied via `handleConflictActions`, which performs the queue mutations. The four action shapes:
   ...
   - **replace**: overwrite a queued request, by array position at the top level and by `requestIndex` in a `nextAction` (see **Index fragility** below).
...
- **`nextAction` replace addressing.** A `nextAction` is applied by `handleFollowUpConflictAction`, which takes only a `replace` addressed by `requestIndex` and refuses anything else with a `Log.alert`.
- **Skipped replaces are never appended.** `update()` resolves `requestIndex` against the live queue at apply time, and a `replace` whose target is no longer queued is skipped, never appended.
- **The in-place mutation is part of the contract.** The skip is lossless only because `resolveEditCommentWithNewAddCommentRequest` mutates the queued request in place, so the mutation and the skip cannot be changed independently.
- **Ambiguous identity.** When more than one live request carries the same `requestIndex`, `update()` skips the update and raises a `Log.alert`.
