...
The fetch also carries a **stall guard**. A successful `GetMissingOnyxMessages` response must advance `lastUpdateIDAppliedToClient` past the ID the fetch was fired from. When a 200 fails to do that, the server has shown it cannot serve the range, so repeating the same fetch cannot catch the client up (production traces showed this loop re-firing once per incoming Pusher update). On the first such response `OnyxUpdateManager` fires a single *incremental* `ReconnectApp` instead. That request goes through the queue normally, so a duplicate is dropped when an identical one is already waiting or in flight, and its response always applies (the command ignores update-ID gaps) and moves the client to the server head. Until the client advances, further fetches from the same client state are skipped, and every skipped cycle still resumes the queue. A stalled fetch therefore cannot hold the queue paused and cannot loop. A failed or unanswered fetch proves nothing about the server's ability to serve the range and is left to the existing retry paths. Both fetch sites (the pending-updates branch and the gap branch) share this guard.
