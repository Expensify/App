...
The reconnect family (`OpenApp` / `ReconnectApp`, wired via `writeWithNoDuplicatesReconnectConflictAction`) uses a dedicated resolver, `resolveReconnectDuplicationConflictAction`, that is the **one exception** to the queue-only rule below: it also reads the in-flight `ongoingRequest` (via `getOngoingRequest()`, the same way at both evaluation passes) and decides by **coverage** — how far back a reconnect refetches (`updateIDFrom`, with `OpenApp`/full `ReconnectApp` = everything) — rather than by command name. An incoming reconnect already covered by one in flight or queued is dropped (`noAction`); a wider one is pushed to run after. An incoming `OpenApp` is only ever covered by another `OpenApp` (it alone owns `IS_LOADING_APP`/`HAS_LOADED_APP`), so a plain reconnect never drops it. This is the durable convergence point for the reconnect family — preserve and extend it in any refactor, do not delete it.
