...
The reconnect family (`OpenApp` / `ReconnectApp`, wired via `writeWithNoDuplicatesReconnectConflictAction`) uses a dedicated resolver, `resolveReconnectDuplicationConflictAction`, that is the **one exception** to the queue-only rule below: it also reads the in-flight `ongoingRequest` (via `getOngoingRequest()`, the same way at both evaluation passes) and decides by **coverage** rather than by command name. Coverage is how far back a reconnect re-fetches (`updateIDFrom`, with `OpenApp`/full `ReconnectApp` = everything). An incoming reconnect already covered by one in flight or queued is dropped (`noAction`); a wider one is pushed to run after. An incoming `OpenApp` is **never** dropped: its `successData` carries caller-specific preservation writes that coverage can't see, so it always runs. A live `OpenApp` does still cover an incoming `ReconnectApp`, since it re-fetches everything. This is the durable convergence point for the reconnect family. Preserve and extend it in any refactor; do not delete it.
...
| `tests/unit/RequestConflictUtilsTest.ts` | Pure push/replace/delete/noAction resolver logic, including the reconnect coverage table (`resolveReconnectDuplicationConflictAction`): full/incremental/`OpenApp` drop-vs-keep against an in-flight or queued reconnect |
