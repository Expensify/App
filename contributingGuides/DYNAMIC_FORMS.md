...
Storybook: `Components/DynamicForm` is built from one table of presets. The Flow story mounts the real `DynamicFormFlow` on a story-owned route stack (`.storybook/mocks/storyRouter.tsx`), so pages, resume, the item editor page and the confirmation are all clickable, with preset, `layout` and confirmation controls. The Playground story loads a preset into editable `fields` and `draftValues` controls, so a schema can be tried without writing a fixture; a field with an unknown type is reported in place of the form. Every other entry is one preset opened in the flow, apart from All Field Types and Hidden File Field, which draw every group on one screen for screenshots. Modal pickers open there because Storybook aliases `@react-navigation/stack` to a copy whose card animation reports a presented card (`.storybook/mocks`).
