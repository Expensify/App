...
React Compiler is enabled via `oxc-transform-react` (the Rust React Compiler) in the web build (Rsbuild), the native build (Re.Pack) and the Jest transform. The CI check and the ESLint processor both run it through `config/reactCompiler/checkWithOxc.mjs`, so they agree with what ships.

`babel-plugin-react-compiler` still runs in the Metro lane of `babel.config.js`, which HybridApp release builds bundle through. It is removed once hybrid bundling moves to Re.Pack.

## React Compiler CI check

A CI check runs on every PR that modifies `.ts` or `.tsx` files. For each changed file it runs `oxc-transform-react` and reports whether the file compiles and whether it is actually memoized.

### What the CI check enforces

The check enforces two rules:

1. **New files must compile.** If you add a new file containing React components or hooks, they must all compile. This prevents new Rules of React violations from entering the codebase.
2. **No regressions.** If a file compiles on `main` and your PR breaks that, the check fails. This prevents introducing violations into files that were previously compliant.
...
The CI output shows which files failed and the reason. Look for lines like:

```
FAILED   src/components/MyComponent.tsx (new file must compile)
    42:8: Hooks must always be called in a consistent order...
```
