...
4. Proposals must only be posted after the `Help Wanted` label is added. Any proposals submitted beforehand will be ignored and not reviewed. Do not post proposals in Slack. MelvinBot is an exception: it may post a proposal before `Help Wanted` is applied.
    - On issues where [MelvinBot](./HOW_TO_WORK_WITH_MELVINBOT.md) has posted a proposal, review Melvin's proposal using the same standards as contributor proposals. Melvin's proposal is not guaranteed to be the earliest if `Help Wanted` was applied before Melvin posted (for example when an employee adds the `External` label). Follow first-come-first-serve review order for all proposals after `Help Wanted`. **Do not open a PR** until a proposal is accepted. C+ members: see [How to Work with MelvinBot](./HOW_TO_WORK_WITH_MELVINBOT.md) for your implementation and PR ownership steps.
