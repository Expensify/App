...
A bottom-tab screen opts in the same way, with `nonTopScreenBehavior: 'activity'` on its `Tab.Screen` options (`HOME_TAB_SCREEN_OPTIONS` for the Home tab). `bottomTabScreenLayoutWrapper` then wraps it with `isScreenBlurred={false}`, because no screen of its own navigator ever sits on top of a tab: it counts as covered while another tab is focused and while a root-level modal or RHP covers the whole tab navigator, both of which the wrapper reads from `useIsFocused`. `'freeze'` is not offered to a tab for the same reason. The Home tab (`SCREENS.HOME`) is the first screen that opted in; the other tabs host split navigators whose persistent sidebars must stay live, so they stay unwrapped. On native, tab blur is still handled by `freezeOnBlur`, which suspends the tab before the wrapper could react, so Activity applies there only under a modal or RHP.
