...
To benchmark the post-process pipeline without re-running ESLint, capture a raw dump once (`npm run lint -- --dump-raw /tmp/lint-raw.json`) and replay it (`npm run lint -- --from-raw /tmp/lint-raw.json --timings`).
