...
Native back-swipe pops the RHP at the native layer before any JS cleanup can run, which can leave the pre-inserted destination as the topmost route for a frame - flashing it. This is handled entirely by `RightModalNavigator.tsx`, not by `usePreMountDestination` or its callers - there's no option to configure. `RightModalNavigator.tsx` already listens for `DISABLE_RHP_ANIMATION`/`RESTORE_RHP_ANIMATION` (emitted by `preInsertFullscreenUnderRHP` and `clearFullscreenPreInsertedFlag`/`removePreInsertedFullscreenIfNeeded`) to toggle the slide animation; it sets `gestureEnabled: false`/`true` in the same two listeners.

Because it's tied to the same events that flip the global pre-insert flag - not to any hook instance's mount lifecycle - the gesture stays disabled for exactly as long as `Navigation.getIsFullscreenPreInsertedUnderRHP()` would report true, including through a `shouldPreservePreInsertedRouteOnUnmount: () => true` unmount (the flag, and the disable, survive until whoever finishes the dismiss actually clears it). Header/hardware back is unaffected either way, since `reveal()`/`cleanupPreMount()` already run before `Navigation.goBack()`/`Navigation.dismissModal()`.
