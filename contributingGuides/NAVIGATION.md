...
- [Guides](#guides)
  - [Moving between screens](#moving-between-screens)
    ...
    - [Pre-mounting a destination behind an RHP](#pre-mounting-a-destination-behind-an-rhp)
      - [Native swipe-back is disabled automatically while a pre-insert is live](#native-swipe-back-is-disabled-automatically-while-a-pre-insert-is-live)
...
#### Native swipe-back is disabled automatically while a pre-insert is live

Native back-swipe pops the RHP at the native layer before any JS cleanup can run, which can leave the pre-inserted destination as the topmost route for a frame - flashing it. This is handled entirely by `RightModalNavigator.tsx`, not by `usePreMountDestination` or its callers - there's no option to configure. `RightModalNavigator.tsx` listens for `DISABLE_RHP_ANIMATION` from `preInsertFullscreenUnderRHP` and `RESTORE_RHP_ANIMATION` from `removePreInsertedFullscreenIfNeeded` to toggle both the slide animation and `gestureEnabled`.

On cancel, `removePreInsertedFullscreenIfNeeded` clears the global flag and restores both options before removing the route. On reveal, `clearFullscreenPreInsertedFlag` clears the flag without emitting the restore event: both options intentionally remain disabled through the RHP dismissal and reset when `RightModalNavigator` unmounts. A preserved pre-insert keeps the flag and disabled options until a later reveal or cancel consumes it. Header/hardware back is unaffected either way, since `reveal()`/`cleanupPreMount()` already run before `Navigation.goBack()`/`Navigation.dismissModal()`.

This only gates swiping the **whole RHP away**. Swiping between steps of a multi-step RHP flow is a separate, unaffected gesture (each step lives in its own inner stack), so multi-step confirmation flows keep normal in-RHP swipe navigation.
...
The hook's interaction with native navigation gestures has also turned out to be genuinely subtle, not incidental complexity: keeping native swipe-back from flashing the pre-inserted destination took three separate rounds of bugs across roughly ten commits - wrong navigator level, then a mount-order race, then a vote-count that was scoped to the wrong lifecycle (hook-mount instead of "does a pre-insert actually exist"), each only caught by independent review, not by testing. None of that is a reason to avoid the hook for a flow that fits - the fix (deriving the disable from the existing global pre-insert flag) is now structurally simple - but it's a reason to be careful if you're ever extending this mechanism rather than just calling `reveal()`/`cleanupPreMount()`.
