...
Three places own the consequences, and no other place should reimplement the comparison:
...
| A split never loses its sidebar as a result of an action | `SplitRouter` |
...
`getPopsToFocusBackToRoute` in `Navigation.ts` removes the cause rather than the symptom, and it does so before the resolution runs. It walks the same path the descent would, one level per step, but follows the route that **matches** rather than the one that happens to be **focused**, and returns the pop that makes each of those routes focused. `goUp` dispatches that list and then resolves once, against a state where everything it addresses is focused, so all of its branches apply unchanged and none of them is duplicated a level down. The list holds pops and nothing else, because going back must not add a screen or a browser history entry.

Being a walk down a finite state tree rather than a retry loop is what keeps it honest: it ends on its own, and it never has to ask whether a dispatch took effect. A level contributes nothing when its route is already focused, and it stops the walk whenever it must not be popped: its route is missing or popping to it would lose visited pages, so `goUp` replaces there and popping below a replace would only discard the history the replace exists to keep; its navigator has not mounted yet, which leaves a stale state with no key to target; or its navigator is not a stack, since only `StackRouter` handles `POP` and switching tabs is the `jumpTo` case `goUp` owns. The levels above still pop, which is what puts that replace or that tab switch in the right navigator.

Everything that can stop `goUp` outright is decided before that list is dispatched, so nothing reports a failure to go up after already having moved the user.
